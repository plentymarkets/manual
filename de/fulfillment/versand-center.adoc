...
In einem offenen Versandauftrag kannst du Versandpakete bearbeiten, hinzufügen oder entfernen. Die Kundendaten des Empfängers bearbeitest du im Menü <<crm/kontakte-verwalten#300, CRM » Kontakte>>. Wenn du im Versandauftrag auf die ID des Kunden klickst, wird der Kundendatensatz geöffnet.
...
. Öffne das Menü *Aufträge » Versand-Center*.
. Wähle die Suchkriterien. Beachte dazu die Informationen in Kapitel <<fulfillment/versand-center#100, Versandauftrag suchen>>.
. Klicke auf die Lupe. +
...
. Klicke im Tab *Versandpakete* auf den Stift, um den Versandauftrag zu bearbeiten.
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<tabelle-versandpaket-bearbeiten>>.
...
. Öffne das Menü *Aufträge » Versand-Center » Tab: Suche*.
. Wähle die Suchkriterien. Beachte dazu die Informationen in Kapitel <<fulfillment/versand-center#100, Versandauftrag suchen>>.
. Klicke auf die Lupe. +
→ Die Versandaufträge, die mit den von dir gewählten Suchkriterien übereinstimmen, werden angezeigt.
. Klappe den Versandauftrag auf.
...
Gehe wie im Folgenden beschrieben vor, um ein Versandpaket zu entfernen.
...
→ Die Versandaufträge, die mit den von dir gewählten Suchkriterien übereinstimmen, werden angezeigt.
. Klappe den Versandauftrag auf.
. Klicke im Tab *Versandpakete* auf *Löschen*. +
...
Im Menü *Aufträge » Versand-Center » Tab: Anmelden* meldest du deine Aufträge bei einem Versanddienstleister an und überträgst diese gleichzeitig an den Dienstleister. Voraussetzung für die Anmeldung ist, dass du zuvor im Tab *Suche* <<fulfillment/versand-center#100, Aufträge gesucht>> hast, die jetzt hier angezeigt werden. Du hast z.B. die Möglichkeit, mit den Filtereinstellungen in der Suche des Versand-Centers nur Aufträge für einen oder für alle Versanddienstleister anzuzeigen. Bei der Anmeldung erhältst du sowohl im Erfolgsfall als auch im Fehlerfall eine Meldung vom Versanddienstleister.
...
Beachte auch das Kapitel <<fulfillment/versand-center#2000, Mögliche Fehlerquellen beim Anmelden des Versandauftrags>> auf dieser Handbuchseite.

[#800]
== Angemeldeten Versandauftrag bearbeiten

Wenn du einen Versandauftrag bei einem Versanddienstleister anmeldest, wird der Versanddienstleister in Klammern hinter dem Versandstatus *angemeldet* eingetragen, z.B. *angemeldet (DHL Intraship/Versenden)*.
...
Im Folgenden ist beschrieben, wie du z.B. beim Dienstleister DHL Intraship/Versenden angemeldete Aufträge anzeigst.

[.instruction]
Angemeldeten Auftrag anzeigen:

. Öffne das Menü *Aufträge » Versand-Center » Tab: Suche*.
. Wähle aus der Dropdown-Liste *Versandstatus* die Option *angemeldet DHL Intraship/Versenden*.
. Klicke auf die Lupe. +
...
. Klappe im Menü *Aufträge » Versand-Center » Tab: Suche* den angemeldeten Auftrag auf. +
→ *_Hinweis:_* Wie du den angemeldeten Auftrag anzeigst, ist im Kapitel <<fulfillment/versand-center#900, Angemeldeten Auftrag anzeigen>> beschrieben.
. Klicke auf den Stift im Tab *Versandpakete*. +
→ Die Paketnummer wird geöffnet.
. Gib eine andere Paketnummer ein.
. *Speichere* (icon:save[role="green"]) die Einstellungen.

Informationen zum Import von Paketnummern findest du im Kapitel <<fulfillment/dokumente-erzeugen#800, Paketnummern importieren>> auf der Handbuchseite <<fulfillment/dokumente-erzeugen#, Dokumente erzeugen>>.
...
Die Funktion *Zurücksetzen* setzt lediglich die Anmeldung zurück. Der Versandauftrag wird nicht beim Dienstleister storniert. Storniere deshalb den Auftrag, wie im Kapitel <<fulfillment/versand-center#1100, Versandauftrag stornieren >> beschrieben.
...
. Wähle die Suchkriterien. Beachte dazu die Informationen in Kapitel <<fulfillment/versand-center#100, Versandauftrag suchen>>.
. Klicke auf die Lupe.
. Öffne den angemeldeten Versandauftrag.
...
Im Menü *Aufträge » Versand-Center » Tab: Polling* erzeugst du für die Software eines Versanddienstleisters eine Polling-Datei, die zuvor gewählte Aufträge enthält. Eine Polling-Datei wird benötigt, wenn keine direkte Schnittstelle zu der Software des Versanddienstleisters besteht und somit die Anmeldung des Versandauftrags nicht über das Tab *Anmelden* erfolgen kann. Die in plentymarkets erzeugte Polling-Datei ist im Regelfall eine CSV-Datei.

[.instruction]
Polling-Datei erzeugen:

. Öffne das Menü *Aufträge » Versand-Center*.
. Wähle die Suchkriterien für den Auftrag. Beachte dazu die Informationen in Kapitel <<fulfillment/versand-center#100, Versandauftrag suchen>>.
...
Im Menü *Aufträge » Versand-Center » Tab: Zusammenfassung* erzeugst du eine Auftragszusammenfassung bzw. Tagesabschlussliste von in deinem plentymarkets System eingerichteten Versanddienstleistern.

[.instruction]
Auftragszusammenfassung erzeugen:

. Öffne das Menü *Aufträge » Versand-Center » Tab: Zusammenfassung*.
...
====
Als Alternative zur Auftragszusammenfassung eines Tages im Menü *Aufträge » Versand-Center » Tab: Zusammenfassung* kannst du einen <<daten/daten-exportieren/elastischer-export#, elastischen Export>> durchführen. Erstelle dazu im Vorfeld ein Exportformat mit dem <<daten/daten-exportieren/FormatDesigner#, FormatDesigner>>.
====

[#1600]
== Retourenetikett abrufen

Im Menü *Aufträge » Versand-Center » Tab: Retoure* meldest du Retouren bei einem Versanddienstleister an. Nach dem Anmelden der Retoure sind die Etiketten als PDF-Dateien verfügbar. Du meldest eine Retoure entweder über den Hauptauftrag oder über die Retoure an.
...
. Klicke auf *Anmelden*, um die Retoure anzumelden. +
...
Die Etiketten kannst du dem Endkunden im Mein-Konto-Bereich des Webshops zum Download zur Verfügung stellen. Beachte dazu auch das Kapitel <<fulfillment/versand-vorbereiten#3200, DHL Retoure Online>> auf der Handbuchseite <<fulfillment/versand-vorbereiten#, Versand vorbereiten>>.

Bei Retouren, die du hier im Tab *Retoure* des *Versand-Centers* anmeldest und dann das Retourenetikett erhältst, wurde das Paket schon ausgeliefert und der Kunde hat die Ware erhalten. Bei <<fulfillment/versand-vorbereiten#3100, DHL Retoure Beileger>> legst du Paketen schon vor der Lieferung an den Kunden Retourenetiketten bei. Diese Beilegeretiketten werden ebenfalls im Tab *Retourenetiketten* des Versandauftrages angezeigt.

[#1900]
== Label erzeugen

Im Menü *Aufträge » Versand-Center » Tab: Label* hast du die Möglichkeit, für deine angemeldeten Aufträge Versandlabels zu erzeugen und diese dann anschließend zu drucken. Die Labels werden in einer PDF-Datei zum Download zur Verfügung gestellt. Pro Versanddienstleister wird eine PDF-Datei erzeugt.

[.instruction]
Labels erzeugen:

. Öffne das Menü *Aufträge » Versand-Center*.
. Suche anhand der Filter die Aufträge, für die du ein Sammel-PDF der Labels generieren möchten.
...
Beachte bei der Fehlersuche auch das Menü *Daten » API-Log » Tab: Versand-Center API-Log*. Die darin verfügbaren Einträge bleiben 7 Tage lang gespeichert.
...
Es ist nicht möglich, Pakete im Auftrag im Tab <<auftraege/auftraege-verwalten#1540, Einstellungen>> hinzuzufügen. Wenn du weitere Pakete zu einen Auftrag hinzufügen oder die Anzahl ändern möchtest, öffne den Auftrag im Versand-Center und bearbeite  diesen im Tab <<fulfillment/versand-center#400, Hinzufügen>>.
