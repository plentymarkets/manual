...
Sobald du das Plugin <<plugins/hinzugefuegte-plugins-installieren#plugin-installieren, installiert>> und das <<plugins/hinzugefuegte-plugins-installieren#plugin-set-verknuepfen, Plugin-Set gespeichert und bereitgestellt>> hast, ist das Plugin im Menü *Einrichtung » Aufträge » Versand* sichtbar.
...
. Aktiviere im Bereich *Plugins* die Berechtigungen anhand von <<#tabelle-benutzerrechte-backend-benutzer>>.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
|Erlaubt dem Backend-Benutzer, die Adressen zu bearbeiten.
