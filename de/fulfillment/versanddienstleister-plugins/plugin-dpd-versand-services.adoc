...
. Klicke auf das Plugin-Set, das das DPD Versand-Services-Plugin enthält.
. Klicke in die Zeile des DPD Versand-Services-Plugins.
...
. Wähle aus der Dropdown-Liste *Ceres* rechts daneben die Option *Script loader: After scripts loaded*.
...
. Öffne das Menü *Container-Verknüpfungen*.
. Klappe den Bereich *Container-Links* auf.
. Wähle aus der Dropdown-Liste *Data Provider* die Option *DPD IdentCheck container*.
. Wähle aus der Dropdown-Liste *Ceres* rechts daneben die Option *Checkout: After shipping method*.
. Wähle aus der Dropdown-Liste *Data Provider* die Option *DPD IdentCheck styles*.
. Wähle aus der Dropdown-Liste *Ceres* rechts daneben die Option *Template: Style*.
. Wähle aus der Dropdown-Liste *Data Provider* die Option *DPD IdentCheck scripts*.
. Wähle aus der Dropdown-Liste *Ceres* die Option *Script loader: After scripts loaded*.
. *Speichere* (icon:save[role="green"]) die Einstellungen. +
