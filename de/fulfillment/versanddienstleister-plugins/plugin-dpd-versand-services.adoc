...
Aktiviere die Option *ParcelShopFinder nutzen* (icon:toggle-on[role="skyBlue"]), wenn du deinen Kund:innen den ParcelShopFinder in deinem plentyShop anbieten möchtest.
...
Wenn du international versendest, aktiviere die Schaltfläche (icon:toggle-on[role="skyBlue"]). 3 weitere Felder erscheinen: Hier gibst du den Inhalt der Sendung ein, wählst eine der internationalen Handelsklauseln (Incoterms) aus der Dropdown-Liste und legst fest, welche Zollpapiere versendet werden.

Wenn du Gefahrgut versendest, aktiviere die Schaltfläche (icon:toggle-on[role="skyBlue"]). 2 weitere Felder erscheinen: Hier gibst du eine Beschreibung des Gefahrguts ein und wählst die Art der Verpackung aus der Dropdown-Liste.
