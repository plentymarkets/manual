...
Um die Schnittstelle für DPD Deutschland in deinem plentymarkets System einrichten und nutzen zu können, musst du dich bei link:https://www.dpd.com/de/de/[DPD^]{nbsp}icon:external-link[] registrieren. Dein Account muss für den Webservice freigeschaltet sein. Um die Zugangsdaten zu erhalten, die du für die Einstellungen in plentymarkets benötigst, wende dich bitte an deine DPD Kontaktperson.
...
Sobald du das <<plugins/hinzugefuegte-plugins-installieren#plugin-installieren, Plugin in einem Plugin-Set installiert>> hast, sind die beiden DPD-Assistenten im Menü *Einrichtung » Assistenten » Plugins* im relevanten Plugin-Set sichtbar und du kannst mit dem Einrichten beginnen.
...
Im Menü *Daten » DPD Tagesabschlussliste* kannst du für ausgewählte oder alle über den Versanddienstleister *DPD Versand-Service* angemeldeten Aufträge eine Tagesabschlussliste erstellen. Mit einem Klick auf *PDF erstellen* (icon:document[set=plenty]) wird die Liste als PDF-Datei auf deinem Computer gespeichert.
