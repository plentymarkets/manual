...
In diesem Schritt gibst du deine Zugangsdaten ein, indem du deinen API-Schlüssel wählst und einen passenden Namen dazu eingibst. Mit einem Klick auf icon:plus-square[role="green"] fügst du weitere Zugangsdaten hinzu.
...
Wenn du ein Produkt wählst, werden in einer Liste darunter die zugehörigen Services geladen, die du dann wählen kannst.
...
Außerdem wählst du in diesem Schritt, ob du den Produktivmodus oder den Testmodus aktivieren möchtest. Wenn du international versendest (icon:toggle-on[role="green"]), musst du zusätzlich noch den Zollerklärungstyp und die Standardzollnummer eingeben.
...
** Versandeinstellung/Versandprofil
** Region
