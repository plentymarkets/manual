...
[IMPORTANT]
.DHL Shipping (Versenden) mit Hilfe zweier Assistenten einfach einrichten
====
Die Schnittstelle zu DHL Shipping (Versenden) wird über ein Plugin integriert. Um das DHL Shipping (Versenden)-Plugin nutzen zu können, lade das Plugin im link:https://marketplace.plentymarkets.com/plugins/integration/dhlshipping_4871[plentyMarketplace^]{nbsp}icon:external-link[] herunter. +
...
Bevor du die beiden DHL Shipping (Versenden)-Assistenten durchläufst, durchlaufe den Assistenten *Versandeinstellungen*. Dieser Assistent ist standardmäßig in deinem plentymarkets System vorhanden. Mit diesem Assistenten nimmst du die folgenden Einstellungen vor:
...
* Du gibst der Versandeinstellung einen Namen. +
-> Dieser Name sollte eindeutig sein, weil du ihn später benötigst, wenn du dein Versanddienstleister-Plugin im Assistenten *DHL Shipping (Versenden): Einstellungsverknüpfung* verknüpfst.
...
In diesem Schritt speicherst du deine Zugangsdaten zum DHL Geschäftskundenportal. Du hast hier die Möglichkeit, mehrere Login-Daten zu hinterlegen. Im zweiten Assistenten *DHL Shipping (Versenden): Einstellungsverknüpfung* kannst du diese Daten dann verknüpfen.
...
====
Die DHL Wunschpaket-Services Wunschtag, Wunschzeit, Wunschort und Wunschnachbar stehen dir in diesem Bereich erst zur Verfügung, wenn du das DHL Wunschpaket-Plugin im link:https://marketplace.plentymarkets.com/plugins/integration/dhlwunschpaket_5435[plentyMarketplace^]{nbsp}icon:external-link[] heruntergeladen und es in einem Plugin-Set gespeichert und bereitgestellt hast. +
...
Stelle sicher, dass deine Pakete nur an den Empfänger persönlich übergeben werden - nachdem die Identität und ggf. das Alter ausweisgestützt und dokumentiert geprüft wurden.

Kunden können, wenn der Service zur Verfügung steht, Namen und Geburtsdatum bzw. Mindestalter angeben, damit das Paket nur der angegebenen Person übergeben wird.

Damit deine Kunden den Service Ident-Check nutzen können, ist die folgende Einstellung wichtig:
...
. Aktiviere in den Bereichen *Berechtigungen* und *Plugins* die Berechtigungen anhand von <<#tabelle-benutzerrechte-backend-benutzer>>.
...
| *Plugins > DHLShipping > Grundeinstellungen*
|Erlaubt dem Backend-Benutzer, die Grundeinstellungen zu bearbeiten.

| *Plugins > DHLShipping > Abrechnungsnummern*
|Erlaubt dem Backend-Benutzer, die Abrechnungsnummern zu bearbeiten.

| *Plugins > DHLShipping > Absenderdaten*
|Erlaubt dem Backend-Benutzer, die Absenderdaten zu bearbeiten.

| *Plugins > DHLShipping > Versandeinstellungen*
|Erlaubt dem Backend-Benutzer, die Versandeinstellungen zu bearbeiten.

| *Plugins > DHLShipping > Wunschpaket*
|Erlaubt dem Backend-Benutzer, die Wunschpaket-Daten zu bearbeiten. +
*_Hinweis:_* Das DHL Wunschpaket-Plugin muss installiert und das Plugin-Set gespeichert und bereitgestellt sein.
////
|====
...
. Klicke auf das Plugin *DHLShipping*. +
→ Der Bereich *Konfiguration > Allgemein* öffnet sich.
