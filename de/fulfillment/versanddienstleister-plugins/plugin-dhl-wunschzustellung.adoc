...
Anschließend musst du die Services im Assistenten *DHL Versenden: Einstellungsverknüpfung* aktivieren. Wie das funktioniert, ist auf der Handbuchseite <<fulfillment/versanddienstleister-plugins/plugin-dhl-shipping-versenden#dhl-assistent-einstellungsverknuepfung, DHL Shipping (Versenden)>> beschrieben.
...
*_Hinweis:_* Zusätzlich zu diesen Einstellungen musst du im Assistenten *DHL Versenden: Einstellungsverknüpfung* die Schaltfläche <<fulfillment/versanddienstleister-plugins/plugin-dhl-shipping-versenden#dhl-assistent-einstellungsverknuepfung, aktivieren>>, mit der du entscheidest, ob du deinen Kund:innen den Finder im Bestellvorgang deines plentyShops anbietest. Damit deine Kund:innen den Finder verwenden können, muss die Schaltfläche aktiviert sein (icon:toggle_on[set=material, role=skyBlue]).
