...
*_Hinweis:_* Zusätzlich zu diesen Einstellungen musst du im Assistenten *DHL Shipping (Versenden): Einstellungsverknüpfung* die Schaltfläche aktivieren, mit der du entscheidest, ob du deinen Kund:innen den Finder im Bestellvorgang deines plentyShops anbietest. Damit deine Kund:innen den Finder verwenden können, muss die Schaltfläche aktiviert sein (icon:toggle-on[set=material, role=skyBlue]).
