...
// Screenshot ergänzen / Menüaufbau kenntlich machen
...
|Fasst die Versandlabels bereits angemeldeter Aufträge im PDF-Format zusammen. Die Anzahl der PDF-Dateien entspricht der Anzahl der Versanddienstleister; das bedeutet: pro Versanddienstleister wird eine PDF-Datei generiert.

| *Retourenlabels gruppieren*
| icon:order_return_slip[set=plenty]
|Fasst die Retourenlabels bereits angemeldeter Retouren im PDF-Format zusammen. Die Anzahl der PDF-Dateien entspricht der Anzahl der Retourendienstleister; das bedeutet: pro Retourendienstleister wird eine PDF-Datei generiert.

// eine Datei mit allen Labels / pro Label eine Datei / jedes Paket ein Label 
...
Gehe wie im Folgenden beschrieben vor, um für mehrere Aufträge Versandlabels über die Gruppenfunktion zu gruppieren.
...
. Suche die Aufträge anhand der Filtereinstellungen wie im Kapitel <<#auftrag-suchen, Auftrag im Versand-Center suchen>> beschrieben. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden in der Übersicht angezeigt.
. Wähle (icon:check-square[role="blue"]) die Aufträge, für die du Versandlabels gruppieren möchtest.
. Klicke in der Symbolleiste ganz oben auf *Versandlabels gruppieren* (icon:order_open[set=plenty]). +
...
+
--
Gehe wie im Folgenden beschrieben vor, um für mehrere Aufträge Retourenlabels über die Gruppenfunktion zu gruppieren.
...
. Suche die Aufträge anhand der Filtereinstellungen wie im Kapitel <<#auftrag-suchen, Auftrag im Versand-Center suchen>> beschrieben. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden in der Übersicht angezeigt.
. Wähle (icon:check-square[role="blue"]) die Aufträge, für die du Retourenlabels gruppieren möchtest.
. Klicke in der Symbolleiste ganz oben auf *Retourenlabels gruppieren* (icon:order_return_slip[set=plenty]). +
→ Die PDF-Datei wird generiert und du kannst sie auf deinem Computer speichern.
...
. Öffne das Menü *Aufträge » Versand-Center 2.0*.
. Suche den Auftrag, für den du eine Polling-Datei erzeugen möchtest, anhand der Filtereinstellungen wie im Kapitel <<#auftrag-suchen, Auftrag im Versand-Center suchen>> beschrieben. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden in der Übersicht angezeigt.
. Öffne den Auftrag.
. Klicke in der Symbolleiste ganz oben auf *Polling* (icon:file_extension_csv[set=plenty]).
. Wähle den Dienstleister aus der Dropdown-Liste. +
...
// erscheint auch erst bei erfolgreicher Anmeldung die Paketnummer? Ja.
// ergänzen, wie man Retourenlabel über den Hauptauftrag und über den Auftragstyp Retoure abruft.
...
Pakete, die du in Länder außerhalb der EU versendest, benötigen eine Zollinhaltserklärung, das sogenannte _CN23-Formular_. Sobald du eine Sendung erfolgreich angemeldet hast, deren Lieferland ein Land außerhalb der EU ist, ist im Bereich *Pakete* automatisch das Exportlabel verfügbar.

[.instruction]
Exportlabel herunterladen:

. Öffne das Menü *Aufträge » Versand-Center 2.0*.
. Suche den Auftrag, von dem du das Exportlabel herunterladen möchtest, anhand der Filtereinstellungen wie im Kapitel <<#auftrag-suchen, Auftrag im Versand-Center suchen>> beschrieben. +
→ Die Aufträge, die den eingestellten Suchkriterien entsprechen, werden in der Übersicht angezeigt.
. Öffne den Auftrag.
. Klicke im Bereich *Pakete* auf *Exportlabel herunterladen* (icon:data_export[set=plenty]). +
→ Die PDF-Datei wird generiert und du kannst sie auf deinem Computer speichern.
