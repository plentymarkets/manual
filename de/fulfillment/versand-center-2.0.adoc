...
// Anzahl der PDFs entspricht der Anzahl der Dienstleister (z.B. 2 PDFs, wenn von DHL und DPD)
...
// Am Ende: Reihenfolge der Kapitel bzw. bullet points nochmal prüfen und ggf. anpassen
...
* Anzahl der Pakete
* Anmeldezeit des Versandlabels mit Datum und Uhrzeit
...
Nachdem du die Retoure angemeldet hast, ist das Retourenlabel als PDF-Datei zum Download verfügbar. Du meldest eine Retoure entweder über den Hauptauftrag oder über die Retoure an.

// ist das im neuen SC noch aktuell, dass man die Retoure über den Hauptauftrag oder über die Retoure anmelden kann?
...
Wenn die Retoure erfolgreich angemeldet wurde, ist das Retourenlabel im Bereich *Retouren* verfügbar. Mit einem Klick auf *Retourenlabel* (icon:order_open[set=plenty]) kannst du das Label auf deinem Computer speichern und es anschließend drucken.

<<#image-return-label>> zeigt als Beispiel eine bei DHL Retoure Online angemeldete Retoure mit der Einstellung, dass pro Paket ein Label geneiriert wird. Wenn mehrere Pakete vorhanden sind, wird jeweils eine PDF-Datei pro Retourenlabel generiert.

// stimmt das so mit Label pro Paket
// erscheint auch erst bei erfolgreicher Anmeldung die Paketnummer?

[[image-return-label]]
.Retourenlabel drucken
image::fulfillment/assets/versand-center-retourenlabel-drucken.png[width=640, height=360]

Weitere Informationen zum Generieren von Retourenlabels mit dem Plugin *DHL Retoure Online* findest du auf der Handbuchseite <<fulfillment/versanddienstleister-plugins/plugin-dhl-retoure-online#, DHL Retoure Online>>. 
...
Wenn du im Menü *CRM » EmailBuilder* in der E-Mail-Vorlage die Einstellung *Retourenlabel* aus der Dropdown-Liste der dynamischen Anhänge wählst, werden die Retourenlabels aller in plentymarkets eingebundenen Versanddienstleister – sei es über ein Plugin oder eine feste Integration – als E-Mail-Anhang versendet. Dazu musst du nur noch die E-Mail-Vorlage mit dieser Einstellung mit einer Ereignisaktion verknüpfen, damit die E-Mail-Vorlage bei Eintritt des Ereignisses automatisch an deine Kund:innen versendet wird.
...
// use case ergänzen. In welchem Fall möchte der Händler einen Auftrag zurücksetzen und in welchem Fall möchte er den Auftrag stornieren? Bei versehentlicher Anmeldung? 
// Die Schaltfläche ist selbstverständlich nur sichtbar, wenn der Auftrag bereits angemeldet wurde.
...
Wenn ein Auftrag noch nicht angemeldet wurde, kannst du die Versandpakete jederzeit bearbeiten, hinzufügen oder löschen. 

Gehe wie im Folgenden beschrieben vor, um ein Versandpaket zu bearbeiten. Die Parameter wie Gewicht und Volumen werden aus den Artikeldaten ermittelt. Prüfe daher die Angaben der Versandpakete und nimm ggf. Korrekturen vor.
...
// ist das der richtige Begriff? Es müsste doch "Packstückart" heißen wie in der Artikel-UI
