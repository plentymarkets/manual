...
| *Kundeneigenschaften*
|Die Koordinaten zur Positionierung der Kundeneigenschaften eingeben. Die <<willkommen/allgemeine-funktionen/eigenschaften#, Eigenschaft>> aus der Dropdown-Liste wählen. Auf den Adressetiketten wird der Text angezeigt, der im Menü *CRM » Kontakte » [Kontakt öffnen] » Ansicht: Eigenschaften* gespeichert wurde.
