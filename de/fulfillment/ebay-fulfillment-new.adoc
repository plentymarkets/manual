...
|Auftrag Ausnahme
|Wenn sich der Auftrag in diesem Status in deinem plentymarkets System befindet, wurde der Auftrag nachträglich abgelehnt. Das bedeutet, zunächst hat Orange Connex den Auftrag akzeptiert und nachträglich dann doch abgelehnt. +
Ein Grund dafür könnte z.B. die falsche Menge im Lager oder ein defekter Artikel sein.
...
. Klicke auf *Hinzufügen*.
. Aktiviere die Optionen *Belgien*, *Bulgarien*, *Deutschland*, *Frankreich*, *Griechenland*, *Irland*, *Italien*, *Kroatien*, *Luxemburg*, *Malta*, *Niederlande*, *Österreich*, *Portugal*, *Rumänien*, *Slowakei*, *Slowenien*, *Spanien*, *Tschechische Republik*, *Ungarn*, *Vereinigtes Königreich und Nordirland*, *Zypern*.
...
. Klicke auf *Hinzufügen*.
. Wähle aus der Dropdown-Liste den Status, um den Outbound-Auftrag zu versenden, den du im Assistenten erstellt hast.

. Setze ein Häkchen bei *Aktiv*, um die Ereignisaktion zu aktivieren.
. *Speichere* (icon:save[role="green"]) die Einstellungen.

[#ea-outbound-auftrag-senden]
=== Ereignisaktion zum automatisierten Senden des Outbound-Auftrags an eBay Fulfillment by Orange Connex einrichten
...
. Wähle aus der Dropdown-Liste *Ereignis* die Option *Auftragsänderung > Statuswechsel*.
. Wähle aus der Dropdown-Liste *Status* den Status, um den Outbound-Auftrag zu senden, den du im Assistenten erstellt hast.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
Wenn eBay Fulfillment die Bestellung abwickeln kann, wird automatisch die Paketnummer im Auftrag hinterlegt. Um anschließend den Warenausgang zu buchen und die Paketnummer an eBay Fulfillment by Orange Connex zu übermitteln, damit deine Kund:innen die Sendungsverfolgung nutzen können, richte eine weitere Ereignisaktion ein.
...
=== Ereignisaktion für eine automatische Stornierungsanfrage an Orange Connex senden

Du hast die Möglichkeit für einen Auftrag eine Stornierungsanfrage zu senden, wenn du ihn vorher bereits zu eBay Fulfillment übermittelt hast und der Outbound-Auftrag akzeptiert wurde. Richte eine Ereignisaktion ein, um diese Anfrage zu übermitteln. 

[IMPORTANT]
.Auftragsnotiz bei nicht erfolgter Stornierung
====
Wenn der Auftrag seitens eBay Fulfillment noch storniert werden kann, dann setzt dein plentymarkets System den Auftrag  in den von dir im Assistenten gewählten Status. +
Wenn eine Stornierung nicht mehr möglich ist, weil das Paket bereits vollständig verpackt wurde, oder sogar schon abgeholt wurde, dann verbleibt der Auftrag im Status der Stornierungsanfrage und (ab hier Costi fragen) wir erstellen eine Auftragsnotiz, dass die Stornierung nicht möglich war.
====
...
. Wähle den Filter *Auftrag > Auftragstyp*.
. Klicke auf *Hinzufügen*.
. Aktiviere die Optionen *Auftrag* und *Lieferauftrag*.
. Klicke auf *Filter hinzufügen* (icon:plus-square[role="green"]).
. Wähle den Filter *Auftrag > Herkunft*.
. Klicke auf *Hinzufügen*.
...
Ab diesem Moment und unter der Voraussetzung, dass alle Einstellungen in deinem plentymarkets System korrekt vorgenommen wurden, ist es möglich, den ersten Auftrag an eBay Fulfillment by Orange Connex zu übertragen.

Wenn du Bestellungen für den selben Artikel erhältst, den du zuvor aus deinem eigenen Lager verkauft hast, überträgt das plentymarkets System keinen Auftrag an eBay Fulfillment by Orange Connex. Die Übertragung findet so lange nicht statt, bis der Bestand von eBay Fulfillment by Orange Connex für das entsprechende Lager aktualisiert wurde.
