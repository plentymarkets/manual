...
Richte die Artikel ein, die du über eBay Fulfillment by Orange Connex versenden möchtest. Durch das Aktivieren des Markts (Auftragsherkunft) *eBay Fulfillment by Orange Connex* im Tab *Verfügbarkeit* der Variante wird die Variante an Orange Connex übertragen. Orange Connex prüft die Variante und überträgt nach positiver Prüfung die SKU automatisch an plentymarkets.
...
====
Beachte, dass es nach positiver Prüfung der Variante bis zu einer Stunde dauern kann, bis die SKU auch am Artikel sichtbar ist.
====
...
Das Bundle besteht aus mehreren, einzelnen Artikeln. Das heißt, du musst in jedem Bestandteil dieses Pakets die Verfügbarkeit wie in Kapitel <<fulfillment/ebay-fulfillment#310, Artikel einrichten und SKU erhalten>> beschrieben setzen. Dadurch verfügt jeder Artikel im Paket über eine eigene SKU. Bei der Bestellübermittlung an eBay Fulfillment by Orange Connex wird jeder Bestandteil des Artikelpakets wie die Bestellung einzelner Artikel behandelt.
...
Sobald du dich einmal erfolgreich registriert hast, ist der Schritt *Authentifizierung* nicht mehr sichtbar.
...
Im Schritt *Auftragsstatus* speicherst du die Status für die Outbound-Aufträge. Du kannst entweder bestehende Auftragsstatus wählen oder neue Auftragsstatus erstellen.

Wenn du einen bestehenden Auftragsstatus wählst, stelle sicher, dass dieser Status in keinem Prozess und in keiner Ereignisaktion verwendet wird. Andernfalls könnte dies zu Komplikationen führen.
...
|Auftrag erfolgreich gesendet
|Wenn der Auftrag in deinem plentymarkets System in diesem Status landet, war die Übertragung an Orange Connex erfolgreich.

|Auftrag abgelehnt
|Wenn der Auftrag in deinem plentymarkets System in diesem Status landet, wurde der Auftrag von Orange Connex abgelehnt. +
Ein Grund dafür könnten fehlende Daten am Artikel sein.

|Auftrag storniert
|Wenn der Auftrag in deinem plentymarkets System in diesem Status landet, hat Orange Connex die Stornierung des Auftrags akzeptiert.

|Auftrag Ausnahme
|Wenn der Auftrag in deinem plentymarkets System in diesem Status landet, wurde der Auftrag nachträglich abgelehnt. Das bedeutet, zunächst hat Orange Connex den Auftrag akzeptiert und später dann doch abgelehnt.
...
Klicke auf die Dropdown-Liste *Versandprofil* und wähle den Eintrag *+ Neues Versandprofil erstellen*, damit plentymarkets automatisiert ein neues Versandprofil und einen Versanddienstleister für dich erstellt.
...
Wenn du den Assistenten *eBay Fulfillment by Orange Connex* erfolgreich durchlaufen hast, ist die Auftragsherkunft *eBay Fulfillment by Orange Connex* ist automatisch im Menü *Einrichtung » Aufträge » Auftragsherkunft* vorhanden. Du musst diese Auftragsherkunft aktivieren.

[TIP]
.Hinweis für Händler, die bereits die eBay Fulfillment-Lösung mit Fiege nutzen
====
Wenn du bereits die eBay Fulfillment-Lösung mit Fiege nutzt, ist es wichtig, dass du die korrekte Auftragsherkunft für *eBay Fulfillment by Orange Connex* aktivierst.
====
