...
====
Die Schnittstelle zu CLC wird über ein Plugin integriert. Um das CLC-Plugin nutzen zu können, lade das Plugin im link:https://marketplace.plentymarkets.com/plugins/integration/dhlshipping_4871[plentyMarketplace^]{nbsp}icon:external-link[] herunter.
====
...
Sobald du das Plugin installiert und in einem Plugin-Set bereitgestellt hast wie oben im Abschnitt *Erste Schritte und Anforderungen* beschrieben, ist das Plugin im Menü *Plugins » Plugin-Set-Übersicht* sichtbar.
...
Beide Prozesse importieren CSV-Dateien von einem FTP-Server. Daher ist die erfolgreiche Verbindung zu diesem FTP-Server zwingend notwendig, damit die Prozesse erfolgreich ausgeführt werden können. Die Zugangsdaten gibst du im Menü *Plugin-Sets* » _Plugin-Set wählen_ » *CLC » Konfiguration » Zugangsdaten* ein.
...
Wenn du in diesem Menü keine Einstellungen vornimmst, übermittelt dein plentymarkets System *keine* zusätzlichen Dokumente an CLC.
...
. Gib die IDs der Auftragsherkünfte für den Rechnungsexport, Lieferscheinexport und Versandlabelexport ein.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
Weitere Informationen zu Eigenschaften findest du auf der Handbuchseite <<willkommen/allgemeine-funktionen/eigenschaften#, Eigenschaften>>.
...
*_Hinweis:_* Du musst die IDs _nicht_ eingeben, wenn du einen einfachen Import ausführen möchtest, der keine ARN und ARN-Werte enthält. 
...
. Wähle das Ereignis *Auftragsänderung > Statuswechsel*.
