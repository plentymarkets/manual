...
Im folgenden Video-Tutorial lernst du anhand des Beispiels *DHL Shipping (Versenden)*, wie du deine Versandeinstellungen einrichtest. *_Hinweis:_* Beachte, dass das Video ab Minute 3:42 nicht mehr aktuell ist, weil die Einrichtung des Plugins nicht mehr über den Menüpunkt DHL Shipping (Versenden) eingerichtet wird, sondern über die Assistenten *DHL Shipping (Versenden): Grundeinstellungen* und *DHL Shipping (Versenden): Einstellungsverknüpfung*. Die Assistenten stehen dir nach erfolgreicher Installation des Plugins im Menü *Einrichtung » Assistenten » Plugins* im relevanten Plugin-Set zur Verfügung.
...
| *Name (xy)*
|Name des Versanddienstleisters eingeben, wie er im Warenkorb und auf der Rechnung angezeigt werden soll. Den tatsächlichen Namen des Versanddienstleisters oder eine allgemeine Bezeichnung eingeben, z.B. Versicherter Versand. Dieser Name ist für Kund:innen in deinem Webshop sichtbar.

| *Name (Backend)*
|Name des Versanddienstleisters eingeben, wie er systemintern angezeigt werden soll. Den tatsächlichen Namen des Versanddienstleisters oder eine alternative Bezeichnung, die den Dienstleister charakterisiert, z.B. Pakete Schweiz, eingeben. Dieser Name ist für Kund:innen im Webshop _nicht_ sichtbar.
...
| *URLs splitten*
|Option aktivieren, wenn die Tracking-URLs gesplittet werden sollen. In diesem Fall wird für jede Tracking-Nummer eine separate URL verwendet. +
...
| *Name*
|Der Name in diesem Feld wird im Webshop angezeigt. Name wählen, der deinen Kund:innen angezeigt werden soll, z.B. _Standard_ oder _Express_. Wenn der Webshop in mehreren Sprachen betrieben wird, die Dropdown-Liste verwenden, um den Namen in jeder benötigten Sprache zu speichern.

| *Name (Backend)*
|Name für das Versandprofil eingeben. Der Name wird nur im System angezeigt und ist für deine Kund:innen im Webshop _nicht_ sichtbar.
...
| *Nachnahme*
|Option aktivieren, damit Kund:innen im Ceres-Checkout im Webshop die Zahlungsart _Nachnahme_ wählen können. +
...
| *Paketstation aktiv*
|Option aktivieren, damit für deine Kund:innen in deinem Ceres-Webshop verbesserte Eingabemöglichkeiten für das Feld *Paketstation* verfügbar sind. +
...
| *Postfiliale aktiv*
|Option aktivieren, damit für deine Kund:innen im Ceres-Webshop verbesserte Eingabemöglichkeiten für das Feld *Postfiliale* verfügbar sind. +
*_Wichtig:_* Voraussetzung ist, dass die Plugins link:https://marketplace.plentymarkets.com/plugins/templates/Ceres_4697[Ceres^]{nbsp}icon:external-link[] und link:https://marketplace.plentymarkets.com/plugins/templates/IO_4696[IO^]{nbsp}icon:external-link[] installiert sind.

| *Einwilligung zur Datenübermittlung*
|Option aktivieren, damit im Ceres-Webshop eine Checkbox angezeigt wird. Wenn deine Kund:innen diese Checkbox aktivieren, stimmen sie zu, dass ggf. die Telefonnummer und E-Mail-Adresse an den Versanddienstleister übertragen werden.
...
| *Inselzuschlag*
|Viele Versanddienstleister verlangen für den Versand in Inselregionen höhere Gebühren. Auf diesen Kosten solltest du als Händler:in nicht sitzenbleiben. Daher pro Versandprofil einen pauschalen Aufschlag für den Versand in Inselregionen festlegen. Die Postleitzahlen der Inselregionen sind systemseitig gespeichert. Der Auftrag wird automatisch abgeglichen. Bei Aktivierung des Inselzuschlags erfolgt eine Berechnung nur, wenn die Postleitzahl im System als Inselregion erkannt wird. +
...
*_Wichtig:_* Prüfen, ob Expressversand vom Versanddienstleister unterstützt wird. Nur dann kann die Option genutzt werden.
...
|Warenwert eingeben, ab dem die Lieferung für deine Kund:innen versandkostenfrei ist. Der Warenwert bezieht sich auf den Bruttowarenwert. +
...
_Flexible Verpackung:_ Für den Versand von Möbeln verwendet du als Händler:in Verpackungsmaterial, das je nach Größe des Versandguts individuell angepasst wird (Wellpappe, Folie etc.). Das Volumen wird aus den Artikeldaten berechnet.
====
...
|Daten eingeben, wie von DHL erhalten. +
*_Hinweis:_* Beachte, dass das Sonderzeichen *"* von der Schnittstelle nicht korrekt verarbeitet werden kann. Wenn *"* in deinen Zugangsdaten enthalten ist, ändere dein Passwort bzw. Benutzernamen beim Versanddienstleister und hier in deinem plentymarkets System.
...
+
--
Die in plentymarkets fest integrierte Schnittstelle wird weiterhin unter *DHL Intraship/Versenden* geführt, bis DHL uns darüber informiert hat, dass alle Kund:innen *DHL Versenden* nutzen. Die Schnittstelle DHL Intraship/Versenden sowie Einstellungen, die diese Schnittstelle in anderen Menüs betreffen, gelten bis zur Abschaltung von DHL Intraship für bestehende DHL Intraship-Kund:innen.
...
|Bei Auslandssendungen wird die Telefonnummer des Empfängers standardmäßig von DHL auf dem Etikett gedruckt, damit DHL diesen bei Bedarf erreichen kann. Wenn das nicht gewünscht ist, hier z.B. eine Folge von Nullen oder "Keine Tel.-Nr." eingeben. Dieser Eintrag überschreibt dann die Telefonnummer von Auslandskund:innen. +
Die Option ist auch wichtig für den Fall, dass im Layout die Telefonnummer für Auslandskund:innen nicht als Pflichtfeld markiert wurde und die Sendung somit von DHL abgelehnt würde.
...
| *Flex/Predict aktiv*
|Aktivieren, um bei DPD den kostenpflichtigen Vorankündigungsservice link:https://www.dpd.com/de/de/empfangen[Predict^]{nbsp}icon:external-link[] (ehemals Flexzustellung) anzumelden. In der CSV-Datei wird dann der Zusatz *PRO* ausgegeben.
|====
...
*_Hinweis:_* Beachte, dass das Sonderzeichen *"* von der Schnittstelle nicht korrekt verarbeitet werden kann. Wenn *"* in deinen Zugansdaten enthalten ist, ändere deinen Login-Namen bzw. Passwort beim Versanddienstleister und hier in deinem plentymarkets System.
...
|Daten eingeben. +
*_Hinweis:_* Beachte, dass das Sonderzeichen *"* von der Schnittstelle nicht korrekt verarbeitet werden kann. Wenn *"* in deinen Zugangsdaten enthalten ist, ändere deinen Benutzernamen bzw. Passwort beim Versanddienstleister und hier in deinem plentymarkets System.
...
→ *_Hinweis:_* Beachte, dass das Sonderzeichen *"* im Benutzernamen von der Schnittstelle nicht korrekt verarbeitet werden kann. Wenn *"* in deinen Zugangsdaten enthalten ist, ändere dein Passwort bzw. Benutzernamen beim Versanddienstleister und hier in deinem plentymarkets System.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
| *JA* wählen. Aufgrund einer Umstellung bei Swiss Post wird die Schnittstelle angepasst. Alle Kund:innen erhalten neue Zugangsdaten. Diese Zugangsdaten hier eingeben. Bei Problemen mit der neuen Konfiguration Swiss Post direkt kontaktieren.
|====
...
Du hast in plentymarkets die Möglichkeit, Filter einzustellen, damit Aufträge mit den Versandbedingungen von Amazon Prime gesondert behandelt werden. Außerdem kannst du Versandlabel für Amazon Prime-Aufträge über die Amazon Merchant Fulfillment API generieren. Welche Einstellungen du vornehmen musst, ist auf der Handbuchseite  <<maerkte/amazon/amazon-einrichten#, Amazon einrichten>> im Kapitel <<maerkte/amazon/amazon-einrichten#4200, Amazon Prime-Aufträge bearbeiten>> beschrieben.
