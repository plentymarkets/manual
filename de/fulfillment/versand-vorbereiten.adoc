...
Im folgenden Video-Tutorial lernst du anhand des Beispiels *DHL Shipping (Versenden)*, wie du deine Versandeinstellungen einrichtest. *_Wichtig:_* Bitte beachte, dass das Video ab Minute 3:42 nicht mehr aktuell ist, da die Einrichtung des Plugins nicht mehr über den Menüpunkt DHL Shipping (Versenden) eingerichtet wird, sondern über die Assistenten *DHL Shipping (Versenden): Grundeinstellungen* und *DHL Shipping (Versenden): Einstellungsverknüpfung*. Die Assistenten stehen dir nach erfolgreicher Installation des Plugins im Menü *Einrichtung » Assistenten* im Bereich *Integration* zur Verfügung.
