...
Im folgenden Video-Tutorial lernst du anhand des Beispiels *DHL Shipping (Versenden)*, wie du deine Versandeinstellungen einrichtest. *_Hinweis:_* Bitte beachte, dass das Video ab Minute 3:42 nicht mehr aktuell ist, weil die Einrichtung des Plugins nicht mehr über den Menüpunkt DHL Shipping (Versenden) eingerichtet wird, sondern über die Assistenten *DHL Shipping (Versenden): Grundeinstellungen* und *DHL Shipping (Versenden): Einstellungsverknüpfung*. Die Assistenten stehen dir nach erfolgreicher Installation des Plugins im Menü *Einrichtung » Assistenten* im Bereich *Integration* zur Verfügung.
...
Weitere Informationen zu den API-Log-Einträgen von DHL Fulfillment und DPD Cloud Webservice (My DPDBusiness) findest du auf der Handbuchseite <<daten/api-log#, API-Log>>.
...
|Für Händler innerhalb Deutschlands. Versand: Deutschland.
...
DHL Retoure Online ermöglicht das Erzeugen von Retourenetiketten für Aufträge aus verschiedenen Lieferländern. Die Etiketten sind dann im Menü *Aufträge » Versand-Center* im Tab *Retoure* verfügbar. Du kannst deinem Kunden das Etikett dann über eine Ereignisaktion automatisch per E-Mail senden.
...
Bevor du die Schnittstelle zu DPD Print in plentymarkets einrichten, registriere dich bei link:https://www.dpd.com/portal_de[DPD^]{nbsp}icon:external-link[]. Mit dem Programm DPD Print erstellst du als DPD-Kunde DPD-Paketscheine.
