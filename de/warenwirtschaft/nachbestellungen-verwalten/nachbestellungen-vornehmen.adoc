...
Mithilfe der Filter Bookmarks (icon:bookmarks[set=material]) kannst du ausgewählte Filter in der UI speichern. Die damit ausgewählten Filter sind dann bei jedem Öffnen des Menüs vorausgewählt, ähnlich wie Lesezeichen. Jede:r Benutzer:in kann eigene Filter festlegen.
...
* Nettowarenwert = Der Nettowarenwert der bestellten Artikel wird angezeigt. Diese Info box ist immer grau.
* Bestellte Artikel = Die Anzahl der bestellten Artikel wird angezeigt. Diese Infobox ist immer grau.
* Offene Artikel = Die Anzahl der noch nicht gelieferten Artikel wird angezeigt. Grau bedeutet, dass keine Artikel offen sind. Orange bedeutet, dass Artikel offen sind.
...
Beim Öffnen des Menüs *Waren » Bestellwesen » Bestellung* werden dir alle Nachbestellungen und <<warenwirtschaft/umbuchungen-vornehmen#, Umbuchungen>> in einer Übersichtstabelle angezeigt. Durch die Suche (icon:filter[]) kannst du die angezeigten Umbuchungen und Nachbestellungen eingrenzen. Setze dafür die benötigten Filter und klicke dann auf *Suchen* (icon:search[role="blue"]). Du kannst mehrere Filter miteinander kombinieren. In <<table-reorder-search>> werden die Filtereinstellungen erläutert. +
