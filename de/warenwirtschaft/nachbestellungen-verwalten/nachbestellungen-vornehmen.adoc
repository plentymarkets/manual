...
. Aktiviere im Bereich *Berechtigungen* die Berechtigungen anhand von <<table-rights-reorders>>.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
. Wähle einen in <<table-filter-item-search-reorder>> beschriebenen Filter zur Artikelsuche.
...
| *Lieferdatum*
|Du kannst das Lieferdatum für die Nachbestellung manuell eintragen oder die Funktion *Lieferdatum ermitteln* nutzen. Das System berechnet dann auf Basis der *Lieferzeit*, die im Tab *Lieferant* der Varianten hinterlegt ist, das voraussichtliche Lieferdatum. Weitere Informationen zum Hinterlegen der Lieferzeit an der Variante findest du auf der Handbuchseite <<artikel/artikel-verwalten#180, Artikel verwalten>>. +
Sobald du in den Auftragsdetails das Lieferdatum setzt, wird dieses Datum für alle Auftragspositionen in die Tabelle übernommen. In der Tabelle kannst du für jede Auftragsposition individuell ein Lieferdatum setzen. _Beachte_, dass auch diese bereits gesetzten Lieferdaten überschrieben werden, sobald du ein Lieferdatum in den Auftragsdetails eingibst. Die Lieferdaten bleiben bearbeitbar, solange die Bestellung nicht abgeschlossen ist.
...
. Öffne das Menü *Waren » Bestellwesen » Bestellung*. +
. Wähle einen oder mehrere der in <<table-reorder-search>> beschriebenen Filter, um die Ergebnisse einzugrenzen.
. Klicke auf *Suchen* (icon:search[role="blue"]).
