...
*_Tipp:_* Tippe und halte (icon:drag_handle[role="darkGrey"]) für jedes aktivierte Suchkriterium, um die Reihenfolge der angezeigten Suchfelder anzupassen.

| *Fokus auf Artikelsuchfeld*
|Suchst du typischerweise anhand eines bestimmten Kriteriums? Tippe auf dein am häufigsten verwendetes Kriterium. Wenn du später nach Artikeln suchst, wird dein Mauszeiger automatisch in diesem Feld beginnen. Natürlich ist es weiterhin möglich, den Mauszeiger manuell in einem anderen Suchfeld zu platzieren.
|====

[#300]
=== Artikel suchen

//TODO: Hinweis zum Zurücksetzen der Suchkriterien
...
. Tippe unten auf *Einstellungen*. +
→ Das Einstellungsmenü öffnet sich.
. Tippe auf *Lagerort inspizieren*.
...
|Suchst du typischerweise anhand eines bestimmten Kriteriums? Tippe auf dein am häufigsten verwendetes Kriterium. Wenn du später nach Lagerorten suchst, wird dein Mauszeiger automatisch in diesem Feld beginnen. Natürlich ist es weiterhin möglich, den Mauszeiger manuell in einem anderen Suchfeld zu platzieren.

| *Anzahl zu ladender Lagerorte*
|Wähle aus, wie viele Suchergebnisse du auf einmal sehen möchtest.

| *Zweck*
|Verwende die Umschaltflächen, um festzulegen, welche Lagerortzwecken beim Suchen von Lagerorten zur Filterung auswählbar sein sollen. +
*Aktiviert* (icon:toggle-on[role="green"]) = Es wird möglich sein, Lagerorte anhand dieses Kriteriums zu filtern.

2+| Bereich *Bestandsdetails*
...
. Tippe auf *Lagerort inspizieren* (//TODO: ADD ICON (custom). +
→ Die Ansicht zur Suche von Lagerorten wird geöffnet.
. Gib ein Suchkriterium ein. Scanne z.B. mit deinem Gerät den Barcode des Lagerort-Labels.
. Tippe auf *Suchen* (icon:search[role="darkGrey"]). +
→ Der gefundene Lagerort wird angezeigt.

//TODO: Wie sieht das aus und was kann man dann noch machen?