...
//TODO:Funktionssymbole und Links zu Features in Tabelle (Abschnitt "Funktionen")
...
| *Allgemeine Informationen* (icon:person[role="darkGrey"])
...
| *Funktionen* (icon:apps[role="darkGrey"])
...
| *Letzte Aktivitäten* (icon:history[role="darkGrey"])
...
| *Anzeige* (icon:aspect_ratio[role="darkGrey"])
| *Schrittanzeige*: Aktiviere die Option, um während der Nutzung der Funktionen von plentyWarehouse am oberen Bildschirmrand eine Übersicht eine Fortschrittsanzeige einzublenden. So siehst du immer, welche Schritte du schon ausgeführt hast und welche als nächstes folgen. Ist die Option nicht aktiviert, wird am oberen Bildschirmrand lediglich der Name des Schrittes angezeigt, in dem du dich gerade befindest. +
*Buttons für Linkshänder positionieren*: Wenn diese Option aktiviert ist, werden, wo immer möglich, Buttons und Shortcuts auf der linken Seite des Bildschirms positioniert. So wird die Bedienung für Linkshänder:innen erleichtert.

| *plentyBase* (icon:icon-referrer_backend[role="darkGrey"])
...
| *Sprachausgabe* (icon:speaker[role="darkGrey"])
...
| *Sonstiges* (icon:settings[role="darkGrey"])
...
. Tippe unten auf *Abmelden* (icon:logout[role="darkGrey"]). +
