...
Für Inventuren, die mit plentyWarehouse durchgeführt wurden, können außerdem Zählprotokolle im plentymarkets Backend erzeugt und exportiert werden. Weitere Informationen dazu findest du im Kapitel <<#600, Zählprotokoll exportieren>>.
...
Möchtest du nach der Inventur alle Bestandsabweichungen auf einmal prüfen? Dan nutze zunächst das <<warenwirtschaft/inventur-vornehmen#200, Export-Format *Warenbestände*>>, um deine aktuellen Warenbestände zu exportieren.

Nachdem du die Inventur mit der plentyWarehouse App durchgeführt hast, erstellst du über das <<warenwirtschaft/inventur-vornehmen#400, Import-Tool einen Datenimport>>. Im Anschluss kannst du alle Bestandsabweichungen pro Artikel im *Tab: Bestand » Tab: Warenbewegung* des Artikels einsehen. Der Import wird außerdem im Menü *Waren » Wareneingänge* als Eintrag aufgelistet und die geänderten Warenbestände können im Eintrag nachvollzogen werden.
...
. Klicke auf *PDF generieren* oder *CSV generieren*. Weiter unten findest du eine Einscheidungshilfe zur Wahl des passenden Dateiformats. +
