...
Bevor du eine Inventur mit plentyWarehouse durchführst, muss sichergestellt sein, dass alle Benutzer:innen über die entsprechenden Berechtigungen verfügen. Nur so können die gezählten Bestände erfasst und sowohl in plentyWarehouse als auch im plentymarkets Backend korrekt gespeichert werden.

:rights-purpose: Inventuren durchführen kölnnen
...
| *Lagerort*
|Wähle bei Bedarf einen Lagerort innerhalb des oben angegebenen Lagers aus, um ein Zählprotokoll gezielt für diesen Lagerort zu exportieren.
