...
. Tippe auf *Inventur* (icon:call_split[rotate=90]).//TODO: Custom Icon einfügen
...
| //TODO: Was muss man machen, wenn das nicht aktiviert ist? einfach nur nochmal den letzten Artikel piepsen, um die Inventur als Ganzes abzuschließen?

| *Bestätigungsdialog zum Abschließen der Inventur anzeigen*
|Aktiviere diese Option (icon:toggle-on[role="green"]), um vor dem Speichern des gezählten Bestands einen Bestätigungsdialog anzuzeigen. +
...
====
Erfasse immer alle Artikel, die sich an einem Lagerort befinden. Wenn du verschiedene Artikel an demselben Lagerort la…gerst, wird der Bestand aller Artikel, die du bei der Inventur nicht erfasst, auf Null gesetzt. Beispiel: +
