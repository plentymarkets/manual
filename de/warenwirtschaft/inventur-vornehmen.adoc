...
====
Beachten Sie die Hinweise im Handbuch zum Datenformat <<daten/export-import/datenformate/stock#, Stock>> – insbesondere bevor Sie durch einen Import Bestände überschreiben. Anhand eines Testimports mit nur einer Variante können Sie prüfen, ob der Import das gewünschte Ergebnis liefert.
====
...
. Öffnen Sie das Menü <<daten/export-import/daten-exportieren#, Daten » Dynamischer Export>>.
. Öffnen Sie das Untermenü des Datenformats <<daten/export-import/datenformate/stock#, Stock>>, indem Sie auf das Plus klicken. Falls noch kein Untermenü existiert, klicken Sie auf *Neues Datenformat*, um dieses <<daten/export-import/datenformate#2-2, Datenformat>> anzulegen.
...
. Öffnen Sie das Menü **System » Systemeinstellungen » Waren » Lager » Lager wählen » Untermenü: Einstellungen**.
...
In plentymarkets 7 kann eine Inventur der Ware auch mit der plentymarkets App durchgeführt werden. Voraussetzung ist, dass die Person die Berechtigung für die Inventur hat. Außerdem müssen den <<warenwirtschaft/lager-einrichten, Lagerorten Barcodes zugeordnet>> sein. Optional können Sie den Inventurmodus aktivieren, um den Bestand während der Inventur einzufrieren. Das Aktivieren des Inventurmodus ist jedoch nicht erforderlich, da der neue Bestand direkt im Datensatz der Variante gespeichert wird.

[TIP]
.Hardware für plentymarkets App
====
Hardware-Empfehlungen für Inventuren mit der plentymarkets App finden Sie auf der Handbuchseite <<schnelleinstieg/systemvoraussetzungen#400, Systemvoraussetzungen>>.
====
...
Wenn Sie nach der Bestandszählung alle Bestandsabweichungen auf einmal prüfen möchten, exportieren Sie vor Beginn der Inventur den Bestand über das Datenformat <<daten/export-import/datenformate/stock#, Stock>>. Führen Sie einen weiteren Export nach Ende der Inventur durch und vergleichen Sie die Bestandsdaten.

[#1400]
==== App-Berechtigungen für Inventur erteilen

Konten mit Admin-Rechten haben standardmäßig die Berechtigungen, um Inventuren über die App vorzunehmen. Personen mit den Benutzerklassen *Variabel* und *Logistik* müssen Sie diese Berechtigungen erteilen.
...
. *Speichern* Sie die Einstellungen.
. Wechseln Sie in das Tab *Berechtigungen*.
. Klicken Sie auf *Menü und REST-API Rechte verwalten*. +
→ Die Ansicht *Rechte* wird geöffnet.
. Aktivieren Sie in der Spalte *Berechtigungen* die Berechtigung *Warenbestände » Lager » Lagerort » Lagerort anzeigen*.
. *Speichern* Sie die Einstellungen.
