...
In plentymarkets 7 kann eine Inventur der Ware auch mit der plentymarkets App durchgeführt werden. Voraussetzung ist, dass der Benutzer die Berechtigung für die Inventur hat. Außerdem müssen den <<warenwirtschaft/lager-einrichten, Lagerorten Barcodes zugeordnet>> sein.
...
[TIP]
.Datenformat Stock nutzen
====
Wenn Sie nach der Bestandszählung alle Bestandsabweichungen auf einmal prüfen möchten, exportieren Sie vor Beginn und nach Abschluss der Inventur den Bestand über das Datenformat <<basics/datenaustausch/datenformate/stock#, Stock>>.
====
