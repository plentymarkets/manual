...
So kann gezielt nach kleinen, mittleren oder großen Lagerorten gesucht werden und alle Artikel-IDs können damit verknüpft werden. Außerdem kann dir die nähere Charakterisierung deiner Lagerorte dabei helfen, deine Artikel link:https://knowledge.plentymarkets.com/artikel/artikel-verwalten#3000[direkt am passenden Ort zu lagern]{nbsp}icon:external-link[]. +
...
| *Position*
|Positionsnummer für die Eigenschaftsgruppe eingeben. Standardmäßig wird aufsteigend nach Positionsnummer sortiert. Wenn ein Lagerort Eigenschaften aus mehreren Gruppen hat, werden die Gruppen aufsteigend dargestellt. +
