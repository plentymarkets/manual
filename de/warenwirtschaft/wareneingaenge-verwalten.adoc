...
Hier eingetragene Werte gelten für die Marktplätze <<omni-channel/multi-channel/neckermann-at#, Neckermann>>, <<omni-channel/multi-channel/otto#, Otto>> und <<omni-channel/multi-channel/plus-gartenxxl#, Plus.de>>. Die Werte sind gegenüber den hinterlegten Bestandspufferwerten dieser Marktplätze dominant.
...
|[[Formel-fuer-Live-Durchschnitt]]latexmath:[LDEK_neu = {EK_{alt} \cdot Menge_{alt} + EK_{neu} \cdot Menge_{neu}\over Menge_{alt} + Menge_{neu}}]
|latexmath:[GLD_neu = {GLD_{alt} \cdot Menge_{alt} + EK_{neu} \cdot Menge_{neu}\over Menge_{alt} + Menge_{neu}}]

|Bei dieser Methode wird durch jeden Wareneingang und jeden Warenausgang eine Neuberechnung ausgelöst. Der aktuelle Durchschnitt wird z.B. in Statistiken oder Exporten ausgegeben. Damit dieser Durchschnittspreis auch im Artikel angezeigt wird, muss extra eingestellt werden, dass der Durchschnittseinkaufspreis auf Varianten übertragen wird. +
...
|Bei dieser Methode wird durch jeden Wareneingang eine Neuberechnung ausgelöst. Allerdings nur, wenn das Lager, in das Sie Bestand buchen auch für den GLD berücksichtigt wird. Der GLD wird in der Variante automatisch in einem separaten Feld eingetragen. +
