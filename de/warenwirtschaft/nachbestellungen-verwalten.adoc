...
Pro Lager können Sie entscheiden, ob Sie mit den dynamischen Meldebestand nutzen oder nicht. Wenn Sie den dynamischen Meldebestand nutzen, berechnet Ihr System den dynamischen Meldebestand jede Nacht im Hintergrund. Dieser berechnete dynamische Meldebestand wird noch in keinem Feld dargestellt, d.h. Sie sehen den Wert nicht. Die Differenz zum Meldebestand, MeldΔ, basiert jedoch auf dem berechneten Meldebestand. Die Differenz wird Ihnen z.B. in der Bestandsübersicht angezeigt. So können Sie anhand der Differenz absehen wann eine Nachbestellung erfolgen sollte.
...
latexmath:[\mbox{Dynamischer Meldebestand} = {(\mbox{Ø Abverkauf} \cdot Lagerreichweite) + (\mbox{Ø Abverkauf} \cdot \mbox{kürzeste Lieferzeit})}]
...
latexmath:[MeldΔ = {(\mbox{dynamischen_Meldebestand} - (Nettobestand + (\mbox{nachbestellte_Menge})}]
