...
Nutzer vom Typ *Backend* sehen nur bestimmte Bereiche und Menüs im plentymarkets Backend. Somit können sie nur eingeschränkt im System arbeiten. Sollen Mitarbeiter mit diesem Nutzertyp mit Warenbeständen arbeiten können, muss ein Nutzer vom Typ *Admin* die in <<#tabelle-berechtigungen-warenbestaende>> beschriebenen Rechte für diese Benutzer:in vergeben.
...
. Aktiviere im Bereich *Berechtigungen* die Berechtigungen gemäß <<#tabelle-berechtigungen-warenbestaende>>.
. *Speichere* (icon:save[role="green"]) die Einstellungen.

[[tabelle-berechtigungen-warenbestaende]]
.Berechtigungen für Warenbestände vergeben
[cols="1,3"]
|====
|Einstellung |Erläuterung

| *Warenbestände > Anzeigen*
|Erlaubt Backend-Benutzer:innen, Warenbestände zu sehen.

| *Warenbestände > Bearbeiten*
|Erlaubt Backend-Benutzer:innen, Warenbestände zu bearbeiten.
...
In der Zeile einer Variante kann die Lieferant:in noch einmal aus einer Dropdown-Liste gewählt werden. Wenn du mehrere Lieferant:innen für eine Variante hast, sind trotz des Filters alle hier wählbar. Neben dem Lieferant:innen werden dir der Einkaufspreis der Lieferant:in, die Verpackungseinheit, der Rabatt und die Mindest-Abnahmemenge angezeigt.
...
Mit Syncs vom Typ *Artikel*, *Warenbestand* oder *Wareneingang* kann negativer Warenbestand importiert werden. Dies ist beispielsweise hilfreich, wenn du nach einer Inventur eine Differenz zu deinem Warenbestand feststellst oder wenn dir weniger Ware geliefert wurde als du bestellt hast.
Bei den Sync-Typen Artikel und Warenbestand werden die Werte aus der CSV-Datei oder vom Eigenen Wert 1:1 importiert.
Beim Sync-Typ Wareneingang hingegen werden die Werte vom Warenbestand subtrahiert.
...
. Wähle einen Sync. Dieser muss vom Typ Artikel, Warenbestand oder Wareneingang sein (Pflichtfelder sind Lager, Lagerort und Menge). +
icon:map-signs[] *_Oder_*: Klicke auf *Sync hinzufügen*, um einen Sync vom Typ Artikel, Warenbestand oder Wareneingang zu erstellen. Weitere Informationen zum Erstellen eines Syncs findest du auf der Seite <<daten/daten-importieren/ElasticSync#1210, ElasticSync>>.
