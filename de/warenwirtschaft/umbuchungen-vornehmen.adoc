...
. Klicke auf das Hinzufügen-Symbol (icon:plus-square[role="green"]) und wähle *Neue Umbuchung*. +
...
. *_Optional:_* Gib im Feld *Eigner* eine Eigner:in, also die bearbeitende Person, ein.

[TIP]
.Eigner auswählen
====
Fehlt die gewünschte Eigner, liegt das daran, dass kein:e Benutzer deines plentymarkets Systems als Eigner aktiviert wurde. Eine Anleitung zum Aktivieren der Eigner findest du auf der Handbuchseite <<business-entscheidungen/benutzerkonten-zugaenge#40, Benutzer:innen-Konten und Zugänge>>. Prüfe außerdem die oben beschriebenen <<#400, Standardeinstellungen>> zu Eigner.
====

[#700]
=== Auftragspositionen hinzufügen

Im nächsten Schritt fügst du die Auftragspositionen hinzu, die von einem Lager in ein anderes Lager umgebucht werden sollen. Nutze die Suche (icon:filter[]), um die gewünschten Auftragspositionen anhand der vorhandenen Artikel zu finden. Setze dafür die benötigten Filter und klicke dann auf *Suchen* (icon:search[role="blue"]). Du kannst mehrere Filter miteinander kombinieren. In <<table-filters-item-search-redistribution>> werden die Filtereinstellungen erläutert.
...
. Setze die passenden Filter (icon:filter[]) für die Suche und beachte dabei die Erläuterungen in <<table-filters-item-search-redistribution>>.
...
Die Auftragspositionen sind bearbeitbar, solange die Bestellung noch nicht ausgelöst wurde. Du kannst zum Beispiel die *Menge* ändern oder Artikel *löschen*. + Im Feld *Artikelname* kannst du die Bezeichnung für den Artikel anpassen. Die Änderungen gelten nur für die Umbuchung, in der sie vorgenommen wurden. Möchtest du die <<artikel/einstellungen#, Artikeleinstellungen>> generell anpassen, kannst du dies im Menü *Artikel » Artikel bearbeiten* im entsprechenden Tab machen.
...
Beim Öffnen des Menüs *Waren » Bestellwesen » Bestellung* werden dir alle Umbuchungen und <<warenwirtschaft/nachbestellungen-verwalten/nachbestellungen-vornehmen#, Nachbestellungen>> in einer Übersichtstabelle angezeigt. Durch die Suche (icon:filter[]) kannst du die angezeigten Umbuchungen und Nachbestellungen eingrenzen. Setze dafür die benötigten Filter und klicke dann auf *Suchen* (icon:search[role="blue"]). Du kannst mehrere Filter miteinander kombinieren. In <<table-redistribution-search-filter>> werden die Filtereinstellungen erläutert. +
