...
. Nimm die Einstellungen anhand der Informationen in <<tabelle-standard-umbuchung>> vor.
. *Speichere* (icon:save[role="green"]) die Einstellungen. +

[[tabelle-standard-umbuchung]]
...
*Aktiver Benutzer vorausgewählt*: Der Benutzer, der die Umbuchung erstellt, wird im Feld *Eigner* vorausgewählt. +
*Kein Eigner vorausgewählt*: Das Feld *Eigner* bleibt beim Anlegen einer Umbuchung leer. +
Des Weiteren werden dir die angelegten Nutzer angezeigt. Wählst du einen von ihnen, ist dieser beim Anlegen einer neuen Umbuchung vorausgewählt.
...
. Klicke auf das Hinzufügen-Icon (icon:plus-square[role="green"]) und wähle *Neue Umbuchung*. +
...
. Wähle den passenden Filter für die Suche und beachte dabei die Erläuterungen in <<tabelle-filter-artikelsuche-umbuchung>>.
...
. Klicke im Bereich *Warenkorb* (icon:cart-plus[role="green"]) auf *Speichern* (icon:save[role="green"]). +
...
. Klicke auf *Dokument hinzufügen* (icon:plus-square[role="green"]).
...
. Klicke im Bereich *Auftragsdetails* auf *Bestellung auslösen* (icon:cog[]). +
...
. Klicke auf *Warenbewegungen anlegen* (icon:cog[]). +
...
Die Paketinhaltsliste richtet sich in erster Linie an Mitarbeiter im Lager oder Fulfillment-Dienstleister und gibt Auskunft über den Inhalt von Paletten und Paketen. Da eine Umbuchung bedeutet, dass Ware von einem deiner Lager aus- und in ein anderes deiner Lager eingebucht wird, sind hier in der Regel in erster Linie Lagermitarbeiter involviert. Daher besteht die Möglichkeit die Pateinhaltsliste in der Umbuchung mit anzeigen zu lassen. Gehe dafür wie im Folgenden beschrieben vor.
...
. *_Optional:_* Gib für *Lieferdatum* das Datum ein, an dem die Waren ins Lager geliefert wurden.
. Klicke auf *Bestellung abschließen* (icon:cog[]). +
