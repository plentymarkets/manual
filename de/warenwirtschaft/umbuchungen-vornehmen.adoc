...
|Wähle aus der Dropdown-Liste aus, welcher Artikelname in der Nachbestellung angezeigt werden soll. *Name 1*, *Name 2* und *Name 3* vergibst du im Tab *Texte* der Artikdeldaten eines Artikeld.
...
. Wähle über dem Tab-Titel die Sprache aus, in der das Dokument erstellt werden soll.
...
Im nächsten Schritt fügst du die Auftragspositionen hinzu, die von einem Lager in ein anderes Lager umgebucht werden sollen. Nutze die Suche, um die gewünschten Auftragspositionen anhand der vorhandenen zu finden.
...
| *Auftrags-ID*
|Auftrags-ID der Nachbestellung eingeben, um diese konkrete Umbuchung zu suchen.
