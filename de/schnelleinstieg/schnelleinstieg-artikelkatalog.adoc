...
|Beide Optionen aktivieren, damit die Variante bei positivem Nettowarenbestand verfügbar und käuflich wird oder nicht käuflich wird, wenn kein Nettowarenbestand vorhanden ist.
...
. Öffne einige der importierten Artikel, um sicherzustellen, dass die Daten korrekt importiert wurden.
...
|Als Quelle für die Importdaten die Option *Eigener Wert* wählen und in das Feld *Wert* einen Text/eine Zahl eingeben. Der Inhalt des Felds wird für alle Varianten in das Feld in plentymarkets importiert, das als Ziel gewählt wird.
|====
