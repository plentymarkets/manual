...
Zum Einrichten von PayPal ist es zunächst erforderlich das PayPal-Plugin zu installieren und dann zu aktivieren. Im Menü *Plugins » Plugin-Übersicht* siehst du, welche Plugins du installiert hast und auch, welche aktiv sind. Außerdem kommst du von hier aus auch zum *plentyMarketplace* (icon:cart-plus[role="green"]).
...
. Wähle aus der Liste den Bereich *Cash in advance bank details (PrePayment)* (icon:angle-down[]).
...
. Öffne das Menü *CMS » Container-Verknüpfungen*.
. Wähle den Bereich *Cash in advance icon (PrePayment)* (icon:angle-down[]).
