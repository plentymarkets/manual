...
Die installierten Plugins sind im Menü *Plugins » Plugin-Übersicht* verfügbar. Für den Anfang bietet es sich an, die Plugins über die Schaltfläche *Plugin-Set Vorschau* im Vorschaumodus zu testen. Der Vorschaumodus ist nur von Benutzern deines Systems zu erreichen. Einstellungen, die du am Webshop vornimmst, können anschließend über die Schaltfläche *Plugin-Set aktivieren* in den Live-Modus übernommen werden.
...
→ Der ShopBuilder-Editor wird geöffnet.
. Ziehe per Drag&Drop Elemente von der linken Seite in den Vorschaubereich. Beachte dazu die Erläuterungen in <<tabelle-footer-sb>>.
. *Speichere* die Einstellungen.
...
. Gib einen Namen für deinen Checkout ein und wähle die *Standard-Checkout (Ceres)* als Vorlage.
. Klicke auf *Speichern*.
...
. Gib einen Namen für deinen Mein Konto-Bereich ein und wähle die *Standard-Mein Konto (Ceres)* als Vorlage.
. Klicke auf *Speichern*.
...
. Öffne das Menü *Plugins » Plugin-Übersicht*. +
→ Die Plugin-Übersicht wird geöffnet.
. Klicke links in der Werkzeugleiste auf *Plugin-Sets verknüpfen*
→ Das Fenster *Plugin-Sets verknüpfen* wird geöffnet.
. Wähle auf der rechten Seite das Plugin-Set, in dem du deine Anpassungen vorgenommen hast, aus.
. Klicke auf *Speichern*.
→ Das verknüpfte Plugin-Set wird mit einem grünen Punkt gekennzeichnet.
