...
In diesem Unterkapitel wollen wir exemplarisch ein Element in der Artikelansicht ausblenden, in unserem Beispiel den *Zur Wunschliste hinzufügen*-Button.
...
Das Ausblenden von Elementen funktioniert in der Regel immer gleich: Suche die relevante Stelle im Code, entferne sie, speichere die Einstellungen und stell dein Plugin-Set erneut bereit. Wir begleiten dich einmal durch den gesamten Prozess:

[.instruction]
Wunschliste in der Artikelansicht ausblernden:
...
Die entfernte Stelle beinhaltet eine if-Abfrage, die überprüft, ob die Route zur Wunschliste im Plugin *IO* aktiviert ist. Ist diese aktiviert, wird der Button angezeigt und fügt die geöffnete Variante entlang ihrer Varianten-ID zur Wunschliste hinzu. Durch das Entfernen der Stelle, wird der Button permanent ausgeblendet. Nach erneutem Bereitstellen deines Plugin-Sets wird der *Zur Wunschliste hinzufügen*-Button nicht mehr angezeigt.
