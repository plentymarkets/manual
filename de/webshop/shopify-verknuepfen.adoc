...
:keywords: Shopify, plentymarkets App, Shopify Assistent, Shopify Shops, Shopify verbinden, Shopify verknüpfen
:description: Erfahre, wie du dein plentymarkets Konto mit Shopify verknüpfst, um deine Daten zu synchronisieren.
:position: 100
:url: webshop/shopify-verknuepfen
:id: 9L2KQ4U
...
Genau wie plentymarkets, ist auch Shopify eine vollumfängliche E-Commerce-Plattform, mit der du dein Online-Business starten, verwalten und stetig ausbauen kannst. Da Shopify vollständig cloud-basiert und -gehostet ist, brauchst du dir um das Aktualisieren oder Warten von Software oder Webservern keine Sorgen zu machen.
...
. Öffne die Seite link:https://www.shopify.com/signup[Shopify^]{nbsp}icon:external-link[].
...
→ Die App wird installiert und die Seite zur Registrierung bei plentymarkets wird geöffnet.
...
→ Du kannst nun die Import- und Exporteinstellungen wie gewünscht vornehmen.
...
| Shopify-Verbindung
| Zeigt den Shopify-Hostnamen, den verbundenen plentymarkets Mandanten und den Verbindungsstatus an. In diesem Schritt ist keine Aktion notwenig.
...
| Zuordnung plentymarkets Lager zu Shopify-Standort
...
*_Hinweis:_* Nur Warenbestände aus Lagern, die einem Standort zugeordnet wurden, können exportiert und importiert werden.
...
| Zeigt die vorhandenen Eingabefeldzuordnungen an. Falls eine Zuordnung nicht mehr benötigt wird, kannst du sie hier zum Löschen markieren. Sobald der Assistent abgeschlossen ist, werden alle gewählten Zuordnungen gelöscht.
...
*_Hinweis:_* Damit Shopify-Metafelder im Shopify-Backend sichtbar sind, wird eine spezielle Shopify-App benötigt. Solltest du nähere Informationen zu Shopify-Metafeldern benötigen, öffne das link:https://help.shopify.com/de/manual/metafields[Shopify Help Center^]{nbsp}icon:external-link[] und gib das Stichwort *Metafelder* an.
...
| Eigenschaft
| Dieser Schritt ist nur verfügbar, wenn du im Schritt *plentymarkets Quelleingabefeld* die Option *Eigenschaft* wählst. Wähle die Eigenschaft, die als Exportdatenquelle verwendet werden soll.
...
[#70]
=== Beschränkung für Warenbestand einer Variante festlegen

Beim Export von Warenbestand wird auch die *Beschränkung* von Warenbestand zu Shopify exportiert. Je nach gewählter *Beschränkung* ist die Checkbox *Verkauf fortsetzen, auch wenn Produkte nicht auf Lager sind* in Shopify aktiv oder inaktiv. Nähere Informationen dazu findest du in <<table-stock-limitation>>.

[[table-stock-limitation]]
.Beschränkungen für Warenbestand
[cols="2,2"]
|====
| *Beschränkung in plentymarkets* | *Verkauf fortsetzen, auch wenn Produkte nicht auf Lager sind*
...
Mit der Shopify-Integration kannst du außerdem benutzerdefinierte Auftragseigenschaften über den *Auftragsimport* importieren. Mit dem link:https://apps.shopify.com/product-personalizer[Product Personalizer plugin^]{nbsp}icon:external-link[] kannst du beispielsweise benutzerdefinierte Notizen, wie zum Beispiel von Nutzer:innen definierte Gravuren, hinzufügen. +
...
Ereignisaktionen in plentymarkets ermöglichen es dir, deine Aufträge in Shopify noch gezielter zu verwalten. Nähere Informationen zu plentymarkets Ereignisaktionen findest du auf der Handbuchseite <<automatisierung/ereignisaktionen#, Ereignisaktionen>>.

[#100]
=== Berücksichtigung der DSGVO

Aufgrund von Vorgaben der Datenschutzgrundverordnung (DSGVO), gelten die im Folgenden aufgeführten Richtlinien für Konten und Aufträge von Nutzer:innen.

[[table-gdpr-considerations]]
.Richtlinien für Kontaktimport
[cols="1,2"]
|====
|*Szenario* |*Erläuterung*

| *Kund:in wird als Gast importiert*
| Hat die Kund:in kein plentymarkets Konto, kann die Authentizität der Kund:in von keiner der beiden Seiten garantiert werden. Dies gilt auch dann, wenn die Kontaktdaten auf mehreren Aufträgen identisch sind. +

*Tipp:* Wenn notwendig, kannst du Kontakte des Typs *Gast* in reguläre Kontakte umwandeln. Weitere Informationen dazu findest du auf der entsprechenden <<crm/schnellsuche#gastzugang-umwandeln, Handbuchseite>>.
...
| *Ein bestehender plentymarkets Kontakt wird dem importierten Auftrag zugeordnet*
