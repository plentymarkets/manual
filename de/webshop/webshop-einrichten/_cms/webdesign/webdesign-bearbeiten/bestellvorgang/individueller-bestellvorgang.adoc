...
Richte Google reCAPTCHA für die Kundenregistrierung im Webshop ein. Registriere deinen Webshop zunächst für die Verwendung von link:https://www.google.com/recaptcha/admin#list[Google reCAPTCHA^]{nbsp}icon:external-link[]. Gib anschließend den Websiteschlüssel und den Geheimen Schlüssel für deinen Webshop ein.
