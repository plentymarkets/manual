...
. Fügen Sie die Template-Funktion `{% GetCaptcha('normal') %}` bzw. `{% GetCaptcha('compact') %}` oberhalb der Schaltfläche *Registrierung abschließen* ein.
...
. Öffnen Sie die Kategorie *Registrierung*.
. Wechseln Sie in das Tab *Beschreibung 1*.
. Fügen Sie die Template-Funktion `{% GetCaptcha('invisible') %}` oberhalb der Schaltfläche *Registrierung abschließen* ein.
. Suchen Sie den Code `plenty.AuthenticationService.registerCustomer();`.
