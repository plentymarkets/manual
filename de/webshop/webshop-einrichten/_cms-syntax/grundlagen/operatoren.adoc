...
Die Operator-Rangfolge legt fest, wie "eng" ein Operator zwei Ausdrücke miteinander verbindet. Zum Beispiel ist das Ergebnis des Ausdruckes _1 + 5 * 3_ gleich _16_ und nicht _18_, da der Multiplikations-Operator (*) in der Rangfolge höher steht als der Additions-Operator (+). Wenn nötig, können Sie Klammern setzen, um die Rangfolge der Operatoren zu beeinflussen. Zum Beispiel ergibt _(1 + 5) * 3_ gleich _18_. Ist die Rangfolge der Operatoren gleich, so wird links nach rechts Assoziativität benutzt.
