...
Im Menü *System » Mandant* verwaltest du deinen <<#10, Standard-Mandanten>> sowie zusätzliche Mandanten (Shops). Hier verknüpfst du zudem mehrere <<webshop/mandanten-einrichten/webshop#, Webshops>> mit einem Backoffice/Administrationsbereich. Mit dieser Funktionalität kannst du <<#60, weitere Mandanten (Shops)>> parallel zu deinem Standard-Webshop einrichten. In einem plentymarkets System können bis zu 30 eigenständige Mandanten (Shops) eingerichtet und verwaltet werden. Mandanten sind datentechnisch geschlossene, eigenständige Systeme. Es ist somit möglich, über ein plentymarkets System mehrere unterschiedliche Geschäftsbereiche zu realisieren.

[#10]
== Standard-Mandanten einrichten

Im Menü *System » Systemeinstellungen » Mandant » Mandant wählen* speicherst du verschiedene Einstellungen für deinen <<webshop/mandanten-einrichten/webshop#, Webshop>> und richten den <<crm/e-mails-versenden#700, E-Mail-Dienst>> für den Standard-Mandanten ein. Des Weiteren findst du in diesem Menü die Einstellungen zur <<webshop/webshop-einrichten/cms-syntax#fallbeispiele-seo-guide, Suchmaschinenoptimierung>>, mit denen du deinen Webshop im Internet optimal auffindbar machst. Darüber hinaus richtest du die Anzeige von <<artikel/kategorien#, Kategorien>>, den <<webshop/webshop-einrichten/mein-konto#, Mein Konto>>-Bereich und den <<webshop/webshop-einrichten/bestellvorgang#, Bestellvorgang>> deines Webshops ein. Auch Informationen zu den Grundeinstellungen deiner <<webshop/webshop-einrichten/artikelansichten#, Artikelansichten>> findest du hier.

[#20]
=== Grundeinstellungen vornehmen

Im Menü *System » Systemeinstellungen » Mandant » Mandant wählen » Grundeinstellungen* werden die IDs deines Webshops angezeigt sowie weitere Grunddaten wie der Webstore-Typ und die Domain. Nimm die Einstellungen wie folgt vor.
...
. Öffne das Menü *System » Systemeinstellungen » Mandant » Mandant wählen » Grundeinstellungen*.
...
[NOTE]
====
Diese Funktionalität ist nur für den alten Webshop Callisto verfügbar. Für Webshops, die über das Ceres-Template aufgesetzt wurden, passst du Sprachdateien im Menü *CMS » Mehrsprachigkeit* an. Weitere Informationen hierzu findest du <<webshop/ceres-einrichten#231, hier>>.
====
...
. Öffne das Menü *System » Mandant » Global » Sprachpakete*.
. Wähle unter *Sprache* ein Sprachpaket.
. Wähle einen *Bereich*.
. Klicke auf die *Lupe*. +
→ Die Liste der *Texte* wird angezeigt.
. Bearbeite die Texte in der Spalte *Eigene Übersetzung*.
. *Speichere* die Einstellungen.
...
. Öffne das Menü *Start » plentymarkets Konto: Service-Center*. +
→ Der *Mein Konto*-Bereich wird geöffnet.
. Klappe das System auf, für das du weitere Mandanten buchen möchtest.
. Wechsle in das Tab *Vertrag*.
. Klicke auf *Tarifkonfigurator öffnen*.
. Füge im Bereich *Online-Shop* einen oder mehrere Mandanten hinzu.
. Nimm ggf. weitere Änderungen an deinem System vor.
. Klicke auf *Weiter*.
. Klicke im Bestellschritt *Ihr System* auf *Systemänderungen bestellen*. +
...
. Öffne das Menü *System » Mandant » Mandant wählen » Einstellungen*.
...
Für die Arbeit mit dem <<crm/ticketsystem-nutzen#, Ticketsystem>> bzw. für das Versenden eines <<crm/newsletter-versenden#, Newsletters>> musst du deine Konfigurationen im Bereich <<crm/e-mails-versenden#, E-Mail>> hinterlegt haben. Darüber hinaus legst du mit den Einstellungen im Bereich Artikelansichten den Grundstein für die Verwaltung deiner <<artikel/artikel-verwalten#, Artikel>>.
