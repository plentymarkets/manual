...
In Zukunft benötigt Rakuten die Sendungs-Herkunft jedes Auftrags um diesen als versendet zu markieren. Die benötigten Informationen werden automatisch über die Ereignisaktion *Versandbestätigung an Rakuten senden* an Rakuten.de übertragen. Die jeweiligen Daten werden aus dem mit dem Auftrag verknüpften Lager geladen.
...
#To-Do:# Speichert die aktualisierten SFTP-Zugangsdaten, die ihr von DHL Freight erhalten habt, im Menü *Einrichtung » Aufträge » Versand » Dienstleister » DHL Freight*.
...
Bisher wurde nur der Versanddienstleister DPD selbst als DPD zu link:Rakuten.de[Rakuten.de^] übermittelt.
Ab sofort werden alle Versanddienstleister, die DPD im Namen haben, als DPD zu Rakuten.de exportiert.
