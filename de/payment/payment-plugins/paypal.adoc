...
Ist das PayPal-Plugin installiert, werden Zahlungen automatisch stündlich vom verknüpften PayPal-Account ins plentymarkets System importiert. Nach dem Import werden die PayPal-Zahlungen automatisch den entsprechenden <<payment/beta-zahlungen-verwalten#10, Aufträgen zugeordnet>>, wenn Zuordnungskriterien erfüllt sind. Aufgrund technischer Beschränkungen bei PayPal kann sich beim Import unter Umständen eine Verzögerung von bis zu drei Stunden ergeben.
