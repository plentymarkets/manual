...
In diesem Abschnitt sind FAQ zum Einrichten des EBICS-Plugins zusammengestellt.

Du kannst dir auch den link:https://www.plentymarkets.eu/blog/plentyCommunity-Projekt-EBICS-Komplett-auf-den-Anwender-ausgerichtet/b-1941/[Blog-Beitrag^]{nbsp}icon:external-link[] zum Plugin durchlesen, um allgemeine Informationen zur Entwicklung des Plugins zu bekommen.
...
Bereits vorhandene Konten können ganz einfach in das Plugin <<#200, migriert>> werden.
...
* Die Formate können in den <<#130, Kontoeinstellungen>> angepasst werden. +
