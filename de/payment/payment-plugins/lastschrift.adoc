...
:keywords: Zahlungsarten, Zahlungen, Lastschrift
...
Mit dem Plugin *Lastschrift* bindest du die Zahlungsart Lastschrift in deinem Webshop ein und stellst sie so deinen Kund:innen zur Auswahl. Die Integration erfolgt über einen Assistenten. Zudem musst du einige grundlegende Einstellungen in deinem plentymarkets System ausführen, damit die Zahlungsart auch in deinem System aktiviert ist.
...
. Wähle den Assistenten *Lastschrift*.
. Schließe den Assistenten vollständig ab. +
→ Die Zahlungsart Lastschrift ist in deinem Webshop verfügbar.
