...
Mit dem Plugin Kauf auf Rechnung bindest du die Zahlungsart *Rechnung* in deinen Webshop ein und stellst sie so deinen Kund:innen zur Auswahl. Lade dafür das Kauf auf Rechnung-Plugin im link:https://marketplace.plentymarkets.com/invoice_4760[plentyMarketplace^]{nbsp}icon:external-link[] herunter. Die Einrichtung der Zahlungsart erfolgt über einen Assistenten. Zudem musst du einige grundlegende Einstellungen in deinem plentymarkets System ausführen, damit die Zahlungsart auch in deinem System aktiviert ist. +
