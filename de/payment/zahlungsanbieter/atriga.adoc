...
. Öffnen Sie das Menü *System » Mandant » Mandant wählen » Dienste » atriga*.
...
|*Überprüfungsmethode*
|*Nachträgliche Bonitätsprüfung* = der Kunde gibt sein Einverständnis zur Prüfung und wird anschließend geprüft. Wenn er sein Einverständnis nicht gibt, stehen ihm die Zahlungsarten Rechnung, Lastschrift und Kreditkarte nicht zur Verfügung. Für die Prüfung sind Anpassungen an Ihrem Design notwendig. Im Template <<auftragsabwicklung/payment/atriga#120, PageDesignPrepareMainColumn>> müssen die Template-Variablen *AtrigaRequireUserConfirmation* und *AtrigaShowValidationError* angepasst werden.
