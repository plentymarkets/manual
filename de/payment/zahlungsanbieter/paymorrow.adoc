...
Fordern Sie ggf. unter link:mailto:technik@paymorrow.de[technik@paymorrow.de] einen Test-Account an und führen Sie testweise einige Transaktionen durch, bevor Sie die Zugangsdaten einrichten. Zu dieser Vorgehensweise stellt Paymorrow ein link:https://www.plentymarkets.eu/documents/category/2611/Abnahmeprotokoll-paymorrow.pdf[technisches Abnahmeprotokoll^] zur Verfügung, das Sie ausfüllen müssen, um die Zugangsdaten zu erhalten. Im verlinkten Dokument ist auch die Vorgehensweise erläutert. Für den Testbetrieb erfolgt die Authentifizierung über eine eigene <<auftragsabwicklung/payment/paymorrow#40, RSA-Zertifikatsregistrierung>>.
...
Richten Sie die <<auftragsabwicklung/payment/zahlungsarten-verwalten#20, Zahlungsarten>> Paymorrow Rechnungskauf und Lastschrift im Tab *Einstellungen* der Zahlungsarten ein. Prüfen Sie zudem die Einstellungen für *Erlaubte Zahlungsarten* in den <<auftragsabwicklung/payment/zahlungsarten-verwalten#30, Kundenklassen>> und für *Gesperrte Zahlungsarten* in den <<auftragsabwicklung/fulfillment/versand-vorbereiten#1000, Versandprofilen>>.

[#60]
== Rechnungsvorlage für Paymorrow-Aufträge anpassen

Wenn Sie Paymorrow nutzen und Rechnungen für Aufträge erzeugen, die mit der Paymorrow-Zahlungsart Rechnungskauf bezahlt werden, ist es notwendig, dass Sie die Bankdaten von Paymorrow auf der Rechnung anzeigen statt Ihrer Bankdaten. Dies geschieht automatisch über einen Zahlungshinweis. Wenn Sie Ihre eigene <<auftragsabwicklung/auftraege/auftragsdokumente#170, PDF-Vorlage>> verwenden möchten, gehen Sie wie nachfolgend beschrieben vor.
...
Wenn ein Kunde die Ware oder einen Teil davon zurückgeschickt hat, müssen Sie eine Gutschrift anlegen und Paymorrow darüber informieren, damit der Kunde keine Mahnung von Paymorrow erhält. Richten Sie eine <<basics/automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um Gutschriften automatisch an Paymorrow zu senden.
...
Wird ein Auftrag storniert, muss der Status den Einstellungen unter *System » Aufträge » Einstellungen* entsprechen, damit der Vorgang auch als storniert an Paymorrow übermittelt wird. Richten Sie eine <<basics/automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um Stornierungen automatisch an Paymorrow zu senden.
