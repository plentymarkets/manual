...
Zum einen benötigt Ihr Webshop ein <<basics/admin-aufgaben/plentymarkets-konto#ssl-zertifikat, SSL-Zertifikat>>, damit Sie die Zahlungsart Amazon Pay nutzen können. Außerdem ist es notwendig, nach der Konfiguration bzw. Änderungen das <<omni-channel/online-shop/webshop-einrichten/cms#webdesign-webdesign-bearbeiten, Design zu veröffentlichen>>, da verschiedene Elemente der Zahlungsart im *CMS* eingebunden sind.

[#40]
== Bei Amazon Pay registrieren

Sie benötigen für die Zahlungsart *Amazon Pay* ein separates Konto für Amazon. Das im Menü *System » Systemeinstellungen » Märkte » Amazon » Einstellungen* angelegte Amazon Seller Central-Konto können Sie für die Zahlungsart Amazon Pay nicht nutzen. Der Registrierungsprozess als Händler bei *Bezahlen mit Amazon* wird im link:https://www.plentymarkets.eu/documents/category/2077/Login-und-Bezahlen-Registrierungshandbuch-DE-v1-2.pdf[Amazon-Pay-Registrierungshandbuch^] beschrieben.
...
====
Sie müssen bei Amazon Pay unter Händler-URL und Integrator-URL *ihr-plentyshop.de/plenty/api/payment_notification.php?payment=apa* eingeben. Ersetzen Sie *ihr-plentyshop.de* durch die Adresse Ihres Shops. Die Adresse Ihres Shops finden Sie z.B. im Menü *System » Systemeinstellungen » Einstellungen » API » Daten* unter *Host*. Das Protokoll *https://* muss eingegeben werden. Die Vorgehensweise wird auch im link:https://www.plentymarkets.eu/documents/category/2077/Login-und-Bezahlen-Registrierungshandbuch-DE-v1-2.pdf[Amazon-Pay-Registrierungshandbuch^] beschrieben.
====
...
Gehen Sie wie unten beschrieben vor, um die Schaltfläche *Bezahlen mit Amazon* im Overlay für den Warenkorb an gewünschter Stelle einzubinden. Das Overlay wird angezeigt, nachdem der Kunde bei einem Artikel auf *In den Warenkorb* klickt.
...
Gehen Sie wie unten beschrieben vor, um die Schaltfläche *Bezahlen mit Amazon* in der Warenkorbvorschau anzuzeigen.
...
Gehen Sie wie unten beschrieben vor, um die Schaltfläche *Bezahlen mit Amazon* im Warenkorb anzuzeigen.
...
Gehen Sie wie unten beschrieben vor, um die Schaltfläche *Bezahlen mit Amazon* auf der Anmeldeseite anzuzeigen.
...
Richten Sie eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um Versandbestätigungen automatisch zu versenden, nachdem der Warenausgang gebucht wurde. Das Buchen des Warenausgangs und das Versenden der Versandbestätigung sollten nicht in einer Ereignisaktion erfolgen. Sonst kann es passieren, dass der Versand bestätigt wird, bevor der Warenausgang gebucht wurde. Richten Sie daher eine separate Ereignisaktion zur Buchung des Warenausgangs ein, die gleichzeitig den Status z.B. auf 7.1 ändert. Den Statuswechsel nutzen Sie wiederum bei der Ereignisaktion zur Versandbestätigung als Ereignis, damit nur für Aufträge im festgelegten Status eine Versandbestätigung gesendet wird. Den gewünschten Status müssen Sie ggf. vorher im Menü <<auftraege/auftraege-verwalten#1200, System » Systemeinstellungen » Aufträge » Auftragsstatus>> erstellen.
...
Richten Sie eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um Amazon automatisch über Stornierungen zu informieren.
...
Richten Sie eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um Ihre Kunden automatisch über abgelehnte Zahlungen zu informieren. Damit von Amazon abgelehnten Zahlungen automatisch per E-Mail an den Kunden und per Statuswechsel an Sie gemeldet werden, erstellen Sie einen *Auftragsstatus* für *abgelehnte Zahlungen* von Amazon Pay oder nutzen einen vorhandenen Auftragsstatus.
