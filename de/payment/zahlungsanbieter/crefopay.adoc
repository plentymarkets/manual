...
Gehen Sie wie unten beschrieben vor, um die Schnittstelle zu CrefoPay für Ihren Callisto Shop in plentymarkets einzurichten.
...
Damit CrefoPay Beträge an Sie auszahlt, müssen Sie bestätigen, dass Sie dem Kunde die Ware gesendet haben. Richten Sie eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um Versandbestätigungen automatisch zu versenden, nachdem der Warenausgang gebucht wurde.
...
Richten Sie eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um CrefoPay automatisch über Stornierungen zu informieren.
