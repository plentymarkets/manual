...
.SSL-Zertifikat
====
Wenn Sie in Ihrem Webshop keine SSL-Verschlüsselung verwenden, verwenden Sie die URL mit http, statt mit https. Wir empfehlen die Verwendung eines <<basics/admin-aufgaben/plentymarkets-konto#ssl-zertifikat, SSL-Zertifikats>>.
====
...
Im <<basics/daten/api-log#6, API-Log>> finden Sie Einträge mit Informationen zum Verbindungsaufbau zum SFTP-Server von PayPal und z.B. zur Anzahl der im Abrechnungsbericht enthaltenen Transaktionen.
...
Anschließend richten Sie eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um die Beträge automatisch an PayPal zu senden.
