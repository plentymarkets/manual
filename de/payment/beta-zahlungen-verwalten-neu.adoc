...
Optional kannst du über *Ansicht bearbeiten* (icon:design_inline_edit[set=plenty]) noch das Element *Absender der Zahlung* hinzufügen. Verändere die Anordnung der Elemente im Portlet, indem du sie per Drag & Drop verschiebst. +
...
Optional kannst du über *Ansicht bearbeiten* (icon:design_inline_edit[set=plenty]) noch folgende Infoboxen als zusätzliche Elemente zu diesem Portlet hinzufügen:
...
Verändere die Anordnung der Elemente im Portlet, indem du sie per Drag & Drop verschiebst. Entscheide in den *Einstellungen* des Protlets zudem, wieviele Kacheln (Infoboxen) dir in einer Zeile angezeigt werden sollen.
...
Das Portlet *Eigenschaften* (= Properties) im unteren Bereich zeigt dir alle weiteren Informationen, die die Zahlung betreffen. Diese Informationen gehen in der Regel über die Details hinaus. Welche Informationen hier angezeigt werden, hängt von der Zahlung selber, z.B. der Zahlungsart und welche Informationen vom Zahlungsanbieter übermittelt werden, ab.
