...
Für die Suche (icon:filter[]) kannst du verschiedene Filter setzen, um beispielsweise nach Zahlungen mit einer bestimmten Zahlungsart oder Währung zu suchen. Du kannst Suchfilter auch kombinieren, zum Beispiel um alle Zahlungen mit einer Zahlungsart für einen festgelegten Zeitraum zu suchen. Gehe dabei wie im Folgenden beschrieben vor.
