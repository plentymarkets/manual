...
Zahlungen suchst du in der Übersichtsansicht im Menü *Aufträge » Zahlungsverkehr*. Individualisiere dir den Header der Tabelle. So siehst du nach einer Suche immer die Informationen auf einen Blick, die für dich die wichtigsten sind. +
Für die Suche kannst du verschiedene Filter setzen, um beispielsweise nach Zahlungen mit einer bestimmten Zahlart oder Währung zu suchen. Du kannst Suchfilter auch kombinieren, zum Beispiel um alle Zahlungen mit einer Zahlart für einen festgelegten Zeitraum zu suchen. Gehe dabei wie im Folgenden beschrieben vor.
...
Du kannst anpassen, welche Informationen dir in dieser Tabelle angezeigt werden. Individualisiere dafür den Header der Tabelle, so dass dir in den Spalten die Informationen angezeigt werden, die für dich am wichtigsten sind.
...
. Klicke über der Übersichtstabelle entweder auf *Zahlungen löschen* (icon:minus-circle[role=red]) oder auf *Zahlungen automatisch zuordnen* (icon:payment_assign[set=plenty,role="green"]). +
