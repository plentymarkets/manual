= beta: Zahlungen verwalten
:lang: de
include::{includedir}/_header.adoc[]
:description: Payments in plentymarkets: Erfahre alles über Zahlungseingänge und Zahlungsausgänge.
:position: 25
:url: payment/beta-zahlungen-verwalten
:id: VBZTVJ8
:keywords: Zahlung, Zahlungen, Zahlungsverkehr, Zahlungseingang, Zahlungseingänge, Zahlungsquelle, Zahlungsmöglichkeiten, Zahlungsanbieter, Vorab-Überweisung, Überweisung, Payment Provider, Bezahlung, Barzahlung, Payment-Plugins, Payment, Zahlungskulanz, Zahlungszuordnung, Nachkommabeträge, Kundenbankdaten, Kundendaten, Mindestbetrag, PayPal, ebay, WooCommerce, Zahlungsdaten, Bankbuchung, Vorkasse, Überweisung, Auftragszuordnung, Zahlung zuordnen, CSV, CSV-Format, Standardkonto, Standard-Konto, Bankbuchungsdatei, Bankbuchung, Zahlung buchen, Zahlungsübersicht, manuell buchen, Betrag, Haben, Soll, Währung, Wechselkurs, Standard-Währung, Standardwährung, Wechselkurs, Verwendungszweck, manuelle Buchung, nicht buchbar, nicht buchbare Zahlung, Zuordnung, nicht verbuchbar, Vormerkposten, Übereinstimmungsquote, Transaktionstyp, Wert, Zahlungsinformationen, Gesamtbetrag, Bezahlt, Offen, Zahlung aufteilen, Teilzahlung, Überbezahlung, Überzahlung
:author: team-order-payment

Diese Seite beschreibt das Verwalten von Zahlungen mit der neuen Benutzeroberfläche. Diese Funktionen sind derzeit in _beta_, stehen also noch nicht in allen Systemen zur Verfügung. Beachte, dass die Informationen in den Kapiteln <<payment/zahlungen-verwalten#10, Grundeinstellungen für Zahlungseingänge vornehmen>> sowie <<payment/zahlungen-verwalten#20, Zahlungen buchen>> auch für das Arbeiten mit der neuen Benutzeroberfläche gelten.

[#10]
== Neue Zahlungen im System

Nach der Buchung in plentymarkets werden die Zahlungen Aufträgen über eine unscharfe Suche automatisch zugeordnet. Wenn die Zuordnung fehlschlägt, können Buchungen Aufträgen manuell zugeordnet werden. Zahlungen kann man im Menü *Aufträge » Zahlungsverkehr* bearbeiten. Unzugeordnete Zahlungen sind mit einem *grünen Pfeil* gekennzeichnet.
...
Zahlungen suchst du in der Übersichtsansicht im Menü *Aufträge » Zahlungsverkehr*. Du kannst verschiedene Filter setzen, um zum Beispiel nach Zahlungen mit einer bestimmten Zahlart oder Währung zu suchen. Du kannst Suchfilter auch kombinieren, zum Beispiel um alle Zahlungen mit einer Zahlart für einen festgelegten Zeitraum zu suchen. Gehe dabei wie im Folgenden beschrieben vor.
...
|*Zahlungsart*
| Wähle eine bestimmte Zahlungsart aus, um nach Zahlungen zu suchen, die mit dieser Zahlungsart getätigt wurden.
...
|*Haben/Soll*
|Wähle zwischen *Haben* oder *Soll*. +
...
. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Suche (icon:search[role=blue]) anhand der <<payments/beta-zahlungen-verwalten#20, Filtereinstellungen>> die gewünschte Zahlung.
. Öffne die Zahlung, indem du entweder in der Zeile der unzugeordneten Zahlung, die du zuordnen möchtest, , auf die Payment-ID oder auf auf das Icon *Zuordnen* klickst. +
...
. Wähle den passenden Auftrag aus und klicke auf *Zahlung zuordnen*. +
...
. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Suche (icon:search[role=blue]) anhand der <<payments/beta-zahlungen-verwalten#20, Filtereinstellungen>> die gewünschte Zahlung.
. Klicke in der Zeile der Zahlung, bei der du die Zuordnung lösen möchtest, auf das Icon *Zahlung lösen*. +
...
[.instruction]
Zahlungseingang in Detailansicht zuordnen:
...
→ Die Tabelle öffnet sich und die bestehende Zuordnung wird angezeigt.
. Klicke auf *Zahlung lösen*. +
...
====
Das Lösen von Zahlungen von einem Auftrag ist technisch zu jedem Zeitpunkt möglich, auch wenn die Auftragsbearbeitung schon fortgeschritten oder sogar abgeschlossen ist. Das Lösen von Zahlungen sollte aber nur möglichst gezielt geschehen, da durch wiederholtes Zuordnen und Lösen Fehler entstehen können, z.B., dass der Auftrag nicht mehr vollständig durch die Zahlung bezahlt wird. Beachte zudem, dass das Lösen einer Zahlung auch Änderungen an dem Auftrag, von dem die Zahlung gelöst wurde, bewirkt, wie z.B. eine Neuberechnung der Zahlung im Auftrag oder einen Statuswechsel.
====
...
. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Suche (icon:search[role=blue]) anhand der <<payments/beta-zahlungen-verwalten#20, Filtereinstellungen>> die gewünschte Zahlung.
. Klicke in der Zeile der gewünschten Zahlung auf das Icon *Zahlung aufteilen*. +
→ Du wirst weitergeleitet zu einer eigenen Ansicht, in der du das Teilen vornimmst.
. Suche anhand der Filter *Auftrags-ID*, *Rechnungsnummer* und *Kunden-ID* nach den entsprechenden Aufträgen. Du kannst mehrere IDs/Nummern eingeben, indem du sie durch ein Komma separierst.
. Setze Häkchen bei den Aufträgen, zu denen du die Zahlung zuordnen möchtest.
. Klicke auf *Speichern* (icon:save[role=green]). +
→ Die Zahlung wird den ausgewählten Aufträgen in der Reihenfolge der Tabelle von oben nach unten zugeordnet und du wirst zur Zahlungsübersicht zurückgeleitet. Hier lassen sich die entstandenen Teilzahlungen wie in den anderen Kapiteln beschrieben bearbeiten.
...
====
Das Aufteilen einer Zahlung kann nicht rückgängig gemacht werden. Um den ursprünglichen Zahlungseingang wiederherzustellen, lösche alle Teilzahlungen. Wird eine Teilzahlung gelöscht, wird der Betrag wieder zur ursprünglichen Zahlung hinzugerechnet.
====
...
. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Klicke in der Zeile eines Zahlungseingangs auf *Löschen* (icon:minus-square[role=red]). +
...
. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Klicke in die Zeile der Zahlung, die gelöscht werden soll. +
→ Du wirst weitergeleitet zur Detailansicht dieser Zahlung.
. Klicke über den Zahlungsdetails auf *Zahlung löschen* (icon:minus-square[role=red]).
...
Mit der Gruppenfunktion bearbeitest du beliebig viele Zahlungen gleichzeitig. Du wählst entweder durch das Setzen des Häkchens im Header der Tabelle im Menü *Aufträge » Zahlungsverkehr* alle dort angezeigten Zahlungen aus oder du nutzt die Suche, um die gewünschten Zahlungen zu filtern. Nutzt du die Suchoptionen, musst du die Zahlungen einzeln durch das Setzen eines Häkchens auswählen. Um die Gruppenfunktion nutzen zu können, muss mindestens eine Zahlung ausgewählt sein. Nachdem du die gewünschten Zahlungen ausgewählt hast, kannst du diese entweder alle auf einmal automatisch zuordnen oder sie alle auf einmal löschen.

Gehe wie im Folgenden beschrieben vor, um Zahlungseingänge mit der Gruppenfunktion automatisch zuzuordnen oder zu löschen.

[.instruction]
Zahlungseingänge per Gruppenfunktion bearbeiten:

. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Wähle die Zahlungseingänge aus, die du zuordnen oder löschen möchtest, indem du ein Häkchen setzt.
. Klicke über der Übersichtstabelle entweder auf *Zahlungen löschen* oder auf *Zahlungen automatisch zuordnen*. +
...
Der Bereich *Zuordnung* bietet dir eine Auflistung der höchsten Übereinstimmungen bei unzugeordneten Zahlungen. Ist eine Zahlung bereits zugeordnet, wird hier nur dieser entsprechende Eintrag angezeigt.
