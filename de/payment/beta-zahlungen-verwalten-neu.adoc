...
Zahlungen gehen entweder automatisch im System ein oder werden manuell eingebucht. Nach dem Eingang in plentymarkets werden die Zahlungen Aufträgen über eine unscharfe Suche automatisch zugeordnet. Wenn die Zuordnung fehlschlägt, können Zahlungen Aufträgen manuell zugeordnet werden. Verwalte und bearbeite Zahlungen im Menü *Aufträge » Zahlungsverkehr*. Unzugeordnete Zahlungen sind mit einem *grünen Pfeil* (icon:payment_assign[set=plenty,role="green"]) gekennzeichnet. +
...
. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Suche (icon:filter[]) anhand der <<payment/beta-zahlungen-verwalten#20, Filtereinstellungen>> die gewünschte Zahlung.
...
. Öffne das Menü *Aufträge » Zahlungsverkehr*.
. Suche (icon:filter[]) anhand der <<payment/beta-zahlungen-verwalten#20, Filtereinstellungen>> die gewünschte Zahlung.
. Klicke in der Zeile der gewünschten Zahlung auf das Icon *Zahlung aufteilen* (icon:item_variation_distribute_quantity[set=plenty]). +
→ Du wirst weitergeleitet zu einer eigenen Ansicht, in der du das Teilen vornimmst.
. Suche (icon:filter[]) anhand der Filter *Auftrags-ID*, *Externe Auftrags-ID*, *Rechnungsnummer*, *Kunden-ID* und *Name* nach den entsprechenden Aufträgen. Du kannst mehrere IDs/Nummern eingeben, indem du sie durch ein Komma separierst.
...
Neben den Funktionen <<payment/beta-zahlungen-verwalten#30, Zahlungen zuordnen>>, <<payment/beta-zahlungen-verwalten#40, Zahlungen lösen>> und <<payment/beta-zahlungen-verwalten#60, Zahlungen löschen>> bietet dir die Detailansicht einer Zahlung alle mit dieser Zahlung verknüpften Informationen. Infoboxen geben dir einen schnellen Überblick. Die Infobox *Betrag* wird in grün wiedergegeben, wenn der Betrag über null liegt, ansonsten in grau. Der Status wird ebenfalls über Farben der Infobox ausgedrückt:
