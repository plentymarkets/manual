...
[#einlieferungsliste]
Einlieferungsliste:: Enthält die Paketnummer, die Lieferadresse, den Betrag des Auftrags sowie die Auftrags-ID. Bei Abholung der Pakete unterschreibt der Fahrer Versanddienstleisters die Liste, die als Abholbeleg dient.
...
[entnahme]
Entnahme:: Physische Reduzierung des Bargedlbestandes, um beispielsweise Beträge von der Kasse auf andere Konten zu transferieren. <<pos/pos-kassenbenutzer#200, Entnahmen>> werden in der plentymarkets App oder im Backend verbucht und können verschiedenen Buchungskonten zugewiesen werden.
...
[#festpreis]
<<#listing, Listing>>, das zum Festpreis angeboten wird und auf das nicht geboten werden kann.
...
Weitere Informationen findest du auf der Hanbuchseite <<daten/daten-exportieren/FormatDesigner, FormatDesigner>>.
...
[#listing]
Listing:: Im Listing werden Grundeinstellungen unabhängig von der Plattform vorgenommen, zum Beispiel Einstellungen zu Bestandsabhängigkeit oder Inhalt des Listings. Außerdem wird der Listing-Typ festgelegt. Es gibt Listings vom Typ <<#festpreis, Festpreis>>, <<#auktion, Auktion>> oder <<#shop-artikel, Shop-Artikel>>. +
