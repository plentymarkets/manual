...
====
Das Statistikmenü öffnest du über den Menüpfad *Start » Statistik*. Klicke alternativ auf die Schaltfläche *Statistik* in der Titelleiste in plentymarkets.
====
...
Um einzelne Kontakte statistisch auszuwerten, kannst du im Menü *CRM » Kontakte » Kontakt öffnen » Ansicht: Statistik* analog zum Statistikmenü Tabs und Statistiktypen erstellen. Statistiken, die du in diesem Menü für einen Kontakt erstellen, werden nach dem Speichern bei allen Kontaktdatensätzen im Tab *Statistik* angezeigt. Im Gegensatz zur allgemeinen Statistik für Kontakte werden hier nur die Daten eines Kontakts ausgewertet.
...
Zusätzlich zu den Statistiken zur Analyse deiner Unternehmensdaten verfügt plentymarkets über ein Tool zur Auswertung der Besucherzahlen deines Webshops. In den verschiedenen Bereichen des Webalizers erhältst du vielfältige Informationen zu den Webbesuchen deines Webshops, z.B. die Anzahl der Seitenaufrufe mit prozentualer Angabe und die Anzahl der Aufrufe einer Kategorie. Anders als die anderen hier beschriebenen Statistiken rufst du den Webalizer über das Menü *System » Systemeinstellungen » Mandant » Global » Statistik* auf.

[.instruction]
Besucherstatistik öffnen:

. Öffne das Menü *System » Systemeinstellungen » Mandant » Global » Statistik*. +
...
Statistiken werden im Menü *Start » Statistik* in Tabs gruppiert. Pro Tab können bis zu 4 Statistiken erstellt werden. Beachte, dass mit einem Klick auf das *X* des Tabs das Tab nicht geschlossen, sondern gelöscht wird.

Gehe wie unten beschrieben vor, um eine Statistik zu erstellen. <<tabelle-statistik-statistikansicht>> zeigt die verfügbaren Optionen in der Tab-Ansicht.

[.instruction]
Statistik erstellen:

. Öffne das Menü *Start » Statistik*.
. Wähle das gewünschte *Tab*.
. Klicke auf das grüne *Plus* eines der vier Felder des Tabs. +
...
*_Tipp:_* <<tabelle-statistik-statistikansicht>> erläutert die Statistikansicht.
...
====
Wenn zu einer Statistik keine Daten angezeigt werden, klicke auf den Stift und wähle einen größeren Zeitraum oder weitere Auftragsherkünfte, da nicht für alle Statistiken standardmäßig alle Herkünfte angezeigt werden.
====
...
. Öffne das Menü *Start » Statistik*.
. Wähle ein *Tab*.
. Klicke auf den *Stift* der Statistik, die du anpassen möchtest. +
→ Die Einstellungen der Statistik werden angezeigt.
. Nimm die Einstellungen vor. <<tabelle-statistik-bearbeiten>> erläutert die wichtigsten Einstellungsmöglichkeiten.
. *Speichere* die Einstellungen. +
...
. Klicke auf den *Stift* der Statistik, die du exportieren möchtest. +
