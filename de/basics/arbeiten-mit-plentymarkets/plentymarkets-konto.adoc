= plentymarkets Konto verwalten
:lang: de
include::{includedir}/_header.adoc[]
:position: 30

Im Menü **Start » Service-Center** finden Sie Informationen und Konfigurationsmöglichkeiten zu Ihrem plentymarkets System. Im Service-Center können Sie selbständig Änderungen vornehmen, z.B. Module buchen, Domains oder Subdomains bestellen oder Ihren Vertrag ändern.
...
. Klicken Sie auf **Zum Systemkonfigurator** oder auf den Vertrag, zu dem Sie wechseln möchten.
...
. Wählen Sie die Top-Level-Domain (TLD) aus der Dropdown-Liste, z.B. **de** oder **com**.
...
**__Tipp:__** MX steht für Mail Exchange Resource Record. Hierbei handelt es sich um die Parameter der <<basics/arbeiten-mit-plentymarkets/plentymarkets-konto#domain-mailserver, Mailserver>>, die für den Empfang und den Versand von E-Mails zur Verfügung stehen.
...
**MX30**; +
**MX40**
|Mailserver für den E-Mail-Empfang eintragen. Der Server MX10 hat höchste Priorität, d.h. E-Mails werden zuerst an den Mailserver MX10 geschickt. Erst wenn dieser Mailserver nicht verfügbar ist, werden E-Mails an Mailserver MX20 geschickt. Je niedriger der MX-Zahlenwert ist, desto höher die Priorität. +
...
Im Bereich *Systemverknüpfung* können Sie die Domain mit einem plentymarkets System verknüpfen bzw. eine Systemverknüpfung ändern. Pro plentymarkets Mandant können Sie, wenn Sie mehrere Domains eingerichtet haben, eine davon als Hauptdomain festlegen. Die URL wird dann auch bei den anderen Domains in der Adresszeile des Browsers angezeigt. Es kann bis zu 24 Stunden dauern, bis alle Einstellungen bzw. Änderungen auf allen Systemen zur Verfügung stehen.

[WARNING]
.Umstellung der Hauptdomain
====
Wenn Sie die Haupt-Domain umstellen, muss die URL auf allen Plattformen bzw. in den Einstellungen der Schnittstellen Ihres plentymarkets Systems, z.B. bei Zahlungsanbietern, angepasst werden. +
Speichern Sie außerdem im Menü <<auftragsabwicklung/payment/bankdaten-verwalten#70, System » Aufträge » Zahlung » EBICS>> die dort bereits hinterlegten Daten erneut, um die Grundeinstellungen zu aktualisieren.
====
...
. Tragen Sie ggf. die IP-Adresse (A-Record) des externen Servers ein.
...
CNAME bezeichnet eine Subdomain, die namensbasiert auf eine Domain bzw. Subdomain weitergeleitet wird. Sie können damit z.B. über den Anbieter **Shopgate** eine mobile Variante Ihrer Website zur Verfügung stellen oder die Nutzung eines externen E-Mail-Dienstes einrichten. Im ersten Beispiel in Bild 1 ist die Quelle _mobile.ihrshop.de_ die mobile Variante der Domain _ihrshop.de_. Als Ziel verwenden Sie die Shopgate-Subdomain _cname.shopgate.com_. Durch diesen CNAME-Eintrag wird gewährleistet, dass die Auflösung des Shops seitens Shopgate korrekt durchgeführt wird.
...
Speichern Sie z.B. einen Nameserver für Subdomains, um Newsletter2Go-Newsletter über eine link:https://www.newsletter2go.de/features/newsletter-versand-dedizierte-ips/[dedizierte IP-Adresse^] zu versenden und so die Zustellraten weiter zu optimieren. Wenn Sie diesen Service von Newsletter2Go gebucht haben, geben Sie als Ziel-Server den Newsletter2Go-Nameserver an. Die erforderlichen Informationen zu Subdomains und Nameserver erhalten Sie von Newsletter2Go.
...
Wenn Sie Ihren Webhosting-Provider wechseln möchten, wird das KK- bzw. Auth-Code-Verfahren (Auth-Info) durchgeführt. KK bedeutet Konnektivitäts-Koordination. Mit einem KK-Antrag wird der Provider-Wechsel geregelt. Inzwischen wird zunehmend das Auth-Info-Verfahren verwendet.
...
In der Regel ist ein Provider-Wechsel problemlos und kurzfristig möglich. Wenn es jedoch bei der Abwicklung zu Problemen kommt, z.B. weil Daten unvollständig oder unklar sind, kann der Wechsel vom alten Provider zunächst abgelehnt werden. Nehmen Sie in diesem Fall umgehend Kontakt zum alten Provider auf. Der aktuelle Provider kann den Wechsel erst freigeben, wenn keine Unklarheiten mehr bestehen.
...
====
Für gekündigte Domains sowie für Start-up- bzw. Testdomains, also Domains mit den Bestandteilen _plenty-testdrive.eu_, _plentymarkets-x1.com_ usw., können keine SSL-Zertifikate bestellt werden. Daher wird der Button **SSL-Zertifikat bestellen** für diese Domains nicht angezeigt.
====

=== Kostenpflichtige SSL-Zertifikate verwalten

Bei kostenpflichtigen SSL-Zertifikaten müssen Sie vor Ende der Laufzeit über Ihr plentymarkets Service-Center ein neues SSL-Zertifikat bestellen. Unabhängig davon, ob Sie bereits ein SSL-Zertifikat haben, bestellen Sie ein neues Zertifikat. Sie wählen also Zertifizierungsstelle und Laufzeit bei jeder Bestellung neu. Die Bestellung bei der Zertifizierungsstelle führt plentymarkets in Ihrem Auftrag durch und stellt Ihnen den Kaufbetrag in Rechnung. Es fallen keine monatlichen Gebühren für die Nutzung von SSL an.

==== Restlaufzeit eines kostenpflichtigen SSL-Zertifikats anzeigen

Im plentymarkets Service-Center können Sie im Tab <<basics/arbeiten-mit-plentymarkets/plentymarkets-konto#domain, Domain>> die Restlaufzeit Ihres kostenpflichtigen SSL-Zertifikats einsehen und bei Bedarf ein neues SSL-Zertifikat bestellen.
...
→ Die Restlaufzeit wird im Bereich **SSL-Zertifikat** in der Spalte *Gültigkeit Ende* angezeigt. +
**__Tipp:__** Das Zertifikat tritt nach der Bestellung in der Regel innerhalb von ca. 24 Stunden in Kraft. Sie können bei der Bestellung keinen späteren Gültigkeitsbeginn festlegen, d.h. die Gültigkeitsdauer beginnt, sobald die Zertifizierungsstelle die Bestellung bestätigt.

==== Kostenpflichtiges SSL-Zertifikat bestellen

Wenn Sie ein neues kostenpflichtiges SSL-Zertifikat bestellen, erhalten Sie während des Bestellvorgangs je eine E-Mail von plentymarkets und eine E-Mail von der SSL-Zertifizierungsstelle. Diese E-Mails werden an die E-Mail-Adresse hostmaster@IhreDomain versendet. Prüfen Sie daher vor der Bestellung, dass die  <<crm/e-mails-versenden#, E-Mail-Adresse>> hostmaster@IhreDomain eingerichtet ist. Wenn die E-Mail-Adresse hostmaster@IhreDomain noch nicht existiert, richten Sie die E-Mail-Adresse vor der Bestellung ein. Wenn die E-Mail-Adresse hostmaster@IhreDomain bereits vorhanden ist, prüfen Sie, ob für diese E-Mail-Adresse eine <<crm/e-mails-versenden#, Weiterleitung>> an die E-Mail-Adresse **support@plentymarkets.com** eingerichtet wurde, und deaktivieren Sie die Weiterleitung, damit Sie die E-Mails selbst empfangen. Gehen Sie dann wie unten beschrieben vor, um ein neues SSL-Zertifikat zu bestellen.
...
. Klicken Sie auf **SSL-Zertifikat bestellen**.
. Wenn das aktuelle SSL-Zertifikat noch mindestens 21 Tage gültig ist, erfolgt eine Sicherheitsabfrage. Bestätigen Sie diese Sicherheitsabfrage, wenn Sie schon jetzt ein neues Zertifikat bestellen möchten.
**__Hinweis:__** Sie können bei der Bestellung keinen späteren Gültigkeitsbeginn festlegen, d.h. die Gültigkeitsdauer beginnt, sobald die Zertifizierungsstelle die Bestellung bestätigt.
. Klicken Sie im Fenster **SSL-Zertifikat bestätigen** auf **Bestellung fortsetzen**. +
→ Sie erhalten eine E-Mail von plentymarkets an die E-Mail-Adresse **hostmaster@IhreDomain**.
. Lesen Sie die in der E-Mail enthaltenen Informationen durch.
. Klicken Sie auf den Link **SSL-Bestellung bestätigen**. +
→ Wenn Ihr aktuelles SSL-Zertifikat noch mindestens 21 Tage gültig ist, erfolgt erneut eine Sicherheitsabfrage. Bestätigen Sie diese Sicherheitsabfrage, wenn Sie schon jetzt ein neues Zertifikat bestellen möchten. +
→ Die verfügbaren SSL-Zertifizierungsstellen und Laufzeiten werden angezeigt.
. Klicken Sie bei dem gewünschten Angebot auf **Jetzt bestellen**. +
→ Die Zertifizierungsstelle sendet eine E-Mail an die E-Mail-Adresse **hostmaster@IhreDomain**.
. Lesen Sie die in der E-Mail enthaltenen Informationen.
. Klicken Sie auf den in der E-Mail angegebenen Link. +
→ Sie werden auf die Bestätigungsseite der Zertifizierungsstelle geleitet.
. Prüfen Sie die auf der Webseite angezeigten Informationen.
. Bestätigen Sie die Bestellung durch Klick auf den entsprechenden Button. +
→ Die Bestellung ist abgeschlossen. Wenn die Zertifizierungsstelle keine Qualitätskontrolle durchführt, werden die Daten zur Gültigkeit Ihres neuen SSL-Zertifikats nach ca. 10 Minuten im Service-Center-Tab **Domain** angezeigt.

[IMPORTANT]
.Qualitätskontrolle durch die Zertifizierungsstelle
====
In Einzelfällen führt die Zertifizierungsstelle eine Qualitätskontrolle durch, bevor SSL-Zertifikate freigeschaltet werden. Prüfen Sie daher die auf dem Bildschirm angezeigte Bestätigungsnachricht. Eine Qualitätskontrolle dauert in der Regel bis zu 24 Stunden. Sollte der Status im plentymarkets Service-Center nach 24 Stunden nicht auf "Done" stehen, erstellen Sie einen Beitrag im plentymarkets Forum unter Angabe Ihrer Domain.
====
...
**__Hinweis:__** Es kann bis zu 4 Stunden dauern, bis das SSL-Zertifikat deaktiviert bzw. aktiviert wird.

=== Kostenloses SSL-Zertifikat verwalten

Als Alternative zu kostenpflichtigen SSL-Zertifikaten bietet plentymarkets die Möglichkeit, SSL-Zertifikate der Zertifizierungsstelle link:https://letsencrypt.org[Let’s Encrypt^] zu nutzen. Ihr Verwaltungsaufwand ist bei einem Zertifikat von Let’s Encrypt deutlich geringer, da der Zertifizierungsprozess automatisiert und ohne manuelles Eingreifen durch die Zertifizierungsstelle abläuft. Die Zertifikate verlängern sich außerdem automatisch und sind monatlich kündbar. Gehen Sie wie unten beschrieben vor, um ein SSL-Zertifikat von Let’s Encrypt zu aktivieren.

[IMPORTANT]
.Bereitstellungsgebühr beachten
====
SSL-Zertifikats von Let’s Encrypt sind kostenlos. Für Sie fällt bei der Nutzung eines SSL-Zertifikats von Let’s Encrypt lediglich eine geringe monatliche Gebühr an, die plentymarkets für die Bereitstellung erhebt.
====
...
=== SSL-Zertifikate für externe Domains

Eine externe Domain können Sie beim externen Hoster belassen und einfach per A-Record (Weiterleitung der IP-Adresse) auf uns weiterleiten. Das SSL-Zertifikat muss jedoch systemnah liegen, d.h. auch wenn die Domain bei einem anderen Hoster liegt, erfolgt die Verschlüsselung erst nach der Weiterleitung über die IP-Adresse bei uns im System. Somit muss das SSL-Zertifikat bei uns gebucht werden, da wir aus Sicherheitsgründen keine externen Zertifikate zulassen.

== Mandanten (Shops) verwalten

<<omni-channel/online-shop#, Mandant (Shop)>> sind weitere Webshops, die über den zugeordneten Hauptshop in Ihrem plentymarkets System konfigurierbar sind. Im Menü **Start » Service-Center » Tab: Mandant (Shop)** werden die vorhandenen Mandanten (Shops) angezeigt.
...
→ Die verfügbaren Tabs werden eingeblendet.
. Wechseln Sie in das Tab **Vertrag**.
. Klicken auf **Zum Systemkonfigurator**. +
...
Im Tab **Mandant (Shop)** werden außerdem Informationen zu den Domains angezeigt, die Ihren Mandanten zugeordnet sind. Nehmen Sie Änderungen an diesen Einstellungen wie unter <<basics/arbeiten-mit-plentymarkets/plentymarkets-konto#domain, Domain>> vor.

[#cache]
== Cache verwalten

Um statische Inhalte Ihres Webshops, wie z.B. Artikelbilder, jederzeit möglichst schnell anzeigen zu können, werden diese Inhalte für eine gewisse Zeit im Cache vorgehalten. Aus diesem Grund sind Änderungen, z.B. eines Bildes, nach dem Upload nicht sofort sichtbar, sondern erst nach einer Verzögerung von bis zu einer Stunde. Beachten Sie auch unseren link:https://www.plentymarkets.eu/blog/Im-Rollout-Mehr-Geschwindigkeit-bei-durchgehender-SSL-Verschluesselung/b-1207/[Blog-Beitrag] zu diesem Thema.
...
* Tab **Tickets**: Die Dienstleister sehen den Inhalt, der auch Ihnen im Bereich **Mein Konto** unter **Tickets** angezeigt wird. Mitarbeiter können Tickets anlegen oder auf bestehende Tickets antworten.
...
Traffic entsteht, wenn Kunden Ihren Webshop besuchen und dort z.B. Artikel anklicken, Bilder vergrößern oder Dateien herunterladen.
...
**__Tipp:__** Je nach Art der Abfrage ist das Datenvolumen beträchtlich. Optimieren Sie daher Ihre Calls, indem Sie z.B. nicht ständig alle Bestände abfragen, sondern die Abfrage auf Änderungen beschränken.
...
**__Tipp:__**  Um dieses Verhalten von Bots zumindest teilweise zu steuern, können Sie die <<omni-channel/online-shop/cms-syntax#fallbeispiele-seo-guide, Sperrung von Bots>> per robots.txt-Datei veranlassen.
