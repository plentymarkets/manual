...
====
plentymarkets wird ständig weiterentwickelt und optimiert. Der Versionszyklus von plentymarkets umfasst die Entwicklungsstände *Beta*, *Early* und *Stable*. Standardmäßig wird Ihr System in der Version *Stable* bereitgestellt. Wenn Sie jedoch früher von Verbesserungen und neuen Features profitieren möchten, haben Sie die Möglichkeit, Ihr System auf einen <<basics/versionswechsel/versionszyklus, früheren Entwicklungsstand umstellen>>. Mit welcher plentymarkets Version Sie arbeiten, wird im plentymarkets Backend angezeigt, wenn Sie mit der Maus auf das Login-Symbol zeigen.
...
Eine externe Domain können Sie beim externen Hoster belassen und <<basics/arbeiten-mit-plentymarkets/plentymarkets-konto#150, die DNS-Einstellungen bei diesem externen Hoster speichern>>. Das SSL-Zertifikat muss jedoch systemnah liegen, also auch wenn die Domain bei einem anderen Hoster liegt, erfolgt die Verschlüsselung erst nach der Weiterleitung über die IP-Adresse bei plentymarkets im System. Daher muss das SSL-Zertifikat bei plentymarkets gebucht werden, da plentymarkets aus Sicherheitsgründen keine externen Zertifikate zulässt.
...
Eine weitere Ursache ist das erstmalige Hinzufügen einer Domain zu einem Mandanten. Diese Mandanten werden nicht automatisch auf Autoscaling umgestellt, sondern müssen von plentymarkets im Nachhinein umgestellt werden.
...
. Prüfen Sie in der Spalte *DNS Record Type*, ob ein CNAME-Eintrag vorhanden ist. +
→ Wenn ein CNAME-Eintrag vorhanden ist, ist das Autoscaling aktiviert. +
→ Wenn kein CNAME-Eintrag vorhanden ist, sondern zwei A-Records, ist das Autoscaling nicht aktiviert.
...
→ Prüfen Sie die Einstellungen im plentymarkets Service-Center. +
→ Wenn die Parent-Domain aus dem Service-Center entfernt wurde, legen Sie die Parent-Domain neu an. Wenn diese Parent-Domain nicht auf plentymarkets zeigen soll, sondern z.B. auf Shopware, versehen Sie die Parent-Domain mit einem externen A-Record, indem Sie eine <<basics/arbeiten-mit-plentymarkets/plentymarkets-konto#210a, Systemverknüpfung erstellen>>.
...
.Domain angelegt?
====
Um einen Mandanten (Shop) zu bestellen, benötigen Sie eine <<basics/arbeiten-mit-plentymarkets/plentymarkets-konto#domain, Domain>>. Diese Domain müssen Sie vorher im Tab **Domain** für das Hauptsystem erstellen.
====
...
*_Tipp:_*  Um dieses Verhalten von Bots zumindest teilweise zu steuern, können Sie die <<omni-channel/online-shop/webshop-einrichten/cms-syntax#fallbeispiele-seo-guide, Sperrung von Bots>> per robots.txt-Datei veranlassen.
