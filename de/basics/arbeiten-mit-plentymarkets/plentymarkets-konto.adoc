...
* Domain
  ** Die Domain, für die das SSL-Zertifikat bestellt werden soll, muss eine im Service-Center aktivierte <<basics/arbeiten-mit-plentymarkets#210a, Hauptdomain>> sein.
* Autoscaling
  ** Autoscaling muss aktiviert sein. +
  Vereinzelt gibt es noch Systeme, die noch nicht auf Autoscaling (AS) umgestellt wurden. In den meisten Fällen liegt das daran, dass die Domain nicht bei Plentymarkets gehostet wird und aus diesem Grund ein manuelles Eingreifen in die DNS-Einstellungen der Domain durch den Domain-Inhaber notwendig ist. Eine weitere Ursache ist das erstmalige Hinzufügen einer Domain an einen Mandanten. Diese werden nicht automatisch auf AS umgestellt, sondern müssen von uns im Nachhinein umgestellt werden. +
  *_Tipp:_* Ob für das jeweilige System AS aktiviert ist oder nicht, erkennt man anhand der DNS-Einstellungen im Service-Center. Systeme, die auf AS umgestellt wurden, haben immer einen CNAME-Record. Wenn im Service-Center kein CNAME-Record dargestellt wird, sondern stattdessen zwei A-Records, ist das System nicht auf AS umgestellt.
* DNS-Einstellungen
  ** DNS-Einstellungen gemäß der Angaben im Service-Center
  Die Validierung der SSL-Bestellung erfolgt auf Dateiebene.
  Das bedeutet, dass die Domain, für die das Zertifikat bestellt werden möchte zwingend korrekt erreichbar sein muss. Dazu müssen die DNS-Einstellungen gem. den Angaben im Service-Center entsprechen.
  Siehe dazu https://forum.plentymarkets.com/t/selbsthilfe-bei-dns-problemen-wie-hinterlege-ich-korrekte-dns-einstellungen/65559
  ...
  ** Kein negativer CAA-Record
  Für die Domain, für die das SSL-Zertifikat bestellt werden soll, muss sichergestellt werden, dass evtl. vorhandene CAA-Records](https://de.wikipedia.org/wiki/DNS_Certification_Authority_Authorization) die Bestellung von COMODO-Zertifikaten zulässt.
...
. Öffnen Sie das Menü *System » Mandant » Mandant wählen » SSL*. +
...
. Löschen Sie bei Bedarf den DNS-Cache.
...
Eine externe Domain können Sie beim externen Hoster belassen und einfach per A-Record (Weiterleitung der IP-Adresse) auf uns weiterleiten. Das SSL-Zertifikat muss jedoch systemnah liegen, d.h. auch wenn die Domain bei einem anderen Hoster liegt, erfolgt die Verschlüsselung erst nach der Weiterleitung über die IP-Adresse bei uns im System. Daher muss das SSL-Zertifikat bei uns gebucht werden, da wir aus Sicherheitsgründen keine externen Zertifikate zulassen.
