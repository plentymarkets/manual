...
Arbeiten Sie die Checkliste durch, um die zuvor beschriebenen Einstellungen zum Schnelleinstieg zu prüfen. Diese Kontrolle gewährleistet die korrekte Funktionsweise Ihres Systems.
...
Beim <<artikel/artikel-verwalten#950, Cross-Selling>> bieten Sie Ihren Kunden zur aktuellen Auswahl weitere Artikel aus Ihrem Sortiment an, die eine notwendige Erweiterung oder sinnvolle Ergänzung darstellen. Für eine Taschenlampe werden die passenden Batterien benötigt, eine Digitalkamera ist erst mit einer ausreichend dimensionierten Speicherkarte sinnvoll und wer einen hochwertigen Tennisschläger kauft, investiert bestimmt noch in eine schöne Tasche zur Aufbewahrung. Wenn Sie auch bei Ihren Artikeln solche naheliegenden Verbindungen finden, können Sie diese Zusammenhänge mit der Option *Cross-Selling* einrichten und damit lukrative Zusatzverkäufe generieren.
...
Um gekaufte, nicht installierte Plugins in der Plugin Übersicht anzuzeigen, wählen Sie ein *Plugin-Set* und wählen in der *Installiert* Dropdown-Liste die Option *Alle* oder *Nicht installiert*. Klicken Sie den *Suchen*-Button, um alle gekauften, nicht installierten Plugins anzeigen zu lassen.

[#545]
=== Plugin-Sets erstellen

Ein *Plugin-Set* ist eine Gruppierung von verschiedenen Plugins, über die Plugins für einen oder mehreren Mandanten verwaltet werden können.
...
Weitere Informationen zum Verwalten von *Plugin-Sets* finden Sie <<basics/erste-schritte/plugins#20, hier>>.

[#550]
=== Plugins installieren

Nachdem Sie die Plugins im plentyMarketplace gekauft haben, sind die Plugins im Menü *Plugins » Plugin Übersicht* verfügbar und können dort installiert werden.
...
. Wählen Sie die Option *Alle* aus den Dropdown-Listen *Herkunft*, *Installiert* und *Aktiv* im Suchfilter.
. Klicken Sie den *Suchen*-Button. +
→ Alle im plentyMarketplace erworbenen Plugins werden hier angezeigt.
. Klicken Sie in der Zeile des Plugins *IO* auf die Aktion *Plugin installieren*. +
...
Wiederholen Sie die Schritte 5 bis 7 für die Plugins *Ceres* und *Vorkasse*.
