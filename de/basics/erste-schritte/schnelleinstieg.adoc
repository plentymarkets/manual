...
// TODO: Ggf. dieses Unterkapitel entfernen, da der Stage-Modus sonst nicht aufgerufen werden kann.
...
Kategorien lassen sich in der Navigationsleiste mithilfe der Position sortieren. Neue Kategorien erhalten immer die Position 0.
...
* Die <<#590, Cross-Selling-Liste>> muss mit dem passenden Container in Ceres verknüpft sein.
...
Sie haben den Artikelkatalog aufgebaut und die Kategorien eingerichtet. Vor dem endgültigen Start sind nur noch wenige Einstellungen erforderlich. Erfahren Sie, wie Sie die benötigten Webshop-Plugins kaufen und installieren sowie Ihren Webshop einrichten.
...
Beginnen Sie mit der Einrichtung des Plugins **IO**. Zum Beispiel werden hier die Nachkommastellen für Maße und Gewichte festgelegt. In der Grundeinstellung sind übliche Standardwerte eingegeben. In der Konfiguration von IO würden Sie zudem ein anderes Template-Plugin angeben, wenn Sie nicht mit Ceres arbeiten.
...
. Wechseln Sie in das Tab *Number formats*.
...
Globale Einstellungen, die sich auf den gesamten Webshop auswirken, nehmen Sie im Tab *Global* der Konfiguration des Plugins *Ceres* vor. Speichern Sie zunächst den Namen des Webshops und ändern Sie das Firmenlogo. Zudem richten Sie im Tab *Global* die <<#650, Kontaktseite>> ein.
...
. Geben Sie unter *Name of your store* den Namen des Webshops ein. +
→ Der Name wird als Seitentitel für den Webshop verwendet.
. Geben Sie unter *URL to your company logo* die URL des Firmenlogos ein. +
...
. Wählen Sie unter *Cross-selling: Cross-selling relation* die Beziehung für Cross-Selling-Artikel.
. Wählen Sie unter *First item list* die Option *Cross-selling* aus dem Dropdown-Menü. +
...
Aktivieren Sie die gewünschten Formularfelder und die zugehörige Validierung der Felder analog für das Lieferland *Vereinigtes Königreich* (UK).

[#610]
===== Kundenbereich einrichten

Passen Sie zudem den *Mein Konto*-Bereich für Ihre Kunden im Tab *Checkout and My account* an. Zusätzlich richten Sie hier auch <<omni-channel/online-shop/ceres-einrichten#430, Retouren>> für Ihren Webshop ein.
...
// TODO: User Guide des Plugins in übersichtlicher Form einfügen.
...
[%interactive]
* [ ] Haben Sie Ihre <<#570, Startseite>> ansprechend gestaltet?
* [ ] Stimmen die angegebenen <<#640, Versandinformationen und -kosten>> mit Ihren Einstellungen überein?
* [ ] Sind alle erforderlichen Angaben im <<#630, Impressum und in der Datenschutzerklärung>> enthalten?
