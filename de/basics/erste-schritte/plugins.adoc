...
Das Menü **Plugins » Plugin-Übersicht** zeigt eine Übersicht der Plugins in der plentymarkets Inbox. Wählen Sie in der oberen Leiste aus der Dropdown-Liste den Mandanten, für den die Plugin-Konfigurationen mandantenabhängig angezeigt werden sollen. In der Kartenansicht werden die Plugins in Form von Karten dargestellt. Eine Plugin-Karte enthält folgende Informationen:
...
Alternativ zur Kartenansicht können Sie zur Tabellenansicht wechseln. <<tabelle-plugin-uebersicht>> erläutert die in den Spalten der Tabellenansicht gezeigten Informationen.
...
. Klicken Sie in der Zeile des Plugins auf *Position festlegen*. +
→ Das Fenster *Plugin-Position* wird geöffnet.
. Geben Sie eine neue Positionsnummer ein.
. *Speichern* Sie die Einstellungen. +
→ Das Plugin wird in der Liste der Plugins neu angeordnet.
...
Plugins werden von der Inbox des Menüs **Plugins » Plugin-Übersicht** aus in Stage bzw. Productive getrennt bereitgestellt. Dabei entspricht Stage einem Vorschaumodus. Der Stage-Modus ist nur für den Hauptmandanten verfügbar. Bei der Bereitstellung in Stage werden alle Plugins bereitgestellt, die für den Modus Stage aktiviert wurden. Bei der Bereitstellung in Productive werden alle Plugins bereitgestellt, die für den Modus Productive aktiviert wurden. Plugins, die in Productive bereitgestellt werden, sind ggf. für Ihre Kunden und Anwender sichtbar. <<tabelle-stage-vs-productive>> verdeutlicht das Konzept von Stage und Productive.

[[tabelle-stage-vs-productive]]
.Stage vs. Productive
[cols="a,a"]
|====
|Modus |Erläuterung
...
Gehen Sie wie unten beschrieben vor, um die Plugins bereitzustellen. <<tabelle-bereitstellung-menueleiste>> erläutert die erforderlichen Optionen der Menüleiste.
...
. Wählen Sie den gewünschten Mandanten aus der Dropdown-Liste.
. Öffnen Sie das Menü **Plugins » Plugin-Übersicht**.
...
[[tabelle-bereitstellung-menueleiste]]
...
|Aktivieren, um Plugins bei Aktualisierungen über link:plentyDrive[https://marketplace.plentymarkets.com/plugins/integration/plentyDrive_5091^] automatisch in **Stage** bereitzustellen. Plugins müssen bereits in **Stage** aktiv sein.
|====
...
. Wählen Sie die Version aus der Dropdown-Liste.
. Klicken Sie auf **Plugin aktualisieren**. +
