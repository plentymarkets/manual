...
* Mit Klassen verteilen Sie Berechtigungen an Benutzerkonten und Schnittstellen. Einem Benutzerkonto der Klasse REST-API zum Beispiel weisen Sie individuell REST-Calls zu.
...
*_Hinweis_*: Diese Klasse existiert nicht mehr und wurde zu einem Backend-Nutzer migriert.

|**REST-API**
|Klasse, für die im Benutzerkonto <<basics/admin-aufgaben/benutzer-verwalten#120, REST-Calls>> freigeschaltet werden. Die REST-Calls werden dann einzeln aktiviert. Es werden dem Benutzerkonto also nicht pauschal alle REST-Calls freigeschaltet. +
...
|**Benutzerklasse**
|Benutzerklasse des Benutzerkontos wählen. Es stehen die Klassen **Admin**, **Backend** und **REST-API** zur Verfügung.
...
[#105]
== Rechte
Mithilfe von Rechten legen Sie fest, welche Bereiche und Einstellungen Personen mit einem Benutzerkonto in plentymarkets sehen und was die Person bearbeiten kann. Sie können Rechte für jedes Benutzerkonto einzeln festlegen oder mithilfe von Rollen für mehrere Benutzerkonten.
...
Im Tab **Berechtigung** legen Sie fest, welche Berechtigungen ein Benutzerkonto der Klasse **Backend** für die REST-API erhält. In der Standardeinstellung sind keine REST-API-Berechtigungen vergeben. Benutzerkonten der Klasse **Admin** verfügen über alle Berechtigungen.
...
Filter helfen Ihnen bei der Suche nach Benutzerkonten. Mit dem Filter **Klasse** zeigen Sie beispielsweise nur Benutzerkonten der Klasse **Backend** an. Ohne Filter werden in der Übersicht alle Benutzerkonten angezeigt. Folgende Elemente und Funktionen sind im Menü **Benutzer** verfügbar.
