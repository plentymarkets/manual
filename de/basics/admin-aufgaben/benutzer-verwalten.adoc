...
<ol>
<li>
Öffnen Sie das Menü <strong>System » Systemeinstellungen » Einstellungen » Benutzer » Konten</strong>.</li>
<li>
...
Wurde das Passwort beim Login mehrmals hintereinander falsch eingegeben, wird das Benutzerkonto für das Login eine gewisse Zeit gesperrt. Über den Button **Login entsperren** im Menü **System » Systemeinstellungen » Einstellungen » Benutzer » Konten » Benutzer öffnen** kann der Login durch einen Administrator frühzeitig entsperrt werden.
...
[#160]
== Anzahl gleichzeitig eingeloggter Benutzer im Backend

Abhängig von Ihrem plentymarkets Tarif ist die Anzahl der gleichzeitig im plentymarkets Backend eingeloggten Benutzerkonten möglicherweise beschränkt. Das können Sie tun, wenn Sie das Benuzterlimit überschreiten:

* Benutzer ausloggen
* Benutzerlimit erhöhen

[#170]
=== Laufende Sessions beenden, um Benutzer auszuloggen

Wenn die zulässige Anzahl gleichzeitig eingeloggter Benutzerkonten erreicht oder überschritten wird, wird eine Fehlermeldung eingeblendet. Über einen Link in der Fehlermeldung werden Admin-Benutzer direkt zur Session-Verwaltung weitergeleitet. So können Admin-Benutzer eine oder mehrere laufende Sessions beenden, damit  sich ein anderer Benutzer einloggen kann. Der Login ist für 10 Minuten gültig. Danach wird der Admin-Benutzer automatisch ausgeloggt.

[.instruction]
Laufende Sessions von Benutzern beenden:
...
→ Die eingeloggten Benutzer werden angezeigt.
. Aktivieren Sie die Kontrollkästchen der Benutzer, deren Session Sie beenden möchten, um sie auszuloggen.
. Klicken Sie auf *Ausgewählte Sessions beenden*. +
→ Die gewählten Benutzer werden ausgeloggt.
