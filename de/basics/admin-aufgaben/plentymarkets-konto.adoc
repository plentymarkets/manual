...
////
//braucht man das noch überhaupt? Weil es gibt ja diese Seite dafür. Der Link hier im Info-Box funktioniert übrigens nicht.
https://knowledge.plentymarkets.com/basics/admin-aufgaben/versionszyklus
[TIP]
...
plentymarkets enthält einen <<basics/assistenten#, Assistenten>>, der dir bei der Verwaltung deiner Domains unterstützt.
...
<<tabelle-konto-domains>> enthält grundlegende Erläuterungen zu den verschiedenen Domain-Typen.
...
. Wähle den gewünschten Domain-Typ aus.
Je nachdem, welchen Domain-Typ du wählst, ändern sich die nachfolgenden Schritte und Einstellungen des Assistenten.
...
//a|* <<Schritt: Domain benennen>>
//** Domain
//** Top Level Domain
//** Kostenpflichtig bestellen
//* Schritt: Auswahl des Mandanten
//* Schritt: Domain-Handle
...
Im ersten Beispiel in Bild 1 ist die Quelle _mobile.ihrshop.de_ die mobile Variante der Domain _ihrshop.de_. Als Ziel verwendest du die Shopgate-Subdomain _cname.shopgate.com_. Durch diesen CNAME-Eintrag wird gewährleistet, dass die Auflösung des Shops seitens Shopgate korrekt durchgeführt wird.
...
//Gehe wie unten beschrieben vor, um Weiterleitungen einzurichten.
...
[[tabelle-weiterleitung-aufbau]]
.Eine Umleitung besteht aus vier Angaben
[cols="1,3"]
|====
|Angabe |Erläuterung
...
[[tabelle-weiterleitung-beispiele]]
...
== Domains im Service-Center verwalten
////
[#70]
...
. Öffne das Menü *plentmarkets Logo (Start) » Service-Center*. +
*_Tipp:_* Zeige dazu mit der Maus auf das plentymarkets Logo oben links. +
→ Das Service-Center wird in einem neuen Browser-Tab geöffnet.
. Klicke auf das blaue *Plus* des Systems. +
→ Die verfügbaren Tabs werden eingeblendet.
...
. Gib die gewünschten Weiterleitungen in das vorgesehene Feld ein. Pro Zeile kann eine Umleitung definiert werden. Beachte bei der Eingabe die nachfolgende <<basics/admin-aufgaben/plentymarkets-konto#265, Informationen zum Syntax>>.
...
<<webshop#, Mandant (Shop)>> sind weitere Webshops, die über den zugeordneten Hauptshop in deinem plentymarkets System konfigurierbar sind. Im Menü *plentymarkets Logo (Start) » Service-Center » Tab: Mandant (Shop)* werden die vorhandenen Mandanten (Shops) angezeigt.
...
Du kannst einen Mandanten (Shop) kündigen, indem du im Menü *plentymarkets Logo (Start) » Service-Center » Tab: Mandant (Shop)* auf *Mandant (Shop) kündigen* klickst. Bestätige im angezeigten Fenster deine Kündigung. Vertragsbeginn und -ende werden angezeigt.
...
Im Menü *plentymarkets Logo (Start) » Service-Center » Tab: Ihre Agentur* gibst du bis zu 2 von plentymarkets zertifizierten Dienstleistern Zugriff auf bestimmte Bereiche des Systems. Du ermöglichst damit einer Design-Agentur und/oder einem Consultant folgende Arbeiten in deinem plentymarkets System:
...
Die Verbrauchsdaten werden im Menü *plentymarkets Logo (Start) » Hosting » Tab: Verbrauch* angezeigt. <<tabelle-konto-verbrauchsdaten>> bietet einen Überblick der in diesem Menü angezeigten Daten zu deinem Verbrauch.
