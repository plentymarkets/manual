...
Willst du neue Domains oder Subdomains einrichten? Musst du ein neues SSL-Zertifikat bestellen? Soll der Cache vorübergehend deaktiviert werden? Alle diese Themen werden auf dieser Handbuchseite beschrieben.
...
.*Weitere Informationen über die einzelne Optionen*
--
Welche Einstellungen hier angezeigt werden, hängt davon ab, welche <<basics/admin-aufgaben/plentymarkets-konto#70, Domain-Typ>> du im vorherigen Schritt ausgewählt hast.
...
→  Die DNS-Einstellungen werden angezeigt. Diese Einstellungen werden in  <<table-account-dns-settings>> erläutert.
...
. Klicke auf _eine der zuvor abgeschlossenen Domains_, um den Assistenten erneut zu öffnen.
. Klicke im Navigationsbaum links auf den Schritt *Aktuelle DNS-Einstellungen*. +
→  Die DNS-Einstellungen werden angezeigt. Diese Einstellungen werden in <<table-account-dns-settings-2>> erläutert.
