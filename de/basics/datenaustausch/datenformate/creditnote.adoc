...
Das Datenformat **CreditNote** wird verwendet, um Gutschriften, z.B. für statistische Zwecke, zu exportieren. Dieses Datenformat steht nur zum Export zur Verfügung. Sie erstellen und exportieren das Datenformat im Menü **Daten » Dynamischer Export**. Wie Sie das Datenformat exportieren, ist auf der Handbuchseite <<basics/datenaustausch/daten-exportieren#, Daten exportieren>> beschrieben.
