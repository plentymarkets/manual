...
Im Menü *Daten » Dynamischer Export* erstellen Sie neue Datenformate, die genau die Datenfelder enthalten, die Sie exportieren möchten. Die Datenfelder sind zwar vorgegeben, aber Sie haben im Gegensatz zu den <<basics/datenaustausch/export-import/daten-exportieren#10, vorkonfigurierten Datenformaten>> mehr Möglichkeiten zu bestimmen, welche Felder exportiert werden. Die Daten werden in eine Datei exportiert, die Sie dann einem Tool oder einer Schnittstelle, z.B. einem Marktplatz, zur Verfügung stellen. Außerdem bearbeiten Sie in diesem Menü bestehende Datenformate.
