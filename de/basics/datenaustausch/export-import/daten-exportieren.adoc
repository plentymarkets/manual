...
Im Menü *Daten » Dynamischer Export* erstellen Sie neue Datenformate, die genau die Datenfelder enthalten, die Sie exportieren möchten. Die Datenfelder sind zwar vorgegeben, Sie haben aber im Gegensatz zu den <<basics/datenaustausch/export-import/daten-exportieren#10, vorkonfigurierten Datenformaten>> mehr Möglichkeiten zu bestimmen, welche Felder exportiert werden. Die Daten werden in eine Datei exportiert, die Sie dann einem Tool oder einer Schnittstelle, z.B. einem Marktplatz, zur Verfügung stellen. Außerdem bearbeiten Sie in diesem Menü bestehende Datenformate.
...
Dieses Kapitel beschreibt, wie Sie neue Exportformate erstellen und welche Einstellungen Sie vornehmen können. Bis auf BolOfferExport und BolItemExport müssen alle weiteren Formate zunächst aus dem link:https://marketplace.plentymarkets.com//[plentyMarketplace^] installiert werden, bevor die Formate im elastischen Export zur Verfügung stehen.
...
.  Nehmen Sie die Einstellungen vor. Beachten Sie dazu die Erläuterungen in <<tabelle-neuer-export>>.
. *Speichern* Sie die Einstellungen. +
→ Eine ID für das Exportformat wird vergeben und das Exportformat erscheint in der Übersicht *Exporte*.

[[tabelle-neuer-export]]
...
.  Ändern Sie die Einstellungen. Beachten Sie dazu die Erläuterungen in <<tabelle-neuer-export>> und <<tabelle-symbolleiste-exportformat>>.
. *Speichern* Sie die Einstellungen.
...
[[tabelle-symbolleiste-exportformat]]
.Symbolleiste eines Exportformats
[cols="1,3"]
|====
| Symbol | Erläuterung
...
.  Öffnen Sie das Menü *Daten » Elastischer Export*.
.  Nehmen Sie die Einstellungen im Bereich *Filter* vor. Beachten Sie dazu die Erläuterungen in <<tabelle-filtereinstellungen-elastischer-export>>.
.  Klicken Sie auf *Suchen*.

[[tabelle-filtereinstellungen-elastischer-export]]
