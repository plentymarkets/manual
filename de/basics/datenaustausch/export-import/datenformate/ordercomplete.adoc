...
Mit dem Datenformat **OrderComplete** exportieren Sie Aufträge. Dieses Datenformat steht ausschließlich für den Export zur Verfügung. Sie erstellen und exportieren das Datenformat im Menü **Daten » Dynamischer Export**. Wie Sie das Datenformat exportieren, ist auf der Handbuchseite <<basics/datenaustausch/export-import/daten-exportieren#, Daten exportieren>> beschrieben.
...
|Debitorenkonto im Menü <<auftragsabwicklung/auftraege/buchhaltung#, Buchhaltung>>
|
...
Die IDs der Länder werden im Menü **System » Aufträge » Versand » Optionen » Tab: Lieferländer** angezeigt.
|
...
|**OrderValuta**
|Valuta ist der Zeitraum ab Rechnungserstellung, bevor der Zeitraum Zahlungsziel beginnt. Siehe Handbuchseite <<crm/kundendaten-verwalten#800, Kundendaten verwalten>>.
|
...
Erst nachdem Sie das Datenformat gespeichert haben, können Sie einen Barcode wählen. Den Barcode müssen Sie vor jedem Export wählen.
|
...
Nachdem Sie ein Datenformat vom Typ *OrderComplete* gespeichert haben, können Sie wählen, ob Sie auch gelöschte Aufträge exportieren möchten oder nicht. Wenn Sie Barcode als Datenfeld exportieren, können Sie festlegen welcher Barcode exportiert wird. +
...
Im Tab **Aktionen** wählen Sie, was beim Datenexport ausgeführt werden soll. Aktivieren Sie die Aktion **OrderStatus** und wählen Sie einen Auftragsstatus aus der Dropdown-Liste. Den Aufträgen wird dann beim Export dieser Auftragsstatus hinzugefügt.
