...
*FTP* = Host,Port, Benutzername, Passwort und Dateiname werden als Einstellungen sichtbar
    ...
    * Port: Port des FTP-Servers. Wenn kein Port eingetragen ist wird der Standard-Port verwendet.
...
==== Sync-Übersicht einsehen

Die Sync-Übersicht erreichen Sie über das Menü *Daten » ElasticSync*. Hier sehen Sie eine Übersicht der bereits angelegten Syncs. In der Spalte "Last run" können Sie bei einem eingestellen Intervall das Datum und die Uhrzeit der Imports einsehen.

===== Sync-Logs einsehen

In der Sync-Übersicht können Sie die Logs Ihrer ausgeführten Syncs einsehen. Die Sync-Logs erreichen Sie in der Sync-Übersicht *Daten » ElasticSync* über den Button "Logs".

===== Sync kopieren

Wählen Sie einen oder mehrere Syncs aus und klicken Sie auf den "Sync kopieren" button. Es wird eine kopie der ausgewählten Syncs angelegt. Dabei wird an den Namen des Syncs in Klammern stehend "copy" angefügt.

===== Sync exportieren/importieren

Wählen Sie einen oder mehrere Syncs aus und klicken Sie auf den "Sync exportieren" Button. Der download wird automatisch gestartet. Über den Button "Sync importieren" können Sie Syncs in ElasticSync importieren.

[NOTE]
.Syncs aus einem anderen plentymarkets System importieren
====
Wenn Sie Syncs aus einem anderen plentymarkets System importiert, müssen Sie eine Anpassung Ihres Abgleichs und der Zuordnung vornehmen.
====

===== Sync testen und Daten importieren

In der Sync-Übersicht können Sie Syncs testen und ausführen. Die Sync-Übersicht erreichen Sie über das Menü *Daten » ElasticSync*. Wenn Sie "Sync testen" wählen, werden nur die ersten zehn Zeilen der Importdatei importiert. Wenn Sie "Sync ausführen" wählen wird die Importdatei vollständig importiert.

=== REST-API & ElasticSync

*Sync anlegen: POST /rest/elasticsync/sync/{syncId}/import
*Alles Syncs abrufen: GET - /rest/elasticsync/all_syncs
