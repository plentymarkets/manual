...
Der Eintrag *letzter Import* zeigt Ihnen, an welche Datum und Uhrzeit der Sync zuletzt ausgeführt wurde.

[.instruction]
. Öffnen Sie das Menü *Daten » ElasticSync*.
. In der Tabelle sehen sie in der vorletzten Zeile den Eintrag *letzter Import*.
...
Syncs können beliebig kopiert werden

[.instruction]
. Öffnen Sie das Menü *Daten » ElasticSync*.
. Makieren Sie ein oder mehrere Syncs.
...
Syncs können importiert und exportiert werden.
...
→ Das Dateibrowser wird geöffnet.
. Klicken Sie auf *Datei hochladen* und wählen Sie die Datei aus.
. Makieren Sie die gewünschte Datei im Dateibrowser und klicken Sie auf *wählen*. +
→ Die Datei wird hinzugefügt.
. Klicken Sie auf *Import*. +
→ Der Sync wurde importiert.

[NOTE]
.Syncs aus einem anderen plentymarkets System importieren
====
Wenn Sie Syncs aus einem anderen plentymarkets System importiert, müssen Sie eine Anpassung Ihres Abgleichs und der Zuordnung vornehmen. Es können nur *.json* Dateien importiert werden.
====

[#1700]
==== Sync testen und Daten importieren

Sie können einen Sync testen und ausführen.

[.instruction]
. Öffnen Sie das Menü *Daten » ElasticSync*. +
→ In der Tabelle finden Sie in der letzten Zeile die Funktionen *Sync ausführen* und *Sync testen*.
...
ElasticSync kann per REST-API angesprochen werden. Folgen Rest calls können Sie verwenden:
