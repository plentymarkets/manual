...
=== Verwendung von Platzhaltern in Layout-Vorlagen

Wenn Sie über Ihren eBay-Shop Artikel außerhalb von Deutschland verkaufen und in link:https://knowledge.plentymarkets.com/omni-channel/multi-channel/ebay/ebay-einrichten#2800[Layout-Vorlagen^], die mit Listings verknüpft sind, Platzhalter für Artikelmerkmale verwenden, müssen Sie vor dem Wechsel auf plentymarkets 7 Übersetzungen in den Landessprachen hinterlegen. Beachten Sie, dass Sie für jedes Land, in dem Sie Ihre Artikel zum Verkauf anbieten, Übersetzungen hinterlegen. Andernfalls wird das Listing fehlerhaft in Ihrem eBay-Shop angezeigt.

Übersetzungen für den Merkmalnamen und den Merkmalwert sind erforderlich, wenn Sie die Platzhalter *{% GetItemPropertyByName() %}* und *{% GetItemPropertyById() %}* in Layout-Vorlagen verwenden.
