...
Der Download der Payment Plugins erfolgt im link:https://marketplace.plentymarkets.com/plugins/payment[plentyMarketplace^]{nbsp}icon:external-link[]. Auf der Handbuchseite <<payment/payment-plugins#, Payment Plugins>> findest du Informationen sowie Anleitungen zum Einrichten für die von plentymarkets entwickelten Payment Plugins. Dort findest du auch die Information, ob die Einrichtung des jeweiligen Plugins mit einem Assistenten stattfindet. +
