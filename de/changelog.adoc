...
+
Um OTTO market nutzen zu können, musst du von OTTO als Händler freigeschaltet werden.
+
OTTO Market befindet sich aktuell in der Betaphase. Wenn OTTO dich als Betatester kontaktiert hat, kannst du mit der Einrichtung beginnen. Bei Fragen wende dich bitte direkt an OTTO.
+
Solltest du nicht an der Betaphase von OTTO market teilnehmen, bitten wir noch um etwas Geduld. Nach Abschluss der Betaphase werden wir euch mit weiteren Informationen zur Integration von OTTO market versorgen.
+
In plentymarkets wird OTTO market im Menü *Einrichtung » Assistenten » Omni-Channel* mit den Assistenten *OTTO market Grundeinstellungen* und *OTTO market Kataloge* erstellen eingerichtet. Weitere Informationen findest du auf der <<otto/otto-market#, OTTO market-Handbuchseite>>.
