...
Wie das allgemeine Vorgehen beim Importieren und Exportieren von Daten ist, wird im Handbuch unter <<basics/datenaustausch/daten-exportieren#, Daten exportieren>> und <<basics/datenaustausch/daten-importieren#, Daten importieren>> erläutert.
Welche Datenformate noch so zur Verfügung stehen, findest du <<basics/datenaustausch/datenformate#, hier>> ebenfalls.