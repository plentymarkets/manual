...
Formatiere UI-Texte und Menüpfade *fett*. Schreibe die Texte auf Buttons etc. genau so, wie du sie in der UI siehst.
...
* << öffnet den Link.
* Dann folgt der Pfad *order/unterordner/seite#anker*.
...
Ähnlich der Links nutzt du Anker, um zu einer bestimmten Stelle auf der Seite zu springen. Lege dafür das Ziel auf der Seite in eckigen Klammern `[#Ziel]` fest und springe dann mit einem <<#Ziel, internen Link>> an dieses Ziel. Der Anker muss eindeutig sein.
