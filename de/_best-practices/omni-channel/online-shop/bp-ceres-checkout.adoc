...
. Ersetze den Code durch: `{% if( $CustomerID ) { $_check = Link_Checkout(3); $_backlink = ''; } else { $_check = Link_Checkout(2); $_backlink = '?backlink={%if $Lang !== 'de' %}$Lang{% endif %}/checkout'; } %} $_check.$_backlink`
. *Speichere* die Einstellungen.
