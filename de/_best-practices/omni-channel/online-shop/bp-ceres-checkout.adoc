...
Du willst den neuen Checkout mit seiner Plugin-Fähigkeit und individuellen Anpassbarkeit in deinem Callisto-Webshop nutzen? Kein Problem – im Folgenden erfährst du Schritt für Schritt, was dafür nötig ist.
...
====
Führe die Änderungen zuerst im <<basics/erste-schritte/plugins#50, Vorschaumodus Stage>> aus, bevor du sie in deinen Webshop übernimmst.
====
...
. Suche den Code `{% if ( $CustomerID ) { Link_Checkout(3); } else { Link_Checkout(2); } %}`.
