...
. Gib die Login-Daten ein. Beachte dazu die Erläuterungen in <<tabelle-POS-kassenpersonal-anmeldung>>.
. Tippe auf **Login**. +
...
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<tabelle-POS-darstellung>>.
...
. Tippe auf *Neuer Auftrag*, um mit dem Kassiervorgang für den nächsten Kunden fortzufahren.
...
Mit plentyPOS kannst du unkompliziert Anzahlungen entgegennehmen. Dies ist zum Beispiel bei Artikeln mit hohen Verkaufspreisen oder bei Ware, die speziell für Kunden angefertigt wird, sinnvoll. Dazu muss im plentymarkets Backend das Plugin link:https://marketplace.plentymarkets.com/plugins/payment/PosInvoice_5590[plentyPOS Kauf auf Rechnung^]{nbsp}icon:external-link[] aktiviert sowie bereits ein Kontaktdatensatz für den betreffenden Kunden vorhanden sein.
...
→ Weitere Optionen werden eingeblendet.
. Tippe auf die gewünschte Einstellung. Beachte dazu die Erläuterungen in <<tabelle-POS-kassenpersonal-auftragspositionen-bearbeiten>>.
...
* Die Aufträge können erst ab App-Version 1.9.2 mit Mehrzweckgutscheinen bezahlt werden. Voraussetzung ist, dass für den Auftrag noch keine Zahlung durchgeführt wurde und noch kein Rechnungsdokument für den Auftrag erstellt wurde.
