...
*_Oder:_* Tippe in der App oben links auf das Menüsymbol und dann auf *Ausloggen*. +
...
. Tippe oben rechts im POS-Warenkorb auf das Dreipunktmenü. +
→ Weitere Optionen werden angezeigt.
. Tippe auf *Auftrag laden*. +
→ Die Auftragssuche wird geöffnet.
. Gib in das Feld *Auftrags-ID* die Auftrags-ID ein.
. Tippe auf *Suchen*. +
...
. Tippe auf *Auftrag laden*. +
→ Die Auftragssuche wird geöffnet.
. Gib in das Feld *Auftrags-ID* die ID des Auftrags ein.
. Tippe auf *Suchen*. +
→ Die Auftragsdaten werden in die Kasse geladen. +
...
*_Oder:_* Tippe auf den Auftrag, um die Detailansicht zu öffnen, und dort auf das Icon *Stornieren*.
...
Wenn du mit plentyBase und Etikettendruckern arbeitest, kannst du in der Artikelsuche der plentymarkets App auch Artikeletiketten drucken. Wie das geht, erfährst du im Kapitel <<artikel/artikel-verwalten#3340, Etikett in der Artikelsuche der plentymarkets App drucken>>.
...
====
Standardmäßig wird der Soll-Bestand _nicht_ angezeigt. Beim Zählen siehst du also _nicht_ wie viel Geld in die Kasse erwartet wird. Es ist sinnvoll den Soll-Bestand ausgeblendet zu lassen, damit Nutzer den Soll-Bestand wirklich selbst zählen. Ist die Information eingeblendet, können Nutzer den Soll-Bestand direkt übernehmen, ohne selbst gezählt zu haben. Falls du trotzdem den <<pos/pos-einrichten#50, Soll-Bestand einblenden>> möchtest, kannst du diese Einstellung für die Kasse vornehmen.
====
...
*_Oder:_* Um statt Stückelung und Menge den Gesamtbetrag einzugeben, tippe in das Feld **Überschreiben** und gib den gezählten Betrag ein.
