...
:author: team-mobile
:keywords: plentymarkets POS, plentyPOS, Kasse, POS, plentymarkets Kasse, Kasse installieren, Kasse benutzen, POS benutzen, Kassenbenutzer, POS-Benutzer, Kassenpersonal, Kassierer, POS Auftrag, POS Aufträge, Kassenbeleg, Kassenabschluss, Tagesabschluss, Kassiervorgang, Bon, Kassensturz
:position: 20
...
. Gib die Login-Daten ein. Beachte <<tabelle-POS-kassenpersonal-anmeldung>>.
. Tippe auf **Login**. +
...
. Nimm die Einstellungen vor. Beachte <<tabelle-POS-darstellung>>.
...
. Tippe im POS-Warenkorb auf die Lupe. +
...
Mit plentyPOS kannst du unkompliziert Anzahlungen entgegennehmen. Dies ist zum Beispiel bei Artikeln mit hohen Verkaufspreisen oder bei Ware, die speziell für Kunden angefertigt wird, sinnvoll. +
Im Allgemeinen basiert das Vorgehen bei Anzahlungen auf den Schritten, die du auch beim <<#430, Entgegennehmen mehrerer Zahlungsarten>> durchläufst. Im diesem Kapitel wird beschrieben, wie du zuerst einen Teilbetrag als Anzahlung in bar oder per Kartenzahlung einnimmst und eine Rechnung über den Restbetrag erstellst.
...
Gehe wie unten beschrieben vor, um eine Anzahlung entgegenzunehmen.

[.instruction]
Anzahlung entgegennehmen:

. Gib über das Ziffernfeld den Anzahlungsbetrag ein.
. Tippe auf die Schaltfläche für die Zahlungsart, in der du den Anzahlungsbetrag erhalten hast. +
. Tippe auf das Symbol *Rechnungskauf*. +
→ Die Anzahlung wird gebucht. +
→ Der Restbetrag wird als Rechnungsbetrag übernommen.
...
Manchmal ist es nötig, abgeschlossene Transaktionen zu finden und weiter zu bearbeiten. Beispielsweise ist es sinnvoll, den ursprünglichen Kaufvorgang zu finden, wenn Kunden bereits gekaufte Artikel <<pos/pos-kassenbenutzer#175, retournieren>> oder wenn Artikel <<pos/pos-kassenbenutzer#450, online bestellt und im Laden abgeholt>> werden.
...
Gib die Auftrags-ID oder Belegnummer in das Suchfeld ein. Tippe auf *Suchen*, um die Auftragsdaten abzurufen. Nun kannst du die <<pos/pos-kassenbenutzer#370, Retoure abschließen>> oder die <<pos/pos-kassenbenutzer#450, online bestellte Ware>> aushändigen.
...
Scanne den Barcode auf dem Kaufbeleg. Nun kannst du die <<pos/pos-kassenbenutzer#370, Retoure abschließen>> oder die <<pos/pos-kassenbenutzer#450, online bestellte Ware>> aushändigen.
...
. Lege den Gutscheinartikel mit dem gewünschten Gutscheinwert in den POS-Warenkorb.
. Wähle die Menge 1. +
**__Tipp:__** Wenn du mehr als einen Gutschein als Menge wählst, werden die Gutschein-Codes automatisch generiert. +
→ Das Fenster **Gutschein wählen** wird angezeigt.
...
. Lege den Gutscheinartikel mit dem gewünschten Gutscheinwert in den POS-Warenkorb.
. Wähle die Menge 1. +
**__Tipp:__** Wenn du mehr als einen Gutschein als Menge wählst, werden automatisch Gutschein-Codes generiert. +
→ Das Fenster **Gutschein wählen** wird angezeigt.
. Gib in das Feld **Gutschein-Code** den Code ein.
. Tippe auf **Generieren**. +
...
**__Tipp:__** Über das Symbol **Zurücksetzen** kannst du den Standardkunden wiederherstellen.
...
→ Weitere Optionen werden eingeblendet.
. Tippe auf die gewünschte Einstellung. Beachte <<tabelle-POS-kassenpersonal-auftragspositionen-bearbeiten>>.
...
. Tippe auf **Rabatt**. +
//. Gib in das Feld **Rabatt** den Rabatt ein.
//. Tippe auf **Prozentual** oder **Fest**, je nachdem ob der Rabatt als Prozent oder als einen festen Betrag zu verstehen ist. +
...
. Tippe in der Zeile des Auftrags auf das Icon *Stornieren*. +
icon:map-signs[] *_Oder:_* Tippe auf den Auftrag, um die Detailansicht zu öffnen, und dort auf das Icon *Stornieren*.
...
//Wie du dabei vorgehst ist jedoch davon abhängig, wie die Kasse eingestellt ist. Denn je nach Kasseneinstellung sind beim Öffnen des Auftrags entweder alle Artikel eines Auftrags für die Retoure vorausgewählt oder es sind keine Artikel für die Retoure vorausgewählt. Der Retourenwarenkorb ist also beim Öffnen des Auftrags entweder voll oder leer:
...
////
* Keine Auftragspositionen sind vorausgewählt, wenn du den Auftrag öffnest. +
→ <<#360, Füge die Artikel zur Retoure hinzu, die zurückgegeben werden sollen.>> +
*_Tipp:_* Beim Öffnen des Auftrags sind alle Auftragspositionen ausgegraut. Rechts neben den Auftragspositionen wird ein grünes Plus angezeigt, über das du die Artikel hinzufügst.

Ob du beim Öffnen des Hauptauftrags einen vollen oder einen leeren Retourenwarenkorb siehst, wird in den Grundeinstellungen der Kasse im plentymarkets Backend über die Funktion *Retoure mit Hauptauftrag: Zu retournierende Auftragspositionen manuell wählen* festgelegt.
////
[#350]
[.subhead]
...
====
Ob Retourenartikel beim Öffnen des Auftrags vorausgewählt sind hängt davon ab, wie die Kasse eingestellt ist. Dass die Retourenartikel vorausgewählt sind erkennst du daran, dass rechts neben den Auftragspositionen ein kleiner Papierkorb angezeigt wird. An diesem Papierkorbsymbol erkennst du, dass der Retourenwarenkorb voll ist und du die Artikel entfernen musst, die nicht zurückgegeben werden sollen.
====
////
[.instruction]
...
Wenn du im Bereich *Retoure* einen Auftrag öffnest, sind keine Auftragspositionen des Auftrags für die Retoure vorgemerkt. Gehe wie unten beschrieben vor, um die Auftragspositionen zur Retoure hinzuzufügen, die zurückgegeben werden.

[TIP]
.Sind die Retourenartikel vorausgewählt?
====
Ob Retourenartikel beim Öffnen des Auftrags vorausgewählt sind hängt davon ab, wie die Kasse eingestellt ist. Dass die Retourenartikel nicht vorausgewählt sind erkennst du daran, dass die Auftragspositionen ausgegraut sind und rechts daneben ein grünes Plus angezeigt wird. An diesem grünen Plus erkennst du, dass der Retourenwarenkorb leer ist und du die Artikel hinzufügen musst, die zurückgegeben werden sollen.
====
...
. Tippe auf *Auftrag suchen*. +
→ Die Auftragssuche wird geöffnet.
. Gib in das Feld *Externe Auftragsnummer* die Belegnummer ein.
. Tippe auf *Suchen*. +
→ Die Auftragsdaten werden angezeigt. +
→ Keine Auftragspositionen sind für die Retoure vorausgewählt.
. Füge die Artikel zur Retoure hinzu, die zurückgegeben werden sollen. +
  .. Um die gesamte Menge einer Auftragsposition zur Retoure hinzuzufügen: +
    1. Tippe auf das grüne Plus neben der Auftragsposition. +
    → Die gesamte Stückzahl wird zur Retoure hinzugefügt. +
    → Als Ziellagerort wird der *Standard-Reparaturlagerort* der Kasse angezeigt.
  .. Um eine Teilmenge einer Auftragsposition zur Retoure hinzuzufügen:
    1. Tippe auf das grüne Plus neben der Auftragsposition. +
  → Die gesamte Stückzahl wird zur Retoure hinzugefügt.
    2. Tippe auf die Auftragsposition. +
    → Weitere Optionen werden eingeblendet.
    3. Tippe auf *Menge ändern*. +
    → Das Fenster *Menge ändern* wird angezeigt.
    4. Gib die Anzahl des Artikels ein, die zurückgegeben wird. +
    → Die Menge der Auftragsposition wird aktualisiert. +
    → Als Ziellagerort wird der *Standard-Reparaturlagerort* der Kasse angezeigt.
. *_Optional:_* Wähle einen anderen Ziellagerort für den Artikel: +
  .. Tippe auf den Artikel.
