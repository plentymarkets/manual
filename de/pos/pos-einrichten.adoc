...
|**Externer Name DE**
|Externen Namen eingeben. Der externe Name wird im Webshop angezeigt und ist für Kunden sichtbar.
...
Damit Kassenaufträge korrekt in plentymarkets gespeichert werden, muss jedem Auftrag ein Kunde zugeordnet sein. Deshalb erstellen Sie einen anonymen Standardkunden für Ihre Laufkundschaft. Gehen Sie wie unten beschrieben vor, um einen Standardkunden für Kassenaufträge anzulegen. Nur die Einstellungen werden beschrieben, die für das Erstellen des Standardkunden für Kassenaufträge relevant sind. Allgemeine Informationen zum Erstellen von Kunden finden Sie auf der Handbuchseite <<crm/kontakte-verwalten#, Kontakte verwalten>>.
...
* Über die Kasse können für alle im System gespeicherten Aufträge Retouren gebucht werden. Kunden können also auch Artikel aus Aufträgen mit anderen Herkünften zurückzugeben. Die Entscheidung, ob Kunden Artikel zurückgeben können, trifft das Kassenpersonal. Informieren Sie deshalb das Kassenpersonal, ob und wenn ja welche Herkünfte von der Retoure ausgeschlossen sind.
...
* Weisen Sie das Kassenpersonal an, zurückgegebene Artikel auf dem Kaufbeleg durchzustreichen. So verhindern Sie, dass Kunden Artikel mehrmals zurückzugeben.
* Bei Kauf auf Rechnung oder Anzahlungen wird Kunden bei einer Retoure erst Geld ausgehändigt, wenn der Preis der zurückgegebenen Artikel höher ist als der Betrag, der bisher bezahlt wurde.
...
Statt den Retourenbetrag in bar auszuzahlen, kann ein Gutschein über den Retourenbetrag erstellt und Kunden ausgehändigt werden. Bei einer solchen Retoure wird der Retourenbetrag nur theoretisch ausgezahlt und sofort ein Gutschein über denselben Betrag gekauft. Bei einer Retoure mit Gutschein werden also eine Retoure, eine Gutschrift und ein neuer Auftrag erstellt.
...
→ Prüfen Sie diese Ereignisaktionen und Prozesse, bevor Sie eine Abholung im Laden anbieten. Stellen Sie sicher, dass Aufträge erst in den Status *Warenausgang gebucht* gesetzt werden, wenn die Kundin die Ware im Laden abgeholt hat. +
→ Die Kasse zeigt an, dass die Kundin die Ware erhalten hat, wenn in plentymarkets ein Warenausgang für den Auftrag gebucht wurde. Falls Probleme auftreten und eine Kundin darauf besteht, die Ware noch nicht erhalten zu haben, prüfen Sie den Auftrag im plentymarkets Backend. Möglicherweise wurde der Warenausgang zu früh automatisch gebucht.
