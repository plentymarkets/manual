...
Über den Webshop, Marktplätze oder andere Herkünfte bestellte Ware kann entweder online bezahlt und im Ladengeschäft abgeholt oder im Ladengeschäft bezahlt und abgeholt werden. Befolge die unten beschriebenen Schritte, um Click & Collect einzurichten und an der Kasse abzuwickeln.
...
→ Stelle sicher, dass Auftragsbestätigungen die Auftrags-ID enthalten. Weise deine Kund:innen explizit darauf hin, dass diese Auftrags-ID benötigt wird, um den Auftrag in der Filiale zu bearbeiten.
* Erkläre deinen Kassenkräften, wie sie die Identität von Personen prüfen müssen, die bezahlte Ware im Ladengeschäft abholen. Sonst kann es passieren, dass Kassenkräfte die Ware an Unbefugte aushändigen.
...
*_Hinweis:_* Bei Kassen mit Standort Österreich und aktivierter Sicherheitseinrichtung muss der <<pos/pos-einrichten#920, Endbeleg in der App>> erstellt werden. Dadurch wird die Kasse automatisch gelöscht.
