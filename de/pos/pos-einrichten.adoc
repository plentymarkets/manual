...
* Ergänze bei Aushändigung der Kundenkarte manuell die Kundennummer im entsprechenden Kontaktdatensatz. Trage dazu du die Kundennummer (ohne Präfix) im Feld *Kundennummer* im Menü *CRM » Kontakte » [Kontakt öffnen]* ein. *Speichere* (icon:save[role="darkGrey"]) die Einstellungen.

|====

Sobald du dich für eine Vorgehensweise entschieden hast, kannst du mit der Beschaffung der Kundenkarten fortfahren.

* Wähle einen Anbieter aus, um link:https://lmgtfy.com/?q=Barcode-Karten+Druck[Barcode-Karten^]{nbsp}icon:external-link[] mit vorgefertigten Codes drucken zu lassen. Die meisten Anbieter erwarten eine CSV-Datei, die die einzelnen Codes enthält, die auf den Karten platziert werden sollen, sowie eine Layout-Vorlage im Photoshop- oder Adobe-Illustrator-Format für das Rahmendesign.

* Nutze ein link:https://lmgtfy.app/?q=online+barcode+generator[Online-Tool^]{nbsp}icon:external-link[], um die benötigten Barcodes zu generieren und anschließend im geforderten Format an den Anbieter weiterzuleiten.

* *_Wichtig:_* Stelle der Kontakt-ID oder der Kundennummer beim Generieren der Barcodes jeweils den Präfix *+* (ohne Leerzeichen) voran. Nur so kann beim späteren Scan des Barcodes der Kontaktdatensatz korrekt durchsucht und zugeordnet werden.
...
→ Die Konfigurationsanleitung findest du in der <<automatisierung/prozesse/plentybase-installieren#, Beschreibung des Plugins>>. +
...
→ Wie du die plentyBase Verbindungsdaten in der App speicherst, erfährst du in der <<automatisierung/prozesse/plentybase-installieren#, Beschreibung des Plugins>>.
...
→ Die Konfigurationsanleitung findst du in der <<automatisierung/prozesse/plentybase-installieren#, Beschreibung des Plugins>>. +
...
====
Um Etiketten drucken zu können, benötigst du das kostenlose Plugin plentyBase. plentyBase stellt eine Schnittstelle bereit, über die du aus dem plentymarkets Backend und/oder der plentymarkets App die Drucker nutzen kannst, die an den Computer angeschlossen sind. Wie du plentyBase installierst und konfigurierst erfährst du in der <<automatisierung/prozesse/plentybase-installieren#, Beschreibung des Plugins>>.
====
