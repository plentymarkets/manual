...
* Ergänze bei Aushändigung der Kundenkarte manuell die Kundennummer im entsprechenden Kontaktdatensatz. Trage dazu du die Kundennummer (ohne Präfix) im Feld *Kundennummer* im Menü *CRM » Kontakte » [Kontakt öffnen]* ein und *speichere* (icon:save[role="darkGrey"]) die Einstellungen.
...
* Nutze ein link:https://lmgtfy.app/?q=online+barcode+generator[Online-Tool^]{nbsp}icon:external-link[], um die benötigten Barcodes zu generieren.
