...
Das Kassensystem plentyPOS wird über die <<app/installation#, plentymarkets App>> bereitgestellt. Wirf mal einen Blick in unsere Dokumentation und lerne:
...
[#20]
== plentyPOS Kasse buchen

Um plentyPOS verwenden zu können, link:https://www.plentymarkets.eu/preise/[buche zunächst eine oder mehrere Kassen^]. Informationen zu den Konditionen findest du direkt bei der Buchung oder auf der link:https://www.plentymarkets.eu/produkt/pos/[plentyPOS Produktseite^].
...
Nimm nun im Tab **Grundeinstellungen** der neu erstellten Kasse die gewünschten Einstellungen vor. Beachte dazu die Erläuterungen in <<tabelle-POS-grundeinstellungen>>.
...
====
Kassenpersonal kann eigene Favoriten in plentyPOS speichern. Die Favoriten des Kassenpersonals haben dabei Priorität gegenüber den im plentymarkets Backend gespeicherten Favoriten.
====
...
. Nimm die gewünschten Einstellungen vor. Beachte dazu die Erläuterungen in <<tabelle-POS-belegvorlage>>.
. **Speichere** die Einstellungen. +
...
|*Artikel-Zusatzinfo*
|Eine der Optionen *Varianten-ID*, *Variantennummer*, *Attribute* oder *Lieferanten-Artikel-Nr.* aktivieren, um auf allen Belegen der Kasse die entsprechende Zusatzinformation zum verkauften Artikel auszugeben. Die Information wird unterhalb des Artikelnamens auf dem Kassenbeleg angezeigt. Die Option *Keine* aktivieren, um keine Zusatzinformation anzuzeigen.
...
Um Kassenpersonal den Zugriff auf plentyPOS zu ermöglichen, sind die folgenden Vorbereitungen nötig:
...
Damit neues Kassenpersonal Zugriff auf plentyPOS erhält, erstelle zunächst im plentymarkets Backend <<basics/admin-aufgaben/benutzer-verwalten#, Benutzerkonten>> der Benutzerklasse **Backend** für diese Personen. Gehe dazu wie unten beschrieben vor.
...
Erteile nun Berechtigungen für die Kommunikation zwischen plentyPOS und dem plentymarkets Backend über die REST-API.
...
. Öffne das Tab **Mobile**.
. Aktiviere die POS-Funktionen, die der Kassenkraft zur Verfügung stehen sollen. Beachte dazu die Erläuterungen in <<tabelle-POS-berechtigungen>>.
. **Speichere** die Einstellungen.
...
|**POS**
|Gibt Kassenpersonal Zugang zu plentyPOS.
...
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<tabelle-POS-zahlungsarten>>.
...
Informationen dazu, wie du diese Plugins an plentyPOS anbindest, findest du in der Beschreibung des Plugins.

[#1080]
=== Kauf auf Rechnung und Anzahlungen anbieten

Wenn du Bestandskunden die Möglichkeit geben möchtest, Kassenaufträge per Rechnung oder mithilfe einer Anzahlung zu bezahlen, benötigst du das folgende Plugin:
...
Um Verkaufsgutscheine und Aktionsgutscheine mit plentyPOS anzubieten, sind die folgenden Vorbereitungen im plentymarkets Backend erforderlich:
...
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<tabelle-POS-gutscheinkampagnen>>.
. **Speichere** die Einstellungen. +
...
Verkaufsgutscheine werden in plentyPOS eingelöst, indem das Kassenpersonal den Gutschein-Code eingibt. Damit der Gutschein-Code eingelöst werden kann, muss der Code zum Zeitpunkt der Einlösung in einer Kampagne des plentymarkets Backends gespeichert sein.
...
. Nimm die weiteren Einstellungen vor. Beachte dazu die Erläuterungen in <<tabelle-POS-verkaufsgutscheine-artikeleinstellungen>>.
. **Speichere** die Einstellungen.
...
. **Speichere** die Einstellungen. +
→ Der Artikel wird erstellt und zur weiteren Bearbeitung geöffnet.
. Nimm die weiteren Einstellungen vor. Beachte dazu die Erläuterunngen in <<tabelle-POS-verkaufsgutscheine-artikeleinstellungen>>.
. **Speichere** die Einstellungen. +
...
. Öffne das Menü *Aufträge » Gutscheine*.
. Klicke auf *Neue Kampagne*.
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<tabelle-POS-retourenkampagne>>.
. *Speichere* die Einstellungen. +
→ Die neue Kampagne wird erstellt und in der Übersicht angezeigt.
...
. *Speichere* die Einstellungen. +
→ Der Artikel wird erstellt und zur weiteren Bearbeitung geöffnet.
. Nimm die weiteren Einstellungen vor. Beachte dazu die Erläuterungen in <<tabelle-POS-retourengutscheine-artikeleinstellungen>>.
. *Speichere* die Einstellungen.
...
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<tabelle-POS-geraet-koppeln>>.
. **Speichere** die Einstellungen. +
...
*_Hinweis:_* Aktiviere beim Konfigurieren die Option *HTTP-Server*. Nur so kann plentyPOS mit plentyBase kommunizieren.
...
. Öffne die plentymarkets App.
. Öffne das Menü *plentymarkets App » Einstellungen » POS*.
. Nimm im Bereich *Drucker* die Einstellungen vor. Beachte dazu die Erläuterungen in <<tabelle-POS-drucker>>.
. **Speichere** die Einstellungen.
...
. Wähle die Filter. Beachte dazu die Erläuterungen in <<tabelle-POS-kassenjournal-menue>>.
. Klicke auf **Suchen**. +
