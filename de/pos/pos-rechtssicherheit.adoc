...
In Deutschland gilt für alle Registrierkassen die Kassensicherungsverordnung (KassenSichV) in Kraft. Ab diesem Datum bist du verpflichtet, alle deine Kassen mit einer so genannten technischen Sicherheitseinrichtung (TSE) auszustatten. Die TSE signiert alle Kassenbelege und legt die Kassenvorgänge unveränderbar in einem speziellen Dateiformat ab. Einen TSE-Export solltest du täglich erstellen. Im Rahmen einer Prüfung durch die Finanzbehörden musst du außerdem einen Export der vorhandenen gesammelten TSE-Daten vorlegen können.
...
|1.
|Hardware beschaffen
|Nach dem 01.01.2020 angeschaffte Hardware muss per Gesetz bereits entweder KassenSichV-konform oder aufrüstbar sein. Kontaktiere deinen Kassenanbieter, um neue Bondrucker mit angeschlossener TSE zu beschaffen. *_Alternativ_*: Informiere dich bei deinem Kassenanbieter, ob deine aktuell verwendete Hardware aufgerüstet werden kann oder muss.

|2.
|TSE-Einheit koppeln
|Nach dem Anschluss der Hardware müssen die Daten der TSE-Einheit(en) im plentymarkets Backend eingegeben werden. +
Öffne dazu das Menü **Einrichtung » Mandant » Mandant wählen » POS** und wähle die Kasse, die mit der TSE verbunden werden soll. Das Tab *Grundeinstellungen* öffnet sich. Trage im Bereich **Fiskalisierung (Deutschland)** die **Seriennummer der TSE** und den **Public Key** der TSE ein, wie in den Unterlagen deiner Hardware vermerkt. Klicke anschließend auf **Speichern**.

|3.
|Registrierung der TSE-Einheit beim Finanzamt
|Melde deine TSE inklusive der Seriennummer an dein zuständiges Finanzamt. Weitere Informationen zur Meldepflicht folgen.
...
====
Wir empfehlen, die IP-Adresse der TSE separat zu notieren. Bestenfalls bringst du nach erfolgreicher Kopplung ein Etikett mit der IP-Adresse außen am Bondrucker an. So kannst du beispielsweise nach Verbindungsabbrüchen einfach wieder auf die zuvor gespeicherte IP-Adresse zugreifen und vermeidest, dass die falsche TSE gekoppelt wird.
====
...
[IMPORTANT]
.Verfügbarkeit von TSE-Exporten
====
Im oben beschreibenen Menü werden nur Exportdateien angezeigt, wenn zuvor in der App ein entsprechender TSE-Export erzeugt wurde. Wir empfehlen, täglich einen TSE-Export mit dem Tagesabschluss an der Kasse zu erstellen.
====
