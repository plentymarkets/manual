...
[IMPORTANT]
.Anpassbare Artikelsuche erst ab plentyShop LTS 5
====
Es ist möglich, die Frontend-Artikelsuche für deinen link:https://marketplace.plentymarkets.com/plugins/sales/online-shops/ceres_4697[plentyShop LTS]{nbsp}icon:external-link[] anzupassen.
...
. Erweitere den Bereich *plentyShop LTS » Artikelsuche* (icon:chevron-down[role="darkGrey"]).
