...
Die Artikeldaten werden in einer Datenbank gespeichert. Da das Auslesen dieser Daten aber zu lang dauern würde, sendet die Datenbank diese Daten zur Zwischenspeicherung an ElasticSearch. Die Artikelansicht und Artikelsuche in Ceres und der Elastische Export beziehen Artikeldaten nicht aus der Datenbank, sondern aus ElasticSearch. So werden im Webshop Suchergebnisse schneller gefunden und Artikeldaten schneller geladen. Auch der Elastische Export wird durch ElasticSearch beschleunigt. Allerdings dauert es je nach Anzahl der geänderten Artikel zwischen einigen Minuten und mehreren Stunden, bis die Daten in ElasticSearch vollständig aktualisiert sind.
