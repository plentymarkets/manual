...
|Ist dein Lieferant aus einem anderen Land tätig und verwendet eine andere Währung? Gib den Einkaufspreis der Variante in die Währung deines Lieferantens ein. Welche Währung hier angezeigt wird, hängt davon ab, welche <<crm/kontakte-verwalten#100, Währung im Kontaktdatensatz gespeichert>> wurde. Bei einer <<warenwirtschaft/nachbestellungen-verwalten/nachbestellungen-vornehmen#, Nachbestellung>> wird diese Währung verwendet, falls verfügbar.
