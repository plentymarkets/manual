...
. Klicke auf *Hinzufügen* (icon:plus[role="green"]). +
→ Der Barcode-Typ wird hinzugefügt und in der Liste angezeigt.
. Klicke auf *GTIN generieren* (icon:cog[role="darkGrey", stack="arrow-right,lr"]), um automatisch den nächsten verfügbaren Barcode einzufügen.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
. Klicke auf *Etiketten generieren* (icon:cog[role="darkGrey", stack="arrow-right,lr"]). +
