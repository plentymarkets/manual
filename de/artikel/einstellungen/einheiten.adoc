...
//später :position: 20 einfügen wenn die Seite live sein soll
...
. Nimm die Einstellungen vor. Beachte <<tabelle-einheiten-bearbeiten>>.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
<<artikel/artikel-verwalten#20, Beim Erstellen eines Artikels>> kannst du die Einheit und den Inhalt des Artikels direkt eingeben. Wenn du <<artikel/artikel-verwalten#160, zusätzliche Varianten erstellst>>, kannst du auch die Einheit und den Inhalt jeder Variante eingeben.

[#300]
== Grundpreis auf Vertriebskanälen und Etiketten anzeigen

Sicherlich hast du schon den Grundpreis gesehen, der in den Regalen der Lebensmittelgeschäfte aufgeführt ist. Der Grundpreis gibt die Kosten pro Pfund, Liter oder andere Maßeinheit an.

Berechnet wird er auf Grundlage von <<artikel/artikel-verwalten#270, Einheit, Inhalt>> und <<artikel/artikel-verwalten#240, Preis>> des Artikels. Dargestellt werden kann er <<artikel/artikel-verwalten#270, im Webshop>>, auf Vertriebskanälen wie <<maerkte/ebay/ebay-einrichten#4000, eBay>> oder <<maerkte/plus-gartenxxl#365, Netto>> und <<artikel/artikel-verwalten#3300, auf Artikeletiketten>>.
