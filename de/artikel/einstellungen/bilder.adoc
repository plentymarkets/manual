...
====
Lade die benötigten Bilder hoch und gib an, dass die Attribute im Webshop grafisch dargestellt werden sollen.
...
| *Qualität in Prozent*
|Gib den Grad der JPG-Komprimierung als Prozent ein.
...
a|Gib eine Zahl zwischen 0 und 9 ein. Dies ist die Komprimierungsstufe, die für PNG-Dateien verwendet wird.
...
Klicke in diesem Fall auf icon:undo[role="orange"] *CDN-Cache invalidieren*, um die Änderungen schneller anzuzeigen.
...
//Sobald das neue Menü in beta veröffentlicht wird, kann die alte Konfig oben gelöscht werden und durch die neue Konfig unten ersetzt werden - auch im nächsten Kapitel
////
. Öffne das Menü *Einrichtung » Mandant » Mandant öffnen » CDN-Domains*. +
...
. Werden im Menü *Einrichtung » Mandant » Mandant öffnen » CDN-Domains* grüne Häkchen in den Spalten *SSL*, *DNS* und *Cloud-Status* angezeigt? Dann ist der Vorgang abgeschlossen.
Wenn alle drei Häkchen angezeigt werden, klicke in der Spalte *Status* auf *Aktivieren*. +
→ Die Domain wird aktiviert.

//Sobald das neue Menü in beta veröffentlicht wird, kann die alte Konfig oben gelöscht werden und durch die neue Konfig unten ersetzt werden - auch im vorherigen Kapitel
////
[.instruction]
Eigene Domain für Bild-URLs speichern bei externer Domain:
...
. Kopiere die in plentymarkets angezeigten DNS-Einstellungen in die dafür vorgesehenen Felder des Domain-Providers. +
→ Informationen zur Vorgehensweise findest du in der Dokumentation des Domain-Providers.
. Werden im Menü *Einrichtung » Mandant » Mandant öffnen » CDN-Domains* grüne Häkchen in den Spalten *SSL*, *DNS* und *Cloud-Status* angezeigt?
