...
//später :position: 30 einfügen wenn die Seite live sein soll

Manche Artikel, wie Schuhe und T-Shirts, werden in verschiedenen Größen und Farben verkauft. Solche Charakteristiken werden in plentymarkets als Attribute bezeichnet. Mit Hilfe von Attributen kannst du deine Produkte charakterisieren und gleichzeitig auch <<artikel/einleitung/struktur#, verkaufbare Varianten>> erzeugen, wie z.B. das T-Shirt in der Farbe rot und in Größe S.

Der Ablauf ist wie folgt. Lege zunächst <<artikel/einstellungen/attribute#100, Attribute>>, wie Farbe sowie die entsprechenden <<artikel/einstellungen/attribute#200, Werte>>, wie rot, blau und grün an. <<artikel/einstellungen/attribute#300, Erzeuge danach Varianten>>, indem du Attributwerte miteinander kombinierst. Zum Beispiel ergeben sich 9 verschiedene Varianten, wenn du die Größen S, M, L mit den Farben rot, blau und grün kombinierst. Dieses Video bietet dir eine spielerische Einführung in das Thema:

.Lerne Attribute spielerisch kennen
video::265389643#t=0m54s[vimeo]

Verkaufst du deine Produkte auf Märkten wie Amazon und eBay? Einige Märkte erfordern, dass du <<artikel/einstellungen/attribute#500, deine eigenen Attributwerte mit ihren Attributwerten>> verknüpfst.

Optional kannst du auch <<artikel/einstellungen/attribute#600, Bilder hinzufügen>>, um deine Attributwerte grafisch darzustellen. Oder du kannst die Namen von Attributen und Werten <<artikel/einstellungen/attribute#1000, in mehreren Sprachen>> speichern.

[TIP]
.Andere Möglichkeiten der Produktcharakterisierung
====
Möchtest du deine Produkte charakterisieren, _ohne_ dabei Varianten zu erzeugen? Möchtest du beispielsweise nur die technischen Features eines Produkts in deinem Shop hervorheben? Dies ist mit Hilfe von <<artikel/einstellungen/eigenschaften-merkmale#, Eigenschaften und Merkmalen>> möglich! Der Unterschied besteht darin, dass Eigenschaften _spezifische Varianten_ charakterisieren, während Merkmale _ganze Artikel_ charakterisieren.
====
...
. Nimm weitere Einstellungen vor. Beachte <<tabelle-attribut-erstellen>>.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
|*Position*
a|Welches Attribut soll als erstes, zweites, drittes, usw. angezeigt werden? Gib eine Positionsnummer in dieses Feld ein. Wenn ein Artikel mehrere Attribute hat, werden die Attribute aufsteigend nach Positionsnummern in deinem Webshop dargestellt.

|[#bild-link-erlauben]*Bildverknüpfung möglich*
...
|[#attribut-auswahl]*Attribut-Auswahl im Webshop*
a|Wie sollen Kunden Größen oder Farben in deinem Webshop wählen? Sollen sie den Namen aus einer Dropdown-Liste wählen oder auf eine Bildkachel klicken?

* *Per Dropdown* = Attributnamen werden in einer Dropdown-Liste angezeigt.
* *Per Bild* = Attribute werden als <<artikel/einstellungen/attribute#700, Bildkacheln im Webshop>> angezeigt.
* *Per Box* = Attribute werden als Boxen mit Attributwerten im Webshop angezeigt. Diese Einstellung betrifft nur Callisto-Nutzer und ist nur bei der <<webshop/webshop-einrichten/artikelansichten#, Attributverfügbarkeit mit Client-Prüfung>> möglich.
...
. Nimm weitere Einstellungen vor. Beachte <<tabelle-attributwerte-festlegen>>.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
Es ist technisch möglich, Attribute beim <<artikel/artikel-verwalten#20, manuellen Anlegen eines neuen Artikels>> auszuwählen. Wir empfehlen diese Vorgehensweise jedoch nicht.

Vielmehr empfehlen wir dir, zuerst einen neuen Artikel, d.h. eine Hauptvariante, ohne Attribute anzulegen. Anschließend empfehlen wir dir, Attribute auszuwählen, während du <<artikel/artikel-verwalten#160, zusätzliche Varianten erstellst>>.

Warum? Verknüpfst du Attribute bereits bei der Artikelanlage, müssen _alle folgenden Varianten_ zwangsläufig aus den gleichen Attributen erzeugt werden. Dies ist nicht der Fall, wenn du Attribute erst bei der Variantenanlage verknüpfst. Im Nachhinein bist du also viel flexibler.
...
Einige Märkte, wie z.B. <<maerkte/amazon/amazon-einrichten#1350, Amazon>> und <<maerkte/ebay/ebay-einrichten#6100, eBay>>, verlangen, dass du _deine eigenen_ Attributwerte mit _ihren Attributwerten_ verknüpfst.
...
. Klicke auf das Attribut.
. Wechsele in das Tab *Werte*.
. Klicke in der Spalte *Bild* auf *Bild hochladen* (icon:upload[role="purple"]).
...
Soll sich das Artikelbild im Webshop ändern, je nachdem, welche Größe oder Farbe dein Kunde wählt?
...
Die <<artikel/artikel-verwalten#330, Varianten-Gruppenfunktion>> ist ein nützliches Werkzeug dafür.
