...
. <<artikel/einstellungen/eigenschaften#700, Bearbeite die Einstellungen weiter>>.
In diesem Schritt gibst du z.B. an, auf welchen Märkten und Webshops die Eigenschaften sichtbar sein sollen.
. <<artikel/einstellungen/eigenschaften#1200, Gruppiere die Eigenschaften>> bei Bedarf. Dies hilft dir, den Überblick in deinem plentymarkets-Backend zu behalten.
. <<artikel/einstellungen/eigenschaften#1400, Verknüpfe die Eigenschaften>> mit den passenden Varianten. Verknüpfe zum Beispiel die Bluetooth-Eigenschaft mit deinen Bluetooth-fähigen Produkten.
...
. *Speichere* (icon:save[role="green"]) die Einstellungen. +
→ Die Eigenschaft wird erstellt und <<artikel/einstellungen/eigenschaften#700, zur weiteren Bearbeitung>> geöffnet.
...
. <<artikel/frontend-artikelsuche-verwalten#80, Drittens>> erstellst du Webshop-Filter, die es deinen Kunden ermöglichen, gezielt nach Varianten mit einer bestimmten Wasserfestigkeitsstufe zu suchen.
...
. <<artikel/frontend-artikelsuche-verwalten#80, Drittens>> erstellst du Webshop-Filter, die es deinen Kunden ermöglichen, gezielt nach Varianten mit solchen Besonderheiten zu suchen.
!===
...
. Öffne das Menü *Einrichtung » Einstellungen » Eigenschaften » Konfiguration*.
. <<artikel/einstellungen/eigenschaften#800, Suche>> und öffne die betreffende Eigenschaft.
. Bearbeite die Einstellungen, die für deinen Anwendungsfall relevant sind.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
====
Du wirst diesen Bereich nur sehen, wenn du mit einer Eigenschaft vom Typ *Auswahl* oder *Mehrfachauswahl* arbeitest.
...
plentymarkets enthält ein <<daten/daten-importieren/ElasticSync#, Import-Tool>>, mit dem du Eigenschaftsverknüpfungen importieren kannst. Die Idee ist ganz einfach. Anstatt Eigenschaften manuell mit Varianten zu verknüpfen, trägst du die gleiche Informationen in eine CSV-Datei ein und importierst diese Datei dann in dein System.
