...
|**Bestellmerkmal**
|Aktivieren, wenn das Merkmal im Bestellvorgang verfügbar sein soll. Der Kunde kann dann je nach *Merkmaltyp* des Bestellmerkmals zum Beispiel einen Text oder Wert eingeben oder eine Option aus der Dropdown-Liste wählen.
...
|Marktplatz-Merkmal aus der Dropdown-Liste wählen, um das Merkmal mit einem Merkmal des Marktplatz zu verknüpfen. Weitere Informationen zum Erstellen von Merkmalen für Marktplätze und Preisportale finden Sie auf den Handbuchseiten der Marktplätze im Themenbereich <<omni-channel/multi-channel#, Multi-Channel>>.
|====
...
Mit Eigenschaften bieten Sie zum Beispiel Serviceleistungen gegen Aufpreis an oder binden ein Feld ein, in das Kunden Zusatzwünsche eingeben können. Über Eigenschaften verknüpfen Sie außerdem Ihre Varianten mit den Varianteneigenschaften auf einem Marktplatz. Über die Eigenschaftssuche finden Sie Artikel, die mit der gesuchten Eigenschaft verknüpft sind.
...
Viele Händler haben für ihre Artikel keine GTINs (Barcodes), da sie die Artikel direkt importieren und die Hersteller keine GTIN vergeben. Hier besteht die Möglichkeit, bei GS1 Germany GTINs zu erwerben, um diese GTINs zum Beispiel auf den Marktplätzen <<omni-channel/multi-channel/amazon/amazon-einrichten#, Amazon>> oder <<omni-channel/multi-channel/real-de/real-de-einrichten#, real.de>> einstellen zu können. 13-stellige GTINs (früher EAN) setzen sich zusammen aus Basisnummer, Artikelnummer und Prüfziffer. Die Basisnummer hat 7 bis 9 Stellen und besteht aus dem Länderpräfix der GS1-Mitgliedsgesellschaft, zum Beispiel 400 bis 440 für Deutschland, und der Unternehmensnummer. Die Artikelnummer des Herstellers hat 3 bis 5 Stellen (je nach Länge der Basisnummer). Die Prüfziffer nimmt die letzte Stelle ein.
...
**Aktionspreis** = Aktivieren, um einen Verkaufspreis als Aktionspreis zu kennzeichnen. Aktionspreise werden zum Beispiel für Marktplätze wie <<omni-channel/multi-channel/amazon/amazon-einrichten#, Amazon>> und <<omni-channel/multi-channel/real-de/real-de-einrichten#, real.de>> verwendet. +
...
.Tags verwenden
====
Markierungen können nur auf der Artikelebene verwendet werden. Verwenden Sie statt der Markierungen die flexibleren <<artikel/artikel-verwalten#150, Tags auf Variantenebene>>.
====
////
...
|**Tags**
|Tags für den Artikel hinzufügen. Wie Sie Tags verwenden, erfahren Sie im Kapitel <<artikel/artikel-verwalten#150, Varianten mit Tags verknüpfen>>.
...
Wert größer 0 = Kunden können maximal diese Gesamtmenge aller Varianten des Artikels bestellen. Kommawerte sind möglich, um zum Beispiel Bestellungen nach Gewicht oder Länge abzubilden. Dieser Wert hat Priorität gegenüber der maximalen Bestellmenge der einzelnen Varianten des Artikels.
...
Im Tab **Texte** geben Sie die Artikeltexte ein. Neben dem Vorschautext, der zum Beispiel auch in der Artikelliste des Webshops angezeigt wird, geben Sie die Artikelbeschreibung und weitere Texte ein, zum Beispiel technische Daten und Keywords.
...
Fügen Sie Freitextfelder in die Artikelansicht von Artikeln des Typs **Standard** ein. In Freitextfelder kann der Kunde Werte eingeben, zum Beispiel einen Zusatzwunsch oder Maße. Definieren Sie zunächst die Freitextfelder.
...
Im Tab **Statistik** haben Sie die Möglichkeit, statistische Daten zum Artikel des Typs **Standard** auszuwerten. Zum Beispiel stehen Statistiken zur Verfügung, mit denen Sie die Umsätze des Artikels des Typs **Standard** anhand verschiedener Kriterien beurteilen können. Weitere Informationen finden Sie auf der Handbuchseite <<basics/statistik/statistiken-verwalten#90, Artikelstatistiken>>.
...
Die ASIN (Amazon Standard Identification Number) ist eine Produktkennzeichnung von <<omni-channel/multi-channel/amazon/amazon-einrichten#, Amazon>>. Die ePID (eBay Product ID) ist eine Produktkennzeichnung von <<omni-channel/multi-channel/ebay/ebay-einrichten#, eBay>>. Die ASIN besteht aus 10 Buchstaben und/oder Ziffern. Bei Büchern entspricht die ASIN der ISBN-Nummer. Die ASIN finden Sie auf der Produktseite bei Amazon. Die ePID wird von eBay zur Verfügung gestellt. Verknüpfen Sie ASIN und ePID mit Varianten.
...
*_Hinweis:_* Wenn Hauptvarianten gewählt werden, werden diese Hauptvarianten nicht gelöscht.
...
Kontrollkästchen rechts neben *Tags* aktiviert = Aktiviert die Vererbung der Tags der Hauptvariante an die markierten Varianten. Die bisher mit der der Variante verknüpften Tags werden überschrieben. +
Kontrollkästchen rechts neben *Tags* nicht aktiviert = Deaktiviert die Vererbung der Tags der Hauptvariante an die in der Artikelübersicht markierten Varianten. xxxDie mit der Hauptvariante verknüpften Tags werden an der Variante gespeichert.xxx +
*_Hinweis:_* Diese Gruppenfunktion steuert die Vererbung aller Tags  der Hauptvariate. Eine Vererbung einzelner Tags an eine Variante ist nicht möglich.
|====
