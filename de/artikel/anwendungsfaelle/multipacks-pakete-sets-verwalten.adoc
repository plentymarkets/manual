...
Sobald die Set-Funktion veröffentlicht wird, werden alle <<bereits bestehenden Sets migriert>>.
====
...
//Entscheide zunächst, welche Artikel du zusammen als Set verkaufen möchtest. Überlege, welche Varianten in jedem Artikel enthalten sind.
//<<table-set-mind-map>> zeigt die Artikel und Varianten, die beispielsweise zusammen als Fußballkleidung verkauft werden könnten.

//Es kann hilfreich sein, eine Skizze zu zeichnen und zu planen, welche Artikel und Varianten im Set vorkommen werden.

Vielleicht hilft es dir, die Artikel und Varianten, die im Set vorkommen werden, zu skizzieren.
...
In _späteren Schritten_ wirst du:

* die <<einzelnen Bestandteile zum Set hinzufügen>>.
* den <<Verkaufspreis mit den einzelnen Bestandteilen verknüpfen>>.
* den <<monetären Preis für jeden Bestandteil eingeben>>.

//_In einem späteren Schritt_ wirst du die <<einzelnen Bestandteile zum Set hinzufügen>>.
//Zu diesem Zeitpunkt verknüpfst du auch den erstellten Verkaufspreis mit den einzelnen Bestandteilen und gibst den monetären Preis für jeden Bestandteil ein.

[TIP]
.Können Artikel günstiger sein, wenn sie im Set sind?
====
