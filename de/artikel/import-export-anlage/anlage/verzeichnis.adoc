...
//prio 1 zukunftsmusik - hier die Beschreibung trennen, die erste hälfte in vorbereitende Einstellungen und die zweite hier ausführlicher
...
Wann sollen Kunden die Variante in deinem Webshop sehen können? Wann sollten sie sie kaufen können?

Sollen Kunden zum Beispiel Artikel sehen können, die derzeit nicht vorrätig sind?
...
*_Hinweis_*: Welche konkrete Lieferzeit dieses Symbol repräsentiert, konfigurierst du in den <<vorbereitenden Einstellungen>>.
...
Diese beiden Funktionen ergänzen sich und greifen nur, wenn die Option <<Beschränkung>> auf *Auf Netto-WB* gestellt ist.
//Die Verfügbarkeit im Webshop wird automatisch von plentymarkets geregelt. Varianten müssen nicht manuell unsichtbar gestellt werden, wenn kein Bestand vorhanden ist.
...
* *Automatisch unsichtbar wenn kein Netto-WB* = Aktivieren, damit die Variante automatisch unsichtbar wird, wenn kein Nettowarenbestand vorhanden ist. Wenn dies passiert, kann die Variante auch nicht mehr über die direkte URL aufgerufen werden.
...
##add note about availability for client and markets##
...
*_Hinweis_*: Die VPE hat keinen Einfluss auf die Berechnung eines <<Grundpreises>>.
...
Berechnet wird er auf Grundlage von <<artikel/einstellungen/einheiten#, Einheit>>, <<Inhalt>> und <<Preis>> des Artikels.
...
##im Tab Bilder in die obere Tab-Leiste werden die Bilder hochgeladen.
Nach dem Hochladen kannst du die Bilder mit einzelne Varianten verknüpfen.
Box-Hinweis auch oben bei Tab Bilder der obere Tab-Leiste ergänzen, dass man dann nachträglich mit Varianten verknüpfen kann##
...
.Text
====
##auch Beziehung zu die Verfügbarkeitseinstellungen im Tab Einstellung einer Variante klar machen##
##in that tab it's more like when should it be available instead of where##
====
...
Dieser Automatismus wird durch die Einstellungen im <<Tab: Einstellungen » Bereich: Verfügbarkeit>> gesteuert.
