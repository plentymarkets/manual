...
. Öffne das Menü *System » Systemeinstellungen » Mandant » Global » Dienste » Facettensuche*.
. Klicke auf *Neu*.
...
. Öffne das Menü *System » Systemeinstellungen » Mandant » Mandant wählen » Dienste » Facettenverknüpfungen*.
...
Im alte Design Callisto können für die Frontend-Suche verschiedene Suchen genutzt werden. Welche Suche verwendet wird, legst du im Menü *System » Systemeinstellungen » Mandant » Mandant wählen » Webshop » Artikelsuche* fest. Die folgenden Optionen sind verfügbar:
...
* *Facettensuche* +
→ Wie du die alte Facettensuche einrichtest, erfährst du im Kapitel <<artikel/frontend-artikelsuche-verwalten#220, Callisto mit alter Facettensuche nutzen>>.
* *Facettensuche (ElasticSearch - BETA)* +
→ Wie du die neue Facettensuche (ElasticSearch - BETA) einrichtest, erfährst du im Kapitel <<artikel/frontend-artikelsuche-verwalten#270, Callisto mit Facettensuche (Elastic Search- BETA) nutzen>>.

[#220]
=== Callisto mit alter Facettensuche nutzen

Die alte Facettensuche nutzt du, indem du im Menü *System » Systemeinstellungen » Mandant » Mandant wählen » Webshop » Artikelsuche* die Option *Facettensuche* aktivierst. Richte die Facettensuche wie im Kapitel <<artikel/frontend-artikelsuche-verwalten#90, Facettensuche einrichten>> ein und gehe dann wie unten beschrieben vor, um die alte Facettensuche in Callisto zu nutzen.

[#230]
==== Callisto + alte Facettensuche: Such-Index definieren

Im Tab *Such-Index* legst du Kriterien für eine verbesserte Suche im Webshop fest. Eine facettenbasierte Suche bietet eine hohe Fehlertoleranz. Suchbegriffe werden also auch bei Tippfehlern erkannt. Artikelnamen oder Herstellernamen müssen nicht exakt eingegeben werden, um zu einem Ergebnis zu führen. Zum Beispiel werden Produkte des Herstellers "adidas" auch bei fehlerhaften Eingaben wie "addidas" oder "adids" gefunden. Konfiguriere dafür die folgenden 3 Tabs:

* Im Tab *Felder* aktivierst du die Felder, die bei der Suche berücksichtigt werden sollen, bestimmst die Gewichtung und legst fest, welche Felder für die unscharfe Suche berücksichtigt werden sollen.
* Im Tab *Einstellungen* nimmst du Einstellungen für die unscharfe Suche vor.
* Im Tab *Volltext* legst du fest, welche Artikeldaten bei einer Volltextsuche berücksichtigt werden.

[#240]
==== Callisto + alte Facettensuche: Felder zum Suchindex hinzufügen

Gehe wie unten beschrieben vor, um Felder zum Suchindex hinzuzufügen. Bei der Suche im Webshop werden nur aktivierte Felder durchsucht.

[.instruction]
Suchfelder definieren:

. Öffne das Menü *System » Systemeinstellungen » Mandant » Global » Dienste » Facettensuche*.
. Wechsele in das Tab *Such-Index*. +
→ Das Tab *Felder* wird angezeigt.
. Aktiviere in der Spalte *Aktiv* die Felder, die bei der Suche berücksichtigt werden sollen.
. Gib für die aktivierten Felder eine Gewichtung zwischen 1 und 1000 ein. +
...
. *_Optional:_* Aktivere für die aktivierten Felder die Option *Unscharfe Suche*.
. *_Optional:_* Aktiviere die Option *Volltext*.
. *Speichere* die Einstellungen.

[#250]
==== Callisto + alte Facettensuche: Unscharfe Suche anpassen

Passe nun die Einstellungen für die unscharfe Suche an.
...
. Nimm die Einstellungen vor. Beachte die Erläuterungen in <<tabelle-unscharfe-suche>>.
. *Speichere* die Einstellungen.
...
Im Tab *Volltext* legst du fest, welche Artikeldaten durchsucht werden, wenn im Tab *Felder* die Option *Volltext* aktiviert ist.
...
. Öffne das Menü *System » Systemeinstellungen » Artikel » Suche » Frontend » Sucheinstellungen*.
. Aktiviere in der Spalte *Aktiv* die Felder, die bei der Suche berücksichtigt werden sollen.
...
. Öffne das Menü *System » Systemeinstellungen » Artikel » Suche » Frontend » Sprachen*.
