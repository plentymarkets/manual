...
[IMPORTANT]
.Kennzahlen je nach Tarif verfügbar
====
Um Kennzahlen anzulegen, ist die Buchung eines entsprechenden plentyBI-Tarifs notwendig. Öffne dazu das Menü *plentymarkets Logo (Start) » Mein Konto » Verträge*. Im Bereich *plentyBI Plan buchen* stehen dir plentyBI-Tarife mit verschiedenen Umfängen zur Auswahl.
====

Damit Daten im Dashboard angezeigt werden können, musst du zuerst entsprechende Kennzahlen erstellen.

Kennzahlen erlauben dir, verschiedene Datensätze zusammenzustellen. Kennzahlen liefern somit die Basis für die Werte und Daten, die du später auf dem Dashboard anzeigen und nachverfolgen kannst. Bei der Konfiguration von Kennzahlen gibt es vielfältige Optionen. Hierzu zählen klassische Filter sowie Brutto-und Netto-Berechnungsmöglichkeiten. Pro Kennzahl sind zudem sogenannte Dimensionen wählbar. Dimensionen bestimmen, nach welchen Kriterien die Daten der Kennzahl gruppiert werden.
So kannst du pro Kennzahl genau eingrenzen, wie das System die Daten pro Kennzahl berechnen soll.
...
Das Dashboard erlaubt dir viel Flexibilität beim Anordnen der Inhalte. Grundsätzlich platzierst du Darstellungsbereiche und Elemente intuitiv per Drag-and-Drop auf dem Dashboard. Jeder Bereich und jedes Element kann per Klick auf icon:design_inline_edit[set=plenty] weiter bearbeitet werden. <<table-dashboard-edit-mode>> enthält einen Überblick über die Funktionen, die im generellen Bearbeitungsmodus des Dashboards zur Verfügung stehen.
...
| *Standardansicht*
| Dieser Bereich zeigt an, welche Dashboard-Ansicht aktuell geöffnet ist. Klicke auf icon:sort-down[role=darkGrey], um zu einer anderen Ansicht zu wechseln oder eine neue Ansicht zu erstellen.
...
//TODO: Warum ist das gut, wie geht das, Ausblick auf Rechtevergabe

[#408]
=== Rechte für Dashboard-Ansichten vergeben

Bei der Einrichtung eines Dashboards ist eine Standardansicht vorhanden. Diese ist für alle Benutzer:innen deines plentymarkets Systems sichtbar. +
Überlege dir vor der Arbeit mit dem Dashboard, welche Benutzer:innen die Standardansicht sehen dürfen. Du kannst im Dashboard auch weitere Ansichten anlegen {hier noch link zum Kapitel einfügen}, die für Benutzer:innen sichtbar sein sollen. Die Zuweisung dieser Sichtbarkeit kannst du pro Rolle oder pro Benuter:in festlegen. Weitere Informationen und eine Anleitung für die Zuweisung pro Rolle oder Benutzer:in findest du auf der Handbuchseite <<business-entscheidungen/benutzerkonten-zugaenge#116, Benutzer:innen-Konten und Zugänge>>.
...
Kennzahlen lassen sich auf dem Dashboard auch in Tabellenform übersichtlich darstellen. Grundsätzlich stehen für Tabellen dieselben Einstellungsmöglichkeiten wie für Kacheln (<<table-dashboard-tile-settings>>) zur Verfügung. 
...
. Öffne das Menü *plentymarkets Logo (Start) » Dashboard (beta)*.
. Klicke auf *Ansicht bearbeiten* icon:design_inline_edit[set=plenty].
. Platziere das Element *Portlet für Benachrichtigungen* per Drag-and-drop auf dem Dashboard.
. *Speichere* icon:save[set=plenty] die Einstellungen.