...
Das Dashboard erlaubt dir viel Flexibilität beim Anordnen der Inhalte. Grundsätzlich platzierst du Darstellungsbereiche und Elemente per Drag-and-Drop auf dem Dashboard. Jeder Bereich und jedes Element kann per Klick auf icon:design_inline_edit[set=plenty] weiter bearbeitet werden. <<table-dashboard-edit-mode>> enthält einen Überblick über die Funktionen, die im generellen Bearbeitungsmodus des Dashboards zur Verfügung stehen.
