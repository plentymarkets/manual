...
. Platziere die gewünschten Elemente.
. *Speichere* icon:save[set=plenty] die Einstellungen. +
...
Überlege dir, welche Benutzer:innen die Standardansicht sehen dürfen. Du kannst im Dashboard auch weitere Ansichten anlegen, deren Sichtbarkeit du dann auf Ebene von Benutzer:innen und Rollen eingrenzen kannst.

Wenn du bei einer <<#407, Dashboard-Ansicht>> den Bearbeitungsmodus (icon:design_inline_edit[set=plenty]) aktivierst, gelangst du über die Schaltfläche *Rechteverwaltung* zu den Einstellungen für Benutzer:innen und Rollen.

Weitere Informationen und eine Anleitung für die Zuweisung pro Rolle oder Benutzer:in findest du auf der Handbuchseite <<business-entscheidungen/benutzerkonten-zugaenge#112, Benutzer:innen-Konten und Zugänge>>.
