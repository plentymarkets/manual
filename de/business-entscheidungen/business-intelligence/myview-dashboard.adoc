...
* Umsatzsteuer gesamt +
* Nachrichten +
* Marktplatz Warenbestandsbewegungen
...
Füge zunächst <<business-entscheidungen/business-intelligence#420, wie oben beschrieben>> ein *Diagramm* hinzu. Gehe danach wie folgt vor, um individuelle Einstellungen für das Kuchendiagramm vorzunehmen.

[.instruction]

. Klicke an der oberen rechten Ecke des Kuchendiagramms auf *Bearbeiten* icon:edit[set=plenty]. +
→ Das Einstellungsmenü öffnet sich.
. Nimm die Einstellungen für das Kuchendiagramm vor. <<table-dashboard-pie-chart-settings>> enthält weitere Informationen zu den verfügbaren Optionen.
. *Speichere* icon:save[set=plenty] die Einstellungen.
...
| *Titel*
| Gib einen aussagekräftigen Titel ein. Der Titel wird als Name auf dem Kuchendiagramm angezeigt.
...
//TODO:  zB. Balkendiagramm: aktueller monat als Zeitraum. Dann wird das Ergebnis der kennzahl für den aktuellen Monat einbezogen. Zusätzlich Woche oder Tag einstellen: Dann werden für jeweils diese Intervalle eigene "Balken" u.ä. dargestellt (aufgespalten). Kachel: Zeitraum Monat, Zeitintervall Woche: nur Ergebnis der aktuellen Woche.
