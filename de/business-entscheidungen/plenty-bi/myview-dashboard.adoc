...
:keywords: Statistik, Kundenstatistik, Auftragsstatistik, Amazon Statistik, eBay Statistik, Dashboard, Besucherstatistik, Rohdaten, Kennzahlen, Unternehmenskennzahlen, BI Dashboard, Business Intelligence, Reporting, Kachel, Kacheln, Tortendiagramm, Tortendiagramme, Liniendiagramm, Liniendiagramme, Säulendiagramm, Säulendiagramme, Heatmap, Heatmaps, Tacho-Diagramm, Tacho-Diagramme, Tachodiagramm, Tachodiagramme, 
:position: 10
...
* <<business-entscheidungen/plenty-bi/kennzahlen/verbrauch#, Verbrauch>>
...
Aktiviere bei einer bereits angelegten <<#407, Dashboard-Ansicht>> den Bearbeitungsmodus (icon:design_inline_edit[set=plenty]). Klicke auf die Schaltfläche *Rechteverwaltung*, um zu den Einstellungen für Benutzer:innen und Rollen zu gelangen und die Dashboard-Ansicht freizugeben.
...
Füge zunächst <<#410, wie oben beschrieben>> eine Kachel hinzu. Gehe dann wie folgt vor, um individuelle Einstellungen für die Kachel vorzunehmen.
...
| *Titel*
| Gib einen aussagekräftigen Titel ein. Der Titel wird als Name auf der Kachel angezeigt. Klicke auf (icon:preview[role="skyBlue"]), wenn der Titel überhaupt nicht angezeigt werden soll.
...
| Mit dieser Einstellung kannst du eine individuell konfigurierte Option zum Menü *Aktionen* (icon:more_vert[set=material, role=blue]) auf der Kachel hinzufügen. Du kannst auch bestimmen, unter welchen Voraussetzungen diese Option angezeigt wird und was geschieht, wenn Nutzer:innen darauf klicken. +
...
| Wähle, welche bereits angelegte Dashboard-Ansicht geöffnet werden soll, wenn du an der Kachel auf *Aktionen* (icon:more_vert[set=material, role=blue]) klickst und die Option *Details zur Kachel* klickst. Wählst du die Option *Keine*, wird beim Klick auf *Details zur Kachel* eine vom System vorgegebene Standard-Ansicht geöffnet. Diese enthält weitere Details zum Ergebnis und den Daten der Kennzahl, die als Grundlage für die Kachel gewählt wurde.

| *Parameter für Detailansicht*
| Hast du bei der Einstellung *Link zu detaillierter Dashboard-Ansicht* eine bereits existierende Dashboard-Ansicht ausgewählt? Dann kannst du hier festlegen, nach welchen Kriterien diese Ansicht eingegrenzt wird. Klicke auf *Parameter-Konfiguration* (icon:settings[role=darkGrey]), um eine oder mehrere Kriterien festzulegen. Je nach der Kennzahl, die auf der Kachel dargestellt wird, stehen die Parameter *Lieferland*, *Benutzer* oder *Nachrichten-Tag* zur Verfügung.
// TODO: Später nochmal genauer beschreiben, funktioniert noch nicht richtig laut DB.
...
Füge zunächst <<#410, wie oben beschrieben>> eine Zeile hinzu. Gehe danach wie folgt vor, um individuelle Einstellungen für die Zeile vorzunehmen.
...
Füge zunächst <<#410, wie oben beschrieben>> eine Tabelle hinzu. Gehe danach wie folgt vor, um individuelle Einstellungen für die Tabelle vorzunehmen.
...
Füge zunächst <<#410, wie oben beschrieben>> ein *Diagramm* hinzu. Gehe danach wie folgt vor, um individuelle Einstellungen für das Tortendiagramm vorzunehmen.
...
| *Titel*
| Gib einen aussagekräftigen Titel ein. Der Titel wird als Name auf dem Tortendiagramm angezeigt. Klicke auf (icon:preview[role="skyBlue"]), wenn der Titel überhaupt nicht angezeigt werden soll.
...
Füge zunächst <<#410, wie oben beschrieben>> ein *Säulendiagramm* oder ein *Liniendiagramm* hinzu. Gehe danach wie folgt vor, um individuelle Einstellungen für die Diagramme vorzunehmen.
...
| *Vorlagen für Säulendiagramm*
| Klicke auf diese Schaltfläche, um eine vom System bereitgestellte Vorlage für das Säulendiagramm zu wählen. So zeigt das Säulendiagramm die ausgewählten Daten an, ohne dass du die Daten in weiteren Schritten manuell auswählen musst.
...
| Diese Einstellung zeigt unterhalb des Diagramms zusätzliche Schaltflächen an. Diese erlaube es dir, die im Diagramm dargestellten Daten nach zusätzlichen Zeitinvervallen einzugrenzen. So kannst du unkompliziert zwischen verschiedenen Zeitintervallen wechseln. Aktiviere dazu eine der Optionen *Tag*, *Monat*, *Quartal* oder *Jahr*. +
*_Beispiel:_* Wenn du zuvor über die Option *Zeitintervall* den aktuellen Monat ausgewählt hast, kannst du mithilfe des zusätzlichen Zeitintervalls Schaltflächen anzeigen lassen, die dir per Klick die Daten nach Tagen und Wochen anzeigen.
...
| Aktiviere diese Option, um zusätzlich zu den Balken eine Linie anzuzeigen, die die Durchschnittswerte darstellt. *_Hinweis:_* Diese Option ist nur vorhanden, wenn du mehr als eine Kennzahl für die Darstellung im Säulendiagramm ausgewählt hast.
...
Füge zunächst <<#410, wie oben beschrieben>> eine *Heatmap* hinzu. Gehe danach wie folgt vor, um individuelle Einstellungen für die Heatmap vorzunehmen.
...
Füge zunächst <<#410, wie oben beschrieben>> ein Fortschrittsdiagramm hinzu. Gehe danach wie folgt vor, um individuelle Einstellungen für das Fortschrittsdiagramm vorzunehmen.
...
Tacho-Diagramme eignen sich besonders gut, um darzustellen, in wiefern ein bestimmtes Ziel bereits erreicht wurde. Das Ziel sowie der aktuelle Stand der Kennzahl werden in jeweils unterschiedlichen Farben dargestellt und durch eine Trennlinie begrenzt. Stelle vor Einrichtung eines Tacho-Diagramms sicher, dass an der ausgewählten Kennzahl Zielwerte für die darzustellenden Zeiträume hinterlegt sind. Andernfalls funktioniert die Darstellung nicht. 

Füge zunächst <<#410, wie oben beschrieben>> ein Tacho-Diagramm hinzu. Gehe danach wie folgt vor, um individuelle Einstellungen für das Tacho-Diagramm vorzunehmen.

[.instruction]
Tacho-Diagramm verwenden:
...
| *Kennzahl*
| Wähle aus der Dropdown-Liste eine Kennzahl aus, die in im Tacho-Doagramm dargestellt werden soll. Klicke alternativ auf *Neue Kennzahl erstellen* (icon:plus[role="green"]), um eine neue Kennzahl anzulegen.

| *Titel*
| Gib einen aussagekräftigen Titel ein. Der Titel wird als Name über dem Tacho-Diagramm angezeigt. Klicke auf (icon:preview[role="skyBlue"]), wenn der Titel überhaupt nicht angezeigt werden soll.
...
| *Hauptwert*
| Bei Tacho-Diagrammen ist der Hauptwert an sich nicht veränderbar. Er besteht immer aus dem Ergebnis der Kennzahl, die du für die Darstellung im Tacho-Diagramm ausgewählt hast. Über (icon:settings[set=plenty]) kannst du jedoch bestimmen, wie viele Nachkommastellen für den Wert angezeigt werden sollen.
...
. Klicke an der rechten oberen Ecke des Diagramms auf (icon:more[set-plenty]). +
...
| *Dashboard öffnen*
| Wähle aus der Dropdown-Liste eine Dashboard-Ansicht aus, die beim Klick auf die Schaltfläche geöffnet werden soll. *_Wichtig:_* Das Dashboard muss bereits existieren und Benutzer:innen müssen über die erforderlichen <<#408, Rechte zur Ansicht>> verfügen, um alle benötigten Dashboards sehen und auswählen zu können.
