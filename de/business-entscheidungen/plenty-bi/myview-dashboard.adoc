...
. Aktiviere die Option *Seitennavigation beim Laden ausklappen* und/oder *In Seitennavigation ausblenden*. Beachte die Informationen in <<table-dashboard-visibility>>. +
...
//TODO: Einleitungstext. Wozu ist das gut und nach welcher Logik werden die Trichterelemente angeordnet? Normal, dass ich nur eine Kennzahl auswählen kann? 
...
| *Zeitintervall*
| Diese Einstellung schränkt die Werte aus dem gewählten *Zeitraum* weiter ein. Das Zeitintervall muss also immer kleiner gewählt werden als der *Zeitraum*. +
*_Beispiel:_* Wenn du den aktuellen Monat als *Zeitraum* gewählt hast und für das *Zeitintervall* die Option *Woche*, wird die Aufteilung des Trichterdiagramms durch den Wert der aktuellen Woche bestimmt.
...
//TODO: Einleitungstext. Wozu ist das gut und nach welcher Logik werden die viereckigen Elemente angeordnet? Normal, dass ich nur eine Kennzahl auswählen kann? 

Füge zunächst ein Element vom Typ *Baumstruktur* zum Dashboard hinzu. Gehe danach wie folgt vor, um individuelle Einstellungen für die Baumstruktur vorzunehmen.
...
[.instruction]
Einstellungen für Baumstruktur vornehmen:

. Klicke an der oberen rechten Ecke der Baumstruktur auf *Bearbeiten* (icon:edit[set=plenty]). +
→ Das Einstellungsmenü öffnet sich.
. Nimm die Einstellungen für die Baumstruktur vor.
. *Speichere* (icon:save[set=plenty]) die Einstellungen.
...
| *Titel*
| Gib einen aussagekräftigen Titel ein. Der Titel wird als Name über der Baumstruktur angezeigt. Klicke auf (icon:eye[role="lightGrey"]), wenn der Titel überhaupt nicht angezeigt werden soll.

| *Kennzahlen und Zeitraum*
| Gib im Suchfeld den Namen der Kennzahlkonfiguration ein und wähle den passenden Eintrag, um die Kennzahl in der Baumstruktur darzustellen. Klicke alternativ auf *Neue Kennzahl erstellen* (icon:plus[role="green"]), um eine neue Kennzahlkonfiguration anzulegen.

| *Zeitintervall*
| Diese Einstellung schränkt die Werte aus dem gewählten *Zeitraum* weiter ein. Das Zeitintervall muss also immer kleiner gewählt werden als der *Zeitraum*. +
*_Beispiel:_* Wenn du den aktuellen Monat als *Zeitraum* gewählt hast und für das *Zeitintervall* die Option *Woche*, wird die Aufteilung der Baumstruktur durch den Wert der aktuellen Woche bestimmt.
...
====
Das Element *Globales Zeitintervall* kann pro Dashboard nur ein Mal eingebunden werden. Wir empfehlen dir daher, das Element an einer gut sichtbaren Stelle auf dem Dashboard zu platzieren. In den meisten Fällen bietet sich eine Platzierung ganz oben auf dem Dashboard, oberhalb aller anderen Elemente an.
====
