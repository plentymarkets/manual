...
| *Titel*
| Gib einen aussagekräftigen Titel ein. Der Titel wird als Name innerhalb der Zeile angezeigt. Klicke auf (icon:preview[role="skyBlue"]), wenn der Titel überhaupt nicht angezeigt werden soll.
...
Interner Link: Ein interner Link öffnet ein Menü innerhalb des plentymarkets Backends. Gib dabei die URL des zu öffnenden Menüs beginnend mit einem */* ein. Der Link muss alles beinhalten, was nach dem Bestandteil *terra* in der URL des zu öffnenden Menüs angegeben ist. *_Beispiel_*: Soll durch Klick auf die Zeile die Auftragsübersicht geöffnet werden, gib den Link */order/order-search ein*.
+
Externer Link: Gib den vollständigen Link zur externen Seite ein, beginnend mit *http://*. +
*_Tipp:_* Klicke auf (icon:plus[role="green"]), um weitere Bedingungen für die Einfärbung zu speichern.
...
| *Parameter für Detailansicht*
| Hast du bei der Einstellung *Link zu detaillierter Dashboard-Ansicht* eine bereits existierende Dashboard-Ansicht ausgewählt? Dann kannst du hier festlegen, nach welchen Kriterien diese Ansicht eingegrenzt wird. Klicke auf *Parameter-Konfiguration* (icon:settings[role=darkGrey]), um eine oder mehrere Kriterien festzulegen. Je nach der Kennzahl, die auf der Zeile dargestellt wird, stehen die Parameter *Lieferland*, *Benutzer* oder *Nachrichten-Tag* zur Verfügung.
...
| *Titel*
| Gib einen aussagekräftigen Titel ein. Der Titel wird als Name der Tabelle angezeigt. Klicke auf (icon:preview[role="skyBlue"]), wenn der Titel überhaupt nicht angezeigt werden soll.
...
| *Zugeordnete Spalte*
| Für jede zugeordnete Spalte wird eine weitere Spalte in der Tabelle hinzugefügt. Wähle für jede Spalte eine der Optionen *Berechnungsdatum*, *Dimension*, *Primäres Ergebnis*, *Ergebniseinheit*, *Skalenbasis* oder *Ziel*. Du kannst so bestimmen, welche Werte in der Tabelle dargestellt werden. Soll ein Wert wie beispielsweise das Berechnungsdatum nicht dargestellt werden, ordne die Spalte nicht zu.

| *Primärwert 0 anzeigen*
| //TODO Meeting
...
Das einfache Säulendiagramm erlaubt dir, auf einen Blick den aktuellen Stand der Kennzahl sowie den Zielwert auf einen Blick zu sehen. Jeder der beiden Werte wird in nebeneinander platzierten Säulen dargestellt.

Füge zunächst ein Element vom Typ *Einfaches Säulendiagramm* zum Dashboard hinzu. Gehe danach wie folgt vor, um individuelle Einstellungen für das einfache Säulendiagramm vorzunehmen.
...
. Klicke an der oberen rechten Ecke des einfachen Säulendiagramms auf *Bearbeiten* (icon:edit[set=plenty]). +
→ Das Einstellungsmenü öffnet sich.
. Nimm die Einstellungen für das einfache Säulendiagramm vor.
. *Speichere* (icon:save[set=plenty]) die Einstellungen.

//TODO: Einstellungen prüfen und beschreiben
...
| *X-Achse*
| Wähle, welche Grundlage für die X-Achse verwendet werden soll. Die Option *Skalenbasis* ist als Standardeinstellung vorausgewählt. Wähle bei Bedarf stattdessen die *Dimension* oder das *Berechnungsdatum* aus.

| *Y-Achse*
| Wähle, welche Grundlage für die Y-Achse verwendet werden soll. Die Option *Dimension* ist als Standardeinstellung vorausgewählt.
