...
** sollte nach 5 Monaten neu bestellt werden.
...
* Verlängerung:
** verlängert sich nicht automatisch
** sollte nach Ende der Laufzeit neu bestellt werden.
...
[IMPORTANT]
.Nicht alle SSL-Zertifikate werden im Assistenten bestellt
====
...
Beispiel: Wenn deine Domain bereits ein SSL-Zertifikat im Einrichtungsmenü besitzt, und du dann im Assistenten ein neues Zertifikat bestellst, wird das bestehende Zertifikat durch das neue überschrieben.
====
...
[IMPORTANT]
.Nicht alle SSL-Zertifikate werden im Einrichtungsmenü bestellt
====
...
. Klicke auf den Pfeil nach unten (icon:chevron-down[role="darkGrey"]) ganz weit rechts in der Zeile *Neues SSL bestellen für Domain: [Domain-Name]*. +
...
SSL-Zertifikate werden _nicht automatisch verlängert_.
Stattdessen müssen sie nach Ende der Laufzeit <<business-entscheidungen/systemadministration/ssl-zertifikat_bestellen#1200, neu bestellt werden>>.
...
[.collapseBox]
.*Sectego: OV und EV Zertifikate*
--

. Öffne das Menü *Einrichtung » Assistenten » Grundeinrichtung*.
. Klicke auf den Assistenten *SSL-Verwaltung*. +
→ Deine bereits bestehende SSL-Konfigurationen werden hier angezeigt.
. Klicke auf eine der vorhandenen SSL-Konfigurationen, um ihre Einstellungen zu öffnen.
. Navigiere zum Schritt *Zusammenfassung*.
. Klappe das Feld *SSL Typ* auf (icon:chevron-down[role="darkGrey"]).
. In der Zeile *Aktiv bis* findest du das Datum, an dem das Zertifikat abläuft.
. <<business-entscheidungen/systemadministration/ssl-zertifikat_bestellen#1220, Bestelle das Zertifikat bei Bedarf neu>>.

--

[.collapseBox]
.*AlwaysOn oder COMODO: DV Zertifikate*
--
...
. <<business-entscheidungen/systemadministration/ssl-zertifikat_bestellen#1280, Bestelle das Zertifikat bei Bedarf neu>>.
...
Nach Bestellung eines neuen Zertifikats muss dieses durch einen regelmäßig ausgeführten *Systemprozess* veröffentlicht werden.
*Dies kann bis zu 120 Minuten dauern*.
In Einzelfällen führt die Zertifizierungsstelle eine Qualitätskontrolle durch bevor sie SSL-Zertifikate freischaltet.
Prüfe daher die auf dem Bildschirm angezeigte Bestätigungsnachricht.
Eine Qualitätskontrolle dauert in der Regel bis zu 24 Stunden.
Sollte der Status im Backend nach 24 Stunden nicht "Abgeschlossen" lauten, melde dich im Forum unter Angabe der betroffenen Domain.
