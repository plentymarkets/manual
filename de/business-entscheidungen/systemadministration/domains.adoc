...
Wenn die Domain bereits vorhanden ist, hast du die Möglichkeit, die Domain umzuziehen. Gehe dazu wie im Kapitel <<#70, Neue Domain bestellen>> vor. Du wirst durch das Menü geführt. Beachte außerdem die folgenden Hinweise zum Domain-Transfer:
...
.CNAME speichern
image::business-entscheidungen/assets/DE-ServiceCenter-Domain-AuthCode.png[CNAME speichern]
...
* Erzeuge einen <<#domain-auth-code, Auth-Code>>.
...
. Gib die gewünschten Weiterleitungen in das vorgesehene Feld ein. Pro Zeile kann eine Umleitung definiert werden. Beachte bei der Eingabe die nachfolgende <<#265, Informationen zum Syntax>>.
...
Eine externe Domain kannst du beim externen Hoster belassen und <<#150, die DNS-Einstellungen bei diesem externen Hoster speichern>>. Das SSL-Zertifikat muss jedoch systemnah liegen, also auch wenn die Domain bei einem anderen Hoster liegt, erfolgt die Verschlüsselung erst nach der Weiterleitung über die IP-Adresse bei plentymarkets im System. Daher muss das SSL-Zertifikat bei plentymarkets gebucht werden, da plentymarkets aus Sicherheitsgründen keine externen Zertifikate zulässt.
...
* Die Domain, für die das SSL-Zertifikat bestellt werden soll, muss eine im plentymarkets Service-Center aktivierte <<#210a, Hauptdomain>> sein. +
...
→ Prüfe die Einstellungen im plentymarkets Service-Center. +
→ Wenn die Parent-Domain aus dem Service-Center entfernt wurde, legst du die Parent-Domain neu an. Wenn diese Parent-Domain nicht auf plentymarkets zeigen soll, sondern z.B. auf Shopware, versehe die Parent-Domain mit einem externen A-Record, indem du eine <<#210a, Systemverknüpfung erstellst>>.

==== Bei externen Domains: DNS-Einstellungen

* Die beim <<#150, externen Provider gespeicherten DNS-Einstellungen>> müssen mit den DNS-Einstellungen im plentymarkets Service-Center identisch sein. +
...
.Domain angelegt?
====
Um einen Mandanten (Shop) zu bestellen, benötigst du eine <<#domain, Domain>>. Diese Domain musst du vorher im Tab *Domain* für das Hauptsystem erstellen.
====
...
Im Tab *Mandant (Shop)* werden außerdem Informationen zu den Domains angezeigt, die deinen Mandanten zugeordnet sind. Nimm die Änderungen an diesen Einstellungen wie unter <<#domain, Domain>> vor.
