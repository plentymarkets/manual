...
Da es in seltenen Fällen zu Problemen beim automatischen Artikelexport kommen kann, können Sie Ihren auf französisch hinterlegten Artikelkatalog unter <<datenaustausch/daten-exportieren#, **Datenaustausch » Export » elastischer Export**  >> auch als CSV-Datei an Cdiscount exportieren. Nachdem Sie alle dafür notwendigen Einstellungen wie unten beschrieben vorgenommen und gespeichert haben, müssen Sie die komplette URL kopieren und an Cdiscount übermitteln. Der stündliche Bestandsabgleich erfolgt weiterhin automatisch.
...
. Nehmen Sie die Einstellungen gemäß  der Handbuchseite <<datenaustausch/daten-exportieren#20, **Daten exportieren** >> vor.
. **Speichern** Sie die Einstellungen. +
