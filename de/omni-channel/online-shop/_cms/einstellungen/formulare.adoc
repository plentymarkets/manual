...
**plentymarkets » System » CMS » Formulare**

== Allgemeine Informationen zu Formularen

In diesem Menü legen Sie Formulare an und bearbeiten sie. Ein Beispiel für ein Formular ist das Kontaktformular in Ihrem Webshop. Dort können Webshopbesucher ihre Adressdaten eingeben und einen Hinweis oder eine Frage per E-Mail an die jeweils im Menü **System » Mandant (Shop) » Standard » E-Mail » Zugangsdaten** hinterlegte E-Mail-Adresse schicken. Ein Formular wird mit der Template-Variable **{% Form(X) %}** in Ihren Webshop eingebunden.
