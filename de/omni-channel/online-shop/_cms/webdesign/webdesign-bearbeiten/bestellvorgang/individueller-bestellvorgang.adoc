...
Der dritte Bestellschritt, die Kategorie *Kasse*, stellt einen Sonderfall dar, da er die in <<tabelle-bestellschritte-individueller-bestellvorgang>> beschriebenen vier Bestellschritte beinhaltet. In der Kategorie *Kasse* wird die Abfolge dieser vier Bestellschritte geregelt. Die Kasse wird im Individuellen Bestellvorgang von *Callisto Light* als Kopfzeile und Fortschrittsanzeige genutzt, anhand derer Kunden sehen, in welchem Schritt sie sich befinden (<<bild-bestellschritte-kasse>>).

[[bild-bestellschritte-kasse]]
.Bestellschritte der Kategorie *Kasse*
image::omni-channel/online-shop/_cms/webdesign/webdesign-bearbeiten/bestellvorgang/assets/DE-CMS-IndividuellerBestellvorgang-03.png[]
...
__Codebeispiel 2: Bestellschritte der Kategorie Kasse__
...
Im Ordner Ihres Designs (z.B. callisto_light_3) in der Kategorie *Zahlung &amp; Versand* wird die globale Funktion *Container_CheckoutMethodsOfPaymentList* aufgerufen. Um alle verfügbaren <<auftragsabwicklung/payment/zahlungsarten-verwalten#, Zahlungsarten>> auszugeben, wird in *Callisto Light* das Template *CheckoutMethodsOfPaymentList* in *CMS » Webdesign » Layout » Checkout* verwendet. Es wird eine HTML-Liste mit Radio-Buttons erzeugt, über die Kunden eine Zahlungsart auswählen können (Codebeispiel 3).
...
__Codebeispiel 3: Template *CheckoutMethodsOfPaymentList*__
...
__Codebeispiel 4: Change-Event im Template *CheckoutMethodsOfPayment*__
...
__Codebeispiel 5: Template *ValidatorPlaceOrder*__

[#10000]
== Google reCAPTCHA für Kundenregistrierung einrichten

Richten Sie Google reCAPTCHA für die Kundenregistrierung im Webshop ein. Registrieren Sie Ihren Webshop zunächst für die Verwendung von link:https://www.google.com/recaptcha/admin#list[Google reCAPTCHA^]. Geben Sie anschließend den Websiteschlüssel und den Geheimen Schlüssel für Ihren Webshop ein.

[.instruction]
Google reCAPTCHA einrichten:

. Öffnen Sie das Menü *System » Mandant » Mandant wählen » Webshop » Einstellungen*.
. Wählen Sie *Ja* unter *Google reCAPTCHA in Registrierung abfragen*.
...
. Fügen Sie die Template-Funktion `{% GetCaptcha('normal') %}` bzw. `{% GetCaptcha('compact') %}` oberhalb des Buttons *Registrierung abschließen* ein.
. Suchen Sie den Code `plenty.AuthenticationService.registerCustomer();`.
. Ersetzen Sie den Code durch `registration();`.
. Suchen Sie nach `<script>`.
. Fügen Sie innerhalb des Script-Bereichs das folgende Code-Beispiel ein.
. *Speichern* Sie die Einstellungen.
...
. Öffnen Sie die Kategorie *Registrierung*.
. Wechseln Sie in das Tab *Beschreibung 1*.
. Fügen Sie die Template-Funktion `{% GetCaptcha('invisible') %}` oberhalb des Buttons *Registrierung abschließen* ein.
. Suchen Sie den Code `plenty.AuthenticationService.registerCustomer();`.
. Ersetzen Sie den Code durch `grecaptcha.execute();`.
. Suchen Sie nach `<script>`.
. Fügen Sie innerhalb des Script-Bereichs das folgende Code-Beispiel ein.
. *Speichern* Sie die Einstellungen.
...
. Öffnen Sie das Menü *CMS » Webdesign » Layout » Checkout » CheckoutCustomerInvoiceAddress*.
. Suchen Sie nach `</form>`.
. Fügen Sie die Template-Funktion `{% GetCaptcha('normal') %}` oder `{% GetCaptcha('compact') %}` oberhalb des `</form>`-Tags ein.
. *Speichern* Sie die Einstellungen.
...
. Öffnen Sie das Menü *CMS » Webdesign » Layout » Checkout » CheckoutCustomerInvoiceAddress*.
. Suchen Sie nach `</form>`.
. Fügen Sie die Template-Funktion `{% GetCaptcha('invisible') %}` oberhalb des `</form>`-Tags ein.
. *Speichern* Sie die Einstellungen.
...
. Öffnen Sie das Menü *Artikel » Kategorien*.
. Öffnen Sie den Ordner des Designs, das Sie bearbeiten möchten, z.B. Callisto Light 3.
. Öffnen Sie die Kategorie *Anschrift*.
. Wechseln Sie in das Tab *Beschreibung 1*.
. Suchen Sie den Code `plenty.CheckoutService.registerGuest().done(function()`.
. Ersetzen Sie den 4-zeiligen Code-Block durch das nachfolgende Code-Beispiel.
. *Speichern* Sie die Einstellungen.
