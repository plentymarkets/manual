= Global
:lang: de
include::{includedir}/_header.adoc[]
:keywords: Global
:position: 109

//  auto generated content Thu, 06 Jul 2017 00:59:34 +0200
== Kurzbeschreibung

In diesem Bereich finden Sie **PHP-Funktionen**, welche auch innerhalb der CMS-Syntax verwendet werden können. Diese Funktionen sind sowohl im Bereich **Webdesign** und **Kategorien** als auch in **E-Mail-Vorlagen** verfügbar.

== Inhaltsverzeichnis

* <<omni-channel/online-shop/cms-syntax/global/phpfunctions#, PHPFunctions  >> — Folgende PHP-Funktionen stehen auch innerhalb der CMS-Syntax zur Verfügung. Die Verwendung ist weitestgehend identisch mit der Verwendung direkt in PHP.

