...
Wenn im Menü **System » Mandant (Shop) » Standard » Artikelansichten » Einstellungen** für die Einstellung **Attribut-Verfügbarkeit** die **Client-Prüfung** aktiviert ist, wird durch das nachfolgende Code-Beispiel das zuvor erzeugte Formular je nach Verfügbarkeit der Variante dynamisch ein- und ausgeblendet.
