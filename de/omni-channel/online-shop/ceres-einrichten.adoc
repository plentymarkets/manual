...
[#2040]
==== Schrägstrich am Ende von URLs anpassen.

Im Menü *System » Mandant » Mandant wählen » SEO » URL-Aufbau » Sonstige* können Sie einstellen, ob die URLs der Routen Ihres Webshops mit einem angehängten Schrägstrich (dem sogenannten "trailing slash") angezeigt werden, oder nicht.

[#2041.instruction]
Einstellungen für Schrägstriche am Ende von URLs vornehmen:

. Öffnen Sie das Menü *System » Mandant » Mandant wählen » SEO » URL-Aufbau » Sonstige*.
. Wählen Sie die gewünschte Einstellung aus der Dropdown-Liste *Schrägstrich am Ende von URLs*.
. *Speichern* Sie die Einstellungen.
...
Bitte beachten Sie, dass vermehrte Änderungen an dieser Einstellung negative Auswirkungen auf die Erfassung durch Suchmaschinen haben kann.
