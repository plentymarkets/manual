...
.  Öffnen Sie den Artikel.
.  Wechseln Sie in das Tab *Multi-Channel*.
.  Nehmen Sie die Einstellungen im Bereich *Amazon* vor. Beachten Sie dazu die Erläuterungen in Tabelle 5 im Kapitel <<omni-channel/multi-channel/amazon/amazon-einrichten#3, *Artikelverfügbarkeit einstellen*>>.
. *Speichern* Sie die Einstellungen.
...
.  Öffnen Sie das Menü *System » Märkte » Amazon » Einstellungen*.
.  Wechseln Sie in das Tab *Auftragseinstellungen*.
.  Nehmen Sie die Einstellungen im Bereich *Versand durch Amazon (FBA)* vor. Beachten Sie dazu die Erläuterungen in Tabelle 11.
. *Speichern* Sie die Einstellungen.
