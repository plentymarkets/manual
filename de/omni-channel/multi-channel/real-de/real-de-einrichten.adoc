...
|Artikel mit der Einstellung *Keine Beschränkung* im Tab *<<warenwirtschaft/warenbestaende-verwalten#, Bestand>>* werden in der hier definierten Anzahl auf real.de angeboten. Sobald ein Verkauf stattgefunden hat, wird zur nächsten Stunde die hier hinterlegte Anzahl wieder neu zum Verkauf eingestellt.
...
Für die gewählte Preisspalte muss bei den Artikeln jeweils ein Tiefstpreis eingegeben sein. Der Tiefstpreis muss für die Herkunft real.de freigegeben sein. +
*Wichtig*: Die Tiefstpreisautomatik greift nicht bei Variantenartikeln mit Aufpreisen für Attribute. In diesem Fall diese Automatik auf *deaktiviert* einstellen oder jede Variante als separaten Artikel mit definierter Preisspalte für die Tiefstpreisautomatik erstellen, wenn die Tiefstpreisautomatik genutzt werden soll.
...
*Wichtig*: Sie müssen jedem Ihrer Dienstleister einen Dienstleister aus der Dropdown-Liste zuordnen.
...
Bei real.de haben Sie die Möglichkeit, Attribute durch plenty-Merkmale zu überschreiben oder real-Attribute hinzuzufügen. Damit diese im real.de-Export erscheinen, müssen diese denselben Namen haben wie in der Dokumentation von real.de vorgegeben. Die real.de-Attribute ean (EAN), title (Titel), description (Beschreibung), manufacturer (Hersteller) und picture (Bild) sind für den Artikelexport Pflichtangaben und werden standardmäßig von plenty übertragen. Diese könnten nur durch ein Merkmal überschrieben werden, andere Attribute wie z.B. "material_compostion", "clothing_size" oder "weight" können entsprechend hinzugefügt werden, wenn ein entsprechendes Merkmal mit Namen in plentymarkets angelegt wird.
...
. Geben Sie *ean* unter *Name (intern)* ein.  +
...
Aktivieren Sie die <<auftragsabwicklung/payment/zahlungsarten-verwalten#, Zahlungsart>> *real.de Payment* wie im Folgenden beschrieben.
...
Richten Sie eine <<basics/automatisierung/ereignisaktionen#, Ereignisaktion>> ein, damit Rechnungen für real.de automatisch erstellt werden.
...
Richten Sie eine <<basics/automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um real.de automatisch über Stornierungen zu informieren. Bei Stornierungen muss ein Grund für die Stornierung an real.de übertragen werden. Erstellen Sie im Menü <<auftragsabwicklung/auftraege/auftraege-verwalten#, System » Aufträge » Auftragsstatus>> ggf. zusätzliche Auftragsstatus. Folgende Stornierungsgründe können an real.de übertragen werden:
...
Wenn Sie Stornierungen für real.de in plentymarkets über eine Ereignisaktion erstellen, wird bei real.de eine Gutschrift ausgelöst.
...
.Retouren bei real.de
====
Im Gegensatz zu anderen Marktplätzen können Retouren für real.de nicht mit einer Ereignisaktion über plentymarkets gesendet werden. Retouren für real.de bearbeiten Sie direkt in Ihrem real-Händlerkonto.
====
