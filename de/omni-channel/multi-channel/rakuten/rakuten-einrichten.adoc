...
[#2000]
== Vorgänge über Log und API-Log abrufen

In den Menüs *Daten » Log* und *Daten » API-Log* finden Sie eine Übersicht über Vorgänge, die über die Rakuten-Schnittstelle ausgeführt wurden.
...
|*SendOrderStatusInLogistics* = Ereignisaktion *Versandvorbereitung an Rakuten melden* +
*SendOrderStatusShipped* = Ereignisaktion *Versandbestätigung an Rakuten senden* +
*SendOrderTrackingData* = Ereignisaktion *Sendungsverfolgung an Rakuten senden* +
*SendOrderStatusCanceled* = Ereignisaktion *Stornierung an Rakuten senden* +
*SendOrderStatusReturned* = Ereignisaktion *Retoure an Rakuten senden*
...
Im Menü *Daten » API-Log » Tab: API-Log* finden Sie eine Historie über alle anderen Vorgänge, die über die Rakuten-Schnittstelle ausgeführt wurden. Das Format *Rakuten* steht Ihnen für Rakuten zur Verfügung.
