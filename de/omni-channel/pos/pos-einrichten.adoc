...
Um eine Kasse mit Standort Österreich dauerhaft außer Betrieb zu nehmen, erstellen Sie einen Endbeleg. Ein Endbeleg ist ein signierter Kontrollbelege mit einem Betrag von Null (0) Euro, der nur erstellt werden kann, wenn für die Kasse auch ein Startbeleg erstellt wurde. Den Endbeleg erstellen Sie im POS-Menü der plentymarkets App. Nachdem ein Endbeleg für die Kasse erstellt wurde, kann die Kasse nicht mehr verwendet werden.
