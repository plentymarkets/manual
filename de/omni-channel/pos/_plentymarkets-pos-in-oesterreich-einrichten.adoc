...
**__Hinweis:__** In vielen Ländern ist die Adresse eine Pflichtangabe, um ein rechtssicheres Arbeiten mit der Kasse zu ermöglichen. Genaue Angaben mit einem Steuerberater klären. +
...
In plentymarkets werden alle Kassenvorgänge dokumentiert, um ein rechtssicheres Arbeiten zu ermöglichen. Im Menü **Aufträge » POS » Kassenjournal** können Sie Kassenvorgänge suchen, anzeigen und bei Bedarf exportieren. Mit den Filtern des Tabs **Filter** grenzen Sie die im Kassenjournal angezeigten Kassenvorgänge anhand bestimmter Kriterien ein. Die gefundenen Kassenvorgänge werden im Tab **Kassenjournal** aufgelistet. Die Ergebnisse sind standardmäßig nach dem Datum sortiert. Gehen Sie wie unten beschrieben vor, um Kassenvorgänge im Kassenjournal zu suchen.
...
Laut §7 der österreichischen Registrierkassensicherheitsverordnung (RKSV) muss jede Registrierkasse über ein Datenerfassungsprotokoll (DEP) verfügen. Im DEP sind mindestens die Signierung von Belegen im Zusammenhang mit Barumsätzen und die Stornierung von Barumsätzen zu speichern. Die Unveränderbarkeit der Daten ist dabei durch die Signaturverkettung gegeben. Pro DEP darf nur eine Kasse abgebildet werden.
...
Das Datenerfassungsprotokoll einer Registrierkasse muss dabei jederzeit auf einen externen Datenträger im vorgesehenen Exportformat exportiert werden können. Gehen Sie wie unten beschrieben vor, um einen DEP-Export zu erstellen:
