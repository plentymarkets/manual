...
*_Tipp:_* Wenn im Ziffernblock der Kasse der Button *X* eingeblendet ist, können Sie die Menge eingeben und dann den Artikel scannen oder einen Favoritenartikel wählen. Der Artikel wird dann mit der eingegebenen Menge in den POS-Warenkorb gelegt. Diese Komfortfunktion muss ein Administrator für die Kasse aktivieren.
...
*_Tipp:_* Wenn unten rechts auf dem Ziffernblock der Button *Passend* angezeigt wird, können Sie stattdessen auf diesen Button tippen, wenn Kunden den Betrag passend in bar zahlen. Die erhaltene Summe wird dann automatisch in das Feld *Erhalten* eingefügt. Diese Komfortfunktion muss ein Administrator für die Kasse aktivieren.
