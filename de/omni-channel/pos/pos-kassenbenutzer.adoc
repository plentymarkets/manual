...
*_Hinweis:_* Die Gutscheine werden in derselben Reihenfolge eingelöst, in der Sie die Gutscheincodes eingeben. Wenn Sie einen Gutscheincode wieder entfernen, wird die Restsumme neu berechnet.
...
[#xxx]
=== Bestellungen anderer Herkünfte abschließen (ab Version 1.xx)

Wenn Ihr Unternehmen nicht nur ein Ladengeschäft betreibt, sondern auch einen Webshop hat und/oder Artikel auf einem Marktplatz wie Amazon oder eBay verkauft, können Kundinnen und Kunden Waren auch über diese anderen Verkaufskanäle bestellen und in Ihrem Ladengeschäft entweder online bezahlen und im Laden abholen oder eine Online-Bestellung im Laden abholen und bezahlen. Es können also die folgenden Fälle auftreten:

* Der Auftrag wurde online bezahlt. Die Ware wird im Laden nur abgeholt.
* Der Auftrag wurde online bestellt. Der Auftrag wird im Laden bezahlt und die Ware abgeholt.

Für Aufträge anderer Herkünfte gelten die folgenden Einschränkungen:

* Nur Zahlungen anlegen und Notizen speichern, alle anderen Kassenvorgänge sind gesperrt.
* Aufträge können noch nicht mit Verkaufsgutscheinen bezahlt werden.
* Nicht stornierbar.
* Auftragsdaten sind nicht änderbar.

==== Zahlung für Aufträge anderer Herkünfte buchen

* Eine Kundin hat online einen oder mehrere Artikel bestellt. Sie kommt in den Laden, um die Ware abzuholen und vor Ort zu bezahlen.

[.instruction]
Zahlung für externe Aufträge buchen:

. Tippen Sie oben rechts im POS-Warenkorb auf den Menüpunkt mit den drei Punkten. +
...
. Tippen Sie auf *Suchen*. +
*_Wichtig:_* Vergewissern Sie sich, dass die Kundin wirklich die Person ist, die den Auftrag erstellt und bezahlt hat. +
...
. Tippen Sie auf *Auftrag abschließen*. +
→ ????Die Ware wird automatisch aus dem Lager ausgebucht. +
???? *_Hinweis:_* Wenn Sie stattdessen auf *Auftrag abschließen* auf *Neuer Auftrag* tippen, wird der Warenausgang nicht gebucht.


==== Aufträge nur bezahlen
...
====
Die Kasse zeigt an, dass die Kundin die Ware erhalten hat, wenn im plentymarkets System für den Auftrag ein Warenausgang gebucht wurde. Falls Probleme auftreten und die Kundin darauf besteht, die Ware nicht erhalten zu haben, muss der Auftrag im plentymarkets Backend geprüft werden. Möglicherweise liegen  Fehler in den Automatismen vor, über die Aufträge abgearbeitet werden.
====




Kassenjournal: typ "zahlung"
...
JEZT NEU IM KASSENJOURNAL BEI MEHREREN ZAHLARTEN:
bei mehreren zahlarten werden diese einzeln in der spalte zahlungsart aufgeführt
-> betrag wird auch separat aufgeschlüsselt

externe aufträge: nur zahlungen anlegen und notizen speichern, alles andere ist gesperrt.
gutscheinzahlung geht auch erstmal nicht.
nix löschen, noch dazu packen, etc.
nicht stornierbar

aus auftrag raus: entweder zurück oder *...* und dann *Reset*
...
==== Bezahlte externe Aufträge an Kunden aushändigen

* Ein Kunde hat online einen oder mehrere Artikel bestellt und online bezahlt. Er kommt in den Laden, um die Ware abzuholen. Die Ware ist bereits vollständig bezahlt.

[IMPORTANT]
.Identität prüfen
====
Erkundigen Sie sich, welche Richtlinien es in Ihrem Ladenbeschäft gibt, um sicherzustellen, dass Ware, die online bestellt und bezahlt wurde, auch wirklich an die richtige Person ausgehändigt wird.
====
...
. Tippen Sie auf *Suchen*. +
→ Die Auftragsdaten werden in die Kasse geladen. +
*_Wichtig:_* Vergewissern Sie sich, dass der Kunde wirklich die Person ist, die den Auftrag erstellt und bezahlt hat.
...
*_Hinweis:_* Wenn der Auftrag mit mehr als einer Zahlungsart bezahlt wurde, Stornieren Sie jede Zahlart einzeln.
...
. *_Optional:_* Fügen Sie eine <<omni-channel/pos/pos-kassenbenutzer#100, Notiz zur Retoure hinzu>>.
...
. *_Optional:_* Wählen Sie einen anderen Ziellagerort für den Artikel: +
 ...
 → Der Ziellagerort wird geändert. +
 *_Tipp:_* Über die Option *Ziellagerorte ändern* im Menü oben rechts ändern Sie die Ziellagerorte aller Retourenartikel gleichzeitig.
 . *_Optional:_* Passen Sie <<omni-channel/pos/pos-kassenbenutzer#140, den Preis des Artikels an>>.
 . *_Optional:_* Fügen Sie eine <<omni-channel/pos/pos-kassenbenutzer#100, Notiz zur Retoure hinzu>>.
