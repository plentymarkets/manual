...
Eine Zahlung über externes EC-Terminal führen Sie durch, wenn das EC-Terminal nicht mit plentymarkets POS verbunden ist. Da keine Verbindung zwischen plentymarkets POS und dem EC-Terminal besteht, sendet plentymarkets POS keine Auftragsdaten an das EC-Terminal. Deshalb geben Sie den Betrag selbst am EC-Terminal ein. Nachdem Sie die Zahlung am EC-Terminal durchgeführt haben, bestätigen Sie in plentymarkets POS manuell, dass der Auftrag bezahlt wurde. Gehen Sie wie unten beschrieben vor, um eine Zahlung über ein externes EC-Terminal entgegenzunehmen.
...
====
Wenn Sie auf das Symbol *Rechnungskauf* tippen, wird der Auftrag abgeschlossen und der gesamte offene Betrag als Rechnungsbetrag übernommen. Wenn die Kundin einen Teil des Auftrags per Rechnung zahlen möchte, buchen Sie deshalb erst alle anderen Zahlungsarten und wählen Sie erst dann die Option *Rechnungskauf*, um den Auftrag abzuschließen.
====

Nachdem für einen Auftrag eine erste Zahlung gebucht wurde, sind die folgenden Änderungen nicht mehr möglich:

* Preise, Rabatte und Artikel im Warenkorb können nicht mehr angepasst werden.
...
→ Der Auftrag wird erstellt. +
→ Ein Beleg wird gedruckt. Alternativ tippen Sie auf *Beleg drucken*, um den Beleg zu drucken. +
→ Wenn ein Teil des Auftrags per Rechnung bezahlt wird, wird statt eines Kassenbelegs ein Zahlungsbeleg erstellt. +
