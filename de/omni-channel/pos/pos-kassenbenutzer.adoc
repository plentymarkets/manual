...
Wenn im plentymarkets Backend das Plugin POS Kauf auf Rechnung aktiviert ist, können Sie Artikel auch auf Rechnung verkaufen. Kunden können Artikel nur auf Rechnung kaufen, wenn ihre Kundendaten im System gespeichert sind, da über plentyPOS keine neuen Kundendaten erfasst werden können.
...
*_Wichtig:_* Stellen Sie sicher, dass die Kundin wirklich die in den Kundendaten gespeicherte Person ist. +
...
Wenn Kunden einen oder mehrere Artikel zurückgeben möchten, haben Sie zwei Möglichkeiten, einer Retoure zu buchen:

* Sie rufen den Originalauftrag auf und führen die Retoure über diesen ursprünglichen Hauptauftrag durch (ab Version x.xxx).
* Wenn der Hauptauftrag nicht bekannt ist oder nicht geöffnet werden kann, erstellen Sie eine Retoure ohne Hauptauftrag.

=== Retoure mit Zuordnung zum Hauptauftrag buchen (ab Version x.xxx)

Wenn die Auftrags-ID oder die Belegnummer (externe Auftragsnummer) des ursprünglichen Hauptauftrags bekannt ist, rufen Sie diesen Hauptauftrag auf und entfernen die Artikel aus dem Auftrag, die der Kunde zurückgibt. Dadurch werden eine Retoure und eine Gutschrift erstellt.

Gehen Sie wie unten beschrieben vor, um eine Retoure mit Zuordnung zum Hauptauftrag zu buchen.
...
*_Tipp:_* In grau angezeigte Artikelpositionen wurden bereits zurückgegeben.
...
. Händigen Sie dem Kunden den Retourenbetrag aus.


==== Teilretoure: Bestimmte Artikel eines Auftrags zurücknehmen
...
[.instruction]
Bestimmte Artikel eines Auftrags zurücknehmen:
...
. Tippen Sie auf *Suchen*. +
→ Die Auftragsdaten werden angezeigt. +
*_Hinweis:_* Beim Öffnen des Auftrags werden alle Auftragspositionen für die Retoure vorgemerkt. Sie müssen also die Auftragspositionen entfernen, die nicht zurückgenommen werden sollen.
. Um die gesamte Menge einer Auftragsposition aus der Retoure zu entfernen, tippen Sie auf den Papierkorb neben der Auftragsposition.
. Um eine Teilmenge einer Auftragsposition aus der Retoure zu entfernen, tippen Sie auf die Auftragsposition. +
→ Weitere Optionen werden eingeblendet.
. Tippen Sie auf *Menge ändern*. +
→ Das Fenster *Menge ändern* wird angezeigt.
. Geben Sie die Anzahl des Artikels ein, die zurückgegeben wird. +
→ Die Menge der Artikelposition wird aktualisiert.



==== Preis eines Retourenartikels ändern

In einigen Fällen möchten Sie den Kaufpreis eines Artikels nicht vollständig erstatten, zum Beispiel, weil der Artikel beschädigt ist. In einem solchen Fall passen Sie den Preis der Auftragsposition manuell an. Der Preis einer Auftragsposition kann sowohl bei vollständigen Retouren als auch bei Teilretouren geändert werden.


. *_Optional:_* Wenn dem Kunden nicht der volle Kaufpreis erstattet werden soll, ändern Sie den Preis einer Auftragsposition.

  .. Tippen Sie auf die Auftragsposition. +
  → Weitere Optionen werden eingeblendet.
  .. Tippen Sie auf *Einzelpreis ändern*, um den Preis einer Auftragsposition zu ändern, oder auf *Gesamtpreis ändern*, wenn die gesamten Menge der Auftragsposition angepasst werden soll. +
  → Der Preis wird aktualisiert.
...
In Fällen, in denen der Hauptauftrag nicht bekannt ist oder nicht aufgerufen werden kann, bietet plentymarkets POS auch die Möglichkeit, eine Retoure zu erstellen, die keinem Hauptauftrag zugeordnet ist. Gehen Sie wie unten beschrieben vor, um eine Retoure ohne Zuordnung zum Hauptauftrag zu buchen.
...
. Händigen Sie der Kundin den Retourenbetrag aus.
