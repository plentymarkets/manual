...
Jeder Verkaufsgutschein entspricht einem einzigartigen Gutschein-Code. Dieser Gutschein-Code wird auf dem Beleg ausgegeben, wenn Sie einen Gutscheinartikel verkaufen. Damit Ihre Kunden einen Gutschein-Code einlösen können, muss dieser Gutschein-Code zum Zeitpunkt des Einlösens im System gespeichert sein. Abhängig von den Abläufen in Ihrem Ladengeschäft generieren Sie Gutschein-Codes entweder automatisch beim Verkauf des Gutscheins oder Sie arbeiten mit Gutschein-Code-Listen, die Ihnen Ihr Arbeitgeber zur Verfügung stellt. Diese Codes sind entweder bereits im System vorhanden oder Sie speichern sie beim Verkauf im System. Erkundigen Sie sich bei Ihrem Arbeitgeber, wie Sie beim Verkauf von Gutscheinen vorgehen sollten.
...
→ Der Gutschein-Code wird auf dem Kassenbeleg ausgegeben.
. Händigen Sie dem Kunden den Kassenbeleg aus.
. Dokumentieren Sie, welchen Gutschein-Code Sie verkauft haben. + **__Hinweis:__** Jeder Gutschein-Code darf nur einmal verkauft werden, da der Betrag nur einmal eingelöst werden kann.
...
Auch Bargeldbewegungen aus und in die Kasse, die nicht durch das Buchen von Aufträgen entstehen, müssen erfasst werden. Wenn Bargeld aus der Kasse entnommen wird, spricht man von einer Entnahme. Bargeld wird zum Beispiel aus der Kasse genommen, um es auf der Bank einzuzahlen. Wird Bargeld in die Kasse gelegt, zum Beispiel um ausreichend Wechselgeld vorzuhalten, handelt es sich um eine Einlage. Gehen Sie wie unten beschrieben vor, um Einlagen und Entnahmen zu buchen.

[#190]
=== Einlage buchen

Gehen Sie wie unten beschrieben vor, um eine Einlage zu erfassen.
...
. Wählen Sie die Art des Einlagevorfalls aus der Dropdown-Liste.
...
Gehen Sie wie unten beschrieben vor, um eine Entnahme zu erfassen.
...
. Tippen Sie auf **Einlagen und Entnahmen**. +
→ Das Menü **Einlage/Entnahme** wird eingeblendet.
. Wählen Sie die Art des Entnahmenvorfalls aus der Dropdown-Liste.
...
**__Hinweis:__** Wenn der Menüpunkt ausgegraut ist, haben Sie nicht das Recht, Zwischenberichte zu erstellen.
...
**__Hinweis:__** Wenn der Menüpunkt ausgegraut ist, haben Sie nicht das Recht, Tagesberichte zu erstellen.
...
Hierbei handelt es sich lediglich um Empfehlungen zur Einhaltung der RKSV. Erkundigen Sie sich außerdem bei Ihrer Vorgesetzten darüber, wie Sie bei einem Ausfall genau vorgehen sollen.
