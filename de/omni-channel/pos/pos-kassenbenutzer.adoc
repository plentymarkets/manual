...
[#430]
=== Mehrere Zahlungsarten entgegennehmen (ab Version 1.8.x)

Ab Version 1.8.x der plentymarkets App können Kunden denselben Auftrag auch mit mehreren Zahlungsarten bezahlen. So können Kunden, die nicht genug Bargeld dabei haben, zum Beispiel einen Teil des Auftragswerts in bar und einen Teil mit EC-Karte zahlen.

[IMPORTANT]
.Bei Teilbeträgen Rechnungskauf immer zuletzt
====
Wenn Sie auf das Symbol *Rechnungskauf* tippen, wird der Auftrag abgeschlossen und der offene Restbetrag als Rechnungsbetrag übernommen. Wenn Kunden einen Teil des Auftrags per Rechnung zahlen möchten, buchen Sie deshalb erst alle anderen Zahlungsarten und wählen Sie dann erst die Option *Rechnungskauf*, um den Auftrag abzuschließen.
====
...
. Tippen Sie auf den Button für die Zahlungsart, in der Sie den Betrag erhalten haben. +
*_Tipp:_* Wenn Sie mehr als eine Barzahlung buchen, werden die Teilsummen addiert. +
→ Die nächste Zahlung wird gebucht. +
. Nehmen Sie bei Bedarf weitere Zahlungen entgegen, bis der Auftrag bezahlt ist. +
