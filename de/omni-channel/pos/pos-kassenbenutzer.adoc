...
====
Sie bleiben angemeldet, bis Sie im Hauptmenü der App oder in der Liste der Benutzer auf *Ausloggen* oder *Sperren* tippen. Sie können also die App schließen und öffnen, ohne sich erneut anmelden zu müssen.
====
...
Wenn Sie sich von der Kassen entfernen, aber weiterhin eingeloggt bleiben möchten, sperren Sie den Zugriff auf die App. Gehen Sie wie unten beschrieben vor, um den Zugriff auf die App zu sperren und zu entsperren.
...
*_Tipp:_* Auch bei gesperrtem App-Zugriff können Sie den Benutzer wechseln. Tippen Sie auf ein anderes Benutzerkonto in der Liste, um das Benutzerkonto zu wechseln.

[#390]
=== Abmelden (ab Version 1.8.3)

Gehen Sie wie unten beschrieben vor, um sich von der plentymarkets App abzumelden und die App zu beenden. Über die Option *Aktuellen Benutzer automatisch wieder einloggen* bestimmen Sie, ob beim nächsten Öffnen der App der letzte Benutzer automatisch wieder eingeloggt wird.




==== Abmelden und Anmeldedaten vergessen
...
→ Der Benutzer wird vollständig ausgeloggt und die Daten vergessen. +
→ Die Login-Ansicht wird angezeigt. +
→ Bei der nächsten Anmeldung müssen Sprache, Basis-URL und Benutzerdaten erneut eingegeben werden.


==== App beenden und Benutzerdaten speichern

[.instruction]
Über die Benutzerliste abmelden und App beenden:


. Wählen Sie eine Einstellung für die Option *Aktuellen Benutzer automatisch wieder einloggen*. Beachten Sie Tabelle <<tabelle-POS-session>>.
. Tippen Sie auf *Ausloggen*.

[[tabelle-POS-session]]
.Einstellung *Aktuellen Benutzer automatisch wieder einloggen*
[cols="1,3"]
|====
|Einstellung |Erläuterung

|*Aktuellen Benutzer automatisch wieder einloggen*
|Nicht aktiviert (Standard) = Der Benutzer wird vollständig ausgeloggt und die Daten vergessen. Die Login-Ansicht wird angezeigt. Bei der nächsten Anmeldung müssen Sprache, Basis-URL und Benutzerdaten erneut eingegeben werden. +
Aktiviert = Die App wird beendet. Der aktive Benutzer wird auf dem Server ausgeloggt. Bei der nächsten Anmeldung wird dieser Benutzer automatisch wieder eingeloggt. Beim Start der App muss keine PIN und kein Passwort eingegeben werden. +
