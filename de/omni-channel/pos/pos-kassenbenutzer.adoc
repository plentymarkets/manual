...
. Geben Sie Ihre Login-Daten ein. Beachten Sie <<tabelle-POS-kassenpersonal-anmeldung>>.
...
[#310]
=== Benutzerkonto wechseln (ab Version 1.xx)

* In der Benutzerliste werden die realen Namen aller Personen angezeigt, für die im plentymarkets Backend eine App-PIN im Benutzerkonto gespeichert ist.
* Wenn Sie sich zum ersten Mal bei der App anmelden, müssen Sie immer Ihr Passwort eingeben. Das gilt auch, wenn Sie die App zum ersten Mal über die Funktion *Schneller Benutzerwechsel* öffnen. (Umformulieren als Tipp -> Wenn Sie bisher noch nicht können Sie trotzdem)
* Der schnelle Benutzerwechsel funktioniert nur, wenn bereits ein Benutzer eingeloggt ist.
* Wenn das Benutzerkonto gewechselt wird, bevor ein Auftrag abgeschlossen ist, kann der Auftrag geparkt werden.
* Benutzerwechsel ist in allen Bereichen der plentymarkets App möglich. Wenn das Benutzerkonto, in das gewechselt werden soll, ein Bereich keine Rechte hat, dann XXXXX.
* Das Benutzersymbol enthält die Initialen des realen Namens der Person. Die Hintergrundfarbe des Symbols ist die Farbe, die für das Benutzerkonto im plentymarkets Backend gespeichert wurde.
...
. Wenn ein Auftrag noch nicht abgeschlossen ist, haben Sie die Möglichkeit, den Auftrag zu parken. (????) +
*_Tipp:_* Wenn Sie sich zum ersten Mal in der App anmelden, müssen Sie statt der PIN Ihr Benutzerpasswort eingeben. Erst ab dem zweiten Mal funktioniert die Anmeldung per PIN. +
→ Ihr Benutzerkonto wird geöffnet. +
→ Der Bereich der plentymarkets App wird geöffnet, in dem der Wechsel des Benutzerkontos ausgelöst wurde. +
*_Tipp:_* Wenn Sie für den Bereich keine Berechtigungen haben, wird das Hauptmenü geöffnet.
...
[#160]
==== Verkaufsgutschein entgegennehmen
...
. Legen Sie die Artikel, die der Kunde kaufen möchte, in den POS-Warenkorb.
...
* Sie rufen den Originalauftrag auf und führen die Retoure über diesen ursprünglichen Hauptauftrag durch (ab Version 1.8).
* Wenn der Hauptauftrag nicht bekannt ist oder nicht geöffnet werden kann, erstellen Sie eine Retoure ohne Hauptauftrag.
...
=== Retoure mit Zuordnung zum Hauptauftrag buchen (ab Version 1.8)

Wenn die Auftrags-ID oder die Belegnummer (externe Auftragsnummer) des ursprünglichen Hauptauftrags bekannt ist, rufen Sie diesen Hauptauftrag auf und wickeln die Retoure über diesen Auftrag ab. Gehen Sie wie unten beschrieben vor, um eine Retoure mit Zuordnung zum Hauptauftrag zu buchen.


[NOTE]
.Verkaufsgutscheine sind von Retoure ausgeschlossen
====
...
→ Im plentymarkets Backend wird eine Retoure und eine Gutschrift erstellt und mit dem Hauptauftrag verknüpft.
...
Wenn Sie Kunden Anzahlungen oder den Kauf auf Rechnung anbieten kann es vorkommen, dass Kunden Artikel aus Aufträgen zurückgeben möchten, die sie noch nicht oder nur teilweise bezahlt haben. plentymarkets POS berechnet automatisch, ob Sie dem Kunden Geld auszahlen sollen oder nicht. <<tabelle-POS-Retoure-Rechnung>> verdeutlicht die Berechnung, die im Hintergrund stattfindet, damit Sie sie Kunden bei Bedarf erklären können.
...
* Zurückgegebene Artikel kosten weniger als der noch offene Rechnungsbetrag = Der Kunde bekommt kein Geld zurück. +
...
* Preis der zurückgegebenen Artikel entspricht dem noch offenen Rechnungsbetrag = Der Kunde bekommt kein Geld zurück. +
Eine Gutschrift über den Preis des zurückgegebenen Artikels wird erstellt. +

* Zurückgegebene Artikel kosten mehr als der noch offene Rechnungsbetrag = Der Kunde bekommt die Differenz aus offenem Betrag minus Preis des zurückgegebenen Artikels zurück.  +
...
|Der Kunde bekommt den Betrag zurück, den er bisher bezahlt hat. +
...
. *_Optional:_* Fügen Sie eine Notiz zur Retoure hinzu:
...
→ Im plentymarkets Backend werden eine Retoure und eine Gutschrift ohne Hauptauftrag erstellt.
. Zahlen Sie den Retourenbetrag aus.
