...
→ Der Artikel wird mit einer Menge von 1 in den POS-Warenkorb gelegt. +
→ Die zu zahlende Summe wird aktualisiert. +
*_Tipp:_* Wenn in der untersten Zeile des Ziffernblocks der Kasse der Button *X* eingeblendet ist, können Sie die Menge eingeben und dann den Artikel scannen oder einen Favoritenartikel wählen. Der Artikel wird dann mit der eingegebenen Menge in den POS-Warenkorb gelegt. Ein Administrator für die Kasse muss diese Komfortfunktion aktivieren.
