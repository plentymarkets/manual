...
Eine Zahlung über externes EC-Terminal führen Sie durch, wenn das EC-Terminal nicht mit plentymarkets POS verbunden ist. Da keine Verbindung zwischen plentymarkets POS und dem EC-Terminal besteht, sendet plentymarkets POS keine Auftragsdaten an das EC-Terminal. Deshalb geben Sie den Betrag selbst am EC-Terminal ein. Nachdem Sie die Zahlung am EC-Terminal durchgeführt haben, bestätigen Sie in plentymarkets POS manuell, dass der Auftrag bezahlt wurde. Gehen Sie wie unten beschrieben vor, um eine Zahlung über ein externes EC-Terminal entgegennehmen.
...
Ab Version 1.8.x der plentymarkets App können Kunden einen Auftrag auch mit mehreren Zahlungsarten bezahlen. So können Kunden, die nicht genug Bargeld dabei haben, zum Beispiel einen Teil des Auftragswerts in bar und einen Teil mit EC-Karte zahlen.
...
* Preise, Rabatte und Artikel im Warenkorb nicht mehr angepasst werden.
* Kein Kunde mehr gewählt werden.
* Gutschein noch eingelöst werden, aber....


[.instruction]
Mehrere Zahlungsarten entgegennehmen:

. Geben Sie über das Ziffernfeld den Betrag ein, den Sie von der Kundin erhalten haben.
. Tippen Sie auf *Betrag erhalten*. +
