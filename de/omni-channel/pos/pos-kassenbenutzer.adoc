...
* Retoure dem ursprünglichen Hauptauftrag zuordnen (ab Version x.xxx)
* Retoure ohne Hauptauftrag buchen, wenn der Hauptauftrag nicht bekannt ist

=== Retoure mit Zuordnung zum Hauptauftrag buchen (ab Version x.xxx)

plentymarkets POS bietet außerdem die Möglichkeit, Retouren zu erstellen, ohne sie einem Hauptauftrag zuzuordnen. Gehen Sie wie unten beschrieben vor, um eine Retoure ohne Zuordnung zum Hauptauftrag zu buchen.
...
. Tippen Sie auf *Suchen*. +
→ Die Auftragspositionen und weitere Auftragsdaten werden angezeigt.
[.collapseBox]
. *_Optional:_* Ändern Sie die Menge einer Auftragsposition, wenn der Kunde nicht alle Artikel einer Artikelposition zurückgeben möchte.
  --
  .. Tippen Sie auf die Auftragsposition. +
  → Weitere Optionen werden eingeblendet.
  .. Tippen Sie auf *Menge ändern*, wenn nicht alle Artikel der Auftragsposition zurückgegeben werden sollen. +
  → Das Fenster *Menge ändern* wird angezeigt.
  .. Geben Sie die Anzahl ein, die zurückgegeben werden. +
  → Die Menge der Artikelposition wird aktualisiert.
--
[.collapseBox]
. *_Optional:_* Ändern Sie den Preis einer Auftragsposition, wenn dem Kunden nicht der volle Kaufpreis erstattet werden soll:
  --
  .. Tippen Sie auf die Auftragsposition. +
  → Weitere Optionen werden eingeblendet.
  ...
  --
. Wenn alle Retourenartikel erfasst wurden, tippen Sie auf *Retoure erstellen*. +
...
In Fällen, in denen der Hauptauftrag nicht bekannt oder aufrufbar ist, bietet plentymarkets POS auch die Möglichkeit, eine Retoure zu erstellen, die keinem Hauptauftrag zugeordnet ist. Gehen Sie wie unten beschrieben vor, um eine Retoure ohne Zuordnung zum Hauptauftrag zu buchen.
