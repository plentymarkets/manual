...
Wenn Sie den Kassenbereich verlassen, aber weiterhin eingeloggt bleiben möchten, sperren Sie den Zugriff auf die App. Gehen Sie wie unten beschrieben vor, um den Zugriff auf die App zu sperren.
...
→ Der Zugriff auf die Kasse wird gesperrt. +
→ Ihr realer Name wird als erster Eintrag in der Liste der Benutzerkonten angezeigt.
. Um den Zugriff auf die Kasse zu entsperren, tippen Sie oben rechts auf das Benutzersymbol.
. Tippen Sie auf Ihr Benutzerkonto.
. Geben Sie Ihre PIN ein. +
*_Hinweis:_* Zum Entsperren der Kasse ist eine PIN auch erforderlich, wenn Sie die App schließen und erneut öffnen. +
→ Der Zugriff auf die Kasse wird entsperrt. +
*_Tipp:_* Auch bei gesperrtem Kassenzugriff können Sie den Benutzer wechseln. Tippen Sie auf ein anderes Benutzerkonto in der Liste, um das Benutzerkonto zu wechseln.

[#390]
=== Abmelden und App beenden (ab Version 1.8.3)
...
|Nicht aktiviert (Standard) = Die App wird beendet. Der Benutzer wird vollständig ausgeloggt und die Daten vergessen. +
→ Beim nächsten Login müssen Sprache, Basis-URL und Benutzerdaten neu eingegeben werden. +
Aktiviert = Die App wird beendet. Der aktive Benutzer wird auf dem Server ausgeloggt. Bei der nächsten Anmeldung wird dieser Benutzer automatisch wieder eingeloggt. Beim Start der App muss keine PIN und kein Kennwort eingegeben werden. +
