...
[NOTE]
.Verkaufsgutscheine von Retoure ausgeschlossen
====
...
*_Tipp:_* In grau angezeigte Auftragspositionen wurden bereits zurückgegeben.
...
. Tippen Sie auf *Suchen*. +
→ Die Auftragsdaten werden angezeigt. +
→ Alle Auftragspositionen für die Retoure vorgemerkt.
...
. Tippen Sie auf *Suchen*. +
→ Die Auftragsdaten werden angezeigt. +
*_Hinweis:_* Beim Öffnen des Auftrags werden sämtliche Auftragspositionen für die Retoure vorgemerkt. Sie müssen also die Auftragspositionen entfernen, die nicht zurückgenommen werden sollen.
. Scannen Sie den Barcode der Artikel, die die Kundin zurückgeben möchte. +
→ Sobald Sie einen Artikel scannen, werden alle anderen Artikel aus der Retoure entfernt und der gescannte Artikel wird mit der Menge *1* zur Retoure hinzugefügt. Jeder Artikel, der zurückgegeben werden soll, muss also einzeln gescannt werden.
. Tippen Sie auf *Retoure erstellen*. +
→ Im plentymarkets Backend wird eine Retoure und eine Gutschrift erstellt und mit dem Hauptauftrag verknüpft.
. Händigen Sie dem Kunden den Retourenbetrag aus.
...
Wenn dem Kunden nicht der volle Kaufpreis erstattet werden soll, zum Beispiel, weil Artikel beschädigt sind, ändern Sie den Preis einer Auftragsposition. Der Preis einer Auftragsposition kann sowohl bei einer vollständigen Retoure als auch bei einer Teilretoure geändert werden.
