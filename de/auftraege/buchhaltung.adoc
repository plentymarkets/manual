...
Zolltarifnummern, auch Taric genannt, werden im Handel innerhalb der EU genutzt. Es handelt sich dabei um eine eindeutige, festgelegte Nummer aus 8 Stellen, die einer Ware zugeordnet ist und auf steuerrelevanten Dokumenten mit ausgegeben werden muss.

Zolltarifnummern sind für dich relevant, wenn du beim Verkauf innerhalb der EU Umsatzsteueranmeldungen im Lieferland hast oder die <<auftrage/buchhaltung#550, Lieferschwelle>> im Lieferland überschritten hast. In diesen Fällen oder wenn du weißt, dass du in einem Lieferland die Lieferschwelle in absehbarer Zeit überschreiten wirst, solltest du die Zolltarifnummern rechtzeitig einpflegen. Wie empfehlen, dass du die Einzelheiten zur Besteuerung deiner Waren im EU-Ausland mit deinem Steuerberater klärst.

Damit für Aufträge auch bei abweichenden Besteuerungen im EU-Ausland die richtigen Steuersätze verwendet werden und somit die Auftragsberechnung korrekt verläuft, müssen Zolltarifnummern im System eingegeben und mit den für die verschiedenen Länder hinterlegten Steuersätzen abgeglichen werden.
Grundlegend dafür ist, dass du zunächst die Zolltarifnummern im System pflegst. Die <<artikel/artikel-verwalten#280, Hinterlegung von Zolltarifnummern>> erfolgt an den *Varianten* eines Artikels. Des Weiteren sollten *<<auftrage/buchhaltung#550, Umsatzsteuersätze>>* für die entsprechenden Länder eingerichtet sein. Umsatzsteuersätze lassen sich zwar auch nachtragen, wir empfehlen aber die rechtzeitige Erstellung im System.

Die Tabelle im Menü *Einrichtung » Mandant » Global » Zolltarifnummern* ermöglicht dann die Verknüpfung von Zolltarifnummer und Land. Das bedeutet, dass durch die Auftragslogik geprüft wird, ob an einer Variante eine Zolltarifnummer hinterlegt ist. Diese wird dann mit den Angaben in der Zolltarifnummerntabelle in der globalen Einstellung abgeglichen um zu überprüfen, ob ein abweichender Steuersatz für diese Variante und dieses Lieferland vorhanden ist. Wenn dies so ist, wird der in der Tabelle hinterlegte Steuersatz verwendet. Sollte an der Variante oder in der Tabelle keine Zolltarifnummer hinterlegt sein, wird kein abweichender Steuersatz für die Berechnung verwendet. Dies bedeutet für dich, dass du nur bei _abweichender Besteuerung_ einer Variante die entsprechende Zolltarifnummer in der Tabelle hinterlegen musst.

Ein Beispiel: ???

Zum Import der Zolltarifnummern kannst du ElasticSync verwenden oder die Nummern manuell einpflegen. Wir empfehlen, ElasticSync zu nutzen und nur einzelne Zolltarifnummern oder Änderungen manuell in der Tabelle zu pflegen.
_Beachte_ bei der Eingabe, dass Zolltarifnummern zwar aus 8 Nummern bestehen, aber dennoch auch Nummern bis zu maximal 32 Zeichen Länge hinterlegt werden können. Zudem bestehen Zolltarifnummern nur aus Ziffern, technisch ist es hier aber auch möglich Kleinbuchstaben einzufügen.

Informationen zum Import über ElasticSync findest du auf der Handbuchseite <<daten/daten-importieren/ElasticSync, Daten mit ElasticSync importieren>>. *Hier noch Infos hinzufügen?*

Um die Tabelle manuell zu füllen, gehe wie im Folgenden beschrieben vor.

[.instruction]
Zolltarifnummerntabelle manuell befüllen:

. Öffne das Menü *Einrichtung » Mandant » Global » Zolltarifnummern*. +
. Klicke auf *Neu anlegen* (icon:plus-square[role="green"]). +
...
In der Tabelle werden dir gefundene Variantenverknüpfungen mit einem grün Punkt markiert angezeigt. Das bedeutet, dass diese Zolltarifnummer in einer Variante in Verwendung ist. Wird ein roter Punkt angezeigt, wurde keine Variantenverknüpfung gefunden, die Zolltarifnummer ist dementsprechend in keiner Variante in Verwendung.
...
|*Suche*
|Innerhalb der Tabelle kannst du nach Zuordnungen *suchen*. Dafür stehen dir die Filtern *Zolltarifnummer* oder *Land* zur Verfügung.
