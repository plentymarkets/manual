...
Zolltarifnummern, auch Taric genannt, werden im internationalen Handel genutzt. Es handelt sich dabei um eine eindeutige, festgelegte Nummer aus 8 Stellen, die einer Ware zugeordnet ist und auf steuerrelevanten Dokumenten mit ausgegeben werden muss.
*?externe Verlinkung auf https://www.zolltarifnummern.de/ einfügen?*

- für die korrekte Auftragsberechnung benötigt
- Zolltarifnummer an Varianten (Verlinkung auf Teil von Gretchen) hinterlegen
- Steuersätze (Verlinkung) pflegen; können auch nachgetragen werden, es empfiehlt sich aber, dies vorher zu tun
- Neue UI: bietet einen Abgleich zwischen Steuersatz und der an der Variante hinterlegten Zolltarifnummer; dies kann dann zur Auftragsberechnung genutzt werden
- ist eine globale Einstellung
- Tabelle muss vom Händlern befüllt werden: wird am ehesten über Import stattfinden (Verweis darauf und Verliknung)
- bei Unklarheiten welche Zolltarifnummer etc.: Absprachen mit Steuerberater um sicherzustellen, dass gesetzes-. und vorgabenkonform gearbeitet wird
- neue UI: Zolltarifnummer mit Lieferland und Steuersatz wird angezeigt; Markierung (rot und grün), ob eine Variantenverknüpfung besteht, d.h., ob es in einer Variante in Verwendung ist
- Benutzerrechte: Vergabe erforderlich für Zuordnungen anlegen und löschen (user has edit permission)
- beim Löschen Abfrage: Möchtest du die Zuordnung wirklich löschen?
- Import empfohlen; manuelle Pflege möglich
