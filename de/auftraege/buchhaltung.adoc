...
Erfahre mehr über buchhaltungsrelevante Einstellungen in plentymarkets. Hierzu zählen Umsatzsteuersätze, Währungen, Umrechnungskurse, die Berechnung von Auftragspreisen, Buchungsschlüssel, Erlöskonten, Debitorenkonten, aber auch Dokumente. Die meisten buchhaltungsrelevanten Einstellungen lassen sich in plentymarkets standortspezifisch vornehmen. Lediglich die Berechnung von Auftragspreisen sowie die <<auftraege/buchhaltung#620, Zolltarifnummern>> gelten global für das gesamte System.
...
→ Das Tab *Grundeinstellungen* öffnet sich.
. Wähle für *Umsatzsteuer-ID* eine Option aus dem Dropdownmenü. Beachte die Erläuterungen in <<table-intra-EU-invoices>>.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
====
Steuerliche Informationen zu den europäischen Lieferländern sowie USA und Japan findest du zum Beispiel auf der Website des link:https://www.bzst.de/DE/Home/home_node.html[Bundeszentralamts für Steuern]{nbsp}icon:external-link[].
====
...
|Für Lieferungen aus einem anderen Land nach Großbritannien kann hier ein Schwellenwert eingetragen werden. Der Schwellenwert gibt an, bis zu welchem Betrag ein Auftrag in brutto und mit dem hinterlegten Steuersatz berechnet wird. Der hier eingetragene Schwellenwert bezieht sich auf den Nettowarenwert eines Auftrags, Versandkosten zählen nicht mit rein. Beachte also, dass der _eingegebene Betrag netto_ ist. Aufträge mit einem Nettowarenwert über dem eingetragenen Schwellenwert werden netto berechnet. +
Hintergrund dieser Einstellung ist, dass Großbritannien aus europäischer Sicht ein Drittland ist und dementsprechend andere Ein- und Ausfuhrregelungen gelten. Trage daher keinen Schwellenwert für Lieferungen innerhalb Großbritanniens ein. +
Diese Einstellung wird nur angezeigt, wenn eine neue Steuersatzkonfiguration für Großbritannien angelegt wird.
