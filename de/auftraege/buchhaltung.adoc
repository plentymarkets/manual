...
. Öffne das Menü *Einrichtung » Mandant » _Mandant wählen_ » Standorte » _Standort wählen_ » Buchhaltung*.  +
...
. Öffne das Menü *System » Mandant » _Mandant wählen_ » Standorte*.
...
Du kannst einen Standort nur löschen, wenn ein weiterer Standort vorhanden ist. Ein Standort, der im Menü *Einrichtung » Mandant » Standard Shop* als *Standard-Standort* hinterlegt ist, kann nicht gelöscht werden.
...
. Wähle einen Standort unter *Einrichtung » Mandant » _Mandant wählen_ » Standorte » _Standort wählen_ » Buchhaltung*. +
→ Das Tab *Grundeinstellungen* öffnet sich.
...
====
Steuerliche Informationen zu den europäischen Lieferländern sowie USA und Japan findest du zum Beispiel auf der Website des link:http://www.bzst.de/DE/Steuern_International/Vorsteuerverguetung/02_Unternehmer_Ausland_EU/FAQ/FAQ_node.html[**Bundeszentralamts für Steuern**]{nbsp}icon:external-link[].
====
...
| [#intable-erlöskonto-optional]*Erlöskonto (optional)*
| Dies ist ein optionales Feld, das sich auch nachträglich befüllen lässt. Trage an einem Steuersatz nur dann ein dazugehörendes Erlöskonto ein, wenn der Umsatzsteuersatz von denen unter <<auftraege/buchhaltung#800, Erlöskonten>> zugeordneten abweicht, aber derjenige ist, der gelten soll. +
Beispielsweise lassen sich so die vorübergehend gesenkten Umsatzsteuersätze in Deutschland abbilden. Trage dafür an der Steuerkonfiguration, die die Mehrwertsteuersenkung abbildet, die dazugehörigen Erlöskonten ein. Dadurch haben diese Vorrang und werden priorisiert. +
...
Zolltarifnummern sind für dich relevant, wenn du beim Verkauf innerhalb der EU Umsatzsteueranmeldungen im Lieferland hast oder die <<auftraege/buchhaltung#300, Lieferschwelle>> im Lieferland überschritten hast. In diesen Fällen oder wenn du weißt, dass du in einem Lieferland die Lieferschwelle in absehbarer Zeit überschreiten wirst, solltest du die Zolltarifnummern rechtzeitig einpflegen. Wie empfehlen, dass du die Einzelheiten zur Besteuerung deiner Waren im EU-Ausland mit deinem Steuerberater klärst.

Waren werden in den EU-Ländern unterschiedlich besteuert. Damit für Aufträge auch bei abweichenden Besteuerungen im EU-Ausland die richtigen Steuersätze verwendet werden und somit die Auftragsberechnung korrekt verläuft, müssen Zolltarifnummern im System eingegeben und mit den für die verschiedenen Länder hinterlegten Steuersätzen abgeglichen werden.
Grundlegend dafür ist, dass du zunächst die Zolltarifnummern im System pflegst. Die <<artikel/artikel-verwalten#280, Hinterlegung von Zolltarifnummern>> erfolgt an den *Varianten* eines Artikels. Des Weiteren sollten <<auftraege/buchhaltung#550, Umsatzsteuersätze>> für die entsprechenden Länder eingerichtet sein. Umsatzsteuersätze lassen sich auch nachtragen, wir empfehlen aber die rechtzeitige Erstellung im System.

Die Tabelle im Menü *Einrichtung » Mandant » Global » Zolltarifnummern* ermöglicht die Verknüpfung von Zolltarifnummer, Umsatzsteuer und Land. Diese Zolltarifnummerntabelle ist somit eine Grundlage der Berechnung bei Aufträgen und muss ebenfalls befüllt und gepflegt werden.

_Das passiert im Hintergrund bei der Auftragsberechnung:_ Es kommt zum Beispiel ein Auftrag mit Lieferland Frankreich ins System. Für die Artikelposition im Auftrag ist an der Variante Steuersatz A hinterlegt. In Frankreich gilt dafür aber Steuersatz B. Deswegen ist in der Zolltarifnummerntabelle ein Eintrag mit der gleichen Zolltarifnummer, die auch an der Variante hinterlegt ist, zu finden. In der Tabelle ist diese Zolltarifnummer aber für Frankreich mit Steuersatz B hinterlegt. Dies erkennt die Auftragslogik und nimmt in solchen Fällen immer den in der Zolltarifnummerntabelle hinterlegten Steuersatz.

Sollte an der Variante oder in der Tabelle keine Zolltarifnummer hinterlegt sein, wird kein abweichender Steuersatz für die Berechnung verwendet. Dies bedeutet für dich, dass du nur bei _abweichender Besteuerung_ einer Variante die entsprechende Zolltarifnummer in der Tabelle hinterlegen musst.

Zum Import der Zolltarifnummern kannst du ElasticSync verwenden oder die Nummern manuell einpflegen. Wir empfehlen, ElasticSync zu nutzen und nur einzelne Zolltarifnummern oder kleine Änderungen manuell in der Tabelle zu pflegen.
_Beachte_ bei der Eingabe, dass Nummern bis zu maximal 12 Zeichen Länge hinterlegt werden können. Die Eingabe von Sonder- und Leerzeichen ist nicht erlaubt.

Allgemeine Informationen zum Import über ElasticSync findest du auf der Handbuchseite <<daten/daten-importieren/ElasticSync#, Daten mit ElasticSync importieren>>. Die <<daten/daten-importieren/sync-typen/elasticSync-zuordnung-zolltarifnummer#, Sync-Typen>> zum Import der Zolltarifnummern sind unter *Zuordnung Zolltarifnummer* zu finden.

Um die Zolltarifnummerntabelle manuell zu füllen, gehe wie im Folgenden beschrieben vor.

[.instruction]
Zolltarifnummerntabelle manuell befüllen:

. Öffne das Menü *Einrichtung » Mandant » Global » Zolltarifnummern*. +
. Klicke auf *Zuordnung neu anlegen* (icon:plus-square[role="green"]). +
→ Eine neue Zeile der Tabelle hinzugefügt.
. Gib die *Zolltarifnummer* ein und wähle das entsprechende *Land* sowie die passende *Umsatzsteuer* aus.
...
Es stehen dir weitere Bearbeitungsfunktionen zur Verfügung, beachte hierfür <<tabelle-bearbeitungsfunktionen-zolltarifnummer>>.

[[tabelle-bearbeitungsfunktionen-zolltarifnummer]]
...
Mit dieser Konfiguration für Erlöskonten für steuerpflichtige Erlöse werden demnach definierte Steuersätze einem bestimmten Konto fest zugeordnet. Was machst du aber, wenn ein Steuersatz vorübergehend abweicht, wie es zum Beispiel derzeit der Fall mit der zeitweisen Mehrwertssteuersenkung in Deutschland der Fall ist? Du hinterlegst das passende Erlöskonto direkt an der Umsatzsteuerkonfiguration, die den veränderten Steuersatz abbildet. Dieser hat bei der Ermittlung der Erlöskonten dann Vorrang. Beachte dazu auch den Tabelleneintrag <<auftraege/buchhaltung#intable-erlöskonto-optional, Erlöskonto (optional)>> im Kapitel zur Einrichtung von Umsatzsteuersätzen.
