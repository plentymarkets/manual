...
_Wichtig_ ist, dass du genau darauf achtest die korrekten Daten einzutragen, da sonst falsche Steuersätze oder Fehlkonfigurationen entstehen können, die sich nicht rückgängig machen lassen. Achte auch darauf, bei der Einrichtung der Umsatzsteuersätze den höchsten Steuersatz als *Steuersatz A* und die weiteren dann absteigend einzutragen.
...
Für Informationen zu den verschiedenen Steuersätzen der EU-Länder, nutze die link:https://ec.europa.eu/taxation_customs/economic-analysis-taxation/taxes-europe-database-tedb_en[TEDB ("Taxes in Europe" database)]{nbsp}icon:external-link[].
====
...
*Steuersatz E in %* +
*Steuersatz F in %*
|Trage die Steuersätze ein, beginnend mit dem höchsten als *Steuersatz A* und dann absteigend. Beispiele (Korrektheit der Steuersätze ohne Gewähr): +
Für Deutschland: Steuersatz A = 19%, Steuersatz B= 7% +
Für Frankreich: Steuersatz A = 20%, Steuersatz B = 10%, Steuersatz C = 5,5% und Steuersatz D = 2,1% +
*_Wichtig_*: +
