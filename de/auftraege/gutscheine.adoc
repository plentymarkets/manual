...
*Mehrere Einlösungen pro Kunde* = Ein Kunde kann den Gutschein mehrmals einlösen. Wenn der Gutschein an mehrere Kunden versendet wird, können diese den Gutschein mehrfach einlösen. +
*Eine Einlösung pro Kunde* = Ein Gutschein kann nur einmal pro Kunde eingelöst werden. +
*_Wichtig:_* Ist der Kunde kein registrierter Kunde und loggt sich über einen Gastzugang ein, wird er im System als neuer Kunde geführt. Der Gutschein kann immer wieder eingelöst werden, da eine Einlösung PRO Kunde eingestellt ist - also neuer Kunde, neuer Gutschein.  +
*Einlösung bis Wert aufgebraucht ist* = Der gesamte Betrag kann insgesamt nur einmal eingelöst werden. Löst ein Kunde mehrere Teilbeträge ein, dürfen alle Teilbeträge zusammen nur so hoch sein wie der Gutscheinbetrag. Der Gutschein kann durch mehrere Kunden eingelöst werden.  +
...
Diese Einstellung hat auch Einfluss darauf, wie der Gutschein im Webshop verrechnet wird. <<auftraege/gutscheine#beispiel-berechnung-gutscheine, Ein Beispiel erläutert die Berechnungsmethode>>.
...
[#beispiel-berechnung-gutscheine]
[discrete]
=== Beispiel für die Berechnung von Aktions- und Mehrzweckgutscheinen

In der Kampagne im Tab *Einstellungen* kannst du die Gutscheinart *Aktionsgutschein* oder *Mehrzweckgutschein* wählen. Die Auswahl der Gutscheinart hat Einfluss darauf, wie der Gutschein im Webshop verrechnet wird. Aktionsgutscheine werden z.B. ähnlich wie Rabatte verrechnet. Bei Mehrzweckgutscheinen handelt es sich um einen Festbetrag, der wie ein Zahlungsmittel verstanden werden kann.

[TIP]
.Beispiel für die Berechnung von Aktions- und Mehrzweckgutscheinen
====
Ein Kunde hat einen Gutschein im Wert von 15,00 Euro. Der Kunde legt Artikel im Wert von 25,00 Euro in den Warenkorb. Er kauft 2 Artikel: einen Artikel für 17,95 Euro und einen Artikel für 7,05 Euro. Handelt es sich bei dem Gutschein um einen *Mehrzweckgutschein*, wird der Gutscheinwert vom Gesamtbetrag inkl. Steuern abgezogen. Handelt es sich um einen *Aktionsgutschein*, wird der Gutscheinwert zuerst abgezogen und dann der Restbetrag besteuert.
====
...
So kannst du jedem Kunden einen automatisch generierten Gutschein-Code zusenden. Dazu muss der Kampagne zunächst eine <<crm/e-mails-versenden#4200, Template-Variable>> zugewiesen werden, z.B. `$CouponCode1`.
...
[TIP]
.Hinweis zu persönlichem Gutscheintext
====
...
*_Beachte:_* Erst wenn der Kunde den Gutschein vollständig bezahlt hat, wird der Download der PDF-Vorlage bereitgestellt. Dies ist am PDF-Symbol erkennbar.
