...
====
Stelle im Menü *Artikel » Artikel bearbeiten » Artikel öffnen* im Tab *Global* die Option *Aktionsgutschein* auf *Verboten*, wenn der Artikel nicht per Gutschein bezahlbar sein soll. Legen Kund:inen einen solchen Artikel in den Warenkorb, wird der Rabattbetrag für diesen Artikel nicht abgezogen. Dies ist z.B. bei Sonderangeboten sinnvoll, die nicht weiter rabattiert werden sollen.
...
*Mehrere Einlösungen pro Kunde* = Eine Kund:in kann den Gutschein mehrmals einlösen. Wenn der Gutschein an mehrere Kund:innen versendet wird, können diese den Gutschein mehrfach einlösen. +
*Eine Einlösung pro Kunde* = Ein Gutschein kann nur einmal pro Kund:in eingelöst werden. +
*_Wichtig:_* Wenn Käufer:innen nicht registriert sind und sich über einen Gastzugang einloggen, werden sie im System als neue Kund:innen geführt. Der Gutschein kann immer wieder eingelöst werden, da eine Einlösung PRO Kund:in eingestellt ist - also neue Kund:in, neuer Gutschein.  +
*Einlösung bis Wert aufgebraucht ist* = Der gesamte Betrag kann insgesamt nur einmal eingelöst werden. Löst eine Kund:in mehrere Teilbeträge ein, dürfen alle Teilbeträge zusammen nur so hoch sein wie der Gutscheinbetrag. Der Gutschein kann durch mehrere Kund:innen eingelöst werden.  +
...
So kannst du jeder Kund:in einen automatisch generierten Gutschein-Code zusenden. Dazu muss der Kampagne zunächst eine <<crm/e-mails-versenden#4200, Template-Variable>> zugewiesen werden, z.B. `$CouponCode1`.
...
====
Um einen persönlichen Text zu speichern, muss Kund:innen in ihrem *Mein Konto*-Bereich des Webshops beim Artikel noch einmal auf *Ändern* klicken. Schreibe am besten einen kurzen Hinweis in der Beschreibung des Gutscheinartikels, um Kund:innen darüber zu informieren.
====
...
*_Beachte:_* Erst wenn die Kund:in den Gutschein vollständig bezahlt hat, wird der Download der PDF-Vorlage bereitgestellt. Dies ist am PDF-Symbol erkennbar.

Nachdem das PDF-Dokument durch Klick auf *PDF erzeugen* erstellt wurde, kann die Kund:in die Datei drucken oder speichern.
