...
. Suche (icon:search[role="blue"]) anhand der Filtereinstellungen den Aufrag, für dein eine Auftragsbestätigung erzeugt werden soll.
...
. *Speichere* (icon:save[role="green"]) die Einstellungen.
. Nimm die weiteren Einstellungen gemäß <<tabelle-ereignisaktion-auftragsbestaetigung-versenden>> vor.
. Setze ein Häkchen bei *Aktiv*.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
--
