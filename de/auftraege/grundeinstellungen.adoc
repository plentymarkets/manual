...
[#250]
== Übernahme von Properties

//Noch anpassen: Labels UI; am Ende speichern?; Wie sieht es in der UI aus, wenn dann für mehrere Typen gespeichert?

Du kannst festlegen, welche Properties von einem Auftrag in einen anderen Auftrag übernommen werden sollen. Eine Übernahme findet zum Beispiel beim Erstellen von Lieferaufträgen oder Retouren statt. Properties sind Informationen, die an einem Auftrag hängen, z.B. das Versandprofil, der Zahlungsstatus oder die Dokumentensprache. +
Bestimme im Menü *Einrichtung » Aufträge » Übernahme* für einzelne Auftragstypen, welche Properties übernommen werden sollen. Nicht alle Properties sind übernehmbar. Verwende den Toggle *xy*, um entweder alle Properties oder nur die übernehmbaren anzeigen zu lassen.

[.instruction]
Properties übernehmen:

. Öffne das Menü *Einrichtung » Aufträge » Übernahme*.
. Wähle unter *Auftragstyp Quelle* den Auftragstyp aus, von dem die Properties übernommen werden sollen.
. Wähle unter *Auftragstyp Ziel* den Auftragstyp aus, in den die Properties übernommen werden sollen.
. Aktiviere in der Tabellenspalte *Übernahme* die Properties, die übernommen werden sollen.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
