...
Beachte, dass du zur Nutzung dieser Einstellung mindestens Ceres/IO in Version 5.0.40 benötigst.
...
Du kannst festlegen, welche Auftragsinformationen (Eigenschaften eines Auftrags) in einen anderen Auftrag übernommen werden sollen. Eigenschaften werden zum Beispiel kopiert beim Erstellen von Lieferaufträgen oder Retouren. Eigenschaften sind Informationen, die an einem Auftrag hängen, z.B. das Versandprofil, der Zahlungsstatus oder die Dokumentensprache. Eigenschaften können am Auftrag oder an einer Auftragsposition hängen.

Bestimme im Menü *Einrichtung » Aufträge » Eigenschaftskopie*, welche Eigenschaften kopiert werden sollen. Du bestimmst dies individuell für verschiedene Kombinationen, z.B. von Auftrag (Quelle) zu Lieferauftrag (Ziel). Nicht alle Eigenschaften sind zum Kopieren freigeschaltet. Verwende den Toggle *Nur kopierbare Eigenschaften anzeigen*, um nur die kopierbaren anzeigen zu lassen.

[.instruction]
Eigenschaften fürs Kopieren aktivieren:

. Öffne das Menü *Einrichtung » Aufträge » Eigenschaftskopie*.
. Wähle unter *Auftragstyp Quelle* den Auftragstyp aus, von dem die Eigenschaften kopiert werden sollen.
. Wähle unter *Auftragstyp Ziel* den Auftragstyp aus, in den die Eigenschaften kopiert werden sollen.
. Aktiviere in der Tabellenspalte *Auftragseigenschaft kopieren* die Eigenschaften, die kopiert werden sollen.
. Aktiviere in der Tabellenspalte *Auftragspositionseigenschaft kopieren* die Eigenschaften, die kopiert werden sollen.
