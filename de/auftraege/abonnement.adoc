...
. Klicke unter *Artikelliste* auf *hinzufügen* (icon:plus-square[]). +
→ Du wirst zu den *Auftragspositionen* weitergeleitet.
. Setze die passenden Filter für die Suche und beachte dabei die Erläuterungen in <<table-filters-item-search-subscription>>.
. Führe die Suche aus, indem du auf *Suchen* klickst.
. Möchtest du die gewählten Filter zurücksetzen, klicke auf *Reset*.
. Füge nach der Suche den oder die Artikel in gewünschter Menge zum Warenkorb (icon:cart-plus[]) hinzu.
. *Speichere* (icon:save[]) den Warenkorb (icon:cart-plus[]). +
→ Das Abonnement wird angelegt und du wirst zu den Einstellungen des Abonnements zurückgeleitet.
. *Speichere* (icon:save[]) das Abonnement. +
...
Ist bereits eine Eigenschaft mit einer Auftragsposition verknüpft, wird diese in einer zweiten Tabellenzeile direkt unterhalb der dazugehörigen Auftragsposition angezeigt. Durch Klicken auf icon:expand_more[set=material] wird die zweite Tabellenzeile angezeigt. Hier findest du Angaben zu *Name*, *Wert* und *Aufpreis* der Eigenschaft. Zudem kannst du die Eigenschaften der Auftragspositionen *löschen* (icon:trash-o[]). Gelöschte Eigenschaften werden dann für dieses Abonnement nicht berücksichtigt. +
Um die Eigenschaften zu bearbeiten oder weitere bereits bestehende Eigenschaften zu einer Auftragspositionen hinzuzufügen, klicke auf *Eigenschaften bearbeiten* (icon:pencil[]). Das Bearbeitungsfenster *Eigenschaften von [ausgewählte Auftragsposition mit ID] bearbeiten* öffnet sich und du kannst weitere Anpassungen vornehmen.
...
Zudem hast du in diesem Fenster einige weitere Bearbeitungsmöglichkeiten. Durch Klicken auf *Löschen* (icon:trash-o[]) entfernst du eine Eigenschaft von einer Auftragsposition. Diese Eigenschaft wird dann für dieses Abonnement nicht berücksichtigt. +
...
Speichere (icon:save[role="green"]) den Warenkorb, wenn du mit der Bearbeitung der Auftragseigenschaften und des Warenkorbs fertig bist. Nach dem Speichern wirst du zur Detailansicht des Abonnements zurückgeleitet.
...
Zusätzlich zu den automatischen Durchläufen hast du die Möglichkeit, einen *manuellen Durchlauf* anzustoßen. _Beachte_, dass ein manueller Durchlauf immer _sofort_ ausgeführt wird. Gehe für das Anstoßen eines manuellen Durchlaufs wie folgt vor:
...
Du möchtest das Abonnement einer Kund:in anpassen oder alle Abonnements überprüfen, die noch keinen initialen Durchlauf hatten? Dann führe eine Suche (icon:filter[]) in der Abonnements-Übersicht aus.

[.instruction]
Suche von Abonnements:

. Öffne das Menü *Aufträge » Abonnement*.
. Setze die passenden Filter (icon:filter[]) für die Suche und beachte dabei die Erläuterungen in <<table-search-subscriptions>>.
