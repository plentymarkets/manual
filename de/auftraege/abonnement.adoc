...
→ Du kannst weitere <<auftraege/abonnement#425, Änderungen in der Warenkorbtabelle>> vornehmen oder den Warenkorb *Speichern* (icon:save[set=material]). +
...
Werden bei einem bestehenden Abonnement Änderungen an der *Währung*, der *Herkunft* oder dem *Mandanten* vorgenommen, ist eine Aktualisierung der Verkaufspreise notwendig. Erst danach kannst du das Abonnement *Speichern* (icon:save[set=material]). +
Gehe dafür in die *Artikelliste* des Abonnements und klicke auf *Verkaufspreise aktualisieren* (icon:refresh[set=material]). Speichere danach den Warenkorb und dann das Abonnement, um die Änderungen zu übernehmen.
