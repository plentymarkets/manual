...
Über *Hinzufügen* (icon:plus-square[]) wirst du direkt zur Artikelsuche weitergeleitet und kannst wie unter <<auftraege/abonnement#400, Artikelliste erstellen>> beschrieben Artikel hinzufügen. Auch die unten aufgeführten Bearbeitungsmöglichkeiten stehen dir hier offen.

Oder gehe über *Bearbeiten* (icon:pencil[]) und gelange direkt zum bestehenden Warenkorb (icon:cart-plus[]) für das ausgewählte Abonnement. Folgende Bearbeitungsmöglichkeiten stehen dir dort zur Verfügung:
...
|Füge weitere Artikel zum Warenkorb des Abonnements hinzu (icon:plus-square[]). Gehe dabei wie unter <<auftraege/abonnement#400, Artikelliste erstellen>> beschrieben vor.

|[#intable-subscription-delete-items]*Artikel löschen*
|Entferne Artikel aus dem Warenkorb des Abonnements über *Artikel löschen* (icon:minus-circle[role="red")]).
...
*Speichere* (icon:save[]) den Warenkorb, nachdem du die gewünschten Änderungen vorgenommen hast. Du wirst dann direkt zu den Einstellungen des Abonnements zurückgeleitet.
