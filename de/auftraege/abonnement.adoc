...
. Klicke im Bereich *Artikelliste* auf hinzufügen (icon:plus-square[role="green"]). +
→ Du wirst zur Suche der Auftragspositionen weitergeleitet.
. Wähle passende Filter für die Suche und beachte dabei die Erläuterungen in <<table-filters-item-search-subscription>>.
. Füge Artikel in gewünschter Menge zum Warenkorb (icon:cart-plus[role="green"]) hinzu.
. Bearbeite die <<auftraege/abonnement#450, Eigenschaften der Auftragspositionen>>, wenn erforderlich.
