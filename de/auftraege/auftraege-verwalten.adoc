...
. Im Tab *Übersicht* gehe in den Abschnitt *Kundendaten* und klicke dort auf *Neuer Auftrag* (icon:plus-square[role="green"]), um einen Auftrag für diesen Kunden anzulegen. +
→ Das Tab *Neuer Auftrag* wird geöffnet.
...
. *Speichere* (icon:search[role="green"]) die Einstellungen.
...
. Setze bei den betreffenden Artikeln ein *Häkchen* (icon:check-square[role="blue"]).
. *Speichere* (icon:save[role="green"]) die Einstellungen. +
...
→ Die Gewährleistung wird geöffnet und die Artikel kann über das Tab *Bearbeiten* bearbeitet werden.
...
Die Option *Erscheinungsdatum* findest du in der Artikelbearbeitung im Tab *Verfügbar*. Hier kannst du über einen Kalender das Datum hinterlegen, zu dem der Artikel verfügbar sein wird (Eingabe auch per dynamischem Import möglich). Gehe dazu wie im Folgenden beschrieben vor.
...
Gehe wie folgt vor, um einen bestehenden Status zu bearbeiten.
...
. Klicke auf *Gruppenfunktion* (icon:gear[role="darkGrey", stack="gear,darkGrey", stackPosition="lr"]). +
...
. Klicke auf das *Zahnrad* (icon:gear[role="darkGrey", stack="long-arrow-right,darkGrey", stackPosition="lr"]), um die Einstellungen auf die ausgewählten Status zu übertragen. +
...
. Klicke auf *Suchen* (icon:search[role="blue"]), um Aufträge des gewählten Zeitraums anzuzeigen.
...
. Klicke im Tab *Nachbestellung* des Auftrags auf einen *Warenkorb* (icon:shopping-cart[role="white", stack="square-o,darkGrey"]). +
...
. Klicke im Bereich *Auftragsdetails* auf *Bestelung auslösen* (icon:gear[role="darkGrey", stack="long-arrow-right,darkGrey", , stackPosition="lr"]). +
...
. Wechsle in das Tab *Transfer*.
. Klicke unter *Einstellungen aus Standard-Dokumentenvorlage übernehmen* auf *Übernehmen* (icon:gear[role="darkGrey", stack="long-arrow-right,darkGrey", stackPosition="lr"]).
