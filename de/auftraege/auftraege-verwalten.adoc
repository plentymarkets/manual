...
|*Bestandsstatus*
|In dieser Spalte wird durch einen roten Statusbalken angezeigt, wenn kein Netto-Warenbestand für einen Artikel vorhanden ist. Ist Netto-Bestand vorhanden, wird der rote Statusbalken nicht angezeigt. Wurde vorher z.B. *Nur vorrätige Artikel anzeigen* ausgewählt, wird zunächst keiner der Statusbalken in der Tabelle in rot wiedergegeben werden. Erst, wenn durch das Hinzufügen von Artikelpositionen der Netto-Warenbestand unter 0 geht, wird der Statusbalken rot. Auf diese Weise siehst du auf einen Blick, in welchem Lager Netto-Warenbestand vorhanden ist. +
Diese Spalte ist eine Standardspalte.
...
Zudem kannst du für Aufträge im Feld *Gutschein-Codes* einen oder mehrere Gutschein-Codes eingeben, um sie mit diesem Auftrag einzulösen. Gib einen Gutschein-Code ein und drücke _Enter_ oder klicke außerhalb des Feldes. Es wird sofort überprüft, ob der Gutschein einlösbar ist. Einlösbare Gutschein-Codes werden grün, geben den Wert des Gutscheins wieder und zeigen im Tooltip an, um welche Gutscheinart es sich handelt. Nicht einlösbare Gutschein-Codes werden rot und statt des Codes wird *Nicht einlösbar* angezeigt. Wiederhole dies ggf. für weitere Gutschein-Codes. +
Die Werte der Gutscheine werden verrechnet. Gutscheine werden nicht als Auftragsposition im Warenkorb aufgeführt. Für Angebote können keine Gutscheine eingelöst werden. Das bedeutet, dass das Eingabefeld *Gutschein-Codes* nur bei der Erstellung eines Auftrags verfügbar ist, nicht aber bei der Erstellung eines Angebots.
...
====
Du kannst einen unausgefüllten Rücksendeschein in jedem Paket beilegen, dass du versendest. Hierfür generierst du den Rücksendeschein direkt im Auftrag, ohne dass es überhaupt eine Retoure geben muss. Die zweite Möglichkeit ist, dass du deinen Kund:innen erlaubst Retouren im Mein-Konto-Bereich des Webshops anzumelden und hier die Retourengründe anzugeben. Anschließend kann der Rücksendeschein ausgefüllt ausdruckt werden. Egal wie du den Rücksendeschein deinen Kund:innen zukommen lässt: Damit Retourengründe eingetragen werden können, musst du die Spalte Retourengründe unterhalb der Artikelpositionen platzieren.
====
...
Unbezahlte Aufträge können in einem Sammelauftrag zusammengefasst werden. Es gibt zwei Möglichkeiten, Sammelaufträge zu erzeugen. Beide werden nachfolgend beschrieben. Für beide Möglichkeiten übernimmst du im Menü *Aufträge » Sammelaufträge* noch nicht bezahlte Aufträge in einen Sammelauftrag. Im Tab *Belege* des Sammelauftrags ist dann das Erzeugen von Sammelrechnungen möglich.
...
Fallbeispiel: Ein Unternehmen möchte für die Mitarbeiter:innen monatlich die anfallenden Rechnungen in deinem plentyShop bezahlen. Richte dafür eine eigene Kundenklasse ein. Mit der Kunden-ID in der vorgesehenen Kundenklasse wird die Rechnungsempfänger:in bestimmt. Diese Rechnungsempfänger:in erhält dann die Sammelrechnung, die direkt aus dem Sammelauftrag heraus erzeugt wird. +
...
Mahnungen für Sammelaufträge generierst du genau wie Rechnungen im Tab *Belege* des Sammelauftrags. Gehe dafür wie im Folgenden beschrieben vor.
...
_Wichtig:_ Die weitere Option <<#intable-shipped-on, Versanddatum>> im Tab *Einstellungen* hat eine andere Funktion.
