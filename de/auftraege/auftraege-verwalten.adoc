...
Zusätzlich kannst du die Optionen *Nur aktive Artikel anzeigen*, *Nur vorrätige Artikel anzeigen* und *Nur Vertriebslager anzeigen* auswählen. Wenn wie oben beschrieben deine Lagereinstellung in den Grundeinstellungen *a) pro Auftrag kann ein Lager zugeordnet werden* ist, steht dir die Option *Nur Vertriebslager anzeigen* nicht zur Verfügung, weil du das Lager bereits vorher bestimmt hast. Zudem greifen bei der Suche die <<auftraege/auftraege-verwalten#250, Standardeinstellungen>>, die du vorher vorgenommen hast. +
Du kannst Filter miteinander kombinieren. Es ist nicht möglich mehr als einen Wert pro Filter einzugeben. Führe die Suche aus, indem du auf *Suchen* (icon:search[role="blue"]) klickst oder Enter drückst. Eine Tabelle mit den Artikeln, die den Suchkriterien entsprechen, wird geöffnet.
