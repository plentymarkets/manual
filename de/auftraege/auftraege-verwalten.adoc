...
.E-Mail-Versand
====
Bei manuell angelegten Aufträgen wird keine automatische E-Mail, z. B. Auftragsbestätigung, an den Kund:innen versendet. Grund dafür ist, dass das System nicht wissen kann, wann alle Einstellungen zu diesem Auftrag vorgenommen wurden. Stoße daher den Versand einer E-Mail-Vorlage manuell im Auftrag und dort im Tab <<auftraege/auftraege-verwalten#1550, Mail>> an.
====
...
|*Zahlungsart*
|Wenn im Kontakt eine Zahlungsart definiert ist, zum Beispiel über eine <<crm/kontakte-verwalten#90, Kundenklasse>>, ist diese vorausgewählt. Ansonsten kannst du eine Zahlungsart eingeben und dann auswählen. Angeboten werden dir alle Zahlungsarten, die bereits in deinem plentymarkets-System integriert sind.
...
. Wähle aus, welche Spalten angezeigt werden sollen. Beachte dafür <<tabelle-übersicht-artikelsuche>>.
...
|*Aktiv / Inaktiv*
|In dieser Spalte wird angezeigt, ob diese Variante im Webshop aktiv oder inaktiv ist. Informationen zur Aktivierung oder Deaktivierung einer Variante findest du auf der Handbuchseite <<artikel/artikel-verwalten#200, Verfügbarkeit>>.
...
Wenn du erlaubst, dass Kund:innen im Mein-Konto-Bereich des Callisto Webshops <<webshop/webshop-einrichten/mein-konto#grundeinstellungen, Retouren selbst anlegen>> können, stehen die angelegten Retourengründe ort zur Wahl. Wenn dort kein Grund gewählt wird, wird der erste Retourengrund verwendet, da dieser voreingestellt ist.
...
Bevor Rücksendescheine erzeugt werden können, muss die Dokumentenvorlage *Rücksendeschein* im Menü <<auftraege/auftragsdokumente#, Einrichtung » Mandant » _Mandant wählen_ » Standorte » _Standort wählen_ » Dokumente » Rücksendeschein>> eingerichtet werden.
...
Kommt es nach erfolgter Zahlung, Warenausgang und Zustellung der Ware für eine oder alle Auftragspositionen zu einer <<auftraege/auftrage-verwalten#400, Retoure>> oder im Folgenden zu einem <<auftraege/auftrage-verwalten#700, Reparaturauftrag>>, lege die Gutschrift direkt im entsprechenden Retouren- oder Reparaturauftrag an.
====
...
====
Bevor Gutschriftbelege erzeugt werden können, muss die Dokumentenvorlage für <<auftraege/auftragsdokumente/gutschriften-erzeugen#, Gutschriften>> im Menü <<auftraege/auftragsdokumente#, Einrichtung » Mandant » _Mandant wählen_ » Standorte » _Standort wählen_ » Dokumente » Gutschrift>> eingerichtet werden.
====
...
. Öffne das Menü *Einrichtung » Mandant » _Mandant wählen_ » Standorte » _Standort wählen_ » Dokumente » Stornobeleg Gutschrift*.
...
====
Wird eine Sammelrechnung bezahlt und der Sammelauftrag als bezahlt markiert, werden die Zahlungseingänge *nicht* automatisch an den einzelnen im Sammelauftrag zusammengefassten Aufträgen gebucht. Zahlungseingänge musst du also an den einzelnen Aufträgen manuell buchen.
====
...
Um nach IDs oder Nummern zu filtern, gib in die Eintragsfelder eine ID oder Nummer ein. Durch Eingabe einer *Kunden-ID* erhältst du z.B. nur die Aufträge der Kund:in mit dieser ID.
...
|Individuelles Symbol de zugeordneten Dienstleister:in. Ein Symbol wird jedoch nur angezeigt, wenn im <<fulfillment/versand-vorbereiten#1000, Versandprofil>> eine Markierung eingestellt wurde.
...
Die Option *IP-Adresse des Käufers* wird im Menü <<webshop/webshop-einrichten/bestellvorgang#, Einrichtung » Mandant » _Mandant wählen_ » Bestellvorgang » Einstellungen>> aktiviert bzw. deaktiviert.
...
| *Kundennotizen*
|Du wirst weitergeleitet zum Bereich <<crm/messenger#, Messenger>> des Kundendatensatzes. Hier kannst du entweder eine *Neue Nachricht* (icon:plus-square[role="green"]) erstellen oder bereits vorhandene Nachrichten einsehen.
...
Die Belege werden jeweils vor der Erstellung konfiguriert. Du kannst je nach Beleg verschiedene Parameter anpassen, z.B. bei einer Rechnung das Datum, das *Zahlungsziel*, ob Skonto angerechnet werden soll und ob der Warenausgang gebucht werden soll. Außerdem kann ein Statuswechsel bei Erzeugung des Belegs veranlasst werden.

_Beachte_, dass erstellte Belege nicht gelöscht werden können, lediglich externe Dokumente. Zu einigen Dokumenten, zum Beispiel einer Rechnung oder Mahnung, können Stornobelege erstellt werden. Aber alle Dokumente, die erzeugt wurden, bleiben im Tab Belege erhalten. +
Es ist jedoch möglich Dokumente zu archivieren. Klicke dazu auf *Archivieren* (icon:receipt_archive[set=plenty]). Die archivierten Dokumente findest du im Tab *Archivierte Dokumente* unter dem Tab *Belege*.

[IMPORTANT]
.Dokumente im Archiv
====
Das Archivieren von Dokumenten ist nicht rückgängig zu machen. Liegen die Dokumente einmal im Tab *Archivierte Dokumente*, können sie nicht wieder in den Tab *Dokumente* verschoben werden.
====
...
. Öffne das Menü *Einrichtung » Mandant » _Mandant wählen_ » Standorte » _Standort wählen_ » Dokumente » Stornobeleg Rechnung*.
. Wechsle in das Tab *Transfer*.
...
.Grundeinstellungen zur Auftragsbearbeitung durch variable Benutzer:innen
====
Damit variable Benutzer:innen Aufträge öffnen können, müssen im Menü <<auftraege/grundeinstellungen#, Einrichtung » Aufträge » Einstellungen>> die Standard-Status für *Retouren* und *Gutschriften* gespeichert worden sein. In der Grundeinstellung ist dort kein Status voreingestellt.
Außerdem müssen diese Benutzer:innen für die Erstellung und Bearbeitung von Aufträgen im Benutzerprofil unter <<business-entscheidungen/benutzerkonten-zugaenge#110, Berechtigung » Tab: Menü » Bereich: Aufträge>> berechtigt werden. +
Damit variable Benutzer:innen Aufträge der Typen *Retoure*, *Gewährleistung* und *Reparatur* anlegen können, müssen sie für die im Menü <<auftraege/grundeinstellungen#, Einrichtung » Aufträge » Einstellungen>> hinterlegten Standard-Status dieser drei Auftragstypen berechtigt sein. Die Berechtigung erfolgt im *Benutzerprofil* der variablen Benutzer:in unter <<business-entscheidungen/benutzerkonten-zugaenge#100, Berechtigung » Tab: Sichtbarkeiten>>.
====
