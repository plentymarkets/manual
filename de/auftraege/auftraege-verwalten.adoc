...
|*Weiterleitung nach Abschließen eines Auftrags*
|Wähle aus, wohin im System du nach Abschließen eines Auftrages geleitet werden möchtest. Du kannst wählen zwischen der Detailansicht des gerade angelegten Auftrags, der allgemeinen Auftragsübersicht oder der Auftragsübersicht des Kontaktes. Die Standardeinstellung ist, dass du zur Auftragsübersicht des Kontaktes geleitet wirst.
...
|*Warenkorb Icon*
|Durch Klick auf den diesen Warenkorb (icon:shopping-cart[role="darkGrey"]) wird der Artikel in der ausgewählten *Menge* dem Warenkorb hinzugefügt. +
Diese Spalte gehört zu den Standardspalten.
...
→ Wenn ein Artikel erfolgreich hinzugefügt wurde, verändert der Warenkorb (icon:shopping-cart[role="darkGrey"]) kurzzeitig Größe und Farbe.
...
Du kannst sowohl die *Menge* als auch den *Verkaufspreis* anpassen. Wenn ein Verkaufspreis eingegeben wird, der niedriger ist als der Einkaufspreis, wirst du durch eine Warnung darauf hingewiesen. Allerdings ist es möglich einen niedrigeren Verkaufspreis zu wählen, wenn es gewünscht wird.
...
In den Auftragsdetails kannst du die *Versandart* und die *Versandkosten* anpassen, wenn gewünscht. Bis auf die Adressen, die Währung und die Lagereinstellung lassen sich die Auftragsdetails in dieser Ansicht bearbeiten.

Unterhalb der Auftragsdetails wird eine Tabelle mit den ausgewählten Artikelpositionen angezeigt. Zusätzlich zu den Angaben in der vorherigen Ansicht werden dir in dieser auch Preise und Umsatzsteuer ausgegeben. Die Preise sind bearbeitbar, solange der Auftrag noch nicht abgeschlossen ist. Steuersätze lassen sich anpassen, wenn du dies in den <<auftraege/auftraege-verwalten#250, Standardeinstellungen>> erlaubt hast.
