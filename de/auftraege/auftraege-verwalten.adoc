...
=== Auftragsvorlagen
////
- Auftragspositionen erst dazu, wenn Vorlage ausgewählt. Danach Hinzufügen weiterer Artikel kein Problem.
- Hinweis, wo zu finden (bei drei gleich, new order create Dropdown oberhalb; hier Hinweis, wie Verhalten beim vor- und zurückgehen)
- default: keine Vorlage ausgewählt
- Frage (vielleicht): Wenn Vorlage auf einen Auftrag angewendet wird, werden dann immer vorher hinterlegte Daten (also nur Status und Zahlungsart) überschrieben?
////
Im Menü *Einrichtung » Aufträge » Auftragsvorlage* kannst du Auftragsvorlagen erstellen. Diese kannst du dann beim Anlegen eines manuellen Auftrags auswählen oder über eine <<automatisierung/ereignisaktionen#intable-auftragsvorlage, Ereignisaktion>> auf einen Auftrag anwenden lassen. Wählst du eine Auftragsvorlage für einen Auftrag aus, werden die dort hinterlegten Daten zum Auftrag hinzugefügt.
...
|*Zahlungsart*
|Wähle eine Zahlungsart aus der Dropdown-Liste aus. Diese Zahlungsart wird dann bei Anwendung der Vorlage für den entsprechenden Auftrag genutzt.

|*Auftragsstatus*
|Wähle einen Auftragsstatus aus. Bei Anwendung der Vorlage auf einen Auftrag, wird dieser Auftrag dann in diesen Auftragsstatus geschoben.
