...
Um einen Auftrag abzuschließen und ihn anzulegen, müssen Artikel hinzugefügt werden. Führe eine Suche (icon:search[set=material]) aus, um die passenden Artikel zu finden und dann hinzuzufügen. Für die Artikelsuche kannst du zwischen verschiedenen Filtern (icon:filter_alt[set=material]) wählen:
...
. *Suche* anhand der Filtereinstellungen (icon:filter_alt[set=material]) den gewünschten Artikel.
...
→ Bei Preis- und Mengenänderungen muss der Verkaufspreis durch klicken auf *Verkaufspreis aktualisieren* aktualisiert werden.
...
Am Warenkorb-Symbol (icon:add_shopping_cart[set=material]) über der Tabelle wird dir die Anzahl der bereits hinzugefügten Artikel angezeigt. Möchtest du während des Hinzufügens von Artikeln den Warenkorb überprüfen oder hinzugefügte Artikel löschen, klicke auf diesen Warenkorb (icon:add_shopping_cart[set=material]).
