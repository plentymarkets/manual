...
Am Warenkorb-Symbol (icon:shopping-cart[role="darkGrey"]) über der Tabelle wird dir die Anzahl der bereits hinzugefügten Artikel angezeigt. Möchtest du während des Hinzufügens von Artikeln den Warenkorb überprüfen oder hinzugefügte Artikel löschen, klicke auf diesen Warenkorb (icon:shopping-cart[role="darkGrey"]).
...
Zudem können Auftragspositionen im Warenkorb auch gelöscht werden. Klicke dafür auf *Löschen* (icon:minus-circle[role="red"]). +
Eine weitere Bearbeitungsmöglichkeit der Auftragspositionen im Warenkorb stellen die Eigenschaften dar. Beachte dafür das folgende Kapitel <<auftraege/auftraege-verwalten#287, Eigenschaften von Auftragspositionen>>.

Nachdem du alle gewünschten Artikel im Warenkorb abschließend bearbeitet hast hast, klicke auf *Übersicht*, um zum dritten und letzten Schritt zu gelangen. Du könntest auch zu den Auftragsdetails zurückkehren, indem du auf *Auftragsdetails* klickst. Artikel im Warenkorb werden gespeichert, wenn du vor oder zurück gehst.
...
Vom Warenkorb aus kannst du die Eigenschaften der Auftragspositionen eines Auftrags bearbeiten. Die hier vorgenommenen Änderungen gelten nur für die Auftragspositionen dieses Auftrags. Die im System angelegten Eigenschaften werden nicht überschrieben. In der Auftragsanlage stehen dir nur Eigenschaften zur Verfügung, die du bereits im System angelegt hast. +
Möchtest du generell Eigenschaften erstellen und bearbeiten oder bist noch nicht mit der Struktur von Eigenschaften vertraut, informiere dich im Handbuchkapitel <<artikel/einstellungen/eigenschaften#, Eigenschaften und Merkmale>>.

Damit eine Eigenschaft als Bestelleigenschaft an einer Auftragsposition gesetzt werden kann, muss sie in den Eigenschaftseinstellungen im Bereich *Optionen* als *Bestelleigenschaft* definiert werden. Lege unter <<artikel/einstellungen/eigenschaften#property-options, Optionen>> außerdem mögliche Aufpreise und Besteuerungen fest. Die hier hinterlegten Werte werden in die Auftragsberechnungen einbezogen.
...
+
--
Die Eigenschaften werden unter der Auftragsposition als Chips angezeigt. Entfernst du einen Chip, wird diese Eigenschaft gelöscht und für diesen Auftrag nicht berücksichtigt. +
Um die Eigenschaften zu bearbeiten oder weitere bereits bestehende Eigenschaften zu einer Auftragspositionen hinzuzufügen, klicke auf icon:pencil[] unter *Auftragspositionseigenschaften*. Das gleiche Bearbeitungsfenster wie vom Warenkorb als Tabelle aus öffnet sich.
...
+
--
Die Eigenschaften werden in einer zweiten Tabellenzeile direkt unterhalb der dazugehörigen Auftragsposition angezeigt. Hier findest du Angaben zu *Name*, *Wert*, *Aufpreis* und *Ust. %* der Eigenschaft. *Ust. %*, also den hinterlegten Steuersatz, kannst du direkt in der Tabelle bearbeiten, wenn der Eigenschaftstyp dies zulässt. Zudem kannst du die Eigenschaften der Auftragspositionen *löschen* (icon:trash-o[]). Gelöschte Eigenschaften werden dann für diesen Auftrag nicht berücksichtigt. Möchtest du eine weitere bereits bestehende Eigenschaft zu einer Auftragsposition hinzufügen, klicke auf *Eigenschaften bearbeiten* (icon:pencil[]). Das gleiche Bearbeitungsfenster wie vom Warenkorb als Side cart aus öffnet sich.
...
* *Wert* = Der Wert der Eigenschaft. Der Wert ist anpassbar. Je nach Eigenschaftstyp kannst du eine Auswahl treffen, den Wert ändern oder eingeben oder eine Datei hochladen. Die Änderungen gelten nur für diesen Auftrag.
* *Aufpreis* = Der Aufpreis der Eigenschaft, sofern sie einen zugeordnet hat. Der Aufpreis ist bearbeitbar. Die Änderungen gelten nur für diesen Auftrag.

Zudem hast du in diesem Fenster einige Bearbeitungsmöglichkeiten. Durch Klicken auf *Löschen* (icon:trash-o[]) entfernst du eine Eigenschaft von einer Auftragsposition. Diese Eigenschaft wird dann für diesen Auftrag nicht berücksichtigt. +
Du kannst weitere bereits angelegte Eigenschaften hinzufügen. Wähle eine Eigenschaft aus der Dopdown-Liste des Feldes *Eigenschaft wählen* aus und füge sie durch Klicken auf *Eigenschaft hinzufügen* (icon:plus-square[role="green"]) zu dieser Auftragsposition hinzu. +
...
// Noch mit Dorin klären, ob er wirklich *Verwerfen* / *Cancel* eingefügt hat oder wie es dann umgesetzt wurde. Momentan werden die Änderungen auch durch Klicken auf diesen Button übernehmen, was aber nicht intuitiv ist. Um Änderungen nicht zu übernehmen, müssten die Eigenschaften gelöscht werden.

Wenn du mit der Bearbeitung der Auftragseigenschaften und des Warenkorbs fertig bist, folgt der Schritt der *Übersicht* in der Auftragsanlage. In diesem Schritt werden die Eigenschaften für Auftragspositionen nicht noch mal angezeigt. Stelle daher sicher, dass du alle Anpassungen vorgenommen hast, bevor du in der Erstellung des Auftrags weitergehst. +
Wenn hinzugefügte Eigenschaften Aufpreise und Umsatzsteuerangaben enthalten, wird dies entsprechend der Einstellungen in der dazugehörigen Eigenschaft in die Berechnungen der Auftragsbeträge einbezogen. Auf den Auftragsdokumenten werden Gruppennamen und Eigenschaftsnamen von Eigenschaften ausgegeben.
//Bez. Ausgabe auf Dokumenten noch mal Emmanouil gefragt. Auf Antwort warten.
...
* errechneter Gesamtbruttobetrag des Auftrags
* errechneter Gesamtnettobetrag des Auftrags
* Anzahl der Artikelpositionen des Auftrags
* Rabatt (Ermäßigung durch Aktionsgutscheine)
* offener Betrag (von Kund:in zu zahlender Betrag)
