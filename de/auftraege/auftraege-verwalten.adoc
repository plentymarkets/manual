...
Dieses Kapitel beschreibt das Anlegen von Aufträgen oder Angeboten über die Kontaktübersicht im Menü *CRM » Kontakte*. Hierbei handelt es sich um eine neue Benutzeroberfläche, die zunächst nur in _beta_ zur Verfügung steht. Alle anderen Methoden Aufträge anzulegen sind ebenfalls weiterhin möglich.
...
|Wählst du einen Standard-Eigner aus, ist dieser in den Auftragsdetails des neuen Auftrags oder Angebots standardmäßig vorausgewählt. Der Eigner wird generell nicht aus den Kontaktdaten übernommen.
...
. Wähle entweder *Beta: Neuer Auftrag* oder *Beta: Neues Angebot*. +
→ Du wirst automatisch zu den Auftragsdetails für den neuen Auftrag bzw. des neuen Angebotes weitergeleitet. +

Über den Auftragsdetails kannst du aus Dropdown-Liste auch nachträglich zwischen Auftrag und Angebot wählen.
...
|*Zahlungsart*
|Wenn im Kontakt eine Zahlungsart definiert ist, zum Beispiel über eine <<crm/kontakte-verwalten#90, Kundenklasse>>, ist diese vorausgewählt. Ansonsten kannst du eine Zahlungsart aus der Dropdown-Liste auswählen. Angeboten werden dir alle Zahlungsarten, die bereits in deinem plentymarkets-System integriert sind.

|*Lager*
|Welche Optionen dir angezeigt werden, hängt von den Grundeinstellungen ab, die du für die <<auftraege/grundeinstellungen#intable-lagerzuordnung, Zuordnung von Lagern>> gewählt hast: +
...
→ Das Fenster *Tabelle individualisieren* öffnet sich.
. Wähle durch Setzen eines Häkchens aus, welche Spalten angezeigt werden sollen. Beachte dafür <<tabelle-übersicht-artikelsuche>>.
...
Wenn du die Tabelle einmal angepasst hast, wird diese Auswahl gespeichert. Die Tabelle bleibt also gleich, auch wenn du diesen Auftrag abgeschlossen hast und weitere anlegst. Du kannst das Layout jederzeit ändern. Die zu Verfügung stehenden Spalten findest du in <<tabelle-übersicht-artikelsuche>>.
...
|*Warenkorb Icon*
|Durch Klick auf den Warenkorb (icon:shopping-cart[role="darkGrey"]) wird der Artikel in der ausgewählten *Menge* dem Warenkorb hinzugefügt.

|*Artikel-ID / Varianten-ID*
|Wenn du auf die Artikel-ID oder die Varianten-ID klickst, gelangst du direkt in die Einstellungen des jeweiligen Artikels bzw. der Variante.
...
Im Warenkorb siehst du die hinzugefügten Artikel. Du kannst sowohl die *Menge* als auch den *Verkaufspreis* anpassen. Wenn ein Verkaufspreis eingegeben wird, der niedriger ist als der Einkaufspreis, wirst du durch eine Warnung darauf hingewiesen. Allerdings ist es möglich einen niedrigeren Verkaufspreis zu wählen, wenn es gewünscht wird.

Artikelpositionen können im Warenkorb auch gelöscht werden. Klicke dafür auf *Löschen* (icon:minus-circle[role="red"]).
...
Unterhalb der Auftragsdetails wird eine Tabelle mit den ausgewählten Artikelpositionen angezeigt. Auch diese Tabelle kannst du individuell anpassen. Gehe dabei wie in Kapitel <<auftraege/auftraege-verawalten#280, Aufträge hinzufügen>> beschrieben vor. Zusätzlich zu den Angaben in der vorherigen Ansicht werden dir in dieser auch der *Verkaufspreis* und die *Umsatzsteuer* mit ausgegeben. Der Verkaufspreis ist bearbeitbar, solange der Auftrag noch nicht abgeschlossen ist.

Wenn du Änderungen an Mengen, Preisen, Versandkosten oder der Versandart vornimmst, müssen *Neuberechnungen* ausgeführt werden, da die Preiskalkulation von diesen Faktoren abhängig ist. Bevor du den Auftrag abschließen kannst, musst du daher auf *Auftrag neu berechnen* klicken. Durch die Neuberechnung passt sich die Gesamtsummen an. Um danach den Auftrag fertigzustellen, klicke auf *Auftrag anlegen*. Der Auftrag ist nun erstellt und du wirst zur Auftragsübersicht des Kontaktes weitergeleitet. Der Auftrag ist in der allgemeinen Auftragsübersicht im Menü *Aufträge » Aufträge bearbeiten* verfügbar.
