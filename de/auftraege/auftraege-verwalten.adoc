...
Wende die Suche (icon:search[]) an, um die gewünschten Auftragspositionen anhand der vorhandenen Artikel mit Bestand zu finden.

[.collapseBox]
.*Suchfunktionen*
--
Du hast mehrere Möglichkeiten, die Suche zu nutzen. Du kannst einen Wert im Suchfeld eingeben und dann den entsprechenden Filter auswählen. Gibst du z.B. eine Zahl ein, werden dir mögliche Filter in der diese Zahl vorkommt vorgeschlagen, wie IDs oder Variantennummer. Gib den Wert vollständig ein und wähle den passenden Filter aus den Vorschlägen. Wiederhole dies, um Filter miteinander zu kombinieren. Klicke auf icon:search[], um die Suche auszuführen. +
