...
. Wähle aus der Dropdown-Liste *Lieferadresse* die Option *Neue Lieferadresse*. +
→ Das Fenster *Neue Lieferadresse* öffnet sich.
. Wähle aus, ob es sich um die *Primäre* Lieferadresse handelt.
. Gib die Daten ein. Beachte hierzu <<tabelle-neue-lieferadresse>>.
. Speichere (icon:save[role="green"]) die neue Adresse.
...
|*Typ* und *Wert*
|Einen Typ aus der Dropdown-Liste auswählen. Zur Verfügung stehen: Umsatzsteuernummer, Externe Adress-ID, Gelangensbestätigung, Postnummer, Personennummer, FSK, Geburtstag, Titel, Ansprechpartner. Den passenden *Wert* für den ausgewählten Typ eingeben.
...
Auf Dokumenten werden die Werte übernommen, die am dazugehörigen Auftrag hinterlegt sind. Beachte auch die <<auftraege/auftragsdokumente#intable-zahlungsbedingungen-anzeigen, Dokumenteneinstellungen>>. _Wichtig_: Die Zahlungsbedingungen werden nur auf Dokumenten übernommen, wenn es sich um einen Auftrag mit der _Zahlungsart Rechnung_ handelt.
...
Im Tab *Belege* erzeugst du Belege, d.h. z.B. eine Auftragsbestätigung, einen Lieferschein oder eine Rechnung. Beachte, dass nachdem du Dokumente zu einem Auftrag erzeugt hast, kann das Eingabedatum des Auftrags nicht mehr geändert werden.
