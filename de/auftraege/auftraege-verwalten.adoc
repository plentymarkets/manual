...
Um die Auftragsanlage zu erleichtern und schneller vorgehen zu können, solltest du zunächst diese Standardeinstellungen festlegen. _Beachte_, dass die Standardeinstellungen auch für über diese Benutzeroberfläche angelegte Angebote gelten. Gehe dafür ins Menü  *Einrichtung » Aufträge » Auftragstypen » Auftrag [Beta]*.
...
|Wählst du einen Standard-Eigner aus, ist dieser in den Auftragsdetails des neuen Auftrags oder Angebots dann standardmäßig vorausgewählt. Der Eigner wird generell nicht aus den Kontaktdaten übernommen.
...
|*Option ‚Nur Vertriebslager anzeigen‘ für Artikelsuche vorausgewählt*
...
→ Hier kannst du in Dropdown-Liste über den Auftragsdetails jederzeit auswählen, ob es sich um einen Auftrag oder ein Angebot handelt. Die im Folgenden beschriebenen Schritte sind für Auftrag und Angebot gleich.
...
|*Zahlungsart*
|Wenn im Kontakt eine Zahlungsart definiert ist, zum Beispiel über eine <<crm/kontakte-verwalten#90, Kundenklasse>>, ist diese hier vorausgewählt. Ansonsten kannst du eine Zahlungsart aus der Dropdown-Liste auswählen. Angeboten werden dir alle <<payment/zahlungsarten-verwalten#20, Zahlungsarten>>, die bereits in deinem plentymarkets System integriert sind.
...
|*Eigner*
|Wähle aus der Dropdown-Liste den Eigner für diesen Auftrag aus. Hast du einen <<auftraege/auftraege-verwalten#intable-standard-eigner, Standard-Eigner>> gesetzt, ist dieser hier vorausgewählt. Ansonsten werden dir in der Dropdown-Liste nur Eigner angezeigt, die das Benutzerrecht Aufträge zu erstellen und zu bearbeiten haben.
...
Während der Hinzufügens von Artikeln sind die Auftragsdetails nicht sichtbar. Erst im dritten Schritt, der Überprüfung, sind die Auftragsdetails wieder verfügbar, so dass sich einige eventuell erforderliche Anpassungen dort machen lassen. _Beachte_ allerdings, dass die Adressen, die Währung und das ausgewählte Lager auch dort nicht mehr bearbeitet werden können.

Um einen Auftrag abzuschließen und ihn anzulegen, müssen Artikel hinzugefügt werden. Führe eine Suche aus, um Artikel hinzuzufügen. Für die Artikelsuche kannst du zwischen verschiedenen Filtern wählen: +
...
Zusätzlich kannst du die Optionen *Nur vorrätige Artikel anzeigen* und *Nur Vertriebslager anzeigen* aktivieren. Wenn wie oben beschrieben deine Lagereinstellung in den Grundeinstellungen *a) pro Auftrag kann ein Lager zugeordnet werden* ist, kannst du hier nur die Option *Nur vorrätige Artikel anzeigen* wählen, weil du das Lager bereits vorher bestimmt hast. Zudem greifen bei der Suche die <<auftraege/auftraege-verwalten#250, Standardeinstellungen>>, die du vorher vorgenommen hast.
...
Zeilen mit Artikeln, für die im ausgewählten Lager Bestand vorhanden ist, werden weiß angezeigt. Rot sind die Zeilen mit Artikeln, für die im ausgewählten Lager kein Bestand verfügbar ist. Wurde vorher z.B. *Nur vorrätige Artikel anzeigen* ausgewählt, wird zunächst keine der Zeilen der Tabelle in rot wiedergegeben werden. Erst, wenn durch hinzufügen von Artikelpositionen der Bestand unter 0 geht, wird die Zeile rot. Auf diese Weise siehst du auf einen Blick, in welchem Lager welcher Bestand vorhanden ist und die Bestellung kann schneller abgeschlossen werden.
...
. Suche anhand der Filtereinstellungen den gewünschten Artikel.
. Gib die Menge des Artikels ein, die dem Auftrag hinzugefügt werden soll.
. Füge die Artikel dem Auftrag hinzu, indem du am Ende der jeweiligen Zeile auf den Warenkorb (icon:shopping-cart[role="darkGrey"]) klickst.
...
Am Warenkorb-Symbol (icon:shopping-cart[role="darkGrey"]) über der Tabelle wird dir die Anzahl der bereits hinzugefügten Artikel angezeigt. Möchtest du während des Hinzufügens von Artikeln den Warenkorb überprüfen oder hinzugefügte Artikel löschen, gehe in diesen Warenkorb (icon:shopping-cart[role="darkGrey"]).

[.discrete]
Warenkorb

Im Warenkorb siehst du die hinzugefügten Artikel. Du kannst hier sowohl die *Menge* als auch den *Verkaufspreis* anpassen. Wenn ein Verkaufspreis eingegeben wird, der niedriger ist als der Einkaufspreis, wirst du durch eine Warnung darauf hingewiesen. Allerdings ist es möglich einen niedrigeren Verkaufspreis zu wählen, wenn es gewünscht wird.

Artikelpositionen im Warenkorb können nicht nur hinzugefügt, sondern auch gelöscht werden. Klicke hierzu auf *Artikel löschen* (icon:minus-circle[role="red"]).

Nachdem du alle gewünschten Artikel in entsprechender Menge und den gewünschten Preisen im Warenkorb hast, klicke auf *Übersicht*, um zum dritten und letzten Schritt zu gelangen. Du könntest auch zu den Auftragsdetails zurückkehren, indem du auf *Auftragsdetails* klickst. _Beachte_ aber, dass dann alle Daten aus dem Warenkorb gelöscht werden.

[#280]
==== Übersicht: Auftrag abschließen

In der letzten Ansicht wird der Auftrag abgeschlossen. Über den Auftragsdetails wird dir für den Auftrag sowohl der errechnete Gesamtbruttobetrag als auch der Gesamtnettobetrag angezeigt. Zusätzlich siehst du die Anzahl der Artikelpositionen.

In den Auftragsdetails kannst du die *Versandart* und die *Versandkosten* anpassen, wenn gewünscht. Bis auf die Adressen, die Währung und das Lager lassen sich die Auftragsdetails in dieser Ansicht bearbeiten.

Unterhalb der Auftragsdetails wird eine Tabelle mit den ausgewählten Artikelpositionen angezeigt. Zusätzlich zu den Angaben in der vorherigen Ansicht werden dir hier auch der *Verkaufspreis* und die *Umsatzsteuer* mit ausgegeben. Der Verkaufspreis ist bearbeitbar, solange der Auftrag noch nicht abgeschlossen ist.

Wenn du Änderungen an Mengen, Preisen, Versandkosten oder der Versandart vornimmst, müssen *Neuberechnungen* ausgeführt werden, da die Preiskalkulation von diesen Faktoren abhängig ist. Bevor du den Auftrag abschließen kannst, musst du daher auf *Auftrag neu berechnen* klicken. Durch die Neuberechnung passt sich die Gesamtsumme an. Um danach den Auftrag fertigzustellen, klicke auf *Auftrag anlegen*. Der Auftrag ist nun erstellt und du wirst zur Auftragsübersicht des Kontaktes weitergeleitet. Der Auftrag ist in der allgemeinen Auftragsübersicht im Menü *Aufträge » Aufträge bearbeiten* verfügbar.
