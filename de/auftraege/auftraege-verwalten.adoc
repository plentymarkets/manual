...
Du kannst Filter miteinander kombinieren. Es ist nicht möglich mehr als einen Wert pro Filter einzugeben. Führe die Suche aus, indem du auf *Suchen* klickst oder Enter drückst. Eine Tabelle mit den Artikeln, die den Suchkriterien entsprechen, wird geöffnet.
...
|[#intable-item-search-sales-price]*Verkaufspreis*
|In einer Dropdown-Liste werden dir alle für diesen Auftrag ermittelten Verkaufspreise angezeigt. Du kannst einen der angezeigten Preise wählen, dieser wird dann als Grundlage für die weiteren Auftragsberechnungen genommen. Eine manuelle Anpassung des Preises kannst du im <<auftraege/auftraege-verwalten#284, Warenkorb>> vornehmen. +
...
Je nachdem, welchen <<auftraege/auftraege-verwalten#intable-shopping-cart-type, Warenkorbtyp>> du in den Standardeinstellungen ausgewählt hast, öffnet sich entweder der seitliche Warenkorb oder die Ansicht springt direkt zur Warenkorbtabelle. Unabhängig vom Warenkorbtyp kannst du dort sowohl die *Menge* als auch den *Verkaufspreis* anpassen. Sobald du die Angaben in einem der Felder änderst, müssen Neuberechnungen (icon:refresh[set=plenty]) durchgeführt werden. Folgendes gilt für die Verkaufspreise:
...
Zudem können Auftragspositionen im Warenkorb auch gelöscht werden. Klicke dafür auf *Löschen* (icon:minus-circle[role="red"]). Eine weitere Bearbeitungsmöglichkeit der Auftragspositionen im Warenkorb stellen die Eigenschaften dar. Beachte dafür das folgende Kapitel <<auftraege/auftraege-verwalten#287, Eigenschaften von Auftragspositionen>>.
