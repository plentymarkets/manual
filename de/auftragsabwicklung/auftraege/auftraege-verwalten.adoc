...
Aufträge bilden Ihre Verkäufe sowie damit einhergehende und übliche Geschäftsvorfälle ab. Mithilfe von Auftragstypen werden die unterschiedlichen Geschäftsvorfälle kenntlich gemacht. Der Fortschritt bei der Bearbeitung eines Auftrags wird durch Auftragsstatus angezeigt. Welche Möglichkeiten Ihnen Auftragstypen und Auftragsstatus bieten und wie Sie Aufträge sinnvoll abwickeln, erfahren Sie in den folgenden Beschreibungen.

[#10]
== Auftragsstatus nutzen

Mit Auftragsstatus wird der Fortschritt bei der Abwicklung von Aufträgen abgebildet. 190 Status stehen im System zur Verfügung. 30 sind bereits zu Beginn vergeben und 160 Status stehen Ihnen zur individuellen Kennzeichnung des Fortschritts zur Verfügung. Die meisten der 30 vergebenen Status sind mit Systemautomatismen verknüpft und sollten von Ihnen auch nur für die systemseitig-vorgesehenen Zwecke verwendet werden.
Jeder Auftrag befindet sich in einem Status. Der Auftragsstatus ändert sich durch Systemautomatismen, selbst eingerichtete Automatismen oder manuell. Selbst einrichten können Sie Automatismen durch den Aktionsmanager, Ereignisaktionen oder mit Prozessen. Auftragsstatus dient auch zur besseren Auftragssuche.
...
=== Automatische Statusvergabe durch das System verstehen

Die folgenden 30 Statuseinträge sind bereits zu Beginn in Ihrem plentymarkets-System vorhanden und sind mit Systemautomatismen verknüpft:
...
|wird automatisch zugeordnet, wenn ein Auftrag vom Typ Auftrag bezahlt ist, aber der Bestand der Artikel für den Auftrag nicht ausreicht. Hierfür muss die Beschränkung von Artikeln auf Nettowarenbestand eingestellt sein.

|**[5]** Freigabe Versand
|wird automatisch zugeordnet, wenn ein Auftrag vom Typ Auftrag bezahlt ist und der Bestand der Artikel für den Auftrag ausreicht oder der Bestand unbeschränkt ist.
...
|wird automatisch zugeordnet, wenn der Warenausgang in einem Auftrag gebucht wurde. Das manuelle Setzen des Status sorgt wiederum nicht dafür, dass der Warenausgang tatsächlich gebucht wird.

|**[7.1]** Auftrag exportiert
|Zelle 2

|**[7.2]** Versandabwicklung abgeschlossen
|Zelle 2

|**[8]** Storniert
|empfohlen, um zuzuordnen, dass ein Auftrag vom Typ Auftrag storniert wurde

|**[8.1]** Storniert durch Kunden
|Zelle 2

|**[9]** Retoure
|ist beim Anlegen eines Auftrags vom Typ Retoure automatisch vorausgewählt.
...
|**[11]** Gutschrift
|wird einem Auftrag vom Typ Gutschrift automatisch zugeordnet, wenn eine Gutschrift angelegt wird.
...
__Bild 3: Aufträge suchen, Filteroptionen und Auftragsübersicht__

[#30]
==== Beispieleinstellung

Mit den Einstellungen in Bild 4 (siehe Pfeile) werden bei Klick auf **Suchen** alle Aufträge angezeigt, die sich aktuell im Status 7 oder niedriger befinden und für die der Warenausgang noch nicht gebucht wurde. Die Aufträge im Suchergebnis werden nach der **Auftrags-ID** (also dem Auftragseingang) sortiert angezeigt. Mit diesen Einstellungen werden also alle Aufträge angezeigt, die noch zu bearbeiten sind.

image::auftragsabwicklung/auftraege/assets/DE-AuftraegeSuchen-02.png[]

__Bild 4: Aufträge suchen, Beispieleinstellung__

[#40]
==== Nach IDs und Nummern filtern

Um bestimmte Aufträge zu suchen, ändern Sie die Standardeinstellungen der Filter. Eine Möglichkeit, bestimmte Aufträge schnell zu finden, sind die Suchfelder (Bild 5, Eingabefelder links). Geben Sie in diese Felder IDs oder Nummern ein, um nach Aufträgen, Kunden oder Artikeln in Ihrem System zu suchen und so zugehörige Aufträge zu finden. Durch Eingabe einer **Kunden-ID** erhalten Sie z.B. nur die Aufträge des Kunden mit dieser ID.

image::auftragsabwicklung/auftraege/assets/DE-AuftraegeSuchen-03.png[]

__Bild 5: Aufträge suchen, Suche anhand einer ID__

Wenn Sie beispielsweise eine 3 in das Feld **Auftrags-ID** eingeben und auf **Suchen** klicken, wird auch nur der Auftrag mit der ID 3 in der Übersicht angezeigt. Sie können auf diese Weise auch nach mehreren Aufträgen suchen, indem Sie die **Auftrags-IDs** durch ein Komma getrennt eintragen (Bild 5). Wenn Sie anhand einer **Auftrags-ID** suchen, werden alle übrigen Eingabefelder gesperrt.
...
.Status-Suchfilter
====
Wenn Sie mit einem der Suchfelder nach einem oder mehreren Aufträgen suchen, dürfen Sie auf keinen Fall vergessen, den Suchfilter **Status** (Bild 4, Mitte) korrekt einzustellen. Suchen Sie nach einer bestimmten Rechnungsnummer, setzen Sie den Statusfilter am besten auf den Wert **ALLE**, da der Auftrag ggf. schon ausgeliefert ist oder einen anderen Status jenseits von **7** hat. Bei der Auftragssuche sollten Sie also auf keinen Fall vergessen, dass nur Aufträge angezeigt werden, die wirklich alle eingestellten Kriterien erfüllen. Sollten Sie einen Auftrag trotz eindeutiger ID nicht finden, haben Sie vielleicht vergessen, den Suchfilter **Status** auf den Wert **ALLE** umzustellen.
====

[#50]
==== Nach Kunden- bzw. Artikeldaten und Rechnungsnummern filtern

Für gezielte Suchen anhand von kunden- oder artikelbezogenen Angaben nutzen Sie am besten die Suchfelder **Kundendaten** und **Artikeldaten** (Bild 4 bzw. 5). Hier ist die Eingabe von Suchbegriffen und Zahlen gleichermaßen zulässig. Bei den **Kundendaten** sind neben den üblichen Namensabfragen auch Postleitzahlangaben möglich, jedoch keine Telefonnummern. Bei den **Artikeldaten** werden Artikel-ID, Artikel-Nummer und Name abgefragt, nicht aber die Felder mit den Beschreibungstexten (short_description, description). Bei der Suche nach **Rechnungsnummern** ist auch die Suche nach externen Rechnungsnummern möglich.
...
__Bild 6: Aufträge suchen, **Datumsfilter**__

[.instruction]
Aufträge nach Datum filtern:

. Öffnen Sie das Menü **Aufträge » Aufträge bearbeiten**.
. Wählen Sie aus dem Dropdown-Menü unter **Datum** eine der Filteroptionen, z. B. **Auftragseingang** (Bild 6, Position 1).
. Wählen Sie unter **von** und **bis** das **Start**- und das **Enddatum** (Bild 6, Position 2).
...
__Bild 7: Anzahl der Aufträge pro Seite wählen__
...
__Bild 8: Zu einer Seite wechseln__
...
__Bild 9: Aufträge suchen, Artikelliste geöffnet__

[NOTE]
.Artikelliste öffnen und schließen
====
Klicken Sie auf den blauen Pfeil (Bild 9, roter Pfeil), um die Artikelliste zu öffnen oder zu schließen.
====
...
__Bild 10: Informationen in der Auftragsübersicht__

In der Titelzeile eines Auftrags sehen Sie in der Übersicht auf einen Blick den <<auftragsabwicklung/auftraege/auftraege-verwalten#, **Status**>> und direkt daneben das Logo des <<auftragsabwicklung/fulfillment/versand-vorbereiten#, **Versanddienstleisters**>> sowie der <<auftragsabwicklung/payment#, **Zahlungsart**>> (Bild 10, Pfeil). Je nach Konfiguration und den durchgeführten Aktionen erscheinen dort weitere Symbole.

image::auftragsabwicklung/auftraege/assets/DE-AuftraegeSuchen-09.png[]

__Bild 11: Wechselnde Symbole in der Auftragsübersicht__

Wurde der Warenausgang gebucht, verschwindet das Symbol **Warenausgang buchen** und das Symbol für **Warenausgang gebucht** wird angezeigt (Bild 11, grüner Pfeil). Gleichzeitig wechselt die Anzeige des Status in Status 7 (Bild 11, blauer Pfeil). Anhand der Farbe der Statusanzeige erkennen Sie Aufträge mit gleichem Status auf einen Blick.
...
.Symbole eines Auftrags in der Auftragsübersicht
[cols="1a,2"]
|====
|Symbol |Information
...
.Symbole eines Auftrags in der Auftragsübersicht; Aktionen
[cols="1a,2"]
|====
|Symbol |Funktion
...
__Bild 13: Nachbestellung aus Auftragsübersicht anlegen__
...
. Öffnen Sie das Menü **Aufträge » Aufträge bearbeiten**.
. Klicken Sie bei dem gewünschten Auftrag in der Auftragsübersicht auf das Symbol **Warenbestand ermitteln** (Bild 13, Pos. 1).
. Klicken Sie im eingeblendeten **Warenbestand** auf den **Warenkorb** (Bild 13, Pos. 2). +
...
. Tragen Sie in das Textfeld links oben die gewünschte **Menge** für die Bestellung ein (Bild 13, Pos. 3).
. Wählen Sie den **Lieferanten** aus dem Dropdown-Menü (Bild 13, Pos. 4).
...
__Bild 14: Aufträge suchen, Gruppenfunktionen__
...
. Klicken Sie auf den Button **Status** (Bild 15, Pos. 1). +
→ Eine Liste mit den verfügbaren Status wird angezeigt.
. Wählen Sie den gewünschten Status (Bild 15, Pos. 2).
. Klicken Sie auf **Speichern**, um die Änderung zu sichern (Bild 15, Pos. 3). +
...
__Bild 15: Symbole und Änderungsmöglichkeiten in der Auftragsübersicht__
...
__Bild 16: Tab **Übersicht** eines Auftrags__
...
__Bild 17: Tab **Einstellungen** eines Auftrags__
...
__Bild 18: Tab **Zahlung** eines Auftrags__
...
__Bild 19: Erstellte **Dokumente** im Tab **Belege** eines Auftrags__
...
__Bild 20: **Konfiguration** des Belegs **Rechnung** eines Auftrags__
...
__Bild 21: Markierte Positionen für Nachbestellung wählen__
...
. Wählen Sie die **Varianten**, die Sie nachbestellen möchten (Häkchen setzen, Bild 21, Pos. 1).
. Klicken Sie auf **Markierte Positionen nachbestellen** (Bild 21, Pos. 2). +
...
__Bild 28: Auftragspositionen nachbestellen__
...
. Klicken Sie im Tab **Nachbestellung** des Auftrags auf einen **Warenkorb** (Bild 28, roter Pfeil). +
...
. Klicken Sie im Bereich **Grunddaten** bei der Option **Bestellen** auf das **Zahnrad** (Bild 29, roter Pfeil). +
...
__Bild 29: Nachbestellung ausführen__
...
__Bild 30: Variantenmengen bei Nachbestellungen erhöhen__

[.instruction]
Menge für Variante erhöhen:

. Klicken Sie auf **Mengen für Varianten erhöhen** (Bild 30, Pos. 1).
...
. Klicken Sie auf **Menge erhöhen** (Bild 30, Pos. 2).
...
__Bild 31: Artikelpositionen im Tab **Lieferaufträge**__
...
__Bild 32: Anwendung der Gruppenfunktion__
...
. Klicken Sie auf **Gruppenfunktion** (Bild 32, Pos. 1).
. Markieren Sie die Option **Warenausgang buchen** (Bild 32, Pos. 2).
. Wählen Sie das **Datum** und die **Uhrzeit** (Bild 32, Pos. 3). +
...
. Klicken Sie auf **Anwenden**, um die Aktion auszuführen (Bild 32, Pos. 4).
...
. Öffnen Sie anschließend den Auftrag durch einen Klick in der Zeile in der Auftragsübersicht. +
...
__Bild 33: Eine Gutschrift anlegen__
...
__Bild 34: Gutschrift verfügbar__
...
=== Gutschrift korrigieren und stornieren
...
. Öffnen Sie das Menü **Einstellungen » Mandant (Shop) » Shop wählen » Standorte » Standort wählen » Dokumente » Stornobeleg Gutschrift**.
. Wechseln Sie in das Tab **Transfer**.
. Klicken Sie unter **Einstellungen aus Standard-Dokumentenvorlage übernehmen** auf **Übernehmen**.

Da die Einstellungen der Vorlage für Stornobelege mit den Einstellungen der Vorlage für eine Rechnung übereinstimmen, finden Sie nährere Erläuterungen zu den einzelnen Einstellungen auf der Seite <<omni-channel/online-shop/mandanten-einrichten/dokumente/rechnung#, **Rechnung**>>.
...
.  Öffnen Sie das Menü **Aufträge » Aufträge bearbeiten**.
.  Öffnen Sie den Auftrag, in dem Sie die Gutschrift stornieren möchten.
.  Wechseln Sie in das Tab **Belege**.
.  Wählen Sie aus dem Dropdown-Menü **Beleg erstellen** den **Stornobeleg Gutschrift**.  +
→ Ein Fenster mit optionalen Einstellungen wird geöffnet.
. **Speichern** Sie die Einstellungen.  +
→ Der Stornobeleg wird erzeugt und die Auftragseinstellungen entsperrt.
.  Bearbeiten Sie jetzt die Daten, die Sie korrigieren möchten.
.  Wechseln Sie wieder in das Tab **Belege**.
.  Wählen Sie aus dem Dropdown-Menü **Beleg erstellen** die **Gutschrift**.  +
→ Ein Fenster mit optionalen Einstellungen wird geöffnet.
. **Speichern** Sie die Einstellungen.  +
...
__Bild 36: Gutschrift; Optionen der Rückbuchung des Warenbestandes__
...
.Gutschrift; Optionen zur Rückbuchung des Warenbestandes
[cols="1,2"]
|====
|Einstellung |Erläuterung
...
__Bild 37: Rückzahlung in Gutschrifteinstellungen__

[.instruction]
Rückzahlung ausführen:

. Klicken Sie in der Gutschrift im Tab **Einstellungen** bei der Option **Rückzahlung** auf das **Zahlungs-Icon** (Bild 37).
. Prüfen Sie den **Betrag** der Rückzahlung und passen Sie diesen ggf. an.
. Klicken Sie auf das **Zahnrad**, um die Rückzahlung auszuführen (Bild 38).

image::auftragsabwicklung/auftraege/assets/Auftraege-Gutschrift-06.png[]

__Bild 38: Rückzahlung im Tab **Einstellungen** einer Gutschrift__
...
__Bild 39: Gutschrift-Info: Auftrags-ID__
...
__Bild 40: **Gründe für Retouren** eintragen__
...
__Bild 41: Beispielstatus für zurückgesendete Artikel__
...
. Klicken Sie im Tab **Übersicht** unter **Retoure** auf **anlegen** (Bild 42). +
...
__Bild 43: Einstellungen für eine neue Retoure__

.Optionen beim Anlegen einer Retoure
[cols="1,2"]
|====
|Einstellung |Erläuterung
...
__Bild 44: Retoure bearbeiten__
...
. Wählen Sie den zutreffenden **Artikelstatus** unter **Artikelpositionen** (Bild 44, roter Pfeil).
. Wechseln Sie in das Tab **Einstellungen**.
...
.Optionen beim Bearbeiten einer Retoure
[cols="1,2"]
|====
|Einstellung |Erläuterung
...
__Bild 45: Warenbestand zurückbuchen__
...
.Optionen für Rückbuchungsfunktionen
[cols="1,2"]
|====
|Einstellung |Erläuterung
...
__Bild 46: Menge für die Rückbuchung eintragen__
...
__Bild 47: **Gewährleistung anlegen**__

[.instruction]
Eine Gewährleistung anlegen:

. Öffnen Sie die **Retoure** oder den **Auftrag**.
. Klicken Sie im Bereich **Gewährleistung** auf **anlegen** (Bild 47, roter Pfeil). +
→ Das Fenster **Gewährleistung zum Auftrag** mit der ID wird geöffnet.
. Setzten Sie bei den betreffenden Artikeln ein **Häkchen** (Bild 48, roter Pfeil).
. Klicken Sie auf **Speichern**, um die Einstellungen zu sichern. +
...
__Bild 48: **Artikel markieren**__
...
__Bild 49: Tab **Belege** einer **Gewährleistung**__
...
. Wählen Sie **Rechnung** aus dem Dropdown-Menü **Beleg erstellen** (Bild 49, roter Pfeil). +
→ Der Rechnungsbetrag wird automatisch mit 0,00 angegeben.
. Wechseln Sie in das Tab **Einstellungen**.
. Klicken Sie in der Zeile **Warenausgang** auf das Symbol **Warenausgang buchen** (Bild 50, roter Pfeil). +
...
__Bild 50: Tab **Einstellungen**; Symbol **Warenausgang buchen**__
...
__Bild 51: **Erscheinungsdatum** eintragen__
...
__Bild 52: CMS; Template-Variablen für Erscheinungsdatum__
...
__Bild 53: Tab **Bestand**, Keine Einschränkung__
...
__Bild 54: Aktionsmanager, Tab: **Filter**__
...
__Bild 55: Optionen für das **Splitten von Aufträgen in Lieferaufträge**__
...
.**Parameter des Auftrags**, der gesplittet werden soll
[cols="1,2,3"]
|====
...
.**Aufsplittung nach Versandprofil**
[cols="1,2"]
|====
...
.**Aufsplittung nach Versandprofil bei unterschiedlicher Konfiguration in den Lager-Einstellungen**
[cols="1,2,3,4"]
|====
...
__Bild 56: **Lager-Optionen b) und b)** als Voraussetzung für Lieferaufträge__
...
__Bild 57: **Lager-Optionen b) und c)** als Voraussetzung für Lieferaufträge__
...
__Bild 58: **Optionen** im Menü **Lieferaufträge**__
...
__Bild 59: Ereignisaktion **Lieferauftrag generieren bei Auftragseingang**__
...
__Bild 60: Ereignisaktion **Lieferauftrag generieren bei Wechsel zu Status 5**__
...
__Bild 61: Ereignisaktion **Lieferauftrag generieren bei Rechnungsdruck**__
...
__Bild 62: Lieferaufträge im Tab **Lief** anlegen__
...
. Klicken Sie anschließend auf das **Zahnrad** (s. Bild 62, grüner Pfeil), um den Lieferauftrag anzulegen. +
...
__Bild 63: Lieferauftragshinweis im Hauptauftrag__
...
__Bild 64: Lieferaufträge im Tab **Aufträge** eines Auftrags suchen__
...
__Bild 65: Ereignisaktion **Status Hauptauftrag an Lieferaufträge anpassen; Statuswechsel zu 6.5**__
...
__Bild 66: **Status für Reparaturen** hinterlegen__
...
__Bild 67: **Zustände für Reparaturen** hinterlegen__
...
Verwenden Sie die **Kundenkarte** (Bild 68), um nach einem Kunden zu suchen und einen **__neuen Auftrag__** für ihn anzulegen. Gehen Sie wie unten beschrieben vor, um einen neuen Auftrag über die Kundenkarte anzulegen.

image::auftragsabwicklung/auftraege/assets/DE-Auftraege-bearbeiten-Neue-Auftraege-01.png[]

__Bild 68: **Kundenkarte** aufrufen und Kunden suchen__

[.instruction]
Neuen Auftrag über die Kundenkarte anlegen:

. Klicken Sie auf **Kundenkarte** (Bild 68, Pos. 1). +
→ Das Menü **Kundenkarte** mit einem Eingabefeld wird geöffnet.
. Geben Sie **__mindestens 3 Buchstaben__** des gesuchten Kunden ein (Bild 68, Pos. 2) +
→ Die **Ergebnisliste** wird eingeblendet.
. Wählen Sie den **Kunden** aus der **Ergebnisliste** (Bild 68, Pos. 3), indem Sie mit der Maus darauf klicken. +
...
__Bild 69: **Neuen Auftrag** in der **Kundenübersicht** anlegen__
...
. Klicken Sie in der Zeile des gewünschten **Kunden** auf **Neuer Auftrag**, um einen neuen Auftrag für diesen Kunden anzulegen (Bild 69, roter Pfeil). +
...
__Bild 70: **Neuen Auftrag** im Tab **Kundendaten** anlegen__
...
. Klicken Sie im Tab **Kundendaten** auf **Neuer Auftrag**, um einen neuen Auftrag für diesen Kunden anzulegen (Bild 70, roter Pfeil). +
→ Das Menü **Aufträge » Aufträge bearbeiten » Tab: Neuer Auftrag** wird angezeigt.
. Wählen Sie die Artikel, die Sie zum Auftrag hinzufügen möchten (Kapitel 4).
. Klicken Sie auf **Speichern**, um die Einstellungen zu sichern.
...
__Bild 71: **Neuen Auftrag** im Tab **Kundendaten** anlegen__
...
. Klicken Sie im Tab **Übersicht** unten im Tab **Kundendaten** auf **Neuer Auftrag** (Bild 71, roter Pfeil), um einen Auftrag für diesen Kunden anzulegen. +
→ Das Menü **Aufträge » Aufträge bearbeiten » Tab: Neuer Auftrag** wird angezeigt.
. Wählen Sie die Artikel, die Sie zum Auftrag hinzufügen möchten (Kapitel 4).
. Klicken Sie auf **Speichern**, um die Einstellungen zu sichern.
...
__Bild 72: **Neuen Auftrag** für **Barverkäufe** anlegen__

[.instruction]
Neuen Auftrag für Barverkäufe anlegen:

. Klicken Sie auf **Kundenkarte** (Bild 72, Pos. 1). +
→ Das Menü **Kundenkarte** mit einem Eingabefeld wird geöffnet.
. Geben Sie den Suchtext **bar** ein (Bild 72, Pos. 1) +
→ Die **Ergebnisliste** wird eingeblendet.
. Wählen Sie **Barverkauf** aus der **Ergebnisliste** (Bild 72, Pos. 2), indem Sie mit der Maus darauf klicken. +
...
__Bild 73: Neuer Auftrag, **Artikel hinzufügen**__
...
. Klicken Sie im Bereich **Artikelsuche** auf die Lupe (Bild 73, Pos. 1). +
→ Die Artikel werden aufgelistet.
. Klicken Sie bei den Artikeln, die Sie zum Auftrag hinzufügen möchten, auf den **Warenkorb** (Bild 73, Pos. 2).
. Klicken Sie auf **Speichern**. +
