...
[Note]
.Menü zum Erstellen der Status nur über den zentralen Login erreichbar
====
Das Menü **System » Aufträge » Status** ist nur über den zentralen Login erreichbar, da hier die neusten Technolgien verwendet wurden, die in plentymarkets verfügbar sind.
====
...
Die in folgenden 28 Statuseinträge sind bereits zu Beginn in Ihrem plentymarkets System vorhanden und sind tlw. mit Systemautomatismen verknüpft. Die mit Systemautomatismen verknüpften Status sind nicht löschbar.
...
. Klicken Sie auf den gewünschten Status.
. Ändern Sie die Namen oder fügen weitere hinzu.
...
. Öffnen Sie das Menü **Aufträge » Aufträge bearbeiten**.
. Wählen Sie aus der Dropdown-Liste unter **Datum** eine der Filteroptionen, z. B. **Auftragseingang** (Bild 6, Position 1).
...
|**Artikelliste öffen** oder *schließen*
...
|**Zeichen Kunde**
|Zeigt ein Zeichen bzw. einen Text an, den der Kunde bei der Bestellung hinterlegen kann, wenn das Feld **Ihr Zeichen** für den Bestellvorgang sichtbar ist. Dazu die Hinweise in Tabelle 1 der Handbuchseite **<<omni-channel/online-shop/webshop-einrichten/cms#webdesign-webdesign-bearbeiten-bestellvorgang-kaufabwicklung, Kaufabwicklung>>** beachten.
...
Wurde zu einem Auftrag eine Gutschrift erzeugt, ist diese im Tab **Übersicht** über die Dropdown-Liste **Gutschrift / suchen** verlinkt.
...
.  Bearbeiten Sie jetzt die Daten, die Sie korrigieren möchten.
.  Wechseln Sie wieder in das Tab **Belege**.
.  Wählen Sie aus der Dropdown-Liste **Beleg erstellen** die **Gutschrift**.  +
→ Ein Fenster mit optionalen Einstellungen wird geöffnet.
. **Speichern** Sie die Einstellungen.  +
...
Der Auftragstyp Retoure dient dazu den Geschäftsvorfall einer Retoure abzubilden. Sie und Ihre Mitarbeiter können Retouren manuell erfassen und zusätzlich können Sie auch Ihren Webshop-Kunden erlauben Retouren über den Bereich für registrierte Benutzer anzumelden. Egal auf welchem Weg eine Retoure ins System gelangt, das Erfassen einer Retoure bucht die retournierten Artikel noch nicht zurück in Ihren Warenbestand. Sie erfassen also die retournierten Artikel zunächst und können dann die Artikel prüfen und anschließend entscheiden, ob die Artikel wieder zum Bestand gebucht werden oder ob weitere Schritte, wie z.B. eine Reparatur, notwendig sind. Die Retoure ist ein untergeordneter Auftragstyp, da Retouren entweder einem Auftrag oder einer Gewährleistung zugeordnet sind.
Bevor Sie jedoch Retouren erfassen können, legen Sie Retourengründe und Artikelstatus für retournierte Artikel an. Anhand von Retourengründen ist später nachvollziehbar, ob ein Artikel besonders häufig wegen des selben Grundes retourniert wurde. Mithilfe der Artikelstatus für retournierte Artikel geben Sie an wie gut der Artikelzustand nach der Retoure ist und können so ableiten wie Sie die Artikel weiter verarbeiten.

=== Retourengründe eintragen

Sie können bis zu 20 Retourengründe eintragen. Beim Erfassen einer Retoure wählen Sie dann für jede Artikelposition einen Grund.
...
Sie können bis zu 12 Artikelstatus anlegen. Die Status können Sie z.B. nutzen, um einerseits den Zustand der retournierten Artikel anzugeben, aber auch den Fortschritt beim Bearbeiten der retournierten Artikel. Die Status stehen in einer Retoure im Tab **Übersicht** bei den Artikelpositionen zur Wahl. Um je nach Fortschritt der Bearbeitung verschiedene Status zuzuweisen, legen Sie entsprechend Ihrer Anforderungen die benötigten Status an.
...
Der Standardstatus für Retouren ist Status 9, d.h. jede Retoure, die erfasst wird, erhält automatisch diesen Status. Sie können jedoch jederzeit einen anderen Status als Standard festlegen.

[.instruction]
Standardstatus für Retouren ändern:

. Öffnen Sie das Menü **System » Aufträge » Einstellungen**.
. Wählen Sie für **Standard-Status Retoure** einen Status.
. *Speichern* Sie die Einstellungen.

=== Retoure erfassen und bearbeiten

Eine Retoure erfassen Sie manuell, wenn ein Paket bei Ihnen eingegangen ist oder Sie erlauben Ihren registrierten Webshop-Kunden direkt im Kundenkonto des Shops Retouren anzumelden. Nachdem eine Retoure erfasst wurde, werden weitere Bearbeitungsmöglichkeiten sichtbar. So können Sie z.B. den Artikelstatus der retournierten Artikel erst nach dem Erfassen angeben. Einen Rücksendeschein können Sie Ihren Kunden bei Zustellung der Ware im Paket beilegen. In diesem Fall sollte der Rücksendeschein Blankofelder für die Retourengründe enthalten. Sie können Ihren Kunden aber auch erlauben den Rücksendeschein im Mein-Konto-Bereich auszufüllen und nach Angabe der Retourengründe auszudrucken, um ihn der Retoure beizulegen.
...
. Nehmen Sie die Einstellungen vor. Beachten Sie die Erläuterungen in <<tabelle-Einstellungen-Retoure>>.
. *Speichern* Sie die Einstellungen. +
...
Wenn hier ein Grund gewählt wird, hat dieser Grund Priorität gegenüber den einzel wählbaren Gründen auf Artikelpositionsebene. Gründe legen Sie im Menü **System » Aufträge » Auftragstypen » Retoure**.
...
|**Artikel**
|Artikelname kann hier angepasst werden
|====
...
. Öffnen Sie die Retoure.
. Öffnen Sie das Tab **Einstellungen**.
. Klicken Sie bei der Option **Warenbestand** auf **Warenbestand einbuchen**. +
→ Ein Bearbeitungsfenster wird geöffnet.
. Nehmen Sie die Einstellungen vor. Beachten Sie dazu die Erläuterungen in <<tabelle-Warenbestand-Rückbuchen-Retoure>>
. *Speichern* Sie die Einstellungen.

[TIP]
.Warenbestand zurückbuchen für Retouren von Aufträgen mit Lieferaufträgen
====
Die Vorgehensweise zum Zurückbuchen des Warenbestands ist auch für Retouren zu Aufträgen mit Lieferaufträgen gleich. Das Lieferaufträge vorhanden sind, wird Ihnen im Bearbeitungsfenster für das Rückbuchen angezeigt.
====

image::auftragsabwicklung/auftraege/assets/DE-Auftraege-Retoure-05.png[]

__Bild 46: Warenbestand zurückbuchen bei vorhandenem Lieferauftrag__

[[tabelle-Warenbestand-Rückbuchen-Retoure]]
.Einstellungen beim Rückbuchen
[cols="1,2"]
|====
|Einstellung |Erläuterung
...
. Wählen Sie **Rechnung** aus der Dropdown-Liste **Beleg erstellen** (Bild 49, roter Pfeil). +
...
. Wählen Sie im Bereich **Filter** aus der Dropdown-Liste **Typ** die Option **Lieferauftrag** und wählen Sie die Option **suchen**. +
...
.
====
Diese Status stehen beim Anlegen einer Reparatur in der Dropdown-Liste **Status der Reparatur** zur Verfügung.
====
...
.
====
Diese Zustände stehen beim Anlegen einer Reparatur in der Dropdown-Liste **Zustand** des jeweiligen Artikels zur Verfügung. Sie können dann jedem Artikel den zutreffenden Zustand zuordnen und jeweils bei einer Änderung anpassen.
====
...
. Wählen Sie in der Dropdown-Liste oben links den __**Shop**__, in dem Sie die Auftragsstatus ändern möchten.
. Wählen Sie in der mittleren Dropdown-Liste die __**Sprache**__, für die Sie die Auftragsstatus ändern möchten.
...
. Wählen Sie in der Dropdown-Liste **Von** den Auftragsstatus, den Sie ändern möchten.
. Wählen Sie in der Dropdown-Liste **in**, den neuen Status, den Sie diesen Aufträgen zuweisen möchten.
