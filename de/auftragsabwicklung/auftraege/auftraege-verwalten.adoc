...
. Wählen Sie im angezeigten Fenster die gewünschte Option gemäß den Erläuterungen in [[tabelle-warenbestand-rückbuchung]].
...
[TIP]
.Lieferaufträge filtern
====
Mithilfe der Datumsauswahl und anderer Einstellungen im Bereich Filter können Sie die Genauigkeit Ihrer Suche erhöhen und die Anzahl der angezeigten Lieferaufträge verringern.
====

=== Lieferaufträge im Auftrag selbst suchen

Ob Lieferaufträge zu einem Auftrag bestehen, sehen Sie im Tab *Übersicht* des betreffenden Auftrags. Eine weitere Möglichkeit, sich die Lieferaufträge anzusehen, besteht im Tab *Aufträge* des Auftrags. Hier sehen Sie alle Aufträge des Kunden aufgeführt und somit auch die Lieferaufträge, die Sie dort auch *suchen* können.
...
=== Möglichkeiten zur manuellen Auftragsanlage

Folgende Möglichkeiten sind verfügbar um manuell einen Auftrag für einen Kunden anzulegen:
...
. Klicken Sie in der Zeile des gewünschten *Kunden* auf *Neuer Auftrag*, um einen neuen Auftrag für diesen Kunden anzulegen. +
...
Barverkäufe ohne Kundenzuordnung können Sie über das Kundenprofil *Barverkauf* abwickeln. Das Kundenprofil *Barverkauf* ist in Ihrem System standardmäßig mit der ID 100 eingerichtet.
...
. Klicken Sie im Bereich *Artikelsuche* auf die Lupe. +
→ Die Artikel werden aufgelistet.
. Klicken Sie bei den Artikeln, die Sie zum Auftrag hinzufügen möchten, auf den *Warenkorb*.
. Klicken Sie auf *Speichern*. +
...
. Wählen Sie in der Dropdown-Liste oben links den *Shop*, in dem Sie die Auftragsstatus ändern möchten.
. Wählen Sie in der mittleren Dropdown-Liste die *Sprache*, für die Sie die Auftragsstatus ändern möchten.
. Klicken Sie auf *Gruppenfunktion*. +
→ Das Fenster *Gruppenfunktion* wird angezeigt.
. Wählen Sie die Option *Auftragsstatus aller Aufträge ändern*.
