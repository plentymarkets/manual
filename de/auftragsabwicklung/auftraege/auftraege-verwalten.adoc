...
[#120]
=== Aufträge in der Kontaktübersicht anlegen

Im Menü *CRM » Kontakte* steht pro Kontakt die Option *Neuer Auftrag* zur Verfügung. Gehen Sie wie unten beschrieben vor, um einen neuen Auftrag über einen Kontakt anzulegen.
...
. Wählen Sie aus der Menüleiste die Option *Neuer Auftrag*, um einen neuen Auftrag für diesen Kunden anzulegen. +
→ Das Tab *Neuer Auftrag* wird geöffnet.
. Klicken Sie auf *Suchen*, um sich eine Liste aller verfügbaren Artikel anzeigen zu lassen, oder suchen Sie mithilfe der Suchfelder beispielsweise nach der *Artikel-ID* oder weiteren *Artikeldaten*.
. Wählen Sie die Artikel, die Sie zum Auftrag hinzufügen möchten.
. *Speichern* Sie die Einstellungen.

[130]
=== Auftragsanlage in der Kundenübersicht

Im Menü *CRM » Kunden bearbeiten* legen Sie inen neuen Auftrag wie unten beschrieben über die Kundendaten an.
...
. Klicken Sie im Tab *Kundendaten* auf *Neuer Auftrag*, um einen neuen Auftrag für diesen Kunden anzulegen. +
→ Das Tab *Neuer Auftrag* wird geöffnet.
. Klicken Sie auf *Suchen*, um sich eine Liste aller verfügbaren Artikel anzeigen zu lassen, oder suchen Sie mithilfe der Suchfelder beispielsweise nach der *Artikel-ID* oder weiteren *Artikeldaten*.
. Wählen Sie die Artikel, die Sie zum Auftrag hinzufügen möchten.
. *Speichern* Sie die Einstellungen.
...
[#140]
=== Auftragsanlage über einen bestehenden Auftrag
...
. Klicken Sie im Tab *Übersicht* unten im Tab *Kundendaten* auf *Neuer Auftrag*, um einen Auftrag für diesen Kunden anzulegen. +
→ Das Tab *Neuer Auftrag* wird geöffnet.
. Klicken Sie auf *Suchen*, um sich eine Liste aller verfügbaren Artikel anzeigen zu lassen, oder suchen Sie mithilfe der Suchfelder beispielsweise nach der *Artikel-ID* oder weiteren *Artikeldaten*.
. Wählen Sie die Artikel, die Sie zum Auftrag hinzufügen möchten.
. *Speichern* Sie die Einstellungen.
...
Im Menü *CRM » Kunden bearbeiten* haben Sie die Möglichkeit, Barverkäufe ohne Kundenzuordnung direkt über Kundenprofil *Barverkauf* abzuwickeln. Das Kundenprofil *Barverkauf* ist in Ihrem System standardmäßig mit der ID 100 eingerichtet.

[WARNING]
.Zahlungsart und Auftragsdokumente bei Barverkäufen
====
Achten Sie darauf, bei der Auftragsanlage über Barverkauf in den *Auftragsdaten* die passende *Zahlungsart*, zum Beispiel Barzahlung, auszuwählen. Zudem findet bei dieser Art der Auftragsanlage keine Zuordnung zu einem in Ihrem System gespeicherten Kunden statt. Generieren Sie deshalb die benötigten *<<auftragsabwicklung/auftraege/auftragsdokumente#, Auftragsdokumente>>* wie die Rechnung direkt beim der Abwicklung des Barverkaufs manuell im Tab *Belege* des Auftrags.
====
...
→ Das Kundenprofil für Barverkäufe wird geöffnet. Dieses Kundenprofil hat die ID *100*.
...
er Auftragstyp *Lieferauftrag* ermöglicht es Ihnen, aus einem Hauptauftrag jeweils Lieferaufträge für die enthaltenen Auftragspositionen zu generieren, so dass Sie diese einzeln bw. in Teilen verschicken können. Sie können somit beispielsweise Artikel, die Sie auf Lager haben, sofort losschicken und Artikel, die Sie erst noch geliefert bekommen, später nachsenden. Dies können Sie mit unterschiedlichen oder auch nur mit einem Versanddienstleister durchführen. +
...
* Die Erzeugung eines *Adressetiketts*, eines *Lieferscheins*, einer *Proformarechung* und einer *Gelangensbestätigung* ist direkt im Tab *Belege* des jeweiligen Lieferauftrags möglich.
...
[WARNING]
.Lager pro Artikelposition zuordnen
====
...
Im Menü *System » Aufträge » Auftragstypen » Lieferauftrag* legen Sie fest, nach welchen Kriterien Aufträge in Lieferaufträge gesplittet werden sollen.

[instruction]
.Kriterien für das Splitten von Aufträgen in Lieferaufträge wählen
...
. Klicken Sie anschließend auf das *Zahnrad*, um den Lieferauftrag anzulegen. +
→ Der Lieferauftrag wird angelegt.
. Wiederholen Sie den Prozess mehrmals, um weitere Lieferaufträge anzulegen.
...
Lieferaufträge werden idealerweise über eine Ereignisaktion generiert. Sie als Händler können somit bestimmen, wann und wie die Lieferaufträge zu einem Auftrag generiert und von dem jeweiligen Logistik-Dienstleister abgerufen werden können. Im Folgenden werden drei *Beispiele* zur Generierung von Lieferaufträgen per Ereignisaktion beschrieben.
...
Wenn Sie möchten, dass schon beim Eingang eines Auftrages die zugehörigen Lieferaufträge generiert werden, nutzen Sie folgende Einstellungen.
...
[.instruction]
Ereignisaktion "Lieferauftrag generieren bei Auftragseingang"

. Öffnen Sie das Menü *System » Aufträge » Ereignisse*.
. Klicken Sie auf *Ereignisaktion hinzufügen*. +
→ Das Fenster *Neue Ereignisaktion erstellen* wird geöffnet.
. Tragen Sie einen *Namen* ein, z.B. *Lieferauftrag generieren bei Auftragseingang* und wählen Sie unter *Ereignis* die Option *Neuer Auftrag*.
...
. Tragen Sie im Bearbeitungsfenster einen *Namen* ein, z.B. *Lieferauftrag generieren bei Wechsel zu Status 5* und wählen Sie unter *Ereignis* die Option *Auftragsänderung » Statuswechsel* und als Status *Freigabe Versand*.
. Klicken Sie auf **Speichern**. +
→ Die Ereignisaktion wird angelegt.
. Fügen Sie mit dem *grünen Plus* einen *Filter* des Filtertyps *Auftrag » Auftragstyp* hinzu, markieren Sie im Filter die Option *Auftrag*.
. Fügen Sie mit dem *grünen Plus* einen *Filter* des Filtertyps *Auftrag » Auftragslager* hinzu, markieren Sie im Filter die Option *Mehrere Lager pro Auftrag*.
...
In diesem Beispiel werden die Lieferaufträge zu einem Auftrag erst mit dem Rechnungsdruck des Hauptauftrags generiert. Somit können Sie am Hauptauftrag solange Änderungen vornehmen, bis Sie ihn durch den Rechnungsdruck zum Versand freigeben. Folgende Einstellungen sind nötig:
...
Hauptauftrag folgt immer dem Status des "niedrigsten" Lieferauftrags.
====
...
Der *Status 6.5* bedeutet in unserem Beispiel *Beim Logistiker im Versand*. Analog dazu wäre die Erstellung einer Ereignisaktion bei Wechsel zu **tatus 7.0 Warenausgang gebucht* möglich, wenn der Logistiker den Warenausgang gemeldet hat.
...
====
Wenn Sie einen Hauptauftrag löschen möchten, müssen zuerst die Lieferaufträge gelöscht werden. Erst dann kann der Hauptauftrag gelöscht werden.
====
...
Der Auftragstyp Retoure bildet den Geschäftsvorfall einer Retoure ab. Sie und Ihre Mitarbeiter können Retouren manuell erfassen. Zusätzlich können Sie auch Ihren Webshop-Kunden erlauben, Retouren über den Bereich für registrierte Benutzer anzumelden. Egal auf welchem Weg eine Retoure ins System gelangt, das Erfassen einer Retoure bucht die retournierten Artikel noch nicht zurück in den Warenbestand. Sie erfassen also die retournierten Artikel zunächst und können dann die Artikel prüfen und anschließend entscheiden, ob die Artikel wieder zum Bestand gebucht werden oder ob weitere Schritte, wie z.B. eine Reparatur, notwendig sind. Die Retoure ist ein untergeordneter Auftragstyp, da Retouren entweder einem Auftrag oder einer Gewährleistung zugeordnet sind.
Bevor Sie jedoch Retouren erfassen können, legen Sie Retourengründe und Artikelstatus für retournierte Artikel an. Anhand von Retourengründen ist später nachvollziehbar, ob ein Artikel besonders häufig wegen des selben Grundes retourniert wurde. Mithilfe der Artikelstatus für retournierte Artikel geben Sie an, wie gut der Artikelzustand nach der Retoure ist. So können Sie ableiten, wie Sie die Artikel weiter verarbeiten.
...
Eine Retoure wird benötigt, wenn ein Artikel vom Kunden zurückgesendet wurde. Sie dient unter Anderem dazu, den Warenbestand zu aktualisieren.

[#230]
=== Standardstatus für Retouren ändern

Der Standardstatus für Retouren ist Status 9. Somit erhält jede Retoure, die erfasst wird, automatisch diesen Status. Sie können jedoch jederzeit einen anderen Status als Standard festlegen.
...
Für Retouren steht der Dokumententyp *Rücksendeschein* zur Verfügung. Bevor Rücksendescheine erzeugt werden können, muss die Dokumentenvorlage *Rücksendeschein* im Menü *<<auftragsabwicklung/auftraege/auftragsdokumente#, System » Aufträge » Dokument>>* eingerichtet werden.
...
→ Ein Bearbeitungsfenster wird geöffnet.
. Nehmen Sie die Einstellungen vor. Beachten Sie dazu die Erläuterungen in <<tabelle-warenbestand-rueckbuchen-retoure>>
. *Speichern* Sie die Einstellungen.
...
Bevor Sie eine Gutschrift anlegen, prüfen Sie den Standardstatus für Gutschriften im Menü *<<auftragsabwicklung/auftraege/grundeinstellungen#, System » Aufträge » Einstellungen>>*, passen diesen bei Bedarf an und speichern die Einstellungen.
...
Durch Wahl der Option *Gutschrift / anlegen* im Tab *Übersicht* wird ein neuer Gutschriftauftrag in plentymarkets erzeugt. Hierbei handelt es sich um einen speziellen Auftrag mit allen Artikeln und Preisen des verknüpften Originalauftrags.
...
Es werden alle Gutschriften der Kundenklasse gesammelt in einer Sammelgutschrift abgelegt. Je nach Anforderung empfiehlt es sich, eigene *<<crm/kundendaten-verwalten100, Kundenklassen>>* für Sammelgutschriften anzulegen.
...
Die Option *Rückzahlung an Kunden* wird nur bei den Zahlungsarten *PayPal*, *ClickandBuy* und *heidelpay* angezeigt.

[.instruction]
Rückzahlung ausführen:

. Klicken Sie in der Gutschrift im Tab *Einstellungen* bei der Option *Rückzahlung* auf das *Zahlungs-Icon*.
. Prüfen Sie den *Betrag** der Rückzahlung und passen Sie diesen ggf. an.
. Klicken Sie auf das **Zahnrad**, um die Rückzahlung auszuführen.
...
Wenn die Kosten erstattet werden sollen, legt er eine *<<auftragsabwicklung/auftraege/auftraege-verwalten#, Gutschrift>>* an.

Bei Umtausch oder Reparatur legt er eine *Gewährleistung* an. Ob Sie die *Versandkosten* in einer Gewährleistung übernehmen oder nicht, wählen Sie im Menü *<<auftragsabwicklung/auftraege/grundeinstellungen#, System » Aufträge » Einstellungen>>*.
...
Verschiedene Zustände stehen beim Anlegen einer Reparatur in der Dropdown-Liste *Zustand* des jeweiligen Artikels zur Verfügung. Sie können dann jedem Artikel den zutreffenden Zustand zuordnen und jeweils bei einer Änderung anpassen.

== Auftragstyp Sammelauftrag nutzen

Im Menü **System » Aufträge » Auftragstypen » Multi-Order** konfigurieren Sie die Grundeinstellungen für **<<auftragsabwicklung/auftraege/sammelauftraege#, Sammelaufträge>>**. Die Einstellungen dieses Menüs sind die Grundeinstellungen für alle Sammelaufträge. Hier legen Sie z.B. fest, für welche Auftragstypen und Auftragsstatus Sammelaufträge angelegt werden können.
...
Dieses Tab enthält sämtliche Menüs, die Ihnen auch im Hauptmenü <<crm/kundendaten-verwalten#, CRM » Kunden bearbeiten>> für einen Kunden zur Verfügung stehen. Änderungen, die Sie hier vornehmen, werden auch im Hauptmenü *CRM » Kunden bearbeiten* im Kundendatensatz übernommen.
