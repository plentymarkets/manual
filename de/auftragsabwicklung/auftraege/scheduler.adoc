...
_Bild 1: Einstellungen des Abo-Moduls_
...
→ Ein Code-Beispiel finden Sie unterhalb der Tabelle <<tabelle-template-variablen-abo-modul>>.
. *Speichern* Sie die Einstellungen.
...
_Bild 2: Abo-Funktion: Umsetzung der Template-Variablen im Webshop_
...
_Bild 3: Auflistung Ihrer Scheduler-Aufträge_

Außerdem generieren Sie einen Auftrag aus dem Abonnement heraus durch Klick auf das *Zahnrad*, zu sehen in Bild 3 ganz rechts in der Zeile des Abonnements. Das Generieren eines Auftrags aus dem Abonnement ist notwendig, damit Sie den Auftrag in der Auftragsübersicht finden. Dies wird manuell ausgeführt.
...
. Klicken Sie rechts in der Zeile eines Scheduler-Auftrags auf das *Zahnrad* (Bild 3). +
...
_Bild 4: Spalte *Letzter Durchlauf* gefüllt_

Im Menü *Aufträge* wird ein Auftrag angelegt (Bild 5).

image::auftragsabwicklung/auftraege/assets/DE-Scheduler-Scheduler-Auftraege-03.png[]

_Bild 5: Scheduler-Auftrag in der Auftragsübersicht_
...
_Bild 6: Übersicht im Menü *Aufträge » Durchlauf*_

Sie sehen nach dem Klicken auf eines der Icons *Durchlauf starten* (siehe Bild 6, grüne Pfeile), für welche Abonnement-IDs Aufträge anstehen.
...
Starten Sie den *Durchlauf* dann durch Klick auf eines der Icons (s. grüne Pfeile). Dadurch werden die Abo-Aufträge angelegt und dann im Durchlauf nicht mehr angezeigt.
====
