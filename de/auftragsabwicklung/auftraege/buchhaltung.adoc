...
Die Buchungsschlüssel sind für den DATEV-Export relevant. Wenn Sie nicht mit DATEV arbeiten, überspringen Sie dieses Kapitel. Buchungsschlüssel heißen bei DATEV Steuerschlüssel und dienen der korrekten Übertragung von Buchungsdaten. DATEV hat einige Standardsteuerschlüssel. Der Standardsteuerschlüssel für 19 % Umsatzsteuer ist z.B. 3. Die Steuerschlüssel entnehmen Sie bitte in Ihren DATEV Informationen. plentymarkets ermittelt automatisch, ob es sich um eine Soll- (S) oder eine Haben-Buchung (H) handelt. Diese Information wird dann im Export in einer eigenen Spalte ausgegeben.
...
plentymarkets bietet die Möglichkeit Buchhaltungsdaten zu exportieren und so an ein Buchhaltungsprogramm zu übergeben. Beim Export werden Deibtorenkonten, Erlöskonten und Buchungsschlüssel vergeben, wenn Sie wie oben beschrieben die Einstellungen vorgenommen haben.

Im Menü **Datenaustausch » Daten-Export** wählen Sie das Datenformat und nehmen den <<basics/datenaustausch/daten-exportieren#10, **Datenexport**>> vor.