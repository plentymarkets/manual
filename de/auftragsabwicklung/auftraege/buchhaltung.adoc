...
Die Buchungsschlüssel sind für den DATEV-Export relevant. Wenn Sie nicht mit DATEV arbeiten, überspringen Sie dieses Kapitel. Buchungsschlüssel heißen bei DATEV Steuerschlüssel und dienen der korrekten Übertragung von Buchungsdaten. DATEV hat einige Standardsteuerschlüssel. Der Standard-Steuerschlüssel für 19 % Umsatzsteuer ist z.B. 3. Die Steuerschlüssel entnehmen Sie bitte in Ihren DATEV Informationen. plentymarkets ermittelt automatisch, ob es sich um eine Soll- (S) oder eine Haben-Buchung (H) handelt. Diese Information wird dann im Export in einer eigenen Spalte ausgegeben.
...
Debitorenkonten richten Sie z.B. für Ihre Forderungen ein. In plentymarkets können Sie wählen anhand welches Kriteriums Forderungen Debitorenkonten zugeordnet werden. Sie können z.B. die Nachnamen der Debitoren oder das Lieferland zur Zuordnung nutzen. Legen Sie also zuerst das Kriterium fest und tragen dan die Konten ein.
