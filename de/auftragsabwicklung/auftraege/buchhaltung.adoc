...
Bei Einlagen und Entnahmen wählen Kassenbenutzer ein Konto aus einer Dropdown-Liste, damit die Beträge spezifischen Buchungskonten zugewiesen werden. Systemseitig sind eine Entnahme und eine Entnahme bereits hinterlegt und können nicht gelöscht werden. Speichern Sie zunächst Konten für diese Standardvorfälle. Optional speichern Sie dann weitere Konten.
...
→ Das Tab *Buchungsschlüssel* wird geöffnet.
. Wechseln Sie in das Tab *Kassenvorfälle*.
. Geben Sie im Bereich *Entnahme* für die Bezeichnung *Entnahme* in die Spalte *Konto* das Konto ein, auf das allgemeine Entnahmen gebucht werden.
. Geben Sie im Bereich *Einlage* für die Bezeichnung *Einlage* in die Spalte *Konto* das Konto ein, auf das allgemeine Einlagen gebucht werden.
. *Speichern* Sie die Einstellungen. +
→ die Buchungskonten werden gespeichert. +
