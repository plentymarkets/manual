...
Buchungsschlüssel heißen bei DATEV Steuerschlüssel und dienen der korrekten Übertragung von Buchungsdaten.
Erlöskonten richten Sie ein, um Ihre Umsatzerlöse zu verbuchen. Sie haben die Möglichkeit Erlöskonten pro Steuersatz anzugeben sowie für umsatzsteuerfreie Erlöse.
Debitorenkonten richten Sie ein, um Ihre Forderungen zu verwalten.

=== Buchungsschlüssel einrichten

Die Buchungsschlüssel sind für den DATEV-Export relevant. Wenn Sie nicht mit DATEV arbeiten, überspringen Sie dieses Kapitel. DATEV hat einige Standardsteuerschlüssel.
...
plentymarkets ermittelt automatisch, ob es sich um eine Soll- (S) oder eine Haben-Buchung (H) handelt. Diese Information wird dann im Export in einer eigenen Spalte ausgegeben.
