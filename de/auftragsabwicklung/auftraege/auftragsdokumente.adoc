...
Die Option **Barcode** kann entweder aus der Auftrags-ID oder der Dokumentnummer erstellt werden. Des Weiteren können Sie zwischen **EAN13** oder **Code128** als Kodierung des Barcodes wählen. Wenn Sie die Option **EAN13** wählen und Ihre Auftrags-ID z.B. aus 6 Ziffern besteht, wird diese automatisch ergänzt. Es werden führende Nullen und eine 1 an den Anfang des Barcodes gestellt, so dass sich aus der Auftrags-ID eine EAN13 ergibt. Wenn Ihre Auftrags-ID so übernommen werden soll, wie sie ist, wählen Sie die Option **Code128**. Wählen Sie außerdem, ob die Ziffern unterhalb des Barcodes, innerhalb des Barcodes oder nicht angezeigt werden sollen.
