...
Stellen Sie die Versandkosten für Bestellungen über eBay im Menü *Einstellungen » Listings » Kaufabwicklung* ein. Wählen Sie dazu bei der Option *Versandkosten neuer eBay-Aufträge* die Einstellung *von eBay übernehmen* oder *von plentymarkets berechnen lassen*.
...
====
Nutzen Sie über die Schnittstelle zum DHL Geschäftskundenportal die Funktion *Versenden*. Das Plugin *DHL Shipping (Versenden)* steht Ihnen im link:https://marketplace.plentymarkets.com/plugins/integration[plentyMarketplace, window="_blank"] zur Verfügung.
...
DHL-Exportdokumente werden ausschließlich für Aufträge erzeugt, die mit dem Produkt *DHL Paket International* versendet werden und bei denen es sich um Ausfuhrlieferungen in Länder außerhalb der EU handelt. Sendungen ins Ausland muss eine Zollinhaltserklärung, ein sogenanntes CN23-Dokument, beigelegt werden. *DHL Paket International* kann auch für Sendungen innerhalb der EU genutzt werden, allerdings werden dann keine Exportdokumente erzeugt.

Sendungen an Länder außerhalb der EU dürfen maximal 5 Artikelpositionen enthalten. Pro Artikelposition werden die folgenden Parameter gemeldet: Gewicht, Zolltarifnummer, Herstellerland, Artikelname, Warenwert und Menge.

Wenn Sie die DHL Versenden-Schnittstelle verwenden, wird das Exportdokument automatisch mit der Anmeldung im Versand-Center beim Versandetikett gespeichert.

Wenn Sie die DHL Intraship-Schnittstelle verwenden, müssen Sie das Exportdokument über das Menü *Aufträge » Fulfillment* manuell erzeugen. Damit Aufträge für das Erzeugen der Exportdokumente gefunden werden können, müssen diese zuvor erfolgreich über das Menü *Aufträge » Versand-Center* bei DHL Intraship/Versenden angemeldet worden sein.

Voraussetzung für das Erzeugen eines Exportdokuments ist sowohl bei der DHL Versenden-Schnittstelle als auch bei der DHL Intraship-Schnittstelle, dass eine Rechnung für den Auftrag vorhanden ist.

Gehen Sie wie im Folgenden beschrieben vor, um bei Verwendung der Schnittstelle von DHL Intraship die Exportdokumente manuell zu erzeugen.

[.instruction]
Exportdokument für DHL Intraship erzeugen:
...
====
Wenn Sie mit dem Erzeugen der Exportdokumente gleichzeitig den Warenausgang buchen möchten, können Sie das über eine Ereignisaktion im Menü *Einstellungen » Aufträge » Ereignisaktionen* erledigen. Weitere Informationen finden Sie auf der Handbuchseite <<basics/automatisierung/ereignisaktionen#, *Ereignisaktionen*>>.
====
...
Gehen Sie wie im Folgenden beschrieben vor, um die Schnittstelle zu link:http://www.parcelforce.com/[**Parcelforce Worldwide**, window="_blank"] in plentymarkets einzurichten.

[.instruction]
Schnittstelle einrichten:

. Öffnen Sie das Menü *Einstellungen » Aufträge » Versand » Versanddienstleister » Parcelforce Worldwide*.
