...
Wenn ein Auftrag fertig gepackt wurde, sollte in plentymarkets der Warenausgang gebucht werden. Der *Status [7] Warenausgang gebucht* und das aktuelle Datum als Warenausgangsdatum werden am Auftrag gespeichert. Dieser Vorgang kann beispielsweise mittels Abscannen des Barcodes auf der Rechnung, Lieferschein, Packliste oder dem Adressetikett durchgeführt werden. Achten Sie auf die korrekte Konfiguration Ihres Barcodescanners.
...
|*Kundeneigenschaften*
|Die Koordinaten zur Positionierung der Kundeneigenschaften eingeben. Die <<crm/kundendaten-verwalten#400, Eigenschaft>> aus der Dropdown-Liste wählen. Auf den Adressetiketten wird der Text angezeigt, der im Menü <<crm/kundendaten-verwalten#1500, CRM » Kunden bearbeiten » Kunde suchen » Kunde öffnen » Tab: Eigenschaften>> gespeichert wurde.
