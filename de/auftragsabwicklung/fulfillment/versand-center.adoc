...
Im Menü *Aufträge » Versand-Center* steuern Sie alle Aktionen, die im Zusammenhang mit dem Versand Ihrer Artikel über die integrierten Versandschnittstellen stehen. Neben der Suche und Bearbeitung von Versandaufträgen haben Sie die Möglichkeit, Versandaufträge direkt bei einem Versanddienstleister <<auftragsabwicklung/fulfillment/versand-center#700, anzumelden>> oder eine <<auftragsabwicklung/fulfillment/versand-center#1400, Polling-Datei>> zu erzeugen, um diese einem Versanddienstleister zu übergeben. Je nach Versanddienstleister und Versanddienstleister-Software ist der eine oder andere Weg notwendig.

Außerdem erfahren Sie auf dieser Handbuchseite, wie Sie eine <<auftragsabwicklung/fulfillment/versand-center#1500, Auftragszusammenfassung>> der in plentymarkets <<auftragsabwicklung/fulfillment/versand-vorbereiten#2500, eingerichteten Versanddienstleister>> erzeugen, <<auftragsabwicklung/fulfillment/versand-center#1600, Retouren anmelden>> und <<auftragsabwicklung/fulfillment/versand-center#1900, Labels erzeugen>>.
...
*_Hinweis_*: Wenn ein Benutzer mit der Benutzerklasse *Variabel* die Filteroption *ALLE* wählt, werden ihm trotzdem nur die Aufträge angezeigt, deren Status für ihn im Benutzerkonto als *sichtbare Auftragsstatus* ausgewählt sind.
...
. Öffnen Sie das Menü *Aufträge » Versand-Center*.
. Wählen Sie die Suchkriterien. Beachten Sie dazu die Informationen in Kapitel <<auftragsabwicklung/fulfillment/versand-center#100, Versandauftrag suchen>>.
...
*_Wichtig_*: Die Anzahl der Versandpakete im Menü *Aufträge » Aufträge bearbeiten » Auftrag öffnen » Tab: Einstellungen* hat keinen Einfluss auf die hier hinterlegten Pakete und wird bei Änderungen nicht angepasst. Daher die Anzahl der Pakete ausschließlich im Versand-Center ändern.
...
Weitere Informationen finden Sie im Bereich <<auftragsabwicklung/auftraege#, Aufträge>>.

[#700]
== Versandauftrag anmelden

Im Menü *Aufträge » Versand-Center » Tab: Anmelden* melden Sie Ihre Aufträge bei einem Versanddienstleister an und übertragen diese gleichzeitig an den Dienstleister. Voraussetzung für die Anmeldung ist, dass Sie zuvor im Tab *Suche* <<auftragsabwicklung/fulfillment/versand-center#100, Aufträge gesucht>> haben, die jetzt hier angezeigt werden. Sie haben z.B. die Möglichkeit, mit den Filtereinstellungen in der Suche des Versand-Centers nur Aufträge für einen oder für alle Versanddienstleister anzuzeigen. Bei der Anmeldung erhalten Sie sowohl im Erfolgsfall als auch im Fehlerfall eine Meldung vom Versanddienstleister.
...
→ Diese Funktion kann mit einer <<basics/automatisierung/ereignisaktionen#, Ereignisaktion>>, die z.B. automatisch den Warenausgang bucht, kombiniert werden.
...
Beachten Sie auch das Kapitel <<auftragsabwicklung/fulfillment/versand-center#2000, Mögliche Fehlerquellen beim Anmelden des Versandauftrags>> auf dieser Handbuchseite.
...
. Klappen Sie im Menü *Aufträge » Versand-Center » Tab: Suche* den angemeldeten Auftrag auf. Wie Sie den angemeldeten Auftrag anzeigen, ist im Kapitel <<auftragsabwicklung/fulfillment/versand-center#900, Angemeldeten Auftrag anzeigen>> beschrieben.
. Klicken Sie auf den Stift im Tab *Versandpakete*. +
→ Die Paketnummer wird geöffnet.
. Geben Sie eine andere Paketnummer ein.
. *Speichern* Sie die Einstellungen.

Informationen zum Import von Paketnummern finden Sie im Kapitel <<auftragsabwicklung/fulfillment/dokumente-erzeugen#800, Paketnummern importieren>> auf der Handbuchseite <<auftragsabwicklung/fulfillment/dokumente-erzeugen#, Dokumente erzeugen>>.
...
Das Versandlabel steht Ihnen im Menü *Aufträge » Versand-Center* im angemeldeten Versandauftrag zur Verfügung. Außerdem haben Sie im Versand-Center im Tab <<auftragsabwicklung/fulfillment/versand-center#1900, Label>> die Möglichkeit, die komplette Label-Anzahl sämtlicher Versandaufträge eines Dienstleisters in einer PDF-Datei zu generieren und auszudrucken.
...
. Öffnen Sie das Menü *Aufträge » Versand-Center*.
. Wählen Sie die Suchkriterien für den Auftrag. Beachten Sie dazu die Informationen in Kapitel <<auftragsabwicklung/fulfillment/versand-center#100, Versandauftrag suchen>>.
...
====
Wenn der Versanddienstleister *DHL Easylog* gewählt ist, steht die Option *Kopfzeile* zur Verfügung. Mit dieser Option wählen Sie, ob die Kopfzeile in der CSV-Datei angezeigt werden soll. Weitere Informationen dazu finden Sie auf der Handbuchseite <<auftragsabwicklung/fulfillment/versand-vorbereiten#, Versand vorbereiten>> im Kapitel <<auftragsabwicklung/fulfillment/versand-vorbereiten#2700, DHL Easylog>>.
====
...
====
Als Alternative zur Auftragszusammenfassung eines Tages im Menü *Aufträge » Versand-Center » Tab: Zusammenfassung* können Sie einen <<basics/datenaustausch/export-import/daten-exportieren#, dynamischen Datenexport>> durchführen. Nutzen Sie dazu das Datenformat <<basics/datenaustausch/export-import/datenformate/order#, Order>> und filtern Sie nach Versanddienstleister und Datum. Mit dem Datenfeld *PackageNo* wird z.B. die Paketnummer exportiert.
====
...
. Öffnen Sie das Menü *Aufträge » Versand-Center » Tab: Label*.
. Klicken Sie auf das PDF-Symbol hinter dem Versanddienstleister.  +
→ Die Labels werden zum Download hinzugefügt.
. Kicken Sie auf *Generieren*. +
→ Sie erhalten die PDF-Dateien per Download und können die Labels drucken.
...
Es ist nicht möglich, Pakete im Auftrag im Tab <<auftragsabwicklung/auftraege/auftraege-verwalten#auftrag-aendern-40, Einstellungen>> hinzuzufügen. Wenn Sie weitere Pakete zu einen Auftrag hinzufügen oder die Anzahl ändern möchten, öffnen Sie den Auftrag im Versand-Center und bearbeiten Sie diesen im Tab <<auftragsabwicklung/fulfillment/versand-center#400, Hinzufügen>>.
