...
|**Warenkorbumrandung**
|Der hexadezimale HTML-Code der Haupt-Erkennungsfarbe. PayPal lässt diese Farbe in einem Rahmen um die Warenkorb-Übersicht in der PayPal Checkout-Benutzeroberfläche stufenlos zu weiß verlaufen.
...
Richten Sie die **<<auftragsabwicklung/payment/zahlungsarten-verwalten#20, Zahlungsart >>** PayPal in Ihrem plentymarkets ein und aktivieren Sie die Zahlungsart. Prüfen Sie zudem die Einstellungen für **Erlaubte Zahlungsarten** in den **<<auftragsabwicklung/payment/zahlungsarten-verwalten#30, Kundenklassen >>** und für **Gesperrte Zahlungsarten** in den **<<auftragsabwicklung/fulfillment/versand-vorbereiten#1000, Versandprofilen >>**.
...
Diese Adresse funktioniert auch, wenn Sie die PayPal-Standard-Integration verwenden oder im Webshop PayPal als Zahlungsart nicht anbieten.

[WARNING]
.SSL-Zertifikat
====
Wenn Sie in Ihrem Webshop keine SSL-Verschlüsselung verwenden, verwenden Sie die URL mit http, statt mit https. Wir empfehlen die Verwendung eines <<basics/arbeiten-mit-plentymarkets/plentymarkets-konto/service-center/ssl-zertifikat#, **SSL-Zertifikats**  >>.
====
...
. Wählen Sie aus dem Dropdown-Menü **Codierung** die Option **UTF-8** (Bild 2).
. Aktivieren Sie **Ja** für die Option, ob diese Codierung auch verwendet werden soll, wenn PayPal Daten an Sie sendet.
. **Speichern** Sie die Einstellungen.
...
Eine weitere Einstellung im PayPal-Konto kann für Händler sehr sinnvoll sein: **Deaktivieren von direkten Zahlungen an Ihre E-Mail-Adressen**. Mit der Funktion **Geld senden** können alle PayPal-Kunden nach Belieben Zahlungen an E-Mail-Adressen senden. Dabei ist die Angabe einer Lieferanschrift nicht erforderlich, wodurch wiederum der Verkäuferschutz außer Kraft gesetzt wird. Sie können diese Art von Zahlungen in Ihren PayPal-Einstellungen blockieren.
...
. Wählen Sie im Dropdown-Menü **Dokument** die Option **Abrechnungsbericht**. +
...
Anschließend richten Sie eine **<<auftragsabwicklung/auftraege/einstellungen/ereignisaktionen#, Ereignisaktion  >>** ein, um die Beträge automatisch an PayPal zu senden.
