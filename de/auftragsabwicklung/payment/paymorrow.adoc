...
Für die Einrichtung von Paymorrow in plentymarkets benötigen Sie einen Akzeptanzvertrag mit Paymorrow sowie die nötigen Zugangsdaten. Im **link:http://www.paymorrow.de/onlinehaendler/integration/index.php[Händlerbereich, window="_blank"]** von Paymorrow wird die Vorgehensweise bezüglich des Vertrags erläutert.
...
Fordern Sie ggf. unter link:mailto:technik@paymorrow.de[technik@paymorrow.de] einen Test-Account an und führen Sie testweise einige Transaktionen durch, bevor Sie die Zugangsdaten einrichten. Zu dieser Vorgehensweise stellt Paymorrow ein **link:https://www.plentymarkets.eu/documents/category/2611/Abnahmeprotokoll-paymorrow.pdf[technisches Abnahmeprotokoll, window="_blank"]** zur Verfügung, das Sie ausfüllen müssen, um die Zugangsdaten zu erhalten. In diesem Dokument ist auch die Vorgehensweise erläutert. Für den Testbetrieb erfolgt die Authentifizierung über eine eigene <<auftragsabwicklung/payment/paymorrow#40, **RSA-Zertifikatsregistrierung** >>.
...
Wenn Sie Paymorrow nutzen und Rechnungen für Aufträge erzeugen, die mit der Paymorrow-Zahlungsart Rechnungskauf bezahlt werden, ist es notwendig, dass Sie die Bankdaten von Paymorrow auf der Rechnung anzeigen statt Ihrer Bankdaten. Dies geschieht automatisch über einen Zahlungshinweis. Wenn Sie Ihre eigene **<<auftragsabwicklung/auftraege/einstellungen/dokumente#170, PDF-Vorlage >>** verwenden möchten, gehen Sie wie nachfolgend beschrieben vor.
...
Wenn ein Kunde die Ware oder einen Teil davon zurückgeschickt hat, müssen Sie eine Gutschrift anlegen und Paymorrow darüber informieren, damit der Kunde keine Mahnung von Paymorrow erhält. Richten Sie eine <<auftragsabwicklung/auftraege/einstellungen/ereignisaktionen#, **Ereignisaktion**  >> ein, um Gutschriften automatisch an Paymorrow zu senden.
...
Wird ein Auftrag storniert, muss der Status den Einstellungen unter **Einstellungen » Aufträge » Grundeinstellungen** entsprechen, damit der Vorgang auch als storniert an Paymorrow übermittelt wird. Richten Sie eine <<auftragsabwicklung/auftraege/einstellungen/ereignisaktionen#, **Ereignisaktion**  >> ein, um Stornierungen automatisch an Paymorrow zu senden.
