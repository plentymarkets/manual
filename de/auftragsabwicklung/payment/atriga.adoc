...
Am Ende der Registrierung erhalten Sie eine atriga-Kundennummer. Diese Kundennummer senden Sie einfach per E-Mail an link:mailto:info@atriga.com[info@atriga.com]. In dieser E-Mail geben Sie das Stichwort „plentymarkets“ im Betreff an, um die oben beschriebenen Konditionen zu erhalten. atriga stellt Sie dann auf plentymarkets Kunden um. Nach der Umstellung erhalten Sie von atriga eine Nachricht und können anschließend die automatisierte Übertragung der Inkassomandate einrichten.  +
...
|*Überprüfungsmethode*
|*Nachträgliche Bonitätsprüfung* = der Kunde gibt sein Einverständnis zur Prüfung und wird anschließend geprüft. Wenn er sein Einverständnis nicht gibt, stehen ihm die Zahlungsarten Rechnung, Lastschrift und Kreditkarte nicht zur Verfügung. Für diese Prüfung sind Anpassungen an Ihrem Design notwendig. Im Template <<auftragsabwicklung/payment/atriga#120, PageDesignPrepareMainColumn>> müssen die Template-Variablen *AtrigaRequireUserConfirmation* und *AtrigaShowValidationError* angepasst werden.
...
→ <<bild-aktion-atriga-konfigurieren>> zeigt eine Kombination der Einstellungen in diesem Tab.
. Wählen Sie im Tab *Aktion* für *atriga (Inkasso)* die Einstellung *AKTIV*.
. *Speichern* Sie die Einstellungen.

Im Beispiel in <<bild-aktion-atriga-konfigurieren>> werden alle Aufträge, die im Status *3 Warten auf Zahlung* sind und schon *20 Tage* diesen Status haben, an atriga geschickt. Im atriga link:http://dm.atriga.de/[DebitManager™^] finden Sie Informationen zu Ihren Inkassoaufträgen.
