...
Warum funktioniert mein Plugin nicht?::
    ...
    image:plugins/assets/plugin-aktive-version.png[]
...
Bei mir wird kein Zeitstempel angezeigt. Was kann ich tun?::
    Wenn bei dir kein Zeitstempel angezeigt wird, ist der Bereitstellungsprozess fehlgeschlagen. Du kannst dir stattdessen anzeigen lassen, warum der Prozess gescheitert ist, indem du auf die Schaltfläche *Log* klickst: +
    image:plugins/assets/plugin-set-fehlerlog.png[]

Im *Log* steht `Plugin MeinPlugin requires "xy".:`. Was bedeutet das?::
...
Es erscheint weder ein Zeitstempel noch ein Log-Eintrag. Was nun?::
    Prüfe, dass die DNS-Einstellungen deiner Domain richtig hinterlegt sind und du ein gültiges SSL-Zertifikat besitzt. +
    Weitere Informationen zum Hinterlegen von DNS-Einstellungen findest du im Forumsbeitrag link:https://forum.plentymarkets.com/t/selbsthilfe-bei-dns-problemen-wie-hinterlege-ich-korrekte-dns-einstellungen/65559[Selbsthilfe bei DNS-Problemen^]{nbsp}icon:external-link[]. +
    Weitere Informationen zum Bestellen von SSL-Zertifikaten findest du im Forumsbeitrag link:https://forum.plentymarkets.com/t/leitfaden-ssl-bestellung/214417[Leitfaden SSL-Bestellung^]{nbsp}icon:external-link[].
...
Nachdem ich ein Plugin installiert und bereitgestellt habe, kann ich mich nicht mehr in mein System einloggen. Wie komme ich wieder in mein Backend?::
    Log dich im <<../basics/arbeiten-mit-plentymarkets/backend-bereiche-aufrufen.adoc#20, abgesicherten Modus>> ein. <<installierte-plugins-entfernen#plugin-deaktivieren, Deaktivere>> das Plugin. Du solltest dich jetzt wieder normal einloggen können.