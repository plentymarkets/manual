...
→ Das Menü *Konfiguration* ist automatisch geöffnet.
. Öffne (icon:plus-square-o[]) das Menü *Einstellungen*.
. Gib deine E-Mail-Adresse, das Passwort, die Basis-URL und die Herkunfts-ID ein. +
...
Damit die plentymarkets Attribute den Attributen bei Scalerion korrekt zugeordnet werden können, musst du die in deinem plentymarkets System gespeicherte ID für die jeweiligen Attribute in den Einstellungen des Scalerion-Plugins speichern.
...
. Klicke in die Zeile des Plugin-Sets, das das Scalerion-Plugin enthält.
. Klicke in die Zeile des Scalerion-Plugins.
. Öffne (icon:plus-square-o[]) das Menü *Attributzuordnung*.
. Gib die jeweiligen IDs für die Attribute _Größe_ und _Farbe_ ein. +
→ Du findest die IDs im Menü *Einrichtung » Artikel » Attribute*.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
