...
'''
*Datenfeld wählen* = Ein bereits in plentymarkets vorhandenes Datenfeld wird an OTTO Market exportiert. Das Datenfeld aus der Dropdown-Liste *Datenfeld* wählen. +
...
* Aufträge mit dem OTTO Market-Status ANNOUNCED werden ohne Adressdaten in Status *[1] Unvollständige Daten* in plentymarkets importiert. Sobald der Auftrag bei OTTO in den Status PROCESSABLE wechselt, werden die Adressen am Auftrag ergänzt und der Auftrag wird in Status *[3] Warten auf Zahlung* verschoben. +
*_Wichtig:_* Standardmäßig wird für Aufträge im Status *[1] Unvollständige Daten* kein Warenbestand reserviert. Damit Warenbestand für diese Aufträge reserviert wird, wähle im Menü *Einrichtung » Aufträge » Einstellungen*. Wähle bei der Einstellung *Status Auftragsreservierung (Reservierung von Beständen)* für die Option *von* den Status *[1] Unvollständige Daten* und für die Option *bis* den Status *[18] Uneinbringliche Forderungen*. Wähle in der letzten Dropdown-Liste die Option *Alle Aufträge*.
...
Für die Annahme einer Retoure muss zuerst ein Auftrag vom Typ *Gutschrift* erstellt werden. In einem zweiten Schritt wird die Retoure an OTTO gemeldet. Für Retourenmeldungen benötigst du also zwei Ereignisaktionen:

* Ereignisaktion 1: Aufträge vom Typ *Gutschrift* erstellen
* Ereignisaktion 2: Retouren an OTTO melden

Richte diese Ereignisaktionen wie unten beschrieben ein.

[[bild-retoure-otto-gutschrift]]
.Ereignisaktion, um Aufträge vom Typ *Gutschrift* zu erstellen
...
Richte eine weitere <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um OTTO Market über die Retouren zu informieren.
...
[#1000]
=== Ablehnung von Retouren an OTTO melden

Richte eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um OTTO Market über die Ablehnung von Retouren zu informieren.
