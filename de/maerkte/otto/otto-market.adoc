...
:keywords: OTTO Market, OTTO, Otto einrichten, Otto-Assistent, Otto Assistent, Otto-Kategoriegruppe, Otto Kategoriegruppe, Otto Marktplatz
...
(Pflichtfeld)
| Die "My Otto"-Identifikationsnummer (MOIN) ist eine eindeutige Produktkennzeichnung von OTTO Market. Ab 11.08.2021 muss jede Variante für den Verkauf auf OTTO Market über eine MOIN verfügen.

* Dem Marktplatz-Datenfeld *"My Otto"-Identifikationsnummer* wird im Katalog automatisch das plentymarkets Datenfeld *MOIN* zugewiesen.
...
* Es gibt zwei Möglichkeiten, wie deine Varianten zu einer MOIN kommen:
  ...
  ** *_Möglichkeit 2:_* Das Produkt wird auf OTTO Market noch nicht angeboten. +
  → OTTO Market erstellt eine neue Variante und weist sie deiner Variante zu. +
  In beiden Fällen ist der Ablauf wie folgt:
  . Die Variante wird zu OTTO Market exportiert.
  . 15 Minuten später vergibt OTTO Market eine MOIN für die Variante.
  . Die MOIN wird in dein plentymarkets System importiert und im Menü *Artikel » Artikel bearbeiten » Tab: Einstellungen » Bereich ASIN/ePID* der Variante gespeichert.
