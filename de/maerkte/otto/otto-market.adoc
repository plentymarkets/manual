...
Richte eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um OTTO Market über Retouren zu informieren.
...
* Durch die Aktion *Retoure an OTTO melden* erstellt OTTO einen Gutschriftenbeleg. Durch diesen Beleg wird in plentymarkets ein Auftrag des Typs *Gutschrift* erzeugt. Der Auftrag des Typs *Gutschrift* erhält anschließend einen Gutschriften-Beleg. Durch den Beleg wird ein Kindauftrag des Typs *Gutschrift* an der Retoure erzeugt.
* Wenn im ursprünglichen Auftrag ein Coupon verwendet wurde, wird dieser Coupon auch im Gutschriften-Auftrag als Artikelposition importiert und die Preise werden entsprechend berücksichtigt.
* OTTO erstattet immer den kompletten Verkaufspreis einer Position. Teilerstattungen für eine Auftragsposition können nicht an OTTO Market gemeldet werden. Auch wenn du einzelne Positionen im Preis korrigiert hast, hat dies also keinen Einfluss auf das von OTTO erstellte Erstattungsdokument.
|====
