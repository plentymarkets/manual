...
OTTO Market ist das Marktplatzmodell von OTTO. Du wirst als Verkäufer:in an deinen Produkten angezeigt und die Kommunikation mit Kund_innen wird von dir selbst verwaltet. +
...
Um den Assistenten zu starten, öffne das Menü *Einrichtung » Assistenten » Omni-Channel* und klicke auf den Assistenten *OTTO Market Grundeinstellungen*. Der Assistent wird geöffnet und du kannst mit der Kontoeinrichtung beginnen.
...
*_Tipp:_* Pflichtfelder sind mit Sternchen gekennzeichnet. Diese Pflichtfelder wurden bereits im Assistenten *OTTO Market Kataloge erstellen* verknüpft.
...
*_Tipp:_* Wenn du Bestand aus mehreren Lagern zu OTTO Market übertragen willst, klicke auf *Quelle hinzufügen* (icon:link[role="yellow"]) und füge weitere Lager hinzu.
. *Speichere* (icon:save[role="green"]) die Einstellungen. +
...
[[bild-retoure-otto]]
.Ereignisaktion um einen Retoure an OTTO Market zu melden
