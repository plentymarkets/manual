...
*Hinweis:* Wenn du die Schnittstelle zu real.de noch nicht eingerichtet hast und dir testweise eine Vorschau der Produktdatei ansehen möchtest, musst du die Schnittstelle kurzzeitig aktivieren, da die Vorschau der Produktdatei ansonsten leer ist. Um dir eine Vorschau der Produktdatei anzusehen, aktiviere die Schnittstelle und klicke dann auf *Vorschau Produktdatei* (icon:eye[role="blue"]). Deaktiviere die Schnittstelle danach sofort wieder, indem du für die Option *Schnittstelle zu real.de aktivieren* die Einstellung *Nein* wählst und speicherst. Dadurch vermeidest du, dass vorzeitig Artikel übertragen werden.
...
|*Lager*
|Wählen, aus welchen Lagern Bestand zu real.de übertragen werden soll. Ein Lager oder *Summe aus allen Lagern* wählen. +
...
*Hinweis:* Bei real gelten Montag bis Freitag als Werktage. +
...
Damit du Artikel, Merkmale etc. mit real.de verknüpfen kannst, muss im Menü *Einrichtung » Aufträge » Auftragsherkunft* die Auftragsherkunft real.de aktiviert werden.
...
====
Beim Auftragsimport von real.de findet der Abgleich über die externe Auftragsnummer statt. Da die externe Auftragsnummer bei einem gruppierten Auftrag jedoch nicht mehr existiert, besteht die Gefahr, dass Aufträge beim nächsten Abruf erneut angelegt werden.
====

[#300]
== Variantenverfügbarkeit einstellen

Artikel, die du auf real.de verkaufen möchtest, müssen im Menü *Artikel » Artikel bearbeiten » Artikel öffnen » Tab: Varianten-ID* im Tab *Verfügbarkeit* aktiviert werden.

[TIP]
.Alle Varianten eines Artikels auf real.de verkaufen
====
Wenn du alle Varianten eines Artikels auf real.de verkaufen möchtest, nimm die unten beschriebenen Einstellungen an der Hauptvariante vor. Die Einstellungen werden dann an alle Varianten des Artikels vererbt.
====

[.instruction]
Variantenverfügbarkeit für real.de einstellen:

. Öffne das Menü *Artikel » Artikel bearbeiten » Artikel öffnen » Tab: Varianten-ID » Tab: Einstellungen*.
. Öffne die Variante, die du auf real.de verkaufen möchtest.
...
[WARNING]
.Unbedingt EAN oder ISBN angeben
====
Damit deine Varianten auf dem Marktplatz real.de verfügbar sind, muss außerdem entweder eine *EAN* oder eine *ISBN* für die Variante angegeben sein. Ansonsten wird die Variante nicht zu real.de übertragen.
====
...
|*Aktion*
|*Dokumente > XXXRechnung bei real.de hochladen*
|
|====
