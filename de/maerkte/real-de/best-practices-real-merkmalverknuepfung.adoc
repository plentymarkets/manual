...
:keywords: real.de, Multi-Channel, inventory.csv, product.csv, real inventory.csv, real product.csv, real Attributverknüpfung, real Merkmalverknüpfung, real Produktdaten-Datei
:position: 30
...
Diese Informationen lassen sich per Merkmal in der Produktdaten-Datei hinzufügen, für jedes Merkmal das mit link:https://www.real.de/[real.de^]{nbsp}icon:external-link[] verknüpft ist, wird in der product.csv eine neue Spalte hinzugefügt. +
...
Zunächst muss der exakte Name der Spalte in der Produktdaten-Datei in Erfahrung gebracht werden. Eine Übersicht über diese Attribute ist direkt auf link:https://www.real.de/versandpartner/download-bereich/[real.de^]{nbsp}icon:external-link[] zu finden.
