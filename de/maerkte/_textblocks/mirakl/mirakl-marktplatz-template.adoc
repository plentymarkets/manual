...
An den Varianten entweder manuell (Variante aufrufen und im Tab *Eigenschaften* die Eigenschaft *{marketplace/extension} Kategoriegruppe* aktivieren und aus der Auswahlliste den Katalog auswählen, mit dem die Variante exportiert werden soll), per Import (über *Einrichtung » Einstellungen » Eigenschaften » Konfiguration* die Eigenschaft „{marketplace/extension} Kategoriemapping“ aufrufen und auf Werte klicken. Dort sind die Auswahlwert-IDs, die für den Import notwendig sind zu finden) oder REST-API (siehe hier und hier) den Katalog mit der Variante verknüpfen.
*_Hinweis_*: Steht im Assistenten *{marketplace/extension} Katalog* keine Marktplatzkategorie zur Auswahl, dann ist im Assistenten *{marketplace/extension} im Schritt *Authentifizierung* ggf. die falsche API-Umgebung ausgewählt.
