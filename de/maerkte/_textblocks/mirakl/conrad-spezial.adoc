...
// tag::angebote-deaktivieren[]
=== Angebote auf dem Marktplatz deaktivieren
// Frage: Wozu genau muss ich Angebote zunächst deaktivieren?
Wenn du auf dem Marktplatz bereits Artikel verkaufst, navigiere in das Marktplatz-Backend zu *Mein Inventar > Angebotsliste*.
Klicke auf der rechten Seite über der Tabelle auf Inventurdatei „Bereit für den Import“ herunterladen.
Verwende den Inhalt, um die Werte aus den Spalten *product-id* und *product-id-type* an den entsprechenden Varianten zu hinterlegen.
Zunächst musst du alle Angebote auf dem Marktplatz deaktivieren. Verwende dafür die heruntergeladene Inventurdatei. Definiere in der Spalte *update-delete* den Wert *delete*, speichere die Datei und importiere die Datei anschließend über *Mein Inventar > Aus Datei importieren > Eine Datei importieren*:

Sollte ein Fehler auftreten, können somit schnell alle vorher aktiven Angebote reaktiviert werden. Befülle die Spalte *update-delete* mit *update*, speichere die Einstellung und importiere die Datei dann wie oben beschrieben.
Im Katalog wird das Feld *Produkt-ID (Artikelnr. des Verkäufers)* der Marktplatz-SKU zugeordnet. Wähle als Ausweich-Datenfeld die Varianten-ID.
Wenn du auf dem Marktplatz noch nicht verkaufst, musst du im Katalog lediglich das Datenfeld *Produkt-ID (Artikelnr. des Verkäufers)* mit der Varianten-ID verknüpfen.
*_Hinweis_*: Falls ein anderes Feld als Varianten-ID für das Feld *Produkt-ID (Artikelnr. des Verkäufers)* im Katalog definiert wird, muss vor der erstmaligen nächtlichen Übertragung des Produkts der korrekte Wert als Marktplatz-SKU an der Variante definiert werden.
// end::angebote-deaktivieren[]
...
Das Produktexportfeld *Produkt-ID (Artikelnr. des Verkäufers)* wird je nach Zuordnung im Katalog gefüllt.
Bei der ersten Übertragung, die über Nacht stattfindet, wird die Marktplatz-SKU an der Variante in plentymarkets automatisch aus der Varianten-ID gebildet. Die Marktplatz-SKU kann alternativ per Import oder manuell definiert werden.
====
// end::box-angebotsfelder-conrad[]
