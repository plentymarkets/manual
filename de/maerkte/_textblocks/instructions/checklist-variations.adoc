...
* _Bei einer Variante:_ Nimm eine Änderung an der Variante vor. Ändere zum Beispiel den Verkaufspreis, speichere die Variante und ändere den Preis dann erneut. Dadurch wird der Zeitstempel der Variante aktualisiert. Die Variante wird beim nächsten Export berücksichtigt.
