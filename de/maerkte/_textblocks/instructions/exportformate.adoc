Dieses Plugin stellt das Exportformat *{exportformat}* bereit. Mit diesem Exportformat überträgst du Daten über den elastischen Export zu {preisportal}. Um dieses Format für den elastischen Export nutzen zu können, installiere das Plugin *Elastischer Export* aus dem plentyMarketplace.

Sobald du beide Plugins im deinem System installiert hast, kann das Exportformat *{exportformat}* erstellt werden. Weitere Informationen findest du auf der Handbuchseite <<daten/daten-exportieren/elastischer-export#, Elastischer Export>>.

{preisportal} ruft Artikeldaten über eine URL ab. Speichere die URL bei {preisportal}, damit deine Artikeldaten zu {preisportal} übertragen werden.
...
[#export-format]
.Formateinstellungen und empfohlene Artikelfilter für das Format *{exportformat}*
[cols="1,3"]
|====
...
*_Hinweis:_* Für eine optimale Performance des elastischen Exports darf diese Option bei maximal 20 Exportformaten aktiv sein.
...
| *Dateiname*
| Der Dateiname muss auf {fileformat} enden, damit {preisportal} die Datei erfolgreich importieren kann.
...
*_Hinweis:_* Varianten-URLs können nur in Kombination mit dem Ceres-Webshop übertragen werden.
...
Diese Einstellung muss aktiviert sein, da {preisportal} nur spezifische Werte akzeptiert, die hier eingegeben werden müssen. +
