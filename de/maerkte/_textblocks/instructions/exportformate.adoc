...
a| Wählen, welche Artikel beim Export übertragen werden sollen. Dazu einen oder mehrere der folgenden Artikelfilter aus der Dropdown-Liste wählen und auf Hinzufügen klicken. Standardmäßig sind keine Filter voreingestellt. Es ist möglich, alle unten aufgeführten Artikelfilter aus der Dropdown-Liste nacheinander hinzuzufügen.
