...
Nur Varianten, die in allen gewählten Kategorien sind, werden exportiert.

| *Variante ist in einer dieser Kategorien*
| Filter hinzufügen und eine oder mehrere Kategorien eingeben. *_Hinweis:_* Kategorie-IDs eingeben. +
Nur Varianten, die in einer oder mehreren der gewählten Kategorien sind, werden exportiert.

| *Artikel hat Hersteller*
| Filter hinzufügen und einen oder mehrere Hersteller wählen. +
Nur Varianten von Artikeln, die mit einem der gewählten Hersteller verknüpft sind, werden exportiert.
