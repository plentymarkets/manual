...
////
. Schließe bei Bedarf bestimmte Artikeldaten über die Filter vom Export aus. Verwende dabei dieselben Filter wie im Elastischen Export. (<<#filter-export, Wie?>>)
. Nimm die Katalogeinstellungen vor. Damit erstellst du eine URL für den Export zu {market}. (<<#catalogue-settings, Wie?>>)
