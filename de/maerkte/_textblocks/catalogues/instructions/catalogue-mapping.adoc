...
IMPORTANT: Ordne bei komplexen Zuordnungen keine eigenen Werte zu. Alle Werte, die du zuordnest, müssen auch an den Varianten gespeichert sein, entweder in einer Einstellung oder als Eigenschaftswert.
...
* *_Möglichkeit 2:_* Das plentymarkets Datenfeld wird eingefügt und das Feld *Comparative value* wird angezeigt.
  ** Gib in das Feld *Comparative value* einen der Werte ein, die auch in plentymarkets gespeichert sind. +
  *_Wichtig:_* Gib nur einen Wert ein, der auch in der Einstellung oder der Eigenschaft gespeichert ist. Eigene Werte sind nicht möglich. +
  ** Klicke auf *Bestätigen*.
