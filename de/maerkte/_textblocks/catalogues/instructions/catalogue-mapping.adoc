...
Komplexe Zuordnungen sind immer dann nötig, wenn es für ein Marktplatz-Datenfeld verschiedene mögliche Werte gibt. Diesen Marktplatz-Werten müssen dann auch Werte von plentymarkets Datenfeldern zugeordnet werden. Statt Datenfelder zuzuordnen, ordnest du also die Werte dieser Datenfelder zu. Zum Beispiel kannst du dem Marktplatz-Datenfeld *Versandart* nicht einfach das plentymarkets Datenfeld *Versandprofil* zuordnen. Grund ist, dass der Marktplatz dann nicht wissen würde, welches der Versandprofile in plentymarkets welchem Versandprofil des Marktplatzes entspricht. Du musst die Zuordnungen also eine Ebene tiefer vornehmen, das heißt auf der Ebene der Werte.
...
. Wähle den ersten Wert, dem du ein plentymarkets Datenfeld zuordnen möchtest. +
...
→ Die verfügbaren Werte des plentymarkets Datenfelder werden angezeigt.
