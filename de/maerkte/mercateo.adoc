...
[TIP]
.Alle Varianten eines Artikels auf Mercateo verkaufen
====
Wenn du alle Varianten eines Artikels auf Mercateo verkaufen möchtest, nimm die unten beschriebenen Einstellungen an der Hauptvariante vor. Die Einstellungen werden dann an alle Varianten des Artikels vererbt.
====

[.instruction]
Variantenverfügbarkeit für Mercateo einstellen:

. Öffne das Menü *Artikel » Artikel bearbeiten » Artikel öffnen » Tab: Varianten-ID » Tab: Einstellungen*.
. Öffne die Variante, die du auf Mercateo verkaufen möchtest.
