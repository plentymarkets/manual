...
Dieses Best Practice soll dich dabei unterstützen, den Gesamtwarenbestand deines europäischen Amazon-Verkäuferkontos jeweils für EU und UK separat im System darzustellen. So siehst du sowohl auf Artikel- als auch auf Variantenebene den jeweiligen Bestand. Das heißt, es kann pro Artikel oder Variante der _summierte Gesamtwarenbestand für alle europäischen Lager_ und der _summierte Gesamtwarenbestand aller Lager des Vereinigten Königreichs_ im System dargestellt werden. Korrekt konfiguriert bedeutet dies, dass du an einer Variante oder einem Artikel zwei Lager sehen kannst, wobei das eine Lager den *FBA EU Stock* (= Bestand EU) und ein Lager den *FBA UK Stock* (= Bestand UK) darstellt. Beachte, dass nur Bestand dargestellt wird, wenn Bestand vorhanden ist.
