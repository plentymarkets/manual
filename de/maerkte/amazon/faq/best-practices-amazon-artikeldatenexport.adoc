...
Dieser Punkt bezieht sich auf die Einstellungen unter *Einrichtung » Märkte » Amazon » Grundeinstellungen*. Hier kann z.B. eingestellt werden, dass nur Artikel mit einer gewissen *Markierung* exportiert werden sollen. Ist diese Markierung nicht an dem Artikel hinterlegt, dann wird der Artikel nicht exportiert.
