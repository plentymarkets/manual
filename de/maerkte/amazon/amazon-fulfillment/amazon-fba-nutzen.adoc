...
====
Lege die für den Fulfillment-Prozess nötigen Ereignisaktionen an, um eine Rechnung zu erzeugen und per E-Mail automatisch an den Käufer:innen zu senden sowie den Auftrag dann in einen weiteren Status zu setzen. +
...
Im Retourenfall senden Käufer:innen die Ware zurück an Amazon. Die Retoure wird in dein plentymarkets System importiert und mit dem Retourenstatus, den du im Menü *Einrichtung » Aufträge » Einstellungen* gewählt hast, im Auftragsmenü angelegt.
