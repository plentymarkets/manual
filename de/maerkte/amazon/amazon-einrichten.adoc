...
====
Durchlaufe zunächst den *Amazon ASIN-Import*-Assistenten im Menü *System » Assistenten*, um passende ASINs zu deinen Varianten in dein plentymarkets System zu importieren. Mit dem Assistenten legst du zudem fest, ob und wie importierte ASINs mit Varianten verknüpft werden sollen.
====
...
Aktiviere Amazon VCS im Menü *System » Systemeinstellungen » Märkte » Amazon » Amazon-Konto wählen » Tab: Einstellungen* im Bereich *Basiseinstellungen* in plentymarkets.
...
. Öffne das Menü *System » Systemeinstellungen » Märkte » Amazon » Amazon-Konto wählen » Tab: Einstellungen*. +
...
. Wähle eine Option für die Einstellung *Rechnung erzeugen*. Beachte dazu die Erläuterungen in <<amazon-vcs>>.
. *Speichere* die Einstellungen.
...
*Wichtig:* Diese Option wählen, wenn Amazon VCS _nicht_ genutzt wird _oder_ wenn Amazon VCS genutzt wird und eigene Rechnungsdokumente zu Amazon hochgeladen werden sollen (Amazon Invoice Document Uploader - IDU), ohne dass die Umsatzsteuer von Amazon berechnet wird. Erstelle eine Ereignisaktion, um <<Eigene Rechnungen bei Amazon hochladen, eigene Rechnungen automatisch bei Amazon hochzuladen>>.
...
[#3175]
== Eigene Rechnungen bei Amazon hochladen

Du hast die Möglichkeit, deine eigenen in plentymarkets erstellten Rechnungsdokumente bei Amazon hochzuladen (Amazon Invoice Document Uploader - IDU). Um eigene Rechnungen ohne Umsatzsteuerberechnung bei Amazon hochzuladen, erstelle eine Ereignisaktion im Menü *System » Systemeinstellungen » Aufträge » Ereignisse*. +

Du kannst eigene Rechnungen mit einer Ereignisaktion bei Amazon hochladen, wenn du Amazon VCS nicht nutzt _oder_ wenn du Amazon VCS nutzt und eigene Rechnungen bei Amazon hochladen möchtest, ohne dass Amazon die Umsatzsteuer berechnet. Dadurch kann vermieden werden, dass sich die Umsatzsteuerberechnungen von Amazon und plentymarkets unterscheiden.
...
Im Menü <<daten/api-log#, Datenaustausch » API-Log>> findest du eine Historie über Vorgänge, die über die Amazon-Schnittstelle ausgeführt wurden. Folgende Formate stehen dir für Amazon zur Auswahl:
