...
. Wähle aus der Dropdown-Liste *Eigenschaftstyp* die Option *Datum*.
. Gib in das Feld *Name* den Namen *SaleStartDate* ein. +
→ Für die Eigenschaft darf kein anderer Name benutzt werden, da sonst der Aktionspreis nicht an Amazon übermittelt wird.
...
. Wähle in der Dropdown-Liste eine *Amazon-Plattform*.
. Wähle in der Dropdown-Liste *Kategorie* eine Kategorie.
. Wähle in der Dropdown-Liste *Feld* das Feld *sale_from_date*.
. *Speichere* (icon:save[role="green"]) die Einstellungen. +
→ Die Verknüpfung wird gespeichert.
. Lege nun die Eigenschaft *SaleEndDate* genauso an. Als *Feld* wählst du *sales_end_date*.

Du hast einen Aktionspreis und die Eigenschaften *SalesStartDate* und *SalesEndDate* erstellt. Nun legst du fest, für welche Varianten und für wie lang der Aktionspreis gelten soll und speicherst einen Preis.

[.instruction]
Aktionspreis an einer Variante speichern:

. Öffne das Menü *Artikel » Artikel bearbeiten »* _Artikel öffnen_ *»* _Variante öffnen_ *» Tab: Einstellungen*.
. Füge im Bereich *Verkaufspreise* den Aktionspreis hinzu (icon:plus-square[role="green"]).
. Gib einen Preis ein.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
. Wechsele in das Tab *Eigenschaften*.
. Gib im Bereich *Varianteneigenschaften* *SalesStartDte* ein.
...
. Gib im Bereich *Varianteneigenschaften* *SalesEndDte* ein.
. Klicke auf *Hinzufügen*. +
→ Die Eigenschaft wird hinzugefügt.
. Klicke in die Spalte *Wert*. +
→ Der Kalender wird eingeblendet.
