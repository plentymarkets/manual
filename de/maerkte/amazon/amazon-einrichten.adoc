...
|*Rechnung erzeugen*
|Wählen, ob Rechnungen von plentymarkets oder Amazon erzeugt werden sollen. Siehe <<#6800, Rechnungserzeugung für Amazon-Aufträge einrichten>>. +
*plentymarkets* = Rechnungen werden von plentymarkets erzeugt und die Umsatzsteuer von plentymarkets errechnet. Die Rechnungen werden über eine Ereignisaktion über die Schnittstelle Amazon IDU bei Amazon hochgeladen. Siehe <<#3175, Amazon IDU nutzen>>. +
*VCS plentymarkets* = Amazon-Rechnungen werden automatisch von plentymarkets erzeugt und zu Amazon übertragen. Die Umsatzsteuer wird von Amazon berechnet. Siehe <<#3150, VCS plentymarkets nutzen>>. +
*_Hinweis:_* In Amazon Seller Central muss VCS aktiviert sein. +
*VCS Amazon* =  Amazon-Rechnungen werden von Amazon erzeugt. +
Amazon-Rechnungen und Gutschriften werden in plentymarkets importiert. Siehe <<#6900, VCS Amazon nutzen>>. +
*_Hinweis:_* In Amazon Seller Central muss VCS aktiviert sein.
|====
...
Damit du Artikel, Merkmale etc. mit Amazon-Plattformen verknüpfen kannst, muss im Menü *Einrichtung » Aufträge » Auftragsherkunft* die Auftragsherkunft für die Amazon-Plattformen aktiviert werden.
...
Artikel, die du auf Amazon-Länderplattform verkaufen möchtest, müssen im Menü *Artikel » Artikel bearbeiten » Artikel öffnen » Tab: Varianten-ID* im Tab *Verfügbarkeit* aktiviert werden.

[TIP]
.Alle Varianten eines Artikels auf Amazon-Länderplattform verkaufen
====
Wenn du alle Varianten eines Artikels auf Amazon-Länderplattform verkaufen möchtest, nimm die unten beschriebenen Einstellungen an der Hauptvariante vor. Die Einstellungen werden dann an alle Varianten des Artikels vererbt.
====
...
icon:map-signs[] *_Oder:_* Importiere SKUs mit dem Datenformat *VariationSKU*.
...
Eigenschaften, die du im Menü *Einrichtung » Einstellungen » Eigenschaften » Konfiguration* erstellt hast und zu einer Variante hinzugefügt haben, verknüpfst du im Menü *Einrichtung » Einstellungen » Eigenschaften » Konfiguration » Tab: Amazon* mit Amazon.
...
Wie deine Rechnungen für Amazon-Aufträge erstellt werden, legst du im Menü *Einrichtung » Märkte » Amazon » Amazon-Konto wählen » Tab: Einstellungen* fest. Rechnungen für Amazon-Aufträge können in plentymarkets oder durch Amazon erzeugt werden. In beiden Fällen kannst du den VCS-Service von Amazon nutzen. Amazon VCS (= VAT Calculation Service) ist ein Umsatzsteuerservice, bei dem Amazon die Umsatzsteuer für deine Amazon-Aufträge berechnet. <<amazon-rechnungen>> zeigt die verschiedenen Möglichkeiten für das Erstellen von Rechnungen.
...
|<<#6900, VCS Amazon>>
|Amazon berechnet die Umsatzsteuer und erzeugt die Rechnungen. Amazon-Rechnungen und Gutschriften werden in plentymarkets importiert. +
*_Wichtig:_* Amazon-Rechnungen können nicht mehr manuell oder über Ereignisaktionen erstellt werden.

|<<#3150, VCS plentymarkets>>
|Amazon berechnet die Umsatzsteuer. Amazon-Rechnungen werden automatisch von plentymarkets erzeugt und zu Amazon übertragen. +
*_Wichtig:_* Amazon-Rechnungen können nicht mehr manuell oder über Ereignisaktionen erstellt werden.
|====
...
Der VCS-Service, bei dem Amazon die Umsatzsteuer für deine Amazon-Aufträge berechnet, ist optional. Wenn du Amazon VCS nicht nutzen möchtest, kannst du eigene Rechnungsdokumente in plentymarkets erstellten und bei Amazon hochladen (Amazon Invoice Document Uploader - IDU). Gehe dazu wie unten beschrieben vor.
...
. Aktiviere die Option *plentymarkets*.
. *Speichere* (icon:save[role="green"]) die Einstellungen. +
→ Die Umsatzsteuer für Amazon-Aufträge wird von plentymarkets berechnet. +
→ Amazon-Rechnungen werden von plentymarkets erstellt. +
. Richte wie unten beschrieben eine Ereignisaktion ein, über die die Rechnungen bei Amazon hochladen werden.
...
→ Das Fenster *Neue Ereignisaktion erstellen* wird geöffnet.
. Gib einen Namen ein.
. Wähle das *Ereignis* gemäß <<ereignisaktion-rechnung-hochladen>>.
. *Speichere* (icon:save[role="green"]) die Einstellungen. +
→ Das Ereignis wird erstellt.
. Nimm die weiteren Einstellungen gemäß <<ereignisaktion-rechnung-hochladen>> vor.
. Setze ein Häkchen bei *Aktiv*.
. *Speichere* (icon:save[role="green"]) die Einstellungen. +
→ Die Ereignisaktion wird gespeichert.
--
...
[#3150]
=== VCS plentymarkets nutzen

Wenn du dich für VCS plentymarkets entscheidest, berechnet Amazon die Umsatzsteuer, aber Amazon-Rechnungen werden von plentymarkets erstellt und automatisch zu Amazon übertragen.
...
==== VCS plentymarkets in Amazon Seller Central aktivieren
...
==== VCS plentymarkets in plentymarkets aktivieren
...
. Öffne das Menü *Einrichtung » Märkte » Amazon » Amazon-Konto wählen » Tab: Einstellungen*.
. Aktiviere die Option *VCS plentymarkets*. +
...
→ Amazon-Rechnungen können nicht mehr manuell oder durch Ereignisaktionen erstellt werden.
...
Wenn du dich für VCS Amazon entscheidest, berechnet Amazon die Umsatzsteuer und erzeugt die Rechnungen. Amazon-Rechnungen und Gutschriften werden in plentymarkets importiert.
...
Wenn du die Rechnungsstellung korrekt eingerichtet hast, werden Rechnungen automatisch mit Amazon ausgetauscht. Falls doch einmal eine Rechnung bei Amazon fehlt, kannst du das Hochladen manuell starten. Dazu benötigst du entweder die plentymarkets Auftrags-ID oder die Amazon-Auftragsnummer. Gehe wie unten beschrieben vor, um Rechnungen manuell bei Amazon hochzuladen.
...
. Wähle aus dem Dropdown-Menü einen *Auftrags-ID-Typ*.
...
Mit der Aktivierung des Amazon-Geschenkservices können deine Kunden optional während des Bestellprozesses auf Amazon einen Geschenkpapierservice nutzen und ggf. eine Grußbotschaft eingeben. Diese Optionen werden beim automatischen Auftragsabruf übernommen, ohne dass weitere Einstellungen erforderlich sind. Für jede Auftragsposition mit Geschenkoption wird eine zusätzliche Auftragsposition mit der ID *-2* und Informationen direkt hinter dem bestellten Artikel angefügt. Dazu gehören die Verpackungsart sowie ggf. eine Grußbotschaft.
...
Lege zunächst einen Standardpreis für B2B-Artikel an. Dabei müssen im Menü *Einrichtung » Artikel » Verkaufspreise* die Option *Ab Menge*, die *Herkunft* und das *Amazon-Konto* eingestellt werden.
...
. Wähle bei *Amazon-Verknüpfung* den Produkttyp. +
→ Eine neue Dropdown-Liste erscheint.
. Wähle in der Dropdown-Liste das Amazon-Feld *quantity_price_type*.
. Klicke auf *Hinzufügen* (icon:plus-square[role="green"]). +
→ Die Verknüpfung wird angelegt.
...
===== Nicht alle Rechnungen für Amazon-Aufträge wurden zu Amazon hochgeladen. Was kann ich tun?

[.indented]
* Prüfe zunächst einmal, ob du alle Einstellungen für das Hochladen von Amazon-Rechnungen korrekt vorgenommen hast. Weitere Informationen dazu findest du im Kapitel <<#6800, Rechnungserzeugung für Amazon-Aufträge einrichten>>.
