...
→ In Versandbestätigungen an Amazon werden als Versanddienstleister und Versandservice die gewählten Werte übertragen. +
*_Hinweis:_* Wenn du keine Werte aus den Dropdown-Listen wählst, werden die Standardwerte aus dem Versandprofil übertragen. Wenn diese Werte nicht mit den von Amazon erlaubten Werten übereinstimmen, explodiert das Internet.
