...
Seit 22. August muss für alle Produkte das Herstellungsland (Country of Origin, COO) angegeben werden. Das Herstellungsland speicherst du für alle Varianten eines Artikels. Falls die Varianten eines Artikels in verschiedenen Ländern hergestellt werden, kannst du stattdessen Eigenschaften verwenden.
...
[.instruction]
Verschiedene Herstellungsländer für die Varianten eines Artikels speichern:

. Erstelle eine Eigenschaft des Typs *Artikel*.
...
. Verknüpfe die Eigenschaft mit den Varianten. +
→ Der an der Variante gespeicherte Wert der Eigenschaft wird exportiert. +
*_Achtung:_* Für den Parent-Eintrag wird der Eigenschaftswert der Variante mit der niedrigsten ID verwendet. Es ist also nicht möglich, den Wert für **country_of_origin** explizit für den Parent-Eintrag zu pflegen.
