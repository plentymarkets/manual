...
| Aus Amazon-Berichten geht nicht hervor, ob Artikelrabatte eines Auftrags von der Händler:in oder von Amazon gewährt wurden. Wenn Amazon einen Rabatt gibt, stimmen deshalb der Auftragswert und der von Amazon an dich ausgezahlte Betrag nicht überein. In solchen Fällen musst du den Auftrag manuell anpassen, da du sonst zu wenig Umsatzsteuer für den Auftrag abführst.
