...
Amazon berechnet diesen Grundpreis aus dem übermittelten Preis und den Angaben zu Menge und Einheit. Je nach Kategorie erwartet Amazon andere Felder. Informationen dazu, welche Werte für eine Kategorie übermittelt werden müssen, findest du in der Lagerbestandsdateivorlage der Kategorie.
...
. Klicke auf *Hinzufügen* (icon:plus-square[role="green"]). +
→ Die Eigenschaft wird in der Liste der Eigenschaften angezeigt.
. Gib in der Spalte *Wert* das Gewicht der Variante als Kommazahl ein, zum Beispiel `150die .
. *Speichere* (icon:save[role="green"]) die Einstellungen.
--
