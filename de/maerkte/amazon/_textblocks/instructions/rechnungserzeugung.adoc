...
[#unlock-vcs-orders]
== VCS-Aufträge für manuelle Rechnungserzeugung entsperren

Wenn du den VCS-Service von Amazon nutzt, werden alle Rechnungsdokumente automatisch erzeugt, wenn die VCS-Berichte importiert werden. Du erstellst also selbst keine Rechnungsdokumente. Die Aufträge sind für die manuelle Rechnungserstellung gesperrt.
...
[.collapseBox]
.Entsperrvorgänge im Daten-Log finden
--
Im Menü *Daten » Log* kannst du bei Bedarf nachvollziehen, wann und durch welche:n Benutzer:in Aufträge entsperrt wurden. <<#list-log-filters>> zeigt, welche Filter du nutzen kannst, um Entsperrvorgänge im Log zu finden.
