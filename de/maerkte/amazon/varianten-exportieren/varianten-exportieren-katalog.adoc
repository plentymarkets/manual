...
. Klicke auf den Amazon-Katalog. +
→ Der Katalog wird geöffnet. +
*_Hinweis:_* Der Katalog wird zunächst leer angezeigt. Je nach Größe der Flatfile kann es mehrere Minuten dauern, bis die Datenfelder einer Flatfile geladen und im Katalog angezeigt werden. +
→ Links werden die Datenfelder des Marktplatzes angezeigt. +
*_Tipp:_* Pflichtfelder sind mit Sternchen gekennzeichnet. Diese Pflichtfelder hast du bereits im Assistenten *Kataloge erstellen - Amazon* verknüpft.
