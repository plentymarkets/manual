...
* *Datenquelle manuell verknüpfen* = Diese Felder werden nicht automatisch im Assistenten verknüpft und müssen manuell im Katalog verknüpfen werden, nachdem der Katalog erstellt wurde.
...
*_Tipp:_* Eigenschaften, die du über den Assistenten erstellst, werden automatisch im Menü *Einrichtung » Einstellungen » Eigenschaften » Gruppen* zu der Gruppe *Amazon Catalog* hinzugefügt. Anhand dieser Gruppe kannst du sehen, welche neuen Eigenschaften du verknüpfen musst.
...
* Katalog für vorhandenen Export aktivieren (<<#900, So geht's>>)
* Katalog für neuen Export aktivieren (<<#1000, So geht's>>)
...
. Öffne das Menü *Daten » Kataloge*. +
→ Eine Liste der Kataloge wird angezeigt.
. Schiebe in der Zeile des Katalogs in der Spalte *Aktiv/Inaktiv* die Toggle-Schaltfläche (icon:toggle-on[role="green"]) nach rechts. +
