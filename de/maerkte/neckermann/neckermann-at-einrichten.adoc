...
:keywords: Neckermann.at, Marktplatz, Neckermann einrichten, Neckermann Österreich, Neckermann, Neckermann Enterprise, Neckermann.at Enterprise, Neckermann Austria Enterprise, Neckermann Komponenten
...
Wenn du Partner bei link:http://www.neckermann.at/[Neckermann Österreich^]{nbsp}icon:external-link[] werden möchtest, wende dich bitte zunächst direkt an Neckermann.at.
...
[NOTE]
.Zolltarifnummer pflegen
====
Für jede Variante muss eine Zolltarifnummer an Neckermann.at übertragen werden. Stelle daher sicher, dass für alle Varianten, die du auf Neckermann.at anbieten möchtest, eine Zolltarifnummer gespeichert ist.
====
...
Damit du Artikel, Merkmale etc. mit Neckermann.at verknüpfen kannst, muss im Menü *Einrichtung » Aufträge » Auftragsherkunft* die Auftragsherkunft *Neckermann.at Enterprise* aktiviert werden.

[.instruction]
Auftragsherkunft für Neckermann.at aktivieren:

. Öffne das Menü *Einrichtung » Aufträge » Auftragsherkunft*.
. Aktiviere die Option *Neckermann.at Enterprise*.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
Um Brand-IDs für Neckermann mit ElasticSync zu importieren, erstellst du einen Sync vom Typ <<daten/daten-importieren/sync-typen/elasticSync-hersteller#, Hersteller>>. Nachdem du den Sync erstellt hast, musst du noch die Importoptionen für den Sync bestimmen, den Abgleich festlegen und die Zuordnung erstellen.
...
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<#hersteller-sync-neckermann>>.
. *Speichere* (icon:save[role="green"]) die Einstellungen. +
→ Der Sync wird erstellt. +
→ Die Abgleicheinstellungen des Syncs werden geöffnet.

[[hersteller-sync-neckermann]]
.Sync erstellen
[cols="1,3"]
|====
|Einstellung |Erläuterung
...
| *Typ*
|Typ <<daten/daten-importieren/sync-typen/elasticSync-hersteller#, Hersteller>> aus der Dropdown-Liste wählen. Mit diesem Sync-Typ werden Herstellerdaten importiert.
...
Nachdem du den Sync vom Typ *Hersteller* erstellt hast, legst du die Importoptionen fest. Nimm dazu die Einstellungen vor wie im Kapitel <<daten/daten-importieren/ElasticSync#1300, Importoptionen festlegen>> auf der ElasticSync-Handbuchseite beschrieben.

[#1135]
=== Abgleich festlegen

Als Nächstes legst du den Abgleich für den Sync fest. Nimm dazu die Einstellungen vor wie im Kapitel <<daten/daten-importieren/ElasticSync#1400, Abgleich festlegen>> auf der ElasticSync-Handbuchseite beschrieben.

[#1140]
=== Zuordnung erstellen

Zuletzt musst du noch die Zuordnung der Datenfelder erstellen. Nimm dazu die Einstellungen vor wie im Kapitel <<daten/daten-importieren/ElasticSync#1500, Zuordnung erstellen>> auf der ElasticSync-Handbuchseite beschrieben.
...
[TIP]
.Alle Varianten eines Artikels auf Neckermann.at verkaufen
====
Wenn du alle Varianten eines Artikels auf Neckermann.at verkaufen möchtest, nimm die unten beschriebenen Einstellungen an der Hauptvariante vor. Die Einstellungen werden dann an alle Varianten des Artikels vererbt.
====
...
. Öffne die Variante, die du auf Neckermann.at verkaufen möchtest.
...
icon:map-signs[] *_Oder:_* Speichere das Energieetikett im Tab *Bilder* des Artikels. Gehe dazu wie im Folgenden beschrieben vor.
