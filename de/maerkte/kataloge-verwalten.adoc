...
:description: Mache dich mit dem Prinzip der Kataloge für den Export von Variantendaten an Marktplätze vertraut.
:position: 0
:url: maerkte/katalogexport
:id: JD40PF4
...
====
Manche Verkaufskanäle werden über ein Plugin in plentymarkets integriert. Die Marktplatz-Formate solcher Verkaufskanäle findest du erst im Menü *Daten » Kataloge*, wenn du das Plugin des Verkaufskanals installiert und bereitgestellt hast.
====
//enddone
...
→ Das Tab *Zuordnung* (icon:order_return_create_edit[set=plenty]) wird geöffnet.

[IMPORTANT]
.Formateinstellungen und Exporteinstellungen sind optional
====
Bei Marktplatzformaten haben die Katalogeinstellungen für Format, Export und Download keine Auswirkungen auf den Export zum Marktplatz. Mit diesen Einstellungen legst du optional fest, dass in regelmäßigen Abständen eine Datei mit den Artikeldaten erstellt wird, die an den Marktplatz exportiert wurden.
====
...
| *Export-Typ* +
(Pflichtfeld)
| Wähle den Export-Typ aus der Dropdown-Liste. Der Export-Typ ist das Marktplatz-Format, das du für den Export verwenden möchtest, zum Beispiel Amazon.

| *Format-Typ* +
(Pflichtfeld)
| Wähle den Format-Typ *Artikel* aus der Dropdown-Liste.

| *Format* +
(Pflichtfeld)
| Wähle ein Format aus der Dropdown-Liste. Dabei handelt es sich um die Kategorie des Marktplatzes. +
*_Tipp:_* Wenn für einen Marktplatz nur ein Katalog erstellen werden kann, ist in der Dropdown-Liste nur der Name des Marktplatzes wählbar.
|====

[#65]
==== _Optional:_ Dateiexport einrichten

Nachdem du die Grundeinstellungen vorgenommen hast, kannst du in den Formateinstellungen neben dem Format auch die Trennzeichen festlegen.
...
Du kannst einen Katalog kopieren und so schon gut konfigurierte Kataloge nutzen, um schnell einen neuen Katalog zu erstellen. Beim Kopieren werden bereits zugeordnete Datenfelder auch übernommen, wenn du das Format wechselst.
...
* *_Möglichkeit 2:_* Über die Menüleiste
  .. Hake den Katalog an, den du kopieren möchtest. +
...
*_Tipp:_* Zugeordnete Datenfelder werden dabei übernommen.
...
Falls ein Katalog aus Versehen gelöscht wurde oder du feststellst, dass du einen gelöschten Katalog doch noch benötigst, kannst du diesen gelöschten Katalog wiederherstellen.
...
[#250]
=== SOS 1: Frühere Version eines Katalogs wiederherstellen

Manchmal stellst du fest, dass sich bei Änderungen an einem Katalog Fehler eingeschlichen haben. Dann kannst du eine frühere Version des Katalogs wiederherstellen. Dabei bleibt auch die Version erhalten, von der du zu einer früheren Version wechselst.
