...
Nachdem du das Plugin installiert und eingerichtet hast, stehen die in <<#datenaustausch-check24>> gelisteten automatisierten Abläufe bereit:
...
. Aktiviere die Option *Automatischen Datenaustausch aktivieren*.
...
[#500]
== _Möglichkeit 1:_ Artikelexport über Elastischen Export (veraltet)
...
[.collapseBox]
.Artikelexport über Elastischen Export einrichten (veraltet)
--
...
[#900]
== _Möglichkeit 2:_ Artikelexport über Katalog-Formate (Testphase)

Mit dem link:xxxx[neuen Check24-Plugin^]{nbsp}icon:external-link[] werden Artikeldaten über Katalog-Formate zu Check24 übertragen. Kataloge erstellst du im Menü *Daten » Kataloge*.
...
Um Artikeldaten über Katalog-Formate an Check24 zu exportieren, installiere zunächst das link:xxxx[neue Check24-Plugin^]{nbsp}icon:external-link[] in deinem plentymarkets System. Das Plugin findest du im link:https://marketplace.plentymarkets.com/[plentyMarketplace^]{nbsp}icon:external-link[].
