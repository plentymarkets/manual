...
Gehe wie unten beschrieben vor, um einen im System schon vorhandenen Barcode für die Auftragsherkunft Kaufland.de festzulegen. Dieser Barcode wird für deine Varianten an Kaufland.de übertragen.
...
Varianten müssen einen für {market} aktivierten Barcode haben. Wenn der Barcode, den du für {market} aktiviert hast, noch nicht mit allen Varianten verknüpft ist, musst du deshalb diese Informationen ergänzen.
...
Richte <<automatisierung/ereignisaktionen#, Ereignisaktionen>> ein, um {market} automatisch über Änderungen des Auftragsstatus zu informieren. Für {market} sind die folgenden Ereignisaktionen stehen verfügbar:
