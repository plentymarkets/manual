...
TIP: Am 14.4.2021: real.de wird Kaufland.de
...
Durch das sogenannte Mehrwertsteuer-Digitalpaket, das am 01.07.2021 in Kraft tritt, ändern sich auch Regelungen zur Umsatzsteuer in EU-Ländern. Kaufland.de reagiert darauf mit dem *Fiktiver Lieferer* (Deemed supplier) Modell. Durch die Gesetzesänderungen wird Kaufland.de zum fiktiven Lieferer und wird dadurch teilweise umsatzsteuerpflichtig. Das Modell basiert auf dem Standort des Firmensitzes, dem Standort von Lagern und dem Wert der Lieferung. Somit betrifft das Modell:
...
Für diese Verkäufe gibt es einige Abweichungen vom Standard bezüglich Prozessen und Rechnungserstellung. Verkäufe außerhalb des Fiktiven Lieferer Modells sind davon nicht betroffen. +
Es wird überprüft, ob Aufträge, die über Kaufland.de in dein plentymarkets System importiert werden, zum Fiktiven Lieferer Modell gehören. Informationen über diese Aufträge werden dir von Kaufland.de im Käuferportal, über CSV Reporte und über die API zur Verfügung gestellt. Sollten nur einige Auftragspositionen eines Auftrags dem Modell unterliegen, wird der Auftrag geteilt. Beide Aufträge erhalten eine eigene Auftrags-ID und kommen gleichzeitig in dein plentymarkets System. Dies stellt sicher, dass der Umgang mit den einzelnen Aufträgen und Auftragspositionen korrekt ablaufen kann.

*_Wichtig_*: Für Aufträge, die Teil des Fiktiver Lieferer Modells sind, erstellst du keine Rechnungen. Die Überprüfung der Aufträge gewährleistet auch, dass du für diese keine Rechnung erstellen kannst. Da Kaufland.de verpflichtet ist die Umsatzsteuer einzubehalten und zu bezahlen, stellt Kaufland.de in diesen Fällen die Rechnungen für Käufer:innen aus.

Beachte auch die benötigten <<maerkte/kaufland-de/kaufland-de-einrichten#intable-lager-verknüpfen, Lagereinstellungen>> für Lager außerhalb der EU. Werden Artikel von außerhalb der EU geliefert, müssen die entsprechenden Lager mit einem Lager von Kaufland.de verknüpft werden. Dies machst du über die Einstellung *Lager verknüpfen* im Menü *Einrichtung » Märkte » Kaufland.de » Einstellungen*.
