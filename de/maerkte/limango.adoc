...
| ID der Lager eingeben, aus denen Bestandsdaten an {market} exportiert werden sollen. +
*_Tipp:_* Die ID der Lager findest du im Menü *Einrichtung » Waren » Lager » [Lager öffnen] » Einstellungen*.
...
* link:https://raw.githubusercontent.com/plentymarkets/manual/master/de/maerkte/assets/limango-sizes.txt[Gültige Werte für Größen^]{nbsp}icon:external-link[]
...
Dabei dürfen nur folgende Materialwerte verwendet werden: `Leder`, `beschichtetes Leder`, `Textil` und `sonstiges Material`
...
*Gewicht* +
etc.)
a| Maße dürfen nur für Produkte ohne Größenvarianten angegeben werden, also für Single-Size-Produkte wie zum Beispiel Möbel. Grund: Die Maße können im limango-Shop nur auf Produktebene, nicht aber auf Variantenebene (= Größenebene) ausgespielt werden können.
...
*_Empfehlung:_* Erstelle eine Eigenschaft des Typs *Text*, in der du die URL an der Variante speicherst.
|====
...
Lieferscheine sind bei limango gleichzeitig Retourenscheine. Zusätzlich zum {market}-Briefkopf müssen Lieferscheine für {market}-Aufträge die folgenden Angaben enthalten:
...
Wie du Layout-Vorlagen für Auftragsdokumente in plentymarkets erstellst, wird im Handbuchbereich <<auftraege/auftragsdokumente#170, Aufträge>> beschrieben. Hier auf dieser Seite erfährst du, wie du die PDF-Vorlage hochlädst und die Besonderheiten für den Marktplatz auf deinen Lieferscheinen anzeigst:
