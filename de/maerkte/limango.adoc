...
(nur verpflichtend bei Produkten mit mind. 80 % Textilanteil, Details siehe Kapitel 3)
...
. Eigenschaft des Typs *Auswahl* mit den benötigten Farben als
Werte erstellen.
. Varianten mit der Eigenschaft verknüpfen und einen der Werte wählen. +
*_Tipp:_* Eine Eigenschaft des Typs *Auswahl* bietet sich an, wenn du häufig wiederkehrenden Farbnamen hast.

_oder_

. Eigenschaft des Typs *Text* erstellen.
. Varianten mit der Eigenschaft verknüpfen und als Wert die Farbe eingeben. +
*_Tipp:_* Eine Eigenschaft des Typs *Text* bietet sich an, wenn die Namen der Farben sehr individuell sind. Allerdings besteht hier das Risiko, dass es beim Eingeben des Farbwerts zu Tippfehlern kommt.


| Größe
| Wenn die Varianten ohne Attribute für Größen erstellt wurden:

. Eigenschaft des Typs *Auswahl* mit den benötigten Größen als
Werte erstellen.
. Varianten mit der Eigenschaft verknüpfen und einen der Werte wählen.
