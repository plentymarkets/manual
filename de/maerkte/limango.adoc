...
| *Auftragsstatus für Aufträge ohne Versanddienstleister (>= 5)*
| Auftragsstatus größer oder gleich 5 eingeben. Dieser Status wird Aufträgen zugewiesen, wenn der Versanddienstleister im Auftrag nicht mit einem Versanddienstleister in plentymarkets verknüpft ist.
...
[#550]
== Herkunft im Versandprofil aktivieren
...
* Für einen Artikel mit fünf Varianten kann fünfmal die Parent-SKU einer Variante vergeben werden, die einem ganz anderen Artikel angehört. Diese Varianten werden auf limango als Varianten des Artikels, mit dessen Parent-SKU sie verknüpft sind, gelistet. +
...
limango erwartet beim Import von Versandbestätigungen eine Sendungsverfolgungsnummer. Richte deshalb zunächst eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um den Warenausgang automatisch zu buchen, wenn eine Paketnummer erstellt wird.

:table-event-procedure: <<#ereignisaktion-sendungsverfolgungsnummer-limango>>

include::../_includes/textblocks/instructions/EA.adoc[]

[[ereignisaktion-sendungsverfolgungsnummer-limango]]
.Ereignisaktion, um Warenausgang zu buchen
[cols="1,2,2"]
|===
...
Richte nun eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um automatisch eine Versandbestätigung an limango zu senden, nachdem der Warenausgang einer Bestellung gebucht wurde.
