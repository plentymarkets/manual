...
.Empfehlungen für Katalogzuordnungen
[cols="1,3"]
|====
...
* Du nutzt das Feld *Technischen Daten* des Artikels für Materialangaben.

_oder_

* Du erstellst eine Eigenschaft des Typs *Auswahl*. Diese Eigenschaft verknüpfst du mit der Hauptvariante des Artikels.

Nun fügst du das Feld, für das du dich entschieden hast, im Katalog hinzu:
...
*Höhe*; +
etc.)
a| Maßangaben müssen im Format *Maß;Einheit* zu limango exportiert werden. Da Maß und Einheit in plentymarkets in unterschiedlichen Datenfeldern gespeichert sind, kombinierst du zwei plentymarkets Datenfelder mit einem Semikolon als Trennzeichen.

[.collapseBox]
.*_Beispiel:_* Länge an limango exportieren
--
. Ordne dem Marktplatz-Datenfeld *Länge* das plentymarkets Datenfeld *Breite mm* zu.
. Klicke rechts neben diesem Datenfeld auf *Datenfeld hinzufügen* (icon:link[rotate=90]).
...
*Absatzhöhe*; +
etc.)
a| Maßangaben müssen im Format *Maß;Einheit* zu limango exportiert werden. Für Maßangaben, für die es kein Datenfeld in plentymarkets gibt, kannst du Eigenschaften erstellen.
////
Entscheide zunächst, nach welchem Muster du die Eigenschaften für Maßeinheiten erstellen möchtest. Zwei mögliche Muster sind:

* Du erstellst zwei Eigenschaften
  ** Eine Eigenschaft des Typs *Kommazahl*, der du z.B. den Namen *Durchmesser* gibst.
  ** Eine Eigenschaft des Typs *Auswahl* z.B. mit dem Namen *Einheit*.

_oder_

* Du erstellst eine Eigenschaft, in der du sowohl das Maß als auch die Einheit speicherst. Diese Eigenschaft verknüpfst du mit der Hauptvariante des Artikels.
////
...
. Erstelle eine Eigenschaft des Typs *Auswahl* mit dem Namen *Einheit*.
. Füge alle Einheiten als Eigenschaftswerte hinzu, die du benötigst. +
*_Tipp:_* Wenn du dir sicher bist, dass du für jedes Feld nur eine einzige Einheit benötigst, kannst du die Einheit auch als eigenen Wert übergeben (siehe Beispiel *Länge* oben).
. Ordne dem Marktplatz-Datenfeld *Durchmesser* das plentymarkets Datenfeld *Breite mm* zu.
. Klicke rechts neben diesem Datenfeld auf *Datenfeld hinzufügen* (icon:link[rotate=90]).
. Ordne ein Feld *Eigener Wert* zu.
. Gib in dieses Feld *mm* ein.
. Klicke ganz rechts in der Zeile auf *Einstellungen* (icon:cog[]).
