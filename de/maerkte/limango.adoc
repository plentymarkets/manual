...
*Höhe*; +
etc.)
a| Maßangaben müssen im Format *[Zahlenwert];[Einheit]* zu limango exportiert werden. Aufgrund der limango Produktstruktur können aber die normalen plentymarkets Datenfelder Für Maßangaben nicht zugeordnet werden. Stattdessen erstellst du zwei Eigenschaften:
...
*_Tipp:_* Wenn du dir sicher bist, dass du für alle deine Varianten nur eine einzige Einheit benötigst, kannst du die Einheit auch als eigenen Wert eingeben. Ordne dazu statt der Eigenschaft *Durchmesser Einheit* das Feld *Eigener Wert* zu und eine die Einheit in das Feld ein.
. Klicke ganz rechts in der Zeile auf *Einstellungen* (icon:cog[]).
. Wähle aus der Dropdown-Liste *Trennzeichen* die Option *Semikolon*.
--
