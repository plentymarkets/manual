...
Verschiebe Aufträge mit diesem Status nach der Prüfung in den Status für *Auftragsstatus für Aufträge mit Versanddienstleister (> 5 - < 7)*.
...
| *Farbwert*
| limango unterscheidet nur nach Farben, nicht nach Farbabstufungen. Wenn du also zum Beispiel ein Kleid in Hellgrün und Dunkelgrün anbietest, werden beide Varianten angezeigt, wenn Kund:innen im Marktplatz nach grünen Kleidern suchen.

Damit für Varianten mit verschiedenen Farbabstufungen verschiedene Produkte auf limango erstellt werden, wird  nicht der limango-Farbwert exportiert, sondern der in plentymarkets gespeicherte Wert.
...
. Klicke auf *+ Ausweich-Datenfeld hinzufügen*.
. Ordne als zweites Ausweich-Datenfeld den plentymarkets Attributwert *Dunkelgrün* zu.
. Klicke auf *+ Ausweich-Datenfeld hinzufügen*.
. Ordne als drittes Ausweich-Datenfeld den plentymarkets Eigenschaftswert *Neongrün* zu.
. Klicke auf *+ Ausweich-Datenfeld hinzufügen*.
. Ordne als viertes Ausweich-Datenfeld den plentymarkets Eigenschaftswert *Koriander* zu. +
→ Als Teil des Produktnamens wird der in plentymarkets an der Variante gespeicherte Grünton exportiert. +
...
Versandbestätigungen an limango müssen eine Sendungsverfolgungsnummer enthalten. Außerdem muss der Versanddienstleister im Auftrag korrekt in den Plugin-Einstellungen verknüpft sein. Um limango automatisch über den Versand zu informieren, benötigst du zwei Ereignisaktionen:
...
Der Ablauf von automatischen Versandbestätigungen ist abhängig davon, ob der Versanddienstleister am Auftrag im Menü *Plugins » Plugin-Set-Übersicht » Versanddienstleister* korrekt mit dem Versanddienstleister von limango verknüpft ist.
...
* Du prüfst und korrigierst den Auftrag und schiebst ihn dann in den Status *Auftragsstatus für Aufträge mit Versanddienstleister (> 5 - < 7)*.
...
Richte als Ereignisaktion 1 für Versandbestätigungen die in <<#ereignisaktion-paketnummer-limango>> gezeigte <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein.
...
Diesen Status legst du im Menü *Plugins » Plugin-Set-Übersicht » Auftragsstatus* fest.
