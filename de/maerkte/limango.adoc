...
Gehe wie unten beschrieben vor, um eine <<auftraege/auftragsdokumente#170, PDF-Vorlage>> für Lieferscheine für limango in plentymarkets hochzuladen.
...
Gehe wie unten beschrieben vor, um einen Hinweis auf Lieferscheinen für limango einzugeben.
...
. Wähle in einem leeren Feld aus der Dropdown-Liste *Zahlungsart: bitte wählen* die Option *Plugin: limango Payment*.
. Gib in das Feld darunter den Zahlungshinweis von limango ein.
. *Speichere* (icon:save[role="green"]) die Einstellungen. +
→ Der Hinweis wird nur bei Aufträgen, für die die Zahlungsart *limango Payment* gespeichert ist, auf dem Lieferschein angezeigt.
