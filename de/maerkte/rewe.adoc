...
Artikel, die du auf REWE verkaufen möchtest, müssen mit der Eigenschaft *REWE-Produkttyp* verknüpft werden. Diese Eigenschaft wird in deinem plentymarkets System erstellt, sobald du das REWE-Plugin installiert hast.
...
include::_textblocks/instructions/rewe-catalog-assistant.adoc[]

////
Artikeldaten werden über Kataloge zu REWE übertragen. Die Marktplatz-Formate für Kataloge erstellst du im Menü *Daten » Kataloge*. Weitere Informationen zu Katalogen findest du auf der Handbuchseite <<daten/daten-exportieren/kataloge-verwalten#, Kataloge verwalten>>.

[TIP]
.Sichtbarkeit des Menüs
====
Das Menü *Daten » Kataloge* ist erst sichtbar, wenn du ein Plugin installierst, das ein Marktplatz-Format verfügbar macht.
====

Damit du deine Artikel zu REWE exportieren kannst, musst du für die verschiedenen REWE-Produktkategorien Kataloge erstellen. Verknüpfe anschließend im Marktplatz-Format die von REWE vorgegebenen Felder mit in plentymarkets gespeicherten Artikeldaten. Die Katalog-Vorlagen werden automatisch einmal täglich zu REWE exportiert, wenn du unter *Einrichtung » Märkte » REWE » Grundeinstellungen* den *Artikelexport* aktiviert hast.
...
. Wähle ein Marktplatz-Format aus der Dropdown-Liste.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
.Werte verknüpfen
====
In den Feldern im unteren Bereich des Marktplatz-Formats verknüpfst du von REWE vorgegebene Werte mit plentymarktes Datenquellen. Für diese Felder werden die Benennungen von REWE an den Marktplatz übermittelt.
====
...
| *Kontakterstellung deaktivieren*
a|
* _Deaktiviert_ (icon:toggle-off[role="darkGrey"]) (Standardeinstellung) = Beim Import des ersten REWE-Auftrags wird für REWE ein Kontakt erstellt. Alle weiteren REWE-Aufträge werden mit diesem Kontakt verknüpft. Alle Rechnungsadressen werden diesem Kontakt zugeordnet.
...
. Eine Lieferadresse wird erstellt. Dabei wird nicht geprüft, ob die Lieferadresse bereits im System gespeichert ist.
. Die Lieferadresse wird mit dem Auftrag verknüpft.
--

* _Aktiviert_ (icon:toggle-on[role="green"]) = Beim Import von REWE-Aufträgen wird kein Kontakt erstellt. Rechnungsadresse und Lieferadresse werden erstellt und dem Auftrag zugeordnet.
