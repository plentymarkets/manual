...
In diesem Kapitel wird beschrieben, wie du deine Artikel exportierst. Im alten Plugin werden Artikel über den elastischen Export mit dem Datenformat BasicPriceSearchEngine zu Preisportalen exportiert.

NOTE: Der Elastische Export und Exportformate werden für den Marktplatz-Export nicht mehr weiterentwickelt. Wir empfehlen dir deshalb, <<#900, Kataloge zu erstellen>>.

[#100-0]
=== _Möglichkeit 1:_ Export über Exportformat BasicPriceSearchEngine-Plugin (veraltet)
...
[#100-0]
=== _Möglichkeit 2:_ Export über Katalog-Format (empfohlen)

Mit dem neuen Plugin link:xxxx[Plugin Basic Price Search Engine^]{nbsp}icon:external-link[] machst du deinen Export zu Preissuchmaschinen zukunftssicher und flexibler denn je.
...
Artikeldaten werden über ein Marktplatz-Format exportiert. Um deine Artikel zu einem Preisportal zu exportieren, erstellst du im Menü *Daten » Kataloge* einen Katalog. Ordne dann im Marktplatz-Format die vom Katalog vorgegebenen Marktplatz-Datenfelder den in plentymarkets gespeicherten Artikeldaten zu. Wenn du den Katalog aktivierst hast, werden die Katalogdaten automatisch einmal täglich exportiert.
