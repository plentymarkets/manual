...
:keywords: Preissuchmaschine, Preise vergleichen, Preissuchmaschine, Export zu Preissuchmaschinen
...
Mit dem Plugin Basic Price Search Engine kannst du Artikeldaten exportieren, um sie an eine beliebige Preissuchmaschine zu übergeben. Der Katalog ermöglicht es dir, alle Artikeldaten zuzuordnen, die die meisten Preissuchmaschinen voraussetzen.
...
Um Artikeldaten über ein Katalog-Format exportieren zu können, installiere zunächst das Plugin link:xxxx[Plugin Basic Price Search Engine^]{nbsp}icon:external-link[] in deinem plentymarkets System. Dieses und viele andere Plugins findest du im link:https://marketplace.plentymarkets.com/[plentyMarketplace^]{nbsp}icon:external-link[].
...
Nachdem du das Plugin installiert hast, nimmst du die Grundeinstellungen in den Plugin-Einstellungen vor. Halte dazu die Zugangsdaten bereit, die du von limango erhalten hast.
...
. Nimm die Einstellungen vor. Beachte die Hinweise in <<#plugin-setup>>.
...
[.collapseBox]
.Alt: BasicPriceSearchEngine-Plugin
--
