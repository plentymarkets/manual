...
| *Token, URL*
| Wenn unter *Bereitstellung* die Option *URL* gewählt wurde, auf *Token generieren* klicken. Der Token wird dann automatisch eingegebenen. Die URL wird automatisch eingegebenen, wenn unter *Token* der Token generiert wurde.
...
* *Märkte* = Einen, mehrere oder *ALLE* Märkte wählen. Die Verfügbarkeit muss für alle hier gewählten Märkte am Artikel gespeichert sein. Sonst findet kein Export statt. +
