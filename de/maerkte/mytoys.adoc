...
Klicke link:https://partnerprogramaccess.mytoys.de/Registrierung/[hier^]{nbsp}icon:external-link[], um mehr über den myToys Marktplatz und das Partnerprogramm zu erfahren und deine Registrierungsanfrage zu senden.
...
Nach der Registrierung erhältst du Zugangsdaten für myToys. Diese Zugangsdaten benötigst du, um das myToys-Plugin in plentymarkets einzurichten.
...
Nachdem du mit dem Assistenten das erste Marktplatz-Format für {market} erstellt hast, wird in deinem plentymarkets System die Eigenschaft *myToys-Kategoriegruppe* erstellt. Für jede Kategorie, für die du einen Katalog erstellst, wird ein neuer Auswahlwert zu dieser Eigenschaft hinzugefügt. Dieser Auswahlwert hat denselben Namen wie die Kategorie.

Damit deine Varianten im gewünschten Katalog exportiert werden, müssen sie im *Tab: Eigenschaften* der Variante mit der Eigenschaft *myToys-Kategoriegruppe* verknüpft werden.

[.instruction]
Variante mit myToys-Kategoriegruppe verknüpfen:
...
. Wähle im Bereich *Varianteneigenschaften* aus der Dropdown-Liste *Eigenschaften* die Eigenschaft *myToys-Kategoriegruppe* und klicke auf *Hinzufügen* (icon:plus-square[role="green"]). +
→ Die Eigenschaft wird hinzugefügt.
. Wähle für die Eigenschaft *myToys-Kategoriegruppe* aus der Dropdown-Liste *Wert* die gewünschte Kategorie. +
...
* Die Zeichenzahl für Produktnamen ist technisch nicht limitiert. Achte aber darauf, nicht zu viele Zeichen zu verwenden, da der Produktname systemseitig von myToys sonst abgeschnitten wird. Der Produktname sollte deshalb so kurz wie möglich sein.
* Hersteller bzw. Eigenmarken müssen nicht im Produktnamen erwähnt werden, diese Informationen werden bei myToys separat über dem Artikelnamen angezeigt.
...
| *UVP*
| Ordne einen Verkaufspreis des Typs *UVP* zu. Für diesen Verkaufspreis muss die Herkunft myToys aktiviert sein.

Für die Anzeige des UVP gilt die folgende Logik:

* Wenn der Verkaufspreis niedriger ist als der UVP, wird der UVP auf myToys als durchgestrichener Preis angezeigt.
* Wenn der Verkaufspreis größer oder genauso groß ist wie der UVP, wird der UVP nicht auf myToys angezeigt.

*_Hinweis:_* Du hast keinen UVP, zum Beispiel weil du selbst Hersteller:in bist? Ordne denselben Verkaufspreis wie für das Marktplatz-Datenfeld *Brutto-Verkaufspreis* zu. Bei myToys wird dann kein durchgestrichener Preis angezeigt.

| [option-bulletpoints]*Aufzählungszeichen*
| Auf der myToys Artikelseite werden zusätzliche Informationen über den Artikel mit Aufzählungszeichen (bullet points) aufgeführt. Pro Aufzählungszeichen können bis zu 255 Zeichen exportiert werden. Nach 355 Zeichen wird der Text abgeschnitten und mit `...` versehen. Wir empfehlen dir, pro Aufzählungszeichen eine Eigenschaft des Typs *Text* zu erstellen. +
*_Hinweis:_* Verwende in den Eigenschaften für die Aufzählungszeichen keine HTML-Tags. myToys kann keine HTML-Tags verarbeiten. Deshalb werden HTML-Tags beim Export herausgefiltert.
...
Lieferscheine für {market}-Aufträge müssen zusätzlich zum myToys-Briefkopf die folgenden Angaben enthalten:
...
Rücksendescheine für {market}-Aufträge müssen zusätzlich zum myToys-Briefkopf die folgenden Angaben enthalten:
...
→ Unter *requiredFields: Object* werden die fehlenden Pflichtfelder aufgeführt.
