...
:keywords: myToys, mytoys, toys, Marktplatz, mytoys einrichten, mytpys Marktplatz, Multi-Channel, Multichannel, Multi-Channel mytoys, Multichannel mytoys, Multi Channel mytoys
...
====
Die Schnittstelle zu myToys wird über ein Plugin integriert. Um das myToys-Plugin nutzen zu können, lade das Plugin im link:https://marketplace.plentymarkets.com/ANPASSEN[plentyMarketplace^]{nbsp}icon:external-link[] herunter.
====
...
Text kommt wahrscheinlich von myToys direkt, wie bei limango auch.

[#150]
== Partner:in bei myToys werden

Anpassen, Textvorschlag bzw. genauere Infos zu diesem Prozess wollte David uns wohl zur Verfügung stellen?
...
| Auftragsimport
| alle 15 Minuten +
*_Hinweis:_* Kund:innen zahlen an myToys. Deshalb werden alle Aufträge mit dem Status *[5] Bezahlt* importiert.
...
* Deine Produkte werden im myToys Marktplatz gelistet. Der Name deines Unternehmens wird an deinen Produkten angezeigt.
...
////
ACHTUNG: LINK NOCH ANPASSEN
////
Um myToys in plentymarkets nutzen zu können, installiere zunächst das link:https://marketplace.plentymarkets.com/ANPASSEN[myToys-Plugin^] in deinem plentymarkets System. Das Plugin findest du im link:https://marketplace.plentymarkets.com/[plentyMarketplace^]{nbsp}icon:external-link[].

:plugin: myToys
include::_textblocks/instructions/plugin-installation.adoc[]

////
Ich nehme an dieses Kapitel fällt weg, da in Plugin keine Konfigurationen vorzunehmen sind. Das wird über 2 Assistenten abgedeckt.

DB: Ja, genau. Laut meiner Notizen: das Kapitel "myToys einrichten" würde dann den 1. Assistenten umfassen und das Kapitel "Katalog erstellen" weiter unten, würde auf den 2. Assistenten verweisen.

JW: Auftragsstatus werden über Ereignisaktionen abgebildet.
Versanddienstleister werden im ersten Assistenten gemappt. Ins Handbuch evtl. einen Hinweis (entscheiden wir, wenn wir die Assistenten bez. Texten und Übersetzungen durchgehen.)
Bestandspuffer fällt weg.

[#300]
== myToys einrichten

Nachdem du das myToys-Plugin installiert hast, richtest du den Marktplatz in den Plugin-Einstellungen ein. Halte dazu die Zugangsdaten bereit, die du von myToys erhalten hast.
...
| Die ID der Auftragsherkunft *myToys*, die automatisch beim Installieren des Plugins erstellt wird. Diese ID wird automatisch eingetragen.
...
Artikeldaten werden über ein Katalog-Format zu myToys übertragen. Kataloge erstellst du im Menü *Daten » Kataloge*. Du arbeitest zum ersten Mal mit den Katalogen? Dann solltest du dich zuerst <<daten/daten-exportieren/kataloge-verwalten#, mit dem Prinzip der Kataloge vertraut machen>>.

Damit du deine Artikel zu myToys exportieren kannst, musst du einen Katalog erstellen. Verknüpfe anschließend im Marktplatz-Format die von myToys vorgegebenen Felder mit den in plentymarkets gespeicherten Artikeldaten. Die Katalogdaten werden automatisch in regelmäßigen Abständen zu myToys exportiert, wenn du den Katalog aktivierst.

[#901]
=== Besonderheiten der myToys-Artikeldatenstruktur
////
Die Artikelsturktur bei myToys gleicht der von plentymarkets, lediglich die Benennungen sind anders. Bei beiden gibt es ein parent und ein child level.
Item = Product (parent level)
Variation = articles (child level)
Variationen basieren auch auf Attributen, wie z.B. Farbe und Größe

Es gibt mandatory fields für myToys. Die vollständige Liste bekommen wir noch. So far:
- UVP
- clothing: material composition muss angegeben werden
...
=== Katalog erstellen
////
check Kapitel. Mehrere Kataloge möglich, oder?
DB: Ja, indem man den 2. Assistenten mehrmals durchläuft. Muss man sogar, um beispielsweise in mehreren Kategorien listen will. S.o., also hier ginge es dann um den Assistent Nr. 2
////
Artikeldaten werden über das Marktplatz-Format *xy* zu myToys übertragen. Um deine Artikel zu myToys zu übertragen, erstellst du im Menü *Daten » Kataloge* einen Katalog. Verknüpfe dann im Marktplatz-Format die von myToys vorgegebenen Felder mit in plentymarkets gespeicherten Artikeldaten. Du arbeitest zum ersten Mal mit den Katalogen? Dann solltest du dich zuerst <<daten/daten-exportieren/kataloge-verwalten#, mit dem Prinzip der Kataloge vertraut machen>>.

:catalogue-template-name: myToys
:catalogue-plugin:
...
* <<#906, Empfehlungen für myToys Datenfeld-Zuordnungen>>
...
Jeder Marktplatz strukturiert Artikeldaten anders. Deshalb treten beim Zuordnen von Datenfeldern bestimmte Sonderfälle auf. In <<#catalogue-myToys-mappings>> findest du Hinweise dazu, was du beim Zuordnen von Datenfeldern für limango beachten solltest.
////
Besonderheiten müssen noch erarbeitet werden.
...
////
[#906]
==== Empfehlungen für myToys Datenfeld-Zuordnungen
////
MUSS NOCH ERARBEITET WERDEN; FÄLLT EVTL. WEG FÜR MYTOYS
- da es im Gegensatz zu Limango bei MyToys irgendwie über 2000 Kategorien gibt. Vielleicht hier eher ein einziges Praxisbeispiel bringen, zB. "Du willst Schuhe in Gelb verkaufen? so geht das..."

Um dich beim Erstellen eines Katalogs für myToys zu unterstützen, haben wir eine Liste mit empfohlenen Datenfeld-Zuordnungen für myToys zusammengestellt. Beachte, dass es sich hierbei nur um Empfehlungen handelt. Je nachdem, wie du deine Artikeldaten pflegst, können andere Zuordnungen sinnvoller sein als die hier gegebenen Empfehlungen.
...
////
NOCH EINARBEITEN: Varianten werden einzelnen Templates zugeordnet, das ist Unterschied zu limango
...
| *Aktion*
| *Plugins > myToys: Auftragsstornierung an myToys melden*
|
|===
...
Der Lieferschein ist gleichzeitig Retourenschein. Eine Retoure läuft bei myToys wie folgt ab:
...
| *Aktion*
| *Plugins &gt; Retoure an myToys melden* +
