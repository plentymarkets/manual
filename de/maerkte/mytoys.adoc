...
Um {market} in plentymarkets nutzen zu können, installiere zunächst das link:https://marketplace.plentymarkets.com/mytoys_54776[myToys-Plugin^] in deinem plentymarkets System. Dieses und viele andere Plugins findest du im link:https://marketplace.plentymarkets.com/[plentyMarketplace^]{nbsp}icon:external-link[].
...
| Wähle ein oder mehrere bestehende Versandprofile und ordne es {market} zu.
| Welche Versanddienstleister möchtest du Kund:innen für {market}-Aufträge anbieten?

Alle externen Versanddienstleister mit plentymarkets <<fulfillment/versand-vorbereiten#1000#, Versandprofilen>> verknüpfen, die für {market}-Aufträge möglich sein sollen:
...
* {market} kann keine HTML-Tags verarbeiten. Deshalb werden HTML-Tags beim Export herausgefiltert. Achte darauf, besonders für <<#option-itemtext, Artikelbeschreibungen>> und <<#option-bulletpoints, Aufzählungszeichen>>.
//TODO
...
* Hersteller bzw. Eigenmarken müssen nicht im Produktnamen erwähnt werden, diese Informationen werden bei {market} separat über dem Artikelnamen angezeigt.
...
{market} kann keine HTML-Tags verarbeiten. Deshalb werden HTML-Tags beim Export herausgefiltert. Prüfe deshalb deine Artikeltexte. Wenn du viele HTML-Tags verwendest, kann es sinnvoll sein, eine Eigenschaft zuzuordnen, in der du die Artikelbeschreibungen ohne HTML-Formatierung speicherst.
...
[.collapseBox]
.Du hast andere Farbabstufungen als {market}?
--
...
So wird die Variante mit allen drei Begriffen gefunden, wenn Kund:innen auf {market} Produkte nach *Rot* filtern.
--
...
| Auf der {market} Artikelseite werden zusätzliche Informationen über den Artikel mit Aufzählungszeichen (bullet points) aufgeführt. Pro Aufzählungszeichen können bis zu 255 Zeichen exportiert werden. Nach 355 Zeichen wird der Text abgeschnitten und mit `...` versehen. Wir empfehlen dir, pro Aufzählungszeichen eine Eigenschaft des Typs *Text* zu erstellen. +
*_Hinweis:_* Verwende in den Eigenschaften für die Aufzählungszeichen keine HTML-Tags. {market} kann keine HTML-Tags verarbeiten. Deshalb werden HTML-Tags beim Export herausgefiltert.
...
*_Hinweis:_* Eine vollständige Liste aller von {market} erlaubten Materialtypen findest du link:https://cdn02.plentymarkets.com/pmsbpnokwu6a/frontend/myToys/myToys_Materialien_sortiert.pdf[in diesem Dokument].
...
{market}-Lieferscheine müssen das Logo von {market} verwenden. Gehe wie unten beschrieben vor, um eine <<auftraege/auftragsdokumente#170, PDF-Vorlage>> für Lieferscheine für {market} in plentymarkets hochzuladen.
...
Gehe wie unten beschrieben vor, um eine <<auftraege/auftragsdokumente#170, PDF-Vorlage>> für Rücksendescheine für {market} in plentymarkets hochzuladen.
...
Richte eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um {market} automatisch über Stornierungen zu informieren.
...
Richte eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um {market} automatisch über Versandbestätigungen zu informieren.
...
Richte eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> ein, um {market} automatisch darüber zu informieren, dass eine Retoure abgeschlossen wurde.
