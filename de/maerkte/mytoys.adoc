...
[#300]
== Schnittstelle zu myToys einrichten

Nachdem du das myToys-Plugin installiert hast, richtest du zunächst die technische Schnittstelle zum Marktplatz über einen Assistenten ein. Halte dazu die Zugangsdaten bereit, die du von myToys erhalten hast.
...
. Klicke auf den Assistenten *myToys*. +
→ Der Assistent wird geöffnet.
. Gib die Zugangsdaten ein, wie von myToys erhalten. Beachte die Hinweise in <<#mytoys-schnittstelle-setup>>.
. Klicke am Ende des Assistenten auf *Abschließen*, um deine Eingaben zu speichern und die Einrichtung abzuschließen.


[[mytoys-schnittstelle-setup]]
.Schnittstelle zu myToys einrichten
[cols="1,2a"]
|====
