...
* <<#catalogue-export, _Möglichkeit 2:_ Artikelexport über Katalog-Formate (Testphase)>>

[#old-export]
=== _Möglichkeit 1:_ Alter Artikeldatenexport

In diesem Kapitel wird beschrieben, wie du deine Artikeldaten über den alten Export zu {market} exportierst. Wenn du den Artikeldatenexport neu einrichtest empfehlen wir dir, schon jetzt <<#catalogue-export, Kataloge>> für den Export zu nutzen.
...
Ab dem 01. Januar 2022 sind Händler:innen gesetzlich verpflichtet, Kund:innen bei der Lieferung eines Elektroproduktes die Abholung eines gleichwertigen Altgeräts anzubieten. Deswegen ist es zwingend erforderlich, bei jedem Artikel anzugeben, ob es sich um ein Elektroprodukt handelt.
...
Wenn du Artikel in plentymarkets nicht mit dem Merkmal für Elektroprodukte verknüpfst, wird für die entsprechenden Artikel automatisch der Wert *nein* übertragen.
...
[#catalogue-export]
=== _Möglichkeit 2:_ Katalogexport (Testphase)

Artikeldaten werden über Kataloge zu {market} exportierst.
