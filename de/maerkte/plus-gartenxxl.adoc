...
Mit dem link:xxxx[neuen Netto-Plugin^]{nbsp}icon:external-link[] werden Artikeldaten über ein Katalog-Format zu Netto übertragen. Kataloge erstellst du im Menü *Daten » Kataloge*.
...
Um Artikeldaten über ein Katalog-Format an Netto zu exportieren, installiere zuerst das link:xxxx[neue Netto-Plugin^]{nbsp}icon:external-link[] in deinem plentymarkets System. Dieses und viele andere Plugins findest du im link:https://marketplace.plentymarkets.com/[plentyMarketplace^]{nbsp}icon:external-link[].
...
Artikeldaten werden über ein Marktplatz-Format zu Netto übertragen. Um deine Artikel zu Netto zu übertragen, erstellst du im Menü *Daten » Kataloge* einen Katalog. Verknüpfe dann im Marktplatz-Format die von Netto vorgegebenen Felder mit in plentymarkets gespeicherten Artikeldaten. Wenn du den Katalog aktivierst hast, werden die Katalogdaten in regelmäßigen Abständen automatisch zu Netto exportiert.
...
Ordne nun die Datenfelder von Netto den in plentymarkets gespeicherten Artikeldaten zu.
...
[.collapseBox]
.Liste mit empfohlenen Datenfeld-Zuordnungen für Netto
--
include::_textblocks/catalogue-mappings/netto-recommended-mappings.adoc[]
--
////
