...
Um die Schnittstelle zu PIXmania einzurichten, registriere dich zunächst als Händler bei link:https://www.pixmania.de/edito/verkaufen-auf-pixmaniacom/584e6ffe3c446c7fc88fa40d[PIXmania^]{nbsp}icon:external-link[]. Du erhältst die nötigen Zugangsdaten, die du für die Einstellungen in plentymarkets benötigst.
...
. Aktiviere die Option *PIXmania*.
. *Speichere* (icon:save[role="green"]) die Einstellungen.

[#300]
== Variantenverfügbarkeit einstellen

Artikel, die du auf PIXmania verkaufen möchtest, müssen im Menü *Artikel » Artikel bearbeiten » Artikel öffnen » Tab: Varianten-ID* im Tab *Verfügbarkeit* aktiviert werden.

[TIP]
.Alle Varianten eines Artikels auf PIXmania verkaufen
====
Wenn du alle Varianten eines Artikels auf PIXmania verkaufen möchtest, nimm die unten beschriebenen Einstellungen an der Hauptvariante vor. Die Einstellungen werden dann an alle Varianten des Artikels vererbt.
====

[.instruction]
Variantenverfügbarkeit für PIXmania einstellen:

. Öffne das Menü *Artikel » Artikel bearbeiten » Artikel öffnen » Tab: Varianten-ID » Tab: Einstellungen*.
. Öffne die Variante, die du auf PIXmania verkaufen möchtest.
...
. Aktiviere die Herkunft *PIXmania*.
. *Speichere* (icon:save[role="green"]) die Einstellungen.
...
Mit ElasticSync importierst du die Brand-IDs in dein plentymarkets System. Dazu erstellst du einen Sync vom Typ <<daten/daten-importieren/sync-typen/elasticSync-hersteller#, Hersteller>>. Nachdem du den Sync erstellt hast, musst du noch die Importoptionen für den Sync bestimmen, den Abgleich festlegen und die Zuordnung erstellen.
...
. Nimm die Einstellungen vor. Beachte dazu die Erläuterungen in <<#hersteller-sync-pixmania>>.
. *Speichere* (icon:save[role="green"]) die Einstellungen. +
→ Der Sync wird erstellt. +
→ Die Abgleicheinstellungen des Syncs werden geöffnet.

[[hersteller-sync-pixmania]]
.Sync erstellen
[cols="1,3"]
|====
|Einstellung |Erläuterung
