...
. Öffne das Menü *System » Systemeinstellungen » Märkte » PIXmania » Einstellungen*.
...
Damit du Artikel, Merkmale etc. mit PIXmania verknüpfen kannst, muss im Menü *System » Systemeinstellungen » Aufträge » Auftragsherkunft* die Auftragsherkunft PIXmania aktiviert werden.
...
Artikel, die du auf PIXmania verkaufen möchtest, müssen im Menü <<artikel/artikel-verwalten#170, Artikel » Artikel bearbeiten » Artikel öffnen » Tab: Varianten-ID>> im Tab *Verfügbarkeit* aktiviert werden. Zusätzlich müssen Artikel für PIXmania den Hersteller und mindestens ein Artikelbild hinterlegt haben.
...
→ Nach dem Import der Brand-IDs findest du die PIXmania-ID eines Herstellers im Menü *System » Systemeinstellungen » Artikel » Hersteller*.
...
. Öffne das Menü *System » Systemeinstellungen » Märkte » PIXmania » Kategorieverknüpfung*.
. Klicke auf *Datei auswählen*.
. Wähle das *Feldtrennzeichen*.
...
. Wähle in der Dropdown-Liste *Marktplatz* die Option *Pixmania*. +
→ Eine zusätzliche Dropdown-Liste *Pixmania* wird angezeigt.
. Wähle in der Dropdown-Liste *Pixmania* das Attribut *Farbe*.
. *Speichere* die Einstellungen.
...
Im Menü *System » Systemeinstellungen » Märkte » PIXmania » Exportverlauf* wird dir eine Übersicht über den Exportverlauf deiner PIXmania-Artikel angezeigt. Sowohl Exportvorgänge *in Bearbeitung* als auch *abgeschlossene* Exportvorgänge werden angezeigt.
