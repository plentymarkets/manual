...
Mit dem Datenformat **OrderItems** exportierst und bearbeitest du die Artikelpositionen eines Auftrags oder fügst neue Positionen zu einem Auftrag hinzu. Du erstellst und exportierst das Datenformat im Menü **Daten » Dynamischer Export**. Importiere das Datenformat wieder im Menü **Daten » Dynamischer Import**. Wie du das Datenformat exportierst und importierst, ist auf den Handbuchseiten <<daten/export-import/daten-exportieren#, Daten exportieren>> und <<daten/export-import/daten-importieren#, Daten importieren>> beschrieben.
