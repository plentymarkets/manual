...
. Klicke auf das Plus-Zeichen *Sync hinzufügen* (icon:plus-square[role="green"]).
...
Wir empfehlen bei der Herkunft die ID 1 einzutragen, da ID 1 sowohl die Herkunft *Webshop* als auch alle Marktplätze abdeckt. ID 0 (= manuelle Eingabe) wird nicht greifen, da der ElasticSync genau genommen keine manuelle Eingabe ist.
====
...
Zu guter Letzt klickst du auf die Abspielschaltfläche *Sync ausführen* (icon:play-circle-o[]). Je nach der zu importierenden Datenmenge kann dies einige Zeit dauern. Wenn der Sync durchgelaufen ist, erscheint die Meldung *Sync beendet*.
