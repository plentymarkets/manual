...
[#kontaktdaten]
== Best Practice: Kontaktdaten importieren

In diesem Beitrag beschreiben wir, wie du Kontaktdaten in plentymarkets importierst. Dazu benötigst du eine CSV-Datei mit den Daten, die importieren möchtest. Speichere diese Datei auf deinem Computer. Gehe dann wie im Folgenden beschrieben vor.
...
====
Beim Import von Kontaktoptionen sind die Felder *Typ* und *Herkunft* Pflichtfelder.
====
...
. Klicke auf *Vorname*.
. Klicke unterhalb der Liste auf den grünen Haken (*Auswählen*).
. Aktiviere in dieser Zeile ganz links den Toggle-Button (*Zeile aktivieren*). +
*_Wichtig:_* Die zu importierenden Datenfelder müssen alle aktiviert sein, damit die Zuordnung korrekt erfolgen kann.
. Klicke ganz unten links auf den Button *Feld*.
. Wähle aus der Dropdown-Liste *Eigener Wert*.
. Gib als Wert 0 oder 1 ein. +
...
. Klicke auf *Herkunft**.
. Klicke unterhalb der Liste auf den grünen Haken (*Auswählen*).
. Aktiviere auch in dieser Zeile ganz links den Toggle-Button (*Zeile aktivieren*). +
. *Speichere* die Einstellungen.

Klicke auf *Vorschau*, um dir eine Vorschau der Datei anzeigen zu lassen und die Inhalte zu prüfen.

Zu guter Letzt klickst du auf den Play-Button (*Sync ausführen*). Je nach der zu importierenden Datenmenge kann dies einige Zeit dauern. Wenn der Sync durchgelaufen ist, erscheint die Meldung *Sync beendet*.
