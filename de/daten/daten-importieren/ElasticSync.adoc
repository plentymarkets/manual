...
:keywords: Daten importieren, Datenimport, ElasticSync, Sync, Elastic Sync, Elastic-Sync, Synchronisieren, Synchronisation, Datenformat, Datenformate, Abgleich, Importoptionen, Zuordnung, Importdaten, Sync-Verlauf, Sync erstellen, Datentyp, manueller Upload, Daten manuell hochladen, Eigener Wert, RegEx-Funktion, Reg-Ex, Reg Ex, Regular Expression, FTP, HTTPS, SFTP, Mathematische Berechnung, Eigene Zuordnung, Importdaten filtern, Importdatei manipulieren, Fehlermeldung, Fehlerbehebung, Status, Protokoll, Fehlerprotokoll, Sync testen, Sync ausführen, Sync kopieren, Sync importieren, Sync exportieren, Abarbeitung, Jobs, Queue, Import abbrechen, Sync abbrechen, Sync-Import abbrechen, Backup, Backup einspielen
...
Diese Handbuchseite beschreibt den Datenimport in plentymarkets mit ElasticSync. Ein Import von Daten ist ratsam, wenn du z.B. neue Artikel, Kund:innen-Daten oder Lagerbestände in dein plentymarkets System einpflegen möchtest und die Daten bereits digital vorliegen. ElasticSync ermöglicht es über das Erstellen von sogenannten Syncs, Datenimporte auszuführen. Ein Import kann manuell oder automatisiert erfolgen.

[#1600]
== Benutzerrechte für Backend-Benutzer:innen vergeben

Benutzer:innen vom Typ *Backend* sehen nur bestimmte Bereiche und Menüs im plentymarkets Backend. Somit können sie nur eingeschränkt im System arbeiten. Sollen Mitarbeiter:innen mit dem Zugang *Backend* mit Syncs und Zuordnungen arbeiten können, muss eine Person mit *Admin*-Zugang die in <<tabelle-berechtigungen-elasticsync>> beschriebenen Rechte für Benutzer:innen mit dem Zugang *Backend* vergeben.
...
.Berechtigungen für Syncs und Zuordnungen
[cols=”1,2”]
|====
| Einstellung | Erläuterung

| *Daten > Sync > Aktualisieren*
|Erlaubt Backend-Benutzer:innen, Syncs zu aktualisieren.

| *Daten > Sync > Anzeigen*
|Erlaubt Backend-Benutzer:innen, Syncs zu sehen.

| *Daten > Sync > Erstellen*
|Erlaubt Backend-Benutzer:innen, Syncs zu erstellen.

| *Daten > Sync > Löschen*
|Erlaubt Backend-Benutzer:innen, Syncs zu löschen.

| *Daten > Sync > Zuordnung > Aktualisieren*
|Erlaubt Backend-Benutzer:innen, eine Zuordnung zu aktualisieren.

| *Daten > Sync > Zuordnung > Anzeigen*
|Erlaubt dem Backend-Benutzer, eine Zuordnung zu sehen.

| *Daten > Sync > Zuordnung > Erstellen*
|Erlaubt Backend-Benutzer:innen, eine Zuordnung zu erstellen.

| *Daten > Sync > Zuordnung > Löschen*
|Erlaubt Backend-Benutzer:innen, eine Zuordnung zu löschen.

| *Daten > Sync Daten-Log*
|Erlaubt Backend-Benutzer:innen, den Daten-Log eines Syncs zu aktualisieren, anzuzeigen, zu erstellen und zu löschen.
|====
...
Wenn du nun den Import durchführen möchtest, kannst du den <<Daten hochladen-Assistenten>> verwenden. Alternativ kannst du die Importdatei auch im Menü *Daten » Import* im jeweiligen Sync hochladen.


[#1900]
=== Assistent Artikeldatenimport durchlaufen

Für den Import von Artikeldaten steht dir neben dem Import-Tool der Artikeldatenimport-Assistent zur Verfügung. Dieser Assistent hilft dir dabei, eine auf deine Bedürfnisse zugeschnittene Importvorlage zu erstellen. Diese kannst du anschließend herunterladen und mit deinen Daten befüllen.

Du findest den Artikeldatenimport-Assistenten im Menü *Einrichtung » Assistenten » Grundeinrichtung*. In diesem Assistenten nimmst du die folgenden Einstellungen vor:
...
Wenn du nun den Import durchführen möchtest, kannst du dafür den <<Daten hochladen-Assistenten>> verwenden. Alternativ kannst du die Importdatei auch im Menü *Daten » Import* im jeweiligen Sync hochladen.
...
[#1605]
== Fehlermeldungen und Fehlerbehebung in ElasticSync

ElasticSync bietet dir verschiedene Möglichkeiten, fehlerhafte Syncs frühzeitig zu identifizieren und die Fehlerbehebung durchzuführen. So kannst du beispielweise Syncs vor der Ausführung auf Fehler überprüfen oder nach dem Import konkrete Fehlermeldungen in Form eines Protokolls einsehen. Nachfolgend sind die verschiedenen Möglichkeiten aufgeführt.
[#1610]
=== Syncs vor der Ausführung auf Fehler prüfen

Mithilfe der Validierung können Syncs bereits vor der Ausführung auf Fehler geprüft werden, um sicherzustellen, dass ein reibungsloser Import möglich ist.
...
[#1615]
=== Sync testen und Beispieldaten importieren

In der Sync-Übersicht kannst du einen Sync testen (icon:flask[role="yellow"]) und ausführen (icon:play-circle[role="grey"]). Das Testen erlaubt es dir, schnell festzustellen, ob deine Datei korrekt importiert werden kann.

[NOTE]
.Syncs testen
====
Beachte, dass beim Testen des Syncs bereits Bestand importiert wird. Wenn du *Sync testen* (icon:flask[role="yellow"]) wählst, werden nur die ersten zehn Zeilen der Importdatei importiert.
====

[#1620]
=== Protokoll eines verarbeiteten Syncs ansehen

Für jeden verarbeiteten Sync wird automatisch ein Protokoll erstellt, das eventuell aufgetretene Fehler anzeigt.
...
→ Die Status-Übersicht wird geöffnet.
. Klicke auf einen verarbeiteten Sync. +
→ Die Detailansicht wird geöffnet.
. Klicke oben auf *Protokoll* (icon:bars[role="yellow"]). +
→ Ein Fenster wird geöffnet und zeigt das Protokoll des verarbeiteten Syncs an.
. Wenn Fehler aufgetreten sind und du Hilfe aus dem Forum benötigst, klicke auf *In Zwischenablage kopieren* (icon:clone[role="yellow"]) und füge die Informationen im Forum ein.
...
Syncs können importiert und exportiert werden. So kannst du sowohl Syncs lokal sichern als auch Syncs von anderen Nutzer:innen importieren.
...
[#1645]
== Sync während des Imports abbrechen

Im Menü *Daten » Status* kannst du den bereits gestarteten Import eines Syncs abbrechen. Die Funktion ist somit nur verfügbar, wenn der Sync gerade verarbeitet wird.
...
. Klicke auf *Status* (icon:bars[role="yellow"]). +
→ Die Status-Übersicht wird geöffnet.
. Klicke in der Zeile des Syncs, den du abbrechen möchtest, ganz rechts auf *Import abbrechen* (icon:window-close[role="red"]). +
...
Die Status-Übersicht der Syncs bietet einen Überblick über den aktuellen Fortschritt von Sync-Jobs und zeigt Details zu eventuell aufgetretenen Fehlern während des Importvorgangs an.
...
In jeder Sync-Zeile steht außerdem eine separate Status-Schaltfläche zur Verfügung, die allein den Status des jeweiligen Syncs aufruft.
...
| *Verarbeitungs-ID*
|Jedes mal, wenn für einen Sync ein Import gestartet wird, wird eine Verarbeitungs-ID erzeugt, um die einzelnen Importe besser voneinander unterscheiden zu können.
...
| *Startzeit von*
|Uhrzeit wählen, ab der gefiltert werden soll. Die Startzeit bezieht sich auf den Start des Syncs.

| *Startdatum bis*
|Datum wählen, bis zu dem gefiltert werden soll.

| *Startzeit bis*
|Uhrzeit wählen, bis zu der gefiltert werden soll. Die Startzeit bezieht sich auf den Start des Syncs.
|====
