...
====
Möchte man einen Sync importieren und aktualisieren, müssen eine Zuordnung und ein Abgleich gesetzt sein. Ansonsten ist es nicht möglich, einen Sync zu testen oder auszuführen. Um den Sync speichern zu können, muss in der Zuordnung ein Datenfeld aktiviert sein. +
...
→ Das Fenster *Meine Dateien* wird geöffnet.
. Wähle eine Datei und klicke auf *Wählen*. +
