...
In diesem Beispiel arbeite ich mit dem Zuordnungsfeld *Attributauswahlkombination* und habe das Format *Attribut:Wert;Attribut:Wert* gewählt.
Gib schließlich die Attribute und Werte in deine CSV-Datei mit genau dieser Formatierung ein.

##image gegenüberstellung zuordnungsfeld und csv datei##
...
//Wenn Kunden sich später diesen Artikel in deinem Webshop anschauen, geben die Breadcrumbs oben die Standardkategorie an. Da sich diese Einstellung auf deinen Webshop bezieht und manche Händler plentymarkets verwenden, um mehr als einen Webshop zu verwalten, musst du plentymarkets mitteilen, auf welchen Webshop du dich beziehst.
...
Falls noch nicht bereits getan, ade den Beispiel-Sync herunter, entpacke die ZIP-Datei und <<daten/daten-importieren/ElasticSync#1640, Importiere die JSON-Datei in dein plentymarkets System>>.
...
.Prüfe die Zuordnungseinstellungen
##image::daten/daten-importieren/assets/zuordnung_neu.png[]##

Es empfiehlt sich, nur die Felder in die Zuordnung aufzunehmen, die tatsächlich übergeben werden sollen. Überflüssige Felder sollten deaktiviert werden (icon:toggle-off[role="green"]), da sie sonst Fehler verursachen können.
