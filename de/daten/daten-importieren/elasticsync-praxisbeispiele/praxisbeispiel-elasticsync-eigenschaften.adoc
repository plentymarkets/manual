...
//ToDo - Bilder
//ToDo - Check the ID numbers (Verknüpfung)
//ToDo - figure out whether the Verknüpfungen need to be done in multiple Zuordnungen or if it can all be done at once (testing)
...
Diese Eigenschaft hat die ID 180.
* *Wert* = Welchen Wert hat die Eigenschaft?
...
Diese Eigenschaft hat die ID 200.
* *Auswahl-ID* = Welche Auswahlmöglichkeit trifft in diesem Fall zu?
Zum Beispiel: "Ja", die Kopfhörer haben ein integriertes Mikrofon. Dieser Auswahlwert hat die ID 200.
...
Zum Beispiel: die Kopfhörer unterstützen die Profile A, B, C und D.
...
Zum Beispiel, die Reichweite.
Diese Eigenschaft hat die ID 160.
* *Wert* = In dieser Spalte habe ich angegeben, welchen Wert die Eigenschaft hat.
Zum Beispiel hat die Eigenschaft "Reichweite" den Wert "10" Meter.
* *Sprache* = Stell dir vor, du willst eine Text-Eigenschaft verknüpfen.
Das ist ein bisschen komplizierter, weil Texte sprachspezifisch sind.
In dieser Spalte musst du zusätzlich angeben, in welcher Sprache der Text verfasst wurde.
////
