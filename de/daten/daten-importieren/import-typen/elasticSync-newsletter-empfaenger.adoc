...
// welche Daten darf man überhaupt anpassen? Die Bestätigung des Kunden ja schon mal nicht.

// wird dann beim Import ein neuer Kontaktdatensatz erstellt?
...
Das Ergebnis des Imports findest du im Backend im Menü <<crm/newsletter-versenden#, *Einrichtung » CRM » Newsletter » E-Mails* » _Ordner aufklappen_>>, nachdem du die Suche (icon:search[role="blue"]) ausgeführt hast.
...
Gib die ID des Kontakts in deine CSV-Datei ein.

*_Standardwert:_* kein Standardwert

*_Zulässige Importwerte:_* Numerisch (interne ID)

Das Ergebnis des Imports findest du im Backend im Menü <<crm/newsletter-versenden#, CRM » Newsletter>>.

// wie muss man die Verbindung zum Kontakt hier verstehen? In der Einrichtung-CRM-Newsletter-UI wird die Kontakt-ID ja nicht abgebildet. Generell müssen ja Newsletter-Abonnenten nicht zwangsläufig registrierte Kunden mit einem Kontaktdatensatz sein, sondern können auch bloß "Interessenten" sein.
...
Das Ergebnis des Imports findest du im Backend im Menü <<crm/newsletter-versenden#, *Einrichtung » CRM » Newsletter » E-Mails* » _Ordner aufklappen_>>. Dann die Suche (icon:search[role="blue"]) ausführen, den Datensatz aufklappen (icon:plus-square-o[]). Der Wert ist im Feld *Vorname* gespeichert.
...
Das Ergebnis des Imports findest du im Backend im Menü <<crm/newsletter-versenden#, *Einrichtung » CRM » Newsletter » E-Mails* » _Ordner aufklappen_>>. Dann die Suche (icon:search[role="blue"]) ausführen und den Datensatz aufklappen (icon:plus-square-o[]). Der Wert ist im Feld *Nachname* gespeichert.
'''
...
Das Ergebnis des Imports findest du im Backend im Menü <<crm/newsletter-versenden#, *Einrichtung » CRM » Newsletter » E-Mails* » _Ordner aufklappen_>>. Dann die Suche (icon:search[role="blue"]) ausführen und den Datensatz aufklappen (icon:plus-square-o[]). Der Wert ist im Feld *E-Mail* gespeichert.
...
// ist hier die Anrede im Kontaktdatensatz gemeint? wie muss man die Verbindung hier verstehen? In der Einrichtung-CRM-Newsletter-UI wird das Geschlecht ja nicht abgebildet.
...
// ist hier das Geburtsdatum im Kontaktdatensatz gemeint? wie muss man die Verbindung hier verstehen? In der Einrichtung-CRM-Newsletter-UI wird das Geburtsdatum ja nicht abgebildet.
...
// prüfen, ob es sich hier um die Sprachkürzel handelt.

// wo finde ich das? im Kontaktdatensatz? Im Einrichtung-CRM-Newsletter-Menü wird nichts bezgl der Sprache angezeigt.
...
Das Ergebnis des Imports findest du im Backend im Menü <<crm/newsletter-versenden#, *Einrichtung » CRM » Newsletter » E-Mails* » _Ordner aufklappen_>>. Dann die Suche (icon:search[role="blue"]) ausführen und den Datensatz aufklappen (icon:plus-square-o[]). Der Wert ist im Feld *Bestätigt am* gespeichert.
...
// wo im Backend sieht man die IP-Adresse? Oder ist dieses Feld nur für interne Zwecke gedacht? Wenn ja warum? Was bringt mir die IP-Adresse?
...
// wo im Backend sieht man die URL? Oder ist dieses Feld nur für interne Zwecke gedacht? Wenn ja warum? Was bringt mir der Link? Laut Asad sind die Bestätigungslinks ohnehin nur 24 Stunden gültig.
