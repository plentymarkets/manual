...
Gib die Auftragspositions-ID in deine CSV-Datei ein. Über diese ID erfolgt die Verknüpfung mit der richtigen Auftragsposition. Die Auftragspositions-ID erhältst du, wenn Aufträge exportierst. Nutze für den Export entweder den <<daten/daten-exportieren/katalog-datenfelder-auftraege#150, Katalog>> oder den <<daten/daten-exportieren/FormatDesigner/format-typen/auftragspositionen#1500, Formattyp Auftragspositionen>>.
...
Gib einen selbst gewählten Identifikator in deine CSV-Datei ein. Dieser erlaubt auch Leer- und Sonderzeichen und kann frei gewählt werden. Über diesen Identifikator erfolgt dann die Verknüpfung mit der richtigen Auftragsposition. Daher musst du zusätzlich das Zuordnungsfeld *Verknüpfe mit eigenem Identifikator* hinzufügen.
...
Gib den selbst gewählten Identifikator in deine CSV-Datei ein. Über diesen Identifikator erfolgt dann die Verknüpfung mit der richtigen Auftragsposition. Daher musst du zusätzlich das Zuordnungsfeld *Setze eigenen Identifikator* hinzufügen.
