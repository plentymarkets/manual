...
In <<tabelle-zuordnungsfelder-kampagnendaten>> werden Felder erläutert, die für den Datenabgleich von Kontakten, Firmen und Adressen zur Verfügung stehen. Anhand des gewählten Abgleichfelds wird geprüft, ob der importierte Datensatz in plentymarkets vorhanden ist.
...
|Kategorien
|Kategorien, bei denen der Gutschein einlösbar ist.

|Artikel
|Artikel, für die die Gutscheine der Kampagne einlösbar sind. +
*_Hinweis:_* Es können bis zu 20 Artikel hinzugefügt werden.

|Mandant (Shop)
|Mandant (Shop), für die der Gutschein einlösbar sein soll.

|Kundenklasse
|Kundenklasse, für die der Gutschein einlösbar sein soll.
