= stockManagementStockMovements
:lang: de
include::{includedir}/_header.adoc[]
:keywords: orderItemAmounts, Report, Rohdaten
:position: 130
:url: daten/report/datenformate/stockmanagementstockmovements

Der Datenformat-Creator *stockManagementStockMovements* exportiert Daten zu den im System vorhandenen Warenbewegungen und speichert diese im Cloud-Speicher ab.

== Technische Spezifikationen

<<tabelle-spezifikationen-stockmanagementstockmovements>> erläutert die verfügbaren Eigenschaften und die dazugehörigen Werte.

[[tabelle-spezifikationen-stockmanagementstockmovements]]
.Technische Spezifikationen des Formats *stockManagementStockMovements*
[cols=4]
|====
|Eigenschaft |Bedeutung |Wert |Erläuterung

|dataName
|Name
|stockManagementStockMovements
|Name des Creator-Prozesses und Präfix der erstellten Datei. +
Dateiname: *stockManagementStockMovements-xxx.csv.gz*

|fileFormat
|Dateiformat
|CSV, GZIP
|Die Daten werden als per GZIP (RFC 1952) komprimierte CSV-Datei bereitgestellt.

|limitPage
|Zeilen pro Anfrage
|bis zu 10.000
|Die maximale Anzahl der Datenzeilen, die in eine einzelne Datei geschrieben wird.

|limitDay
|Zeilen pro Tag
|bis zu 200.000
|Die maximale Anzahl der Datenzeilen, die insgesamt an einem Tag geschrieben wird.

|earliestPossibleDataDate
|Frühester Zeitpunkt
|2019-01-01
|Zeitpunkt, ab dem die Rohdaten erhoben werden können. Die Daten werden nach dem Datum gefiltert, das in der Spalte *dataUpdatedAtColumnName* enthalten ist.

|deleteRawDataAfterDaysMinimum
|Minimale Anzahl an Tagen bis zur Löschung
|20
|Die minimale Anzahl an Tagen, bis die generierte Rohdaten-Datei wieder vom Cloud-Speicher entfernt wird. Eine Weiterverarbeitung muss somit vorher erfolgen. Die genaue Dauer kann im Menü *Daten » Rohdaten* definiert werden.

|deleteRawDataAfterDaysMaximum
|Maximale Anzahl an Tagen bis zur Löschung
|80
|Die maximal Anzahl an Tagen, bis die generierte Rohdaten-Datei wieder vom Cloud-Speicher entfernt wird. Eine Weiterverarbeitung muss somit vorher erfolgen. Die genaue Dauer kann im Menü *Daten » Rohdaten* definiert werden.

|defaultActive
|Standardmäßig aktiv
|false
|Der Export läuft erst dann automatisch im Hintergrund, wenn der Creator zuvor im Menü *Daten » Rohdaten* aktiviert wurde.

|interval
|Intervall
|täglich
|Mit dieser Einstellung werden 1 x pro Tag, gewöhnlich über Nacht, alle seit dem letzten Durchlauf geänderten Daten exportiert.

|====

== Datenspalten

<<tabelle-datenspalten-stockmanagementstockmovements>> erläutert die verfügbaren Eigenschaften und die dazugehörigen Werte.

[[tabelle-datenspalten-stockmanagementstockmovements]]
.Datenspalten des Formats *stockManagementStockMovements*
[cols="1,3,3"]
|====
|Spaltenname |Vollständige Bezeichnung |Erläuterung

|plenty_id
|plenty id
|ID des plentymarkets Systems

|i_id
|item ID
|ID des Artikels

|iv_id
|item variation ID
|ID der Artikelvariante

|smw_id
|stock management warehouse ID
|ID des Lagers

|sl_name
|storage location ID
|Name des Lagerorts

|smsm_id
|stock management stock movement ID
|Enthält den Primärschlüssel der Datenzeile.

|smsm_quantity
|stock management stock movement quantity
|Artikelmenge, die in der Warenbewegung enthalten ist

|smsm_reason_id
|stock management stock movement reason ID
|ID des Grundes der Warenbewegung +
Mögliche Werte: +
101 = Wareneingang gebucht +
102 = Während Inventur eingebucht +
301 = Lagerkorrektur (allgemein) +
307 = Lagerkorrektur wegen Schaden am Artikel +
401 = Umbuchung +
Weitere mögliche Werte sind in unserer link:https://developers.plentymarkets.com/rest-doc/item_variation_stock/details#intake-stock[Entwicklerdokumentation^] zu finden.

|smsm_created_at
|stock management stock movement created at
|Zeitstempel der Erstellung +
Datumsformat: MM/DD/YYYY HH:MI:SS TZ

|====
