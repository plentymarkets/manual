= orderProperties
:lang: de
include::{includedir}/_header.adoc[]
:keywords: orderProperties, Report, Rohdaten
:position: 100
:url: daten/report/datenformate/orderproperties

Der Datenformat-Creator *orderProperties* exportiert Daten zu den Eigenschaften der im System vorhandenen Aufträge und speichert diese im Cloud-Speicher ab.

== Technische Spezifikationen

<<tabelle-spezifikationen-order-properties>> erläutert die verfügbaren Eigenschaften und die dazugehörigen Werte.

[[tabelle-spezifikationen-order-properties]]
.Technische Spezifikationen des Formats *orderProperties*
[cols=4]
|====
|Eigenschaft |Bedeutung |Wert |Erläuterung

|dataName
|Name
|orderProperties
|Name des Creator-Prozesses und Präfix der erstellten Datei. +
Dateiname: *orderProperties-xxx.csv.gz*

|fileFormat
|Dateiformat
|CSV, GZIP
|Die Daten werden als per GZIP (RFC 1952) komprimierte CSV-Datei bereitgestellt.

|limitPage
|Zeilen pro Anfrage
|bis zu 8.000
|Die maximale Anzahl der Datenzeilen, die in eine einzelne Datei geschrieben wird.

|limitDay
|Zeilen pro Tag
|bis zu 100.0000
|Die maximale Anzahl der Datenzeilen, die insgesamt an einem Tag geschrieben wird.

|earliestPossibleDataDate
|Frühester Zeitpunkt
|2017-01-01
|Zeitpunkt, ab dem die Rohdaten erhoben werden können. Die Daten werden nach dem Datum gefiltert, das in der Spalte *dataUpdatedAtColumnName* enthalten ist.

|deleteRawDataAfterDaysMinimum
|Minimale Anzahl an Tagen bis zur Löschung
|1095
|Die minimale Anzahl an Tagen, bis die generierte Rohdaten-Datei wieder vom Cloud-Speicher entfernt wird. Eine Weiterverarbeitung muss somit vorher erfolgen. Die genaue Dauer kann im Menü *Daten » Rohdaten* definiert werden.

|deleteRawDataAfterDaysMaximum
|Maximale Anzahl an Tagen bis zur Löschung
|1095
|Die maximal Anzahl an Tagen, bis die generierte Rohdaten-Datei wieder vom Cloud-Speicher entfernt wird. Eine Weiterverarbeitung muss somit vorher erfolgen. Die genaue Dauer kann im Menü *Daten » Rohdaten* definiert werden.

|dataUpdatedAtColumnName
|Spaltenbezeichnung
|oip_updated_at
|Name der Spalte, die den Zeitstempel der letzten Aktualisierung der Daten enthält.

|defaultActive
|Standardmäßig aktiv
|ja
|Der Export läuft standardmäßig automatisch im Hintergrund.

|interval
|Intervall
|täglich
|Mit dieser Einstellung werden 1 x pro Tag, gewöhnlich über Nacht, alle seit dem letzten Durchlauf geänderten Daten exportiert.

|====

== Datenspalten

<<tabelle-datenspalten-order-properties>> erläutert die verfügbaren Eigenschaften und die dazugehörigen Werte.

[[tabelle-datenspalten-order-properties]]
.Datenspalten des Formats *orderProperties*
[cols="1,3,3"]
|====
|Spaltenname |Vollständige Bezeichnung |Erläuterung

|plenty_id
|plenty ID
|ID des plentymarkets Systems

|op_id
|order property ID
|ID der Eigenschaft des Auftrags +
Mögliche Werte: +
1 = Lager +
2 = Versandprofil +
3 = Zahlungsart +
4 = Zahlungsstatus +
5 = Externes Versandprofil +
6 = Dokumentensprache (in den Auftragsdokumenten) +
7 = Externe Auftrags-ID +
Weitere mögliche Werte sind in unserer link:https://developers.plentymarkets.com/rest-doc/order_order_property/details[Entwicklerdokumentation^] zu finden.

|o_id
|order ID
|ID des Auftrags

|opt_id
|order property type ID
|plenty ID und ID des Typs der Auftragseigenschaft. +
 Mögliche Werte: +
 1 = Auftrag +
 2 = Lieferauftrag +
 3 = Retoure +
 4 = Gutschrift +
 5 = Gewährleistung +
 6 = Reparatur +
 7 = Angebot +
 8 = Vorbestellung +
 9 = Sammelauftrag +
 10 = Sammelgutschrift +
 11 = Scheduler +
 12 = Nachbestellung +
 13 = Teillieferung

|op_value
|order property value
|Wert der Eigenschaft des Auftrags

|op_updated_at
|order property updated at
|Zeitstempel der letzten Aktualisierung +
Datumsformat: MM/DD/YY HH:MI:SS TZ

|====
