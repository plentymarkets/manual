= orderItemAmounts
:lang: de
include::{includedir}/_header.adoc[]
:keywords: orderItemAmounts, report, raw data, rohdaten, export
:position: 50

Der Datenformat-Creator *orderItemAmounts* export die Beträge zu den im System vorhandenen Auftragspositionen und speichert diese im Cloud-Speicher ab.

== Technische Spezifikationen

<<tabelle-spezifikationen-orderitemamounts>> erläutert die verfügbaren Eigenschaften und die dazugehörigen Werte.

[[tabelle-spezifikationen-orderitemamounts]]
.Technische Spezifikationen des Datenformats *orderItemAmounts*
[cols=4]
|====
|Eigenschaft |Bedeutung |Wert |Erläuterung

|dataName
|Name
|orderItemAmounts
|Name des Creator-Prozesses und Präfix der erstellten Datei. +
Dateiname: *orderItemAmounts-xxx.csv*

|fileFormat
|Dateiformat
|CSV, GZIP
|Die Daten werden als per GZIP (RFC 1952) komprimierte CSV-Datei bereitgestellt.

|limitPage
|Zeilen pro Anfrage
|bis zu 8.000
|Die maximale Anzahl der Datenzeilen, die in eine einzelne Datei geschrieben wird.

|limitDay
|Zeilen pro Tag
|bis zu 100.0000
|Die maximale Anzahl der Datenzeilen, die insgesamt an einem Tag geschrieben wird.


|earliestPossibleDataDate
|Frühester Zeitpunkt
|01.01.2017
|Zeitpunkt, ab dem die Rohdaten erhoben werden können. Die Daten werden nach dem Datum gefiltert, das in der Spalte *dataUpdatedAtColumnName* enthalten ist.
////
TODO Rückfrage: hier sollte Jan nochmal schauen, ob das sachlich richtig ist.
////
|deleteRawDataAfterDaysMinimum
|Minimale Anzahl an Tagen bis zur Löschung
|1095
|Die minimale Anzahl an Tagen, bis die generierte Rohdaten-Datei wieder vom Cloud-Speicher entfernt wird. Eine Weiterverarbeitung muss somit vorher erfolgen. Die genaue Dauer kann im Menü *Daten » Rohdaten* definiert werden.

|deleteRawDataAfterDaysMaximum
|Maximale Anzahl an Tagen bis zur Löschung
|1095
|Die maximal Anzahl an Tagen, bis die generierte Rohdaten-Datei wieder vom Cloud-Speicher entfernt wird. Eine Weiterverarbeitung muss somit vorher erfolgen. Die genaue Dauer kann im Menü *Daten » Rohdaten* definiert werden.

|defaultActive
|Standardmäßig aktiv
|ja
|Der Export läuft standardmäßig automatisch im Hintergrund.

|interval
|Intervall
|täglich
|Mit dieser Einstellung werden 1 x pro Tag, gewöhnlich über Nacht, alle seit dem letzten Durchlauf geänderten Daten exportiert.

|====

== Datenspalten

<<tabelle-datenspalten-orderitemamounts>> erläutert die verfügbaren Eigenschaften und die dazugehörigen Werte.

[[tabelle-datenspalten-orderitemamounts]]
.Datenspalten des Foärmats *orderItemAmounts*
[cols="1,3,3"]
|====
|Spaltenname |Vollständige Bezeichnung |Erläuterung

|oi_id
|order item ID
|ID der Auftragsposition

|oia_is_system_currency
|order item amount is system currency
|Markierung, die anzeigt, ob die an der Auftragsposition hinterlegte Währung der Systemwährung entspricht +
Mögliche Werte: +
0 = Nein +
1 = Ja

|oi_price_gross
|order item price gross
|Bruttopreis der Auftragsposition

|oi_price_net
|order item price net
|Nettopreis der Auftragsposition

|oi_price_currency
|order item price currency
|Währung des Preises der Artikelposition

|oi_purchase_price
|order item purchase price
|Einkaufspreis der Artikelposition

|oi_updated_at
|order item updated at
|Zeitstempel der letzten Aktualisierung +
Datumsformat: MM/DD/YY HH:MI:SS TZ

|====
