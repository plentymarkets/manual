...
Angenommen, du möchtest zur Auswertung einige grundlegende Auftragsdaten aller Aufträge, die im Laufe der letzten 14 Tage eingegangen sind, exportieren. Dann gehe wie in den nächsten Kapiteln beschrieben vor.
...
Hast du alle Änderungen in den Katalog eingepflegt und ihn gespeichert, kannst du mithilfe der <<daten/daten-exportieren/kataloge-verwalten#255, Vorschau>> wieder überprüfen, ob alles passt. Nach Überprüfung der Exporteinstellungen kann der <<daten/daten-exportieren/kataloge-verwalten#260, Katalog exportiert>> werden.
...
//Wenn Alex die Liste der Versanddienstleister ins Handbuch eingefügt hat, Verlinkung hinzu.
* *Dokumente > Dokumenttyp*
* *Dokumente > Benutzer-ID* +
Hier wird die ID der Benutzer:in angegeben, die das Dokument erstellt hat.
* *Kontakt > Kontakt-ID* verknüpfen mit *Kontakt > Vorname* verknüpfen mit *Kontakt > Nachname* +
So werden dir diese drei Angaben zusammen ausgegeben.
* *Adresse > Adress-ID*
...
* *Erstellungsdatum* +
Füge den Filter über *Filter hinzufügen* hinzu und wähle dann *Heute* aus der Dropdown-Liste aus. Somit werden nur die Aufträge von heute exportiert.
* *Auftragsstatus* +
Füge den Filter über *Filter hinzufügen* hinzu und wähle dann den oder die Auftragsstatus aus, die du für den Warenausgang nutzt. Standardmäßig ist dies *7 | Warenausgang gebucht*.
* *Lieferland* +
Füge den Filter über *Filter hinzufügen* hinzu und wähle dann das oder die Lieferländer aus, die für dich relevant sind. Angezeigt werden alle Lieferländer, nicht nur deine aktiven.
* *Auftragsherkunft*
...
Auch die Filter lassen sich individuell erweitern. Durch Hinzufügen von weiteren Filter werden die Ergebnisse eingegrenzt. +
Speichere (icon:save[set=material]) den Katalog nach Setzen der Filter erneut.
