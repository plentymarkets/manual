...
|*Artikelpaket*
|Ermöglicht es, nach <<artikel/anwendungsfaelle/multipacks-pakete-sets#2000, Artikelpaketen>> zu filtern. Es sind die Optionen `Ja` und `Nein` verfügbar. +
Wähle `Ja`, um nur Artikelpakete zu exportieren. Wähle `Nein`, um nur normale Varianten (die nicht Bestandteile eines Artikelpakets sind) zu exportieren. +
*_Hinweis:_* Zurzeit ist es noch nicht möglich, einen Bestandteil eines Artikelpakets zu exportieren. Wähle den Filter in diesem Fall nicht, um alle Varianten zu exportieren.
