...
:keywords: Export, elastisch, Elastic Export, FormatDesigner, Format Designer, Exportformat, Exportformate, Elastic Export Plugin, Elastischer Export Plugin, Preisportale, Preisvergleiche, Preisvergleich
:position: 10
:url: daten/daten-exportieren/elastischer-export
:id: COEO41G
:author: team-multi-channel, team-elasticsync

Mit dem elastischen Export exportierst du Daten aus deinem plentymarkets System. Du hast zwei Möglichkeiten, den elastischen Export zu nutzen:

* Exportiere Daten aus deinem plentymarkets System. Verwende dazu zuerst den link:https://marketplace.plentymarkets.com/plugins/integration/formatdesigner_6483[FormatDesigner^], um sogenannte Datenformate zu erstellen. Mit anderen Worten entscheidest du, welche Daten exportiert werden sollen. In einem zweiten Schritt verwendest du dann den link:https://marketplace.plentymarkets.com/plugins/channels/marktplaetze/elasticexport_4763[Elastischen Export^]{nbsp}icon:external-link[], um die soeben erstellten Datenformate zu exportieren.

* Erstelle Exportformate für Preissuchmaschinen, Preisportale, Marktplätze und Affiliate-Dienstleister, mit denen Daten an die Schnittstellen übertragen werden. Weitere Informationen dazu erhältst du im Kapitel <<Exportformate für Schnittstellen erstellen, Exportformate für Schnittstellen erstellen>>.

[#10]
== Elastischer Export-Plugin einrichten

[IMPORTANT]
.Elastischer Export-Plugin herunterladen
====
Beim Elastischen Export handelt es sich um ein Plugin. Um das Elastischer Export-Plugin nutzen zu können, lade es im link:https://marketplace.plentymarkets.com/[plentyMarketplace^]{nbsp}icon:external-link[] herunter.
====

Mit dem Elastischer Export-Plugin werden Exporte für die oben genannten Bereiche erstellt. Die Auswahl der im Elastischen Export zur Verfügung stehenden Datenformate kann durch Marketplace-Plugins erweitert werden. Um weitere Plugin-Formate nutzen zu können, muss das Elastischer Export-Plugin zuvor bereitgestellt worden sein.

[#20]
=== Lizenz
Das gesamte Projekt unterliegt der GNU AFFERO GENERAL PUBLIC LICENSE – weitere Informationen findest du in der link:https://github.com/plentymarkets/plenty-plugin-elastic-export/blob/master/LICENSE.md[LICENSE.md^]{nbsp}icon:external-link[].

[#50]
== Exportformate für Schnittstellen erstellen

Mit dem elastischen Export im Menü *Daten » Elastischer Export* erstellst du Exportformate für Preissuchmaschinen, Preisportale, Marktplätze und Affiliate-Dienstleister, mit denen du Daten an die Schnittstellenpartner überträgst. Um den elastischen Export nutzen zu können, muss das Plugin installiert sein. +
...
====
Installiere das Plugin link:https://marketplace.plentymarkets.com/plugins/channels/marktplaetze/elasticexport_4763[Elastischer Export^]{nbsp}icon:external-link[] und die Plugins für die Exportformate in einem Plugin-Set mit Standard-Mandantenverknüpfung.
====

[#100]
=== Beim Schnittstellenpartner registrieren

Bevor du dein Exportformat an den Schnittstellenpartner übertragen kannst, musst du dich auf der Webseite der Schnittstellenpartner registrieren. Die Links zu den Webseiten der Preissuchmaschinen, Preisportale, Marktplätze und Affiliate-Dienstleister, zu denen es eine Schnittstelle über Plugins in plentymarkets gibt, findest du in <<#exportformate>>. Alle Exportformate bis auf *BolOfferExport* und *BolItemExport* (diese beiden werden ohne zusätzliche Plugins aus dem Marketplace bereitgestellt) müssen zunächst aus dem plentymarketplace installiert werden, damit die Datenformate im elastischen Export zur Verfügung stehen.
...
| *BilligerDE*
|link:https://marketplace.plentymarkets.com/plugins/channels/preisportale/elasticexportbilligerde_4901[billiger.de^]{nbsp}icon:external-link[] ist ein deutsches Preisvergleichsportal mit TÜV-Zertifikat, das neben Preisvergleichen auch Testberichte und Nutzerbewertungen anbietet.
...
| *CdiscountCOM*
|Auf dem Marktplatz link:https://marketplace.plentymarkets.com/plugins/markets/elasticexportcdiscountcom_4738/[Cdiscount^]{nbsp}icon:external-link[] bietest du deine Artikel zum Verkauf an. Weitere Informationen zu diesem Marktplatz findest du auf der Handbuchseite <<maerkte/cdiscount#, Cdiscount einrichten>>.
...
| *FashionDE*
|link:https://marketplace.plentymarkets.com/plugins/markets/elasticexportfashionde_4742/[Fashion.de^]{nbsp}icon:external-link[] ist ein Preisvergleichsportal für Mode und Lifestyle.
...
| *GuenstigerDE*
|link:https://marketplace.plentymarkets.com/plugins/markets/elasticexportguenstigerde_4745/[guenstiger.de^]{nbsp}icon:external-link[] ist ein Preisvergleichsportal, das neben Preisvergleichen auch Nutzermeinungen, Testberichte und Händlerbewertungen anbietet.
...
| *KaufluxDE*
|Auf dem Marktplatz link:https://marketplace.plentymarkets.com/plugins/markets/elasticexportkaufluxde_4737/[Kauflux^]{nbsp}icon:external-link[] bietest du deine Artikel zum Verkauf an. Weitere Informationen zur Einrichtung von Kauflux findest du auf der Handbuchseite <<maerkte/kauflux#, Kauflux einrichten>>.
...
| *RakutenDE*
|Auf dem Marktplatz link:https://marketplace.plentymarkets.com/plugins/markets/elasticexportrakutende_4721/[Rakuten^]{nbsp}icon:external-link[] bietest du deine Artikel zum Verkauf an. Weitere Informationen zur Einrichtung von Rakuten findest du auf der Handbuchseite <<maerkte/rakuten/rakuten-einrichten#, Rakuten einrichten>>.
...
Dieses Kapitel beschreibt, wie du neue Exportformate erstellst und welche Einstellungen du vornehmen musst. Bis auf *BolItemExport* und *BolOfferExport* müssen alle weiteren Formate zunächst aus dem link:https://marketplace.plentymarkets.com/plugins/channels/marktplaetze[plentyMarketplace^]{nbsp}icon:external-link[] installiert werden, bevor die Formate im elastischen Export zur Verfügung stehen.
...
*Übertragung:* Wenn unter *Bereitstellung* die Option *FTP* gewählt wurde, ein Intervall für die Übertragung der Daten wählen. Wenn du die Einstellung *Zeitplan* wählst, lege unter *Zeitplan* einen Zeitraum fest, in dem die Daten übertragen werden sollen.

| *Dateiname*
| Wenn unter *Bereitstellung* die Option *Admin-Bereich* gewählt wurde, Dateinamen eingeben. Der Dateiname muss bei allen Bereitstellungstypen die Endung des Formats haben, z.B. Suffix .csv bei einer CSV-Datei.

2+| Artikelfilter

| *Artikelfilter wählen*
a|Wählen, welche Artikel beim Export übertragen werden sollen. Dazu Artikelfilter aus der Dropdown-Liste wählen und auf *Hinzufügen* (icon:plus-square[role="green"]) klicken. Standardmäßig sind keine Filter voreingestellt. Es ist möglich, alle Artikelfilter aus der Dropdown-Liste nacheinander hinzuzufügen.

* *Aktiv* = Nur aktive Varianten werden übertragen.
* *Artikeltyp* = Nur Artikel des gewählten <<artikel/multipacks-pakete-sets-verwalten#, Typs>> werden übertragen.
...
* *Preise* = Nur Artikel mit einem bestimmten Preis werden übertragen. +
  ** *Ohne Preis* = Artikel, die keinem hinterlegten Preis haben, werden übertragen. +
  ** *Mit Preis* = Artikel, die einen hinterlegten Preis haben, werden übertragen. +
  ** *Mit Preis unter* = Gib einen Preis ein. Artikel, die einen niedrigeren Preis haben, werden übertragen. +
  ** *Mit Preis über* = Gib einen Preis ein. Artikel, die einen höheren Preis haben, werden übertragen. +
  ** *Mit Preis zwischen* = Gib eine Unter- und eine Obergrenze ein. Artikel, die einen Preis zwischen den zwei Werten haben, werden übertragen.
* *Tag (UND-Modus)* = Ein oder mehrere Tag-Namen eingeben. Artikel, die mit _allen_ der gewählten <<basics/arbeiten-mit-plentymarkets/tags#, Tags>> verknüpft sind, werden übertragen.
* *Tag (ODER-Modus)* = Ein oder mehrere Tag-Namen eingeben. Artikel, die mit _mindestens einem_ der gewählten <<basics/arbeiten-mit-plentymarkets/tags#, Tags>> verknüpft sind, werden übertragen.
...
In der Exportübersicht im Menü *Daten » Elastischer Export » Tab: Exporte* werden alle erstellten Exportformate angezeigt. Über die Funktion *Ansicht einrichten* wählst du, welche Spalten angezeigt werden. Zeige mit der Maus auf die Symbolleiste und klicke auf *Ansicht einrichten* (icon:cog[]). Ein neues Fenster *Ansicht einrichten* wird geöffnet und du aktivierst oder deaktivierst die Spaltennamen. Die folgenden Spalten stehen zur Verfügung:
...
. Öffne das Menü *Daten » Elastischer Export*.
. Nimm die Filtereinstellungen im Bereich *Filter* vor. Beachte dazu die Erläuterungen in <<#filtereinstellungen-elastischer-export>>.
...
====
Mit einem Klick auf *Zurücksetzen* (icon:undo[role="orange"]) löschst du die Suchkriterien. Klicke anschließend erneut auf *Suchen* (icon:search[role="blue"]), damit alle bereits erstellten Exportformate wieder in der Übersicht angezeigt werden und du neue Filter einstellen kannst.
====
