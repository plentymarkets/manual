...
→ Die verfügbaren plentymarkets Datenfelder werden links als Gruppen angezeigt.
...
. Um einem Datenfeld ein weiteres plentymarkets Datenfeld zuzuordnen, klicke rechts auf *Datenfeld hinzufügen* (icon:link[rotate=90]). +
→ Der Bearbeitungsmodus wird geöffnet.
. Wähle beliebig viele Datenfelder oder *Eigener Wert*.
. Hast du *Eigener Wert* gewählt, gib einen Wert in das Eingabefeld darunter ein.
...
*_Hinweis_*: Ist in einer Verknüpfung ein Datenfeld nicht vorhanden, so werden nur die übrigen Datenfelder exportiert. Wenn du einem Marktplatz-Datenfeld mehr als ein plentymarkets Datenfeld zuordnest, wird nur ein Wert übertragen. Die Felder werden in der Reihenfolge geprüft, in der sie zugeordnet wurden. Wenn also das erste Datenfeld keinen Wert liefert, wird das zweite Datenfeld übertragen usw. +
...
. Wähle einen Katalog. +
→ Der Katalog wird geöffnet.
. Öffne links das Tab *Einstellungen* und wähle dort den Bereich *Download*. +
