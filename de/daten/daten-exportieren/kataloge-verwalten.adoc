...
Nachdem du alle Einstellungen vorgenommen und mindestens ein Datenfeld hinzugefügt hast, kannst du einen Katalog exportieren und die Export-Datei im gewünschten Format herunterladen. Zurzeit stehen die Formate CSV, TXT, JSON und XML zur Verfügung.

[IMPORTANT]
.Via Export starten erfolgt kein Export an Marktplätze
====
Wenn du die Funktion *Export starten* nutzt, handelt es sich nicht um den automatischen, sondern lediglich um einen manuellen Export. Deine Daten werden in diesem Fall nicht an einen Marktplatz übertragen.
====
...
. Klicke auf *Export starten* (icon:download[]). +
