...
| *Status*
|Aufträge werden anhand des Auftragsstatus gefiltert. Hier ist ein bestimmter Status wählbar. Mit den Operatoren gleich (==), größer als (>), größer gleich (>=), kleiner als (<) oder kleiner gleich (⇐) filterst du nach Aufträgen, die dem gewählten Status beispielsweise entsprechen oder darüber oder darunter liegen.
...
|Es werden alle Aufträge gefiltert, die die angegebenen Varianten enthalten. Gib die Varianten-IDs der Artikel ein, nach denen gefiltert werden soll. Du kannst mehrere
...
|Die Aktion wendet automatisch eine vorher eingerichtete Auftrags-Vorlage auf einen Auftrag an. Wähle für diese Aktion eine Auftrags-Vorlage aus der Dropwdown-Liste aus. +
...
|Die Aktion lädt eine Rechnung bei Amazon hoch.
...
|Die Aktion meldet die Stornierung eines Auftrags an den Marktplatz <<maerkte/real-de/real-de-einrichten#, real.de>>. Wähle aus der Dropdown-Liste den Grund der Stornierung aus (Stornierung durch Kunde, Umtausch durch Kunde, Falsche Artikelbeschreibung, Falsche Lieferadresse, Kunde hat Ware nicht erhalten, Artikel ausverkauft, Falscher Verkaufspreis, Lieferadresse wird nicht beliefert, Sonstiger Grund).

|*Rechnung bei Kaufland.de hochladen*
|Die Aktion lädt eine Rechnung bei real.de hoch.
...
|Die Aktion erzeugt ein <<auftraege/auftraege-verwalten#1670, Stornobeleg Rechnung>> für einen Auftrag. Ein Stornobeleg storniert eine Rechnung und nicht einen Auftrag. Deshalb wird der Stornobeleg nur erzeugt, wenn der Auftrag eine Rechnung hat. +
Optional kannst du einen *Kommentar* einfügen, der auf dem Dokument unterhalb der Artikelpositionen eingefügt wird.

| *Stornobeleg Gutschrift erzeugen*
|Die Aktion erzeugt ein <<auftraege/auftraege-verwalten#540, Stornobeleg Gutschrift>> für einen Auftrag. +
Optional kannst du einen *Kommentar* einfügen, der auf dem Dokument unterhalb der Artikelpositionen eingefügt wird.
...
| *E-Mail versenden*
|Die Aktion sendet eine E-Mail. Klicke auf *Hinzufügen* (icon:plus-square[role="green"]), um die Einstellungen für die E-Mail zu wählen. Wähle aus der ersten Dropdown-Liste die Vorlage aus, also welche Art von E-Mail versendet werden soll. Wähle aus der zweiten Dropdown-Liste die Epfänger:innen aus, also an wen die E-Mail verschickt werden soll. Klicke dann auf *Speichern* (icon:save[role="green"]). Es wird eine ID vergeben und die Auswahl wird angelegt. +
...
| *Lagerort zuweisen*
|Die Aktion weist den ausgewählten Aufträgen ein Lagerort zu. Die Zuordnung erfolgt gemäß deiner <<auftraege/grundeinstellunge#intable-warehouse-selection, Lagerhauseinstellungen>>. Mit der Option *Alten Lagerort lösen* kannst du vorher die bereits zugeordneten Lagerorte lösen.
...
|Die Aktion sendet eine Versandbestätigung an einen Marktplatz oder Zahlungsanbieter, wenn die Ware, die über diesen Marktplatz oder Zahlungsanbieter erworben wurde, versendet wurde. Die Aktionen sind einzeln aufgelistet, wähle die passende mit dem entsprechenden Zahlungsanbieter oder Marktplatz aus. +
...
|Die Aktion sendet eine Gutschrift an einen Zahlungsanbieter. Die Aktionen sind einzeln aufgelistet, wähle die passende mit dem entsprechenden Zahlungsanbieter aus. +
*_Hinweis_*: Es kann vorkommen, dass einzelne dieser Aktionen nicht mehr verwendbar sind, da sie sich ausschließlich auf die alte Integration über Callisto beziehen, z.B. für Klarna. Aus technischen Gründen sind diese Aktionen jedoch weiterhin gelistet. Die Aktionen in der Aktionsgruppe *Plugins* haben Priorität.
...
|Die Aktion sendet eine Retoure an einen Marktplatz oder Zahlungsanbieter. Die Aktionen sind einzeln aufgelistet, wähle die passende mit dem entsprechenden Zahlungsanbieter oder Marktplatz aus. +
*_Hinweis_*: Es kann vorkommen, dass einzelne dieser Aktionen nicht mehr verwendbar sind, da sie sich ausschließlich auf die alte Integration über Callisto beziehen, z.B. für Klarna. Aus technischen Gründen sind diese Aktionen jedoch weiterhin gelistet. Die Aktionen in der Aktionsgruppe *Plugins* haben Priorität.
...
| *Karte erstellen*
|Die Aktion fügt im Menü *plentymarkets Logo (Start) » Boards* eine <<willkommen/allgemeine-funktionen/boards#30, neue Karte>> hinzu. +
...
| *Karten löschen*
|Die Aktion <<willkommen/allgemeine-funktionen/boards#40, löscht>> im Menü *plentymarkets Logo (Start) » Boards* ausgewählte Karten. +
Innerhalb der Aktion wählst du das *Board* und die *Spalte(n)*, deren enthaltene Karten gelöscht werden sollen.

| *Karten verschieben*
|Die Aktion <<willkommen/allgemeine-funktionen/boards#40, verschiebt>> im Menü *plentymarkets Logo (Start) » Boards* ausgewählte Karten. +
...
|Die Aktion sendet eine Retoure an <<maerkte/zalando#1600>>.
