...
. Klicke auf das *Ereignisaktion hinzufügen* (icon:plus-square[role="green"]), um eine neue Ereignisaktion zu erstellen.
...
|Wenn eine Kund:in in der *Auftragsabwicklung* eine neue Information für dich hinterlässt.

| *Erweiterung der Bestellung durch Kunde*
|Wenn eine Kund:in die Bestellung im *Bestellvorgang* erweitert.
...
| *Überzahlung*
|Wenn eine Kund:in zu viel für einen Auftrag gezahlt hat.
...
|Wenn eine geleistete Zahlung von einer Kund:in storniert wurde.
...
. Klicke im Bereich Filter auf das *Filter hinzufügen* (icon:plus-square[role="green"]).
...
Ein *Zahlungsziel* einstellen, *Skonto* hinterlegen sowie einen *Kommentar* einfügen. +
_Achtung_: Die Optionen *Zahlungsziel* und *Skonto* beziehen sich auf die alte Logik. Diese Logik wird zum *01.01.2021* abgeschaltet. Wir empfehlen daher die zeitnahe Umstellung auf die neue Logik für die <<auftraege/auftraege-verwalten#intable-zahlungsbedingungen-auftrag, Zahlungsbedingungen>>.

| *Sammelrechnung erzeugen*
|Automatische Erzeugung einer Sammelrechnung bei Eintreten des eingestellten Ereignisses. +
Ein *Kommentar* kann optional eingegeben und wird dann unterhalb der Artikelpositionen angezeigt.
...
Siehe dazu auch die Handbuchseite der <<payment/zahlungsanbieter#, Zahlungsanbieter>>.
