...
. Erstelle die Nachricht. Beachte dazu das Bild <<bild-messenger-neue-nachricht>> und die Erläuterungen in <<tabelle-neue-nachricht-messenger>>.
...
|Mit einem Klick auf icon:success[set=plenty] icon:toggle-off[role="red"] wird die Nachricht geschlossen. In der Nachrichtenübersicht wird die Nachricht mit einem icon:success[set=plenty, role=green] angezeigt. Dies signalisiert anderen Mitarbeitern, dass diese Nachricht bereits bearbeitet wurde. +
Mit einem Klick auf icon:success[set=plenty] icon:toggle-on[role="green"] wird die geschlossene Nachricht wieder geöffnet.
...
|Mit einem Klick auf icon:pencil[role="yellow-background"] änderst du die Sichtbarkeiten. Du kannst bereits ausgewählte Empfänger entfernen und neue Empfänger hinzufügen. +
