...
Erfahren Sie in den folgenden Unterkapiteln, wie Sie Events zu Ihren Kontakten hinzufügen, anzeigen und löschen. Für kostenpflichtige Events aktivieren Sie die Option *Abrechnen*.

[#2300]
=== Event hinzufügen

Fügen Sie beliebig viele Events hinzu. Ein Event kann z.B. ein Anruf oder E-Mail sein. Sie können außerdem die Dauer eingeben eine Information zum Vorgang wählen und einen Kommentar zum Event eingeben.
...
. Geben Sie die Dauer des Events ein.
...
In der Ansicht *Zugangsdaten* haben Sie die Möglichkeit, das Passwort für Ihren Kontakt manuell zu ändern, das Passwort zurückzusetzen, einen Link zum Ändern des Passworts zu versenden, den Login zu entsperren und den Mein Konto-Bereich des Kontakts über die Login-URL zu öffnen.
...
Damit die Login-URL automatisch erzeugt wird, muss sowohl die E-Mail-Adresse im Menü *CRM » Kontakte » Kontakt öffnen* im Bereich *Kontaktoptionen* als auch das Passwort im Menü *CRM » Kontakte » Kontakt öffnen » Bereich: Passwort* gespeichert sein. Nach dem Speichern wird die Login-URL erzeugt. Verwenden Sie ein sicheres Passwort, andernfalls wird eine Fehlermeldung ausgegeben.
