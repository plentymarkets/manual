...
Nun hast du die Möglichkeit, die einzelnen Komponenten zu bearbeiten (icon:pencil[]) oder zu löschen (icon:trash[set=plenty]). Wenn du die Komponente bearbeitest, siehst du auf der rechten Seite, welche Elemente noch nicht verwendet wurden und dir somit noch zur Verfügung stehen. Bereits verwendete Elemente sind ausgegraut und können nicht ein weiteres Mal verwendet werden. Bestimmte Elemente kannst du zur besseren Bedienbarkeit auch nebeneinander positionieren; dies sind z.B. PLZ und Ort sowie Vorname und Nachname.
...
Du kannst den Bearbeitungsmodus jederzeit mit einem Klick auf *Bearbeitungsmodus schließen* (icon:times[]) verlassen. Wenn du die Änderungen nicht gespeichert hast, erscheint eine Sicherheitsabfrage, dass alle nicht gespeicherten Änderungen verloren gehen.
