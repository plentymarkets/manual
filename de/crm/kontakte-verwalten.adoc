...
Damit für Benutzer mit dem Zugang *Backend* das Menü *CRM » Kontakte* sichtbar ist, muss dieses Menü im Benutzerkonto freigeschaltet werden.

Da die Benutzeroberfläche des Menüs *CRM » Kontakte* über REST abgerufen wird, müssen zusätzlich Berechtigungen aktiviert werden. Im Folgenden werden die Berechtigungen, die mindestens aktiviert werden müssen, damit das Menü sichtbar ist, beschrieben. Aktivieren Sie weitere Berechtigungen analog. Wenn der Benutzer z.B. Firmendatensätze erstellen soll, aktivieren Sie im Bereich *CRM > Firma* die Option *Firma erstellen*.
