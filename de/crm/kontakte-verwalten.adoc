...
Nun hast du die Möglichkeit, die einzelnen Komponenten zu bearbeiten (icon:pencil[]) oder zu löschen (icon:trash[set=plenty]). Wenn du die Komponente bearbeitest, siehst du auf der rechten Seite, welche Elemente in dieser Komponent noch nicht verwendet wurden und dir somit noch zur Verfügung stehen. Bereits verwendete Elemente sind ausgegraut und können nicht ein weiteres Mal verwendet werden.

Bestimmte Elemente kannst du zur besseren Bedienbarkeit auch nebeneinander anzeigen; dies sind z.B. Vorname und Nachname sowie Postleitzahl und Ort. Klicke dazu bei diesen Feldern auf icon:pencil[] und aktiviere dann die Option *Felder nebeneinander anzeigen*. Du kannst auch das Verhältnis, wie die Felder angezeigt werden, bestimmen. Der Standardwert ist 50%/50%. Klicke anschließend auf icon:times[], um den Bearbeitungsmodus für diese Felder zu schließen.
