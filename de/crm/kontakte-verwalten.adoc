...
→ Ein Fenster mit einer Sicherheitsabfrage wird angezeigt.
. Beachten Sie die Erläuterungen in der grünen Box.
. Klicken Sie auf *Anonymisieren*. +
→ Der Datensatz wird anonymisiert.

[TIP]
.Welche Daten werden anonymisiert, gelöscht und beibehalten?
====
Mit einem Klick auf *Anonymisieren* werden die folgenden Daten im Datensatz des Kontakts anonymisiert, gelöscht oder auch beibehalten:

* Daten werden anonymisiert: Vorname, Nachname, Adressdaten, Bankdaten
* Daten werden gelöscht: Kontaktoptionen, Adressoptionen, Verknüpfungen zu einer Firma
* Daten werden beibehalten: Adressdaten, die mit einem Auftrag verknüpft sind. Die Verknüpfung zum Kontakt wird jedoch gelöscht.
====
...
. Öffnen Sie die Ansicht *Dokumente*.
