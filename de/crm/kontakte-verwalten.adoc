...
. Öffne das Menü *CRM » Kontakte*.
. Suche (icon:search[]) den Kontakt anhand der Filtereinstellungen wie im Kapitel <<crm/kontakte-verwalten#200, Kontakt suchen>> beschrieben. +
→ Die Kontakte, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Klicke in die Zeile des Kontakts. +
→ Die Detailansicht des Kontakts wird geöffnet.
...
. Klicke in die Zeile des Kontakts. +
→ Die Detailansicht des Kontakts wird geöffnet.
. Klicke im Bereich *Firma* auf *Bearbeiten* (icon:pencil[]).
...
. Klicke auf *Bearbeiten* (icon:pencil[]), um die Detailansicht der Adresse zu öffnen. +
...
. Klicke in der Adresskarte, die du bearbeiten möchtest, auf *Bearbeiten* (icon:pencil[]). +
...
. Öffne das Menü *CRM » OP-Liste*.
. Klicke im Bereich *OP-Liste generieren* auf *Generieren*. +
