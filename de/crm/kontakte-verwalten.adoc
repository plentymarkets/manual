...
In jedem Datensatz wird die Historie eines Kontakts gespeichert. Die Historie beinhaltet z.B. das Datum, seit dem der Kontakt bei Ihnen registriert ist, wann er das letzte Mal in Ihrem Webshop eingeloggt war und wann der letzte Auftrag generiert wurde. Die Daten werden automatisch angepasst und können nicht geändert werden.
...
Nachdem der Sammelauftrag erfolgreich angelegt wurde, öffnet sich dieser automatisch im Menü *Aufträge*. Im Sammelauftrag generieren Sie dann im Tab *Belege* eine Sammelrechnung.
