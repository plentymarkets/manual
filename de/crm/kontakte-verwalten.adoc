...
Wenn du Nachrichten erstellen möchtest, die über den Messenger kostenpflichtig abgerechnet werden sollen, erstelle nach dem selben Schema Tags für den Bereich *Nachrichten (kostenpflichtig)*.
...
. Suche (icon:search[role="blue"]) den Kontakt anhand der Filtereinstellungen wie im Kapitel <<crm/kontakte-verwalten#200, Kontakt suchen>> beschrieben. +
→ Die Kontakte, die den eingestellten Suchkriterien entsprechen, werden angezeigt.
. Öffne das Kontextmenü *Messenger* mit einem Rechtsklick in die Zeile des Kontakts. +
icon:map-signs[] *_Oder:_* Klicke in die Zeile des Kontakts und im Kontextmenü auf der linken Seite auf *Messenger*. +
→ Der Bereich *Messenger* wird geöffnet.
. Klicke auf *Neue Nachricht* (icon:plus-square[role="green"]).
. Erstelle die Nachricht. Beachte dazu die Erläuterungen in <<tabelle-neue-nachricht-messenger>>.
. Klicke auf *Senden*. +
...
|Flüstern
|icon:eye[] icon:toggle-off[role="red"]
|Wenn *Flüstern* aktiviert ist, ist die Nachricht nur von deinen Mitarbeitern, die die Berechtigung für den Messenger haben, sichtbar. Deine Kontakte können die Nachricht nicht einsehen.
...
|Dateien anhängen
|icon:paperclip[]
|Beliebig viele Dateianhänge zur Nachricht hochladen. + Die folgenden Dateitypen sind als Anhang in der Nachricht zulässig: +
bmp, csv, gif, jpeg, json, pdf, png, tiff, txt, zip.

|Minuten
|
|Minuten in das Feld eingeben, die du für eine bestimmte Aufgabe oder Einheit speichern oder auch später abrechnen möchtest. +
Wenn du die hier eingegebenen Minuten abrechnen möchtest, musst du einen Tag vom Typ *Nachrichten (kostenpflichtig)* wählen und die Schaltfläche icon:circle-thin[]icon:circle-thin[]icon:circle-thin[] icon:toggle-off[role="red"] aktivieren.

|kostenpflichtig
|icon:circle-thin[]icon:circle-thin[]icon:circle-thin[] icon:toggle-off[role="red"]
|Schaltfläche aktivieren, um die im Feld *Minuten* eingegeben Minuten kostenpflichtig abzurechnen. +
*_Wichtig:_* Wenn du die Schaltfläche aktivierst hast, musst du zwingend ein Tag vom Typ *Nachrichten (kostenpflichtig)* mit der Nachricht verknüpfen. +
Dieses Tag muss zuvor im Menü *Einrichtung » Einstellungen » Tags* für den Bereich *Nachrichten (kostenpflichtig)* erstellt worden sein. Andernfalls kann die Nachricht nicht erstellt werden.
...
//Bild einfügen, ggf. mit Nummerierung.
