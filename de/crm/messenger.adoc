...
:keywords: Messenger, massenger, Nachrichten, Nachrichten abrechnen, kostenpflichtige Nachrichten, kostenpflichtig
...
| *Kostenpflichtig*
|Schaltfläche icon:pay_cash[set=plenty] icon:toggle-off[role="red"] aktivieren, um die im Feld *Minuten* eingegeben Minuten kostenpflichtig abzurechnen. +
*_Wichtig:_* Wenn du die Schaltfläche aktivierst hast (icon:pay_cash[set=plenty] icon:toggle-on[role="green"]), musst du zwingend ein <<crm/kontakte-verwalten#1396, Tag>> vom Typ *Nachrichten (kostenpflichtig)* mit der Nachricht verknüpfen. Dieses Tag muss zuvor im Menü *Einrichtung » Einstellungen » Tags* erstellt worden sein. Andernfalls kann die Nachricht nicht erstellt werden.
