...
→ Klicke auf *Neue Nachricht*. Die Stoppuhr oben rechts startet automatisch, sobald der Datensatz geöffnet wird.  Klicke auf *Kostenpflichtig* icon:pay_cash[set=plenty] icon:toggle-off[role="red"], um die Zeit abzurechnen. +
...
Sobald du im Menü *CRM » Schnellsuche* einen Datensatz öffnest, startet die Stoppuhr automatisch oben rechts in der Ecke. Mit einem Klick auf *Pause* (icon:pause-circle-o[]) hältst du die Uhr an. Mit einem weiteren Klick auf *Start* (icon:play-circle[]) läuft die Zeit weiter. Klicke auf *Zurücksetzen* (icon:undo[role="orange"]), um die Zeit auf 0 zu setzen.

////
Die Zeiten der Stoppuhr werden automatisch in den Kontaktdatensatz in den Bereich *Events* übertragen. In diesem Bereich kannst du weitere Einstellungen vornehmen und den Typ wählen sowie einen Infotext eingeben.
