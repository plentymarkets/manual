...
. Klicke auf icon:bookmark-_border[set=material] *Aktuellen Filter speichern*.
...
[TIP]
Lege die Reihenfolge der Filter per Drag-and-drop mit einem Klick auf *Verschieben* (icon:drag_indicator[set=material]) fest. Klicke auf icon:delete[set=matrial], um einen Filter zu löschen.
