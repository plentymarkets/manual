...
Als Ereignis muss hier *Neue Nachricht vom Mitarbeiter* eingestellt werden und als Aktion empfiehlt sich *E-Mail versenden*. Die E-Mail-Vorlage, die hier gewählt wird, muss wiederum die Template-Variable `$TicketLastMessage` enthalten und als Empfänger die Option *Kontakt (CRM)*. Der E-Mail-Betreff sollte außerdem die Ticket-ID enthalten. Nutze dafür die Template-Variable `$TicketID`.
====
