...
Nachdem Sie die Einstellungen gespeichert haben, wird die ID des Tickets im Tab angezeigt. Außerdem erscheint eine Symbolleiste. Beachten Sie die Erläuterungen zur Symbolleiste in Tabelle 3.
...
Im Menü *CRM » Ticketsystem » Tab: Tickets* können Sie oberhalb der Ticketübersicht ein neues Ticket manuell oder aus einer Vorlage erstellen. Sie können dort auch persönliche Vorlagen wählen oder die Gruppenfunktion nutzen. Tabelle 7 erläutert die Einstellungen, die Sie im Tab *Tickets* vornehmen können.
...
. Öffnen Sie das Menü *CRM » Ticketsystem*.
. Wählen Sie im Tab *Tickets* im Bereich *Suchen* die Filter und geben Sie die Daten in die Eingabefelder ein. Beachten Sie dazu die Erläuterungen in Tabelle 8.
...
Im Menü *CRM » Ticketsystem » Ticket öffnen » Tab: Ticket* sehen Sie die Einstellungen Ihres Tickets auf einen Blick. Einige Einstellungen können Sie hier auch ändern. Tabelle 9 erläutert die Einstellungsmöglichkeiten in Ihren Tickets.
...
Beachten Sie die Stichworte, die zu einer Zuordnung führen, und die Schreibweisen in Tabelle 10. Groß- und Kleinbuchstaben spielen dabei keine Rolle. Prüfen Sie dazu auch die <<crm/e-mails-versenden#1200, E-Mail-Vorlagen>>, die Sie im Ticketsystem verwenden, und korrigieren bzw. ergänzen Sie je nach Bedarf Template-Variablen.
...
E-Mails werden bei eingerichtetem E-Mail-Konto in den Einstellungen des Ticketsystems automatisch ins Ticketsystem überführt. Dort werden sie anhand der in Tabelle 10 genannten Parameter wie folgt erstellt:
