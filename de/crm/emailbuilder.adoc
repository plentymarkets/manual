...
Möchtest du wissen, welche Variablen für die einzelnen Kategorien und Unterkategorien zur Verfügung stehen und wie die twig expressions lauten, die du im Code-Widget verwenden kannst? Dann klappe die Bereiche mit einem Klick auf icon:expand_more[set=material] einfach auf.
...
Wenn du im Code-Widget beispielsweise mit link:https://twig.symfony.com/doc/3.x/tags/if.html[if-Bedingungen^] arbeitest, benötigst du natürlich keine doppelt geschweiften Klammern zusätzlich.
...
Im Kapitel <<#code-beispiele-code-widget, Code-Beispiele für das Code-Wigdet>> findest du einige if-Bedingungen, die du einfach in dein Code-Widget kopieren kannst.
...
Setze in deinen E-Mail-Vorlagen Fallunterscheidungen nach den Prinzipien der if-Abfragen ein. Im Folgenden findest du einige if-Bedingungen, die du einfach in dein Code-Widget kopieren kannst.
...
Füge einen Code ein, der bewirkt, dass ein Hinweis bezüglich der Spedition und Kontaktaufnahme in der E-Mail an deine Kund:innen nur angezeigt wird, wenn es sich bei dem Versanddienstleister um eine Spedition (ID `11` ) handelt.

// {# Shipping Provider 11 = Spedition #}


[source,plenty]
----
include::assets/emailbuilder-code-beispiel-versanddienstleister.html[]
...
[source,plenty]
----
include::assets/emailbuilder-code-beispiel-bankdaten.html[]
