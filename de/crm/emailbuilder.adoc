...
Welche Variablen dir für deine E-Mail-Vorlagen zur Verfügung stehen, findest du in den Listen ganz unten auf dieser Seite im Kapitel <<#variablen-emailbuilder, Verfügbare Variablen und Twig-Ausdrücke für das Code-Widget und den Betreff>>.
...
== Verfügbare Variablen und Twig-Ausdrücke für das Code-Widget und den Betreff
...
Möchtest du wissen, welche Variablen für die einzelnen Kategorien und Unterkategorien zur Verfügung stehen und wie die Twig-Ausdrücke lauten, die du im Code-Widget verwenden kannst? Dann klappe die Bereiche mit einem Klick auf icon:expand_more[set=material] einfach auf.

[#twig]
[discrete]
=== Twig-Ausdrücke für das Code-Widget und den Betreff

In den folgenden aufklappbaren Bereichen findest du neben den Variablen auch die Twig-Ausdrücke, die du im Code-Widget und im Betreff einer Vorlage verwenden kannst.


*Twig-Ausdruck im Betreff*

Wenn du einen Twig-Ausdruck im Betreff der Vorlage verwenden möchtest, muss dieser von doppelt geschweiften Klammern umgeben sein.

*_Beispiel:_* Wenn du im Betreff deiner Vorlage den Versanddienstleister anzeigen möchtest, öffnest du den aufklappbaren Bereich *Auftrag / Versand*, suchst nach der Variable *Versanddienstleister* und setzt dann den entsprechenden Twig-Ausdruck wie folgt in den Betreff deiner Vorlage ein:  `&#123;&#8288;&#123;order.formattedShipping.providerName&#125;&#8288;&#125;`.

*Twig-Ausdruck im Code-Widget*

Wenn du im Code-Widget beispielsweise mit link:https://twig.symfony.com/doc/3.x/tags/if.html[if-else-Funktionen^] arbeitest, benötigst du natürlich keine doppelt geschweiften Klammern zusätzlich.

Weitere Informationen zu Twig findest du in der link:https://twig.symfony.com/doc/3.x/[Twig Documentation^]{nbsp}icon:external-link[].

Im Kapitel <<#code-beispiele-code-widget, Code-Beispiele für das Code-Wigdet>> findest du einige häufig genutzte if-else-Funktionen, die du einfach in dein Code-Widget kopieren kannst.
...
| `Wert der Artikel`
|
| `order.formattedGeneral.valueOfItems | number_format(2, '.', ',')`
...
|
| `order.formattedGeneral.invoiceTotalNet | number_format(2, '.', ',')`


|`Gesamtbetrag der Mehrwertsteuer`
|
| `order.formattedGeneral.totalVat | number_format(2, '.', ',')`


| `Gesamtbetrag des Auftrags netto`
|
| `order.formattedGeneral.totalNet | number_format(2, '.', ',')`
...
| `Bezahlter Betrag`
|
| `order.formattedGeneral.paidAmount | number_format(2, '.', ',')`


| `Offener Betrag`
|
| `order.formattedGeneral.openAmount | number_format(2, '.', ',')`


| `Überbezahlter Betrag`
|
| `order.formattedGeneral.overpaidAmount | number_format(2, '.', ',')`
...
| `Gutscheinwert`
|
| `order.formattedGeneral.couponValue | number_format(2, '.', ',')`
...
|
| `orderItem.formattedProperties.all | raw`
...
| `Bruttopreis`
|
| `orderItem.formattedAmounts.grossPrice | number_format(2, '.', ',')`


| `Nettopreis`
|
| `orderItem.formattedAmounts.netPrice | number_format(2, '.', ',')`
...
| `orderItem.variation.item.free1` bis `orderItem.variation.item.free20`
...
|
| `orderItem.formattedAmounts.totalPrice | number_format(2, '.', ',')`


| `Rabatt pro Auftragsposition`
|
| `orderItem.formattedAmounts.discount | number_format(2, '.', ',')`
...
|
| `order.formattedGeneral.shippingCosts | number_format(2, '.', ',')`
...
| `contact.formattedUser.signatureHtml | raw`
...
| `contact.newsletterAllowanceAt ? contact.newsletterAllowanceAt | date('d-m-Y')`
...
| `globals.coupons.couponCode1` bis `globals.coupons.couponCode10`
...
| `newsletter.formattedConfirmUrl | raw`
...
In diesem Kapitel findest du einige häufig genutzte if-else-Funktionen, die du einfach in dein Code-Widget kopieren kannst.

[#code-beispiel-versanddienstleister]
=== Code-Beispiel: Versanddienstleister
