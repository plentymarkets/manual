...
Benutzer:innen vom Typ *Backend* <<business-entscheidungen/benutzerkonten-zugaenge#105, sehen nur bestimmte Bereiche und Menüs>> in plentymarkets. Sie können also nur eingeschränkt im System arbeiten. Sollen Mitarbeiter:innen mit dem Zugang *Backend* mit dem EmailBuilder arbeiten können, benötigen diese Mitarbeiter:innen mindestens das im folgenden beschriebene Recht.
...
. Aktiviere im Bereich *Menü-Sichtbarkeiten* die Option *CRM » EmailBuilder*.
...
Du kannst beliebig viele Ordner und Unterordner erstellen. Nachdem der Ordner erstellt ist, kannst du über das Kontextmenü (icon:more_vert[set=material]) neue Unterordner, neue Vorlagen allgemein und neue Vorlagen für Header und Footer erstellen und die Ordner umbenennen oder löschen.
...
|Klappe den Bereich auf (icon:caret-down[]), um eine der Standardvorlagen mit der neuen Vorlage zu verknüpfen.
...
Beispiel: Ersetze `orderId` durch `{{order.id}}`.
...
Möchtest du wissen, welche Variablen für die einzelnen Kategorien zur Verfügung stehen? Dann klappe die Bereiche mit einem Klick auf icon:expand_more[set=material] einfach auf.
