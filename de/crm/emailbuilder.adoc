...
Möchtest du wissen, welche Variablen für die einzelnen Kategorien und Unterkategorien zur Verfügung stehen und wie die twig expressions lauten, die du im Code-Widget und im Betreff einer Vorlage verwenden kannst? Dann klappe die Bereiche mit einem Klick auf icon:expand_more[set=material] einfach auf.
...
Wenn du im Code-Widget beispielsweise mit link:https://twig.symfony.com/doc/3.x/tags/if.html[if-Abfragen^]{nbsp}icon:external-link[] arbeitest, benötigst du in der if-Abfrage selbst keine doppelt geschweiften Klammern zusätzlich, sondern nur im Fließtext.
...
Anzeigen, wenn divers: leere Option
...
[source,plenty]
----
include::assets/emailbuilder-code-beispiel-auftragspositionen.html[]
