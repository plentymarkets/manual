...
Im Menü *Einstellungen » Aufträge » Ereignisse* kannst du eine <<automatisierung/ereignisaktionen#, Ereignisaktion>> einrichten, die bei Eintritt eines bestimmten Ereignisses in deinem plentyShop eine E-Mail an deine Kund:innen versendet. Füge in der Ereignisaktion die Aktion *Kunde > E-Mail versenden* hinzu und wähle dann dort nacheinander die passende Vorlage sowie eine der folgenden Optionen, wenn die E-Mail an mehrere Empfänger:innen versendet werden soll:
