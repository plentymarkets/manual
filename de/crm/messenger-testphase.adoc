...
. Gib den Betreff ein und wähle Tags und Empfänger:innen.
...
. Beachte die Erläuterungen in <<#table-create-new-message>> einfügen.
...
| *Abrechnen*
|Du kannst eine Nachricht abrechnen. Klicke dazu zunächst auf icon:more_vert[set=material]. Wenn du dann die Schaltfläche *Abrechnen* aktivierst (icon:toggle_on[set=material, role=skyBlue]), wird diese Nachricht abgerechnet. Die Schaltfläche *SENDEN* wird geändert in *ABRECHNEN UND SENDEN*.
...
| *Herkunft*
|Wähle (icon:check-square[role="blue"]) eine oder mehrere Herkünfte, um nach Nachrichten mit dieser Herkunft zu suchen.
...
Du hast die Möglichkeit, die Ansichten von einer vertikalen zu einer horizontalen Ansicht zu wechseln. Klicke dazu auf *xxx* (icon:list[set=material]) ganz oben rechts in der Übersicht.

// welche Ansicht ist standardmäßig gewählt?
