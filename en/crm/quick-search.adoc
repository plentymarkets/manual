...
// The stop watch in the top right corner starts automatically once you open the data record.
Click on *Billable* icon:pay_cash[set=plenty] icon:toggle-off[role="red"] to bill the time. +
*_Note:_* For further information, refer to the <<crm/messenger#, Messenger>> page.
. Click in the line *Events* to display events. +
→ The stop watch in the right corner starts automatically once the data record is opened. Click on *Add event* to save this time. +
