...
. *Alternative 2*\: Click on icon:filter_alt[set=material] to narrow down the search results with filters.
...
The following fields are activated by default: ID, form of address, first name, last name, email private, mobile private, email business and action.
