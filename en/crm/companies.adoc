...
To make sure that you don’t have to select a filter that you use quite often from the list of your <<#apply-saved-filters, saved filters>> every time when opening the *CRM » Companies* menu, you can set a created filter as standard filter. Every time when you open the companies UI, this filter will be automatically applied.

You can set the filter as standard filter directly when creating it as described in <<#save-current-filter, Saving the current filter>> or you can set the filter as standard in the overview afterwards.

Click in the line of the saved filter on icon:star-o[] *Set as standard*. If you want to set another filter as standard, deactivate the currently selected standard filter by clicking on icon:star[] *Do not use as standard*.
