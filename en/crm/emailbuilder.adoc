...
| *Maximum width (px)*
|Here, you can determine the email template’s width to a certain value in px. +
*_Note:_* The default value is 1200 px and corresponds to a width of 100 %. If you determine a value, the template width will be adjusted accordingly.
...
| Use this widget to upload images such as your company logo to show it in your email templates. Click on icon:settings[set=material] to open the settings. You can upload the image, enter the width and height of the image in px or %, enter an alternative text and adjust the styling in CSS format. +
*_Tip:_* If you want to place the image to a certain position, you can adjust the CSS styling in the field *Custom styling*, e.g. `margin-left: auto; margin-right: 0; display: block;` or `float:right;display:block;`. Further information can be found link:https://www.w3schools.com/css/css3_images.asp[here^]{nbsp}icon:external-link[] and link:https://www.caniemail.com/[hier^]{nbsp}icon:external-link[], for example.
...
| `Order type ID`
|ID of the order type, e.g. `1` for order, `4` for credit note and `6` for repair. +
*_Note:_* You can find the complete list of all order types and their IDs on this <<data/internal-IDs#80, page of the manual>>.
| `order.typeId`

| `Order type name`
|Name of the order type, e.g. `Order`, `Credit note` and `Repair`. +
*_Note:_* You can find the complete list of all order types and their IDs on this <<data/internal-IDs#80, page of the manual>>.
...
| `Item type`
|Returns the item type that is saved in the item’s *Global* tab. Possible values: Stocked item, production item, colli, special order item.
