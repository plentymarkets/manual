...
| *Text*
|In this widget, you can enter text and use variables. You can also format the text you entered. To do so, double-click on the word that you would like to format. 
...
The following sub-chapters give you an overview which variables are available and how the twig expressions are called that you can insert in the code widget and in the template’s subject line.
...
Do you want to know which variables are available for the individual categories and sub-categories and how the twig expressions are called that you can use in the code widget in the template’s subject line? Simply expand the areas further below by clicking on icon:expand_more[set=material].
...
If you use link:https://twig.symfony.com/doc/3.x/tags/if.html[if queries^]{nbsp}icon:external-link[] in your code widget, for example, there is no need to insert double curly brackets in the if query itself, but in the body text. General information about twig can be found in the link:https://twig.symfony.com/doc/3.x/[Twig Documentation^]{nbsp}icon:external-link[].
...
Do you want to know which variables are available for the individual categories and sub-categories and how the twig expressions are called that you can use in the code widget? Simply expand the following area by clicking on icon:expand_more[set=material].
...
| `Total invoice amount, separated by a dot`
|*_Example:_* 1234.00
| `order.formattedGeneral.invoiceTotal \| number_format(2, '.', '')`
...
Insert a code that causes a note regarding the freight-forwarding company and contacting to only be displayed in the email to your customers if the shipping service provider in the shipping profile is a freight-forwarding company. In our example, the shipping profile has the ID `6`.

Click on link:https://github.com/plentymarkets/manual/blob/master/en/crm/assets/emailbuilder-code-example-shipping-profile.txt[this link^]{nbsp}icon:external-link[] to see the code example.

//// 
[source,plenty]
----
include::assets/emailbuilder-code-example-shipping-profile.txt[]
...
Click on link:https://github.com/plentymarkets/manual/blob/master/en/crm/assets/emailbuilder-code-example-bank-details.txt[this link^]{nbsp}icon:external-link[] to see the code example.

//// 
[source,plenty]
----
include::assets/emailbuilder-code-example-bank-details.txt[]
...
Insert a code that causes the order items with variation IDs, item names, item quantity and price to be shown in a table.

Click on link:https://github.com/plentymarkets/manual/blob/master/en/crm/assets/emailbuilder-code-example-order-items.txt[this link^]{nbsp}icon:external-link[] to see the code example.

//// 
[source,plenty]
----
include::assets/emailbuilder-code-example-order-items.txt[]
