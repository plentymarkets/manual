...
Do you want to know which variables are available for the individual categories and sub-categories and how the twig expressions are called that you can use in the code widget in the template’s subject line? Simply expand the following area by clicking on icon:expand_more[set=material].
...
If you use link:https://twig.symfony.com/doc/3.x/tags/if.html[if queries^]{nbsp}icon:external-link[] in your code widget, for example, there is no need to insert double curly brackets in the if query itself, but in the body text.

For further information about twig, refer to the link:https://twig.symfony.com/doc/3.x/[Twig Documentation^]{nbsp}icon:external-link[].
