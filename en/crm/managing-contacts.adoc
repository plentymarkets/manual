...
. Activate the option *New address layout* in the area *Basic settings: Delivery note*. +
...
Currently, this setting is only available for delivery notes. This setting will be implemented for further documents.
...
You can create and individually set up as many customer classes as you like. An ID is automatically assigned to customer classes. This ID is displayed to the left of the customer classes' name in the overview. However, customer classes are not ordered by ID. Instead, the order is set using the option *Position*.
...
====
Instead of carrying out the settings in this menu, it is also possible to directly save a global discount for a payment method. An example is a discount in percent for the payment method *Cash in advance*. For further information, refer to the <<order-processing/payment/managing-payment-methods#20, Managing payment methods>> page of the manual. You are not able to completely customise the settings if you save a discount directly for a payment method. It is only possible to completely customise the settings when using the option *Discount based on the method of payment* in the *System » CRM » Customer classes* menu.
====
...
. Expand the *property* section.
. Activate the *show* option.
. Expand the *propertyOption* section.
. Activate the *show* option.
...
*_Important:_* If there is no assignment to a class, the contact is automatically assigned to the class with the lowest position. As the price display in the online store is set for each class, check this option for the class with the position 1 so that the correct price is displayed during automatic assignments.
...
2+|*Assigned tags*
2+|Click into the field in order to show all tags that are available for the area *Contacts* and that you have created beforehand in the *System » Settings » Tags* menu. Alternatively, click into the field and enter the first 3 letters of the tag name. For further information about tags, refer to the <<crm/managing-contacts#150, Creating tags>> chapter.

2+|*Company*

|*Name*
|Enter the name of already <<crm/managing-contacts#220, created>> company in the suggestion list in order to assign this company to the contact. Alternatively, click into the field and enter the first 3 letters of the company name. +
...
|*Value date*
|Enter the time period (starting when the invoice is created) when you wish for an additional time period before the *payment due date* time period begins. Pay attention to the box *Example: terms of payment* below the table. +
...
|Enter the number of days (starting when the invoice is created) in which the contact can pay the pending amounts at a discounted price. Pay attention to the box *Example: terms of payment* below the table. +
...
|Enter the value in percent that the contact may subtract if he or she makes the payment within the time period *Days for early payment discount*. Pay attention to the box *Example: terms of payment* below the table. +
*_Important:_* The early payment discount in the contact details can only be selected for companies, but not for private persons. It is, however, possible to save a value for the payment due date in the invoice. To do so, go to *System » Client » Select client » Locations » Select location » Documents » Invoice*.

|*Country of postcode area sales representative*
|Only visible if the type *Sales representative* was selected. Select a country that the postcode area refers to. The system country is selected by default. +
*_Tip:If a sales representative is working in multiple countries, create one contact data record per country.

|*Postcode area sales representative*
|Only visible if the type *Sales representative* was selected. Enter the sales representative's postcode area. Use commas to separate multiple areas, e.g 33,34. The more numbers per postcode area, the more this area is limited. It is necessary to enter the sales representative's postcode area if you want to use the statistic <<basics/statistics/managing-statistics#20, Revenue by sales representative>> to measure how much revenue the representative generated.
...
If you use the statistic <<basics/statistics/managing-statistics#20, Revenue by sales representative>> to analyse how much revenue the sales representative generated, you need to use the event procedure <<basics/automation/event-procedures#, Assign sales representative to customer>> to automatically save the sales representative in the contact data. +
*_Important:_* The sales representative selected in the contact data record is  not taken into account for the orders. Thus, the order is not taken into account in the statistic *Revenue by sales representative*. In order for the sales representative to be taken into account for an order, they have to be assigned to the order by an event procedure. For further information, refer to the notes about the event procedure <<basics/automation/event-procedures#_Assign sales representative to order>>.
...
[TIP]
.Example: example terms of payment
====
...
→ The contacts that correspond to the search criteria entered are displayed.
. Click in the line of the contact. +
→ The detail view of the contact opens. As an alternative, open the context menu with a right click in order to access the views of the contact such as *New order* or *New note*.
...
|*Search*
|Click on *Search* without selecting any filter settings to display all contacts. Alternatively, limit the search results by using filters and click on *Search*. The contacts found are listed in the overview.
...
|*Telephone*
|Enter the telephone number to search for the contact with this number. The telephone number must be completely entered. It is not possible to search only for parts of the telephone number.
...
→ Alternatively, enter the name of the company in the suggestion list and click on *Save*.
...
If the contact has placed an order in your online store as a guest, this account type is saved as contact option in the detail view of the contact. Guest accounts have the value 1. Delete the contact option with the value 1 in order to convert the guest account into a regular account.
...
.Inform the contact
====
Send your customer an email and inform them about the change. They can then create a password to be able to log into your online store.
====
...
|*Postcode* +
*Town*
|Enter the postcode and town of the contact. This data is used for the invoice address, for example. The order of the options *Postcode* and *Town* is reversed for certain countries, e.g. Germany. +
*Town* is a mandatory field.
...
Within the open contact, you can access all orders of the contact in the *Orders » Edit orders* menu. When switching to the order menu, orders of contacts with this ID are automatically searched for.
...
→ All orders that are saved for the contact are displayed in the *Orders » Edit orders* menu. +
...
For further information, refer to the <<order-processing/orders/multi-orders#, Multi-orders>> page of the manual.
...
. Open the context menu with a right click in the line of the contact.
. Click on *New note*. +
. Alternative: Click in the line of the contact. +
→ The detail view of the contact opens.
...
*_Important:_* This option needs to be activated before pain.008 formats can be retrieved for <<order-processing/payment/managing-bank-data#200, SEPA payments>>.

|*SEPA direct debit mandate granted on*
|Only visible when the option *SEPA direct debit mandate* has been activated. +
The date when the contact agreed upon the SEPA direct debit mandate is displayed. As an alternative, select the date from the calendar.
...
*_Important:_* A migration is planned for the properties that you created in the past in the *System » CRM » Properties* menu. That means that you do not have to transfer these properties to the new *System » Settings » Properties » Configuration* menu. Please keep an eye on the link:https://forum.plentymarkets.com/c/changelog[Changelog^] category of the plentymarkets forum where we inform you about the time when the migration takes place.
...
|*Search*
|Click on *Search* without selecting any filter settings to display all properties. Alternatively, limit the search results by using filters and click on *Search*. The properties found are listed in the overview.
...
. Click on the *Menu* tab.
. Expand the *CRM* section.
. Activate the *Quick search* option.
. Click on the *REST API* tab.
. Expand the *Accounts* section.
...
. The overview of the *orders* opens by default. +
→ Click in the line of the order to directly access the order in the *Orders » Edit orders* menu.
. In the *Addresses* line, click on the arrow on the right side to display further addresses of the contact. +
→ Click in the line of the address to directly access the address and edit it.
. In the *Events* line, click on the arrow on the right side to display the events of the contact.
. In the *Tickets* line, click on the arrow on the right side to display the tickets of the contact. +
→ Click in the line of the ticket to directly access the ticket in the *CRM » Ticket system* menu.
. In the *Notes* line, click on the arrow on the right side to display the notes of the contact. +
...
|*Search*
|Displays the contacts that correspond to the search criteria. At least one filter must be selected.
...
|*Contact data*
|The following parameters can be searched for in this field: *Name*, *address*, *copmany* and *customer number*.
...
|*Email*
|Enter the email address to search for the contact with this email address.
...
→ A dialogue window is displayed. Determine a location on your computer where the ZIP file is saved.
...
You can export a list of all the check data or the data for the current month. For further information about exporting the VAT number check data, refer to the <<basics/data-exchange/export-import/exporting-data#, Exporting data>> page of the manual in the <<basics/data-exchange#, Data exchange>> area.

[#5800]
== Exporting, importing and updating contact data

Export the data of your contacts via the elastic export. Installing the plugins link:https://marketplace.plentymarkets.com/en/plugins/integration/ElasticSync_4750[ElasticSync^] and link:https://marketplace.plentymarkets.com/en/plugins/markets/ElasticExport_4763[Elastic Export^] as well as deploying them in Productive is required. Both plugins are available in the link:https://marketplace.plentymarkets.com/en/[plentyMarketplace^]. The ElasticSync plugin is not needed for the export, but for the import and update of data, but it contains the function FormatDesigner.
