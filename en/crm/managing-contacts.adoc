...
Further information about converting Ceres guest orders into regular contact data records can be found on the <<crm/quick-search#, Quick search>> page in the <<crm/quick-search#convert-guest-account, Converting a guest account into a contact data record>> chapter.
...
*_Important:_* The email template has to be <<crm/sending-emails#1200, created>> in the *Setup » Client* » _Select client_ » *Email » Templates* menu in advance.
...
. Note the explanations given in <<#table-amounts-contact>> and <<#table-account-balance-contact>>.

The revenues of the contact are displayed in detail above the table. Apart from *Balance* also information about invoices and credit notes. Note also the explanations given in <<table-amounts-contact>>.

[TIP]
.Filter settings
====
When you open the area *Account balance* in a contact data record, totals are displayed above the table. _Note_:When you set <<table-filter-account-data, filters>> to search for specific information, the amounts will adapt according to the chosen filters.
====
...
*+* = Total of the contact's open payments. +
...
*0,00* = No open amounts.

|[#intable-account-balance-delta]*Credit note amounts ÷ invoice amounts*
|Ratio of credit notes to invoices. 0 % means that no credit note has been paid out to the customer yet. 100 % means that every amount paid by the customer has been paid back in form of credit notes.

|[#intable-account-balance-invoices]*Invoices*
|Number of invoices in parentheses and total invoice amounts of all orders. +
*Open*: The amount the contact still has to pay. +
...
*Open*: This amount still has to be paid to the contact. +
*Paid*: The amount paid to the contact.

|====

Orders, credit notes, returns etc. for this contact are listed as processes in the table. The table rows *Paid* and *Open* each refer to the individual process. If a payment for an order is listed in the *Open* column, the contact still has to pay the amount. As soon as the contact has paid, the amount is listed under *Paid*. If a payment for a credit note is listed in the *Open* column, you still have to pay this amount to the contact. As soon as you have paid, the amount is listed under *Paid*.
...
|[#intable-account-balance-outstanding]*Open*
...
|[#intable-account-balance-filter-outstanding]*Open*
| Select *ALL*, *Yes* or *No*. +
*Yes* = Only open amounts are displayed. +
*No* = Only paid amounts are displayed. +
*ALL* = open and paid amounts are displayed. +
...
In order that the login URL is automatically created, both the email address in the *Contact options* area of the *CRM » Contacts* » _Open contact_ menu and the password in the *CRM » Contacts* » _Open contact_ » *View:{nbsp}Login details* have to be saved. The login URL is generated after the data has been saved. Make sure to use a secure password. Otherwise an error message is displayed.
...
Create contact-specific statistics in the *Statistics* area. This allows you to analyse data related to the contact and display this data as a statistic. For example, you could create a statistic that displays the contact's total revenue development during a particular time period. You can also edit existing statistics in this area. For further information, refer to the <<business-decisions/business-intelligence/statistics#70, Customer statistics>> chapter.
...
Via the change history you can track which user has changed data of a contact data record at which time in your system. Furthermore, it is displayed if the contact have made changes in the My account area of your online store.  
