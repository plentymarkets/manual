...
:keywords: contact, customer, address, adress, addresses, adresses, address customer, invoice address, delivery address, primary, primary delivery address, primary invoice address, account, accounts, company, companies, company address, account address, VAT number, VAT no., sales representative, guest account, guest order, logged in customer, registered customer, regular account, myView, my view, individual user interface, individually adjustable UI, individual adjustable, events, properties, access data, login details, change password, customer cannot log in, unlock login, bank details, bank data, messages, messenger, customer class, contact class, massenger, anonymise contact data, delete customer, delete data record, delete contact, GDPR, download all data of a customer, address layout, address design, customer type, discount system, give discounts, grant discounts, discount for customer class, contact option, contact options, address options, address option, quick search, search address, account balance, cost center, import contacts, export contacts,
...
Afterwards, you can edit (icon:pencil[]) or delete (icon:trash[set=plenty]) the individual components. When you edit a component, you can see on the right side which elements have not been used yet and are therefore still available. Elements that are already in use are disabled and cannot be used a second time. For better usability, you can place certain elements next to each other; these are for example postcode and town as well as first name and last name.
...
*_Important:_* The email template must have been <<crm/sending-emails#1200, created>> in the *Setup » Client* » _Select client_ » *Email » Templates* menu in advance.
...
. Note <<#table-account-balance-contact>>.
...
. Click on *Account* on the left side in the context menu. +
→ All data that is available for the account of the contact is displayed.
. Carry out the settings in the *Filter* area. Note <<#table-filter-account-data>>.
. Click on *Search* (icon:search[role="blue"]).
