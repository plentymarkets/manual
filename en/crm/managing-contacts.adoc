...
[#350]
=== Old online store:{nbsp}Converting a guest account into a regular account
...
You can delete individual contact data records. All addresses that are assigned to the contact are also deleted if they are not linked to an order.
A contact data record cannot be deleted when it is linked to an order or a ticket.

[IMPORTANT]
.When the contact is linked to an order or a ticket, the data record must be anonymised
====
When the contact is linked to an order or ticket or when the contact has the type *Supplier* and is linked to a reorder or redistribution, it is not possible to delete the data record. +
You can, however, anonymise the data record. How to proceed is described in the <<crm/managing-contacts#1707, Anonymising a data record>> chapter.
====
...
|Click on *Invoice address* (icon:credit_note[set=plenty, role=blue]) to select this option. +
By clicking on icon:add[set=plenty] *Add invoice address type*, the type *Invoice address* is added to the address. The ID does not change. +
...
image::crm/assets/new-message-messenger.png[width=640, height=360]
...
|*Stop watch*
|The stop watch starts counting as soon as the *Messenger* section opens. +
...
In order that the login URL is automatically created, both the email address in the *Contact options* section of the *CRM » Contacts* » _Open contact_ menu and the password in the *CRM » Contacts* » _Open contact_ » *View:{nbsp}Login details* must be saved. The login URL is generated after the data has been saved. Make sure to use a secure password. Otherwise an error message is displayed.
...
====
The <<crm/managing-contacts#350, Old online store:{nbsp}Converting a guest account into a regular account>> chapter describes how to convert Callisto guest orders into regular accounts. It is not possible via the contact data record to convert Ceres guest orders into regular accounts.
