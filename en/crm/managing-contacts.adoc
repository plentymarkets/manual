...
First of all, you have to create the tags in the *Setup » Settings » Tags* menu and activate the area *Contacts* in the Availability area. For further information, refer to the <<welcome/general-functions/tags#, Tags>> page of the manual.
...
If you want that the user to create account data records, for example, go to the *CRM > Accounts* area and select the options *Show* and *Create*.
====
...
|Enter the first name and last name of the contact. These are mandatory fields, if no account is selected.
...
|Enter the name of the <<crm/managing-contacts#220, created account>> in the suggestion list to assign this account to the contact. Or click into the field and enter the first 3 letters of the company name. +
...
|Enter the contact’s first name. This a mandatory field when there is no entry under *Account* and *Last name*.

|[#intable-invoice-address-delivery-address-name3-last-name]*Name 3 (Last name)*
|Enter the contact’s last name. This is a mandatory field when there is no entry under *Account* and *First name*.
...
| *Section*
|Select the setting *Contact* from the drop-down list to make sure that the property can be used in this area and that you can show the contact properties on your documents. +
...
*_Note:_* This property type _cannot_ be shown on your documents. +
*Short text* = Enter a short text. You cannot format this text. +

*Text* = Enter the text you want to display. You can format this text. +
...
| *Name*
|Enter a name to only search for the property with this name. It is also possible to search only for parts of the name.

| *Language*
|Select a language to only display properties for which a name was saved in this language.
...
| *Property type*
|Select the property type to only display properties of this type.
|====
...
| *Name*; +
*Description*
|For multilingualism, enter a name and a description for other languages.

2+|Tab *General* > Area *Group*

| *Group*
|Select the groups to assign one or multiple groups to the property.
...
|Enter the values that should be visible to your customers in the drop-down list. +
...
|The payment status of an order is depicted by colours. Cancelled orders are not highlighted by a colour because no payment status has to be displayed for them. +
...
The *CRM » AR list* menu provides you with an overview of all outstanding receivables. All contacts with outstanding claims are listed. The table includes information about the contact, the number of claims held for each contact as well as the total pending amount for these claims. If a contact has overpaid an outstanding claim, this amount is offset against other outstanding claims. If no further outstanding claims exist, a deficit is displayed in the *Pending amount* column. +
When opening the AR list, a filtered view of the table is displayed. The table is automatically filtered according to the contact types *Customer* and *Guest*. Use the filter <<crm/managing-contacts#intable-ar-list-filter-contact-type, contact type>> to display open claims of further contact types.
...
|The company of the contact with open claims is displayed, if it is set in the contact data set. +
This is a default column.
...
|*Action*
| Different actions are available to you in this column. By clicking on icon:external-link[], you are redirected to the order overview in the *Orders » Edit orders* menu. There, a filtered view is displayed. You directly see the unpaid and partially paid, not cancelled orders of the corresponding customer. +
By clicking on icon:ticket_message[], you are redirected to the *Messenger* area in the contact data set of the customer you selected. +
This is a default column.

|====

The last row of the table displays the total number of accounts receivable as well as the total pending amounts. Summed up is always everything displayed on the current table page. If your AR list consists of more than one page, not all of the accounts receivable and pending amounts of the further pages are summed up in the last row.
...
Also note the <<crm/managing-contacts#intable-ar-list-default-filter-customer-type, default setting>> for this filter.

|*Customer class*
|Use this filter to search for open claims of certain <<crm/managing-contacts#15, customer classes>>. To do so, select one or more customer classes from the drop-down list. Also note the <<crm/managing-contacts#intable-ar-list-default-filter-customer-class, default setting>> for this filter.
...
You can set default settings for filters of the AR list in the *Setup » CRM » Settings* menu. Pay attention to the explanations given in <<table-default-settings-filters-ar-list>>.
...
|Decide which <<crm/managing-contacts#15, customer classes>> should be available for the corresponding filter in the AR list. If you don’t select a customer class in this setting, all customer classes are available for the corresponding filter in the AR list.
