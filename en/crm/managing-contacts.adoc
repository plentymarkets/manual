...
Afterwards, you can edit (icon:pencil[]) or delete (icon:trash[set=plenty]) the individual components. When you edit a component, you can see on the right side which elements have not been used yet in this component and are therefore still available. Elements that are already in use are disabled and cannot be used a second time.

For better usability, you can place certain elements next to each other; for example the first name and last name as well as the postcode and town. To do so, click at these fields on icon:pencil[] and activate the option *Display fields next to each other*. You can also determine the relation how the fields are displayed. The standard value is 50%/50%. Afterwards, click on icon:times[] to close the editing mode for these fields.
...
2+| *Assigned tags*
2+|Click into the field in order to display all tags that are available for the *Contacts* area and that you created in advance in the *Setup » Settings » Tags* menu. +
