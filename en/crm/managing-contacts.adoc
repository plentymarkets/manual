...
|Authorises back end users to see address layouts.

| *CRM > Address layout > Create*
|Authorises back end users to create address layouts.
...
====
In order that persons with the access *Back end* can also create, update and delete address layouts in the *Setup » CRM » Address layout* menu, the rights have to be set in the user account.
====
...
If you want that the user to create account data records, for example, go to the *CRM > Accounts* area and activate the options *Show* and *Create*.
====
...
| *CRM > Address > Show*
|Authorises back end users to see addresses.

| *CRM > Contact > Show*
|Authorises back end users to see contacts. +
...
. Carry out the desired settings. Note also <<#table-edit-contact>> in the *Account* area.
. *Save* (icon:save[role="green"]) the settings. +
...
. Carry out the desired changes. Note also <<#table-edit-contact>> in the *Account* area.
. *Save* (icon:save[role="green"]) the settings. +
...
*_Important:_* Make sure that no countries of delivery were saved in the settings of the payment method. Otherwise, the payment method would be available to all customers in the online shop, which is not desired in this case. +
...
2+|[#intable-edit-contact-account]*Account*
...
*_Important:_* The payment due date in the contact data record can only be selected for accounts, but not for private persons. It is, however, possible to define a payment due date in the <<crm/managing-contacts#intable-customer-class-payment-due-date, terms of payment of a customer class>>. The value that is saved here in the contact data record has priority.
...
*_Important:_* The days for early payment discount in the contact data record can only be selected for accounts, but not for private persons. It is, however, possible to define days for early payment discount in the <<crm/managing-contacts#intable-customer-class-payment-due-date, terms of payment of a customer class>>. The value that is saved here in the contact data record has priority.
...
*_Important:_* The percentage for early payment discount in the contact data record can only be selected for accounts, but not for private persons. It is, however, possible to define a percentage for early payment discount in the <<crm/managing-contacts#intable-customer-class-payment-due-date, terms of payment of a customer class>>. The value that is saved here in the contact data record has priority.
...
[#350]
=== Old online shop:{nbsp}Converting a guest account into a regular account

If the contact ordered as a guest in your old online shop (Callisto online shop) and not in the plentyShop, this account type is saved as contact option in the detail view of the contact. Guest accounts have the value 1. Delete the contact option with the value 1 in order to convert the guest account into a regular account.

Send your customer an email and inform them that the guest account was changed to a regular account. They can then create a password to be able to log into your online shop.
...
When creating the contact data record, the history of a contact is saved. The history includes e.g. how long the contact has been registered and when they have been logged into your online shop for the last time and when the last order was generated. This data is automatically updated and cannot be changed.
...
*Guest account* = The contact has registered and placed and order via your Callisto online shop.
...
|[#intable-invoice-address-delivery-address-name1-account]*Name 1 (Account)*
...
The following sub-chapters describe how to <<crm/managing-contacts#960, create>>, <<crm/managing-contacts#970, link>>, <<#search-property, search>>, <<#edit-property, edit>> and <<#delete-property, delete>> properties and how to <<#group-properties, group>>, <<#import-properties, import>> and <<#export-properties, export>> them.

[#960]
=== Creating properties

Go to *Setup » Settings » Properties » Configuration* and create properties. Afterwards, link them with the contact.
...
. In the drop-down list *Area*, select the option *Contact*.
...
| *Section*
|Select the setting *Contact* from the drop-down list to make sure that the property can be used in this area and shown on your documents. +
Additionally, the areas <<item/settings/properties#500, Item>> and <<stock-management/setting-up-a-warehouse#850, Warehouse>> are available.
...
*_Note:_* This property type can be shown on your documents. +
...
*_Note:_* This property type can be shown on your documents. +
*Short text* = Enter a short text. You can enter a text without formatting. +

*Text* = Enter a text. You can enter text with formatting. +
...
For further information about property groups, refer to the <<#group-properties, Grouping properties>> chapter.
...
Go to *Setup » Client » [Select client] » Locations » [Select location] » Documents » [Select document]* to create the template. In the document’s *Customer properties* area, you can see all properties that are available for the *Contacts* area.
...
→  The order opens in the *Orders » Edit orders* menu.
...
| *Section*
|Select the *Contact* area to only display properties that are used in this area.
...
. Go to *Setup » Settings » Properties » Configuration*.
. Click on *Search* (icon:search[role="blue"]) to display all properties or select filters to limit the search results. Note <<#table-filter-properties>>.
...
| *Required*
|Activate this option to make sure that the property is a mandatory input. Mandatory fields are displayed in bold in the detail view of the contact in the *Properties* area and in the order process in the online shop.

| *Order process*
|Activate this option to make sure that the property is displayed in the order process in the online shop.

| *Customer registration*
|Activate this option to make sure that the property is displayed in the customer registration in the online shop.
...
|(not relevant for the *Contact* area)
...
*_Note:_* Once you’ve created the property group, you can save the name in further languages.

| *Description*
|Enter a description of the property group in your plentymarkets system’s default language. +
*_Note:_* The description of the system language is displayed by default. Once you’ve created the property group, you can save the description in further languages.
...
*_Note:_* When you delete a property group, the properties that are assigned to the group are _not_ deleted.
...
Use the ElasticSync and the sync type *Properties* to import properties into your plentymarkets system. The fields that you can use for mapping and matching are described on the page of the manual of the sync type <<data/importing-data/sync-types/elasticSync-properties#, Properties>>.

For general information about importing data into your plentymarkets system, refer to the <<data/importing-data/ElasticSync#, ElasticSync>> page of the manual.
...
Within the open contact, you can create a new order for the contact with only one click. If the contact has ordered in your online shop, the order is automatically saved in your plentymarkets system. In case of orders via telephone, for example, you create the order manually. Proceed as described below.
...
The revenues of the contact are displayed in detail above the table. Apart from *Balance* also information about invoices and credit notes. Note <<#table-amounts-contact>>.

[TIP]
.Filter settings
====
When you open the area *Account balance* in a contact data record, totals are displayed above the table. +
_Note:_ When you set <<#table-filter-account-data, filters>> to search for specific information, the amounts will adapt according to the chosen filters.
====
...
|Number of invoices in parentheses and total invoice amounts of all orders. +
...
|[#intable-account-balance-document]*Document*
...
|[#intable-account-balance-paid]*Paid*
|Amount already paid.
...
You can use various filters to retrieve specific information about your contact's account balance. To do this, set the required filters in the *Account balance* area of the contact and then click on *Search* (icon:search[role="blue"]). The filter settings are explained in the table below. +
_Note_ that the amounts above the table change when you search according to the selected filters. For example, when you select a certain time range or when you want to have processes in a certain order status displayed.
...
|[#intable-account-balance-filter-period]*Period*
|You can set a time range, for example to have only the data of the current month displayed. Select one of the following settings from the drop-down list to set a time range: *ALL*, *This year*, *Last year*, *This month*, *Last month* or *Last 3 months*.

|[#intable-account-balance-filter-outstanding]*Outstanding*
...
|[#intable-account-balance-filter-relational-operator-status]*Relational operator / Status*
|Select a relational operator in combination with an order status from the dropdown-list to narrow down processes to those in a certain status.

|[#intable-account-balance-filter-invoice-number]*Invoice no.*
|Search for a specific process by entering an invoice number.
...
The *CRM » AR list* menu provides you with an overview of all contacts with unpaid orders. Here, AR stands for accounts receivable. The list includes the number of claims held for each contact as well as the total pending amount for these claims.
...
It is possible to open the contact data record from this list. Simply click on the ID in the *ID* column. Or open a list of all orders that were placed by the contact and that have not been paid yet. Simply click on *Show orders* in the *Orders* column.
...
Import or update the data of your contacts including address options, contact options and account data via the import tool. For further information, refer to the <<data/importing-data/ElasticSync#, Using the import tool>> page.
