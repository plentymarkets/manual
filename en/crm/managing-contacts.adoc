...
====
Instead of configuring the settings in this menu, it is also possible to directly save a global discount for a payment method. An example is a discount in percent for the payment method *Cash in advance*. For further information, refer to the <<payment/managing-payment-methods#20, Managing payment methods>> page of the manual. You are not able to completely customise the settings if you save a discount directly for a payment method. It is only possible to completely customise the settings when using the option *Discount based on the method of payment* in the *System » CRM » Customer classes* menu.
====
...
For further information, refer to the <<orders/multi-orders#, Multi-orders>> page of the manual.
...
*_Important:_* This option needs to be activated before pain.008 formats can be retrieved for <<payment/managing-bank-data#200, SEPA payments>>.
