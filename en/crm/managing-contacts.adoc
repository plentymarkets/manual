...
====
If you click on a contact in order to display or edit the contact, the detail view of the contact opens. Via the context menu in the left section of the contact, you can access views such as *Addresses* or *New address*.

By clicking on *Addresses* for example, all addresses that are saved for the contact are initially displayed as cards. By clicking on *Switch to table view*, the addresses are displayed in a table. From this view, you can switch back by clicking on *Switch to card view*. Your plentymarkets system recognises which of the 2 views you selected. When opening the context menu *Addresses* the next time, the view that was selected last is displayed.
...
Go to the *System » CRM* menu to carry out preparatory settings such as creating <<crm/managing-contacts#15, customer classes>> and <<crm/managing-contacts#20, types>>. Furthermore, you use this menu to set up <<crm/managing-contacts#25, discounts>> for your customers.
...
[IMPORTANT]
.Contact options: Sub-type *Private* for types *Email* and *Telephone*
====
Make sure that you select the sub-type *Private* for the types *Email* and *Telephone* when creating the contact options. By doing so, this data is automatically saved to the order.
====
...
|*Class*
|Select the class. Classes serve as an internal distinction between customers and determine which discount is granted this class, for example. Go to <<crm/managing-contacts#15, System » CRM » Customer classes>> to create further classes. If you want to offer different prices to end customers and sellers in the online store, this can be done with the *Display of the prices in the online store* option in the configuration of the classes. It is very common that gross prices are displayed for end customers (B2C) in the online store and net prices for sellers (B2B). +
Empty option = No assignment to a class +
*_Important:_* If there is no assignment to a class, the contact is automatically assigned to the class with the lowest position. As the price display in the online store is configured for each class, check this option for the class with the position 1 so that the correct price is displayed during automatic assignments.
...
Mobile work +
Private +
*_Important:_* In order that the telephone number is automatically saved to the order, the sub-type *Private* must be selected for the type *Telephone*.

|Email
|Work +
Private +
PayPal +
*_Important:_* In order that the email address is automatically saved to the order, the sub-type *Private* must be selected for the type *Email*.
...
→ The contacts that correspond to the search criteria entered are displayed.
. Click in the line of the contact. +
→ The detail view of the contact opens. As an alternative, open the context menu by clicking right in order to access the views of the contact such as *New order* or *New note*.
...
|*Owner*
|Select an owner to search for the contacts that this user is responsible for. The owner must be saved within the contact data.
...
|*Type*
|Select the type from the drop-down list. +
Available types: VAT number, External address ID, Entry certificate, Telephone, Email, Post number, Personal ID, BBFC, Birthday, Title, Contact person.
...
Use the quick search to get an overview of all information that is saved for a contact. Besides the invoice address, the master data and general information such as the customer type, pending amounts and the data of the last order, also orders, further addresses, notes and tickets are displayed.
...
. Click in the line of the contact. +
→ The detail view of the contact opens.
. Click on *Commission & Cost units* in the context menu.
...
. Click in the line of the contact. +
→ The detail view of the contact opens.
. Click on *Statistics* in the context menu.
. Click on *New sheet*.
. Enter a name.
. *Save* the settings. +
...
*_Important:_* A migration is planned for the properties that you created in the past in the *System » CRM » Properties* menu. That means that you do not have to transfer these properties to the new *System » Settings » Properties » Configuration* menu. Please keep an eye on the link:https://forum.plentymarkets.com/c/changelog[Changelog^] category of the plentymarkets forum where we will inform you about the time when the migration takes place.
...
The accounts receivable list is automatically updated once per day. As such, claims may not be displayed in the list until the next day. However, it is also possible to <<crm/managing-contacts#3800, update the list manually>>. Proceed as described below to display the entire accounts receivable list.
