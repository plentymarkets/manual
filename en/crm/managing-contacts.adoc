...
:keywords: Contact, customer, address, adress, addresses, adresses, customer address, invoice address, delivery address, primary, primary delivery address, primary invoice address, company, companies, company address, VAT number, VAT no, sales representative, guest account, guest order, registered customer, logged in customer, regular account, events, properties, login details, change password, customer cannot log in, unblock login, bank details, login details, message, messenger, customer class, customer, contact class, massenger, anonymize customer, anonymize, anonymise contact, GDPR, download all data of customer, address layout, address design, customer type, contact type, discount system, grant discount, give discount, discount for customer class, contact option, contact options, address option, address options, quick search, search addresses, account balance, provision, sales representative, cost center, cost centre, import contacts, import customer data, export customer data, import contact
...
You can create and individually configure as many customer classes as you like. An ID is automatically assigned to customer classes. This ID is displayed to the left of the customer classes' name in the overview. However, customer classes are not ordered by ID. Instead, the order is set using the option *Position*.

If a new customer class is created, the customer class is available in the detail view of the contact in the *CRM » Contacts* » _Open contact_ menu.
...
. Activate the customer classes in the *Class* section for which the sales price should be valid.
. *Save* (icon:save[role="green"]) the settings.
...
Only those clients are displayed that have a name entered for them in the *Setup » Client* » _Select client_ » *Settings* menu.
...
|*Owner*
|Select the owner for the contact. If you do not want to select an owner, select the empty option. Only owners for whom the *Customer* option has been activated can be selected from the drop-down list. Go to *Setup » Settings » User » Account* » _Open user_ » *Tab: Owner* to activate owners.
...
|*Allow invoice* / +
*Allow debit*
|Allow the payment methods invoice and/or debit for the contact, even if this method is not used globally. +
Place check mark = The contact can pay using this payment method, even if you do not offer this payment method globally. +
...
*_Important:_* The valuta in the contact details can only be selected for companies, but not for private persons. It is, however, possible to save a valuta value in the invoice. To do so, go to *Setup » Client* » _Select client_ » *Locations* » _Select location_ » *Documents » Invoice*.
...
*_Important:_* The payment due date in the contact details can only be selected for companies, but not for private persons. It is, however, possible to save a value for the payment due date in the invoice. To do so, go to *Setup » Client* » _Select client_ » *Locations* » _Select location_ » *Documents » Invoice*.
...
*_Important:_* The early payment discount in the contact details can only be selected for companies, but not for private persons. It is, however, possible to save a discount value in the invoice. To do so, go to *Setup » Client* » _Select client_ » *Locations* » _Select location_ » *Documents » Invoice*.
...
Mobile work +
Private +
*_Important:_* In order that the telephone number is automatically transferred in the order, the sub-type *Private* has to be selected for the type *Telephone*.

|Email
|Work +
Private +
PayPal +
*_Important:_* In order that the email address is automatically transferred in the order, the sub-type *Private* has to be selected for the type *Email*.
...
*_Note:_* Click on the arrow in the columns *First name* and *Last name* in the overview to show the contact data records found in alphabetical order.
...
*_Important:_* Back end users can only select those clients that they have a right to access. Go to *Setup » Settings » User » Accounts* and set the rights in the user account in the <<basics/admin-tasks/manage-users#90, Rights>> tab.
...
. Click in the line of the contact. +
→ The detail view of the contact opens.
. In the *Company* section, click on *Edit* (icon:pencil[role="yellow"]).
...
*_Important:_* The email template must have been <<crm/sending-emails#1200, created>> in the *Setup » Client* » _Select client_ » *Email » Templates* menu beforehand.
...
====
After you saved the primary invoice address and the primary delivery address, these addresses are directly displayed when opening the contact data record. You can find them on the right-hand side in the *Addresses* section. Click on *Edit* (icon:pencil[role="yellow"]) to open the detail view of the address and to edit the address. +
...
*_Note:_* If the invoice address has been defined as primary, this can be undone by clicking on *Reset primary invoice address*.
...
====
If the address is of the type *Invoice address* and if it is linked to an order, you cannot delete the address. An error message appears.
====
...
For further information, refer to the <<orders/scheduler#, Scheduler>> page of the manual. The filter options are also described there. You can use the filter options both in the open contact data record in the *Scheduler* section and in the *Orders » Scheduler » Scheduled orders* menu.
...
In the following sub-chapters, you learn how to create new notes for your contact, display all notes that are available for your contact as well as edit and delete existing notes.
...
How to create tags is described on the <<basics/working-with-plentymarkets/tags#, Tags>> page of the manual.
...
. Create the message. Pay attention to <<image-messenger-new-message>> and to the information given in <<table-new-message-messenger>>.
. Click on *Send* (icon:envelope-o[role="purple"]). +
→ The message is created.

[[image-messenger-new-message]]
.New message in the messenger
image::crm/assets/messenger-new-message.png[width=640, height=360]
...
|*Attach files*
|Click on icon:paperclip[] to upload as many file attachments to the message as you like. + The following file types can be sent as an attachment: +
...
From the open contact, you can display all messages iin the messenger that are saved for the contact.
...
You can edit messages at any time. Proceed as described below.
...
. Open the context menu with a right click in the line of the contact and select *Messenger*. +
icon:map-signs[] *_Or:_* Click in the line of the contact and in the contact data record on the left side on *Messenger*. +
→ The *Messenger* section opens and the saved messages are displayed.
. Click on the message that you want to edit.
. Carry out the changes. Pay attention to <<image-messenger-edit-message>> and to the information given in <<table-messenger-edit-message>>.



[[image-messenger-edit-message]]
.Editing a message
image::crm/assets/messenger-edit-message.png[width=640, height=360]
...
|Click on icon:success[set=plenty] icon:toggle-off[role="red"] to close the message. In the message overview, the message is displayed with a icon:success[set=plenty, role=green]. This tells other employees that this message has already been completed. +
Click on icon:success[set=plenty] icon:toggle-on[role="green"] to open the closed message again.

|Contact ID
|Click on icon:user_my_account[set=plenty]+contact ID to open the detail view of the contact.

|*Change visibility*
|Click on icon:pencil[role="yellow-background"] to change the visibilities. You can remove recipients and add new recipients. +
...
icon:map-signs[] *_Or:_* Add the documents to the tab with the drag and drop function.
...
.Settings in the *Account* section of the contact data record
[cols="1,3"]
|====
|Setting |Explanation
...
.Filter settings in the *Account* section of the contact data record
[cols="1,3"]
|====
|Setting |Explanation
...
.Settings in the *Bank details* section of the contact data record
[cols="1,3"]
|====
|Setting |Explanation
...
In order that the login URL is automatically created, both the email address in the *Contact options* section of the *CRM » Contacts* » _Open contact_ menu and the password in the *CRM » Contacts* » _Open contact_ » *View: Access data* have to be saved. The login URL is generated after the data has been saved. Make sure to use a secure password. Otherwise an error message is displayed.
...
→ The detail view of the contact opens.
. Click on *Login details* on the left side in the context menu.
. Click on *Open link* in the *Login URL* section or copy the login URL to the clipboard by clicking on *Copy to clipboard* (icon:clipboard[role="yellow"]) and insert the link into the browser. +
...
Create customer-specific statistics in the *Statistics* section. This allows you to analyse data related to the contact and display this data as a statistic. For example, you could create a statistic that displays the contact's total revenue development during a particular time period. You can also edit existing statistics in this section. For further information, refer to the <<basics/working-with-plentymarkets/statistics/managing-statistics#70, Customer statistics>> chapter.
...
//There are 2 possibilities for using the properties later: On the one hand, you can ask your customers to provide the necessary information during the order process or customer registration. On the other hand, you can use the properties to enter information about the contacts in the contact data record.
...
Use the ElasticSync and the sync type *Properties* to import properties into your plentymarkets system. Which fields you can use for mapping and matching is described on the page of the manual of the sync type <<data/importing-data/sync-types/elasticSync-properties#, Properties>>.

Check the <</data/importing-data/ElasticSync#, Importing data with ElasticSync>> page of the manual for general information about ElasticSync.
...
General information can be found on the page of the manual <<data/exporting-data/FormatDesigner#, FormatDesigner>>.
...
→ The overview with the sections *Assigned tags*, *Invoice address*, *Master data* and *Info* appears.
...
Further information can be found in the chapter <<crm/managing-contacts#1706, Downloading all data of a contact>>.
...
|icon:user-secret[]
|Anonymises the data record. Anonymising data is only possible when the data record is a "real" contact and not a guest account. +
For further information, refer to the <<crm/managing-contacts#1707, Anonymising a data record>> chapter.

|*Convert guest into contact*
|icon:exchange[role="yellow"]
|Converts the guest account into a regular contact data record. In the *History* section of the newly created contact data record, the entry *Converted by* as well as the user name are added. +
