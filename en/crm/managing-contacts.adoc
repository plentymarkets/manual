...
The multi-order function makes it possible for you to take all of the unpaid orders that belong to a contact and then compile them into one multi-order. This might be useful if you have agreed with your contact that they do not pay each invoice individually, but pay all invoices per month or per quarter, for example.
...
In order that the login URL is automatically created, both the email address in the *Contact options* section of the _CRM » Contacts_ » *Open contact* menu and the password in the *CRM » Contacts* » _Open contact_ » *View:{nbsp}Login details* must be saved.  The login URL is generated after the data has been saved. Make sure to use a secure password. Otherwise an error message is displayed.
