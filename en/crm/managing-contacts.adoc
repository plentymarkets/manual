...
Go to the *System » CRM* menu to carry out the preparatory settings such as creating <<crm/managing-contacts#15, customer classes>> and <<crm/managing-contacts#20, types>>. Furthermore, you set up <<crm/managing-contacts#25, discounts>> for your customers.

[#12]
=== Old online store: Creating contact data records in case of guest orders

Go to *System » CRM » Settings* and select from the drop-down list *Guest orders* whether contact data records are created or not created for guest orders that are placed via your old online store (Callisto online store).
...
. *Save* the settings. +
→ The *System » CRM » Address layout* menu is visible for the variable user.

[TIP]
.Variable users: REST API rights to create, delete and update address layouts
====
In order that variable users can create, update and delete address layouts in the *System » CRM » Address layout* menu, the right must be set in the user account in the *Rights » REST API* area.
====

[.instruction]
Setting up the address layout for delivery notes:

. Go to *System » CRM » Address layout*.
. Click on *New*.
...
In order that the address layouts are automatically displayed on delivery notes, you have to carry out a setting in the delivery note. Your plentymarkets system automatically checks the country of delivery saved in the order and displays the delivery note in the layout listed in the *System » CRM » Address layout* menu. Proceed as described below.

. Go to *System » Client » Select client » Locations » Select location » Documents » Delivery note*.
. Activate the option *New address layout* in the area *Basic settings:{nbsp}Delivery note*. +
→ The option above *Structure of the invoice/delivery address* is hidden and the delivery notes are displayed in the address layouts listed in the *System » CRM » Address layout* menu.
. *Save* the settings.
...
. Go to *System » CRM » Customer classes*.
...
====
Instead of configuring the settings in this menu, it is also possible to directly save a global discount for a payment method. An example is a discount in percent for the payment method *Cash in advance*. For further information, refer to the <<payment/managing-payment-methods#, Managing payment methods>> page of the manual. You are not able to completely customise the settings if you save a discount directly for a payment method. It is only possible to completely customise the settings when using the option *Discount based on the method of payment* in the *System » CRM » Customer classes* menu.
====
...
|*Type*
|Select the type. The *Customer*, *Sales representative*, *Supplier*, *Partners*, *Manufacturers* and *Interested party* types can be selected by default. Create further types in the <<crm/managing-contacts#20, System » CRM » Types>> menu. +
...
2+|*Assigned tags*
2+|Click into the field in order to show all tags that are available for the area *Contacts* and that you have created beforehand in the *System » Settings » Tags* menu. +
...
*_Important:_* The payment due date in the contact details can only be selected for companies, but not for private persons. It is, however, possible to save a value for the payment due date in the invoice. To do so, go to *System » Client » Select client » Locations » Select location » Documents » Invoice*.
...
First of all, you have to create the tags in the *System » Settings » Tags* menu and activate the option *Contacts* in the *Availability* section. How to create tags is described on the <<basics/working-with-plentymarkets/tags#, Tags>> page of the manual.
...
*_Important:_* Variable users can only select those clients that they have a right to access. Variable users are given rights within the <<basics/admin-tasks/manage-users#90, Right>> tab in the *System » Settings » User » Accounts* menu.
...
====
Go to *System » Settings » Data » Cleansing* and determine the time period after which unused addresses are automatically deleted from your plentymarkets system. Unused addresses are those addresses that do not have any relation to a contact, an order, a POS and a warehouse.
...
*_Important:_* A migration is planned for the properties that you created in the past in the *System » CRM » Properties (deprecated)* menu. That means that you do not have to transfer these properties to the new *System » Settings » Properties » Configuration* menu. Please keep an eye on the link:https://forum.plentymarkets.com/c/changelog[Changelog^] category of the plentymarkets forum where we will inform you about the time when the migration takes place.
...
Properties are centrally managed in the *System » Settings » Properties » Configuration* menu. Change the settings that are described in <<table-create-property>> in this menu.
