...
====
If you click on a contact in order to display or edit it, the detail view of the contact opens. Via the context menu in the left section of the contact, you can access views such as *Addresses* or *New address*.
...
====
Export the data of your contacts via the elastic export. Installing the plugins link:https://marketplace.plentymarkets.com/plugins/integration/ElasticSync_4750[ElasticSync^] and link:https://marketplace.plentymarkets.com/plugins/markets/ElasticExport_4763[Elastic Export^] as well as deploying them in Productive is required. Both plugins are available in the link:https://marketplace.plentymarkets.com/[plentyMarketplace^].
...
|*Class*
|Select the class. Classes serve as an internal distinction between customers and determine which discount is granted this class, for example. Go to <<crm/managing-customer-data#100, System » CRM » Customer classes>> to create further classes. If you want to offer different prices to end customers and sellers in the online store, this can be done with the *Display of the prices in the online store* option in the configuration of the classes. It is very common that gross prices are displayed for end customers (B2C) in the online store and net prices for sellers (B2B). +
...
|*Sub-type*
|Select the sub-type, e.g. business or private, from the drop-down list.
...
If you have created the contact without linking it to a company, you can link them afterwards as well. In order to avoid confusion with companies having the same name, the town, street and country of the company are also displayed next to the company name.

Enter at least three characters of the company name, town, street or country. Your plentymarkets system suggests company data records that have already been saved.
...
→ Once you have entered three characters, company data records that have already been saved are suggested.
...
You can edit company data that has already been saved at any time. Note that the data of the already existing company data record is overwritten.
...
You can delete the link between the company and the contact in the detail view of the contact. Note that you only delete the link and not the company data itself. The company data is still displayed as a suggestion after you have entered at least three characters of the company name, the town, the street or the country.
...
Each data record contains the history of a contact. The history includes e.g. how long the contact has been registered and when he or she has been logged into your online store for the last time and when the last order was generated. This data is automatically updated and cannot be changed.
...
The new address can either be the invoice address only, the delivery address only or both the invoice address and the delivery address. If both options are activated, an address with a new ID and two address types is created. +
...
After you have successfully created the multi-order, it opens automatically in the *Orders* menu. Open the *Receipts* tab in the multi-order to create a multi-invoice.
...
You can open individual notes by clicking on *Open preview*. If more than one note of the contact is open, a triangle is displayed in the view. Click on the triangle to quickly and easily switch between the open notes. Click on the cross to close the note and remove it from the list.
...
. Expand the *Address* area.
. Activate the *Show address* option.
. Expand the *Address option* area.
. Expand the *Type of address option* area.
. Activate the *Show type of address option* option.
. Expand the *Contact* area.
. Expand the *Contact address* area.
...
. Expand the *Address type* area.
. Activate the *Show address type* option.
. Expand the *Contact option* area.
. Expand the *Type of contact option* area.
. Activate the *Show type of contact option* option.
. Activate the *Show contact* option.
. Expand the *Comments* area.
...
→ The contacts that correspond to the search criteria entered are displayed.
. Click in the line of the contact. +
→ The overview of the contact with the areas *Invoice address*, *Master data* and *Info* is displayed.
...
The *Account* view lists the revenues of your contacts. In the top line, you see the total of the contact's invoices, the amount granted to your customer in form of credit notes and the difference between the two amounts. Orders and credit notes are listed as processes in the table. The *Paid* and *Outstanding* columns are especially helpful. If a payment for an order is listed in the *Outstanding* column, the contact still has to pay the amount. As soon as this is done, the amount is listed under *Paid*. If a payment for a credit note is listed in the *Outstanding* column, you still have to pay this amount to the contact. As soon as this is done, the amount is listed under *Paid*.
...
In order that the login URL is created, both the email address in the *Contact options* area of the *CRM » Contacts » Open contact* menu and the password in the *CRM » Contacts » Open contact » View: Access data* must be saved. The login URL is generated after the data has been saved. Make sure to use a secure password. Otherwise an error message is displayed.
...
====
Note that contact properties in the *CRM » Contacts » Open contact » View: Properties* menu are only available in a short while. The properties can then be added and displayed in the plentymarkets back end only. That means that it is not possible for the contact to save data in your online store, e.g. during the order process or when he or she registers in your online store. This functionality is currently being developed.

You can, however, already create properties in the *System » Settings » Properties* menu and also add values for the properties.

*_Important:_* A migration is planned for the properties that you created in the past in the *System » CRM » Properties* menu. That means that you do not have to transfer these properties to the new *System » Settings » Properties* menu. Please keep an eye on the link:https://forum.plentymarkets.com/c/changelog[Changelog^] category of the plentymarkets forum where we will inform you about the time when the migration takes place.
...
|Activate this option to make sure that the property is a mandatory entry. Mandatory fields are displayed in bold in the detail view of the contact in the *Properties* area and in the order process. +
...
Properties are centrally managed in the *System » Settings » Properties* menu. After opening the menu, start the search to display the properties. In order to search for specific properties, enter a search term in the search fields. The drop-down lists help you search for properties that share specific criteria. You can combine as many search filters as you like.
...
*Flat* = The surcharge is entered as percentage.
|====
