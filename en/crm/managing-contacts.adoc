...
The revenues of the contact are displayed in detail above the table. Apart from *Balance* also information about invoices and credit notes. Note <<#table-amounts-contact>>. You can also go directly to the <<crm/managing-contacts#5200, AR list>> by clicking on *Open AR list* (icon: open_external_link[]).
...
. Select which columns are to be displayed. Take <<table-account-balance-contact>> into account.
...
|The payment status of a process is depicted by colours. +
...
|[#intable-account-balance-order-type]*Order type*
...
|Date on which payment is due for the order To have a payment due date displayed here, the <<orders/managing-orders#intable-payment-terms-order, payment terms>> of the corresponding order have to be filled in. +
This is a default column.
...
|[#intable-account-balance-filter-status]*Status from* and *Status to*
...
|Search for all processes in a specific payment status. For example, you can search for all orders which are only partially paid. The payment statuses unpaid, partly paid, paid and overpaid are available.
