...
|**Group forum**
|Select a user group for the forum. User groups are created in the **<<omni-channel/online-store/setting-up-clients/modules#forum, Settings » Client (store) » Standard » Modules » Forum  >>** menu.
...
|**VAT number**
|Enter the customer's VAT number. The VAT number needs to be specified if you are sending goods to a commercial purchaser in another country within the EU (intra-community delivery) and you want net totals to be calculated for the order. After having entered the VAT number, go to <<order-processing/orders/_accounting#, **Settings » Client (store) » Standard » Locations » Open location » Accounting**  >> and select **Net invoice** for the option **VAT number**. This causes invoices to be issued without VAT for customers who have a VAT number saved in the customer data. +
