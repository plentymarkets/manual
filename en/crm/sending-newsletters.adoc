...
Go to *Setup » Client* » _Select client_ » *Email » Newsletter*. Enter the login details to use the newsletter service that is integrated in plentymarkets.

[.instruction]
Entering newsletter login details:

. Go to *Setup » Client* » _Select client_ » *Email » Newsletter*.
. Carry out the settings. Pay attention to the information given in <<#table-newsletter-login-details>>.
. *Save* (icon:save[role="green"]) the settings.

[[table-newsletter-login-details]]
.Settings in the *Setup » Client* » _Select client_ » *Email » Newsletter* menu
[cols="1,3"]
|====
|Setting |Explanation
...
.Settings in the *Setup » CRM » Newsletter » plentymarkets » Email folder* menu
[cols="1,3"]
|====
|Setting |Explanation
...
. Go to *Setup » CRM » Newsletter » plentymarkets » Email folder*.
. Carry out the changes. Pay attention to the information given in <<#table-create-email-folder>>.
. *Save* (icon:save[role="green"]) the settings.

[#500]
=== Assigning email addresses to the newsletter main folder

Go to <<online-store/online-store/order-process#, Setup » Client » Select client » Order process » Settings>> and use the *Newsletter main folder* drop-down list to select which email folder should automatically be linked to a customer's email address when they subscribe to a newsletter during the order process in the online store. All of the visible email folders are listed in the drop-down list. Pay attention to the information about the option *Visibility* in <<#table-create-email-folder>>.
...
→ The email template is created and saved in one of the 3 folders, depending on the selected setting in the *Owner* drop-down list. Pay attention to the information about *Owner* given in <<#table-create-email-template>>.
...
If you would like to use the <<crm/sending-newsletters#800, double opt-in method>>, a further step is required. You have to create two templates and select them for the events *Newsletter opt-in* and *Newsletter opt-in confirmation* in the *Setup » Client* » _Select client_ » *Email » Automatic* menu.


[[table-create-email-template]]
.Creating a new email template in the *Setup » Client* » _Select client_ » *Email » Templates* menu
[cols="1,3"]
|====
|Setting |Explanation
...
| *Use design*
|Activate if the content of the email should be sent with the design that was saved under <<crm/sending-emails#800, Setup » Client » Select client » Email » HTML design>>. If the option is not activated, only the content of the email template is used.
...
The *IP address* and the *confirmation URL* used for a confirmation are saved in the database. This data can be retrieved afterwards via a link:https://developers.plentymarkets.com/rest-doc/newsletter/details#list-a-recipient[REST-Call^]{nbsp}icon:external-link[]. It is not visible in the back end. This is not displayed in the back end.
...
. Expand the section *New email address* (icon:plus-square-o[]).
...
. Expand the section *New newsletter* (icon:plus-square-o[]). +
...
| *Image gallery*
|Opens the <<online-store/online-store/cms#image-gallery#, CMS » image gallery>> menu. Save an image here to attach it to the newsletter.

| *Input layout template*
|Clicking on *Input layout template* inserts the form of address *[Guten Tag]* along with the placeholder *[NAME]* in the *HTML newsletter* section. The customer's name is automatically replaced in the template variable.
...
Proceed as described below to search for an email address in an email folder. You can either search for customers by their email address or by their last name.
...
Once you have found an email address with the search function, you can edit the data that is saved for this customer. For example, this is useful if you need to update a customer's email address or if you want to move the contact data record into a different email folder. Proceed as described below.
...
. Expand the customer (icon:plus-square-o[]).
. Carry out the changes. Pay attention to the information given in <<#table-save-email-address>> in the <<crm/sending-newsletters#900, Saving an email address>> chapter.
. *Save* (icon:save[role="green"]) the settings.

[#1300]
=== Exporting and importing email addresses

The previous chapters describe how to manage individual email addresses. If you want to edit several email addresses at once, it is recommended that you work with the <<data/importing-data/ElasticSync#, ElasticSync>>. Import new and edit existing email addresses with the sync type <<data/importing-data/sync-types/elasticSync-newsletter-recipient#, Newsletter recipient>>.

You can also export email addresses. To do so, create an export format in the <<data/exporting-data/FormatDesigner#, Data » FormatDesigner>> menu and export the data afterwards via the <<data/exporting-data/elastic-export#, Elastic export>>.
...
. Carry out the settings in the *Copy all customer data into email folder(s)* section. Pay attention to the information given in <<#table-copy-customer-data>>.
...
*Only customers who have consented to the receipt of newsletters* = Only the customer data of customers who have subscribed to the newsletter are copied into the email folder.
...
. Go to *Setup » CRM » Newsletter » plentymarkets » Email filter*.
. Carry out the settings in the *Customer filter* section. Pay attention to the information given in <<#table-filter-customer-data>>.
. Click on *Preview* (icon:eye[role="blue"]) to show the customer data. +
→ The *Preview* section that contains the customer data opens. Check the result.
. Click on *Transfer emails into folder* (icon:cog[]).
...
You should make it possible for your newsletter recipients to unsubscribe from your newsletter(s) at any time. In order that the recipient can unsubscribe from the newsletter, you have to insert the unsubscribe link in the newsletter template in the *CRM » Newsletter* menu or as you choose in any other email template in the *Setup » Client* » _Select client_ » *Email » Templates* menu.
...
The domain is saved in the *Setup » Client* » _Select client_ » *Settings* menu.
...
Register for free in order to use CleverReach in plentymarkets. To do so, go to the link:https://www.cleverreach.com/en/login/[CleverReach website^]. For further information about the settings in CleverReach, refer to the link:http://support.cleverreach.de/hc/de[CleverReach help center^]{nbsp}icon:external-link[].
...
.Settings in the *Setup » CRM » Newsletter » CleverReach* menu
[cols="1,3"]
|====
|Setting |Explanation
