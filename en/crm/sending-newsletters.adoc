...
|*Client (store)*|Activate one, multiple or *ALL* clients (stores). The email template is only valid for the selected clients (stores).
...
The *IP address* and used *confirmation URL* used for the confirmation will be saved to the database. These data can be called up via a link:https://developers.plentymarkets.com/rest-doc/newsletter/details#list-a-recipient[REST-Call^] but is not shown in the backend.
...
You can import or export email addresses. You can export all of the email addresses by going to <<basics/data-exchange/exporting-data#, Data » Dynamic export>> and using the <<basics/data-exchange/data-formats/customernewsletter#, CustomerNewsletter>> data format. In the <<basics/data-exchange/importing-data#, Data » Dynamic import>> menu, you import new email addresses.
