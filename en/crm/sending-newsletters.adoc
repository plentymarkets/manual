...
.Settings in the *System » Client » Select client » Email » Newsletter* menu¤
[cols="a,a"]
|====
|Setting |Explanation
...
.Creating a new email template in the *System » Client » Select client » Email » Templates* menu
[cols="1,3"]
|====
|Setting |Explanation
...
Own user name = The template is saved under *My email templates*. You have to be logged into the system as a user in order to create your own email templates under *My email templates*. +
A different user = The template is saved under *Email templates from other users*.

|*Name*
|Enter the name that you would like to save this template under. This name is not displayed to the recipient.
...
*All* = The email template is used for all areas. +
*Order*/*Customers*/*Online store*/*Ticket* = The email template is only available for the selected area. It is not available in other areas, i.e. it is invisible in other areas.

|*PDF attachment*
|Select the attachment from the drop-down list. The documents that can be selected from the drop-down list are generated by the system.
...
|*Use design*
|Activate if the content of the email should be sent with the design that was saved under <<crm/sending-emails#800, System » Client » Select client » Email » HTML design>>. If the option is not activated, only the content of the email is used.
...
In the customer data record in the *CRM » Edit customers » Search customer » Open customer* menu, the *Newsletter* option is activated automatically in the *Customer data* tab when the customer has registered for the newsletter during the order process and has confirmed the newsletter registration via the confirmation link.
====
...
. Select the option *Email address* or *Surname* from the drop-down list. Customers will be searched for by this criterion.
...
The previous chapters describe how to manage individual email addresses. If you want to edit several email addresses at once, it is recommended that you work with the <<basics/data-exchange#, data exchange>>. You can import or export email addresses. You can export all of the email addresses by going to <<basics/data-exchange/exporting-data#, Data » Dynamic export>> and using the <<basics/data-exchange/data-formats/customernewsletter#, CustomerNewsletter>> data format. You can import new email addresses by going to <<basics/data-exchange/importing-data#, Data » Dynamic import>>.
...
====
You can send your Newsletter2Go newsletters from an link:https://www.newsletter2go.com/features/newsletter-dispatch-dedicated-ips/[exclusive, dedicated IP address]. To use this service, you need to access the Service centre in the *Start » Service centre* menu and save the Newsletter2Go <<basics/working-with-plentymarkets/plentymarkets-account#domain-nameserver, name server>> for the sub-domains. You receive the necessary information regarding sub-domains and name server from Newsletter2Go.
====
