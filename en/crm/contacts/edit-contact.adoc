...
Once you have <<#crm/contacts/create-contact, created>> a contact, you can add further details to the various areas of the contact data record. Note also the <<#group-function, group function>> that you can use to easily and quickly carry out different actions for multiple contacts at once.

These areas and their functions are described in the following sub-chapters. By clicking on an entry in the following list, you will be directly forwarded to the corresponding sub-chapter:
...
| *Type*
| Select a type. The *Customer*, *Sales representative*, *Supplier*, *Partners*, *Manufacturers* and *Interested party* types can be selected by default. Create <<crm/contacts/preparatory-settings#types, further types>> in the *Setup » CRM » Types* menu. +
...
Click on icon:refresh[set=material] to reload the *Orders* area.

[#addresses]
== Addresses

Here, you can see all addresses that are saved for the contact. Click on *Configure columns* (icon:settings[set=material]) on the top right and decide which columns to show in the *Addresses* area. Click on icon:sort[set=material] to change the order of the columns with drag-and-drop. The following columns are available:
...
Click in the line of the address to open the address for further editing. Click on *New address* (icon:add[set=material]) to save a new address for the contact. <<#table-new-address>> lists the available fields of the address.
...
After saving, the address is given a consecutive ID that _cannot_ be changed.
...
Click on icon:more_vert[set=material] to edit or delete the bank details. By clicking on *Add new bank details* (icon:add[set=material]), you will be forwarded to a new bank data record and create a new one there. Click on icon:more_vert[set=material] to edit or delete the bank details.
...
image:crm/assets/contacts-company-create-link.png[width=640, alt=Create link to company]
...
In the line of the company, click on icon:more_vert[set=material] and then on icon:[set=material] *Delete relation to company* to remove the relation between contact and company.
...
Would you like to know which types and sub-types are available in the options? Simply expand the following area by clicking on icon:expand_more[set=material].
...
. Go to *CRM » Contacts (Test phase)*.
. Search the contacts by means of the filter settings. Note <<#table-search-contact>> on the <<#crm/contacts/search-contact, Searching for a contact>> page. +
→ The contacts that correspond to the search criteria entered are displayed.
