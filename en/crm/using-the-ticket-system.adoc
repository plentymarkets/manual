...
Ticket system, ticket, role, type, status, signature, ticket signature, complimentary close, message, comment, ticket template, child ticket, parent ticket, main ticket, new message, new comment, new ticket, ticket message, ticket owner, email ticket, e-mail ticket, ticket FAQ, FAQ ticket system
...
. In the *Authorisations* area, activate the rights as listed in <<#table-rights-ticket-system>>.
...
| *Port*
|Standard value: 25. This value is correct for nearly all providers. Otherwise, ask your provider about the correct port. Pay attention to the information about *Encryption*.
...
Users of the type *Back end* can only access limited areas and menus in the plentymarkets back end. Thus, their access to the system is limited. If users of this type should be able to work with the roles, statuses and types in the ticket system, a user of the type *Admin* has to assign the rights listed in <<#table-rights-ticket-system-roles-status-types>> to those users.
...
. Go to *Setup » Settings » User » Rights » User*. +
. Use the search function (icon:search[role="blue"]) and open the user account that should be edited.
. In the *Authorisations* area, activate the rights as listed in <<#table-rights-ticket-system-roles-status-types>>.
. *Save* (icon:save[role="green"]) the settings.
...
. Carry out the settings. Note the explanations in <<#table-new-faq>>.
. *Save* (icon:save[role="green"]) the settings. +
...
. Carry out the settings. For further information, refer to the explanations on the <<crm/sending-emails#, Emails>> page of the manual in the <<crm/sending-emails#1200, Creating an email template>> chapter.
...
. Go to *CRM » Ticket system*.
. In the *Tickets* tab in the upper area, select the filters or enter the data in the search fields. Note the explanations in <<#table-search-ticket>>.
