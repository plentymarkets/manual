...
If you inserted the template variable *$SignatureHTML* in the email template, the signature that is saved in the user account is used. If no signature is saved in the user account, the signature that is saved in the *System » Client* » _Select client_ » *Email » Signature* menu is used. +
If you inserted the template variable *$SignatureText* in the email template, the signature that is saved in the *Setup » Client* » _Select client_ » *Email » Signature* menu is always used.
====

[#400]
=== Activating owners

Go to *Setup » Settings » User » Account* » _Open user_ and activate the owners in the *Owner* tab. Activate the *Ticket* option for each user who should be available as an owner in the ticket system. For further information, refer to the <<basics/admin-tasks/manage-users#40, Managing user accounts>> page of the manual.

[#500]
=== Assigning user rights for back end users

Users of the type *Back end* can only access limited areas and menus in the plentymarkets back end. Thus, their access to the system is limited. If users of this type should be able to work with the ticket system, a user of the type *Admin* has to assign the rights listed in <<table-rights-ticket-system>> to those users.
...
. In the *Authorisations* section, activate the rights as listed in <<table-rights-ticket-system>>.
. *Save* (icon:save[role="green"]) the settings.

[[table-rights-ticket-system]]
.Required rights for back end users
[cols="1,3"]
|====
|Setting |Explanation

|*CRM > Ticket > Update ticket*
|Authorises the back end user to update tickets.

|*CRM > Ticket > Show ticket*
|Authorises the back end user to see tickets.

|*CRM > Ticket > Edit ticket*
|Authorises the back end user to edit tickets.

|*CRM > Ticket > Create ticket*
|Authorises the back end user to create tickets.

|*CRM > Ticket > Delete ticket*
|Authorises the back end user to delete tickets.
...
. Expand the section *Email* (icon:angle-down[]).
. Carry out the settings. Pay attention to the information given in <<table-set-up-email-account>>. +
→ *_Note:_* → The settings in the sections *Account*, *Retrieval* and *Despatch* are mandatory settings.
. *Save* (icon:save[role="green"]) the settings.

[IMPORTANT]
.Observe test mode
====
After you have carried out all tests, you have to deactivate the test mode in the *Setup » Client* » _Select client_ » *Email » Login details* menu. If you do not deactivate the test mode, tickets are only sent to the email address that is saved in the *Setup » CRM » Ticket system » Email accounts* menu.
====

[[table-set-up-email-account]]
.Settings in the *Setup » CRM » Ticket system » Email accounts* menu
[cols="1,3"]
|====
|Setting |Explanation
...
2+|*Account* icon:exclamation-triangle[role="red"] *_Note_* The setting in this section is mandatory.)

|*Email*
|Enter the email address that should be used for the ticket system. This is a mandatory field.

2+|*Retrieval* (icon:exclamation-triangle[role="red"] *_Note_* All settings in this section are mandatory.)
...
|*Port*
|Default value: 25. This value is correct for nearly all providers. Otherwise, ask your provider about the correct port. Pay attention to the information about *Encryption*.
...
2+|*Shipping* (icon:exclamation-triangle[role="red"] *_Note:_*  All settings in this section are mandatory.)

|*Name of sender*
|Enter your own name or the name of your company/online store. This name is displayed to the recipient.
...
Users of the type *Back end* can only access limited areas and menus in the plentymarkets back end. Thus, their access to the system is limited. If users of this type should be able to work with roles, statuses and types in the ticket system a user of the type *Admin* has to assign the rights listed in <<table-rights-ticket-system-roles-status-types>> to those users.
...
. Go to *Setup » Settings » User » Rights » User*. +
. Use the search function (icon:search[role="blue"]) and open the user account that should be edited.
. In the *Authorisations* section, activate the rights as listed in <<table-rights-ticket-system-roles-status-types>>.
. *Save* (icon:save[role="green"]) the settings.

[[table-rights-ticket-system-roles-status-types]]
.Rights for roles, statuses and types for back end users
[cols="1,3"]
|====
|Setting |Explanation

|*CRM > Ticket > Ticket role > Update ticket role*
|Authorises the back end user to update ticket roles.

|*CRM > Ticket > Ticket role > Show ticket role*
|Authorises the back end user to see ticket roles.

|*CRM > Ticket > Ticket role > Create ticket role*
|Authorises the back end user to create ticket roles.

|*CRM > Ticket > Ticket status > Update ticket status*
|Authorises the back end user to update ticket statuses.

|*CRM > Ticket > Ticket status > Show ticket status*
|Authorises the back end user to see ticket statuses.

|*CRM > Ticket > Ticket status > Create ticket status*
|Authorises the back end user to create ticket statuses.

|*CRM > Ticket > Ticket type > Update ticket type*
|Authorises the back end user to update ticket types.

|*CRM > Ticket > Ticket type > Show ticket type*
|Authorises the back end user to see ticket types.

|*CRM > Ticket > Ticket type > Create ticket type*
|Authorises the back end user to create ticket types.
...
A role provides information about the members of your staff involved in handling a ticket. These roles can be individually assigned for each ticket. For example, if an employee provides assistance to customers in a ticket, it would make sense to assign this employee to the previously created role *Support*. The active user is automatically assigned as owner of a new ticket.
...
→ *_Note:_* Use a ticket event procedure to change the status afterwards. It is possible, for example, to activate tickets that are already closed.
...
.Settings in the *CRM » Ticket system* menu in the *New ticket* tab
[cols="1,3"]
|====
|Setting |Explanation
...
|*Tags*
|Click to add tags. You can select the tags that you set up in the *Setup » Settings » Tags* menu. For further information, refer to the <<basics/working-with-plentymarkets/tags#, Tags>> page of the manual.
...
|*Deadline*
|Enter the deadline in the format MM/DD/YY or select the date from the calendar.
...
If the ticket is not linked to a customer, it is not possible to send the email template. To do so, click in the *Customer* field and press the down arrow key on your keyboard. A list of the customers appears. The customers are displayed in order by their ID. The customer with the lowest ID is displayed at the top. By clicking on the customer, the name and the ID of the customer are transferred into this field.
...
|*Hide / Show tags*
|Shows or hides tags. If *Show tags* is activated, the area *Tags* is displayed on the right side of the ticket. Further <<basics/working-with-plentymarkets/tags#, tags>> that were created before in the *Setup » Settings » Tags* menu can be added here. You can save as many tags for the ticket as needed.

|*Create card*
|Creates a new card for the board selected in the drop-down list. It is also possible to add a prefix to the card. Create <<basics/working-with-plentymarkets/boards#, boards>> in the *plentymarkets logo (Start) » Boards* menu.
...
The first time that eBay messages are imported, only the ones from the last 7 days are retrieved. If an import has been carried out before, messages are imported until the date of the last import. However, this last import may not date back more than 7 days. Otherwise, only messages from the last 7 days are imported again. You can access information about the imports by going to <<data/api-log#, Data » API log>> and looking at the *EbayMemberMessages* API log.

[#2300]
==== Creating tickets from forms

Forms allow you to integrate contact forms in your online store. The forms are filled out by visitors in your online store and a ticket is created. You can answer the customer directly from the ticket. However, this is only possible if a valid email account was set up under *System » CRM » Ticket system » Email accounts*.

You receive an email notification whenever a new message is received. This notification is separate from the ticket and any procedures that are carried out in the ticket system. The notification is sent to the address that was saved under *Setup » Client* » _Select client_ » *Email » Login details*. You receive this email notification even if you did not <<crm/using-the-ticket-system#100, book>> the ticket system.

[TIP]
.Example for using the contact form
====
A visitor in your online store asks a questions about your items and contacts you using the contact form in your online store. The contact form is used to create a ticket in the ticket system. The ticket contains the customer's message. You answer the customer by writing a message in the ticket system. With a correspondingly set up event procedure, the online store visitor receives an email. The online store visitor now replies to your message from their own email program. If parameters such as the ticket ID etc. were inserted into the email as template variable, the email automatically assigned to the ticket. +
*_Note:_* If the online store visitor deleted the ticket ID or other parameters from the subject line and from the email, it is not possible to assign the email to a ticket. A new ticket is created. +
A new ticket is also created if the online store visitor decides to use the contact form again instead of answering the email directly. In this case, no assignment parameters would be included in the message and your plentymarkets system would not be able to recognise that the message belongs to a previous thread.
====
...
Use messages to reply to your customers from within the ticket system. Note that a message can only be sent as an email if you have set up an event procedure in the *Setup » CRM » Ticket system » Events* menu.
...
Sending automatic email templates for service units in the *System » Client* » _Select client_ » *Email » Automatic* menu still applies for events that were created in the contact data record.
...
. Carry out the settings. Pay attention to the information given in <<table-faq-new-folder>>.
. *Save* (icon:save[role="green"]) the settings. +
...
. Carry out the settings. Pay attention to the information given in <<table-new-faq>>.
. *Save* (icon:save[role="green"]) the settings. +
...
|*Question*
|Enter the question, e.g.  "Can I return the goods free of charge?"
...
. *Save* (icon:save[role="green"]) the settings.
. Carry out the desired settings. Pay attention to the explanations given in <<table-event-procedure-new-message-ticket-system>>.
...
|*Tags*
|Select tags to create a template with these tags. Go to <<basics/working-with-plentymarkets/tags#, Setup » Settings » Tags>> to create tags.
...
.Settings in the *Tickets* tab of the *CRM » Ticket system* menu
[cols="1,3"]
|====
|Setting |Explanation
...
Various entry fields and filters are available in the ticket search in the *Tickets* tab. They help you narrow down the list of tickets to find the exact ones that you are looking for. Expand the *Search* section to select further filters and entry fields. This allows to search for your tickets even more detailed.

[.instruction]
Searching for tickets:

. Go to *CRM » Ticket system*.
. In the *Tickets* tab in the upper section, select the filters or enter the data in the search fields. Pay attention to the information given in <<table-search-ticket>>.
. Click on *Search* (icon:search[role="blue"]).  +
→ The tickets that match the selected settings are displayed.

[[table-search-ticket]]
.Search options in the *CRM » Ticket system* menu in the *Tickets* tab
[cols="1,3"]
|====
|Setting |Explanation
...
|*Customer class*
|Select a customer class to only search for tickets that belong to customers with this customer class. Go to <<crm/managing-contacts#15, Setup » CRM » Customer classes>> to create further classes.
...
The procedure manager in the *System » CRM » Ticket system » Procedures* menu can be used to remind you of the ticket. An example how to set up a procedure and select filters and the time can be found in this <<crm/using-the-ticket-system#3000, chapter>>.

|*Tags*
|Click into the field. Select one or more tags to only search for tickets that contain these tags. First tags have to be created in the *System » Settings » Tags* menu.
|====
...
[[table-edit-ticket]]
.Settings in the *CRM » Ticket system* » _Open ticket_ » *Tab: Ticket* menu
[cols="1,3"]
|====
|Setting |Explanation
...
The *Customer* tab of a ticket in the *CRM » Ticket system* menu in the contains the contact data as it is also found in the <<crm/managing-contacts#300, CRM » Contacts » Open contact>> menu. You can also edit the customer data record here. This tab is only visible if the ticket is linked to a customer.

[#3800]
=== Showing orders

Go to *CRM » Ticket system* and see the customer’s orders in the *Search orders* tab of the ticket. You can search for orders and edit them in this menu. This tab is only visible if the ticket is linked to a customer.
...
If you activated the option *Attachment* via the <<basics/working-with-plentymarkets/general-functions#50, Customising the layout>> in the *CRM » Ticket system* menu in the *Tickets* tab, you can directly see in the ticket overview the tickets that have an attachment.
...
. Carry out the settings. Pay attention to the explanations in the <<basics/working-with-plentymarkets/tasks-and-calendars#300, Creating a task>> chapter on the <<basics/working-with-plentymarkets/tasks-and-calendars#, Tasks and calendars>> page of the manual.
. *Save* (icon:save[role="green"]) the settings. +
...
. Carry out the settings. Pay attention to the explanations in the <<basics/working-with-plentymarkets/tasks-and-calendars#400, Searching for tasks>> chapter on the <<basics/working-with-plentymarkets/tasks-and-calendars#, Tasks and calendars>> page of the manual.
. Click on *Search* (icon:search[role="blue"]).
...
. Select the filters in the *Search* section of the *Tickets* tab.
. Expand the *Search* section to select further filters.
. Click on *Search* (icon:search[role="blue"]). +
...
====
Create a child ticket in the *Children* tab of the ticket in the *CRM » Ticket system* menu. Click on *New ticket*. Carry out the settings as desired. This new ticket is automatically assigned to the main ticket and can be found in it's *Children* tab.
====
...
. Enter the name for the filter in the *Settings* section.
. Select a symbol from the drop-down list in the *Settings* section.
...
Go to *Setup » CRM » Ticket system » Ticket cards* to create a print template that is used to generate your tickets as PDF files which you can file or print. Setting up ticket cards can basically be compared to the procedure of setting up <<fulfilment/generating-documents#400, label templates>>.
...
Your customers can create tickets in the *My account* area of your online store. They can send messages there and see answers to the ticket. If an event procedure is used to send the contact an email and this email contains the template variable *$TicketURL*, the contact is able to click on this link to directly open up the ticket in the My account area. They are able to answer the message there.
...
For further information, refer to the <<basics/working-with-plentymarkets/statistics/managing-statistics#, Statistics>> page of the manual. It is also described there how to create and analyse statistics.
