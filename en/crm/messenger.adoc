...
:keywords: messenger, massenger, messages, message, billable messages, billable
...
| *Whisper*
|If you click on the button *Whisper* (icon:eye[] icon:toggle-off[role="red"]), only those employees who have rights for the messenger can read the message. Your contacts are not able to see the message. Whisper mode is active if the button looks like this: icon:eye-slash[] icon:toggle-on["role=green"]. +
...
*_Important:_* If you activated the button (icon:pay_cash[set=plenty] icon:toggle-on[role="green"]), it is necessary to save a <<crm/managing-contacts#1396, tag>> of the type *Message (billable)* at the message. These tags have to be created in advance in the *Setup » Settings » Tags* menu. Otherwise, the message cannot be created.
...
. Carry out the desired changes. Pay attention to <<bild-messenger-nachricht-bearbeiten>> and <<#table-new-message-messenger>>.
