...
. Select an email template from the drop-down list in the line of the event. Note <<#table-events-automatic-email-dispatch>>.
. *Save* (icon:save[role="green"]) the settings.

<<#table-events-automatic-email-dispatch>> lists explanations of some of the events.
