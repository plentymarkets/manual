...
*_Tip:_* Use text in HTML format. The link for order processing that is returned via the template function `{%Link_Checkout()%}` might be very long. Pure text emails can lead to line breaks that can damage the link. +
...
====
plentymarkets recommends that you use text which was formatted with HTML because the link for order processing, for example, might be very long. Pure text emails can lead to line breaks that might damage the link. Furthermore, if you use HTML-formatted text, you have the possibility to format the text by means of the integrated HTML editor.
...
[TIP]
.Order processing link in email template
====
If you want that your customers can have a look at the order's current state at any time, insert the template function `Link_Checkout()` in the email template. In order that the URL is correctly translated, you have to insert the template function into the email template as follows: `{%Link_Checkout()%}`.

*_Note:_* The template function `{%Link_Checkout()%}` is _not_ available automatically in the email template *Confirmation of online shop order*.
====
...
Click on *Edit email template*. This opens the email template in the *Setup » Client » [Select client] » Email » Templates* menu. The email template can be <<crm/sending-emails#1700, edited>> there.
...
*_Note:_* If you want that your customers can see the order's current state and thus access the order processing in your online shop via this email, you have to insert the template function `{%Link_Checkout()%}` in the email template that you selected for this automatic event.
...
| `Link_Checkout()`
|This function displays the URL for the order checkout in the online shop. +
...
Go to *Setup » Client » [Select client]» Online store » Legal information » [Expand language]*. Click on the *Terms and conditions* tab and save the text in the *Text* tab.

| `$GeneralTermsConditionsHTML`
|Terms and conditions in HTML format. +
Go to *Setup » Client » [Select client] » Online store » Legal information » [Expand language]*. Click on the *Terms and conditions* tab and save the text in the *HTML* tab.

| `$CancellationRightsText`
|Cancellation rights in plain text. +
Go to *Setup » Client » [Select client] » Online store » Legal information » [Expand language]*. Click on the *Cancellation rights* tab and save the text in the *Text* tab.

| `$CancellationRightsHTML`
|Cancellation rights in HTML format. +
Go to *Setup » Client » [Select client] » Online store » Legal information » [Expand language]*. Click on the *Cancellation rights* tab and save the text in the *HTML* tab.

| `$PrivacyPolicyText`
|Privacy policy in plain text. +
Go to *Setup » Client » [Select client] » Online store » Legal information » [Expand language]*. Click on the *Privacy policy* tab and save the text in the *Text* tab.

| `$PrivacyPolicyHTML`
|Privacy policy in HTML format. +
Go to *Setup » Client » [Select client] » Online store » Legal information » [Expand language]*. Click on the *Privacy policy* tab and save the text in the *HTML* tab.

| `$LegalDisclosureText`
|Legal disclosure in plain text. +
Go to *Setup » Client » [Select client] » Online store » Legal information » [Expand language]*. Click on the *Legal disclosure* tab and save the text in the *Text* tab.

| `$LegalDisclosureHTML`
|Legal disclosure in HTML format. +
Go to *Setup » Client » [Select client] » Online store » Legal information » [Expand language]*. Click on the *Legal disclosure* tab and save the text in the *HTML* tab.

| `$WithdrawalFormText`
|Cancellation form in plain text. +
Go to *Setup » Client » [Select client] » Online store » Legal information » [Expand language]*. Click on the *Cancellation form* and save the text in the *Text* tab.

| `$WithdrawalFormHTML`
|Cancellation form in HTML format. +
Go to *Setup » Client » [Select client] » Online store » Legal information » [Expand language]*. Click on the *Cancellation form* tab and save the text in the *HTML* tab.
...
|Order properties +
This is not a global template variable, but a variable that is set per order item. It can only be used in the *Item list* tab of the email template. +
For further information, refer to the <<item/settings/properties#, Characteristics and properties>> page of the manual.
