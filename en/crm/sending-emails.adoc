...
. *Save* the settings. +
→ The email template is created and saved in one of the three folders - depending on the selected setting in the *Owner* drop-down list.
...
|*PDF attachment*
|Select an attachment. The documents that can be selected from this drop-down list are for example generated in the *Orders » Fulfilment* menu or through shipment-related processes.
...
→ Two toolbars to format the text are displayed.
...
Click on *Edit email template*. This opens the email template in the *System » Client » Select client » Email » Templates* menu. You can <<crm/sending-emails#1700, edit>> the email template there. <<table-events-automatic-emails>> lists explanations of some of the events.
