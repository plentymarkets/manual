...
.Settings in the *Setup » Client* » _Select client_ » *Email » Templates* menu
[cols="1,3"]
|====
|Setting |Explanation
...
→ To help with the formatting, for example, you can insert tabs that you copied from a text program between the template variables (see <<image-available-template-variables-item-list>>).
. *Save* (icon:save[role="green"]) the settings.

[[image-available-template-variables-item-list]]
.Template variables available for the item list
image::crm/assets/available-template-variables-item-list.png[width=640, height=360]
...
For further information, refer to the <<online-store/online-store/cms-syntax#basics-if, CMS>> page of the manual.
...
.Explanations for some events in the *Setup » Client* » _Select client_ » *Email » Automatic* menu
[cols="1,3"]
|====
|Setting |Explanation
...
As well as the field name itself and the alternative value (alias) can be used as *$field*. +
...
The IDs are automatically assigned by the system and cannot be changed. The IDs are listed in the *Setup » Orders » Payment » Payment methods* menu.
