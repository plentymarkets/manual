...
The **Settings » Basic settings » Backup » Download backup** menu allows you to download a backup to the **<<client-store/global-settings/ftp-access#, FTP server  >>**. You can select and export a backup of the last 7 days. Backups are saved as SQL files. The file name is composed of the system ID and the date based on the following pattern: **pSystem-ID_year-month-day**. Please note that only one backup can be created per day.
...
→ The Backup is saved on the **<<client-store/global-settings/ftp-access#, FTP server  >>**.
