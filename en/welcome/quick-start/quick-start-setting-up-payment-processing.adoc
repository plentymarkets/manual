...
To set up PayPal, you first need to install and activate the PayPal plugin. You can see in the *Plugins » Plugin set overview* which plugins are installed and active. Moreover, you can easily access the *plentyMarketplace* (icon:cart-plus[role="green"]) from here.
...
. The option *Cash in advance icon (PrePayment)* is already pre-selected in the drop-down list.
...
include::../../plugins/_textblocks/instructions/plugin-container-verknuepfung-en_gb.adoc[menu-path]
. From the drop-down list, select the option *Cash in advance bank details (PrePayment)*.
