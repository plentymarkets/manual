...
:position: 140
:url: welcome/quick-start/quick-start-ebay
:id: HBY8NX5
:author: team-multi-channel

IMPORTANT: Have you already added a plugin from the plentyMarketplace to your store? Not yet? Then the page <<welcome/quick-start/quick-start-adding-plugins-to-the-system#, Adding plugins to your system>> will teach you how.
...
If you do not have an eBay account, you need to register with link:https://reg.ebay.com/reg/PartialReg[eBay^]{nbsp}icon:external-link[] first.

[#300]
=== Activating and editing accounts

Go to *Setup » Markets » eBay » Accounts* to activate new accounts for eBay. To do so, you will need your eBay access data consisting of the user name and password. When changing your eBay access data, you have to update and synchronise your account in plentymarkets. Proceed as follows to add an account.
...
====
After changing the eBay account data, you have to update your shop categories and return policy. Update the *shop categories* directly in this menu. Go to *Setup » Markets » eBay » Business policies » Tab: eBay return policy* to save the *return policy*.
====

[#500]
=== Carrying out the basic settings

Go to *Setup » Markets » eBay » Settings* to carry out the basic settings for the market eBay and to set up your accounts.
...
If you select *No*, then the eBay variations are sorted by the positions of the attribute values. An attribute value’s position is defined in the *Setup » Item » Attributes » [Open attribute] » Tab: Values* menu.
...
*Yes* = Further item details from the eBay catalogue are automatically added. For further details, refer to link:https://www.ebay.co.uk/help/selling/listings/creating-managing-listings/listing-item-product-details-catalogue?id=4653[eBay^]{nbsp}icon:external-link[]. +
...
*Yes* = If further images are available in the eBay catalogue in addition to your own images, these are added automatically. For further details, refer to link:https://www.ebay.co.uk/help/selling/listings/creating-managing-listings/listing-item-product-details-catalogue?id=4653[eBay^]{nbsp}icon:external-link[]. +
...
Variation availabilities are organised in the *Setup » Item » Availability* menu and set in the *Item » Edit item » [Open item] » Tab: Variation ID » Tab: Settings* menu of each item.
|====
...
Go to *Setup » Markets » eBay » Settings » Tab: Shipping profile links* to link eBay shipping profiles to the shipping profiles you created in plentymarkets. A linked shipping profile is automatically used during order import if the shipping profile matches the order data, e.g. account and market settings, of a listing and is activated in the *Global* tab of an item. If the linked shipping profile is deactivated, the active shipping profile with the highest priority will automatically be used instead.
...
When the *Use out of stock control* is set in the *Setup » Markets » eBay » Settings » Tab: Account settings* menu, the function of the automatic stock update changes. A listing is not ended when an item is out of stock. The item is still displayed but cannot be purchased. The number of items sold is not set to zero on eBay when the item is available again. Rather, the number remains in the sales history. Even when the *End offers automatically* setting is active in the *Setup » Listings » Stock* menu, listings are not ended. They cannot be purchased instead.
...
====
In the *Setup » Orders » Payment » Currencies* menu, you can set up currency conversion for listings by selecting *Yes* for the *Automatically update conversion factors daily* option. You have to enter a conversion factor for each currency that you use. +
...
| *PayPal account*
|Select your PayPal account. Only those PayPal accounts that were set up in the *Setup » Orders » Payment » Methods » PayPal* menu are displayed here.
...
*Express delivery* = Items are shipped the same day they were ordered, if the order was placed before a certain time. For further information, refer to eBay’s link:https://www.ebay.co.uk/help/buying/postage-delivery/delivery-date-options-buyers?id=4025[Selling with eBay FAST & FREE^]{nbsp}icon:external-link[] page.

| *Shipping discount for combined payment*
|Select whether the shipping discount for combined payment should be applied. link:https://www.ebay.com/help/selling/shipping-items/shipping-rates/offering-combined-shipping?id=4169[Shipping discount for combined payment^]{nbsp}icon:external-link[] must be set up on eBay.

| *Rules governing shipping at a special rate*
|Select whether the rule governing shipping at a special rate should be applied. link:https://www.ebay.com/help/selling/posting-items/postage-rates/postage-discounts?id=4168#section4[Rules governing shipping at a special rate^]{nbsp}icon:external-link[] must be set up on eBay.

| *Rate tables (additional regions)*
|Select whether the rate tables should be applied. Rate tables must be set up on eBay. For further information, refer to eBay’s link:https://www.ebay.co.uk/help/selling/posting-items/setting-postage-options?id=4089#tables[Specifying your postage costs and locations^]{nbsp}icon:external-link[] page.

| *Exclude shipping locations*
|Exclude shipping locations, PO boxes and DHL Packstations. The options shown here depend on the *eBay platform* that was chosen. The countries listed here are related to the option *Purchasers who are registered in a country that I do not ship items to* in the *Buyer requirements* section of an eBay shipping profile in the *Setup » Markets » eBay » Business policies » [Open shipping profile]* menu.
|====

[#1400]
==== Determining the return policy

You can set up your return policy either on eBay or in the *Setup » Markets » eBay » Business policies » Tab: eBay return policy* menu. The return policy saved on eBay is overwritten when entering the return policy in plentymarkets. An overview of existing profiles for the return policy is displayed.
...
The menu *Setup » Markets » eBay » Business policies » Tab: Shipping profiles* contains the tabs *Overview* and *New*. The *Overview* tab displays the previously created shipping profiles. Edit or delete existing profiles here. Create additional shipping profiles in the *New* tab.
...
Go to *Setup » Listings* to carry out the settings for all listings regardless of the market.
...
*Status 1* = The customer is guided through the order process and actively has to complete all steps. This takes place within the webshop and the customer is encouraged to buy additional items. +
...
|Select the shipping costs that should apply to further purchases during checkout. Select *Regular*, *No further shipping costs* or *Yes, but with webshop shipping costs*.
...
====
HTML and CSS skills are necessary to create a layout template. We recommend that you commission a service provider to create a layout template if you have little experience with HTML and CSS. Go to *Setup » Listings » Layouts* to import a *layout template* that was created externally into plentymarkets.
====
...
Make sure that the images used are available via URL. To do so, upload the images on the server e.g. using *FTP*.
|====
...
Fill in the item description *{% GetItemDescription() %}* instead of the description that is saved with the listing. This way, you only need to maintain one description text for your webshop and listings.
...
*GetItemPropertyById(3, 'value')* = Unit type of the characteristic with ID 3
...
type = *unit* or *value* +
*__Examples:__* +
*GetItemPropertyByName('abstract_motives', 'unit')* = Unit type of the characteristic with the back end name "abstract_motives" +
*GetItemPropertyByName('abstract_motives', 'value')* = Value of the characteristic with the back end name "abstract_motives"
...
For example, you can have eBay galleries created here: link:http://www.sellerfox.com/?lang=en[www.sellerfox.de^], link:http://www.ayee.net/newpage/[www.ayee.net^]{nbsp}icon:external-link[]
...
For further information, refer to eBay’s link:https://www.ebay.com/pages/br/help/sell/promoting-your-store.html[Promoting and advertising items^]{nbsp}icon:external-link[] page.
...
| *Characteristics*
|Edit characteristics. All characteristics for the selected category are displayed in the *eBay characteristics* window. Click on *Add* to create your own characteristic. Enter a name for the characteristic into the text field on the left-hand side. Enter a value for the characteristic of no more than 128 characters into the text field on the right-hand side. The characteristics saved here take priority over an item's characteristics when transferring to eBay. You will receive the error message *Code: 046 In this category, item characteristics are deactivated by eBay* if there are faulty entries or if no item characteristics are available.
|====
...
| *Parts compatibility listing*
|Select parts compatibility listing. You must have selected a platform. Create parts compatibility listings in the *Setup » Markets » eBay » Parts compatibility listing* menu.
...
| *Store visibility*
|Select one of the following options for the visibility of listings in the webshop: +
...
In plentymarkets, there are multiple ways to automatically start listings or restart listings once they have already been started. Use the listing scheduler to specify *starting times* for listings. Go to *Setup » Listings » Scheduler* to set up the listing scheduler.
...
You can link your item images with attribute values if you activate the *Image link possible* option for an attribute in the *Setup » Item » Attributes » Tab: Basic setting* menu. Note that images currently can only be linked to one attribute.
...
Pay attention to the settings for eBay properties in the *Listings » Edit listings* menu. You find this setting in the *Market* tab of a listing. This option becomes available once you have saved the eBay category. The values have mandatory fields depending on which category was selected. For further information on mandatory fields for eBay categories, refer to link:https://www.ebay.co.uk/help/listings/creating-managing-listings/creating-managing-listings?id=4105[eBay^]{nbsp}icon:external-link[]. All mandatory fields must be set up or filled in.
...
In order to be able to use your own properties for the eBay market, these have to be linked with eBay in the *Setup » Item » Properties* menu.
...
A product which is sold on eBay as a single product and in a set can have multiple ePIDs. Link variations with ePIDs in the *Setup » Markets » eBay » ePID matching* menu.
...
- In the *Setup » Markets » eBay » Settings » Tab: Initial settings* menu, the option *Add item details from the eBay catalogue* has to be set to *Yes*.
...
.eBay OAuth2-Plugin
====
In order to use ePID matching, the link:https://marketplace.plentymarkets.com/en/plugins/channels/marketplaces/ebayoauth2_4787[eBay OAuth2^] plugin is required. The plugin is available in the link:https://marketplace.plentymarkets.com/en[plentyMarketplace^]{nbsp}icon:external-link[].
====
...
[#5350]
== Synchronising data via the import tool

The import tool is available in the *Data » Import* menu to exchange data between your plentymarkets system and eBay. Use the <<data/importing-data/sync-types/elasticsync-market-listing, market listing>> import type to import listing data into plentymarkets.
...
TIP: Continue to <<welcome/quick-start/quick-start-automating-processes#, Automating processes>>
