...
→ All own uncompleted tasks are shown because the filter for the progress is set to *<100%* and the active user is preselected in the drop-down list *Owner* when opening the menu. +
. Click on icon:undo[role="orange"] to remove the filters.
. Select the filter. Pay attention to the information given in <<table-search-task>> in the <<welcome/general-functions/tasks-and-calendars#400, Searching for a task>> chapter. +
→ The tasks that correspond to the search criteria entered are shown.
. Click in the line of the task. +
→ The task opens.
. Carry out the settings. Pay attention to the information given in <<table-create-task>> in the <<welcome/general-functions/tasks-and-calendars#300, Creating a task>> chapter.
. *Save* the settings.
