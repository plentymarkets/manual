...
:keywords: Item search, Backend search, Back end search, Search for items, Search for variations, Edit items, Item » Edit item, Search option, Search options, Filter, Filters, Table type, Active, Browse description, Item list, Search behaviour, Search result, Search results, Example item, Example items, Example
...
*_Searching through the item description_*\: Do you want to search for items whose description contains a particular keyword? Place a check mark next to the <<item/introduction/search#intable-description, *Browse description* option>> (icon:check-square[role="blue"]) and enter the keyword into this search field.
...
|[#intable-tags]*Tags*
|Enter tag names to search for variations that were linked to these <<welcome/general-functions/tags#, tags>>. Go to *Setup » Settings » Tags* to create tags. +
...
|Place a check mark (icon:check-square[role="blue"]) to search for a keyword in the item description. Enter the keyword in <<item/introduction/search#intable-item-data, the *Item data* field>>.
...
| *Flag 1; +
Flag 2*
|Select a flag or *ALL* to search for <<item/settings/flags#100, items that have this flag>>.

|[#intable-manufacturer]*Manufacturer*
|Select a manufacturer or *ALL* to search for <<item/settings/manufacturers#400, items that have this manufacturer>>. +
...
|[#intable-all]*ALL* (list of attributes)
...
a|Do you only want to search for <<item/managing-items#intable-active, active items>>, i.e. items that are available in the online store?
...
*_What about Ceres store specials?_*\: <<item/online-store/store-specials#100, Ceres store specials>> are created with the help of tags. Therefore, use the <<item/introduction/search#intable-tags, tags filter>> to search for items that are part of a Ceres store special.
...
Are you responsible for the item data in multiple stores? Proceed as described below to search for the <<item/managing-items#360, items that belong to a specific store>>.
...
Once you’ve used the <<item/introduction/search#100, search options on the left>>, you’ll see the search results displayed in an overview on the right. Click on an item in the overview to open it and <<item/managing-items#, manually edit the item data record>>.
...
* Click on *Visible in online store* (icon:eye[role="blue"]) to see <<item/online-store#, what the item looks like in your online store>>. If the symbol is crossed out, then the item is *Invisible in the online store* (icon:item_webshop_invisible[set=plenty, role="blue"]).
