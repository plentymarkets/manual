...
You can decide <<item/use-cases/search-behaviour#200, which item data the search function should take into consideration>>. +
*_Searching through the item description_*: Do you want to search for items whose description contains a particular keyword? Enter the keyword into this search field and select the <<item/introduction/search#intable-description, *Browse description* option>> (icon:check-square[role="blue"]) further below.
...
|Select this option (icon:check-square[role="blue"]) to search for a keyword in the item description. Enter the keyword in <<item/introduction/search#intable-item-data, the *Item data* field>> further above.
