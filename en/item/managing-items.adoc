...
Finally, learn how to create <<item/managing-items#920, item bundles>>. How to present your products in the most favourable way using <<items/managing-items#950, cross-selling>> will also be explained.
...
Generate labels directly from the template or with the **Labels** function of an item or variation. You can also create processes for <<basics/automation/processes/procedures#150, generating item labels>>.
...
. Configure the settings **Prefix for new variation numbers** and **Next variation number**. Pay attention to the information given in <<table-variation-number-pattern>>.
. **Save** the settings.
...
=== Changing the item data shown in item tabs
...
Free text fields can be displayed at any position in the online store using the template variables **$Free[1]** to **$Free[20]**. Learn how to use free text fields in the online store on the <<omni-channel/online-store/cms#web-design-editing-the-web-design-itemview-dimension-input, Dimension input>> chapter of the manual.
...
In the **Statistics** tab, you can create different statistics for the **Default** item. These statistics e.g. let you analyse the **Default** item's revenue according to various criteria. For further information, refer to the <<basics/statistics#, Statistics>> page of the manual.
...
Let us assume that you want to sell a bundle consisting of a hand towel and a larger bath towel. You also want to offer this bundle in different colours. To do so, the following steps are necessary:
...
. Go to **Item » Edit item**.
. Open the **Set** item.
. Carry out the settings as desired. Pay attention to the information given in <<table-edit-set-items>>.
. **Save** the settings.
...
Items can be grouped in item lists and highlighted on the homepage of the online store and in the single item view. Find out how to create **Cross-selling** item lists on the <<omni-channel/online-store/setting-up-ceres#110, Setting up item lists>> page of the manual.
...
. Click on the appropriate **relation**. Pay attention to the information given in <<table-select-cross-selling-relationship>>. +
→ The items will be added and displayed in the **Cross-selling items** tab.

[[table-select-cross-selling-relationship]]
