...
In the *System » Item » Images » Sizes* menu, you select settings for each image size.

* *preview* = Preview image
* *secondPreview* = Second preview image
* *middle* = Medium size image
...
Activated = Images are scaled to size if the original image is smaller than the dimensions defined for this size.
...
After the cache is invalidated, versions of the image in different sizes are created as needed for displaying them on screen.
====
...
|**Field name**
|Enter a name for free text field. The field name must not contain special characters. If the field name of a free text field is changed, its content will not be deleted automatically. If the field's content is to be changed as well, it must be changed manually or with the help of a CSV file.
