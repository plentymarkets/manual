...
=== Using own CDN domain for image URLs
...
|https://*yoursubdomain.yourstore.de*/ +
...
[#4000]
*Save an own domain for image URLs for a domain hosted by plentymarkets*
...
[#4100]
*Save an own domain for image URLs for a domain hosted by an external provider*
...
|**L x W x H**
|Enter the variation's length, width and height in millimetres.
...
|The <<stock-management/new-incoming-items#250, net moving average purchase price>>. The moving average net purchase price is recalculated every time stock is booked in. The calculation takes into account the purchase prices of all stock of a variation currently in stock. +
