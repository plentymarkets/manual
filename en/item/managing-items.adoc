...
Create items and item variations for your products in plentymarkets. Items can be sold in your **<<omni-channel/online-store/setting-up-clients/online-store#, online store>>** and on the many **<<omni-channel/multi-channel#, markets>>** integrated with plentymarkets.

First, you will learn how to update product information for your items. Create **<<item/managing-items#390, attributes>>**, **<<item/managing-items#480, properties>>**, **<<item/managing-items#560, manufacturers>>** and **<<item/managing-items#630, barcodes>>**. Link this information with your items. Furthermore, you find information and tips for structuring **<<item/managing-items#870, sales prices>>** for your items and variations. **<<item/managing-items#910, Price calculations>>** will also be explained here. Price calculations allow you to customise and graduate prices for different customer classes as well as referrers.

Next, get familiar with the plentymarkets 7 item architecture. A **<<item/managing-items#20, Default item>>** consists of global settings and data of the main variation as well as additional **<<item/managing-items#160, variations>>**. Global settings apply to all variations of the **Default** item. Information saved with the main variation is passed on to additional variations. However, you can change the settings of every variation individually.
...
variations. How to present your products in the most favourable way using **<<item/managing-items#950, cross-selling>>** will also be explained.
...
. Carry out additional settings as desired. Pay attention to the information given in table 1.
. **Save** the settings.
...
. Carry out additional settings as desired. Pay attention to the information given in table 2.
. **Save** the settings.
...
In order to transfer your **Default** items to certain markets, you have to link attributes and attribute values for these markets. For further information, refer to the markets' pages in the topic **<<omni-channel/multi-channel#, Multi-Channel>>**.
...
You can upload images for attribute values. These images will be displayed as image tiles in the online store. Specify the size of attribute images in the **<<item/managing-items#670, Settings » Item » Images>>** menu.
...
. Select the **With image** option from the **Attribute selection in the store** drop-down menu.
. **Save** the settings.
...
. Select the appropriate attribute value for the item image from the **Market attribute link** drop-down menu.
. **Save** the settings. +
...
. Go to **Item » Edit item**.
. In the **ALL** filter, select the attribute from the drop-down menu.
. Select **ALL** in the **Active** filter.
. Click on **Search**. +
→ All items linked with this attribute will be displayed.
...
. Click on **Execute**. +
→ The variations will be deleted.
...
**Single selection** = One of the grouped order properties can be selected from the drop-down menu. +
...
|**Order property**
|Activate to make the property available in the order process. Depending on the **property type**, the customer can e.g. enter a text or value or select an option from a drop-down menu. The design of order properties can be adjusted in the CSS. Go to **CMS » Web design » Layout » CSS » CSSOrder** and save your own CSS code for the order process.
...
|Select the market property from the drop-down menu or activate the property link for a market. For further information about creating properties for markets and price comparison portals, refer to the markets' pages in the topic **<<omni-channel/multi-channel#, Multi-Channel>>**.
|====
...
. Enter the **new value**.
. **Save** the settings. +
→ The property value will be changed for all items with this value.
...
. Click on **New**. +
→ The **New manufacturer** tab will open.
. Enter the name of the manufacturer.
. Carry out additional settings as desired. Pay attention to the information given in table 5.
. **Save** the settings. +
...
→ The manufacturer will open in a new tab.
. Change the manufacturer's data if needed. Pay attention to the information given in table 5.
. Open the **Commissions** tab.
. Click on **New**. +
→ A new line will be added to the table.
. Pay attention to the information given in table 6 and carry out the settings as desired.
. **Save** the settings. +
→ An ID will be assigned to the commission automatically.
...
→ The commissions will be transferred to the selected manufacturers.
...
. Go to **Item » Edit item**.
. In the **Manufacturer** filter, select the manufacturer from the drop-down menu.
. Select **ALL** in the **Active** filter.
. Click on **Search**. +
→ All items linked with this manufacturer will be displayed.
. Click on **Select all** in the header of the item overview.
. Click on **Item group function**. +
→ The **Item group function** window will open.
. In the **Global** area, activate the option **Manufacturers**.
. Select the empty option from the drop-down menu.
. Click on **Execute**. +
→ The manufacturer links will be deleted.
...
→ The **Delete manufacturers** window will open.
. Click on **Delete**. +
...
. Go to **Settings » Item » Barcode**.
. Click on **New barcode**. +
→ The **New barcode** window will open.
. Enter the name.
. Select the type. Pay attention to the information given in table 7.
. **Save** the settings.
. Open the barcode.
...
. Go to **Settings » Item » GTIN**.
. Enter the starting value. +
→ For 13 digit GTINs this must be a 12 digit value. The check digit is entered automatically.
. Enter the number of GTINs.
. **Save** the settings.

Assign the GTINs generated with GTIN numbering to your items in the **<<item/managing-items#250, Barcodes>>** area in the menu **Item » Edit item » Open item » Open variation » Tab: Settings**. Select the correct GTIN type and click on **Generate GTIN** in the **Code** column of this type. The next available GTIN will be inserted into the **Code** field automatically. To generate GTINs for several variations at the same time, use the **<<item/managing-items#330, Variation group function>>** or **<<item/managing-items#330, Variation batch processing>>**.
...
In the **Settings » Item » Images** menu, carry out basic settings for your item images. These settings relate to importing and exporting images, as well as to image sizes and image names.

[.instruction]
Carrying out settings for item images:

. Go to **Settings » Item » Images**.
...
|Enter the URL of a placeholder image. This will be shown if item images cannot be displayed. Upload a placeholder image in the **<<omni-channel/online-store/cms#image-gallery, Image gallery>>**.

|**URL of placeholder preview image**
|Enter the URL of a placeholder image. This will be shown if preview images cannot be displayed. Upload a placeholder image in the **<<omni-channel/online-store/cms#image-gallery, Image gallery>>**.
...
.Delete image cache
====
It may be necessary to clear the image cache after changing settings in the **Settings » Items » Images** menu. Images may be displayed with old settings because these settings are still saved in the cache. Click on **Delete image cache** if you want the changes to go into effect more quickly. +
...
Go to **Settings » Item » Images** and use the option **Image names of the FTP import** to choose whether the file names of the imported item images include the item ID or the variation No. Depending on this setting, you either name the files with the item ID or the variation No.
...
|**Class**
|The **Class** setting is not yet implemented. In the future, you will be able to link sales prices with customer classes with this setting.
...
. Go to **Settings » Item » Sales prices**.
. Select the **filters**. Pay attention to the information given in table 11.
. Click on **Search**. +
...
. Go to **Settings » Item » Price calculation**.
...
|**Referrer**
|Select an order referrer to be linked with the price calculation.
|====
...
Upload images in the **<<omni-channel/online-store/cms#image-gallery, Image gallery>>**. Copy the graphic's relative URL from the image gallery and insert it into the **URL** field.
...
Generate labels directly from the template or with the **Labels** function of an item or variation. You also can use the plentymarkets Client and the available processes to **<<basics/automation/processes/procedures#150, generate labels>>**.
...
In plentymarkets 7, you use **<<item/categories#, categories>>** for organising and managing your items. We recommend that you create all categories and subcategories at once. This will help you to structure and understand your product portfolio.
...
New categories are assigned the type **Item** by default. The settings required for categories of the **Item** type are described below. These settings are different from the category settings for the **Content** type. For further information about the remaining settings, refer to the **<<item/categories/editing-categories#, Editing categories>>** page of the manual.
...
. Select a template from the **Template page** drop-down menu.
. Select a template from the **Template list** drop-down menu.
. Select a template from the **Template product** drop-down menu.
. **Save** the settings.
...
Variation numbers are assigned automatically when you create **Default** items or variations. You specify the patterns of new variation numbers in the **Settings » Item » Basic settings** menu. Proceed as described below to specify the pattern for creating new variation numbers.

. Go to **Settings » Item » Basic settings**.
. Configure the settings **Prefix for new variation numbers** and **Next variation number**. Pay attention to the information given in table 15.
. **Save** the settings.
...
. Enter the content.
. Select a category.
. Carry out additional settings as desired. Pay attention to the information given in table 16.
. **Save** the settings. +
→ The **Default** item and its main variation will be created and a new tab will open.

Alternatively, create a new **Default** item by clicking on the arrow next to a category in the **<<item/categories#, Item » Categories>>** menu. In the context menu, click on **Create new category**.
...
|**Variation no.**
|The variation number is assigned automatically based on the pattern saved in the **Settings » Items » Basic settings** menu. Variation numbers can be changed subsequently in the **settings** tab of a variation.
...
|**Content**
|Enter the quantity and select a unit. The content is a mandatory entry. Learn how to create and edit units in the **<<item/managing-items#690, Managing units>>** chapter.
...
|**Condition; +
Condition for API**
|Select the item's condition. **New** is set by default. To display the condition in the detailed view of an item in the online store, insert the template variable **$condition** into the template **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewsingleitem, ItemViewSingleItem>>**. The **Condition for API** setting is used for the market **<<omni-channel/multi-channel/amazon#, Amazon>>**.
...
|Enter the customs tariff number. For further information, e.g. refer to the website link:http://auskunft.ezt-online.de/ezto/Welcome.do[**EZT-Online**, window="_blank"] (in German).

|**Revenue account**
|Enter the revenue account. For further information about the revenue account, refer to the **<<order-processing/orders/_accounting#90, Accounting>>** page of the manual.
...
**Special offer** = Use the **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewspecialofferslist, ItemViewSpecialOffersList>>** template to display a list of special offers. +
**New item** = Use the **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewlatestitemslist, ItemViewLatestItemsList>>** template to display a list of the latest items. +
**Top item** = Use the **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewtopsellerslist, ItemViewTopSellersList>>** template to display a list of the best selling items.

|**Item feedback**
|Enter a number between **1** and **5** or **1** and **10** for the initial item feedback. When using the item feedback function, the entered number will be displayed as the initial feedback of an item. Every time new feedback is submitted, an average value will be calculated automatically from these numbers. Go to **Settings » Client (store) » Global settings » Customer feedback » Tab: Item** to change the **Maximum rating** for feedbacks.
...
. In the **Basic settings** area, select the **manufacturer** from the drop-down menu.
. **Save** the settings. +
...
. Select the language.
. Enter the item texts. Pay attention to the information given in table 18.
. **Save** the settings.
...
|**Technical data**
|Enter the item's technical data. To display the technical data in the online store, insert the template variable **$TechnicalData** and, if needed, a title into the template **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewsingleitem, ItemViewSingleItem>>**.
...
|**Meta description**
|Enter the description which is used when searching for the item with search engines.
...
. Enter a field name.
. Select the data type. Pay attention to the information given in table 19.
...
Free text fields can be displayed at any position in the online store using the template variables **$Free[1]** to **$Free[20]**. Learn how to use free text fields in the online store on the **<<omni-channel/online-store/cms#web-design-editing-the-web-design-itemview-dimension-input, Dimension input>>** page of the manual.
...
. Carry out additional settings as desired. Pay attention to the information given in table 20.
. **Save** the settings.
...
. Carry out the optional settings as desired. Pay attention to the information given in table 21.
. **Save** the settings. +
...
.  Click on **Item group function**. +
→ The **Item group function** window will open.
.  Pay attention to the information given in table 23 and carry out the settings as desired.
...
. Carry out the settings in the **Basic settings** area. Pay attention to the information given in table 25.
. **Save** the settings.
...
|**Variation number**
|The variation number is automatically assigned based on the pattern saved in the **Settings » Items » Basic settings** menu. Variation numbers can be changed subsequently in the **settings** tab of a variation.
...
**__Note:__** If **No** is selected for the setting **Automatically sort by monthly sales** in the **Settings » Client (store) » Standard » Item layouts » Basic settings** menu, manually entered position numbers will be overwritten automatically.
|====
...
. Go to **Settings » Item » Item availability**.
. Pay attention to the information given in table 26 and carry out the settings as desired.
. **Save** the settings.
...
|**Language**
|Select the language from the drop-down menu. The system language that was set when the system was first created is selected by default.
...
[.instruction]
Activate availability options for the online store:

. Go to **Settings » Client (store) » Standard » Item layouts » Item availability**.
...
. Open the item. +
→ The **Settings** tab of the main variation will open.
. Pay attention to the information given in table 27 and carry out the settings in the **Availability** area.
. **Save** the settings.
...
|Activate to make the variation invisible in the item list. The variation will neither be displayed in a category nor will it be displayed with the search function. It will only be possible to access the variation with a direct link.
...
Link sales prices with the main variation of a **Default** item. The sales prices of the main variation are passed on to variations. You can also link individual sales prices with your variations.
...
. Open the item. +
→ The **Settings** tab of the main variation will open.
. Go to the **Barcodes** area and select the barcode from the drop-down menu.
. Click on **Add**. +
...
. Select **ASIN** or **ePID** from the drop-down menu.
...
. Open the **Default** item. +
→ The **Settings** tab of the main variation will open.
. Pay attention to the information given in table 28 and carry out the settings in the **Dimensions** area.
. **Save** the settings.
...
|**Content**
|Enter the content and select the unit from the drop-down menu. Learn how to create and edit units in the **<<item/managing-items#690, Managing units>>** chapter.
...
. Open the **Default** item. +
→ The **Settings** tab of the main variation will open.
. Pay attention to the information given in table 29 and carry out the settings in the **Costs** area.
. **Save** the settings.
...
|**VAT**
|Select the VAT rate for the main variation. For further information about VAT rates, refer to the **<<order-processing/orders/_accounting#3, Accounting>>** page of the manual.

|**Net purchase price**
|Enter the net purchase price. This value e.g. is used for **<<item/managing-items#910, price calculations>>**. In general, the purchase price is saved as information about the incoming items in the variation's **Stock** or **Suppliers** tabs. If you have not set up suppliers or incoming items yet, then the value that you entered here will be used to calculate the return on sales in the order. Otherwise, the average purchase price will be calculated automatically from the supplier and incoming item data. This average will then be used e.g. to calculate the return on sales for the item in an order.
...
. Open the **Default** item. +
→ The **Settings** tab of the main variation will open.
. Pay attention to the information given in table 30 and carry out the settings in the **Shipping** area.
. **Save** the settings.
...
. Enter the supplier information. Pay attention to the information given in table 31.
. **Save** the settings.
...
. Enter the warehouse information. Pay attention to the information given in table 32.
. **Save** the settings.
...
.  Go to **Item » Edit item**.
.  In the search, select **Variations** in the **Table type** drop-down menu.
.  Select the variations that you want to edit.
.  Click on **Variation group function**. +
→ The **Variation group function** window will open.
. Activate the check box to the left of the option you want to modify. Pay attention to the information given in table 33.
...
|**Prices**
|Activate to change the inheritance settings for the variations selected in the item overview. +
Check box to the right of **Prices** selected = Activates inheritance of the main variation's prices to the variations selected in the item overview. Prices saved for the variation are overwritten. +
Check box to the right of **Prices** not selected = Deactivates inheritance of the main variation's prices to the variations selected in the item overview. The main variation's prices are saved for the variation. +
...
|**Availability**
|Activate to change the availability level settings for the variations selected in the item overview. +
Check box to the right of **Availability** selected = Activates inheritance of the main variation's availability levels to the variations selected in the item overview. The availability level settings saved for the variation are overwritten. +
Check box to the right of **Availability** not selected = Deactivates inheritance of the main variation's availability levels to the variations selected in the item overview. The main variation's availability level is saved for the variation.

|**Markets**
|Activate to change the market availability settings for the variations selected in the item overview. +
Check box to the right of **Markets** selected = Activates inheritance of the main variation's markets to the variations selected in the item overview. The market availability settings saved for the variation are overwritten. +
Check box to the right of **Markets** not selected = Deactivates inheritance of the main variation's markets to the variations selected in the item overview. The main variation's active markets are saved for the variation. +
...
|**Clients (stores)**
|Activate to change the client availability settings for the variations selected in the item overview. +
Check box to the right of **Clients (stores)** selected = Activates inheritance of the main variation's client (store) availability settings to the variations selected in the item overview. The client (store) availability settings saved for the variation are overwritten. +
Check box to the right of **Clients (stores)** not selected = Deactivates inheritance of the main variation's client (store) availability settings to the variations selected in the item overview. The main variation's active clients (stores) are saved for the variation. +
...
.  Go to **Item » Batch processing**.
. Open the **Variations** tab.
.  Select the filters.  Pay attention to the information given in table 34.
. Carry out the settings as described in table 33.
...
Filters help you search for items by type and variations. In the **Table type** drop-down menu, select whether you want to search for items or variations. The table type **Extended** will display items in an extended view and grouped with all of their variations.

[.instruction]
Searching for items:

. Go to **Item » Edit item**.
. Select the **filters**. Pay attention to the information given in table 35. +
...
|Tags
|Enter a tag name to search for items that were linked to these tags in the **Global** tab. Go to *Settings » Basic settings » Tags* to create tags. +
_*Tip:*_ Enter the first letter of the tag and then use the auto completion feature.
...
|**Attributes**
|Select an attribute in the left drop-down menu or select **ALL**. Select an attribute value in the drop-down menu to the right or select **ALL** to search for items and variations with this attribute value.
...
. Open the item.
. Open the **Variations** tab.
. Select the **filters**. Pay attention to the information given in table 36.
. Click on **Search**. +
...
A search index is needed in order for items to be found more easily during an item search in the plentymarkets back end and in the online store. The search index will be set up automatically when new items are created. The search index will be updated the night after you imported the items and will be available the next day. In the **Settings » Item » Search index** menu, you can manually start updating the search index. In the **Item catalogue** tab, you can see how many items are included in the search index. In the **Item properties** tab, you can see how many items with properties are included in the search index. The following table explains the settings in the **Search index** menu.
...
. Go to **Settings » Item » Search index » Tab: Item catalogue » Tab: Default index**.
...
→ The **New item** tab will open.
. Select the type **Set**.
. Carry out the additional settings. Pay attention to the information given in table 38.
. **Save** the settings. +
...
In the **<<omni-channel/online-store/setting-up-clients/online-store#, basic settings>>** of the online store, you set up cross-selling. On the one hand, you select which type of cross-selling you want to use. On the other hand, you select whether or not to use **dynamic cross-selling**.

[.instruction]
Setting up cross-selling:

. Go to **Settings » Client (store) » Standard » Online store » Basic settings**.
. Carry out the cross-selling settings as described in table 40.
. **Save** the settings.
...
|**Perform cross-selling using item properties**
|Select if you want to display cross-selling items with the same properties as the main item.
...
|**Special cross-selling with item sets based on certain properties**
|Option currently without function.
...
. Click on the appropriate **relation**. Pay attention to the information given in table 41. +
...
Once you have specified which cross-selling items should be offered, decide how the cross-selling items should be displayed in the online store. Use the templates **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewcrosssellingitemslist, ItemViewCrossSellingItemsList>>**, **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewcrosssellingitemslist2, ItemViewCrossSellingItemsList2>>** and **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewcrosssellingitemslist3, ItemViewCrossSellingItemsList3>>** to specify how cross-selling items are displayed. For example, you can specify how many cross-selling items should be displayed for an item and how many of them should be displayed next to each other. Finally, specify where and which cross-selling items should be displayed for a main item. For this, insert a container in one of the templates for single items, e.g. **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewsingleitem, ItemViewSingleItem>>**.
...
The layout for cross-selling items is designed in the templates **ItemViewCrossSellingItemsList** to **ItemViewCrossSellingItemsList3**. The templates **ItemViewCrossSellingItemsList** and **ItemViewCrossSellingItemsList2** are ready to use in the plentymarkets default design Callisto Light. These cross-selling templates are displayed using the containers **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewcrosssellingitemslist, Container_ItemViewCrossSellingItemsList>>** or **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewcrosssellingitemslist, Container_ItemViewCrossSellingItemsList2>>**. In Callisto Light, these containers are already integrated within the template for single items **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewsingleitem, ItemViewSingleItem>>** and used as follows:

* **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewcrosssellingitemslist, Container_ItemViewCrossSellingItemsList>>** lists similar items {%$CrossSellingType = "Similar" %}.
* **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewcrosssellingitemslist, Container_ItemViewCrossSellingItemsList2>>** lists accessories {%$CrossSellingType = "Accessory" %}.
...
The selected top items are displayed in the online store using the template <<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewtopsellerslist, **ItemViewTopSellersList**>> in the **CMS » Web design » Folder: Layout » Folder: ItemView » Template: ItemViewTopSellersList**. Specify the sorting order in the **Settings** tab of the template. For further information about the templates for other store specials, refer to the following pages of the manual:

* Special offer: <<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewspecialofferslist, **ItemViewSpecialOffersList**>>
* New item: <<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewlastseenlist, **ItemViewLatestItemsList**>>

[#1030]
=== Positioning the top items

You can also display top items in the online store sorted by their position numbers. For this option, use the **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewitemsbypositionlist, ItemViewItemsByPositionList>>** template. Assign the position numbers either manually or automatically. To enter positions manually, select **No** for the setting **Automatically sort by monthly sales** in the **Settings » Client (store) » Standard » Item layouts » Basic settings** menu. If you select **Yes**, the positions will be assigned automatically. Any positions entered manually will be overwritten if this setting is selected. The automatic assignment is based on your monthly sales and thus will display your true top sellers.

[#1040]
==== Positioning top items manually

You can individually define your top items by assigning them position numbers manually. The template **<<omni-channel/online-store/cms-syntax#web-design-itemview-container-itemviewitemsbypositionlist, ItemViewItemsByPositionList>>** displays items according to their position. The items are displayed in descending order. This template can be used to display the manually sorted top items in your online store. The template is found in the menu **CMS » Web design » Folder: Layout » Folder: ItemView » Template: ItemViewItemsByPositionList**.
...
. Click on the **Filter** tab.
. Carry out the filter settings as desired. Pay attention to the information given in table 42.
. Click on the **Procedure** tab.
. Carry out the procedure settings as desired. Pay attention to the information given in table 42.
. **Save** the settings. +
...
__Image 1: Inserting the template variable and the additional text__
...
====
You can create large quantities of serial numbers at once with the data format **SerialKeys** in the menu **Data exchange » Export » Dynamic Export**. For further information about importing serial numbers, refer to the **<<basics/data-exchange/data-formats#, Data formats>>** page of the manual.
====
...
. Carry out the additional settings. Pay attention to the information given in table 43.
...
. Enter a name.
. Select the **event** listed in table 44.
. **Save** the settings.
. Pay attention to the information given in table 44 and carry out the settings as desired.
. Place a check mark next to the option **Active**.
. **Save** the settings.
--
...
Serial numbers can also be used in the plentymarkets Client. In order for serial numbers to be available for use in the plentymarkets Client, the **Serial number** option must have been activated in the item's **Global** tab.
...
The plentymarkets Client allows you to **<<basics/automation/processes/procedures#420, automate the registration of serial numbers>>**.
