...
But it's also possible to import item and variation data with <<welcome/assistants#, assistants>> or to send product data back and forth between your own plentymarkets system and other, external systems. For example, item data records can be:
...
* imported from external systems with the <<data/rest-api#, REST-API>>.
...
| **Revenue account**
|Enter up to 11 digits for the <<orders/accounting#90, revenue account>>.
...
| *Permitted* = Customers can redeem promotional codes when they buy a variation of this item. POS users can apply discounts to the item in plentyPOS. Customer class discounts are applied automatically. +
*Not permitted* = Customers cannot redeem promotional codes when they buy a variation of this item. POS users cannot apply any discounts to the item in plentyPOS. Customer class discounts are not applied. +
...
In the **Statistics** tab, you can create different statistics for the **Default** item. For example, statistics are available that let you analyse the **Default** item's revenue according to various criteria. For further information, refer to the <<business-decisions/business-intelligence/statistics#90, Managing statistics>> page of the manual.
...
|Sets the timestamp *Last change* to the current date and time.
...
The reason is that items are only exported to Amazon if they were edited within the last 24 hours.

| *Update price from in sets*
|This option only applies to <<item/use-cases/combining-products#3900, item sets>>. You can use this option to update the so-called "from-price" for several items at once.
...
*_Example_*: In some countries, children's clothing is taxed differently depending on its size.
In this case, it makes sense to save the customs tariff number for each individual variation.

*_Inheritance_*: Sub-variations automatically <<item/introduction/inheritance#, inherit>> the customs tariff number from the main variation. This means:
...
* _enter a different customs tariff number_ for all of the sub-variations that should differ from the main variation.
...
a|Activate and enter the <<orders/accounting#620, customs tariff number>>.
In plentymarkets, it's possible to save a different customs tariff number for each variation.
