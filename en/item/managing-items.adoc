...
Create items and item variations for your products in plentymarkets. Items can be sold in your <<online-store/online-store/setting-up-clients#, online-store>> and on the many <<markets#, markets>> integrated with plentymarkets.
...
Certain items, such as shoes and T-shirts, are typically available in different sizes and colours. Some items are available in different quantities, for example 50 ml or 250 ml. In plentymarkets, these characteristics of an item are called attributes. Attributes allow you to create variations of an item.
...
. Carry out the additional settings. Pay attention to the information given in <<table-create-attribute>>.
. **Save** the settings.
...
**Select with a box** = Select attributes by selecting a box. Attributes are displayed as boxes with attribute values in the online store. This setting is only possible when **Client check** was selected for the attribute's availability. For further information, refer to the <<online-store/online-store/setting-up-clients/online-store#_basic_settings_for_your_online_store#, basic settings>> chapter of the manual.

|*Groupable in item lists* +
(old online store Callisto only)
|You can use the <<online-store/online-store/setting-up-clients/cms-syntax#web-design-itemview-container-itemviewcategorieslist, ItemViewCategoriesList>> template of the old online store Callisto to display the available item variations. Specify the attribute which is used for displaying item variations, for example select the attribute **Colour** for shoes. In this way, all colour variations will be displayed first. After selecting the colour, the second attribute will be available in a drop-down list in the detailed view of an item, for example the **size**. The **$ItemViewCategoriesList_GroupByVariation** option is found in the settings of the Callisto template. Here you can select if you want to group by attributes or by variations.
...
. Carry out the additional settings. Pay attention to the information given in <<table-create-attributes>>.
. **Save** the settings.
...
In order to transfer your **Default** items to certain markets, you have to link attributes and attribute values for these markets. For further information, refer to the markets' pages in the topic <<markets#, Multi-Channel>> of the manual.
...
You can upload images for attribute values. These images will be displayed as image tiles in the online store. Specify the size of attribute images in the <<item/managing-items#670, System » System settings » Item » Images>> menu.
...
|**Property type**
a|Select the property type. Once a property has been linked to one or more items, it is no longer possible to change the property type. +
**No** = Do not select a property type. +
**Whole number** = Allows you to enter a whole number between -2,147,483,648 and 2,147,483,647 in the item's **Properties** tab, for example a size. +
**Decimal number** = Allows you to enter a number with decimal places in the item's **Properties** tab, for example a measurement. +
**Text** = Allows you to enter text in the item's **Properties** tab. +
**Selection** = Additional options will be displayed.

. Enter a name and description for the first selection that you wish to be displayed as a property, for example version 1.0.
. Save.
...
|**Order property**
|Activate to make the property available in the order process. Depending on the *property type*, the customer can for example enter a text or value or select an option from a drop-down list.

|**Note**
|Enter a note, for example to display specific information about the property.

|Property links for markets and price comparison portals
|Select the marketplace property from the drop-down list to link this property with the property of the marketplace. For further information about creating properties for markets and price comparison portals, refer to the markets' pages in the topic <<markets#, Multi-Channel>> of the manual.
|====
...
Properties are frequently occurring information that you can use in different areas of plentymarkets. Properties are centrally managed in the *System » System settings » Settings » Properties » Configuration* menu and can be used in the following areas of plentymarkets:
...
In contrast to attributes, properties are passive. As such, properties cannot be used to create variations. With the new properties, you can highlight recurring features of variations, for example technical details such as Bluetooth or Wi-Fi. Properties are not linked to stock and also cannot be selected in the detailed view of an item in the online store.

By using the new properties, you can provide services for an additional charge or integrate a text field for a customer's additional requests. Properties also allow you to link your items with the properties for variations of a market. Use the property search to find variations that are linked to a specific property.

[IMPORTANT]
.Important notes about the variation properties
====
However, you can create properties in the *System » System settings » Settings » Properties » Configuration* menu and also add values for the properties. You can also link these properties to variations in the *Item » Edit item » Open item » Open variation » Tab: Properties* menu. However, at this point in time you can only view the variation properties in the plentymarkets back end. The variation properties cannot be viewed in the online store or transferred to markets. These features will be available soon.
====

[#3100]
=== Creating a property

Go to *System » System settings » Settings » Properties » Configuration* to create properties. These properties are used for characterising your items. Proceed as described below to create a property.

[.instruction]
Creating a property:

. Go to *System » System settings » Settings » Properties » Configuration*. +
...
*Whole number* = Allows to enter a whole number between -2,147,483,648 and 2,147,483,647. +
...
Properties are centrally managed in the *System » System settings » Settings » Properties » Configuration* menu. Change the settings that are described in <<table-create-property>>.
...
Properties are centrally managed in the *System » System settings » Settings » Properties » Configuration* menu. After opening the menu, start the search to display the properties. In order to search for specific properties, enter a search term in the search fields. The drop-down lists will help you to search for properties that share specific criteria. You can combine as many search filters as you like.
...
*_Or:_* Limit the search results by using filters and click on *Search*. +
...
. Go to *System » System settings » Settings » Properties » Groups*.
...
. Go to **System » System settings » Item » Manufacturers**.
. Click on **New**. +
→ The **New manufacturer** tab opens.
. Enter the name of the manufacturer.
. Carry out the additional settings. Pay attention to the information given in <<table-add-manufacturers>>.
. **Save** the settings. +
...
. *_Optional:_* Change the manufacturer's data. Pay attention to the information given in <<table-add-manufacturer-commission>>.
...
. Go to **System » System settings » Item » Barcode**.
...
In the *System » System settings » Item » Images » Settings* menu, carry out the following settings for item images:
...
. Go to *System » System settings » Item » Images » Settings*.
...
|Enter the URL of a placeholder image. This image will be shown if item images cannot be displayed. Upload a placeholder image in the *CMS » Webspace* menu.

|*URL of placeholder preview image*
|Enter the URL of a placeholder image for preview images. This image will be shown if preview images cannot be displayed. Upload a placeholder image in the *CMS » Webspace* menu.
...
. Go to *System » System settings » Client » Select client » Domains*.
. Click on *Add*. +
...
→ The process is complete once green checkmarks are shown in the columns *SSL*, *DNS* and *Cloud status* of the *System » System settings » Client » Open client » Domains* menu in the plentymarkets back end.
. Once all three checkmarks are shown in the *System » System settings » Clients » Open client » Domains* menu, click on *Activate* in the *Status* column. +
→ The domain is activated.
...
In the *System » System settings » Item » Images » Sizes* menu, you select settings for each image size. The following sizes are available:
...
. Go to *System » System settings » Item » Images » Sizes*.
...
Examples of meta data are description texts, geo data (GPS) or rotation information. +
...
====
It may be necessary to clear the image cache after changing settings in the *System » System settings » Item » Images* menu. Images may be displayed with old settings because these settings are still saved in the cache. Click on *Invalidate CDN cache*, to show the changes more quickly. The cache invalidation is done by CloudFront.
...
Go to *System » System settings » Item » Images » Settings* and use the option *Image names of the FTP import* to choose whether the file names of the imported item images include the item ID or the variation No. Depending on this setting, you either name the files with the item ID or the variation number.
...
**Special offer** = Activate to mark the sales price as a special offer. Special offers are used for markets, for example <<markets/amazon#, Amazon>> and <<markets/real-de#, real.de>>. +
...
. Go to **System » System settings » Item » Price calculation**.
...
. Go to **System » System settings » Item » Units**.
. Click on **New**. +
...
Proceed as described below to create a label template. You can create up to 10 label templates.

[.instruction]
Creating a label template:

. Go to *System » System settings » Item » Labels*.
...
|*Page*
|Enter the page width and height in millimetres. When using <<basics/admin-tasks/printer#, label printers>>, the paper size can be the same size as the label size.
...
*Font size*; +
*Font style*
|Enter the coordinates for positioning the item data on the label. The values are based on a coordinate system with an x-axis and a y-axis. The X-axis runs from left to right and the Y-axis from top to bottom. The points of origin are in the upper left corner. +
...
|*Allow line break*
|Activate to allow line breaks. This means that a title, for example an item name that is longer than the width of the label, will continue on the next line.
...
Upload images in the *CMS » Webspace* menu. Copy the graphic's relative URL from the webspace and insert it into the *URL* field.
...
* Create processes to <<automation/processes/procedures#150, generate item labels>>.
...
[#3340]
==== Printing a label in the item search of the plentymarkets app
...
. Go to **System » System settings » Item » Flags**.
...
====
Instead of creating a new category in the *Item » Categories* menu, you can create a new category while creating a new item in the *Item » Edit item » New item* by clicking *New category*.
====
...
*_Tipp:_* The template settings only apply to the old online store design Callisto.
. **Save** the settings.
...
Variation numbers are assigned automatically when you create **Default** items or variations. You specify the patterns of new variation numbers in the **System » System settings » Item » Settings** menu. Proceed as described below to specify the pattern for creating new variation numbers.
...
Alternatively, create a new *Default* item by clicking on the arrow next to a category in the <<item/managing-categories#, Item » Categories>> menu. In the context menu, click on *Create new item*.
...
|**Variation no.**
|The variation number is assigned automatically based on the pattern saved in the **System » System settings » Item » Settings** menu. Variation numbers can be changed subsequently in the **settings** tab of a variation.
...
. Select the file that you would like to upload.
. Click on *Open*. +
*_Or:_* Drag and drop new files into the *Insert a new document* area. +
→ The file is uploaded and displayed in the overview.
...
**__Note:__** If **No** is selected for the setting **Automatically sort by monthly sales** in the **System » System settings » Client » Select client » Item layouts » Settings** menu, manually entered position numbers will be overwritten automatically.
|====
...
. Go to **System » System settings » Item » Availability**.
...
. Go to **System » System settings » Client » Select client » Item layouts » Availability**.
...
If a variation's availability was not activated as described, then the variation can not be ordered in the online store. The visibility of the variation in the online store depends on the type of verification selected for the attribute availability in the <<online-store/online-store/setting-up-clients/item-layouts#, System » System settings » Client » Select client » Item layouts » Settings>>.
...
If you want to maintain individual prices for your variations, deactivate the inheritance in the variation's tab and add a sales price. Click on **Delete** in the line of a sales price of the main variation or variation to delete the price. For further information about sales prices and price calculations, refer to the chapters <<item/managing-items#870, Managing sales prices>> and <<item/managing-items#910, Creating price calculations>>.
...
*_Or:_* Click on *Generate GTIN* to insert a code generated with <<item/managing-items#650, GTIN numbering>>.
. *Save* the settings.

[#260]
=== Linking ASIN and ePID

ASIN (Amazon Standard Identification Number) is product identification that is provided by <<markets/amazon/amazon-setup#, Amazon and omni-channel/multi-channel/amazon#, Amazon>>. An ePID (eBay Product ID) is a product identification that is provided by <<markets/ebay/ebay-setup#, eBay>>. The ASIN contains 10 characters and/or numbers. For books, the ASIN matches the ISBN. The ASIN can be found on Amazon's product page. The ePID is provided by eBay. Link ASIN and ePID with your variations.
...
|**VAT**
|Select the VAT rate for the main variation. For further information about VAT rates, refer to the <<orders/accounting#3, Accounting>> page of the manual.

|**Net purchase price**
|The net purchase price. The purchase price is calculated or managed manually depending on the settings saved in the <<stock-management/new-incoming-items#250, System » System settings » Stock » Stock » Incoming items>> menu. +
...
**__Note:__** During calculation, only items incoming to those warehouses activated for the moving average purchase price are taken into account. Go to *System » System settings » Stock » Stock » Incoming items* to activate the warehouses. If no warehouse is active, then no moving average purchase price is calculated.
...
Properties are centrally managed in the *System » System settings » Settings » Properties » Configuration* menu and can be linked to variations of *Default* items. Properties of the *Item* area are linked to variations in two different tabs of a variation:
...
Values entered are valid for the markets <<markets/neckermann/neckermann-at-setup#, Neckermann.at>>, <<markets/otto#, OTTO>>, <<markets/rakuten/rakuten-setup#, Rakuten>>, <<maerkte/plus-gartenxxl#, Plus.de>>. The buffer entered here takes priority over the buffer values entered for the particular market.
...
*_Or:_* Select the tag from the list.
. *Save* the variation. +
...
Go to *System » System settings » Item » Search » Back end » Search index* to set up searching for items in the *Item » Editing items*. To do so, you define those item parameters to be added to the search index. These parameters will then be searched during an item search in the back end.
...
In the *System » System settings » Item » Search » Back end » Search index* menu, you can start updating the search index manually.
...
. Go to *System » System settings » Item » Search » Back end » Search index*.
...
The *System » System settings » Item » Search » Back end » Search index » Tab: Overview* shows how many items are saved in the system and how many items are in the search index.
...
The *System » System settings » Item » Search » Back end » Search index » Tab: Item properties* shows how many items with properties are saved in the system and how many items with properties are in the search index.
...
|Tags
|Enter a tag name to search for variations that were linked to these tags. Go to *System » System settings » Settings » Tags* to create tags. +
*_Tip:_* Enter the first letter of the tag and then use the auto completion feature.
...
In the *Category* tab, you filter for items of a specific category.
...
. *_Or:_* To filter by subcategories, expand a category and click on the subcategory. +
→ The items found are displayed in the item overview.
...
. *_Or:_* Expand a client and click on a category. +
...
.YOOCHOOSE
====
The external service provider <<online-store/online-store/setting-up-clients/extras/yoochoose#, YOOCHOOSE>> offers an alternative to plentymarkets cross-selling.
====
...
Find out how to create **Cross-selling** item lists on the <<online-store/online-store/setting-up-ceres#110, Setting up item lists>> page of the manual.
...
Learn how to set up store specials in Ceres in the <<online-store/online-store/setting-up-ceres#110, Setting up item lists>> chapter of the manual.

Learn how to set up a store special in the old online store Callisto in the following example for **top items**. Set up other store specials in the same way.

[#1020]
=== Old online store: Displaying items as top items in Callisto
...
[#1050]
==== Old online store: Automatically positioning top items in Callisto
...
. Go to **System » System settings » Client » Select client » Item layouts » Settings**.
. Select **Yes** for the setting **Automatically sort by monthly sales**.
. **Save** the settings.

The template <<online-store/online-store/setting-up-clients/cms-syntax#web-design-itemview-container-itemviewitemsbypositionlist, ItemViewItemsByPositionList>> displays items according to their position. The items are displayed in descending order. This template can be used to display the automatically calculated top items in your online store.

For further information about setting up the different templates in the layout, refer to the <<online-store/online-store/setting-up-clients/item-sorting#, Item sorting>> page of the manual.

[#1080]
=== Old online store: Linking items to online store specials in Callisto
...
.Old online store: Linking items to online store special in Callisto
[cols="1,2"]
|====
|Setting |Explanation
...
[#720]
== Old online store: Managing item templates in Callisto
...
====
You can create large quantities of serial numbers at once with ElasticSync. For further information about importing serial numbers, refer to the <<data/importing-data/ElasticSync#sync-serial-number, Importing data with ElasticSync>> page of the manual.
====
...
→ The **Serial numbers** window opens.
. Select a status.
. *_Optional:_* Enter the serial number into the search field.
. Click on **Search**. +
...
|**Selection**
|Select a procedure, for example **Select all**, to activate all of the serial numbers. Select **Delete selection** to delete all activated serial numbers.
...
Serial numbers are assigned automatically with the help of an event procedure. The item needs to have been completely paid for before this can happen. The serial numbers will be displayed on the invoice that corresponds to the order. Set up an event procedure which automatically assigns a serial number once the order was fully paid for and the order status **incoming payment: complete** was assigned.
...
Processes allow you to automate the registration of <<automation/processes/procedures#420, serial numbers>>.
