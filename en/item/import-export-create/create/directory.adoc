...
This setting is only relevant if you want to <<markets/rakuten/rakuten-setup#, sell on Rakuten>>.
...
IMPORTANT: Files that are uploaded to the *Media* tab cannot be used in the Ceres online store.

In the *Media* tab, you can upload files for your items. Files, e.g. product information or data sheets, can be made available in the **My account** area or in the detailed view of an item in the online store. The files that you upload in the **Media** tab must meet the following requirements:
...
In the tab *Statistics*, you can analyse <<business-decisions/business-intelligence/statistics#90, statistical data about the item>>.
...
| *Position*
|Enter a position number for the variation. Variations are sorted in ascending order by position.

//zukunftsmusik - herausfinden, ob das nur Callisto betrifft
*__Note:__* Open the menu *Setup » Client » Select client » Item layouts » Settings*. If the option *Automatically sort by monthly sales* is set to *Yes*, then manually entered position numbers will be overwritten.
|====
...
Link <<item/settings/prices#, sales prices>> with the main variation of an item to save prices. You can enter monetary prices manually or you can use a so-called <<item/settings/prices#500, price calculation>> to have the prices automatically calculated. If inheritance is active, the combinations of sales price and price of the main variation are passed from the main variation to the variations of the item.
...
[TIP]
This data is used to <<item/settings/prices#500, calculate>> your sales prices.
...
| **Order picking**
|Specify the type of picking. If you do not want to define the order picking type, select the empty option.
...
|Enter a value for <<fulfilment/preparing-the-shipment#1000, calculating the extra shipping charge>> for the 1st and 2nd item.
|====
