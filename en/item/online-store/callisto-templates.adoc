...
:replaceshipping: \\{{shipping.profile}}
:replaceordertwo: \{\{order.id}}
:replaceshippingtwo: \{\{shipping.profile}}
...
. Finally, deploy the Ceres plugin.
...
<<welcome/quick-start/quick-start-online-store#270, Deploy the Ceres plugin>> and check what the template looks like in your plentyShop.
...
*_Beispiel:_* Ersetze `Order ID` durch `{replaceorder}` und `Shipping profile` durch `{replaceshipping}`.
...
*_Beispiel:_* Ersetze `Order ID` durch `{replaceordertwo}` und `Shipping profile` durch `{replaceshippingtwo}`.

*_Beispiel:_* Ersetze `Order ID` durch `{[anchor-one]{order.id}}` und `Shipping profile` durch `{[anchor-two]{shipping.profile}}`.
