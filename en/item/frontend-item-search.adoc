...
* *Faceted search* +
→ Find out how to set up the old faceted search in the <<item/frontend-item-search#220, Using Callisto with the old faceted search>> chapter.
...
Activate the old faceted search by going to *System » Client » Open client » Online store » Item search* and activating the option *Faceted search*. Set up the faceted search as described in the <<item/frontend-item-search#90, Setting up the faceted search>> chapter. Then proceed as described below to use the old faceted search in Callisto.
