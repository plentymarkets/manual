...
* *Findologic* +
For further information about setting up the item search using Findologic, refer to the <<omni-channel/online-store/extras/findologic#, Findologic>> page of the manual.
* *Faceted search* +
→ Find out how to set up the old faceted search in the <<xxxx#220, Using Callisto with the old faceted search>> chapter.
* *Faceted search (Elastic Search - BETA)* +
→ Find out how to use the new faceted search (Elastic Search - BETA) in the <<xxx#270, Using Callisto with the faceted search (Elastic Search - BETA)>> chapter.

[#220]
=== Using Callisto with the old faceted search

Activate the old faceted search by going to *System » Client » Open client » Online store » Item search* and activating the option *Faceted search*. Set up the faceted search as described in the <<xxx, Setting up the faceted search>> chapter. Then proceed as described below to use the old faceted search in Callisto.

[#230]
==== Callisto + old faceted search: Defining the search index
...
[#240]
==== Callisto + old faceted search: Adding fields to the search index
...
. For the activated fields, enter a boost between 1 and 1000. The higher the number, the more this area will be prioritized in the search results. Results will be displayed first for the field with the highest number.
. *_Optional:_* For the activated fields, activate the option *Fuzzy search*.
. *_Optional:_* Activate the option *Full text*.
. *Save* the settings.

[#250]
==== Callisto + old faceted search: Adjusting the fuzzy search
...
. Carry out the settings. Pay attention to the information given in <<table-fuzzy-search>>.
. *Save* the settings.


[[table-fuzzy-search]]
.Settings for fuzzy searching
[cols="1,3"]
|====
|Setting |Description

|*Character length*
|Enter the number of characters after which a fuzzy search is carried out.

|*Fuzzy search from*
|Enter a number. If the search returns less than this number of search results, a fuzzy search is run.

|*Full text search from*
|Enter a number. If the search returns less than this number of search results, a full text search is run.
|====

[#260]
==== Callisto + old faceted search: Activating fields for the full text search
...
. Click on the *Search index* tab. +
→ The *Fields* tab opens.
. Click on the *Full text* tab.
. Activate the fields to be searched when the option *Full text* is activated in the *Fields* tab.
. *Save* the settings.

[#270]
=== Using Callisto with the faceted search (Elastic Search - BETA)

Activate the new faceted search by going to *System » Client » Open client » Online store » Item search* and activating the option *Faceted search (Elastic Search - BETA)*. Set up the faceted search as described in the <<xxx, Setting up the faceted search>> chapter. Then proceed as described below to use the new faceted search in Callisto.
...
. Go to *System » Item » Search » Front end » Search settings*.
. In the column *Active*, activate those fields to be considered during a search.
...
. Go to *System » Item » Search » Front end » Languages*.
...
In Ceres, the faceted search is used by default. Set up the faceted search as described in the <<xxx, Setting up the faceted search>> chapter. Then proceed as described below to use the faceted search in Ceres.
...
For further information about setting up the faceted search in Ceres, refer to the <<omni-channel/online-store/setting-up-ceres#330, Setting up Ceres>> page of the manual.
