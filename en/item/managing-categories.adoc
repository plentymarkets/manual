...
|**Meta description**
|Enter a meta description. The meta description will be analysed by search engines. As such, it is especially important for the homepage. +
__**SEO tip:**__ The meta description should contain the most important content in one sentence. Currently recommended: max. 140 characters
...
. At least one item must be linked to the category for the client.
. Open the item and click on the **Categories** tab. Make sure that the category was set to **Available** for the desired clients.
. The item also has to be available, i.e. the stock has to be positive and the **Store visibility** option must have been set to **Visible** within the **Available** tab.
...
You can activate and/or deactivate several or all categories of a client (store) centrally in the **Settings » Client (store) » Standard » Categories » Activate** menu.
...
. **Save** the settings. +
→ The discount is applied automatically when a customer orders the required number of items in the online store. +
**__Note:__** These discounts are only applied for the default category of an item.
...
**Discount 2 (%)**; +
**Discount 3 (%)**
|Enter the discount in percent that is to be applied when the required number of items is reached.
