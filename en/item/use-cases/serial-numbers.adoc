...
====
Use the <<data/importing-data/ElasticSync#, import tool>> if you want to enter large quantities of serial numbers at the same time. Do so with the <<data/importing-data/sync-types/elasticSync-item#sync-serial-number, Serial number>> mapping field, which is available for the *Item* sync type.
====
