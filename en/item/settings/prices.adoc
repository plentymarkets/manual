...
* *Set price* = Sets are a way to <<item/use-cases/combining-products#, combine products>> and sell them as a unit. If you decide to combine products and sell them as a set, then you’ll also need to give the individual set components a set price.
...
* If you want to save prices _for lots of items_, then we would recommend using ElasticSync to <<data/importing-data/elasticsync-best-practices/best-practices-elasticsync-creating-variations#800, import the sales prices and monetary amounts>>.
Alternatively, you could also import this data with an <<basics/assistants#, assistant>>.
...
* If you want to save prices _for lots of items_, then we would recommend using ElasticSync to <<data/importing-data/sync-types/elasticSync-item#2300, import the price calculations>>.
Alternatively, you could use the <<item/managing-items#330, variation group function>> to set up price calculations for multiple item variations at once.
