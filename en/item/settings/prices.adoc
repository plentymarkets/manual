...
[#1000]
== New price calculation (Closed Beta)
...
====
This chapter describes the new price calculation, which was launched at the start of 2021.
This tool is currently in the "closed beta" phase.
This means that a small group of sellers are currently testing the tool.
Once we're sure it's performing correctly, we'll make the tool available on all beta systems and then on all stable systems.

There is also an old price calculation, which is no longer being developed and will be phased out in the future.
The old tool is <<item/settings/prices#2300, described here>>.
====
...
*_Example of use_* +
Imagine you sell items in the online shop and on Amazon.
You want the Amazon price to always be 10% more than in the online shop.
...
For example, you might want to calculate the prices for your online shop with this formula and the prices on Amazon with a different formula.
...
For example, if you selected the online shop as the target price in the first line, then you cannot select the online shop as the target price again in the second line.
* To avoid errors.
...
If you want to save the price calculation _for lots of items_, then we would recommend linking the price calculation via import.
Use the mapping field *Item variations / Price calculation UUID* to do so.
...
* <<data/importing-data/sync-types/elasticSync-item#2300, Information about the mapping field>>
=====
====
