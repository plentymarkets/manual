...
It’s technically possible to select attributes while <<item/import-export-create/create/new-item#200, manually creating a new item>>. However, we don’t recommend doing so.

Rather, we recommend that you first create a new item, i.e. a main variation, without any attributes. Afterwards, we recommend that you select attributes while you <<item/import-export-create/create/new-item#400, create additional variations>>.
