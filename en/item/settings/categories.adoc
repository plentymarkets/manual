...
If you only want to have _a few categories_, then we would recommend creating them manually. However, if you have _a lot of categories_, then it will probably be quicker to <<data/importing-data/elasticsync-best-practices/best-practices-elasticsync-categories#, import your category data>>.
...
====
It’s technically possible to create items and categories at the same time. In other words, you can create a new category while <<item/import-export-create/create/new-item#200, manually creating an item>>. This method can save you a lot of time but it might also cause you to lose track of your category structure.
====
...
Click on a category in the menu *Item » Category*. The category will open and you can continue editing it.
...
* *Blog* (icon:document[set=plenty, role="yellow"]) = Do you write a <<online-store/setting-up-ceres#970, blog for your online store>>? You can use blog categories to sort your entries into specific topic like “News” or “About us”.
...
*_Outdated "Callisto" webshop_*: Go to *Setup » Client » Select client » Item layouts » Settings* to specify the <<online-store/online-store/item-layouts#, sorting order>>.
Categories are sorted in ascending order by default.
...
//über ein include lösen. Auch in die Einleitung und auch im Import Kapitel
If you only want to have _a few categories_, then we would recommend <<item/settings/categories#300, creating them manually>>. However, if you have _a lot of categories_, then it will probably be quicker to <<data/importing-data/elasticsync-best-practices/best-practices-elasticsync-categories#, import your category data>>.

plentymarkets has an <<data/importing-data/ElasticSync#, import tool>>.
The idea is simple. Rather than manually saving category information in plentymarkets, you’ll <<data/importing-data/sync-types/elasticSync-categories#, enter the same information into a CSV file>> and then import this file into your system.
