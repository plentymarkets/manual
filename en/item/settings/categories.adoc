...
How do you want to structure your items in the online store?
...
* <<item/settings/categories#1400, Publish your items and categories in the online store>>
...
How will your customers find their way around the online store?
...
====
In the settings window, click on *Subcategory* (icon:dot-circle-o[role="blue"]) and select the category under which you want your new category to appear. Click on a category’s folder icon (icon:folder[role="darkGrey"]) on the far left. This displays the subcategories within.
====
...
====
New plentymarkets systems come equipped with a Ceres webshop and a ShopBuilder tool.
...
So if you want to use ShopBuilder to create your webshop, then you might also want to <<online-store/shop-builder#20, use ShopBuilder to create your categories>> too.
====

[#500]
==== Editing existing categories

Click on an existing category in the menu *Item » Category*. The category will open and you can continue editing it.
The editing window consists of the following areas:


* <<item/settings/categories#600, Toolbar>>
* <<item/settings/categories#700, Tab: Settings>>
* <<item/settings/categories#800, Tab: Description 1 and 2>>
* <<item/settings/categories#900, Tab: Documents>>
* <<item/settings/categories#1000, Tab: Note>>
* <<item/settings/categories#1100, Context menu>>
...
a|Do you manage the same category in several different languages or in several online stores? This symbol transfers the category’s settings to a different client or language version.
...
* *Content* (icon:document[set=plenty, role="yellow"]) = Content pages for the online store, e.g. information pages such as terms and conditions, payment information and shipping information.
* *Container* (icon:document[set=plenty, role="red"]) = This type of category is only relevant for older Callisto stores. Similar to free text field, the content of a category of the type Container can be displayed on a different page, e.g. on a different category. In addition, the order steps of the <<online-store/online-store/cms#web-design-editing-the-web-design-order-process-individual-order-process, individual order process>> are categories of this type.
* *Blog* (icon:document[set=plenty, role="yellow"]) = Do you write a <<online-store/setting-up-ceres#970, blog for your online store>>? You can use blog categories to sort your entries into specific topics like “News” or “About us”.
...
*_Outdated “Callisto” webshop_*: Go to *Setup » Client » Select client » Item layouts » Settings* to specify the <<online-store/online-store/item-layouts#, sorting order>>. Pages are sorted in ascending position order by default. +
...
.Uploading documents
====
You can either add documents by clicking on the icon:upload[set=plenty, role="purple"] *Upload file* button or by dragging and dropping them into the tab.
====
...
Are you responsible for multiple online stores? Or is your online store available in multiple languages? Do you perhaps use categories to create the individual <<item/settings/categories#intable-use-case, content pages of your store>>? Then filter your categories with the three drop-down lists at the top of the screen. This way you can limit which categories you’ll see in the navigation tree on the left.
...
====
Do you see category names displayed in various colours? Don’t worry! The colour simply indicates <<item/settings/categories#intable-use-case, what the category is used for>>.
====
...
====
Sometimes it makes sense to list an item in several different store categories. You can <<item/managing-items#300, manually link categories in the item data record>> or you can enter category IDs <<item/import-export-create/import#700, comma-separated in the import files>>.
====

[#1400]
== Displaying categories in the online store

Categories are displayed in the online store if they fulfil specific criteria.
...
[#1470]
=== Notes for older Callisto webshops
...
*_Tip_*: The default language of the online store is shown on the left side by default.
