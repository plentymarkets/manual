...
Many sellers do not have GTINs (barcodes) for their items because they directly import their items and the manufacturer does not assign GTINs. In this case, sellers can acquire GTINs from an organisation like link:https://www.gs1uk.org/[GS1^]{nbsp}icon:external-link[]. For example, sellers can buy a range of 1000 barcode numbers.
