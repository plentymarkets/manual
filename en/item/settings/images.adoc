...
:keywords: Image, Images, Item image, Item images, Variation image, Variation images, Image file, Image files, S3, CDN, CloudFront, FTP, FTP server, FTP servers, FTP import, Scaling, Image scaling, Resizing, Scale, Resize, Placeholder images, Placeholder image, Attribute image, Attribute images, Size, Image size, Image sizes, Height, Width, Image name, Image names, Prefix, Preview image, Preview images, Pixel, Pixels, Image URLs, Image URL, Domain, Domains, Subdomain, Subdomains, preview, secondPreview, middle, full, EXIF, XMP, IPTC, enlarge, Original image, Compression, Compression level, Progressive
...
.Additional settings
====
Upload the necessary images and specify that you want the attributes to be displayed as images in the webshop.
...
| *Quality in percent*
|Enter the level of JPG compression as a percentage.
Compression reduces load times and can help you achieve better page speed values.

*_Tip_*: Compressing images to 80 % results in no visible loss of quality.
...
a|Enter a number between 0 and 9. This is the compression level that will be used for PNG files.
...
In this case, click on icon:undo[role="orange"] *Invalidate CDN cache* to see the changes more quickly.
...
*_Note_*: This process can take several hours. The process is complete when green checkmarks are shown in the columns *SSL*, *DNS* and *Cloud status*.
...
. When you open the menu *Setup » Client » Select client » CDN-Domains*, do you see green check marks displayed in the columns *SSL*, *DNS* and *Cloud status*? Then the process is complete.
Once all three checkmarks are shown, click on *Activate* in the *Status* column. +
→ The domain is activated.
