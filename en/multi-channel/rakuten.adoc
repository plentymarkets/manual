...
You need to register with Rakuten before setting up Rakuten in plentymarkets. Use the registration forms of the platforms link:http://leadgen.rakuten.de/form?leadsource=marketplace_header[**rakuten.de**, window="_blank"] and link:https://merchant-enquiry.rakuten.uk.com/[**rakuten.co.uk**, window="_blank"].
...
. Go to **Settings » Markets » Rakuten » Basic settings**. +
...
**Activate** = Order confirmations are transferred to the customer. The email template saved under link::/[**Settings » Client (store) » Standard » Email » Automatic email**] will be sent to the customer.
...
To set up an account for Rakuten.co.uk, refer to the chapter <<multi-channel/rakuten#80, **Setting up Rakuten.co.uk in plentymarkets** >>.

[#30]
=== Setting the item availability

Items have to be available for Rakuten.de. This is done in the **Availability** tab of an item within the link::/[**Item » Edit item » Tab: Variation ID**] menu.
...
Shipping profiles are set up directly on Rakuten. The shipping profiles that are saved for items will not be transferred to Rakuten. On Rakuten.de, the shipping profiles are called shipping groups (Versandgruppen). These shipping groups are linked to your items with a link::/[**free text field**].
...
Link the properties in the link::/[**Properties**] tab of an item.

The energy label is saved in the item's link::/[**Images**] tab. To do so, proceed as follows.

[.instruction]
Saving the energy label in the Images tab:

. Go to **Item » Edit item**.
. Open the item and upload an image of the energy label to the **Images** tab.
. Give this image the position number that you use for energy labels.
. Go to **Settings » Markets » Rakuten » Basic settings**.
. Click on the **New account** tab.
. Expand the **Rakuten item export** submenu.
. Pay attention to the explanations given in table 2 and enter the position number of the image into the **Image position for energy label** field.
. **Save** the settings.

Make the product data sheet available for Rakuten in electronic form. This is done by inserting a link in the item description. The link to the product data sheet is saved in an item's link::/[**Texts**] tab. To do so, proceed as follows.
...
. Go to **Settings » Markets » Rakuten » Basic settings » Tab: New account » Submenu: Rakuten settings**.
...
Items have to be available for Rakuten.co.uk. This is done in the **Availability** tab of an item within the link::/[**Item » Edit item » Tab: Variation ID**] menu.
...
. Go to **Settings » Markets » Rakuten » Category link » Rakuten.co.uk**.
. Click on **Search**.  +
...
For further information, refer to the **<<start/statistics#, Statistics  >>** and **<<start/statistics/general-statistics/item/tops-flops#, Tops/Flops  >>** pages of the manual. For an example for flat rate and percentage costs, refer to the **<<multi-channel/rakuten#20, Basic settings >>** page of the manual.
|====
...
For further information, refer to the **<<multi-channel/rakuten#40, Payment methods and customer classes >>** chapter of the manual.
====

[TIP]
.Payment instruction on the <<client-store/standard/documents/invoice#, Invoice  >> document for the payment method Rakuten Rechnung
====
...
Rakuten uses standard delivery periods. However, the market indicates deadlines rather than a time period spanning several days. These deadlines are always the last day in each timespan. Shipping and delivery periods for Rakuten are assigned automatically according to the link::/[**average period of delivery**]. For example, if the **item availability** lists an average delivery time of 2 days, then there wouldn't be an exact equivalent for Rakuten. Therefore, the next longer shipping and delivery period is used for Rakuten.
...
Set up an **<<orders/settings/event-procedures#, event procedure  >>** to automatically send shipping confirmations to Rakuten when the outgoing items are booked.
...
Set up an **<<orders/settings/event-procedures#, event procedure  >>** to automatically send returns to Rakuten.
...
Set up an **<<orders/settings/event-procedures#, event procedure  >>** to automatically send cancellations to Rakuten.
