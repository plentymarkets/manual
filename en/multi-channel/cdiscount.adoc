...
For further information, refer to the **<<start/statistics#, Statistics  >>** and **<<start/statistics/general-statistics/item/tops-flops#, Tops/Flops  >>** pages of the manual. For an example for flat rate and percentage costs, refer to the **<<payment/managing-payment-methods#20, Basic settings >>** page of the manual.
|====
...
In rare cases, the automatic item export may not work correctly. In such cases you can export your French item catalogue as a CSV file to Cdiscount. To do so, go to the <<data-exchange/exporting-data#, **Data exchange » Export » Elastic export**  >> menu and carry out the settings as described below. After you have carried out and saved all necessary settings as described below, copy the complete URL and send it to Cdiscount. The hourly stock synchronisation is still carried out automatically.
...
. Pay attention to the explanations given in the <<data-exchange/exporting-data#20, **Exporting data** >> page of the manual and carry out the settings.
. **Save** the settings. +
...
Set up an **<<orders/settings/event-procedures#, event procedure  >>** to automatically send returns to Cdiscount.
...
Set up an **<<orders/settings/event-procedures#, event procedure  >>** to automatically update the order status on Cdiscount whenever specific events occur. You can use event procedures to automatically give orders the following statuses on Cdiscount:
