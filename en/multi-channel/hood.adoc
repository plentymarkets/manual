...
====
Orders from Hood will be imported into your plentymarkets system every hour and will be available in the <<orders#, **Orders**  >> menu. You can recognise Hood orders by the information displayed in the **Referrer** column. For lack of space, only the ID 125 is displayed. Point your cursor over an ID to view the referrer.
====

[#300]
=== Displaying variation images

In Hood, variation images cannot be linked to selected variations, i.e. variation images are treated like item images. Therefore, change the number of images so that variation images are also displayed on Hood. This setting can only be changed once you have <<multi-channel/hood#230, **created a listing** >>. Proceed as follows to display variation images in Hood after creating a listing.
...
Create additional shipping profiles the same way. The shipping profiles are then available in the **Base** tab of a <<multi-channel/hood#290, **listing** >>.
...
Enter your feedback texts first. Then set up an <<orders/settings/event-procedures#, **event procedure**  >> to automatically send your feedback texts to your buyers. All buyers who gave you feedback will automatically receive a random feedback text in return.
...
Set up an <<orders/settings/event-procedures#, **event procedure**  >> to automatically send a shipping confirmation to Hood when the outgoing items are booked.
...
Set up an <<orders/settings/event-procedures#, **event procedure**  >> to automatically send payment confirmations to Hood.
...
deactivated = The listing will only be listed if there is sufficient stock available. In this case, sufficient means that stock is larger than the <<multi-channel/hood#290, **maximum quantity for sale** >> selected in the listing.
...
The **<<multi-channel/hood#320, duration >>** and the **<<multi-channel/hood#50, payment methods >>** of the listing will be ignored for Hood listings of the type Store item. The payment methods for store items are centrally set up in the store and not for each store item.
...
|**Listing template**
|Select a **<<multi-channel/hood#450, Listing template >>**.
|====
...
For further information about the settings of a market listing, refer to the <<multi-channel/hood#270, **Editing listings** >> chapter.
...
image::multi-channel/assets/EN-Multi-Channel-Hood-01.png[]
...
|**Listing scheduler**
| Select a listing scheduler for the market listing. Select a <<multi-channel/hood#160, **listing scheduler** >> in the **Activate listing scheduler** window.

|**Start**
| Start market listing. Select a starting time in the **Start market listing** window.

|**Listing template**
| Apply a listing template to the market listing. Select a <<multi-channel/hood#450, **listing template** >> in the **Apply listing template** window.
...
|**Warehouse**
|Select a warehouse or **use the basic setting** that was selected for the **Warehouse selection** option in the <<multi-channel/hood#140, **Stock** >> menu.
...
|**Stock dependence**
| Select the **<<multi-channel/hood#240, stock dependence >>** of the listing.

|**Number per lot**
| Define the quantity of items offered in a single listing. The listing price applies to the entire quantity.

|**Transfer as item number**
| Select which number should be transferred as the **<<multi-channel/hood#240, item number >>**.
...
For further information about the settings of a listing, refer to the <<multi-channel/hood#270, **Editing listings** >> chapter.
...
.  Go to **Listings » Batch processing » Listings**.
. <<multi-channel/hood#360, **Filter** >> the listings.
.  Activate the option **Copy listings** in the **Procedures** section.
.  Click on **Execute** to copy the listings.
...
|**Stock dependence**
| Select a **<<multi-channel/hood#270, stock dependence >>** or **ALL** to only display listings with the selected stock dependence.
...
In plentymarkets, there are multiple ways to automatically start listings or restart listings once they were already started. Use the listing scheduler to specify **starting times** for listings. Go to <<multi-channel/hood#160, **Settings » Listings » Listing scheduler** >> to set up the listing scheduler.
...
.  Go to **Listings » Active listings**.
.  Open a market listing.
.  Click on the <<multi-channel/hood#290, **Base** >> tab and activate the **Endless lister**. +
...
.  Click on **Group function**.  +
→ The **Group function** window will open.
.  Carry out the settings as desired. Pay attention to the information given in table 16 in the <<multi-channel/hood#320, **Editing multiple listings** >> chapter.
