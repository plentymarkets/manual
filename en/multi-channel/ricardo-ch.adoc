...
First register with **link:https://www.ricardo.ch/registration/schritt1[ricardo.ch, window="_blank"]**. After you have finished registration, use the access data to set up your ricardo.ch account in plentymarkets. Please note that the ricardo.ch website is currently only available in German and French.
...
Go to **Settings » Markets » ricardo » Accounts** to activate new accounts for ricardo.ch. You need to have your access data consisting of the user name and password. Proceed as follows to add one or more accounts.

[.instruction]
Activating a new account:

.  Go to **Settings » Markets » ricardo » Accounts**.
.  Click on **Add account**. +
→ The window **Unlock or change account** will open.
.  Enter a name for the **Account**.
.  Click on the **gear-wheel icon** to unlock your account. +
→ You will be forwarded to ricardo.ch and will have to authenticate your data.
...
A token can be renewed no earlier than 24 hours before the the regular duration ends.
...
====
You can top up your ricardo.ch credit balance directly on ricardo.ch. You need to have credit in order to **<<multi-channel/ricardo-ch#360, start >>** listings.
====

Go to **Settings » Markets » ricardo » Accounts** to rename or delete accounts.
...
The setting **Update options** in the **Settings » Markets » ricardo » Accounts** menu allows you to update your listing settings and to fix problems, e.g. if no payment method of a ricardo.ch listing has been selected.
...
The setting **Synchronise offer templates** allows you to synchronise your ricardo.ch link:http://www.ricardo.ch/verkauferportal/de-ch/ratgeber/beitr%C3%A4genachverkaufsphasen/einstellen/angebotsvorlageninmyricardo.aspx[**offer templates**, window="_blank"] with plentymarkets. The offer templates will then be available for your ricardo.ch listings in the **Listings » Edit listings** menu. You will find them in the **Market** tab in the **Link to template** drop-down menu.

[#600]
===  Saving text templates for ricardo.ch

Go to **Settings » Markets » ricardo » Basic settings** to save texts for **Terms of guarantee**, **Payment conditions** and **Delivery conditions** as well as texts for **Automatic feedback**. You can enter German and French text templates in the text fields. Expand **Other** to set which item name is to be used in the listing title or listing subtitle for the second language.
...
Payment conditions; +
Delivery conditions**
| Enter the condition texts for ricardo.ch in German and French. The texts will be used in those **listings** for which you have selected the option **According to the description** in the drop-down menus for **Warranty**, **Payment conditions** and **Delivery conditions** in the <<multi-channel/ricardo-ch#270, **Listings » Edit listings » Tab: Market** >> menu.

|**Automatic feedback**
| Enter up to four feedback texts. These texts will randomly be selected by plentymarkets and used to evaluate the purchaser.
...
Set up an **<<orders/settings/event-procedures#, event procedure  >>** to automatically send feedback to ricardo.ch.
...
.  SelecSelect the referrer **ricardo**.
.  Enter the SKU.
.  Click on **Add**. +
...
Go to **Settings » Listings** to carry out the settings for all listings regardless of the market. In order to create listings in plentymarkets, you must have carried out the following settings and finished creating your **<<item#, items  >>**.
...
| Select **Name 1**, **Name 2** or **Name 3**. This item name will be transferred. Item names are saved within the item's **link::/[Texts]** tab.
|====
...
deactivated = The listing will only be listed if there is sufficient stock available. In this case, sufficient means that stock is larger than the <<multi-channel/ricardo-ch#250, **maximum quantity for sale** >> selected in the listing.
...
plentymarkets provides you with multiple possibilities for creating listings. Make sure that you create your **<<item#, items  >>** first. Then create your listings in the item overview using batch processing or via an existing listing. Use the data format **<<data-exchange/data-formats/itemlistingmarketricardo#, ItemListingMarketRicardo  >>** in order to automatically create large quantities of items.
...
**transmit UPC** = The Universal Product Code will be transferred. In order to use this option, an EAN has to be selected for the setting **link::/[UPC from EAN field]**. +
...
|**Listing template**
| Select a **<<multi-channel/ricardo-ch#410, Listing template >>**.
|====
...
For further information about the settings of a market listing, refer to the <<multi-channel/ricardo-ch#230, **Editing listings** >> chapter.

[#2200]
===  Creating multiple listings

Create multiple listings with the **Group function** or the **Batch processing** function in the **Item** menu. The group function creates listings for previously selected items. The batch processing function creates listings for previously filtered items. If no **listing template** is selected in the menu **Item » Edit item » Tab: Other » Tab: eBay options**, then eBay listings will be created automatically. If you want to create multiple ricardo listings, then make sure you selected a **listing template** with specific settings for ricardo in the menu **Item » Edit item » Tab: Other » Tab: eBay options**.
...
. <<multi-channel/ricardo-ch#320, **Filter** >> the items.
.  Activate the option **Create listing** in the **Procedures** area. +
→ Irrelevant settings will be greyed out.
.  Click on **Apply** to create the listings.
...
.  Carry out the settings as desired. Pay attention to the explanations given in tables 10 to 13.
. **Save** the settings.
...
|**Listing scheduler**
| Select a listing scheduler for the market listing. Select a <<multi-channel/ricardo-ch#120, **listing scheduler** >> in the **Activate listing scheduler** window.

|**Start**
| Start market listing. Select a starting time in the **Start market listing** window.

|**Listing template**
| Apply a listing template to the market listing. Select a <<multi-channel/ricardo-ch#410, **listing template** >> in the **Apply listing template** window.
...
|**Warehouse**
| Select a warehouse or **use the basic setting** that was selected for the **Warehouse selection** option in the <<multi-channel/ricardo-ch#100, **Stock** >> menu.
...
|**Warranty**
| Select **No warranty** or **According to the description**. Enter a descriptive text under <<multi-channel/ricardo-ch#60, **terms of guarantee** >>.

|**Payment conditions**
| Select **According to the description**, **upon pick-up** or **in advance**. Enter a descriptive text under <<multi-channel/ricardo-ch#60, **payment conditions** >>. As an alternative, the customer pays upon pick-up of the item or before receiving the item.
...
|**Delivery conditions**
| Select the delivery conditions. Enter a descriptive text for the option **According to the description** under <<multi-channel/ricardo-ch#60, **delivery conditions** >>.
...
|**Additional options**
| Activate one, several or all options. Further information about the additional options for ricardo can be found on the link:http://www.ricardo.ch/ueber-uns/de-ch/geb%C3%BChren/einstelloptionen.aspx[**ricardo website**, window="_blank"] (German and French only).

|**Increment**
| Enter an amount. Customers will need to increase the current offer by this amount.
...
| Activate the secondary language. It is possible to list ricardo listings in German and French at the same time. The secondary language will be exported from the item master data. It is therefore necessary that you save all descriptions for the item in both languages.

|**Link to template**
| Select an link:http://www.ricardo.ch/verkauferportal/de-ch/ratgeber/beitr%C3%A4genachverkaufsphasen/einstellen/angebotsvorlageninmyricardo.aspx[**offer template**, window="_blank"] which should be used for the listing (German and French only).
|====
...
|**Transfer as item number**
| Select which number should be transferred as the **<<multi-channel/ricardo-ch#200, item number >>**.
...
.  Select the filters. Pay attention to the information given in table 15.
.  Carry out the settings as desired. Pay attention to the information given in table 14.
.  Click on **Execute** to apply the settings to the listings.
...
For further information about the settings of a listing, refer to the <<multi-channel/ricardo-ch#230, **Editing listings** >> chapter.
...
.  Go to **Listings » Batch processing » Listings**.
. <<multi-channel/ricardo-ch#320, Filter >> the listings.
.  Activate the option **Copy listings** in the **Procedures** area.
.  Click on **Execute** to copy the listings.
...
|**Stock dependence**
| Select a **<<multi-channel/ricardo-ch#200, stock dependence >>** or **ALL** to only display listings with the selected stock dependence.

// tr> <td> <strong>Varianten</strong></td> <td> Nach Listings mit oder ohne Varianten filtern oder <strong>ALLE</strong> anzeigen. Für ricardo die Option <strong>ALLE</strong> wählen.</td> </tr

// tr> <td> <strong>Sonderaktion</strong></td> <td> Nach Listings mit der ohne Sonderaktion filtern oder <strong>ALLE</strong> anzeigen.</td> </tr
...
.  Select a starting time. Pay attention to the explanations given in table 17.
...
In plentymarkets, there are multiple ways to automatically start listings or restart listings once they were already started. Use the listing scheduler to specify **starting times** for listings. Go to <<multi-channel/ricardo-ch#120, **Settings » Listings » Listing scheduler** >> to set up the listing scheduler.
...
.  Go to **Listings » Active listings**.
.  Open a market listing.
.  Click on the <<multi-channel/ricardo-ch#250, **Base** >> tab and activate the **Endless lister**. +
...
.  Click on **Group function**.  +
→ The **Group function** window will open.
.  Carry out the settings as desired. Pay attention to the information given in table 14 in the <<multi-channel/ricardo-ch#230, **Editing listings** >> chapter.
...
All active listings are displayed in an overview in the **Listings » Active Listings** menu. Use the filters for <<multi-channel/ricardo-ch#330, **settings** >>, <<multi-channel/ricardo-ch#340, **categories** >> or <<multi-channel/ricardo-ch#350, **directories** >> to narrow down the listings that are displayed. In addition to the filters described in table 16, the following options are available:
...
.  Click on **Group function**.  +
→ The **Group function** window will open.
.  Pay attention to the explanations given in table 19 and select the procedure.
...
Batch processing includes the same options as the group function. However, the batch processing function edits previously filtered listings. The filters used for narrowing down listings are the same as described for search filters in tables 16 and 18. The procedures are the same as described for group functions in table 19.

[.instruction]
Editing active listings with batch processing:

.  Go to **Listings » Batch processing » Active listings**.
.  Select the filters. Pay attention to the information given in tables 16 and 18.
...
The menu **Data exchange » API log » Tab: API log** contains a history of processes that run via the ricardo.ch interface. The **ItemListingControllerRicardo** format is available for ricardo.ch.

[.instruction]
Viewing the API log:

.  Go to **Data exchange » API log**.
.  Select the **ItemListingControllerRicardo** or **LeaveFeedback** format from the **Process** drop-down menu.
...
[.indented]
**A1**: In the ricardo options, you selected **According to the description** for this listing's guarantee, payment conditions or delivery conditions. However, you haven't saved these general conditions yet. To do so, go to **Settings » Markets » ricardo » Basic settings** and save the conditions in the submenus **Terms of guarantee**, **Payment conditions** and **Delivery conditions**.[.instruction]
I am unable to select payment methods in the ricardo settings of a listing.

[.indented]
Go to **Settings » Markets » ricardo » Accounts** and update the options for the ricardo markets in the **Update options** area.[.instruction]
My listing was not started or I receive the error message Debited. What could be the problem?

[.indented]
You do not have enough credit on your ricardo account or you have reached your credit limit. Top up your account or apply for a credit limit with ricardo.[.instruction]
I receive the error message TokenErr. What can I do?

[.indented]
Go to **Settings » Markets » ricardo » Accounts** and update the token.[.instruction]
I receive the error message InvalidPromotion. What is the problem?

[.indented]
One of the additional options is invalid. Check the additional options that were selected.[.instruction]
