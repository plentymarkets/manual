...
:position: 70
:url: quick-start/quick-start-shipping-options
:id: ELFJKWG

IMPORTANT: Have you already set up the payment methods PayPal and cash in advance? Not yet? Return to <</quick-start/quick-start-setting-up-payment-processing#, payment processing>>.

On this page, we tell you how to prepare your system so it is ready for shipping items: You determine to which countries you send your items and group these countries into regions. Grouping countries into regions is necessary for saving the shipping costs since shipping costs cannot be saved for single countries of delivery, but for regions only. Furthermore, you set up the shipping service provider and the shipping profile including the shipping costs.
...
Activated countries of delivery must be grouped into regions. Creating regions is necessary since you can determine the shipping costs only for regions and not for single countries. Group all countries of delivery with the same shipping costs and the same shipping service provider-specific services to one region.
...
→ *Name (en)* is visible to your customers in the online store as well as on invoices. *Name (back end)* is not visible to customers. It serves for internal use for you and your employees.
...
.Tracking URL
====
The tracking URL is used to track shipments. If you save the URL in the *System » System settings » Orders » Shipping » Settings » Tab: Shipping service provider* menu and add the template variable *$TrackingURL* to the email template for the shipping confirmation, your customer can click on the URL in the email to track when the parcel will arrive.
...
In plentymarkets, a shipping profile is required so that right from the very beginning the correct shipping conditions are available for the order of your customer and the shipping process can be processed properly.

////
TODO
In order that plentymarkets knows how an item is to be shipped and only this shipping profile can be selected in the online store by your customer, you have to link the item with the shipping profile afterwards.

Link to item page --> Further information can be found in the area
////
...
. Go to *System » System settings » Orders » Shipping » Settings*.
. Click on the *Shipping profile* tab.
. Click on the DHL shipping profile. +
→ The *Base* tab opens.
. Carry out the settings. Pay attention to the information given in <<table-set-up-shipping-profile>>.
. *Save* the settings.
...
|*Name*
|Enter a name that is shown to your customers in the online store.
...
|*Priority*
|The highest priority is selected by default. The priority determines the order in which shipping profiles are shown during the checkout process in the online store.
...
After you have created the shipping profile, you should take care of the shipping costs. For each region that you created, an individual table of shipping charges must be set up.
...
The DHL Shipping (Versenden) plugin is already installed and activated in your plentymarkets system in the *Plugins » Plugin overview* menu. Therefore, it is visible in the *System » System settings » Orders » Shipping » DHL Shipping* menu. You have to carry out some settings in this menu:

* Carrying out the basic settings
* Saving the DHL billing number
* Saving the shipper data
* Carrying out shipping settings
* Activating the productive mode

Please have a look at the link:https://marketplace.plentymarkets.com/en/plugins/integration/DHLShipping_4871[user guide of the DHL Shipping (Versenden) plugin^] in the plentyMarketplace. In the user guide, all settings are described that you need to carry out in order to send your items via the interface for DHL Shipping.
...
* [ ] Item: Did you link your items with the shipping profile?
* [ ] Shipping profile: Did you select the option *Activate for new items* for at least one shipping profile?
* [ ] DHL plugin: Did you carry out all settings in the *System » System settings » Orders » Shipping » DHL Shipping* menu?

[#800]
== More about shipping options

* <<crm/managing-contacts#15, Creating a customer class>>
* link:https://marketplace.plentymarkets.com/en/plugins/payment[Methods of payment^]
...
* <<fulfilment/shipping-centre#register-shipping-order, Registering a shipping order>>
* <<automation/event-procedures#, Creating event procedures>>
* <<fulfilment/best-practices-dhl#_dhl_troubleshooting#, DHL troubleshooting>>

TIP: Continue to <<quick-start/quick-start-catalogue-of-items#, Setting up a catalogue of items>>
