...
:position: 140
:url: quick-start/quick-start-online-store
...
This chapter is divided into three sections: In a first step, you will carry out important <<#510, preparations>>, so that your online store will work properly. In the next step, you need to carry out relevant settings in the *Ceres configuration*. Lastly, you will design the appearance of your online store with the *ShopBuilder*, the plentymarkets construction kit system for online stores.
...
. Enter the text for your terms and conditions. The plugin Ceres does not use the tab *HTML*.
. *Save* the settings.
...
Set up a default country of delivery, a default shipping method, a default shipping profile and a default payment method. These are preselected for non-registered visitors of your online store. You can only use *plugin payment methods* in your Ceres online store, which you can download from the link:https://marketplace.plentymarkets.com/plugins/payment[plentyMarketplace^].
...
The *System » System settings » Client » Select client » Email » Automatic* menu contains various events you can link with an email template. Select the email template that you have previously created in the *System » System settings » Client » Select client » Email » Templates* menu from the drop-down list. This ensures that the email template is automatically sent to your customer once the event occurs.
...
.Explanations for several events in the *System » System settings » Client » Select client » Email » Automatic* menu
[cols="1,3"]
|====
|Setting |Explanation
...
[#60]
== Setting up online store plugins
...
The installed plugins are available in the *Plugins » Plugin overview* menu. Initially, you should test the plugins in the preview mode by clicking the *Plugin Set Preview* button. The preview mode is only accessible for users of your system. You can apply your to your live store by clicking *Link plugin set* and linking the plugin set to a client.

[.instruction]
Publishing plugin set and accessing preview mode:

. Go to *Plugins » Plugin overview.
. Click on your plugin set. +
→ The *Plugin set* details view opens.
. Activate the plugins *IO*, *Ceres* and *Cash in advance* by clicking the globe icon in the *Active* column. +
→ Active plugins are highlighted in blue in the *Active* column.
. Click on *Save & deploy plugin set*. +
→ The deployment process starts. The progress bar indicates when the process has finished.
. Click on *Plugin set preview*. +
→ The online store is opened in the preview mode in a new browser tab.

[IMPORTANT]
====
Please check if you can *update* the plugins in your plugin set in the *active* column. We advise you to alsways use the current versions of the plugins *Ceres* and *IO*. If the current versions of the plugins are already installed, the button is greyed out and the tooltip *No update available* is displayed when you hover over the button with the mouse.
====

[#70]
=== Setting up the Ceres plugin

Your online store comes with a fully functional test design. You can change the test design and add to it according to your needs. You will carry out basic settings of your online store in the following chapters. We will therefore not describe every single option in the Ceres configuration. <<#800, At the end of the page>>, you can find further reading material that provides a more detailed description of the settings.
...
* Store specials: lists with new items, special offers, etc. are integrated into Ceres via *tags*. For further information, refer to the <<item/managing-items#150, Adding tags to items>> chapter of the manual.
...
. Go to the *First item list* section.
...
[#160]
== Setting up the online store using the ShopBuilder
...
. Enter a name for your homepage and select the *Standard homepage (Ceres)* preset.
. Click on *Save*.
...
→ The ShopBuilder editor opens.
. Move elements from the left side into the preview area via drag & drop. Pay attention to the information given in <<table-navigation-sb>>.
. *Save* the settings.
...
→ The ShopBuilder editor opens.
. Move elements from the left side into the preview area via drag & drop. Pay attention to the information given in <<table-footer-sb>>.
. *Save* the settings.
...
|*Logout button*
|Provides the button with which your customers can log out from your online store. Click *Edit* to determine the <<#45, colour>> of the *logout *button and the *button size*.
...
. Click on *Save*.
→ The linked plugin set is indicated with a green dot next to its name.
...
[#800]
== Further information about your online store
...
* <<online-store/setting-up-ceres#, Setting up Ceres>>
* <<online-store/setting-up-io#, Setting up IO>>
* <<online-store/shop-builder#, Setting up the ShopBuilder>>
* <<online-store/best-practices#, Ceres Best Practices>>


TIP: Continue to <<quick-start/quick-start-order-processing#, preparing order processing>>.
