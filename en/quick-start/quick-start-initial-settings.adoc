...
*_Important:_* An invoice template for the location *Germany* is already pre-configured. You can adapt the template to your individual needs. To do so, go to *System » System settings » Client* » _Select client_ » *Locations* » _Select location_ » *Documents » Invoice*. If an invoice already exists for the order and you have set up an event procedure, the invoice will be sent to your customer automatically because the PDF attachment *Invoice* is selected by default in the template *Order: Invoice as PDF attachment*. +
