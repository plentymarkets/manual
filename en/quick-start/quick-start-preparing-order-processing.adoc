...
|*Own VAT number*
|Use this field to display your company’s own VAT number on the invoice. In case you have not already done so, you have to submit your VAT number beforehand in the *Setup » Client* » _Select client_ » *Locations* » _Select location_ » *Accounting*. We recommend to only use this field to display the VAT number on the *Invoice* document in case it is not already included in your company’s letterhead.
...
====
In general, you should never delete orders. One exception can be manually created orders that have only been created a very short time ago. Malfunctions with items and shipping profile settings may occur when deleting orders. Furthermore, orders cannot be deleted when tax relevant documents have already been generated.
====
