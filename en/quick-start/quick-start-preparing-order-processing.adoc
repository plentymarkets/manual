...
:docinfo1:
:url: quick-start/quick-start-preparing-order-processing
:id: C0B259C

IMPORTANT: Have you already set up your online store? Not yet? Return to <<quick-start/quick-start-online-store#, setting up the Ceres online store>>.

You can manage incoming orders from your online store in the back end of your plentymarkets system. On this page, you can find the necessary settings that you should carry out to prepare for order processing.

[#10]
== Carrying out basic settings

First, carry out the basic settings for the *Orders* area. Some of the settings are already preset in your plentymarkets system. We recommend not to change the default settings. However, there are some settings that you have to change. Those settings are explained in <<table-orders-quick-start-basic-settings>>. Proceed as follows to carry out the settings:
...
. In the *Apply settings from default document template* area, click on *Apply* (icon:cog[]).
...
|*Own VAT number*
|Use this field to display your company’s own VAT number on the invoice. In case you have not already done so, you have to submit your VAT number beforehand in the *System » System settings » Client* » _Select client_ » *Locations* » _Select location_ » *Accounting*. We recommend to only use this field to display the VAT number on the *Invoice* document in case it is not already included in your company’s letterhead.

2+| *Optional elements below stock unit table* area

|*Incoming payment*
|Select *YES* to display the date of complete incoming payment on the invoice. This setting is particularly useful if you only offer the payment methods *PayPal* and *Cash in advance* that you installed in an earlier step of this quick start.
...
====
As already described above, the invoice belongs to the location-based documents in your plentymarkets system. This means that the invoice document, as well as all further document templates, have to be set up for all additional languages you require. This becomes necessary if you accept orders from foreign countries in your online store or if your company operates in a location outside your own country. You can easily transfer the settings carried out in the previous steps to invoice documents in other languages. For further information on this process, pay attention to the links at the end of this page.
====
...
. Go to *System » System settings » Client* » _Select client_ » *Locations* » _Select location_ » *Documents*.
. Open the *Reversal document invoice* menu.
...
. Go to *System » System settings » Client* » _Select client_ » *Locations* » _Select location_ » *Documents » Delivery note*.
. Open the *Template* tab.
...
|*Referrer*
|Enter a description in the text field, for example *Order referrer*, and select the respective coordinates. This way, the delivery note can display information on whether the order reached you via your online store or via an integrated market.
...
On the left-hand side of the *Orders » Edit orders* menu, several filters and search possibilities are available to limit the number of orders displayed. This way, you can search for orders according to their *Order ID* or the *Invoice number*. You can also change the *Date* at which the order was received, so that you can display older orders as well. The date filter is set to display orders that were received during the past 2 months by default. +
...
Click on the order ID displayed at the left-hand side of each order to open the order itself. Several tabs are available within the order. As there are numerous possibilities for working with orders using those tabs, we will only outline some important functions. For further information on this topic, pay attention to the links at the end of this page. +
...
* Manually book incoming payment in the *Payment* tab, if necessary.
...
====
In general, you should never delete orders. One exception are manually created orders that have only been created a very short time ago. Malfunctions with items and shipping profile settings may occur when deleting orders.
====

[#70]
== Check list

Work through the check list in order to review your settings for preparing order processing.
...
TIP: Continue with <<quick-start/quick-start-adding-plugins-to-the-system#, Adding plugins to the system>>.
