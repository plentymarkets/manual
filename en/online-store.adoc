...
After the client (store) has been activated, go to *System » System settings » Client* and carry out the settings within the menu of the client in question. The setting options here are mainly the same as those for standard online stores. Therefore, refer to the *<<omni-channel/online-store/setting-up-clients#, System » System settings » Client » Select client>>* pages of the manual.

[IMPORTANT]
.Name of the menu
====
The menu of a client is given the name of the store. You find the main menu of a client in the *System » System settings » Client* menu, below the *Standard* menu (= your standard online store's menu). The menus of further clients (stores) will also be organized here (image 1).
====
...
|*eBay account*
|Displays the eBay accounts that were saved in the *<<omni-channel/multi-channel/ebay#65, System » System settings » Markets » eBay » Accounts>>* menu. Select which eBay account should be used for the client (multiple selection using *Ctrl + mouse click*).
...
*__Important__*: If you use this setting you also need to go to *<<payment/managing-payment-methods#, System » System settings » Orders » Payment » Payment methods>>* and select the option *Show bank details* (place check mark) in the payment methods *Invoice* and *Cash in advance*.
...
__*Important:*__VAT rates have to be saved for the standard country of delivery. This is done in the *<<orders/aaccounting#, System » System settings » Client » Select client » Locations » Select location » Accounting>>* menu. +
