...
|Activate the filter, select the operator and the customer class from the drop-down menu.
...
|Activate the filter and select the language from the drop-down menu.
...
The data fields that are listed in table 2 are available for <<data-exchange/importing-data#25, **data synchronisation** >>. For mandatory synchronisation fields (M), select the option **Synchronisation** as **Import procedure**. At least one of the data fields from the following table must be selected for synchronisation. You can also select both data fields.
...
In the **Procedures** tab, select which procedures you want to be carried out during the data export. Activate the **LastUpdate** procedure. Select the **set the current date** option from the drop-down menu. The date and time when changes were last made will be added to the customer data record during import.
