...
The response includes an error if the credit rating fails. If an error occurs, then a negative credit rating - **<<data-exchange/api-log#50, red credit rating >>** - will be displayed in the customer data record.
|====
...
This chapter provides information about all of the log entries that relate to retrieving the **<<data-exchange/api-log#7, PayPal settlement report >>**.
...
The PayPalSettlement log entries relate to the transactions that are included in the settlement reports. The PayPal transaction ID is displayed in plentymarkets. It is listed as the designated use for a **<<data-exchange/api-log#2, payment >>**.
