...
:keywords: inventory taking, stock-taking, inventory, stocktaking, check stock, checking stock, target stock, actual stock, stock index, stock, stocktaking mode, delete stock, rebook stock, dynamic import, dynamic export, data format stock, test data exchange, edit stock quantity, editing stock quantity, physical stock, activate stocktaking mode, deactivate stocktaking mode, clear stock, clearing stock
:position: 70
:id: YU6LH3H
...
[IMPORTANT]
.Stocktaking only with import tool and Dynamic export
====
Stocktaking can only be performed using the *import tool* and the *Dynamic export*.
Instead of the Dynamic export which provides the stock per storage location, you can also use the Elastic export. However, the latter only exports the stock per warehouse, which means that all storage locations will be exported in one single line.
====
...
. Open the submenu for the data format <<data/export-import/data-formats/stock#, Stock>> by clicking on the plus. If the submenu is not available yet, set up the <<data/export-import/data-formats/stock#, data format>> by clicking on *New data format*.
...
. Tick off the data field *WarehouseID* and select a warehouse from the WarehouseID drop-down list. +
→ The warehouse with the lowest warehouse ID is selected by default.
. *Save* (icon:save[role="green"]) the settings.
. Click on *Export data* (icon:cog[]) in order to export a CSV file.
...
. Enter the current physical stock into the *Stock* column.
. Save the changes.
