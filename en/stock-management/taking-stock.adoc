...
====
Please pay attention to the notes specified on the <<basics/data-exchange/data-formats/stock#, **Stock**>> data format page of the manual – especially before you start to overwrite the stock quantities in your system. To test the import use a test file that only contains one variation and overwrite the stock of that variation. This way you can easily check the result and see if it turned out as desired.
====
...
. Go to <<basics/data-exchange/exporting-data#, **Data » Dynamic export**>>.
. Open the submenu for the data format <<basics/data-exchange/data-formats/stock#, **Stock**>> or set up this **<<basics/data-exchange/data-formats#2-2, data format>>** if the submenu is not yet available.
. Select the settings for the data export.
. Click on the **Filter** tab.
. Select a warehouse from the **WarehouseID** drop-down menu.  +
→ The warehouse with the lowest warehouse ID is selected by default.
. **Save** the settings.
. Click on **Export data**.

[TIP]
.What is the difference between the data field **WarehouseID** and the **WarehouseID** filter?
====
The **WarehouseID** filter lets you define from which warehouse the stock will be exported. The **WarehouseID** data field simply contains the ID of a warehouse. The **WarehouseID** data field does not influence the content of the export.
====
...
* Nonetheless, you should not book stock out either, if the stocktaking mode is active, because the stock will not be calculated.
...
.Dynamic import of the **Stock** format
[cols="a,a"]
|====
|Setting |Explanation
...
====
Check the <<basics/first-steps/system-requirements#_managing_stock_with_the_plentymarkets_app, system requirements, system requirements>> page of the manual to see current hardware recommendations for stocktaking using the plentymarkets app.
====
...
====
To take stock using the plentymarkets app, you must assign barcodes to your storage locations. You can speed up stocktaking by *<<warenwirtschaft/lager-einrichten#800, labelling your storage locations>>*. During stocktaking, you can just scan the storage location's label.
====

==== Exporting stock with the data format Stock

To check all stock discrepancies resulting from stocktaking at once, export your stock data using the data format <<basics/data-exchange/data-formats/stock#, Stock>> before the stocktaking process. Export the data again after stocktaking is complete and compare the stock data.
...
. Carry out the settings. Pay attention to the information given in <<table-stocktaking-app-settings>>. +
...
|Activate to set the cursor in the *Barcode* field of the storage location search. If your storage locations are labelled, users can scan the storage location's label.

2+|Item search

|*Set focus on barcode*
|Activate to set the cursor in the *Barcode* field of the item search. If your storage locations are labelled, users can scan the item's label.
