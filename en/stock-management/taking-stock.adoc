...
====
Please pay attention to the notes specified on the <<basics/data-exchange/export-import/data-formats/stock#, Stock>> data format page of the manual – especially before you start to overwrite the stock quantities in your system. To test the import use a test file that only contains one variation and overwrite the stock of that variation. This way you can easily check the result and see if it turned out as desired.
====
...
. Go to <<basics/data-exchange/export-import/exporting-data#, Data » Dynamic export>>.
. Open the submenu for the data format <<basics/data-exchange/export-import/data-formats/stock#, Stock>> or set up this <<basics/data-exchange/export-import/data-formats#, data format>> if the submenu is not yet available.
...
To check all stock discrepancies resulting from stocktaking at once, export your stock data using the data format <<basics/data-exchange/export-import/data-formats/stock#, Stock>> before the stocktaking process. Export the data again after stocktaking is complete and compare the stock data.
