...
→ The overview opens.
. Open the submenu for the data format <<basics/data-exchange/export-import/data-formats/stock#, Stock>> by clicking on the *plus*. If the submenu is not available yet, set up the  <<basics/data-exchange/export-import/data-formats#, data format>> Stock by clicking on *New data format*.
. Place a check mark for the following data formats: VariationID, LocationID, Batch, BestBeforeDate, PurchasePrice, Stock, WarehouseID.
