...
. Open the submenu for the data format <<basics/data-exchange/data-formats/stock, Stock>> or set up this <<basics/data-exchange/data-formats#, data format>> if the submenu is not yet available.
...
* Nonetheless, you should not book stock out if the stocktaking mode is active, because the stock will not be calculated.
...
. In the *warehouseLocation* area, activate the right *Show*.
. *Save* the settings.
...
. (Version 1.7.11 and higher) *_Optional:_* Select the best before date and/or enter the batch and tap *Add item*. +
