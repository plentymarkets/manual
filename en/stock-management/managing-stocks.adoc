...
:keywords: stock, manage stock, managing stock, stock data, reorder, incoming items, outgoing items, stock terms, physical stock, reserved stock, net stock, reserve stock, reserving stock, result list, stock list, purchase price, redistribute stock, redistribution of stock, automatic stock update, changing order status, change order status, stock movement, stock movements, history, stock movements history
:position: 40
:id: LD1RKJI
...
Users of the type *Back end* can only access limited areas and menus in the plentymarkets back end. Thus, their access to the system is limited. If users of this type should be able to work with stocks, a user of the type *Admin* has to assign the rights listed in <<#table-rights-stock>> to those users.
...
. In the *Authorisations* area, activate the rights as listed in <<#table-rights-stock>>.
. *Save* (icon:save[role="green"]) the settings.

[[table-rights-stock]]
.Assigning rights for stock
[cols="1,3"]
|====
|Setting |Explanation
...
| *Stock > Edit*
|Authorises the back end user to edit stocks.
...
|Net stock
|The quantity of a variation that is available for sale. The net stock is part of the physical stock. Subtract all reserved quantities from the physical stock and you will get the net stock. If a customer tries to buy more copies of an item than are currently available in the net stock, this customer will receive a warning that the delivery will be delayed.
...
If an item is returned by a customer, you can create a <<orders/managing-orders#warranty, warranty>> for one or more items by opening the original order. These warranties are treated as individual orders with a distinct order number. Stock is reserved until either variations are booked out or until the warranty itself is cancelled again.
...
The displayed parameters depend on the user's rights. Options that can be restricted for a user are e.g. the warehouse options. Go to *Setup » Settings » User » Rights » User* and check the settings for users who work with stocks.
...
| **Variation number**
|Enter a variation number to search stock of this variation. The search may return more than one result. Let's assume you enter 987 and because you have a lot of variations you also have a variation that has the ID 9871. In this case, both variations will be found. In addition, also variations of the same item that the variations belong to will be listed.
...
The supplier can also be selected from a drop-down list in the variation’s row. Despite the fact that you have filtered with a supplier, all suppliers of the variation will be listed here. In addition to the supplier, you will also see the purchase price for this variation from this suppler, the units contained, the discount and the minimum order quantity.
...
With syncs of the type *Item*, *Stock* or *Incoming items* you can import negative stock. This is useful if you notice a difference in your stock after inspecting your inventory or if less items were delivered than you actually ordered.
When using the sync types Item and Stock, the values from the CSV file or from the Own value are imported 1:1.
When using the sync type Incoming items, however, the values are subtracted from the stock.
...
. Select a sync. The sync has to be of the type Item, Stock or Incoming items (warehouse, storage location and quantity are mandatory fields). +
icon:map-signs[] *_Or_*: Click on *Add sync* in order to add a sync of the type Item, Stock or Incoming items. For further information, refer to the <<data/importing-data/ElasticSync#1210, ElasticSync>> page.
. Switch to the *Mapping* tab.
. In the field *Source*, select the value *Own value*.
