...
**Matrix** = similiar to the extended result list, only variations that have attributes are displayed differently
...
You can set up barcodes in the **<<item/managing-items#630, Settings » Item » Barcode >>** menu and then you need to **<<item/managing-items#250, link the barcode with your variation >>**.
...
When searching with item data, it is sufficient to only enter the word stem into the field. Search terms can be shortened to almost any extent at the end of words, but they can not be shortened at the beginning. When searching for starter sets it would be enough to just enter starter or start. The same is true for all numbers e. g. IDs.

|**Limitation**
|Select an option from the drop-down menu to only search for variations that have this stock limitation.

|**Order**
|**Yes** = Allows to create reorders within the stock search menu. The items need to be linked with variations to create reorders. The button **Create reorder** is above the result list. The chapter **<<stock-management/managing-reorders#800, Creating reorders >>** explains all necessary steps in defail. +
...
|**Purchase price**
|You can define whether the average purchase price or the purchase price that is currently saved in the variation data will be displayed. Open the **Settings » Stock » Stock » Search** and select an option for the purchase price. +
However, the purchase price saved in the variation data is also based on your settings and might as well be the average purchase price. How the incoming items data is applied to your variation data needs to be defined in the **Settings » Stock » stock » incoming items** menu.
...
|**ø sale**
|The average sales depend on the **sales period** that was saved in the **Settings » Stock » Reorder management » Basic settings** menu. However, you will only see a value for average sales here if the search filter **Order** is set to **Yes**.
...
[#500]
== Redistributing the stock of a variation
...
. Go to **Stock » Stocks**.
. Click on **magnifying glass** in the **storage location** column for the variation that you wish to redistribute. +
→ A window will open.
...
Another menu where you can redistribute stock is the **Stock** tab of a variation. Open the **Item » Edit item** menu and then oben a variation.
...
You can export or import stock data and this way you can let your system know about the latest stocktaking that you carried out. The data format **<<data-exchange/data-formats/stock#, Stock  >>** contains the relevant stock data from your system. Data formats that might generally be usefull when working with stock are:

* **<<data-exchange/data-formats/stock#, Stock  >>**,
* **<<data-exchange/data-formats/variationsupplier#, VariationSupplier  >>**,
* **<<data-exchange/data-formats/variationwarehouse#, VariationWarehouse  >>** and
* **<<data-exchange/data-formats/warehouse#, Warehouse  >>**

All data formats available in plentymarkets as well as a general description on how to export or import data from or into plenymarkets can be found in the **<<data-exchange#, data exchange  >>** chapter of the manuel.

[#1100]
== Viewing the stock movements history

You can view a variation's stock movemtents in the **Stock movement** tab of the variation. You can open the **stock movements** by either opening the variation or by clicking on the physical stock of one of the results in the stock results list. This tab shows you how your stock reached its current level. In other words, you will see the different bookings that led to the current result. This will increase transparency of your stock management and in addition it enables you to understand your bookings also later.  +
