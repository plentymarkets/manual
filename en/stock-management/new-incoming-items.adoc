...
====
When activating a warehouse for a main variation, this setting is <<item/introduction/inheritance#, inherited>> to all variations of the item.
====
...
*_Note:_* If the <<stock-management/setting-up-a-warehouse#reorder-value_variation_warehouse, difference between the current stock and the reorder level>> is calculated dynamically, the only valid value is `0`.
...
However, it is not sufficient to assign a price in the *Setup » Stock » New incoming items* menu in order to display a sales price in a variation. You also need to assign the sales price to a variation. To create a sales price, go *Setup » Item » Sales prices*. For further information about sales prices, refer to the <<item/settings/prices#200, Sales prices and price calculation>> page. On this page, you will also learn how to calculate prices for customer classes.
...
Create a <<crm/managing-customer-data#100, customer data record>> for each supplier and select the customer type Supplier.
Furthermore, at least one user has to be activated as <<business-decisions/user-accounts-access#40, owner of incoming items>> so that the incoming items can be assigned to an owner.
...
You can add variations individually or several at once. However, it is only possible to add variations that already exist as data records in your system. Whenever a new item is delivered, first create a new <<item/import-export-create/create/new-item#, item>> and the corresponding variations.
