...
|latexmath:[$LAPP_{new} = {LAPP_{old} \cdot StockQuantity_{old} + PurchasePrice_{new} \cdot StockQuantiy_{new}\over StockQuantity_{old} + StockQuantity_{new}}$]
|latexmath:[$MAPP_{new} = {MAPP_{old} \cdot StockQuantity_{old} + PurchasePrice_{new} \cdot StockQuantiy_{new}\over StockQuantity_{old} + StockQuantity_{new}}$]

|A recalculation of the live average purchase price (LAPP) will be carried out every time stock is booked out or in. The current average is given in export formats or used for statistics. If you wish to see this purchase price in a variation's data record, then you need select that the average purches price will be transferred. +
...
|A recalculation of the moving average purchase price (MAPP) will be carried out every time stock is booked out or in. However, a recalculation will only be carried out if you defined that stock booked into this warehouse is used for the calculation of the moving average purchase price. Each variation has a separate field for showing the moving average price. +
Go to *System » Stock » Stock » Incoming items* to activate the warehouses that will be used for the calculation of the moving average purchase price.
...
. Carry out the settings. Pay attention to <<table-app-settings-for-incoming-items>> +
