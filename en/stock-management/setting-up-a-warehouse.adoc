...
Consider in advance which dimensions you want to create, and start with the smallest one (e.g. shelf). This one will receive the highest level (e.g. when creating 4 dimensions, it will have level 4). Thus, note that the dimension with the highest level will be the smallest one in the warehouse. Consequently, create the dimensions in ascending order of size (e.g. shelf (level 4) > rack (level 3) > zone (level 2) > storage building (level 1).
