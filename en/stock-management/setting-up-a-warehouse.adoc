...
====
Use the status *Locked* in order to block stock in a storage location. If orders have stock in a locked storage location, they do not automatically switch from status 4 (In preparation for shipping) to status 5 (Cleared for shipping) anymore. To make this work, go to the *Setup » Orders » Settings* menu and activate the option *Status changes to 4 instead of 5 if stock is missing*. To ensure that orders still go into status 5, it is recommended to rebook the stock into active storage locations. +
Note however that blocked stock will still be available for the marketplaces.
====
...
The following sub-chapters describe how to <<stock-management/setting-up-a-warehouse#900, create>>, <<stock-management/setting-up-a-warehouse#950, link>>, <<#search-property, search>>, <<#edit-property, edit>> and <<#delete-property, delete>> properties and how to <<#group-properties, group>>, <<#import-properties, import>> and <<#export-properties, export>> them.
...
| *Section*
|Select the setting *Storage location* from the drop-drop-down list to make sure that the property can be used in this area. +
Additionally, the areas <<item/settings/properties#500, Item>> and <<crm/managing-contacts#950, Contact>> are available.
...
| *Section*
|Select the *Storage location* area to only display properties that are used in this area.
...
[#edit-property]
=== Editing storage location properties

The *Setup » Settings » Properties » Configuration* menu contains all properties that you have created so far. Here you change the settings described in <<table-create-properties>>.

[.instruction]
Editing a property:

. Go to *Setup » Settings » Properties » Configuration*.
. Click on *Search* (icon:search[role="blue"]) to display all properties or select filters to limit the search results. Pay attention to the information given in <<#table-filter-properties>>.
...
You can delete only one or multiple properties at the same time.
...
. Go to *Setup » Settings » Properties » Groups*.
. Click on *Search* (icon:search[role="blue"]) to display all property groups or select filters to limit the search results. Pay attention to the information given in <<#table-filter-properties>>.
