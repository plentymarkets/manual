...
. Repeat steps 1 to 4 until all variations that should be redistributed are included in the cart in the desired quantities.
...
When opening the *Stock » Reorder management » Purchase order* menu, all existing redistributions and <<stock-management/managing-reorders/working-with-reorders#, reorders>> are displayed. Carry out a search (icon:search[set=material]) to limit the search results to the required redistributions.
