...
The overview table in the *Stock » Reorder management » Purchase order* menu as well as the tables in an opened redistribution are customisable. This means that you can decide which information is displayed in the table rows. To do so, proceed as follows.
...
. Select which rows are to be displayed.
...
. Click on Initiate order (icon:cog[]) in the Order details of the redistribution. +
...
. Click on *Close package* or *Close pallet* to close the package or the pallet. _Note_ that when you cannot change anything in this pallet or package once you closed it. You would have to open a new one.
...
. *_Optional:_* Enter the date on which the order items were delivered as the *Delivery date*.
. Click on *Finish redistribution* (icon:cog[]). +
