...
.Selecting an owner
====
If the desired *Owner* is missing, check if the user has been activated as an owner. For general information about user accounts, refer to the <<business-decisions/user-accounts-access, User accounts and access>> page of the manual. In addition, check the <<#400, default settings>> for owners described above.
====

[#700]
=== Adding order items
The next step consists of adding the items that should be redistributed from one warehouse to another. Use the search to find the required order items.
