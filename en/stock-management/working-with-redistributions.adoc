...
You can enter a *delivery date* in the order details of a redistribution. Enter the delivery date by typing it in or by using the calendar. As soon as you set a delivery date in the order details, the same date is also set for the order items in the table. In the table, an individual delivery date can be set for every order item. _But note_ that set individual dates are overwritten when you enter a delivery date in the order details. The delivery dates remain editable until the reorder is completed.
