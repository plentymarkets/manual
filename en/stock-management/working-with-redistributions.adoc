...
To optimise your workflow using warehouse redistributions, we recommend carrying out some preparatory settings. Create new order statuses especially for reorders and create tags to keep a good overview. Setting up a document template for the *PO delivery note* is required as well.
...
Tags are keywords that you create centrally before you can use them in different areas of your plentymarkets system. If you want to use tags with redistributions, you need to create those tags beforehand in the Setup » Settings » Tags* menu. For further information, refer to the <<welcome/general-functions/tags#, Tags>> page of the manual.

[#500]
=== Setting up a template for the PO delivery note
Working with redistributions requires using the document type *PO delivery note* (PO Purchase order). This delivery note lists all variations included in a redistribution.  Warehouse staff and logistics partners can use this document to keep track of the variations included when booking the order items in in the receiving warehouse.
...
. Select the language in which the document is to be generated from the dropdown-list above the tab title.
...
To use the messenger, click on *Messenger* on the left-hand side of the opened redistribution menu or open the messenger in the lower area of the reorder menu.
...
. Click on plus icon (icon:plus-square[role="green"]) and select *New redistribution*. +
...
.Selecting an owner
====
If the required *Owner* is missing, check if the user has been activated as an owner. For general information about user accounts, refer to the <<business-decisions/user-accounts-access, User accounts and access>> page of the manual. In addition, check the <<#400, default settings>> for owners described above.
====

[#700]
=== Adding order items
The next step consists of adding the items that should be redistributed from one warehouse to another. Use the search (icon:filter[]) to find the required order items. To do so, set the required filters and click *Search* (icon:search[role="blue"]). You can combine several filters. <<table-filters-item-search-redistribution>> explains the filter settings.
...
. Set the required filters for the search and also take the explanations given in <<table-filters-item-search-redistribution>> into account.
...
. Click on *Save* (icon:save[role="green"]) in the *Cart* (icon:save[role="green"]) area. +
→ The cart is saved. +
...
You can use the filter bookmarks (icon:bookmarks[set=material]) to save selected filters in the UI. These filters are then preselected every time you open the menu. Every user can decide individually which filters to save.
...
_Note_ that all delivery dates in the table are deleted if you delete an already set delivery date in the order details. +
...
====
Redistributions can be deleted in the detail view as long as the redistribution has not been initiated by setting the order date. To delete a redistribution, click on *Delete purchase order* (icon:trash-o[]) in the right-hand corner above the info boxes.
====
...
. Click on *Add document* (icon:plus-square[role="green"]).
...
Set up the following event procedure to automatically generate the document *PO delivery note* as soon as you have <<stock-management/working-with-redistributions#900, booked out>> the items included in the redistribution from the sending warehouse.
...
→ The selected document is uploaded and is available in the Documents section.

Files can be uploaded as pdfs, but also as csv, zip, doc, docx, xls, xlsx. Files with a size up to 20 megabyte can be uploaded.

[#900]
=== Booking items out
After adding all items that should be redistributions, book the order items out of the sending warehouse. To do so, click on *Initiate order* in the *Details* area. The redistribution is initialised and the current date is set as the order date. +
...
. Click on *Create stock movements* (icon:cog[]). +
...
→ _Note_ that when you cannot change anything in this pallet or package once you closed it. You would have to open a new one.

When you go back to the redistribution after this, the created package content list is displayed in the corresponding area of the redistribution. In case you want to add further pallets or packages, click on *Edit* (icon:pencil[role="yellow-background"])  and you are redirected again to the package content list. If a redistribution is already finished, the package content list cannot be edited anymore. You can find more information about <<orders/package-content-list#, package content lists>> on the corresponding manual page.
...
When opening the *Stock » Reorder management » Purchase order* menu, all existing redistributions and <<stock-management/managing-reorders/working-with-reorders#, reorders>> are displayed. Use the search (icon:filter[]) to limit the search results to the required purchase orders. To do so, set the required filters and click *Search* (icon:search[role="blue"]). You can combine several filters. <<table-redistribution-search-filter>> explains the filter settings. +
...
| *Order date type*
|Choose from the drop-down list between the date types *Order date*, *Finished date*, *Delivery date* and *Created at* and select the date range with the filters *Order date from* and *Order date to*. With these settings, the redistributions that were ordered, finished or created during this date range are displayed.
...
The overview table for redistributions and reorders is customisable. This means that you can decide which table columns are displayed in which order. The following table columns are selected by default when you go to the purchase order menu:
