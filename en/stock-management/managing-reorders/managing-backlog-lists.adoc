...
:keywords: return, undelivered stock, pending deliveries, backlog list, reordered, booked in, undelivered, reorder, undelivered variations, stock, reorder management, warehouse, limit, quantity, supplier, order based, order based backlog list, NRQ, supply
...
Variations that have been *<<stock-management/managing-reorders/working-with-reorders#, reordered>>*, but have not been booked in yet, are listed in the *Stock » Reorder management » Backlog list* menu. A search will not automatically be carried out when you open the menu. The default search is not limited by any filters. This means the default search will search for all undelivered variations from all warehouses and supplieres and regardless of how old the reorder is. Use filters to limit the search results.
