...
Tags are keywords that you create centrally before you can use them to mark data in different areas of your plentymarkets system. If you want to use tags with reorders, you need to create those tags beforehand in the *Setup » Settings » Tags* menu. For further information, refer to the <<welcome/general-functions/tags#, Tags>> page of the manual.
...
Use the search (icon:filter[]) to find the required order items. To do so, set the required filters and click *Search* (icon:search[role="blue"]). You can combine several filters. <<table-filter-item-search-reorder>> explains the filter settings.

[.instruction]
Adding order items:

. Go to *Stock » Reorder management » Purchase order*.
. Use the search filters to search (icon:search[role="blue"]) for the required reorder.
. Click on *Add* (icon:plus-square[role="green"]) in the *Order item* area. +
→ A menu for adding order items opens.
. Set the required filters for the search and also take the explanations given in <<table-filter-item-search-reorder>> into account.
. Click on *Search* (icon:search[role="blue"]). +
...
* Net order value = The net order value of the ordered items is displayed. This info box is always grey.
* Ordered items = The number of ordered items is displayed. This info box is always grey.
* Open items = The number of items not yet delivered is displayed. Grey means that no items are open. Orange means that items are open.
...
When opening the *Stock » Reorder management » Purchase order* menu, all existing reorders and <<stock-management/working-with-redistributions#, redistributions>> are displayed. Use the search (icon:filter[]) to limit the search results to the required purchase orders. To do so, set the required filters and click *Search* (icon:search[role="blue"]). You can combine several filters. <<table-reorder-search>> explains the filter settings. +
