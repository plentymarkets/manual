...
[IMPORTANT]
.New beta version of the reorder
====
This manual page describes how to reorder goods and manage reorders using the beta version of the new reorder functionality. As soon as the test phase ends, the functionality will be implemented in all plentymarkets systems during the upcoming weeks. Until then, you can find further information on the upcoming release in this link:https://forum.plentymarkets.com/t/ankuendigung-releaseprozess-neue-nachbestellung/542252[forum post^] (Last update: 24-May-2019).
====
...
. Go to *System » System settings » Stock » Reorder management » Reorder » Settings*.
...
* *Reorder level* = the quantity at which a reorder should be placed. You can select a <<stock-management/managing-reorders#500, static or dynamic reorder level>>.
...
Tags are keywords that you create centrally before you can use them to mark data in different areas of your plentymarkets system. If you want to use tags with reorders, you need to create those tags beforehand in the *System » System settings » Settings » Tags* menu. To do so, follow the instructions on the <<basics/working-with-plentymarkets/tags#, Managing tags>> page of the manual.
...
. Go to *System » System settings » Stock » Reorder management » Reorder » Documents » Reorder*.
...
. In the *Authorisations* area, activate the rights as listed in <<table-rights-reorders>>.
. *Save* the settings.
...
|Authorises the back end user to display variations.

|*CRM > Contact > Show contact*
|Authorises the abck end user to access the contact data (e.g. of the supplier).
...
*_Or_*: Select the option *Negative net stock or net stock equals 0* if you have not set up reorder levels.
...
*_Or_*: If not all order items should be reordered, select the order items that you wish to reorder and click on *Reorder selected items*. +
→  The reorder opens.
...
*_Or_*: Select the option *Upload reorder document* to upload a delivery note generated by and sent to you by your supplier. +
...
.Event procedure for automatically generating the reorder document
[cols="1,3,3"]
|====
|Setting |Option |Selection
...
.Setting up an email template
====
Before using the email service for processing reorders, you have to set up an email template for this purpose in the *System » System settings » Client » Select client » Email » Templates* menu.  For further information, refer to the <<crm/sending-emails#1200, Sending emails>> page of the manual.
====
