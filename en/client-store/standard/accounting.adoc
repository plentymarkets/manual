...
Go to **Settings » Client (store) » Select store » Locations » Select location » Accounting** to carry out important VAT settings. Decide whether VAT should be displayed on your invoices and how it should be displayed. Save VAT rates for your countries of delivery. Furthermore, save posting keys, debtor accounts and revenue accounts.

Each <<client-store/standard/locations#, **location**  >> has its own **Accounting** submenu. The accounting settings only apply to this location. The menu also allows you to copy some or all of the settings for a location and apply these settings to one or more additional locations.
...
. Go to **Settings » Client (store) » Select store » Locations » Select location » Accounting » Tab: Basic settings**.
...
image::client-store/standard/assets/EN-Client-Accounting-01.png[]
...
The menu only contains those countries that were set to **Active** under <</#, **Settings » Orders » Shipping » Shipping options » Tab: Countries of delivery** >>.

|**Tax rate A to D in %**
|Enter the VAT rates for the country in percent. The text fields **Name** are only relevant for the accounting software <<client-store/standard/accounting#150., **Xero** >>.
...
The standard VAT number is saved in the <<working-with-plentymarkets/basic-settings/master-data#, **Settings » Basic settings » Master data**  >> menu. This standard VAT number is used if you do not save a country-specific VAT number in the tab **VAT rates**.
...
. Create a <</#, **property** >> for digital items and give it a name, e.g. **Digital**. The property may not be an order property.
. Go to **Settings » Item » Basic settings** and select the property for the option **Item property for identifying digital items**.
. Open every item that should be sold as digital media and save the property in the <</#, **Properties** >> tab.
...
. Go to **Settings » Client (store) » Select store » Locations » Select location » Accounting » Tab: Accounts**.
...
image::client-store/standard/assets/EN-Client-Accounting-02.png[]
...
. Go to **Settings » Client (store) » Select store » Locations » Select location » Accounting**. Select the location that includes the settings you want to transfer.
...
====
For Germany, the link:http://www.bzst.de/EN/Steuern_International/Vorsteuerverguetung/02_Unternehmer_Ausland_EU/FAQ/FAQ_node.html;jsessionid=1318FB4ECBD85DED174407DD1AEAC4D8[**Federal Central Tax Office**, window="_blank"] provides tax information about the European countries of delivery as well as the USA and Japan.
====

== Data export

plentymarkets makes it possible to transfer all postings from invoices of outgoing items and from credit notes to a DATEV compliant accounting program (DATEV interface). In order to keep the rest of the processing as simple and efficient as possible, plentymarkets directly assigns the desired booking keys, debtor accounts and revenue accounts. The settings saved under **Settings » Client (store) » Select store » Locations » Select location » Accounting » Tab: Accounts** form the basis for this.

Carry out different <</#, **data exports** >> after all accounts and posting keys have been saved. To do this, go to **Data exchange » Data export**.
