...
NOTE: If this error occurs, the complete catalogue _is not exported to {market}_. Also important to know: The export stops when the first XSD validation error occurs. This is why there is only one error message in the log. However, the error might apply to more variations.
