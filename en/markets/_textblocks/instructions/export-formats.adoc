...
*_Note:_* There are no item filters set by default. If you do not add any item filterBy installing this plugin you will receive the export format *{export-format}*. Use this format to exchange data between plentymarkets and {price-search-engine} using the elastic export. You need to install the plugin *Elastic export* from the plentyMarketplace before you can use the format in plentymarkets.
...
*_Note:_* URLs of variations can only be transferred in combination with the Ceres online store.
...
| *Add item filters*
a| Select the items you wish to export. Select one or more of the following item filters from the drop-down list and click on Add. There are no filters set by default. It is possible to add all of the item filters listed below from the drop-down list one after the other.
