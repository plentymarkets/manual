...
. Carry out the settings. Pay attention to the explanations given in <<basic-settings-fruugo>>.
. *Save* the settings.
...
Items have to be available for Fruugo. This is done in the *Availability* tab of an item within the *Item » Edit item » Tab: Variation ID* menu.
...
. Carry out the settings. Pay attention to the information given in <<payment-method-fruugo>>.
...
The <<data/api-log#, Data » API log>> menu contains a history of processes that run via the Fruugo interface. The following formats are available for Fruugo:
