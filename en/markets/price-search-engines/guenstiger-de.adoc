...
Guenstiger.de is a German price comparison portal that also offers customer opinions, test reports and seller reviews. In order to set up the guenstiger.de plugin in plentymarkets, you have to register with link:https://stats.guenstiger.de/mitmachen.aspx[guenstiger.de^]{nbsp}icon:external-link[] first. Please note that this website is currently only available in German.
...
This project is licensed under the GNU AFFERO GENERAL PUBLIC LICENSE. Find further information in the link:https://github.com/plentymarkets/plugin-elastic-export-guenstiger-de/blob/master/LICENSE.md[LICENSE.md^]{nbsp}icon:external-link[].
