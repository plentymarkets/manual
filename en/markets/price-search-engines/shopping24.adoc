...
[IMPORTANT]
.Download Shopping24 plugin
====
In order to be able to export item data to Shopping24, the plugins link:https://marketplace.plentymarkets.com/en/plugins/channels/preisportale/elasticexportshopping24de_4734[Shopping24.de^] and link:https://marketplace.plentymarkets.com/en/plugins/sales/marktplaetze/elasticexport_4763[Elastic export^] are required. You can download the plugins in the link:https://marketplace.plentymarkets.com/en[plentyMarketplace^].
====

== Registering with Shopping24.de

shopping24.de is a subsidiary of the Otto Group and an online portal for price and product comparisons. Find out more about shopping24.de [here](https://www.shopping24.de/Info/ueber-uns). Please note that this website is currently only available in German.

== Setting up the data format Shopping24DE-Plugin in plentymarkets

By installing the Shopping24.de plugin, you will receive the export format **Shopping24DE-Plugin**. Use this format to exchange data between plentymarkets and Shopping24.de. You also need to install the plugin **Elastic export** from the plentyMarketplace before you can use the format in plentymarkets.

Once both plugins are installed in your plentymarkets system, you can create the export format **Shopping24DE-Plugin**. For more information on export formats, see the <<data/exporting-data/elastic-export#, elastic export>> page.
...
. Click on *New export*.
. Carry out the settings. Pay attention to the information given in <<#format-shopping24>>.
. *Save* the settings. +
→ The export format *Shopping24DE-Plugin* is given an ID. +
→ The export format is shown in the *Exports* overview.

[[format-shopping24]]
.Settings for the data format *Shopping24DE-Plugin*
[cols="1,3"]
|===
| *Setting* | *Explanation*
...
Open the export format *Shopping24DE-Plugin* in the *Data » Elastic export* menu to download and edit the export file.
...
|art_name
|The *item name* according to the format setting
*Item name*.
