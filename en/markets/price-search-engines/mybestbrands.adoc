...
MyBestBrands.de is a recommendation platform for greatly discounted fashion and lifestyle products from well-known brand name manufacturers. In order to set up the MyBestBrands.de plugin in plentymarkets, you have to register with link:https://www.surveymonkey.de/r/LVX5W8K[MyBestBrands.de^]{nbsp}icon:external-link[] first. Please note that this website is currently only available in German.
...
This project is licensed under the GNU AFFERO GENERAL PUBLIC LICENSE. Find further information in the link:https://github.com/plentymarkets/plugin-elastic-export-rakuten-de/blob/master/LICENSE.md[LICENSE.md^]{nbsp}icon:external-link[].
