...
[IMPORTANT]
.Download Shopping.com plugin
====
In order to be able to export item data to Google Shopping, the plugins link:https://marketplace.plentymarkets.com/en/plugins/channels/preisportale/elasticexportshoppingcom_4755[Shopping.com^] and link:https://marketplace.plentymarkets.com/en/plugins/sales/marktplaetze/elasticexport_4763[Elastic export^] are required. You can download the plugins in the link:https://marketplace.plentymarkets.com/en[plentyMarketplace^].
====
...
By installing the Shopping.com plugin, you will receive the export format *ShoppingCOM-Plugin*. Use this format to exchange data between plentymarkets and Shopping.com. You also need to install the plugin *Elastic export* from the plentyMarketplace first before you can use the format in plentymarkets.

Once both plugins are installed in your plentymarkets system, you can create the export format *ShoppingCOM-Plugin*. For more information on export formats, see the <<data/exporting-data/elastic-export#, elastic export>> page.
...
. Click on *New export*.
. Carry out the settings. Pay attention to the information given in <<#shopping-format>>.
. *Save* the settings. +
→ The export format *ShoppingCOM-Plugin* is given an ID. +
...
| *Item name*
| Select *Name 1*, *Name 2*, or *Name 3*. These names are saved in the *Texts* tab of the item. Enter a number into the *Maximum number of characters (def. Text)*This specifies how many characters are exported for the item name.
...
| This option must be activated because Shopping.com only accepts specific values which you have to enter here. For further information, refer to the chapter Available columns for the export file.
...
<<#columns-shopping.com>> shows the available columns for the export file for Shopping.com.
...
|Produktbild-URL
| *_Mandatory field_* +
Allowed file types: jpg, gif, bmp, pngThe *URL path* of the first item image according to the format setting *Image*. Item images are prioritised over variation images.
...
|Produktgewicht
|The weight as set within *Item » Edit item »* _Open item_ *»* _Open variation_ *» Tab: Settings » Dimensions*.

|Produkttyp
|The value of a property of the type *Text* or *Selection* that has the backend name *product_type*.

|Grundpreis
|The base price information in the format "price / unit" (example: 10,00 EUR / kilogram).

|Zustand
| The condition of the item according to the setting *Item » Edit item »* _Open item_ *Tab: Global » Basic settings » Condition for API*.
|===
