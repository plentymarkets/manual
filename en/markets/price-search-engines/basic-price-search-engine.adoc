...
Use the format *Basic Price Search Engine* to export item data for any price search engine. You can map the item data expected by most price search engines.
...
The new plugin link:xxxx[Plugin Basic Price Search Engine^]{nbsp}icon:external-link[] provides you with a long-term and flexible way to export your items to price search engines.

[#170]
=== Installing the plugin

To export item data to a price search engine with a catalogue format, first install the link:xxxx[Plugin Basic Price Search Engine^]{nbsp}icon:external-link[] in your plentymarkets system. You can download this and many other plugins in the link:https://marketplace.plentymarkets.com/en[plentyMarketplace^]{nbsp}icon:external-link[].
...
Item data is exported using a marketplace format. To export your items to a price search engine, create a catalogue in the *Data » Catalogue* menu. Link the data fields in the catalogue with item data saved in plentymarkets in the marketplace format afterwards. When you activate the catalogue, the catalogue data is exported automatically once a day.
...
Now set filters to only export specific items to the price search engine. Check the page <<data/exporting-data/managing-catalogues#, Managing catalogues>> to find out how to set filters.
...
* This list is new and will be continuously expanded. Some recommendations might still be missing. Our supporters are happy to help you with any questions regarding the mapping.
...
:market: the price search engine
