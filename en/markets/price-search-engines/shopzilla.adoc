...
:keywords: price comparison, price comparison portal, price search engine, compare prices, comparing prices, Shopzilla plugin, Shopzilla.de plugin, price search engine plugin, price plugin, Shop zilla, Shopzila
...
By installing this plugin you will receive the export format *ShopzillaDE-Plugin*. Use this format to exchange data between plentymarkets and shopzilla using the elastic export. You also need to install the plugin *Elastic export* from the plentyMarketplace first before you can use the format in plentymarkets.

Once both plugins are installed in your plentymarkets system, you can create the export format *ShopzillaDE-Plugin*. For more information on export formats, see the <<data/exporting-data/elastic-export#, elastic export>> page.
...
. Carry out the settings. Pay attention to the explanations given in <<#settings-shopzilla>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The export format *ShopzillaDE-Plugin* is given an ID and it appears in the overview within the *Exports* tab.

<<#settings-shopzilla>> lists details for settings, format settings and recommended item filters for the format *ShopzillaDE-Plugin*.
...
| *Product URL*
|Choose the URL that you wish to transfer to shopzilla. You can choose between the item’s URL and the variation’s URL. URLs of variations can only be transferred in combination with the Ceres store.
...
Open the export format *ShopzillaDE-Plugin* in the *Data » Elastic export* menu to download and edit the export file.

[.instruction]
Downloading the export file:

. Go to *Data » Elastic export*.
. In the line of the format *ShopzillaDE-Plugin*, click on *Download* (icon:download[role="purple"]). +
...
This project is licensed under the GNU AFFERO GENERAL PUBLIC LICENSE - find further information in the link:https://github.com/plentymarkets/plugin-elastic-export-shopzilla-de/blob/master/LICENSE.md[LICENSE.md^].
