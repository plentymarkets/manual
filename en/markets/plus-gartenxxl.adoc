...
It is mandatory to specify whether items that you would like to sell on Netto have a unit price. Go to *Item » Edit item » [Open item] » Tab: Variation ID » Tab: Settings* and activate the *Show unit price* option to indicate that an item needs to have a unit price. If you do not activate the *Show unit price *option, then *nein* (no) is transferred. This indicates that the item does not need an obligatory unit price.
