...
. Go to *System » System settings » Markets » Netto eStores*. +
...
(icon:map-signs[]) *_Or:_* Instead of linking the _Energy label* property, you can also upload the energy label file to the <<item/managing-items#660, Images>> tab of the item. To do so, proceed as follows.
...
Go to *System » System settings » Item » Properties* to create a property for mandatory unit prices.
...
. Go to *System » System settings » Orders » Procedures » Tab: New procedure*.
...
.Select a status
====
Select a status for the procedure *Change order status* that you have saved within *System » System settings » Orders » Settings* for the option *Status of cancelled orders (No reservation of stocks)* to cancel reservations. Neither select a status that triggers an event action which reports cancellations to Netto eStores.
====
