...
:keywords: Netto, Netto, netto.de, netto.de, plus.de, gartenxxl
...
====
Always activate the order referrer *plus.de* as well. This is to ensure a proper operation.
====
...
It is mandatory to specify whether items that you would like to sell on Netto have a unit price. Go to *Item » Edit item » [Open item] » [Open variation] » Variation tab: Settings* and activate the *Show unit price* option to indicate that an item needs to have a unit price. If you do not activate the *Show unit price *option, then *nein* (no) is transferred. This indicates that the item does not need an obligatory unit price.
