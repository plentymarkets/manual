...
Items have to be available for Kauflux. This is done in the *Availability* tab of an item within the *Item » Edit item »* _Open item_ *» Tab: Variation ID* menu.
...
.Install plugins
====
You need plugins to link plentymarkets to Kauflux. You need the following plugins to send item data to Kauflux:
...
By installing this plugin you will receive the export format
*KaufluxDE-Plugin*. Use this format to exchange data between plentymarkets and
Kauflux.de using the elastic export. You also need to install the
Plugin *Elastic export* from
the plentyMarketplace first before you can use the format in plentymarkets.
...
|*Generate cache file*
|Place a check mark if you want to transfer more than 9999 data records to Kauflux. We recommend that you do not activate this setting for more than 20 export formats. This is to ensure a high performance of the elastic export.
...
|*Image*
|Select *Position 0* or *First image* to export this image. + *Position 0* = An image with position 0 is transferred. +
