...
* Map at least the *mandatory fields*. To only show the data fields that are mandatory, click on icon:more_vert[set=material] in the toolbar. Then activate the toggle button (icon:toggle-on[role="green"]) *Only display mandatory fields*.
...
*_Note:_* Don't have any RRP, for example because you are the manufacturer? Then assign the same sales price as for the marketplace data field *Net sales price*. This way, no strikethrough price will be displayed on {market}.
...
*_Note:_* If the field is empty, the value `12345678` is exported automatically. +
*_Tip:_* Go to *Item » Edit item » [Open variation] » Tab: Settings » Area: Costs » Entry field: Customs tariff number* to save the customs tariff number.
