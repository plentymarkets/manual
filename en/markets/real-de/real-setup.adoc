...
|*Yes* = Offers are updated once per hour and once per day at real.de. Only if both this option and the option *Activate interface to real.de* are activated, the daily complete offer update and the hourly update for offers that were changed is executed. +
...
In order to link items, properties etc. with real.de you have to activate the order referrer real.de in the *System » System settings » Orders » Order referrer* menu.
...
* If you are working with item variations, make sure that the limitation is not set to *No limitation*.
* You are required to specify an *EAN* or *ISBN*. Otherwise, the item will not be transferred to real.de.

[#350]
==  Defining SKU manually

Add SKUs manually for specific referrers in the <<item/managing-items#170, Item » Edit item » Open item » Tab: Variation ID » Tab: Availability>> menu if needed.
...
|*Upload logo*
|If *Show upload logo* was selected from the *Logo* drop-down list, then you can upload an external logo here. +
...
.  Enter a name.
.  Select the *event* listed in <<creating-invoices-real>>.
. *Save* the settings. +
→ The event is created.
.  Pay attention to the explanations given in <<creating-invoices-real>> and carry out further settings as desired.
. Place a check mark next to the option *Active*.
. *Save* the settings. +
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<event-procedure-cancellations-real>>.
. *Save* the settings. +
→ The event is created.
. Pay attention to the explanations given in <<event-procedure-cancellations-real>> and carry out further settings as desired.
. Select a reason.
. Place a check mark next to the option *Active*.
. *Save* the settings. +
→ The event procedure is saved.
