...
. Open the submenu of the online store that you would like to set up access for.
...
|This setting applies to the order import. Orders are imported either as paid or not paid. Regardless of this setting, items will immediately be cleared for shipping when they have sufficient stock. With the setting *open*, one could also set the order to paid when the actual incoming payment from real.de is received. In this way, real.de can be tested.
...
*_Important:_* The minimum price does not apply to item variations which have surcharges for attributes. In this case, *deactivate* this setting. If you would like to use the minimum price, then create each variation as a separate item with a defined minimum price.
...
Items have to be available for real.de. This is done in the *Availability* tab of an item within the *Item » Edit item »* _Open item_ *» Tab: Variation ID* menu.

[TIP]
.Selling all variations of an item on real.de
====
To sell all variations of an item on real.de, carry out the settings described below for the main variation. The settings are then passed on to all variations of the item.
====
...
. Open the variation that you want to sell on real.de.
...
.Provide EAN or ISBN
====
In order for your variations to be available on the market real.de, you are required to specify an *EAN* or *ISBN*. Otherwise, the variation will not be transferred to real.de.
====
...
In order to use <<item/managing-items#480, characteristics>> for the real.de market, those characteristics have to be linked with real.de.
...
====
The status and filters in <<event-procedure-invoices-real>> only serve as an example for creating an event procedure. Select filters and statuses for the event procedure which best suit your work flow.
====
