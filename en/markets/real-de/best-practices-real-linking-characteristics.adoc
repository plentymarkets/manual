...
[#100]
== How do I link a characteristic to a real.de attribute?
...
You can add this information by adding characteristics to the product data file. A new column is added to the product.csv for each characteristic that is linked to link:https://www.real.de/[real.de^]{nbsp}icon:external-link[]. +
...
First of all, you have to find out the exact name of the column in the product data file. An overview of the attributes is provided on link:https://www.real.de/versandpartner/download-bereich/[real.de^]{nbsp}icon:external-link[].
...
Create the new characteristic in plentymarkets in the *Setup » Item » Characteristics* menu. The new characteristic will then appear in the CSV file. The characteristic must have the exact same name as the corresponding real.de attribute. In addition, you have to link the characteristic to real.de.
