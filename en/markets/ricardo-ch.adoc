...
_Image 1: Linking listings to different layout templates_
...
.Use if-logic
====
You can use the <<online-store/online-store/cms-syntax#basics-if, if/else logic>> for layout templates. It functions the same way that the logic works in the <<online-store/online-store/cms-syntax#web-design, plentymarkets web design>> area.
====
...
. <<Searching for listings, Filter>> the items.
. Activate the option *Create listing* in the *Procedures* area. +
→ Irrelevant settings are greyed out.
. Click on *Apply* to create the listings.
...
|*Listing scheduler*
|Select a listing scheduler for the market listing. Select a <<Automatically starting listings with the listing scheduler, listing scheduler>> in the *Activate listing scheduler* window.
...
|*Warranty*
|Select *No warranty* or *According to the description*. Enter a descriptive text under <<Saving text templates for ricardo.ch, terms of guarantee>>.

|*Conditions*
|Select *According to the description*, *upon pick-up* or *in advance*. Enter a descriptive text under <<Saving text templates for ricardo.ch, payment conditions>>. As an alternative, the customer pays upon pick-up of the item or before receiving the item.
...
|*Delivery conditions*
|Select the delivery conditions. Enter a descriptive text for the option *According to the description* under <<Saving text templates for ricardo.ch, delivery conditions>>.
...
|*Stock dependence*
|Select a <<Setting up stock, stock dependence>> or *ALL* to only display listings with the selected stock dependence.
...
. Click on *Group function*. +
→ The *Group function* window opens.
. Carry out the settings as desired. Pay attention to the information given in <<group-function-batch-processing>> in the <<Editing multiple listings, Editing multiple listings>> chapter.
