...
*transmit UPC* = Transfer the Universal Product Code. In order to use this option, an EAN has to be selected for the *UPC from EAN field* setting. +
*transmit MPN* = Transfer the Manufacturer's Part Number. +
*transmit GTIN* = Transfer the Global Trade Item Number. +
...
*Only visible in item listings* = Items are neither displayed in a category nor are they displayed with the search function. Items are only displayed with a direct link.
...
The *Data » API log* menu contains a history of processes that run via the ricardo.ch interface. The *ItemListingControllerRicardo* format is available for ricardo.ch.
...
Use ElasticSync in the *Data » ElasticSync* menu to exchange data between plentymarkets and ricardo. Listing data can be imported into plentymarkets with the <<data/importing-data/sync-types/elasticsync-market-listing#, Market listing>> sync type.
