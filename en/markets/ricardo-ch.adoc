...
Go to *System » System settings » Markets » ricardo » Accounts* to activate new accounts for ricardo.ch. You need to have your access data consisting of the user name and password. Proceed as follows to add one or more accounts.

[.instruction]
Activating a new account:

.  Go to *System » System settings » Markets » ricardo » Accounts*.
...
====
You can top up your ricardo.ch credit balance directly on ricardo.ch. You need to have credit in order to <<omni-channel/multi-channel/ricardo-ch#3600, start>> listings.
====

Go to *System » System settings » Markets » ricardo » Accounts* to rename or delete accounts.
...
The setting *Update options* in the *System » System settings » Markets » ricardo » Accounts* menu allows you to update your listing settings and to fix problems, e.g. if no payment method of a ricardo.ch listing has been selected.
...
The setting *Synchronise offer templates* allows you to synchronise your ricardo.ch link:http://www.ricardo.ch/verkauferportal/de-ch/ratgeber/beitr%C3%A4genachverkaufsphasen/einstellen/angebotsvorlageninmyricardo.aspx[offer templates^] with plentymarkets. The offer templates are available for your ricardo.ch listings in the *Listings » Edit listings* menu. You can find them in the *Market* tab in the *Link to template* drop-down list.

[#600]
===  Saving text templates for ricardo.ch

Go to *System » System settings » Markets » ricardo » Settings* to save texts for *Terms of guarantee*, *Payment conditions* and *Delivery conditions*, as well as texts for *Automatic feedback*. You can enter German and French text templates in the text fields. Expand *Other* to set which item name is to be used in the listing title or listing subtitle for the second language.
...
Payment conditions; +
Delivery conditions*
| Enter the condition texts for ricardo.ch in German and French. The texts are used in those *listings* for which you have selected the option *According to the description* in the drop-down lists for *Warranty*, *Payment conditions* and *Delivery conditions* in the <<omni-channel/multi-channel/ricardo-ch#2700, Listings » Edit listings » Tab: Market>> menu.
...
Go to *System » System settings » Listings* to carry out the settings for all listings regardless of the market. In order to create listings in plentymarkets, you must carry out the following settings and have created your <<item/introducing-items, items>>.
...
.  Carry out the settings as described in <<settings-listings>> below.
. *Save* the settings.
...
*deactivated* = The listing is only listed if there is sufficient stock available. In this case, sufficient means that stock is larger than the <<en/omni-channel/multi-channel/ricardo-ch#2500, maximum quantity for sale>>, selected in the listing.
...
Down-selling means that the customer can reduce the quantity purchased on ricardo during the checkout. Up-selling is generally possible for fixed-price items. This is why it cannot be selected here.

|*Shipping costs for new orders coming from ricardo*
|Select *Transfer from ricardo* or *Have it calculated by plentymarkets*. +
*Transfer from ricardo* = Shipping costs are transferred from ricardo. +
...
Now link the listings to the layout template. Use one <<omni-channel/multi-channel/ricardo-ch#4100, listing template>> for several listings. Individual listings are linked directly within the listing.
...
[IMPORTANT]
.Activate the option Wildcard character in layout template (ricardo)
====
If you use template variables which contain properties, such as *{% GetItemPropertyById() %}* and *{% GetItemPropertyByName() %}*, you have to activate the option *Wildcard character in layout template (ricardo)* for the selected property, so that the property is displayed in the layout template. This is done in the *System » System settings » Item » Properties* menu.
====
...
.Use if-logic
====
You can use the <<omni-channel/online-store/cms-syntax#basics-if, if/else logic>> for layout templates. It functions the same way that the logic works in the <<omni-channel/online-store/cms-syntax#web-design, plentymarkets web design>> area.
====

[IMPORTANT]
.Listings for ricardo
====
Since ricardo does not allow the use of special HTML tags, only the information saved for the linked layout template in the *Main frame* tab are displayed in the ricardo listings.
====
...
plentymarkets provides you with multiple possibilities for creating listings. Make sure that you create your <<item/introducing-items, items>> first. Then create your listings in the item overview using batch processing or via an existing listing. Use the data format <<data/export-import/data-formats/itemlistingmarketricardo, ItemListingMarketRicardo>> in order to automatically create large quantities of items.
...
.  Carry out the settings and pay attention to the explanations given in <<settings-market-listing>>.
. *Save* the settings. +
→ The listing is created and the listing overview opens.

[[settings-market-listing]]
...
|*Listing template*
|Select a <<omni-channel/multi-channel/ricardo-ch#4100, listing template>>.
|====
...
For further information about the settings of a market listing, refer to the <<omni-channel/multi-channel/ricardo-ch#2300, Editing listings>> chapter.

[#2200]
===  Creating multiple listings

Create multiple listings with the *Group function* or the *Batch processing* function in the *Item* menu. The group function creates listings for previously selected items. The batch processing function creates listings for previously filtered items. If no *listing template* is selected in the menu *Item » Edit item » Tab: Multi-Channel* in the section *Listing default settings*, then eBay listings are created automatically. If you want to create multiple ricardo listings, then make sure you selected a *listing template* with specific settings for ricardo in the menu *Item » Edit item » Tab: Multi-Channel* in the section *Listing default settings*.
...
. <<omni-channel/multi-channel/ricardo-ch#3200, Filter>> the items.
.  Activate the option *Create listing* in the *Procedures* area. +
→ Irrelevant settings are greyed out.
.  Click on *Apply* to create the listings.
...
|*Listing scheduler*
| Select a listing scheduler for the market listing. Select a <<omni-channel/multi-channel/ricardo-ch#3800, listing scheduler>> in the *Activate listing scheduler* window.

|*Start*
| Start market listing. Select a starting time in the *Start market listing* window.

|*Listing template*
| Apply a listing template to the market listing. Select a <<omni-channel/multi-channel/ricardo-ch#4100, listing template>> in the *Apply listing template* window.
...
|*Fixed price*
| Enter an amount for a listing of the type *Fixed price*. If you activated the option *Tie to item price* when creating the listing, the fixed price will be displayed in CHF and cannot be changed.
...
|*Warehouse*
| Select a warehouse or *use the basic setting* that was selected for the *Warehouse selection* option in the <<omni-channel/multi-channel/ricardo-ch#1000, Stock>> menu.
...
|*Warranty*
|Select *No warranty* or *According to the description*. Enter a descriptive text under <<omni-channel/multi-channel/ricardo-ch#600, terms of guarantee>>.

|*Conditions*
|Select *According to the description*, *upon pick-up* or *in advance*. Enter a descriptive text under <<omni-channel/multi-channel/ricardo-ch#600, payment conditions>>. As an alternative, the customer pays upon pick-up of the item or before receiving the item.
...
|*Delivery conditions*
| Select the delivery conditions. Enter a descriptive text for the option *According to the description* under <<omni-channel/multi-channel/ricardo-ch#600, delivery conditions>>.
...
|*Additional options*
| Activate one, several or all options. Further information about the additional options for ricardo can be found on the link:http://www.ricardo.ch/ueber-uns/de-ch/geb%C3%BChren/einstelloptionen.aspx[ricardo website^] (German and French only).
...
|*Link to template*
| Select an link:http://www.ricardo.ch/verkauferportal/de-ch/ratgeber/beitr%C3%A4genachverkaufsphasen/einstellen/angebotsvorlageninmyricardo.aspx[offer template^] which should be used for the listing (German and French only).
|====
...
|*Listing scheduler*
| Select a listing scheduler for the market listing. Select a <<omni-channel/multi-channel/ricardo-ch#1200, listing scheduler>> in the *Activate listing scheduler* window.
|====
...
.  Select the filters based on <<filter-options-batch-processing>>.
.  Carry out the settings as described in <<group-function-batch-processing>> below.
.  Click on *Execute* to apply the settings to the listings.
...
Copy a market listing, e.g. if you want to upload the same listing with a different variation to ricardo. The copied listing has the same settings as the original. You only have to change the variation after copying.
...
For further information about the settings of a listing, refer to the <<omni-channel/multi-channel/ricardo-ch#2300, Editing listings>> chapter.
...
.  Go to *Listings » Batch processing » Listings*.
. <<omni-channel/multi-channel/ricardo-ch#3200, Filter>> the listings.
.  Activate the option *Copy listings* in the *Procedures* section.
.  Click on *Execute* to copy the listings.
...
====
If you change settings in the batch processing or group function area and you select the procedure *Copy listings*, then the settings are applied to the copied listings. This means that your listings are changed when copied.
====
...
|*Stock dependence*
| Select a <<omni-channel/multi-channel/ricardo-ch#1000, stock dependence>> or *ALL* to only display listings with the selected stock dependence.
...
.  Go to *Listings » Active listings*.
.  Open a market listing.
.  Click on the <<omni-channel/multi-channel/ricardo-ch#2500, Base>> tab and activate the *Endless lister*. +
...
.  Click on *Group function*. +
→ The *Group function* window opens.
.  Carry out the settings as desired. Pay attention to the information given in <<group-function-batch-processing>> in the <<omni-channel/multi-channel/ricardo-ch#2800, Editing multiple listings>> chapter.
...
All active listings are displayed in an overview in the *Listings » Active Listings* menu. Use the filters for <<omni-channel/multi-channel/ricardo-ch#3300, settings>>, <<omni-channel/multi-channel/ricardo-ch#3400, categories>> or <<omni-channel/multi-channel/ricardo-ch#3500, directories>> to narrow down the listings that are displayed. In addition to the filters described in <<filter-options-listings>> , the following options are available:
...
.  Select the filters based on <<filter-options-batch-processing>> and <<filter-options-listings>>.
...
The menu *Data » API log » Tab: API log* contains a history of processes that run via the ricardo.ch interface. The *ItemListingControllerRicardo* format is available for ricardo.ch.

[.instruction]
Viewing the API log:

.  Go to *Data » API log » Tab: API log*.
.  Select the *ItemListingControllerRicardo* or *LeaveFeedback* format from the *Process* drop-down list.
...
[#4700]
==  Synchronising data via REST API

Use the REST API or the *Data exchange* menu to exchange data between your plentymarkets system and ricardo.ch. The data format <<data/export-import/data-formats/itemlistingmarketricardo, ItemListingMarketRicardo>> is used to create new ricardo listings or edit existing listings, e.g. to change or delete parameters.
...
[.indented]
In the ricardo options, you selected *According to the description* for this listing's guarantee, payment conditions or delivery conditions. However, you haven't saved these general conditions yet. To do so, go to *System » System settings » Markets » ricardo » Settings* and save the conditions in the sub-menus *Terms of guarantee*, *Payment conditions* and *Delivery conditions*.

[.instruction]
I am unable to select payment methods in the ricardo settings of a listing.

[.indented]
Go to *System » System settings » Markets » ricardo » Accounts* and update the options for the ricardo markets in the *Update options* section.
...
[.indented]
Go to *System » System settings » Markets » ricardo » Accounts* and update the token.
