...
:keywords: Neckermann.at, marketplace, market, Neckermann setup, setting up Neckermann, Neckermann Austria, Neckermann, Neckermann Enterprise, Neckermann.at Enterprise, Neckermann Austria Enterprise, Neckermann-Komponenten, Neckermann components
...
[NOTE]
.Save customs tariff numbers
====
For each variation, a customs tariff number must be transmitted to Neckermann.at. As such, ensure that a customs tariff number is saved for all variations that you want to sell on Neckermann.at.
====
...
|Enter the image position of the item image if the energy label is saved as an item image instead of as a characteristic. For further information about the energy label, refer to the <<Providing information on the energy efficiency, Providing information on the energy efficiency>> chapter.
...
In order to link items, characteristics etc. with Neckermann.at, you have to activate the order referrer *Neckermann.at Enterprise* in the *Setup » Orders » Order referrer* menu.

[.instruction]
Activating the order referrer for Neckermann.at:

. Go to *Setup » Orders » Order referrer*.
. Activate the option *Neckermann.at Enterprise*.
. *Save* (icon:save[role="green"]) the settings.
...
In the next step, link your <<item/managing-items#480, characteristics>> with the Neckermann components.

[.instruction]
Linking characteristics:

. Go to *Setup » Item » Characteristics*.
. Expand a characteristic.
...
Neckermann provides you with a file which contains the categories that have to be assigned to the categories of your online store. The categories are then linked and further items from the linked category are assigned automatically.
...
If you have created <<item/managing-items#390, attributes>> in your plentymarkets system, you have to link these attributes with Neckermann. To do so, you have to import the <<Importing components, Neckermann components>> first. Proceed as described below to link the attribute Colour with Neckermann. Link further attributes in the same way.
...
Brand IDs for Neckermann.at can be imported with ElasticSync or they can be entered manually. +

To import brand IDs for Neckerman via ElasticSync, create a sync of the type <<data/importing-data/sync-types/elasticSync-manufacturer#, Manufacturer>>. After you have created the sync, you have to set the import options for the sync, set the matching, and create the mappings.
...
. Carry out the settings. Pay attention to the explanations given in <<#manufacturer-sync-neckermann>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The sync is created. +
→ The matching settings of the sync open.

[[manufacturer-sync-neckermann]]
.Creating a sync
[cols="1,3"]
|====
|Setting |Explanation
...
| *Type*
|Select the type <<data/importing-data/sync-types/elasticSync-manufacturer#, Manufacturer>> from the drop-down list. This sync type is used to import manufacturer data.
...
After running a sync, the file is automatically deleted from the webspace. This is preset by default. In order to save the file you need to remove the check mark. *_Note:_* storing the file on the webspace is subject to a fee.
...
After you created a *Manufacturer* sync, you set the import options. To do so, carry out the settings as described in the <<data/importing-data/ElasticSync#1300, Setting the import options>> chapter on the ElasticSync page.

[#1135]
=== Setting the matching

Next, set the matching for the sync. To do so, carry out the settings as described in the <<data/importing-data/ElasticSync#1400, Setting the matching>> chapter on the ElasticSync page.

[#1140]
=== Creating a mapping

Last, you have to create the mappings for the data fields. To do so, carry out the settings as described in the <<data/importing-data/ElasticSync#1500, Creating a mapping>> chapter on the ElasticSync page.
...
Items have to be available for Neckermann. This is done in the *Availability* tab of an item within the *Item » Edit item »* _Open item_ *» Tab: Variation ID* menu.

[TIP]
.Selling all variations of an item on Neckermann.at
====
To sell all variations of an item on Neckermann.at, carry out the settings described below for the main variation. The settings are then passed on to all variations of the item.
====

[.instruction]
Setting the variation availability for Neckermann.at:

. Go to *Item » Edit item »* _Open item_ *» Tab: Variation ID » Tab: Settings*.
. Open the variation that you want to sell on Neckermann.at.
...
Select the characteristic type *File* and the Neckermann characteristic *Energy label* or *Product data sheet* for the characteristics Energy label and Product data sheet. The following example describes how to create the energy label for Neckermann.at.
...
. Select the characteristic *Energy label* under *Neckermann.at EP characteristic*.
. *Save* (icon:save[role="green"]) the settings.

Link the characteristics in the *Characteristics* tab of an item.

[.instruction]
Linking energy efficiency characteristics:

. Go to *Item » Edit item*.
. Open the item.
. Click on the *Characteristics* tab.
. Activate the characteristics *Energy efficiency class*, *Energy label* and *Product data sheet* that you have just created.
. *Save* (icon:save[role="green"]) the settings. +
→ The characteristics are displayed in the list.
. Upload the image file of the energy label to the *Energy label* characteristic.
. Upload the PDF file of the product data sheet to the *Product data sheet* characteristic.
. *Save* (icon:save[role="green"]) the settings.

icon:map-signs[] *_Or:_* Save the energy label in the item's *Images* tab. To do so, proceed as described below.
...
. Carry out the settings. Pay attention to the explanations given in <<#payment-method-neckermann>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings.

[[payment-method-neckermann]]
...
As there is no possibility to delete cancellations e.g. of individual items in an order, set the quantity of items affected to 0 in this case. Provide a reason for each cancellation, either *Item not in stock* or *Customer cancellation*.
...
You can carry out settings related to the layout of the invoices in the *Setup » Client » Select a client » Locations » Select a location » Documents » Invoice* menu. Go to *Setup » Orders » Events* in order to define which additional procedure should be carried out when the invoice is printed. If you would like the procedure to be limited to orders from the Neckermann market only, then make sure that you create a filter with the *Referrer* filter type and select *Neckermann* within the filter.
