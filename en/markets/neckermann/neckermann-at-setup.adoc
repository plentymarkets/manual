...
You can carry out settings related to the layout of the invoices by going to *Setup » Client » Select a client » Locations » Select a location » Documents » Invoice*. Go to *Setup » Orders » Events* in order to define which additional procedure should be carried out when the invoice is printed. If you would like the procedure to be limited to orders from the Neckermann market only, then make sure that you create a filter with the *Referrer* filter type and select *Neckermann* within the filter.
