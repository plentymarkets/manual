...
Neckermann provides you with a file which contains the categories that have to be assigned to the categories of your plentyShop. The categories are then linked and further items from the linked category are assigned automatically.
...
| *Interval*
|Select an interval for starting the import. +
By selecting the option *Schedule* you can determine how often and at what time the import is started.
...
After running a import, the file is automatically deleted from the webspace. This is preset by default. In order to save the file you need to remove the check mark. +
