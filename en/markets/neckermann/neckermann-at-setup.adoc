...
After joining the Neckermann partner programme, use your access data to set up the market in plentymarkets. You can integrate the platform Neckermann.at Enterprise

 into plentymarkets. To set up Neckermann.at Enterprise in plentymarkets, proceed as follows.
// Set up Neckermann.at Cross-Docking in the same way.

[.instruction]
Setting up Neckermann.at in plentymarkets:

. Go to *System » System settings » Markets » Neckermann Austria Enterprise*.
. Expand the *Basic settings* section.
. Carry out the settings. Pay attention to the explanations given in <<basic-settings-neckermann>>.
. *Save* the settings.
...
|Enter the image position of the item image if the energy label is saved as an item image instead of as a property. For further information about the energy label, refer to the <<Providing information on the energy efficiency, Providing information on the energy efficiency>> chapter.
...
*Item delivery time 1 - 5* = Enter the delivery time in days for each option. Set up the options for item availability in the *System » System settings » Item » Availability* menu.
...
In order to link items, properties etc. with Neckermann.at, you have to activate the order referrer *Neckermann.at Enterprise* in the *System » System settings » Orders » Order referrer* menu.

[.instruction]
Activating the order referrer for Neckermann.at:

. Go to *System » System settings » Orders » Order referrer*.
. Place a check mark next to *Neckermann.at Enterprise*.
. *Save* the settings.
...
. Carry out the desired settings. Pay attention to the explanations given in <<order-status-neckermann>>.
. *Save* the settings.
...
Components of the market Neckermann correspond to the properties in plentymarkets. Properties in plentymarkets have to be linked to Neckermann components. This is done by uploading a CSV file which contains the Neckermann components. You will receive this file after having closed a contract with Neckermann.
...
. Go to *System » System settings » Markets » Neckermann Austria Enterprise*.
. Expand the *Component import* section.
...
. Save the file in CSV format on your computer.
. Go to *System » System settings » Markets » Neckermann Austria Enterprise*.
. Expand the *Import info logo* section.
. Select the field separator.
. Click on *Select file*.
. Open the CSV file.
. *Save* the settings. +
...
*_Or:_* Save the energy label in the item's *Images* tab. To do so, proceed as described below.
...
. *Save* the settings.
. Go to *System » System settings » Markets » Neckermann Austria Enterprise*.
. In the *Basic settings* section, enter the image's position number into the *Image position for energy label* field.
. *Save* the settings.
...
You can carry out settings related to the layout of the invoices by going to *System » System settings » Client » Select a client » Locations » Select a location » Documents » Invoice*. Go to *System » System settings » Orders » Events* in order to define which additional procedure should be carried out when the invoice is printed. If you would like the procedure to be limited to orders from the Neckermann market only, then make sure that you create a filter with the *Referrer* filter type and select *Neckermann* within the filter.
