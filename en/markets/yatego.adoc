...
[.indented]
The stock limitation is set in the *Item » Edit item » Open item » Tab: Variation ID » Tab: Settings » Area: *Visibility* menu. Variations for which the option *Limitation* is set to *None* or *Do not administer stock for this variation* are also transferred to Yatego. plentymarkets sends a value of -1 for these variations, so that Yatego knows that these variations can be bought without limitation.
