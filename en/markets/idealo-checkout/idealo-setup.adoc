...
| *Event*
| Select an event, e.g. *Status change: [8] Cancelled*
|
...
| *Event*
| Select an event, e.g. *Status change: [8.1] Cancelled by buyer*
|
...
====
The *Send refund notification to idealo* event procedure will only be triggered for orders that are paid for using the payment method *idealo Checkout payments*. The event procedure cannot be used for orders that are paid using PayPal.
====
