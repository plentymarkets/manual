...
- *Variation availability:* In addition to the order referrer *idealo*, also activate the <<#500, order referrer>> *idealo Checkout* in the *Markets* area of the *Item » Edit item » [Open item] » Tab: Variation ID » Tab: Availability* menu.
...
| *checkoutApproved*
|Is true if the order referrer idealo Checkout within *Items » Edit item » [Open item] » [Open variation] » Availability » Markets* is set or a characteristic of type *No* and the link to the idealo Checkout characteristic *CheckoutApproved* is configured.
...
For items which have the shipping method shipping company, it is also possible to transfer the additional services *TwoManHandlingPrice* and *DisposalPrice* to idealo Checkout. DisposalPrice can only be used in combination with TwoManHandlingPrice. That means you have to create the characteristic *TwoManHandlingPrice* as well as the property *DisposalPrice* and link them to the item. In both cases, select *decimal number* as the characteristic type. Go to *Item » Edit item » [Open item] » [Open variation] » Tab: Characteristics* and enter the surcharge for the additional service with two decimal places behind the decimal separator. Use a period for the decimal separator (19.99).
