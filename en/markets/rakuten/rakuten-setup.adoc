...
. Carry out the settings. Pay attention to the explanations given in <<rakuten-setup>>.
...
Add SKUs manually for specific referrers in the *Item » Edit item » Open item » Tab: Variation ID* menu.
...
. Click on *New export*.
. Carry out the settings. Pay attention to the information given in <<settings-item-export>>.
. Carry out further settings for item filters and format settings. Pay attention to the information given in table 2 on the <<data/exporting-data/elastic-export#, Elastic export>> page of the manual.
. *Save* (icon:save[role="green"]) the settings. +
...
. Carry out the settings. Pay attention to the information given in <<payment-method-rakuten>>.
...
[.indented]
If the automatic CSV import is set up for Rakuten.de, then the new stock is synchronised by importing the CSV file again. Define how often the synchronisation should take place in the Rakuten.de back end.
