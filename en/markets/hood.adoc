...
:keywords: Hood, market, marketplace, listing, layout template, marketplace Hood, setting up Hood, Hood setup, multichannel, multi-channel, multi channel
...
You need to book a store in order to upload item variations to Hood. Once you have booked a store on Hood, the items are integrated into Hood with category links. Set up the shop categories directly on link:https://www.hood.de/tips/924/artikel-anlegen.htm[Hood^]{nbsp}icon:external-link[]. Then load the categories into plentymarkets. This allows you to select them in the listing.
...
The following table explains the template variables and functions in the *Layout templates* area in alphabetical order:
...
Examples: link:http://www.sellerfox.de/[http://www.sellerfox.de, window="_new"], link:http://www.ayee.net/[http://www.ayee.net, window="_new"]{nbsp}icon:external-link[]
...
Create multiple listings with the *Group function* or the *Batch processing* function in the *Item* menu. The group function creates listings for previously selected items. The batch processing function creates listings for previously filtered items. If no *listing template* is selected in the *Item » Edit item »* _Open item_ *» Tab: Multi-Channel* menu in the *Listing default settings* area, then eBay listings are created automatically. If you want to create multiple Hood listings, then make sure you have selected a *listing template* with specific settings for Hood in the *Item » Edit item »* _Open item_ *» Tab: Multi-Channel* menu in the *Listing default settings* area.
...
Use ElasticSync in the *Data » Import* menu to exchange data between plentymarkets and Hood. Listing data can be imported into plentymarkets with the <<data/importing-data/sync-types/elasticsync-market-listing#, Market listing>> sync type.
