...
Enter your feedback texts first. Then set up an <<basics/automation/event-procedures#, event procedure>> to automatically send your feedback texts to your buyers. All buyers who gave you feedback then automatically receive a random feedback text in return.
...
Set up an <<basics/automation/event-procedures#, event procedure>> to automatically send a shipping confirmation to Hood after the outgoing items have been booked.
...
Set up an <<basics/automation/event-procedures#, event procedure>> to automatically send a payment confirmation to Hood.
...
====
If you use template variables which contain properties, such as *{% GetItemPropertyById() %}* and *{% GetItemPropertyByName() %}*, you have to activate the option *Wildcard character in layout template (Hood)* for the selected property, so that the property is displayed in the layout template. This is done in the *System » Item » Properties* menu.
====
