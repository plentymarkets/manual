...
. Go to *System » System settings » Markets » Hood*.
...
. Go to *System » System settings » Markets » Hood*.
. Enter up to four feedback texts in the *Automatic feedback* section.  +
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<automatic-shipping-confirmation>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out further settings. Pay attention to the explanations given in <<automatic-shipping-confirmation>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<automatic-payment-confirmation>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out further settings. Pay attention to the explanations given in <<automatic-payment-confirmation>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
You need to book a store in order to upload item variations to Hood. Once you have booked a store on Hood, the items are integrated into Hood with category links. Set up the shop categories directly on link:https://www.hood.de/tips/924/artikel-anlegen.htm[Hood^]. Then load the categories into plentymarkets. This allows you to select them in the listing.

[.instruction]
Loading store categories for Hood:

. Go to *System » System settings » Markets » Hood*.
. Click on *Update* in the *Store categories* section. +
...
In order to be able to use your own properties for the Hood market, the properties have to be linked with Hood in the *System » System settings » Item » Characteristics* menu. You can link up to 12 properties per item and use a maximum of 16 characters per name.
...
The content that you want to display for listings can be saved using template variables and functions in the *Main frame* tab in the *System » System settings » Listings » Layouts* menu. The content is structured with HTML and CSS. The different tabs are described below.
...
====
If you use template variables which contain properties, such as *{% GetItemPropertyById() %}* and *{% GetItemPropertyByName() %}*, you have to activate the option *Wildcard character in layout template (Hood)* for the selected property, so that the property is displayed in the layout template. This is done in the *System » System settings » Item » Characteristics* menu.
====
...
|*Stock dependence*
|Select the <<stock-management/managing-stocks#, stock>> of the listing. Select one of the following options: +
...
Create multiple listings with the *Group function* or the *Batch processing* function in the *Item* menu. The group function creates listings for previously selected items. The batch processing function creates listings for previously filtered items. If no *listing template* is selected in the *Item » Edit item » Open item » Tab: Multi-Channel* menu in the *Listing default settings* section, then eBay listings are created automatically. If you want to create multiple Hood listings, then make sure you have selected a *listing template* with specific settings for Hood in the *Item » Edit item » Open item » Tab: Multi-Channel* menu in the *Listing default settings* section.
...
In plentymarkets, there are multiple ways to automatically start listings or restart listings once they have already been started. Use the listing scheduler to specify *starting times* for listings. Go to *System » System settings » Listings » Listing scheduler* to set up the listing scheduler.
...
Use ElasticSync in the *Data » ElasticSync* menu to exchange data between plentymarkets and Hood. Listing data can be imported into plentymarkets with the <<data/importing-data/sync-types/elasticsync-market-listing#, Market listing>> sync type.
