...
First register with link:https://www.hood.de/kostenlos-registrieren.htm?sec=1[Hood^]. Create an API password which you need when setting up the interface to Hood in plentymarkets.
...
====
Orders from Hood are imported into your plentymarkets system every hour and are available in the <<orders#, Orders>> menu. You can recognise Hood orders by the information displayed in the *Referrer* column. For lack of space, only the ID 125 is displayed. Point your cursor over an ID to view the referrer.
====
...
Set up an <<automation/event-procedures#, event procedure>> to automatically send a shipping confirmation to Hood after the outgoing items have been booked.
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<automatic-shipping-confirmation>> below.
. *Save* the settings. +
→ The event is created.
.  Carry out further settings as described in <<automatic-shipping-confirmation>> below.
.  Place a check mark next to the option *Active*.
. *Save* the settings. +
→ The event procedure is saved.
--
...
Set up an <<automation/event-procedures#, event procedure>> to automatically send a payment confirmation to Hood.
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<automatic-payment-confirmation>> below.
. *Save* the settings. +
→ The event is created.
.  Carry out further settings as described in <<automatic-payment-confirmation>> below.
.  Place a check mark next to the option *Active*.
. *Save* the settings. +
...
. Go to *System » System settings » Markets » Hood*.
. Click on *Update* in the section *Store categories*. +
→ The store categories are displayed.
. Update the categories again if you have added new store categories on Hood.
...
The status *Hidden* is assigned to Hood listings of the type *shop item* that do not have any stock. These listings are not ended.
This applies to Hood listings of the type *shop item* with the stock dependence *limited (without reservation)* and *limited (with reservation)*. These listings are not ended in plentymarkets either. Instead, they are displayed with the status *Hidden*. As soon as the item is back in stock in plentymarkets, the listing is reactivated on Hood during an automatic stock update.

[#1160]
===  Linking your own properties

In order to be able to use your own properties for the Hood market, these have to be linked with Hood in the *System » System settings » Item » Properties* menu. You can link up to 12 properties per item and use a maximum of 16 characters per name.
...
. Carry out the settings. Pay attention to the information given in <<basic-settings-listings>>.
. *Save* the settings.
...
. Carry out the settings. Pay attention to the information given in <<options-stock>>.
. *Save* the settings.
...
*deactivated* = The listing is only listed if there is sufficient stock available. In this case, sufficient means that stock is larger than the <<Carrying out the basic settings, maximum quantity for sale>> selected in the listing.
...
. Carry out the settings. Pay attention to the information given in <<settings-checkout>>.
. *Save* the settings.
...
. Carry out the settings. Pay attention to the information given in <<settings-scheduler>>.
. *Save* the settings.
...
====
If you use template variables which contain properties, such as *{% GetItemPropertyById() %}* and *{% GetItemPropertyByName() %}*, you have to activate the option *Wildcard character in layout template (Hood)* for the selected property, so that the property is displayed in the layout template. This is done in the *System » System settings » Item » Properties* menu.
====
...
.Use if-logic
====
You can use the <<en/omni-channel/online-store/setting-up-clients/cms-syntax#basics-if, if/else logic>> for layout templates. It functions the same way that the logic works in the <<omni-channel/online-store/setting-up-clients/cms-syntax#web-design, plentymarkets web design>> area.
====
...
. Carry out the settings. Pay attention to the information given in <<new-market-listing>>.
. *Save* the settings. +
...
====
Hood requires you to specify the EAN for a better Google ranking. If you carried out the settings so that neither an EAN nor any other item number is transferred, the error message *Hood: Error #0 Please enter a brand or a manufacturer.* will be displayed. +
...
.  Carry out the settings. Pay attention to the information given in <<editing-market-listing>> , <<base-tab>> and <<description-layout>>.
. *Save* the settings.
...
.  Carry out the settings. Pay attention to the information given in <<group-function>>.
...
.  Select one or several filters. Pay attention to the explanations given in <<filter-options-batch-processing>>.
.  Carry out the settings. Pay attention to the information given in <<group-function>>.
.  Click on *Execute* to apply the settings to the listings.
...
.  Select a starting time. Pay attention to the information given in <<start-time>>.
...
.  Select one or several filters. Pay attention to the explanations given in <<filter-options-batch-processing>> and <<filter-listings>>.
