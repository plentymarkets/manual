...
. Go to *System » Markets » PIXmania » Settings*.
...
|*Availability 1 - 5*
|Enter the delivery time in days for each option into the *Domestic* and *Foreign country* fields. Only the days that are entered here will be transferred to PIXmania as a delivery period. 0 is not allowed as a number of days. You have to enter a natural number. Set up the options for item availability in the *<<item/managing-items#, System » Item » Item availability>>* menu.
|====
...
. Go to *System » Markets » PIXmania » Category link*.
. Click on *Browse...*.
. Select the *field separator*.
. *Save* the settings. +
...
In the *System » Markets » PIXmania » Export history* menu, you can see an overview of the export history of your PIXmania items. Export histories *in progress* as well as *completed* export histories are displayed here.
