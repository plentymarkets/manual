...
Items have to be available for PIXmania. This is done in the *Availability* tab of an item within the *Item » Edit item »* _Open item_ *» Tab: Variation ID* menu.
...
Import the brand IDs into your plentymarkets system using ElasticSync. To do so, create a sync of the type <<data/importing-data/sync-types/elasticSync-manufacturer#, Manufacturer>>. After you have created the sync, you have to set the import options for the sync, set the matching, and create the mappings.
