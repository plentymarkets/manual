...
!===
! *hourly cron*
! This cron is deactivated very soon. As such, set up the event procedure <<#shipping-confirmation, Send shipping confirmation to Kaufland.de>> now. Then set the option *Send shipping confirmations via* to *event procedure*.

*_Background:_* Kaufland is switching to a new API version. As such, the hourly cron for shipping confirmations will be switched off soon. As such, set up the event procedure <<#shipping-confirmation, Send shipping confirmation to Kaufland.de>> now.
...
[#in-fulfillment]
=== NEW: Sending in fulfilment confirmations

Set up an <<automation/event-procedures#, event procedure>> to automatically inform {market} when an order is handed over to the warehouse.
