...
====
Note that event procedures for OTTO market must be triggered in a specific sequence. First, the event procedure <<Generating a DHL return label, Generate DHL Retoure Online label>> must be triggered, next the event procedure <<Sending an order confirmation to OTTO, Send order confirmation to OTTO>>, and then the event procedure <<Sending a shipping confirmation to OTTO, Send shipping confirmation to OTTO>>. If the event procedures are not triggered in this sequence, the shipping confirmation cannot be sent to OTTO market.
====
