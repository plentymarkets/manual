...
a| As a minimum, plentymarkets data fields must be mapped to all mandatory fields of the marketplace. The mandatory fields are different for different OTTO category groups.
...
+
These errors indicate that you changed an OTTO item attribute. However, OTTO does not accept the change and it is not possible to the change the attribute for this item. The indicated attribute can relate to a variation attribute or a title attribute. Other OTTO attributes are important here, too.
+
[TIP]
.Items cannot be deleted at OTTO Market
====
