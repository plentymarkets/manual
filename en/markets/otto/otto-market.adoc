...
====
Note that event procedures for OTTO market must be triggered in a specific sequence. First, the event procedure <<Generating DHL return labels, Generate DHL Retoure Online label>> must be triggered, next the event procedure <<Sending order confirmations to OTTO, Send order confirmation to OTTO>>, and then the event procedure <<Sending shipping confirmations to OTTO, Send shipping confirmation to OTTO>>. If the event procedures are not triggered in this sequence, the shipping confirmation cannot be sent to OTTO market.
====
...
. Carry out the settings as described in <<dhl-return-label-otto>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
...
[#600]
=== Sending order confirmations to OTTO
...
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the settings as described in <<order-confirmation-otto>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the settings as described in <<shipping-confirmation-otto>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<return-rejection-otto>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out further settings as described in <<return-rejection-otto>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
|*Event*
|Select an event, for example *Order generation: Status change* +
...
+
Screenshots illustrating how OTTO market orders are created in plentymarkets and in the *Mein Konto* (My account) section at OTTO market are provided below.
+
[[otto-market-order-number]]
."Mein Konto” (My account) section at OTTO market
...
[[sku]]
.Parent SKU and SKU
image::markets/otto/assets/otto-market_sku_parent-sku.png[SKU]
+
