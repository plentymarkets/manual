...
a| By default, purchase receipts are imported as normal order documents since 15/12/2020. Up to that date, the order documents provided by OTTO Market were imported with the document type *External invoice* even though the documents were not proper invoices. If you want to continue to import these documents with the document type *External invoices* due to dependencies to other exports, interfaces or software, activate the option *Import purchase receipts as external invoices*. Visit the link:https://forum.plentymarkets.com/t/aenderung-des-dokumententyps-der-kaufbelege-von-otto-change-of-document-type-of-purchase-receipts-from-otto/619731[plentymarkets forum^]{nbsp}icon:external-link[] for more details.
...
| *Select a shipping service provider to be used as fallback for the above settings.*
a| Select the shipping service provider that is to be used if return tracking numbers are added to the system via REST API. For return tracking numbers that are added via REST API, the settings for *OTTO Market shipping service provider for returns* cannot be appied. Return tracking numbers are for example added via REST API if you work with an external fulfilment company that is connected via this API.
...
* *Alternative return shipping service provider* = Select a shipping service provider from the drop-down list. This shipping service provider is used for all shipments to which the settings above cannot be applied.
...
*_Note:_* Every shipping confirmation that you send to OTTO Market must contain a tracking number to be used for a possible return. If you select the option *No alternative shipping service provider*, the shipping confirmation may not be sent to OTTO Market.
...
To export a variation to OTTO Market, this variation must meet the requirements listed in <<#otto-market-variation-mandatory>>.

[[otto-market-variation-mandatory]]
.Requirements for exporting variations
[cols="2,3"]
|====
|Requirement |Explanation

| Is the variation active?
a| The variation must be activated.
...
a| The variation must be linked to the property *OTTO category group*.
...
| Is the net stock positive?
a| The net stock of the variation must be positive.
...
a| The catalogue that the variation is associated with must be activated.

 . Go to *Data » Catalogues*. +
→ The catalogues are listed.
. Scroll to the OTTO Market catalogue.
. In the *Active/Inactive* column, move the toggle button (icon:toggle-on[role="green"]) to the right.

| Are the data fields mapped?
a| As a minimum, plentymarkets data fields must be mapped to all mandatory fields of the marketplace. The mandatory fields are different for different OTTO category groups. All mandatory fields in the catalogue are marked with an asterisk (*).

* <<#600, How to map one or more plentymarkets data fields to the marketplace data feld.>>

| _Couldn’t find any errors?_
a| You have confirmed that the above settings are correct but the variation is still not exported? Then check the data log.
...
* <<#1302, Why aren't my items being exported to OTTO Market?>>
...
* <<#1305, The log shows an error. How do I fix this error?>>
  ** <<#1306, Duplicate entry. Combination of the fields variationId, marketId and accountId must be unique.>>
  ** <<#1307, Code: 200009 - ATTRIBUTE_CHANGE_DENIED or title: Der Wert dieses Feldes kann nicht geändert werden.>>
  ** <<<<#1308, productDescription.brand>>
  ** <<<<#1309, mediaAssets.IMAGE>>
  ** <<#13010, delivery.type>>
  ** <<<<#13011, productDescription.category>>
  ** <<#13012, Error messages concerning order processing: Carrier ID (X) not mapped in OTTO market config.>>
...
[#1305]
=== The log shows an error. How do I fix this error?

The following chapters list common error messages in the log and helps you to resolve them.
