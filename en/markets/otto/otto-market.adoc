...
* Update of item data once every night
...
[.instruction]
Linking optional data fields in the catalogue:

. Go to *Data » Catalogues*.
. Click on the OTTO Market catalogue. +
→ The catalogue opens. +
→ The market's data fields are displayed on the left-hand side. +
*_Tip:_* Mandatory fields are marked with an asterisk. These mandatory fields were already mapped in the *OTTO Market Creating catalogues* assistant.
...
An example of how to map the data field *Stock* to data saved in plentymarkets is provided below. The data field *Stock* defines the warehouse from which stock should be exported to OTTO Market.
...
*_Tip:_* If you would like to transfer stock from multiple warehouses to OTTO Market, click on *Add source* (icon:link[role="yellow"]) and add more warehouses.
. *Save* (icon:save[role="green"]) the settings. +
→ If you do not map the data field *Stock* to a warehouse, then the sum of the net stock of all distribution warehouses (data source *Virtual warehouse*) will be exported to OTTO Market.

[IMPORTANT]
====
.Map all mandatory fields
The mandatory fields for the templates are marked with an asterisk. To make sure that all necessary item data is provided, map at least the mandatory fields. For OTTO Market, mandatory fields are already mapped in the *OTTO Market Creating catalogues* assistant.
====
...
*_Note:_* Select a status that suits your workflow. For example, you can create the status *[8.3] Send return to OTTO*.
|
...
| *_Note:_* The action *Send return to OTTO* triggers a credit note and the buyer receives a refund.
|====
...
| *Event*
|Select an event, for example *Order change: Status change* +
*_Note:_* Select a status that suits your workflow. For example, you can create the status *[8.2] Reject OTTO return*.
|
