...
If you created an OTTO market order in your plentymarkets system, then you have to generate a DHL return label for this order prior to confirming the order. The event procedure *Generate DHL return label* creates a tracking number for a return (retTrackingNumber) which is saved for the order. This is to ensure that a tracking number exists if the customer wishes to return the item.
