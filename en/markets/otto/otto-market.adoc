...
[.instruction]
Setting the variation availability for images:
...
The following chapters describe event procedures for generating a DHL return label and for sending order confirmations to OTTO market. Set up further event procedures in the same way.

[#550]
=== Generating a DHL return label with an event procedure
