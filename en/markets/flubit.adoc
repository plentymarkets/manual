...
. Carry out the settings. Pay attention to the information given in <<basic-settings-flubit>>.
. *Save* the settings.
...
. Carry out the settings. Pay attention to the information given in <<payment-method-flubit>>.
...
The menu <<data/api-log#, Data » API log>>, tab *Flubit* contains a history of processes that run via the Flubit interface. The following formats are available for Flubit:
