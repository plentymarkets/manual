...
. Carry out the settings. Pay attention to the information given in <<basic-settings-bol>>.
. *Save* the settings.
...
Next create <<item/managing-items#480, properties>> and link these to bol.com properties. Matching properties to a bol.com category can be found within the bol.com data model file. Download the data model file link:https://developers.bol.com/datamodel/[here^].
...
. Carry out further settings for item filters and format settings. Pay attention to the information given in table 5 on the <<data/exporting-data/elastic-export#, Elastic export>> page of the manual.
. *Save* the settings. +
...
. Carry out the settings. Pay attention to the information given in <<payment-method-bol>>.
...
The menu <<data/api-log#, Data » API log>> contains a history of processes that run via the bol.com interface.
