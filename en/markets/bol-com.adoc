...
* In the assistant *Base settings - bol.com* (<<#205, How it’s done>>)
* In the *Setup » Markets » bol.com » Settings* menu (<<#206, How it’s done>>) +
...
| *bol_fulfillment*
| The characteristic *bol_fulfillment* determines if the variations of an item are shipped by you or by bol.com.

* _Characteristic not linked to item_ = You ship the variations.
* _Characteristic linked to item_ = bol.com ships the variations (FBB/LVB).

When you create the characteristics, in the line *bol.com characteristic*, enter the bol.com characteristic *bol_fulfillment*.
...
When you create the characteristics, in the line *bol.com characteristic*, enter the bol.com characteristic *bol_used*.
...
To sell a variation with Fulfilment by bol.com (FBB or LVB), the settings described in <<#table-bol-fbb>> are necessary:

[[table-bol-fbb]]
.Settings for bol.com FBB
[cols="1,3a"]
|====
|Setting |Explanation

| Linking the characteristic
| Create the characteristic *bol_fulfillment* and link it to the variation.
...
[.instruction]
Creating the characteristic bol_fulfillment:
...
. Enter the Dutch name that you want to use for this characteristic into the *Name* field.
. Select the option *Text* from the *Characteristic type* drop-down list.
. In the row *bol.com characteristic*, enter the text *bol_fulfillment*.
. *Save* (icon:save[role="green"]) the settings.

[.instruction]
Linking the characteristic bol_fulfillment to an item:

. Go to *Item » Edit item*.
. Open the item.
. Click on the *Characteristics* tab.
. In the *Activate further characteristics* area, activate the characteristic *bol.fulfillment*.
. *Save* (icon:save[role="green"]) the settings. +
→ The characteristic is displayed in the *Characteristics* area.
...
| Availability
a| Activate the market availability *bol.com FBB* for the variation.
