...
. Expand (icon:plus-square-o[]) the *eBay shop categories* section to display an overview of your account’s shop categories. +
...
*No (recommended)* = The listing ID is transferred with the variation ID.
...
| Select whether the image of the first variation should be used as the gallery image if no gallery image is saved for the item. +
...
|Activate or deactivate the <<Activating the automatic stock update, automatic stock update>> for GTC offers (= listings with a fixed price that are automatically restarted after 30 days) +
...
Variation availabilities are organised in the *Setup » Item » Availability* menu and set in the *availability* section of the *Item » Edit item » Open item » Tab: Variation ID » Tab: Settings* menu of each item.
|====
...
Go to *Setup » Markets » eBay » Settings » Tab: Shipping profile links* to link eBay shipping profiles to the shipping profiles of your online store. A linked shipping profile is automatically used during the order import if the shipping profile matches the order data, e.g. account and market settings, of a listing and is activated in the *Global* tab of an item. If the linked shipping profile is deactivated, the active shipping profile with the highest priority is automatically used instead.

Before linking eBay shipping profiles to the shipping profiles in your online store, you have to enter terms of payment and shipping as well as general conditions in the *Setup » Markets » eBay » Business policies* menu.
...
. Click on the tab of the eBay platform that you would like to link return reasons for, e.g. *Germany*.
...
icon:map-signs[] *_Or:_* Set up an <<automation/event-procedures#, event procedure>> to automatically post feedback on eBay.
...
. Enter a name.
. Select the *event* listed in <<automatic-feedback>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out further settings. Pay attention to the explanations given in <<automatic-feedback>>.
...
Add SKUs manually for specific referrers in the *Item » Edit item » Open item » Tab: Variations » Open variation » Tab: Settings* menu if needed.
...
An order is automatically created in plentymarkets after a customer has purchased an item on eBay. After an incoming order you can send an email message to the customer or to the customer’s eBay user account. If you have created an <<crm/sending-emails#1200, email template>> in the *Setup » Client » Select client » Email » Automatic* menu, an email is sent. +

icon:map-signs[] *_Or:_* You can also send messages via the eBay messaging system.

The language versions are assigned to the countries of delivery depending on the languages that were set for each country under *Setup » Orders » Shipping » Settings » Tab: Countries of delivery*.
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<automatic-ebay-messages>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the further settings. Pay attention to the explanations given in <<automatic-ebay-messages>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
icon:map-signs[] *_Or:_* You can also import and update parts compatibility listings with ElasticSync in the *Data » ElasticSync* menu. Use the <</data/importing-data/sync-types/elasticsync-parts-compatibility-listing#, parts compatibility listing>> sync type. For further information, refer to the <<data/importing-data/elasticsync-best-practices/best-practices-elasticsync-parts-compatibility-listing#, Creating parts compatibility listings>> best practice.
...
====
Enter an amount for the item into the *SCO minimum price* field, e.g. 100.00 EUR. The highest bidder’s bid is 110.00 EUR and wins the auction. All defeated bidders with a bid equal to or higher than the entered amount of 100.00 EUR automatically receive an SCO to buy the item at their last bid amount. All defeated bidders with a lower bid than the entered amount do not receive an SCO. +
...
This chapter teaches you how to define *Terms of payment*, *Shipping condition*s, and a *Return policy* for your eBay listings and how to create *Shipping profiles*. These settings must be carried out for every eBay account and every platform individually.
...
. Carry out the settings. Pay attention to the explanations given in <<settings-payment-method-ebay>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings.

[[settings-payment-method-ebay]]
...
GTC offers for which *Yes* was selected for the *Use "out of stock" option* are not ended automatically. For GTC offers, the Use “out of stock” option takes priority over this setting. +
...
|Select *activated* or *deactivated*. +
*activated* = If the settings *limited (with reservation)* or *limited (without reservation)* are saved in the listing for the *Stock dependence* option, then the quantity is adjusted to the net stock. +
...
*Status 1* = The customer is guided through the order process and has to actively complete all steps. This takes place within the online store and therefore, customers are encouraged to buy additional items. +
...
Down-selling means that customers can reduce the quantity purchased on eBay during the checkout. Up-selling is generally possible for fixed-price items. This is why up-selling cannot be selected here.
...
|Select *Yes* or *No*. +
*Important:* Neither up-selling nor down-selling is possible if you selected *No*. This is even true if you selected *Enable* for the option *Down-selling in the checkout process*.
...
*Important:* The setting is only visible if you selected *transfer from eBay* for the *Shipping costs for new orders coming from eBa*y option. +
...
. Pay attention to the explanations given in <<layout-template>> and carry out the settings in the *Settings* tab as desired.
. *Save* (icon:save[role="green"]) the settings.
...
Now link the listings to the layout template. To do so, you need to already have created listings in plentymarkets. How to create listings in plentymarkets is described in the <<Creating listings, Creating listings>> chapter. +
...
. Pay attention to the explanations given in <<template-variables>>.
. *Save* (icon:save[role="green"]) the settings. +
...
The following table explains the template variable and functions in the *Layout templates* section in alphabetical order:
...
For example, you can have eBay galleries created here: link:http://www.ayee.net/newpage/[www.ayee.net^]
...
.Use if-logic
====
You can use the if/else logic for layout templates. It functions the same way that the logic works for the <<online-store/online-store/cms#web-design, plentymarkets web design>>.
====
...
====
You can define a listing template in the *Listing default settings* section of the *Multi-Channel* tab of an item. An eBay listing is created for an item using the group function or batch processing if no *listing template* was defined in the item’s settings.
====
...
Uploading listings with variations is only possible on certain platforms, e.g. Germany, USA, United Kingdom and Italy. Additionally, the function is restricted to certain categories (e.g. clothes). For further information, refer to eBay’s link:https://www.ebay.co.uk/help/selling/listings/creating-managing-listings/creating-listings-variations?id=4150[Creating listings with variations^] page.
...
*Important:* Update the listing one time with the group function if you change the setting for an active listing from transfer RRP to do not transfer RRP. To do so, select the options *Reset RRP* and *Quantity, price and variations* within the group function in the *Listings » Active listings* menu.
...
Copy a market listing, e.g. if you want to upload the same listing with a different variation to another platform. The copied listing has the same settings as the original. You only have to change the platform or the variation after copying.
...
The *None* option must be activated in the *Procedures* section.
|====
...
*Note:* Only updates which contain more than 10 listings are displayed in this window. If you update fewer than 10 listings, then the listings will be updated synchronously.
====
...
[#5950]
== Future listings processes

//Prozess, um Listings zu starten, wurde vebessert (hochladen geht schneller)

Future listings are listings with a starting time in the future. The *Listings » Listing processes* menu contains an overview of all listings that have a starting time in the future. You can open market listings directly from this overview to edit or check them. Moreover, you can delete and thus prevent listings from being started in this menu. +
...
Use the filters on the left-hand side to further narrow down the search results and to search for specific future listings. <<filters-future-listings>> contains an overview of all available filters.
...
|*Job type*
|The listing's job. Listings can have the jobs *Start*, *Update*, and *End*.

|*Status*
|Select a status to only display listings with this status. +
Listings in this menu can have the statuses *Planned*, *Started*, and *Ended*.

|*Platform*
|Select a platform to only display listings that should be listed on this platform.

|*eBay account*
|Select an eBay account to only display listings that should be started with this eBay account.

|*Market listing ID*
|The listing's market listing ID. Enter one or more market listing IDs separated by commas to display listings with those IDs.

|*External market listing ID*
|The external market listing ID. Enter one or more external market listing IDs separated by commas to display listings with those IDs.

|*Tags*
|Tags that the listing was tagged with. Select one or more tags to only display listings with those tags.

|*Date*
|Enter a date to display listings that should be started on this date.
//Listings anzeigen, die an diesem Tag erstellt, gestartet oder abgeschlossen wurden. ???

|*from*
|Display listings that should be started on this day or after.

|*to*
|Display listings that should be started on this day or before.
|====

[#5960]
=== Deleting future listings

Use the group function to delete multiple future listings at once and thus prevent the listings from being started. First, select the listings that you would like to delete by placing a check mark next to them.

[.instruction]
Deleting future listings with the group function:

. Go to *Listings » Listing processes*.
. Place a check mark next to the listings that should be deleted.
. Click on *Group function*. +
→ The *Group function* section is expanded.
. Select *Delete*.
. Click on *Execute* to delete the selected listings and thus cancel them from being started.

[#6000]
== Uploading offers with variations

You can upload items with multiple variations as a single eBay offer. In order to create variations, you have to create attributes and link them with your items. Variations are <<item/introducing-items#_what_is_a_variation, combinations of different attributes>>. Surcharges are applied to every combination, e.g. size, colour and material. Variations linked with a listing are also transferred to eBay when starting, restarting or updating the listing. For further information, refer to eBay’s link:https://www.ebay.co.uk/help/selling/listings/creating-managing-listings/creating-listings-variations?id=4150[Creating listings with variations^] page.
...
.eBay OAuth2 plugin
====
In order to use ePID matching, the link:https://marketplace.plentymarkets.com/en/plugins/channels/marketplaces/ebayoauth2_4787[eBay OAuth2 plugin^] is required. The plugin can be downloaded from the link:https://marketplace.plentymarkets.com/en/plugins/channels/marketplaces[plentyMarketplace^].
====
...
One of the requirements of eBay Plus is link:https://verkaeuferportal.ebay.de/ebay-plus-program-sellers-english#24899anchor5[free and fast shipping^]. Not all shipping methods are suitable for eBay Plus. For further information about suitable shipping methods, refer to eBay. Select at least one permitted shipping method for eBay. Otherwise, an error message will be displayed when starting an eBay Plus listing.
...
Activate Click & Collect by placing a check mark next the option *Buyers can collect the item at Argos with Click & Collect* in the *Setup » Markets » eBay » Business policies » Tab: eBay shipping conditions* menu. Next, you have to link the shipping conditions with the shipping profile from eBay that is saved for the listing.
...
. Place a check mark next to the *Buyers can collect the item at Argos with Click & Collect* option.
. *Save* (icon:save[role="green"]) the settings.
. Click on the *Shipping profiles* tab.
. Now select the shipping condition that you just edited for the *eBay shipping conditions* setting within the *General information* section.
. *Save* (icon:save[role="green"]) the settings.
...
. Click on *Group function*. +
→ The *Group function* window opens.
. Select the shipping profile that is linked with Click & Collect for the *Shipping information* option in the *Update eBay options* section.
. Click on *Execute* to update the listings.
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<event-procedure-accepting-returns>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the further settings. Pay attention to the explanations given in <<event-procedure-accepting-returns>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<event-procedure-return-own-label>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the further settings. Pay attention to the explanations given in <<event-procedure-return-own-label>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<event-procedure-rejecting-returns>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the further settings. Pay attention to the explanations given in <<event-procedure-rejecting-returns>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<event-procedure-partial-refund>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the settings as described in <<event-procedure-partial-refund>> below.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<event-procedure-own-label>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the further settings. Pay attention to the explanations given in <<event-procedure-own-label>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<event-procedure-mark-return>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the further settings. Pay attention to the explanations given in <<event-procedure-mark-return>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<event-procedure-automatic-refund>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the settings as described in <<event-procedure-automatic-refund>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<event-procedure-sent-refunds>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the further settings. Pay attention to the explanations given in <<event-procedure-sent-refunds>> below.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The event procedure *Initiate eBay refund (PayPal)* is triggered and the new status is transferred to eBay. +
...
.  The return request is imported into plentymarkets and saved with the standard status for returns.
.  The seller checks the reason for return under *Orders » Edit orders » Open return » Tab: Stock units » Note*.
.  The seller offers a new price to the purchaser. The seller enters a suggestion for the new item price under *Orders » Edit orders » Open return » Tab: Edit* next to the option *Price [gross]*. +
icon:map-signs[] *_Or:_*  Alternatively, the seller enters the new price into the *Credit note*.
...
If you have linked the eBay purchase cancellation statuses with your own statuses as described above, then purchase cancellation requests are imported with status *8.6 Purchase cancellation requested. By changing the order status, you can process each request individually.

[TIP]
.Status of cancelled orders
====
Consider your setting for the *Status of cancelled orders* option in the *Setup » Orders » Settings* menu when setting up purchase cancellations. The *Status of cancelled orders* must match the statuses selected below. Otherwise, it might happen that the stock stays reserved. +
The status in the example below was selected as an example. Select the status that best suits your workflow. To do so, select an already existing status or <<orders/managing-orders#1220, create>> a new status.
====

[.instruction]
Changing the order status request:

. Open the *Orders » Edit orders* menu.
. Search for orders with the status *[8.6] eBay: Purchase cancellation requested*.
. Select an order.
. Change the order’s status to *[8.5] eBay: Purchase cancellation accepted*.
. *Save* (icon:save[role="green"]) the settings. +
→ Create the event procedure described below to automatically inform eBay about your decision.

Use the group function to change multiple orders at once. If you want to accept the request for unpaid orders, then search for all unpaid orders with status 8.6 and change the orders’ status with the <<orders/managing-orders#1700, group function>>.
...
You can accept requests in 2 different ways. You can either change the order status or set up an event procedure.
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<mark-paid-orders>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the further settings. Pay attention to the explanations given in <<mark-paid-orders>> below.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<automatically-reject-purchase-cancellation>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the further settings. Pay attention to the explanations given in <<automatically-reject-purchase-cancellation>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<message-purchase-cancellation-accepted>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the further settings. Pay attention to the explanations given in <<message-purchase-cancellation-accepted>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--

[[message-purchase-cancellation-accepted]]
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<message-purchase-cancellation-rejected>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the further settings. Pay attention to the explanations given in <<message-purchase-cancellation-rejected>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--

[[message-purchase-cancellation-rejected]]
...
→ The *Create new event procedure* window opens.
. Enter a name.
. Select the *event* listed in <<start-purchase-cancellations>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
. Carry out the further settings. Pay attention to the explanations given in <<start-purchase-cancellations>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
The *Processing status* section gives you access to the current status of the process. In addition, you can cancel and restart the import here. Listings that were already imported are skipped when the import is restarted. Already completed imports can also be viewed in this area.
...
Enter the description, i.e. text content, and specify the listing’s layout in the *Setup » Listings » Layout* menu. You can *copy the item description* and translate it into another language, for example. It is also possible to use templates.
...
[.indented]
This function works with the data entered under *Active listings*. Before activating the automatic function, you have to delete old and unnecessary entries manually. Deleting old entries can potentially be very complex. We recommend using a search filter with duration: *endless (GTC)*, relisted: *no* and stock dependence: *limited (without reservation)*. With these settings, only the listings affected by the automatic stock update are displayed. Additionally, the only listings that are updated are those that have the option *Active* selected for listing on eBay. Remember that you have to clean up the *Active listings* section if you change the length of your offers from having specific durations to being *endless (GTC)*. After changing the duration of listings, new results are shown here.
...
[.indented]
Yes, this is possible. Update the active listing with the help of the group function *Update listings* in the *Active listings* section. If you activated the automatic stock update for endless listings, then this operation is carried out automatically.
