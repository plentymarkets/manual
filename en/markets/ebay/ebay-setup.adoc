...
Layout templates for listings use the same structure as the <<omni-channel/online-store/setting-up-clients/cms#web-design, plentymarkets web design>>. The content that you want to display for listings can be saved using template variables and functions in the *Main frame* tab in the *System » System settings » Listings » Layouts* menu. The content is structured with HTML and CSS. The different tabs are described below.
...
.Use if-logic
====
You can use the <<omni-channel/online-store/setting-up-clients/cms-syntax#basics-if, if/else logic>> for layout templates. It functions the same way that the logic works in the <<omni-channel/online-store/setting-up-clients/cms#web-design, plentymarkets web design>> section.
====
...
image::markets/ebay/assets/example_cross-selling.png[]
...
If you select this option, the following settings must be saved for the item: The item must be *active*, it must be *available* for eBay, and it must have a *sales price*. Further information can be found in the best practice <<markets/ebay/best-practices#listings, Activating listings>>.
