...
Return reasons and return statuses have to be linked if you want to process <<Processing returns, eBay returns>> in plentymarkets and ensure the automatic data exchange with eBay. Go to *System » Markets » eBay » Settings » Tab: Return settings* in order to link eBay return reasons with the reasons set up in the <<orders/managing-orders#410, System » Orders » Order types » Return>> menu in plentymarkets. Reasons for return are assigned automatically when returns are imported.
...
Certain return statuses are used when editing returns in the eBay back end. To import eBay returns into plentymarkets and edit them, link the eBay return statuses to statuses set up in the <<orders/managing-orders#1200, System » Orders » Status>> menu.
...
*_Or:_* Set up an <<//#, event procedure>> to automatically post positive feedback on eBay.
...
The language versions are assigned to the countries of delivery depending on the languages that were set for each country under <<fulfilment/preparing-the-shipment#100, System » Orders » Shipping » Settings » Tab: Countries of delivery>>.
...
|*PayPal account*
| Select a PayPal account. Only those PayPal accounts that were set up in the <<payment/payment-providers/paypal#, System » Orders » Payment » Methods » PayPal>> menu are displayed here.
...
Use this menu to define domestic and international shipping conditions. In addition to the region of delivery, select specific <<fulfilment/preparing-the-shipment#1000, shipping profiles>> or service providers for international shipping. Go to <<fulfilment/preparing-the-shipment#100, System » Orders » Shipping » Settings » Tab: Countries of delivery>> and check your language settings for the countries that you want to ship your eBay items to. For further information, refer to eBay’s link:https://sellercentre.ebay.co.uk/private/global-shipping-programme[Using the Global Shipping Programme^] page.
...
====
If you use template variables which contain properties, such as *{% GetItemPropertyById() %}* and *{% GetItemPropertyByName() %}*, you have to activate the option *Wildcard character in layout template (eBay)* for the selected property, so that the property is displayed in the layout template. This is done in the *System » Item » Properties* menu.
====
...
|*Parts compatibility listing*
| Add a parts compatibility listing. Enter the name of a list or only a part of the name and select a list. Create a parts compatibility listing in the <<Setting up parts compatibility listings, System » Markets » eBay » Parts compatibility listing>> menu.
...
You need to set up an automatic procedure in the <<//procedure-manager#, procedure manager>> in order to automatically send dunning information. You can enter the conditions under which the procedures are carried out and define a time for them.
...
For further information about automatic procedures, refer to the <<//procedure-manager#, Procedure manager>> page of the manual.
