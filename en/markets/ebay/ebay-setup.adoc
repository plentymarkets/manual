...
. Expand (icon:pencil[role="yellow"]) the *eBay shop categories* section to display an overview of your account’s shop categories. +
...
====
You can upload a return label with an event procedure at eBay as described below or you can <<fulfillment/versand-center#retrieve-return-label, create>> and retrieve a return label in plentymarkets.
====
...
[TIP]
.Generate a shipping label or upload your own label to eBay
====
...
====
Consider your setting for the option *Status of cancelled orders* in the *Setup » Orders » Settings* menu when setting up purchase cancellations. The *Status of cancelled orders* must match the statuses selected below. Otherwise, it might happen that stock is reserved again. +
...
Use the group function to change multiple orders at once. If you want to accept the request for unpaid orders, search for all unpaid orders with the status 8.6 and change the status of these orders using the <<Editing multiple listings, group function>>.
