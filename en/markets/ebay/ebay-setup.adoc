...
With more than 120 million buyers worldwide, eBay provides an opportunity to sell on a global and growing market. The eBay API interface can be fully integrated with plentymarkets. You can, among other things, start <<Starting listings, listings>> and upload <<Uploading offers with variations, variation offers>>, process your <<processing returns, returns>> and send <<Sending dunning letters, dunning letters>>. You will also learn how to <<Synchronising data, exchange data>> between eBay and plentymarkets.

[#100]
==  Registering with eBay


You need to have your eBay access data consisting of the user name and password. You will receive this data from eBay. First, you need to become an link:https://reg.ebay.co.uk/reg/PartialReg?src=oldbizreg[eBay member^] and create an eBay account.

[#200]
==  Setting up eBay in plentymarkets

In this chapter, you will find information about the settings and functions for the market eBay. This page of the manual explains how to set up eBay in plentymarkets, manage your <<Activating and changing accounts, eBay accounts>>, <<Setting up parts compatibility listings, parts compatibility listings>>, and how to automatically <<Automatically posting feedback, post feedback>>.
...
Go to *System » System settings » Markets » eBay » Accounts* to rename or delete accounts. For example, you have to rename your account in plentymarkets when changing your eBay user name.

[.instruction]
Renaming an account:

.  Go to *System » System settings » Markets » eBay » Accounts*.
.  Click on the *Edit* button next to the account you want to rename.
...
.  Go to *System » System settings » Markets » eBay » Accounts*.
.  Click on *Delete account* next to the account you want to delete. +
...
====
After changing the eBay account data, update your shop categories and return policy. Reload the *shop categories* directly in this menu. Go to <<Defining the return policy, *System » System settings » Markets » eBay » Business policies » Tab: eBay return policy*>> to determine the *return policy*.
====

[#600]
===  Carrying out the basic settings

Go to *System » System settings » Markets » eBay » Settings* to carry out the initial settings for the market eBay and to set up your accounts. Link shipping profiles to eBay platforms, synchronise eBay’s reasons for return and statuses with your plentymarkets settings and define status links for purchase cancellations.

[.instruction]
Carrying out the initial settings:

.  Go to *System » System settings » Markets » eBay » Settings » Tab: Initial settings*.
.  Carry out the settings. Pay attention to the information given in <<ebay-initial-settings>>.
. *Save* (icon:save[role="green"]) the settings.
...
This setting is defined in the <<Creating a new listing, Transfer as item number>> option of a new market listing. If the *Transfer UPC* option is activated in the <<Creating a new listing, Item » Edit item » New listing>> menu, select one of the *Use EAN 1 - 4* options here.

|*Use variation number as SKU*
| Select whether the variation number is used as the stock keeping unit. +
*Yes* = The variation number is transferred as the SKU. This setting is not recommended because after a variation has been changed and the listing has not been updated, no synchronisation will take place. +
...
*Yes* = Further item details from the eBay catalogue are automatically added. For further details, refer to the link:https://www.ebay.co.uk/pages/help/sell/product-details.html[eBay help^]. +
...
*Yes* = Further item images from the eBay catalogue are automatically added. For further details, refer to the link:https://www.ebay.co.uk/pages/help/sell/product-details.html[eBay help^]. +
...
|Activate or deactivate the <<Activating the automatic stock update, automatic stock update>> for GTC offers. +
...
| Enter the number of items that should not be available for Second Chance Offers. If the net stock is 5 and 3 was entered as the buffer, then 2 Second Chance Offers are submitted if there are that many defeated bidders.

|*Number of days after the submission of an offer in which a potential customer can complete the purchase*
| Select whether the Second Chance Offer should be ended after *1*, *3*, *5* or *7* days.
|====
...
.  Go to *System » System settings » Markets » eBay » Settings » Tab: Account settings*.
.  Carry out the settings. Pay attention to the information given in <<settings-ebay-accounts>>.
. *Save* (icon:save[role="green"]) the settings.
...
| Select whether <<Processing returns, eBay returns>> should be retrieved or not. +
...
*No* = No price synchronisation for special sales.
...
The type refers to the selected *type* in the <<crm/using-the-ticket-system, System » System settings » CRM » Ticket system>> menu. You can also enter your own parameters for eBay there.

|*Message status*
| Select the message status for eBay messages. +
The status refers to the selected *status* in the <<crm/using-the-ticket-system, System » System settings » CRM » Ticket system>> menu. You can also enter your own parameters for eBay there.
...
The amount that was entered is subtracted from the stock and not sold.

|*External eBay order number in order*
|Select which external eBay order number should be displayed in the order in plentymarkets. +
*eBay sales record number* = The old eBay order number. This number is increased per customer and for each new order at eBay. +
*eBay order number* = The new eBay order number. This order number is assigned individually for each order. eBay introduced the new order number 1 July 2019 and will distribute it stepwise. The new eBay order number replaces the old eBay sales record number. +
The *eBay sales record number* is selected by default.
...
====
Orders are automatically retrieved from eBay when the *Retrieve orders* setting is activated for an account. Additionally, you can retrieve eBay orders manually in the <<Synchronising data, System » System settings » Markets » eBay » Data exchange>> menu. Select the *date* for the retrieval an click on *Retrieve orders*. +
...
.Updating shop categories and return policy
====
After changing the eBay account data, update your shop categories and return policy. Load the *shop categories* directly in this menu. Go to <<Defining the return policy, System » System settings » Markets » eBay » Business policies » Tab: eBay return policy>> to determine the *return policy*.
====
////
...
Go to *System » System settings » Markets » eBay » Settings » Tab: Shipping profile links* to link eBay shipping profiles to the shipping profiles of your online shop. A linked shipping profile is automatically used during order import if the shipping profile matches the order data, e.g. account and market settings, of a listing and is activated in the *Global* tab of an item. If the linked shipping profile is deactivated, the active shipping profile with the highest priority is automatically used instead.

Before linking eBay shipping profiles to the shipping profiles in your online shop, you have to enter terms of payment and shipping as well as general conditions in the <<Defining business policies, System » System settings » Markets » eBay » Business policies>> menu.

[#800]
===  Linking return reasons and statuses

Return reasons and return statuses have to be linked if you want to process <<Processing returns, eBay returns>> in plentymarkets and ensure the automatic data exchange with eBay. Go to *System » System settings » Markets » eBay » Settings » Tab: Return settings* in order to link eBay return reasons with the reasons set up in the <<orders/managing-orders#410, System » System settings » Orders » Order types » Return>> menu in plentymarkets. Reasons for return are assigned automatically when returns are imported.

[.instruction]
Linking return reasons:

.  Go to *System » System settings » Markets » eBay » Settings » Tab: Return settings*.
.  Click on the *Return reason links* tab.
.  Click on the tab of the eBay platform that you want to link return reasons for, e.g. *Germany*.
.  Select a <<orders/managing-orders#410, plentymarkets return reason>> for each *eBay return reason* from the drop-down lists.
. *Save* (icon:save[role="green"]) the settings.
...
Certain return statuses are used when editing returns in the eBay back end. To import eBay returns into plentymarkets and edit them, link the eBay return statuses to statuses set up in the <<orders/managing-orders#1200, System » System settings » Orders » Status>> menu.
...
.  Select a plentymarkets <<orders/managing-orders#420, return status>> for each *eBay return status* from the drop-down lists.
. *Save* (icon:save[role="green"]) the settings.
...
====
Import reasons for return that eBay edited or re-published. To do so, go to <<Synchronising return reasons, System » System settings » Markets » eBay » Data exchange » Tab: Other>> and use the *Synchronise return reasons* function. Newly imported reasons for return can be linked as described above.
====

[#900]
===  Linking purchase cancellation statuses

Purchase cancellation statuses are used to process purchase cancellations in the eBay back end. Create new <<orders/managing-orders#1200, order statuses>> and link them with the purchase cancellation statuses from eBay to import and edit eBay purchase cancellations in plentymarkets. When a purchase cancellation request is imported, the correct status is automatically set in the order.

[.instruction]
Linking purchase cancellation statuses:

.  Go to *System » System settings » Markets » eBay » Settings » Tab: Purchase cancellation*.
.  Select a <<orders/managing-orders#1200, plentymarkets status>> for each *eBay status of purchase cancellation* from the drop-down lists.
. *Save* (icon:save[role="green"]) the settings.

For further information about processing purchase cancellations, refer to the <<Processing purchase cancellations, Processing purchase cancellations>> chapter.
...
When the *Use "out of stock" option* is set in the *System » System settings » Markets » eBay » Settings » Tab: Account settings* menu, the function of the automatic stock update changes. A listing is not ended when an item is out of stock. The item is still displayed but cannot be purchased. The number of items sold is not set to zero when the item is available again. Rather, the number remains in the sales history. Even when the *End offers automatically* setting is active in the *System » System settings » Listings » Stock* menu, listings are not ended. They cannot be purchased instead. For further information, refer to eBay’s link:https://www.ebay.co.uk/pages/help/sell/relist.html[Relisting your item^] page.

[#1300]
===  Automatically posting feedback

In the <<automation/procedure-manager#, procedure manager>>, you can set up an automatic function that posts a positive feedback for all of your customers who evaluated you positively. Retrieve the newest positive feedback at the same time every day and post positive feedback for your customers automatically. To do so, proceed as follows.

[.instruction]
Posting feedback automatically on eBay with the procedure manager:

.  Go to *System » System settings » Markets » eBay » Settings » Tab: Account settings*.
.  Enter up to four feedback texts which should be used automatically in the *Automatic feedback* section.
. *Save* (icon:save[role="green"]) the settings.
...
*_Or:_* Set up an <</automation/event-procedures#, event procedure>> to automatically post positive feedback on eBay.
...
.  Enter a name.
.  Select the *event* listed in <<automatic-feedback>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<automatic-feedback>> below.
...
Add SKUs manually for specific referrers in the menu <<item/managing-items#170, Item » Edit item » Open item » Tab: Variations » Open variation » Tab: Settings>> if needed.

[.instruction]
Adding SKU:

. Go to *Item » Edit item » Open item » Tab: Variations » Open variation » Tab: Settings*.
. Enter the SKU into the field *Variation No.*
. *Save* (icon:save[role="green"]) the settings.
...
. In the *Referrer* section, place a check mark next to the desired eBay country platform.
. In the *eBay accounts* section, place a check mark next to the desired eBay account.
. *Save* (icon:save[role="green"]) the settings.

It is possible to create more than one sales price for eBay. For example, you can create separate prices for each eBay account or each eBay platform. Read the <<markets/ebay/best-practices-ebay-faq#400, eBay FAQ>> to learn how the sales prices are prioritised if you have created more than one sales price.

[TIP]
.Currency conversion for eBay listings
====
In the *System » System settings » Orders » Payment » Currencies* menu, you can set up currency conversion for listings by selecting *Yes* for the *Automatically update conversion factors daily* option. For all currencies that you use, a conversion factor must be entered. +
...
An order is automatically created in plentymarkets after a customer has purchased an item on eBay. After an incoming order you can send an email message to the customer or to the customer’s eBay user account. If you have created an <<crm/sending-emails#1200, email template>> in the <<crm/sending-emails#2300, System » System settings » Client » Select client » Email » Automatic>> menu, an email is sent. +

*_Or:_* You can also send messages via the eBay messaging system.

The language versions are assigned to the countries of delivery depending on the languages that were set for each country under <<fulfilment/preparing-the-shipment#100, System » System settings » Orders » Shipping » Settings » Tab: Countries of delivery>>.

[.instruction]
Entering texts for messages after purchase:

.  Go to *System » System settings » Markets » eBay » Settings » Tab: Account settings*.
.  Expand the *Message after purchase* section and enter the messages for different languages.
. *Save* (icon:save[role="green"]) the settings.
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<automatic-ebay-messages>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<automatic-ebay-messages>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
With link:https://www.ebay.co.uk/pages/help/sell/fitment.html[parts compatibility listings^], you link the items that you want to sell to the correct vehicles. That way you increase your sales prospects since items of parts compatibility listings have a higher ranking in the search results. Create, edit and delete parts compatibility listings in the *System » System settings » Markets » eBay » Parts compatibility listing* menu.

[.instruction]
Creating a new parts compatibility listing:

.  Go to *System » System settings » Markets » eBay » Parts compatibility listing*.
...
. Select a *category*.
. *Save* (icon:save[role="green"]) the settings.
.  Carry out the settings. Pay attention to the information given in <<settings-parts-compatibility>>.
. *Save* (icon:save[role="green"]) the settings.

[[settings-parts-compatibility]]
...
Search for parts compatibility listings that you have created. Use the filter settings in the menu *System » System settings » Markets » eBay » Parts compatibility listing* to search for parts compatibility listings. The filter options *ID*, *Name*, *Category* and *eBay platform* are available for the search.

[#1700]
===  Setting up Second Chance Offers (SCO)

You can submit a Second Chance Offer to a defeated bidder after an auction has ended. Bidders will then receive a link:https://www.ebay.co.uk/pages/help/buy/second-chance.html[Second Chance Offer^] and have the possibility to buy the item at their last bid amount via the *Buy it now* function. The bidder then receives a Second Chance offer via email in the *Messages* section of *My eBay*. SCOs are excluded from the eBay search. +
...
====
Enter an amount for the item into the *SCO minimum price* field, e.g. 100.00 EUR. The highest bidder's bid is 110.00 EUR and wins the auction. All defeated bidders with a bid equal to or higher than the entered amount of 100.00 EUR automatically receive a Second Chance Offer to buy the item at their last bid amount. All defeated bidders with a lower bid than the entered amount do not receive a Second Chance Offer. +
 If you enter the value *0.00*, the function is deactivated. However, you can still submit Second Chance Offers manually in the *System » System settings » Markets » eBay » Second Chance Offer* menu.
====

[.instruction]
Viewing SCO errors:

.  Go to *System » System settings » Markets » eBay » Second Chance Offer*. +
...
For further information about troubleshooting, refer to the <<Second Chance Offer, FAQ: Second Chance Offer>> chapter.
...
. Go to *System » System settings » Markets » eBay » Settings » Tab: Account settings*.
. Carry out the settings. Pay attention to the information given in <<table-holiday-settings>>.
. *Save* (icon:save[role="green"]) the settings.
...
. Carry out the settings. Pay attention to the explanations in <<ebay-payment-terms>>.
. *Save* (icon:save[role="green"]) the settings.
...
|*PayPal account*
| Select a PayPal account. Only those PayPal accounts that were set up in the <<payment/payment-providers/paypal#, System » System settings » Orders » Payment » Methods » PayPal>> menu are displayed here.
...
Use this menu to define domestic and international shipping conditions. In addition to the region of delivery, select specific <<fulfilment/preparing-the-shipment#1000, shipping profiles>> or service providers for international shipping. Go to <<fulfilment/preparing-the-shipment#100, System » System settings » Orders » Shipping » Settings » Tab: Countries of delivery>> and check your language settings for the countries that you want to ship your eBay items to. For further information, refer to eBay’s link:https://sellercentre.ebay.co.uk/private/global-shipping-programme[Using the Global Shipping Programme^] page.
...
.  Carry out the settings as described in <<settings-domestic-shipping>> below.
. *Save* (icon:save[role="green"]) the settings.
...
*Express shipping* = Items are shipped the same day they were ordered, if the order was placed before a certain time. For further information, refer to eBay’s link:https://www.ebay.co.uk/pages/help/sell/fast-n-free.html[Selling with eBay FAST & FREE^] page.
...
| Select whether the rate tables should be applied. Rate tables must be set up on eBay. For further information, refer to eBay’s link:https://www.ebay.co.uk/pages/help/pay/shipping-costs.html#tables[Specifying your postage costs and locations^] page.
...
.  Go to *System » System settings » Markets » eBay » Business policies » Tab: eBay shipping conditions » Tab: New*.
.  Carry out the settings as described in <<settings-domestic-shipping>> and <<settings-international-shipping>>.
. *Save* (icon:save[role="green"]) the settings.
...
====
For further information, refer to the <<Selling internationally, Selling internationally>> chapter and eBay’s link:https://sellercentre.ebay.co.uk/private/global-shipping-programme?targetgroup=C2C[Using the Global Shipping Programme^] page.
====
...
.  Carry out the settings as described in <<ebay-return-policy>> below.
. *Save* (icon:save[role="green"]) the settings.
...
.  Go to *System » System settings » Markets » eBay » Business policies » Tab: Shipping profiles » Tab: New*.
.  Carry out the settings as described in <<settings-shipping-profiles>> below.
. *Save* (icon:save[role="green"]) the settings.
...
The payment method eBay purchase on invoice is suitable for items within the price range of 1.50 € - 1.500 €. However, it is not allowed to use this payment method for digital goods. In order to use eBay purchase on invoice the item must be paid for by PayPal. Activate the <<payment/managing-payment-methods#, payment method>> *eBay purchase on invoice* as described below.
...
payment/managing-payment-methods#30, Permitting the payment method in a customer class>> chapter.
====
...
Go to *System » System settings » Listings* to carry out the settings for all listings regardless of the market. In order to create listings in plentymarkets, you must carry out the following settings and have created your <<item/managing-items#20, items>>.
...
.  Carry out the settings as described in <<basic-settings-listings>> below.
. *Save* (icon:save[role="green"]) the settings.
...
.  Carry out the settings as described in <<options-stock>> below.
. *Save* (icon:save[role="green"]) the settings.
...
.  Carry out the settings as described in <<settings-checkout>> below.
. *Save* (icon:save[role="green"]) the settings.
...
*Tip*: You also have the possibility to manually import open orders. For further information, refer to the <<Synchronising data, Synchronising data>> chapter.
|====
...
.  Carry out the settings as described in <<settings-scheduler>> below.
. *Save* (icon:save[role="green"]) the settings.
...
Layout templates for listings use the same structure as the <<online-store/online-store/cms-syntax#web-design, plentymarkets web design>>. The content that you want to display for listings can be saved using template variables and functions in the *Main frame* tab in the *System » System settings » Listings » Layouts* menu. The content is structured with HTML and CSS. The different tabs are described below.

[IMPORTANT]
.Commissioning third-party providers
====
HTML and CSS skills are necessary to create a layout template. We recommend that you commission a service provider to create a layout template if you are not experienced in doing so. Go to *System » System settings » Listings » Layouts* to import a <<Managing layout templates, layout template>> that was created externally into plentymarkets.
====
...
.  Go to *System » System settings » Listings » Layouts*.
.  Click on *Import* in the lower left-hand corner. +
...
.  Carry out the additional settings. Pay attention to the explanations given in <<template-variables>>
. *Save* (icon:save[role="green"]) the settings. +
...
====
If you use template variables which contain properties, such as *{% GetItemPropertyById() %}* and *{% GetItemPropertyByName() %}*, you have to activate the option *Wildcard character in layout template (eBay)* for the selected property, so that the property is displayed in the layout template. This is done in the *System » System settings » Item » Properties* menu.
====
...
Examples: link:http://www.sellerfox.de/[www.sellerfox.de^], link:http://www.ayee.net/newpage/[www.ayee.net^]
...
|Condition description of the listing
...
|Link to the account's listings
...
.Use if-logic
====
You can use the <<online-store/online-store/cms-syntax#basics-if#, if/else logic>> for layout templates. It functions the same way that the logic works in the <<online-store/online-store/cms#web-design, plentymarkets web design>> section.
====
...
Create any number of directories or subdirectories for organising your listings in the *New directory* tab of the *System » System settings » Listings » Directories* menu.

[.instruction]
Creating a directory:

.  Go to *System » System settings » Listings » Directories*.
...
*_Or:_* Open an item and click on *New listing* in the tool bar.
...
.  Carry out the settings and pay attention to the explanations given in <<settings-new-market-listing>>.
. *Save* (icon:save[role="green"]) the settings. +
→ The listing is created and the listing overview opens.

[[settings-new-market-listing]]
.Settings in the window *New market listing*
[cols="1,3"]
|====
| Setting | Explanation
...
|*Stock dependence*
|Select the <<stock-management/managing-stocks#.%20stock%20dependence, stock>> of the listing. Select one of the following options: +
...
*do not transfer* = No item number is transferred. +
...
.  Go to *Item » Edit item*.
.  Place a check mark next to the items that you want to create listings for.
...
|*Special sale*
| Special sales set up for the listing are displayed here. Create a new link:https://www.ebay.co.uk/help/selling/selling-tools/selling-tools-overview?id=4073[special sale^] directly on eBay. Special sales will be available in plentymarkets immediately after they have been created and are automatically synchronised once an hour.
|====
...
.  Carry out the settings and pay attention to the explanations given in <<editing-market-listing>> , <<base-tab-market-listing>> and <<layout-market-listing>>.
. *Save* (icon:save[role="green"]) the settings.

image::markets/assets/toolbar-market-listing.png[]

__Image 1: A market listing's tool bar__
...
| Select whether the unit price should be displayed and where it should be displayed. For listings of the *Fixed price* type or *Buy it now* listings with *Weight* or *Volume* options, the link:https://www.ebay.co.uk/help/selling/selling/pricing-items?id=4133[unit price^] must be entered. Values for *units* which are saved for an item are automatically transferred to eBay.
...
|*Condition*
| Select the item condition. The options become visible once the category has been selected. Item conditions and descriptions are provided by eBay. For further information, refer to eBay’s link:https://www.ebay.co.uk/pages/help/sell/item-condition.html[Item conditions by category^] page.
...
|*Best offer*
| Activate to allow best offers for listings of the *Fixed price* type. Best offers are not available in all categories. Best offers are unavailable for offers with variations. For further information, refer to eBay’s link:https://www.ebay.co.uk/pages/help/sell/best-offer.html[Selling with Best Offer^] page.
...
|*Gallery*
| Selecting the gallery options. For further information, refer to eBay’s link:https://www.ebay.de/help/selling/listings/angeboten-bilder-hinzufgen?id=4148[About gallery upgrades^] page.
...
For further information, refer to eBay’s link:https://www.ebay.co.uk/pages/help/sell/promoting_ov.html[Promoting your item with listing upgrades^] page.
...
*Basic settings* = The option selected for the *Automatically activate eBay Plus for new listings* setting in the *System » System settings » Markets » eBay » Settings » Tab: Account settings* menu is used. *Basic settings* is selected by default. +
...
.  Carry out the settings as described in <<group-function-batch-processing>> below.
...
|*Stock dependence*
|Select the <<stock-management/managing-stocks#.%20stock%20dependence, stock>> of the listing.
...
|*Parts compatibility listing*
| Add a parts compatibility listing. Enter the name of a list or only a part of the name and select a list. Create a parts compatibility listing in the <<Setting up parts compatibility listings, System » System settings » Markets » eBay » Parts compatibility listing>> menu.
...
|*Tags*
|Select one or multiple tags to only display listings with those tags.
...
In plentymarkets, there are multiple ways to automatically start listings or restart listings once they have already been started. Use the listing scheduler to specify *starting times* for listings. Go to <<Setting up the listing scheduler, System » System settings » Listings » Scheduler>> to set up the listing scheduler.
...
.  Click on *Group function*. +
→ The *Group function* window opens.
.  Carry out the settings as desired. Pay attention to the information given in <<group-function-batch-processing>> in the <<Editing listings, Editing listings>> chapter.
...
.  Go to *Listings » Batch processing » Listings*.
.  Carry out the settings as desired. Pay attention to the explanations given in <<group-function-batch-processing>> in the <<Editing listings, Editing listings>> chapter.
.  Click on *Save new template*. +
→ The *Name of template* window opens.
.  Enter a name for the template.
. *Save* (icon:save[role="green"]) the settings. +
...
.  Click on *Group function*. +
→ The *Group function* window opens.
.  Select a procedure according to <<procedures-group-function>>.
...
*Information:* Only updates which contain more than 10 listings are displayed in this window. If you update fewer than 10 listings, then the listings will be updated synchronously.
====

[#5900]
===  Editing active listings with batch processing

Batch processing includes the same options as the group function. However, the batch processing function edits previously filtered listings. The filters used for narrowing down listings are the same as described for search filters in <<filter-options-batch-processing>> and <<filter-options-listings>>. The procedures are the same as described for group functions in <<procedures-group-function>>.

[.instruction]
Editing active listings with batch processing:

. Go to *Listings » Batch processing » Active listings*.
. Select the filters based on <<filter-options-listings>> and <<filter-options-active-listings>>.
. Carry out the settings and pay attention to the explanations given in <<procedures-group-function>>.
...
.  Select *eBay* from the *Market* drop-down list.
.  Select an *eBay platform*.
.  Select a category from the *eBay category* drop-down list. +
...
Link variations with ePIDs in the menu *System » System settings » Markets » eBay » ePID matching*.
...
- In the menu *System » System settings » Markets » eBay » Settings » Tab: Initial settings*, the option *Add item details from the eBay catalogue* has to be set to *Yes*.
...
Go to *System » System settings » Markets » eBay » ePID matching* to filter variations without ePID matches. For example, you can search for MLIDs or item IDs, or you can display only variations which are sorted into a specific directory. For further information about filter options for variations without ePID matching, refer to <<filter-epid>>.
...
Use the <<Carrying out the basic settings, System » System settings » Markets » eBay » Settings » Account settings>> menu to decide whether the loyalty programme should be activated for all new listings. Listings can also be activated or deactivated for eBay Plus later on.
...
.  Go to *System » System settings » Markets » eBay » Settings » Account settings*.
.  Select *Yes* for the *Automatically activate eBay Plus for new listings* setting.
.  Carry out additional settings as desired. Pay attention to the explanations given in <<settings-ebay-accounts>>.
. *Save* (icon:save[role="green"]) the settings.
...
The option *Basic settings* in the *Market* tab is set by default for eBay Plus in the *Listings » Edit listings* menu. The setting selected for the option *Automatically activate eBay Plus for new listings* in the *System » System settings » Markets » eBay » Settings » Account settings* menu is transferred.

[NOTE]
.Deactivating eBay Plus for individual listings:
====
Even if you selected *Yes* for the option *Automatically activate eBay Plus for new listings* in the *System » System settings » Markets » eBay » Settings » Account settings* menu, it is still possible to deactivate individual listings for eBay Plus. To do so, select *No* for the option *eBay Plus* in the market listing’s *Market* tab.
====
...
.  Go to *Listings » Active listings*.
.  Place a check mark next to the listings that you want to update.
.  Click on *Group function*. +
→ The *Group function* window opens.
...
Activate Click & Collect by placing a check mark next the option *Buyers can collect the item at Argos with Click & Collect* in the menu <<Defining shipping conditions, System » System settings » Markets » eBay » Business policies » Tab: eBay shipping conditions>>. Next, you have to link the shipping conditions with the shipping profile from eBay that is saved for the listing.

[.instruction]
Activating Click & Collect for listings:

.  Go to *System » System settings » Markets » eBay » Business policies » Tab: eBay shipping conditions*.
...
.  Click on *Group function*. +
→ The *Group function* window opens.
.  Select the shipping profile that is linked with Click & Collect for the option *Shipping information* in the *Update eBay options* section.
.  Click on *Execute* to update the listings.
...
eBay returns and return statuses of the last 14 days are automatically imported from eBay to plentymarkets and updated every hour. When importing, new returns are created with the *Standard status for return* in the *Orders » Edit orders* menu. The reason for return chosen by the customer on eBay is assigned to a <<orders/managing-orders#410, reason for return>> in plentymarkets. Imported eBay returns are edited the same way as other <<orders/managing-orders#400, returns>> in plentymarkets.
...
Set up <<automation/event-procedures#, event procedures>> to automatically exchange information with eBay. With event procedures, you initiate refunds from PayPal payments, mark returns as received and inform eBay that you have paid the refund. Event procedures are triggered when you change the status of a return in the menu *Orders » Edit orders » Open return*.
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<accepting-returns>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<accepting-returns>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
[IMPORTANT]
.Returns with own label are only available for ebay.de, ebay.co.uk, and ebay.com
====
Returns with own label are only available for your customers on ebay.de, ebay.co.uk, and ebay.com.
====
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<return-own-label>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<return-own-label>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
====
Only reject those returns that violate the terms specified in the menu <<Defining the return policy, System » System settings » Markets » eBay » Business policies » Tab: eBay return policy>>.
====
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<rejecting-returns>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<rejecting-returns>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<partial-refund>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<partial-refund>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<event-procedure-own-label>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is saved.
.  Carry out further settings as described in <<event-procedure-own-label>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<event-procedure-marking-returns>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<event-procedure-marking-returns>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--

[[event-procedure-marking-returns]]
...
Set up an <<automation/event-procedures#, event procedure>> to initiate a refund via PayPal to the customer.
...
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<event-procedure-automatic-return>>.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<event-procedure-sent-returns>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<event-procedure-sent-returns>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--

[[event-procedure-sent-returns]]
...
.  The return request is imported into plentymarkets and saved with the standard status for returns.
.  The seller checks the reason for return under *Orders » Edit orders » Open return » Tab: Stock units » Note*.
.  The seller offers a new price to the purchaser. The seller enters a suggestion for the new item price under *Orders » Edit orders » Open return » Tab: Edit* next to the option *Price [gross]*. *_Or:_* Alternatively, the seller enters the new price into the *Credit note*.
...
Use the group function to change several orders at once. If you want to accept the request for unpaid orders, search for all unpaid orders with the status 8.5 and change the status of these orders using the <<orders/managing-orders#1700, group function>>.
...
.  Go to *System » System settings » Markets » eBay » Data exchange*.
...
Set up two <<automation/event-procedures#, event procedures>> to automatically reject purchase cancellation requests for paid orders.
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<marking-paid-orders>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<marking-paid-orders>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<automatically-rejecting-purchase-cancellations>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<automatically-rejecting-purchase-cancellations>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--

[[automatically-rejecting-purchase-cancellations]]
...
|*Event*
|*Order change: Status change*
|*eBay: Purchase cancellation requested*
...
[#9800]
====  Communicating a purchase cancellation acceptance

Set up an <<automation/event-procedures#, event procedure>> to automatically inform eBay about accepting a purchase cancellation.
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<message-accepted-purchase-cancellation>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<message-accepted-purchase-cancellation>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--

[[message-accepted-purchase-cancellation]]
...
[#9900]
====  Communicating a purchase cancellation rejection

Set up an <<automation/event-procedures#, event procedure>> to automatically inform eBay about rejecting a purchase cancellation.
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<message-rejected-purchase-cancellation>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<message-rejected-purchase-cancellation>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--

[[message-rejected-purchase-cancellation]]
...
[#9950]
=== Automatically creating a purchase cancellation on eBay
...
Set up an <<automation/event-procedures#, event procedure>> to automatically start a purchase cancellation on eBay. An example of an event procedure for cancelling a purchase because the buyer gave an incorrect delivery address is described below. Set up further event procedures for a purchase cancellation on eBay in the same way.
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<starting-purchase-cancellations-ebay>> below.
. *Save* (icon:save[role="green"]) the settings. +
→ The event is created.
.  Carry out further settings as described in <<starting-purchase-cancellations-ebay>> below.
.  Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings. +
→ The event procedure is saved.
--
...
.Missing API format
====
It is possible that some API formats are missing in the drop-down list. These formats will only be available after activating functions in the settings. The format *ItemListingHelperSyncInventoryJob*, for example, will only be available in the drop-down list once automatic stock update is activated in the *System » System settings » Markets » eBay » Settings » Tab: Initial settings* menu.
====
...
In the *Orders* tab, you manually retrieve orders. Retrieving orders manually might be necessary if you do not use the automatic retrieval under *System » System settings » Markets » eBay » Settings* or if your token has expired and you now want to import the orders from this time period.
...
Outstanding orders are those orders for which the buyer has not paid yet and thus not yet completed the checkout process on eBay. These orders cannot be found in the <<orders/managing-orders#1300, Orders>> menu. You can import outstanding orders so that they become available in the *Orders* menu.

[IMPORTANT]
.Outstanding orders
====
Outstanding orders only occur if the option *Shipping costs for new orders coming from eBay* was set to *Transfer from eBay*. This option is found under *System » System settings » Listings » Checkout*. It is only possible to import outstanding orders within 30 days.
====
...
.  Place a check mark next to the *orders* that you want to import. +
...
====
Go to *System » System settings » Markets » eBay » Data exchange » Tab: Other*. Click on *Start* in the *Synchronise special sales* section to start the synchronisation.
====
...
====
Go to *System » System settings » Markets » eBay » Business policies » Tab: eBay shipping conditions* to select the *shipping discounts for combined payments* that were imported from eBay.
====
...
.  Carry out the settings as described in <<ebay-importing-listings>> below.
...
//*eBay Turbo Lister* is a listing tool that is especially intended for sellers who upload thousands of items in bulk. The *eBay turbo lister* is a desktop-based offline tool. (Source: link:https://pages.ebay.co.uk/turbo_lister/index.html[eBay^]).
...
//.  Carry out the settings. Pay attention to the information given in the table below.
...
.  Select the *shipping method* in the *international shipping options* section.
...
In the <<Managing layout templates, System » System settings » Listings » Layouts>> menu, enter the description, i.e. text content, and specify the listing’s layout. You can *copy the item description* and translate it into another language, for example. It is also possible to use templates.
...
.  Go to *System » System settings » Markets » eBay » Business policies » Tab: eBay shipping conditions*.
.  Select the *shipping method* in the *domestic shipping options* section.
...
Use ElasticSync in the *Data » ElasticSync* menu to exchange data between plentymarkets and eBay. Listing data can be imported into plentymarkets with the <<data/importing-data/sync-types/elasticsync-market-listing#, Market listing>> sync type. Use the <<data/importing-data/sync-types/elasticsync-parts-compability-listing#, Parts compatibility listing>> sync type to import or update parts compatibility listings.
...
[.indented]
No, stock is not reserved for eBay second chance offers. You can define a *minimum net stock* that is kept, but no reservation is made. We recommend that you use the function *End offers automatically*. Go to *System » System settings » Listings » Stock* in order to activate this function. Enabling this function means that every 15 minutes, the system checks the stock of active auctions with no stock reservation, as well as the stock of Second Chance Offers. As soon as the stock of the item reaches *0*, the offer is terminated.

===== Do I have to pay insertion fees to submit SCOs?

[.indented]
There are no insertion fees for SCOs. You may only need to pay the final value fee in case of a successful sale. → For further information about the current prices, refer to the link:https://www.ebay.co.uk/pages/help/sell/second_chance_offer.html[help information provided by eBay^].

===== Overselling has become a problem since I started using SCO. How can that be?

[.indented]
Check whether the *End offers automatically* function is activated in the *System » System settings » Listings » Stock* menu in your plentymarkets system. Auctions with no stock reservation are only checked every 15 minutes, and SCOs make up a part of these auctions too. If more items than you still have in stock are sold in between two checks, then overselling will result.
...
[.indented]
The value entered into the *Second chance offer for minimum bids* field is converted into the currency used for the listing before an SCO is submitted to a defeated bidder. If no conversion factor for this currency was entered in the *System » System settings » Orders » Payment » Currencies* menu, this error message is displayed. Submit an SCO manually afterwards using the *Submit SCO* function. All offers listed in the *System » System settings » Markets » eBay » Second Chance Offer* menu are displayed with the currency of the auction.
...
Detail 0: You have reached your sales limit and may currently not list any further items. You can list up to 95 items per month. If one of your offers ends and the item has not been sold, you can list other items. For further information about selling limits and how to raise the limit, refer to the link:https://www.ebay.co.uk/pages/help/sell/sellinglimits.html[eBay help^] (Listing and selling allowances for eBay sellers). +
...
Information about opening your shop is provided link:https://sellercentre.ebay.co.uk/business/personalise-shop?cat=36[here^].
...
[.indented]
Originally, the offer was started without variations. In the meantime, the settings were carried out to include variations in the listing. Updating and restarting is not possible under these circumstances. You have to create and publish a completely new offer.
