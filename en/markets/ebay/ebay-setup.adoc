...
Variation availabilities are organised in the *Setup » Item » Availability* menu and set in the *availability* area of the *Item » Edit item » [Open item] » [Open variation] » Tab: Settings* menu of each item.
|====
...
* <<#900-1, Set order statuses.>>
* <<#900-2, Check that the status range for cancelled orders and adjust the range if necessary.>>
* <<#900-3, Link the purchase cancellation status with the eBay status.>>
...
Reserved stock needs to be released when a purchase is cancelled. As such, check the status range for cancelled orders set in your system. If necessary, adjust the range.
...
The order status now need to be linked to eBay’s purchase cancellation statuses. When a purchase cancellation request is imported, the correct status is then automatically set in the order.
...
. From the drop-down list *New order from eBay*, select the email template. Note <<#table-events-automatic-email-dispatch>>.
...
* *Status 3* = The customer receives a summary of the order and does not have to be active. However, the customers can change some data, for example their addresses. +
...
- An EAN must be saved for the variation within *Item » Edit item » [Open item] » [Open variation]* in the *Barcode* area.
...
TIP: The plugin <<markets/ebay/ebay-plugins/ebay-OAuth2#, eBay OAuth2>> must be installed and set up in your system for this event procedure to function.
...
eBay has two types of purchase cancellations:

* <<#8900-1, purchase cancellations by customers>>
* <<#9950, purchase cancellations by the seller>>
...
[#9000]
==== Purchase cancellation: Importing requests
...
[#9700]
==== Creating event procedures to automatically inform eBay of your decisions

Create the following event procedures to automatically inform eBay about your decisions regarding purchase cancellation requests. These event procedures are triggered when the status of an order is changed for which a purchase cancellation was requested.

[#9800]
===== Communicating a purchase cancellation acceptance

Set up an <<automation/event-procedures#, event procedure>>. This event procedure informs eBay automatically that you have accepted a purchase cancellation when an order is moved to the selected status.
...
Set up an <<automation/event-procedures#, event procedure>>. This event procedure informs eBay automatically that you have rejected a purchase cancellation when an order is moved to the selected status.
...
[#9100]
[#9300]
==== Purchase cancellation: Accepting requests
...
[.tabs]
====
Accepting requests: Changing the order status::
+
--
...
Accepting purchase cancellations: Deleting outstanding orders::
+
--
...
[#9400]
==== Purchase cancellation: Rejecting requests
...
[.tabs]
====
Rejecting requests: Changing the order status::
+
--
If you have linked the eBay purchase cancellation statuses with your own statuses yy#900, as described above>>, then purchase cancellation requests are imported with the status that you set for “eBay purchase cancellation requested”. By changing the order status, you can process each request individually.
...
Rejecting purchase cancellations for paid orders: Event procedures::
+
--
...
[#9950]
=== Automating purchase cancellations by the seller

As a seller, you can start purchase cancellations on eBay.

Basic process:

* <<9950-1, Set order statuses for the permitted reasons.>>
* <<9950-2, Create an event procedure for automatically starting purchase cancellations on eBay>>
* <<9950-3, To start a purchase cancellation, you just need to change the status of the order.>>
...
[#9950-2]
[discrete]
===== Creating an event procedure for automatically starting purchase cancellations
