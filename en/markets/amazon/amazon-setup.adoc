...
*Store item* = If you select the filter *Only items visible in online store*, the items which should be exported have to be activated for the shop which was selected for the option *Client* in the *Setup » Markets » Amazon » Settings » Tab: Settings* menu for the selected Amazon account. If you select the filter *All items*, then all items are exported, independent of the selected shop.
...
. Go to <<item/managing-items#480, Setup » Item » Properties » Tab: New property>>.
...
Link variation properties that you created in the <<item/managing-items#3100, Setup » Settings » Properties » Configuration>> and added to a variation with Amazon in the *Setup » Settings » Properties » Configuration » Tab: Amazon* menu.
...
In the *Setup » Markets » Amazon » Data exchange » Reports* menu, you can define interval for Amazon order reports and reset Amazon order reports from a specific date.
...
|*Event*
|Select an event +
*_Important:_* Do not select the event *Order generation: New credit note*. This may lead to credit notes being created at Amazon multiple times.
|
...
====
If you use Amazon VCS, you must not use <<Uploading our own invoices to Amazon, Amazon IDU>> because in this case, invoice documents would be uploaded to Amazon twice. This can result in missing invoices in the Seller Central due to throttling.
====
...
In case of a return, the purchaser sends the items back to Amazon. The return is imported into your plentymarkets system and listed in the order menu. It is given the return status that you selected in the <<orders/managing-orders#420, Setup » Orders » Settings>> menu.
