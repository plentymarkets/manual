...
In order to link items, characteristics etc. with Amazon platforms, you have to activate the order referrer for those Amazon platforms in the *Setup » Orders » Order referrer* menu.
...
Use <<item/settings/properties#500, properties>> to define individual characteristics of a variation. For example you can use properties to highlight a variation's technical features, such as Bluetooth or WLAN.

Link properties that you have created in the *Setup » Settings » Properties » Configuration* and added to a variation with Amazon in the *Setup » Settings » Properties » Configuration » _Open property_ » Tab: Amazon* menu.
...
→ The characteristic's editing section opens.
...
. Activate the characteristic by placing a check mark next to it.
. *Save* (icon:save[role="green"]) the settings. +
...
icon:map-signs[] *_Or:_* You can also display up to 5 more sales prices in percent for quantity based discounts, using characteristic links. To do so, select the Amazon link *quantity_price_type* when creating the characteristic of the type *Text* or *Selection* and enter the value *percent* when linking the characteristic to the item. Additionally, you have to create characteristics for *quantity_lower_bound1-5* and for the discount quantity and *quantity_price1-5* for the percentage.
...
Next, activate the <<item/managing-items#480, characteristic>> in the item settings and enter the value in *percent*.
