...
Next, create the properties *SaleStartDay* and *SaleEndDate* and link them with the Amazon product type. Proceed as described below to create and link the property *SaleStartDate*. Create the property *SaleEndDate* in the same way but enter the name *SaleEndDate* and link the Amazon field *sale_end_date* instead.
...
|*Tag IDs*
|Enter one or more tag IDs. Separate multiple IDs using commas. Only items with the selected filters are exported.
...
In case of a return, the purchaser sends the items back to Amazon. The return is imported into your plentymarkets system and listed in the order menu. It is given the return status that you selected in the *Setups » Orders » Settings* menu.
