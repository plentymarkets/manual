...
.  Go to *System » System settings » Markets » Amazon » Settings*.
...
If you use Amazon VCS, upload your invoices to Amazon with an event procedure.
*VCS plentymarkets* = *Condition:* Amazon VCS is used. Amazon invoices are automatically generated by plentymarkets and transferred to Amazon. +
...
*Store items* = If you select the filter *Only items visible in online store*, the items which should be exported have to be activated for the shop which was selected for the option *Client* in the *System » System settings » Markets » Amazon » Settings » Tab: Settings* menu for the selected Amazon account. If you select the filter *All items*, then all items are exported, independent of the selected shop.
...
In order to link items, properties etc. with Amazon platforms, you have to activate the order referrer for these Amazon platforms in the menu *System » System settings » Orders » Order referrer*.
...
|*Amazon FBA*
| Activate if the FBA function should be used and FBA stock should be imported. +
...
Additional SKUs are automatically included in the order import. Activate the stock update and price update for additional SKUs manually in the *System » System settings » Markets » Amazon » Settings » Tab: Item settings* menu. +

The stock update and price update for additional SKUs is carried out along with the normal stock update and price update at the interval that you have selected within *System » System settings » Markets » Amazon » Settings » Tab: Item settings* for the options *Price changes* and *Stock synchronisation* in the section *Automatic processes*.

[.instruction]
Activating the stock and price update:

. Go to *System » System settings » Markets » Amazon » Settings » Tab: Item settings*.
...
The *System » System settings » Markets » Amazon » ASIN matching* menu contains an overview of imported ASINs. All imported ASINs are displayed here, including ASINs are linked with a variation and ASINs which have not yet been linked. You can also link ASINs individually or with the group function in this menu.
...
If this GTIN is saved for more than one variation, then the item of the first variation that was found is opened.
...
To link ASINs with variations, open the *System » System settings » Markets » Amazon » ASIN matching* menu.

[.instruction]
Linking an ASIN with a variation:

. Go to *System » System settings » Markets » Amazon » ASIN matching*.
...
Use the group function in the *System » System settings » Markets » Amazon » ASIN matching* menu in order to link multiple ASINs at once.
...
.Actions for the group function
[cols="1,3"]
|====
| Setting | Explanation
...
. Go to <<item/managing-items#3100, System » System settings » Item » Properties » Tab: New property>>.
...
.  Go to *System » System settings » Markets » Amazon » Freely definable fields*.
...
Go to *System » System settings » Markets » Amazon » Category link* to link categories. In plentymarkets, you can link each item category to up to two Amazon classification numbers from the Browse Tree Guides.
...
.  Go to *System » System settings » Markets » Amazon » Category link* in your plentymarkets back end.
...
Link variation properties that you have created in the menu <<item/managing-items#3100, System » System settings » Settings » Properties » Configuration>> and added to a variation with Amazon in the menu *System » System settings » Settings » Properties » Configuration » Tab: Amazon*.
...
The link between an item’s property and the Amazon product type is made in the <<item/managing-items#480, System » System settings » Item » Properties>> menu. In plentymarkets, you can save a maximum of 16 links for one property.
...
.  Go to *System » System settings » Markets » Amazon » Data exchange » Data export*.
...
Go to *System » System settings » Markets » Amazon » Data exchange » Order import* to set up the import of Amazon orders. Orders can be imported automatically via the XML interface or they can be imported manually with a .txt file. Both possibilities are described below.
...
.  Go to *System » System settings » Markets » Amazon » Data exchange » Order import*.
...
.  Go to *System » System settings » Markets » Amazon » Data exchange » Shipping confirmation*.
...
In the menu *System » System settings » Markets » Amazon » Data exchange » Reports*, you can define interval for Amazon order reports and reset Amazon order reports from a specific date.

[#2350]
====  Setting intervals for order reports

Go to *System » System settings » Markets » Amazon » Data exchange » Reports » Tab: Scheduled reports* to define the interval in which order reports should be created by Amazon. Orders from these reports are imported during the automatic order import. Therefore, we recommend to use a low interval both for the generation of reports as well as for the order import. +
...
If you have activated Amazon FBA in the *System » System settings » Markets » Amazon » Settings » Tab: Order settings* menu, FBA order reports are automatically retrieved every 4 hours. In the section *FBA orders*, you can optionally define an interval for the retrieval of order reports.

[.instruction]
Defining an interval:

.  Go to *System » System settings » Markets » Amazon » Data exchange » Reports » Tab: Scheduled reports*.
.  Select an Amazon account from the drop-down list.
...
In the menu *System » System settings » Markets » Amazon » Data exchange » Reports » Tab: MFN reports*, you can reset the Amazon order reports from a specific date.
...
.  Go to *System » System settings » Markets » Amazon » Data exchange » Reports » Tab: MFN reports*.
.  Select an Amazon account from the drop-down list.
...
Stock is synchronised with Amazon in the menu *System » System settings » Markets » Amazon » Data exchange » FBA stock*. There are a few different ways to do this. You can use the hourly automatic stock synchronisation, an automatic procedure or a CSV file.
...
.  Go to *System » System settings » Markets » Amazon » Data exchange » FBA stock*.
...
First, go to <<orders/managing-orders#1200, System » System settings » Orders » Status>>. Create an order status for Amazon returns, e.g. [9.6] Return sent to Amazon. You need this order status for the Amazon settings.
...
. *Save* the settings. +
→ The event is created.
.  Carry out further settings as described in <<event-procedure-update-order-status>>.
.  Place a check mark next to the option *Active*.
. *Save* the settings. +
→ The event procedure is saved.
--
...
Amazon VCS is the VAT calculation service offered by Amazon. With this service, Amazon calculates the sales tax. You decide whether Amazon invoices should be generated by Amazon or by plentymarkets. If you have plentymarkets create Amazon invoices, then the invoices are automatically transferred to Amazon. In addition, you can upload your own invoice documents to Amazon without having Amazon calculate the sales tax. If you select this option, you must set up an event procedure to upload your invoice documents to Amazon. +

Activate Amazon VCS in plentymarkets in the *Initial settings* section of the *System » System settings » Markets » Amazon » Select an Amazon account » Tab: Settings* menu.
...
====
Also read the <<markets/amazon/best-practices#vcs, best practice for Amazon VCS>> which contains further information about the setup of Amazon's invoice service VCS in plentymarkets.
====

[.instruction]
Activating the invoice service Amazon VCS in plentymarkets:

1. Go to *System » System settings » Markets » Amazon » Select an Amazon account » Tab: Settings*. +
...
*Important:* Select this option if you do _not_ use Amazon VCS _or_ if use Amazon VCS and would like to upload your own invoice documents to Amazon (Amazon Invoice Document Uplaoder - IDU) without having Amazon calculate the sales tax. Create an event procedure to upload your invoices automatically to Amazon.
...
|*VCS Amazon*
|*Condition:* Amazon VCS is used. +
Amazon invoices are generated by Amazon. +
*Important:* If you activate this option, then Amazon invoices *cannot* be created manually or with an event procedure any longer.
|====

[.subhead]
Uploading your own invoices to Amazon without sales tax calculation

You can upload the invoice documents that were created in plentymarkets to Amazon without having Amazon calculate the sales tax. This can prevent differing sales tax calculations from plentymarkets and Amazon. To upload your invoices to Amazon without sales tax calculation, create an event procedure in the *System » System settings » Orders » Events* menu.
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in <<event-procedure-uploading-invoices>>.
. *Save* the settings. +
→ The event is created.
.  Carry out further settings as described in <<event-procedure-uploading-invoices>>.
.  Place a check mark next to the option *Active*.
. *Save* the settings. +
→ The event procedure is saved.
--

[[event-procedure-uploading-invoices]]
.Event procedure for sending automatic returns to Amazon
[cols="1,3,3"]
|====
| Setting | Option | Selection
...
Go to the menu *System » System settings » Markets » Amazon » Settings* to activate the Amazon account for Amazon Business.

[.instruction]
Activating an Amazon account for Amazon Business:

.  Go to *System » System settings » Markets » Amazon » Settings » Open account » Tab: Settings*.
...
First, create a standard price for B2B items. To do so, select the option *Minimum quantity*, the *Referrer* and the *Amazon account* in the *System » System settings » Item » Sales prices* menu.
...
[.indented]
* Activate the *sales price*: Make sure that the item’s sales price is activated for the Amazon account and the Amazon referrer. This is done in the *System » System settings » Item » Sales prices* menu.
...
* Export filter *Store items*: If you have selected *Only items visible in online store* for the export filter *Store items* in the *System » System settings » Markets » Amazon » Settings » Tab: Item settings* menu, then make sure that the item is activated for the web shop. This is done in the item’s *Settings* tab.
