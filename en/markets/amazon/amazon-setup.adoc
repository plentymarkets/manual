...
*Store item* = If you select the filter *Only items visible in online store*, the items which should be exported have to be activated for the shop which was selected for the option *Client* in the *System » System settings » Markets » Amazon » Settings » Tab: Settings* menu for the selected Amazon account. If you select the filter *All items*, then all items are exported, independent of the selected shop.
...
|Select one of the options *Do not transfer*, *Transmit "average delivery time in days"* and *Transmit "average delivery time in days" + additional delay in delivery*.
...
In order to link items, properties etc. with Amazon platforms, you have to activate the order referrer for those Amazon platforms in the *System » System settings » Orders » Order referrer* menu.
...
Items have to be available for Amazon. This is done in the *Availability* tab of an item within the <<item/managing-items#180, Item » Edit item » Open item » Tab: Variation ID>> menu.
...
. Select *Web API*.
. Click on *Add* (icon:plus-square[role="darkGrey"]).
. In the same list, select the Amazon country platform(s) on which the item should be available.
. Click on *Add* (icon:plus-square[role="darkGrey"]). +
→ The market is added.
. *Save* (icon:save[role="green"]) the settings.
. Open the *Multi-Channel* tab.
. Carry out the settings in the *Amazon* area. Pay attention to the information given in <<item-availability-amazon>>.
. *Save* (icon:save[role="green"]) the settings.
...
. Open the *Item » Edit item » Open item » Tab: Variation ID » Tab: Settings* menu.
. Click on the *Availability* tab.
. Click on *Add* (icon:plus-square[role="darkGrey"]) in the *SKU* section. +
...
. Open the *Item » Edit item » Open item » Tab: Variation ID » Tab: Settings* menu.
. Click on the *Availability* tab.
. Click on *Add* (icon:plus-square[role="darkGrey"]) in the *Additional SKU* section. +
→ The window *New SKU* is displayed.
. Select the referrer *Amazon* to add the SKU.
. Select the corresponding marketplace account.
. Enter the SKU.
...
Additional SKUs are automatically included in the Amazon order import. Activate the stock update and price update for additional SKUs manually in the *System » System settings » Markets » Amazon » Settings » Tab: Item settings* menu. +
...
Next, create the properties *SaleStartDay* and *SaleEndDate* and link them with the Amazon product type. Proceed as described below to create and link the property *SaleStartDay*. Create the property *SaleEndDate* in the same way but enter the name *SaleEndDate* and link the Amazon field *sale_end_date* instead.
...
. Enter the maximum number of *characters* for the free text field.
. *Save* (icon:save[role="green"]) the settings. +
→ The free text fields are available the next day in the editing window of the items in the <<item/managing-items#70, Fields>> tab. Then enter the *number*. Pay attention to the explanations given in <<options-item-condition>> and <<options-will-ship-internationally>>.
...
Go to *System » System settings » Markets » Amazon » Category link* to link categories. In plentymarkets, you can link each item category with up to two Amazon classification numbers from the Browse Tree Guides.
...
Link variation properties that you created in the <<item/managing-items#3100, System » System settings » Settings » Properties » Configuration>> and added to a variation with Amazon in the *System » System settings » Settings » Properties » Configuration » Tab: Amazon* menu.
...
In the *System » System settings » Markets » Amazon » Data exchange » Reports* menu, you can define interval for Amazon order reports and reset Amazon order reports from a specific date.
...
If you activated Amazon FBA in the *System » System settings » Markets » Amazon » Settings » Tab: Order settings* menu, FBA order reports are automatically retrieved every 4 hours. In the *FBA orders* section, you can optionally define an interval for the retrieval of order reports.
...
In the *System » System settings » Markets » Amazon » Data exchange » Reports » Tab: MFN reports* menu, you can reset the Amazon order reports from a specific date.
...
Stock is synchronised with Amazon in the *System » System settings » Markets » Amazon » Data exchange » FBA stock* menu. There are a few different ways to do this. You can use the hourly automatic stock synchronisation, an automatic procedure or a CSV file.
...
. Click on *Add event procedure* (icon:plus-square[role="darkGrey"]). +
...
Activate Amazon VCS in plentymarkets in the *Initial settings* section of the *System » Markets » Amazon » Select an Amazon account » Tab: Settings* menu.
...
. Go to *System » System settings » Markets » Amazon » Select Amazon account » Tab: Settings*. +
...
You can upload the invoice documents that were created in plentymarkets to Amazon (Amazon Invoice Document Uploader - IDU) if you do not use Amazon VCS.  To upload your invoices to Amazon, create an event procedure in the *System » System settings » Orders » Events* menu.
...
. *Save* (icon:save[role="green"]) the settings to create a new warehouse. +
...
In case of a return, the purchaser sends the items back to Amazon. The return is imported into your plentymarkets system and listed in the order menu. It is given the return status that you selected in the <<orders/managing-orders#420, System » System settings » Orders » Settings>> menu.
...
====
In order to handle Amazon Prime orders, certain information has to be saved in the *System » System settings » Orders » Shipping » Shipping service provider » Settings* menu. Amazon requires the following mandatory fields: *Company*, *Street*, *House number,* *Postcode*, *Town*, *Country*, *Telephone*, and *Email*. These mandatory field have to be filled in order to handle Amazon Prime orders.
====

[#4300]
=== Filtering Amazon Prime orders

All of your orders are displayed in the <<orders/managing-orders#1500, Orders » Edit orders>> menu. You can use <<orders/managing-orders#1300, filters>> to narrow down which orders are displayed. For example, you can display Amazon Prime orders only.
...
Open the *System » System settings » Markets » Amazon » Settings* menu to activate the Amazon account for Amazon Business.
...
*_Or:_* You can also display up to 5 more sales prices in percent for quantity based discounts, using property links. To do so, select the Amazon link *quantity_price_type* when creating the property of the type *Text* or *Selection* and enter the value *percent* when linking the property to the item. Additionally, you have to create properties for *quantity_lower_bound1-5* and for the discount quantity and *quantity_price1-5* for the percentage.
...
The <<data/api-log#, Data exchange » API log>> menu contains a history of processes that run via the Amazon interface. The following formats are available for Amazon:
...
* Export filter *Store items*: If you have selected *Only items visible in online store* for the *Store items* export filter in the *System » System settings » Markets » Amazon » Settings » Tab: Item settings* menu, then make sure that the item is activated for the online store. This is done in the item’s *Settings* tab.
