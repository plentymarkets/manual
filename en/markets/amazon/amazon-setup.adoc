...
. In the drop-down list *Property type*, select the option *Date*.
...
You have already created a special offer price and the properties *SalesStartDate* and *SalesEndDate*. Now save this offer price for a variation and specify the period for which the offer price is valid.

[.instruction]
Save offer price for a variation:

. Go to *Item » Edit item »* _Open item_ *»* _Open variation_ *» Tab: Settings*.
