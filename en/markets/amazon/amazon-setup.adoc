...
| *handling time*
| Enter a number of days for the additional delay in delivery. This option is only displayed if the *Average delivery period* option is set to *Transmit "average delivery time in days + handling time* was selected.
...
| *Shipping category*
| *Default* = normal shipping, *Expedited* = express shipping, or *Priority* = fastest shipping service.
|====
...
*_Oder:_* Use the data format *VariationSKU* to import SKUs.
...
include::_textblocks/instructions/amz-payment-method[]
