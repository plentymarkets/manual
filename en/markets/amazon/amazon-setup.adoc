...
|*Status*
|Select the <<orders/managing-sales-orders#order-status, order status>> for orders that were imported from Amazon.
...
First, go to <<orders/managing-sales-orders#order-status, System » Orders » Status>>. Create an order status for Amazon returns, e.g. [9.6] Return sent to Amazon. You need this order status for the Amazon settings.
