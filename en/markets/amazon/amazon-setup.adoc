...
*VCS plentymarkets* = Amazon invoices are automatically generated by plentymarkets and transferred to Amazon. Amazon calculates the VAT. See <<#3150, Setting up VCS plentymarkets>>. +
*_Note:_* VCS must be activated in Amazon Seller Central. +
*VCS Amazon* = Amazon invoices are generated by Amazon. +
Amazon invoices and credit notes are imported into plentymarkets. See <<#6900, Setting up VCS Amazon>>. +
*_Note:_* VCS must be activated in Amazon Seller Central.
|====
...
[.instruction]
Setting the variation availability for an Amazon country platform:
...
. In the *Group function* section, select the action that should be carried out. Pay attention to the explanations given in <<gruppenfunktion-gtin-asin>>.
...
Link variation properties that you have created in the *Setup » Settings » Properties » Configuration* and added to a variation with Amazon in the *Setup » Settings » Properties » Configuration » _Open the property_ » Tab: Amazon* menu.

[IMPORTANT]
.Variation properties are prioritised over characteristics
====
If you linked a characteristic and a property with the same Amazon field, then the property is prioritised over the characteristic. The characteristic is overwritten by the variation property during the item export.
====
...
*_Note:_* VCS must be activated in Amazon Seller Central. +
*_Important:_* Amazon invoices can no longer be created manually or with an event procedure.
...
====
Also read the <<markets/amazon/amazon-setup#markets/amazon/best-practices-amazon-vcs, best practice for Amazon VCS>> which contains further information about the setup of Amazon’s invoice service VCS in plentymarkets. This contains additional information about setting up Amazon’s invoice service VCS in plentymarkets.
====
...
→ Invoices für Amazon orders are generated by plentymarkets. +
...
[#3150]
=== Setting up VCS plentymarkets

If you select VCS plentymarkets, Amazon calculates the VAT, but invoices for Amazon orders are created by plentymarkets and automatically uploaded to Amazon.
...
==== Activating VCS plentymarkets in Amazon Seller Central
...
. Navigate to the section *Invoice generation*.
...
==== Activating VCS plentymarkets in plentymarkets
...
→ Amazon invoices can no longer be created manually or with event procedures.
...
If you activate VCS Amazon, Amazon calculates the VAT and generates the invoices for your Amazon orders. Amazon invoice numbers and credit notes are imported into plentymarkets.
...
. *Save* (icon:save[role="green"]) the settings. +
→ Amazon calculates the VAT for Amazon orders. +
→ Amazon invoice numbers are generated by Amazon and imported into plentymarkets. +
→ Amazon invoices can no longer be created manually or with event procedures.
. *_Important:_* If you have used Amazon IDU up to now, deactivate the event procedure that uploads invoices to Amazon. Otherwise, invoice documents are uploaded to Amazon in douplicates. This can overload the interface so that some invoices are not uploaded to Seller Central at all.
...
[#3900]
=== Setting up the variation availability for Amazon Multi-Channel
...
icon:map-signs[] *_Or:_* You can also display up to 5 more sales prices in percent for quantity based discounts, using characteristic links. To do so, select the Amazon link *quantity_price_type* when creating the property of the type *Text* or *Selection* and enter the value *percent* when linking the characteristic to the item. Additionally, you have to create characteristics for *quantity_lower_bound1-5* and for the discount quantity and *quantity_price1-5* for the percentage.
...
* If all settings are correct and only some invoices are missing, you can upload these invoices to Amazon manually. Jump to the chapter <<#700, Troubleshooting: Manually uploading invoices>>.
