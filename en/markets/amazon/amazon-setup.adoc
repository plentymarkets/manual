...
An ASIN (= Amazon Standard Identification Number) is a unique product identification on Amazon which facilitates selling your items on Amazon. In order to be able to quickly and easily sell items which have already been listed on Amazon, an ASIN should therefore be saved for every variation.

[IMPORTANT]
.Save a GTIN for your variations
====
The barcode GTIN13 is used to search for ASINs on Amazon and to import them into plentymarkets. Therefore, make sure that a barcode (GTIN13) is saved for every variation. Otherwise, no ASINs will be found.
====

First, work through the *Amazon ASIN import* assistant in order to import suitable ASINs into your plentymarkets system. In addition, you can have newly imported ASINs automatically linked to your variations.

To manually link ASINs with variations, open the *System » System settings » Markets » Amazon » ASIN matching* menu.

The *System » System settings » Markets » Amazon » ASIN matching* menu contains an overview of ASINs that were imported into your plentymarkets system. Furthermore, you can use the group function to link ASINs with variations in this menu.

[#773]
=== Managing imported ASINs

The *System » System settings » Markets » Amazon » ASIN matching* menu contains an overview of both linked and unlinked ASINs. Additionally, you can link individual ASINs with variations in this overview.
...
|*GTIN*
|The GTIN that is saved for the variation.

|*Item name*
|The variation's item name.

|*Attribute name*
|The attributes linked to the variation.

|*Platform*
|The Amazon platform that the variation is listed on.

|*ASINs*
|The ASINs that are available for this variation are listed here. There can also be multiple suitable ASINs for one variation. +
ASINs can be linked manually in this column. In order to link an ASIN with a variation, place a check mark next to the ASIN that should be linked and click on *Save*.
//Einzelne Verknüpfung nochmal als separate Handlungsanweisung?

|*Error*
|Errors that occurred during the ASIN import are listed here.
//Welche Fehlermeldungen gibt es?

|*Link*
|Displays whether the ASIN has already been linked with a variation (check mark) or not (X).
|====

[#775]
=== Linking ASINs with the group function

Use the group function in the *System » System settings » Markets » Amazon » ASIN matching* menu to link multiple ASINs at once.
...
→ A section containing actions for the group function is expanded.
. In the *group function* section, select the action that you would like to carry out. Pay attention to the explanations given in <<group-function-gtin-asin>>.
. Place a check mark in the lines of the variations that should be edited with the group function.
...
|If more than one suitable ASINs were found for the variation, the first ASIN that was found is linked automatically.

|*Link all ASINs*
|If more than one suitable ASINs were found for the variation, then all ASINs that were found are linked with the variation.
|====
