...
| *Warehouse*
| Select which <<Creating a warehouse for Amazon FBA, Amazon FBA warehouse>> should be used for stock synchronisation with the account. This warehouse is also used to assign the FBA order to the account if no FBA order could be assigned to the country the order originates in because no warehouse is set up for the order country.
...
| *Shipping category*
a| Select the shipping priority.
...
The package content list prepares the packages based on the settings in the variation. You can change these settings. To do so, go to *Item » Edit item* » _Open item_ » *Tab: Settings » Area: Shipping* and change the option in the drop-down list *Packages*.
