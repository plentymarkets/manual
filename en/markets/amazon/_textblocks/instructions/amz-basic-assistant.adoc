...
| You can keep back a specific quantity of items by entering a number here. This prevents overselling.

| *Enter a number for the stock buffer.*
| This quantity is deducted from the stock that is exported to Amazon.
...
*plentymarkets invoice service* =
*Amazon invoice service* =
*plentymarkets invoice service with data from Amazon* =
...
*_Tip:_* The availability of a variation is set in the *Settings* tab of a variation.
...
*with HTML formatting* = Your item description is exported to Amazon with HTML formatting. Amazon only accepts the following HTML tags: <br>, <b>, <i>, <p>, <ul>, <li>, <table>, <tr>, <td>, <th>, <tbody> and <strong>. Amazon might reject any items with other HTML tags than the ones listed here. If you use other HTML tags, select the default setting *without HTML formatting*.
...
* *Value* = This quantity is deducted from the stock that is exported to Amazon.
...
| Enter the maximum variation quantity.
