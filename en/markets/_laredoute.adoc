...
. Go to *Setup » Markets » La Redoute » Settings*.
. Carry out the settings. Pay attention to the explanations given in <<#basic-settings-laredoute>>.
. *Save* the settings.

[[basic-settings-laredoute]]
...
|Select whether orders from La Redoute should be marked as *paid* or *open* when they are imported. This decision has to be made because customers pay La Redoute and La Redoute pays you afterwards.
|====
...
In order to link items, properties etc. with La Redoute, you have to activate the order referrer La Redoute in the menu *Setup » Orders » Order referrer*.

[.instruction]
Activating the order referrer for La Redoute:

. Go to *Setup » Orders » Order referrer*.
. Activate the referrer *La Redoute*.
. *Save* the settings.

[#300]
== Translating item information

Because La Redoute is a French market, the <<item/managing-items#, items>> that you sell there need to be available in French as well. Another requirement for selling your items on La Redoute is that the item description has to be in French. Furthermore, all of the attributes and properties that are linked with La Redoute need to be in French. The item also needs to have an EAN and item images.
...
You need to activate all variations that you want to sell on La Redoute. This is done in the *Availability* tab of a variation within the *Item » Edit item » [Open item] » Tab: Variation ID* menu.

[TIP]
.Selling all variations of an item on La Redoute
====
If you want to sell all variations of an item on La Redoute, carry out the settings below for the item's main variation. All other variations of the item then inherit the settings of the main variation.
====

[.instruction]
Activating the variation availability for La Redoute:

. Go to *Item » Edit item » [Open item] » Tab: Variation ID » Tab: Settings*.
. Open the variation that you want to sell on La Redoute.
...
Link your <<item/managing-items#480, properties>> with the La Redoute properties. The properties have to be created in French. You will receive an XLS file from La Redoute which contains the properties. Use this file to create a CSV file for importing properties.
...
. Go to *Setup » Markets » La Redoute » Item property import*.
...
. Go to *Setup » Markets » La Redoute » Category link*.
. In the column *La Redoute category ID* on the right-hand side, enter the category ID that best matches your plentymarkets category. +
...
<<item/managing-items#390, Attributes>> also have to be linked before your items can be transferred to La Redoute. An item is only displayed on La Redoute if all of its attributes were linked.
...
The payment method *La Redoute Payment* is for orders that are imported into your plentymarkets system from *link:http://www.laredoute.com/[La Redoute^]{nbsp}icon:external-link[].
...
. Carry out the settings. Pay attention to the information given in <<#payment-method-laredoute>>.
. Place a check mark next to the option *Active*.
. *Save* the settings.

[[payment-method-laredoute]]
...
| *Language*
|The following options are saved for the language selected: *Name*, *Info page*, *Info page (internal)*, *Logo* and *Upload logo*. This makes it possible to enter values for these options in various languages. The values for a language are activated as soon as a buyer selects the language in the plentyShop.
...
For further information, refer to the <<business-decisions/plenty-bi/statistics#, Statistics>> page of the manual. For an example of flat rate and percentage costs, refer to the <<payment/managing-payment-methods#20, Managing payment methods>> page of the manual.
|====
...
Set up an <<automation/event-procedures#, event procedure>> to automatically send shipping confirmations to La Redoute. The shipping confirmation is automatically sent to La Redoute as soon as the order status changes to *[7] Outgoing items booked*. La Redoute automatically informs the buyer of the shipment.

:table-event-procedure: <<#shipping-confirmations-laredoute>>

include::../_includes/textblocks/instructions/EA.adoc[]

[[shipping-confirmations-laredoute]]
...
Set up an <<automation/event-procedures#, event procedure>> to automatically send returns to La Redoute.

:table-event-procedure: <<#returns-laredoute>>
...
If you want to irreversibly delete an <<item/managing-items#, item>> offered on La Redoute from your plentymarkets system, then you first have to delete this item from the La Redoute catalogue.
