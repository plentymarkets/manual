...
a| * *Consider variation setting 'Limitation'* (default setting) = Stock is exported as specified for each variation. Stock limitation is managed in the *Item » Edit item » [Open item] » Tab: Variation ID » Tab: Settings » Area: Availability* menu using the option *Limitation*. +
...
* *Export actual net stock* = The stock *Limit* setting is ignored for all variations during export to REWE. Instead, the actual net stock is exported to REWE for each variation. You can check the actual net stock in the *Item » Edit item » [Open item] » Tab: Variation ID » Tab: Stock » Tab: Stock* menu in the column *Net*. +
...
Variations that you want to sell on REWE have to be linked with the *REWE product type* variation property. This variation property is created in your plentymarkets system as soon as you have installed the REWE plugin.
...
The automatically created property will be saved in the <<item/settings/properties#500, Setup » Settings » Properties » Configuration>> menu.
