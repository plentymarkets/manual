...
////
Item data is exported to REWE via catalogues. Go to *Data » Catalogues* to create a marketplace format for catalogues. For further information about catalogues, refer to the Managing catalogues <<data/exporting-data/managing-catalogues>> page of the manual.

[TIP]
.Visibility of the menu
====
The menu *Data » Catalogues* only becomes visible after you have installed a plugin which provides a marketplace format.
====

In order to export your items to REWE, you have to create catalogues for the different REWE product categories. Link the REWE data fields with item data saved in plentymarkets in the marketplace format afterwards. The catalogue templates are exported automatically to REWE once a day if you have activated the *Item export* in the *Setup » Markets » REWE » Basic settings* menu.
...
. Select a marketplace format from the drop-down list.
. *Save* (icon:save[role="green"]) the settings.
...
.Linking values
====
In the data fields in the lower area of the marketplace format, you have to link values specified by REWE to plentymarkets data sources. For those fields, the names given by REWE are exported to the marketplace.
====
...
| *Deactivating the creation of a contact*
a|
* _Deactivated_ (icon:toggle-off[role="darkGrey"]) (default setting) = When the first REWE order is imported, a contact is created for REWE. All other REWE orders are linked to this contact during the import. All invoice addresses are assigned to this contact.
...
* _Activated_ (icon:toggle-on[role="green"]) = No contact is created when REWE orders are imported. Invoice address and delivery address are created and linked to the order.
