...
Use the filters listed in <<#table-log-filters>> to limit the search results.

[[table-log-filters]]
.Filters
[cols="1a,2a,2a"]
|====
|Log entries |Recommended filters |Explanations
...
| While generating <<#915, price reports>>, an error log is written for all entries that have been in the REJECTED, FAILED or OVERRIDDEN status since the file was last generated (minus one hour as a buffer).

| Price reports
| * Identifier » Zalando
* Reference type » action
* Reference value » PriceReporting
| Log entries for errors when generating the price reports
|====
