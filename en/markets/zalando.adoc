...
In order to link items, characteristics etc. with Zalando, you have to activate the order referrer Zalando in the *Setup » Orders » Order referrer* menu.
...
Items have to be available for Zalando. This is done in the *Availability* tab of a variation within the *Item » Edit item » Open item » Tab: Variation ID* menu.

[TIP]
.Selling all variations of an item on Zalando
====
To sell all variations of an item on Zalando, carry out the settings described below for the main variation. The settings are then passed on to all variations of the item.
====
...
. *_Optional_:* Carry out additional settings. Pay attention to the explanations given in table 5 in chapter <<item/managing-items#80, Activating characteristics>> of the Manually creating items page.
. *Save* (icon:save[role="green"]) the settings.
