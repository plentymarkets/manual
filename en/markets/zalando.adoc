...
include::_textblocks/instructions/sku-manually.adoc[tags=sku-man;!*]
...
. Go to the *Product status reports - Account ID* area. In the row of a processing report click on *Download file* (icon:eye[role="blue"]). +
  * *_ProductErrorReport.csv_* contains the variations that can currently not be sold on Zalando.
  * *_ProductLiveReport.csv_* contains the variations that can currently be sold on Zalando. +
→ The CSV file is created.
. Save the CSV file on your computer.
