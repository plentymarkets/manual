...
NOTE: You can only link one size register and one length register to each attribute.
...
If you still use the old API, changes to the order status are not communicated to Zalando with event procedures. (<<#850, How else?>>)
