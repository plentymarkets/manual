...
a| The export log *zalandoArticleExportLog.txt* contains log entries for variations that could not be exported to Zalando. The report is generated once every night.
...
While the file is being generated, an error log is also written for all entries that have been in the REJECTED, FAILED or OVERRIDDEN status since the file was last generated (minus one hour as a buffer). These log entries are then searchable in the *Data » Log* menu with the following search parameters:
...
Errors that occur during the generation of the file can be found in the *Data » Log* menu with the following search parameters:
