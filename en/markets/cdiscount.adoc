...
.Access data
====
Use the API access data for the setup. Do not confuse the API access data with your access data for Cdiscount's back end. You find the user name within Cdiscount's back end. The back end is also where the password is generated or edited.
====

[.instruction]
Setting up Cdiscount in plentymarkets:

. Go to *System » System settings » Markets » Cdiscount » Tab: Settings*.
...
The item description must not contain HTML formatting. HTML formatting will be deleted during the item export.
...
The options that can be selected here are the settings for Configuration 1 and Configuration 2 that are set up in the <<fulfilment/preparing-the-shipment#1700, System » System settings » Orders » Shipping » Settings » Tab: Calculate shipping costs>> menu.
...
In order to link items, properties etc. with Cdiscount you have to activate the order referrer Cdiscount in the menu *System » System settings » Orders » Order referrer*.
...
====
If the setting *Request product creation* is activated in the menu *System » System settings » Markets » Cdiscount » Tab: Settings*, a SKU is generated which matches the variation ID for each variation during the export that takes place at night.
====
...
. Go to *System » System settings » Markets » Cdiscount.com » Tab: Category list*.
...
. Go to *System » System settings » Markets » Cdiscount.com » Tab: Category link*.
...
. Go to *System » System settings » Markets » Cdiscount.com » Tab: Manufacturer list*.
...
. *Tip:* Download the manufacturer list as a CSV file. You can search for specific manufacturers in this file. +
 Go to *System » System settings » Markets » Cdiscount » Tab: Manufacturer list* and click on *Download manufacturer list as CSV file*.
...
* *Apply shipping costs from shipping cost calculation* or *Overwrite shipping costs* is set up.

| Settings for the variation
a|
...
| Settings for the variation for single items
a|
* The variation is *active*
* The variation *availability* is set.
* A *barcode* is created and linked.
...
. Go to *System » System settings » Markets » Cdiscount.com » Tab: Archive*.
...
====
Link the properties *Gender*, *Main colour*, *Note*, *Comment*, *Marketing description*, *Audience* and *Sport* in the <<item/managing-items#480, System » System settings » Item » Properties>> menu in order to transfer your item catalogue to Cdiscount via a CSV file.
====
...
. Carry out further settings as described in <<event-procedure-returns-cdiscount>> below.
. Place a check mark next to the option *Active*.
. *Save* the settings. +
...
. *Save* the settings. +
→ The event is created.
.  Carry out further settings as described in <<update-order-status-cdiscount>>.
.  Place a check mark next to the option *Active*.
. *Save* the settings. +
→ The event procedure is saved.
--
...
The *Data » API log* menu contains a history of processes that run via the Cdiscount interface. The *Cdiscount.com* format is available for Cdiscount.
