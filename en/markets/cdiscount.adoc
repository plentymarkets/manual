...
If you use the same attribute for single items and variation items on Cdiscount, you can use a characteristic of the type *No* that is linked to the Cdiscount characteristic *Variation item* to list the item as single item.
...
[IMPORTANT]
.Credit notes for returns always for the sum of all order items 
====
If a Cdiscount order has more than one order items and the customer returns only one order item, Cdiscount will always refund the total sum of the order. This is due to a limitation on Cdiscount. +
*_Example:_*
The customer orders a sweater for 30€ and a jacket for 50€ and returns the jacket. A credit note for all order items is created, in this case this would be 80€.
====
