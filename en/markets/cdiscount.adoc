...
|Activate in order to activate the automatic stock synchronisation and the order import via the API interface. +
...
====
Every created item is saved in a database on Cdiscount. Every item can be identified by at least one EAN. Flagging products with EANs enables sellers to create offers for these products. The setup is simpler in comparison to the previous product creation and makes it easy to get started on Cdiscount. +
