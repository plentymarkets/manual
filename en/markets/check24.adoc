...
After installing and setting up the plugin, the automatic processes described in <<#data-exchange-check24>> become available:
...
. Activate the option *Activate automatic data exchange*.
. Enter your *user name* for the FTP partner login.
. Enter your FTP access *password*.
...
[#500]
== _Alternative 1:_ Item export with the Elastic Export (deprecated)
...
NOTE: No new features are planned for the Elastic Export and the export formats Check24 and Check24 Fashion. As such, we recommend that you <<#900, create catalogues for Check24 instead>>.

[.collapseBox]
.Item export with the Elastic Export (deprecated)
--
...
[#900]
== _Alternative 2:_ Item export with catalogue formats (test phase)

With the link:xxxx[new Check24 plugin^]{nbsp}icon:external-link[], item data is exported to Check24 in catalogue formats. Go to *Data » Catalogues* to create catalogues.
...
[#170]
=== Installing the catalogue format plugin

To export item data to Check24 with catalogue formats, first install the link:xxxx[new Check24 plugin^]{nbsp}icon:external-link[] in your plentymarkets system. You can download the plugin in the link:https://marketplace.plentymarkets.com/en[plentyMarketplace^]{nbsp}icon:external-link[].
