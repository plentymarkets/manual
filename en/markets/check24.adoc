...
[#500]
== _Alternative 1:_ Item export with the Elastic Export (deprecated soon)
...
NOTE: No new features are planned for exporting item data to markets with the Elastic Export. As such, we recommend that you <<#900, create catalogues for Check24 instead>>.
...
With the link:xxxx[new Check24 plugin^]{nbsp}icon:external-link[], item data is exported to Check24 in a catalogue format. Go to *Data » Catalogues* to create catalogues.

* Is this your first time working with catalogues? Then read up on the <<en/data/exporting-data/managing-catalogues, principle of catalogues first>>.

[#170]
=== Installing the catalogue format plugin

To export item data to Check24 with a catalogue format, first install the link:xxxx[new Check24 plugin^]{nbsp}icon:external-link[] in your plentymarkets system. You can download the plugin in the link:https://marketplace.plentymarkets.com/en[plentyMarketplace^]{nbsp}icon:external-link[].
