...
:keywords: Rakuten, market, marketplace, Rakuten setup, setting up Rakuten, elastic export, RakutenDE plugin, Rakuten shipping groups, ElasticExportRakutenDE, Rakuten payment, Rakuten Rechnung, Rakuten delivery periods
...
IMPORTANT: The market Rakuten.de 1 is closing down. From the 15th of October, no new orders can be placed on the market.
...
You need to register with Rakuten before setting up Rakuten in plentymarkets. Use the registration forms for the platform link:https://leadgen.rakuten.de/form?leadsource=marketplace_header[rakuten.de^]{nbsp}icon:external-link[].
...
| *Activate*
a| * *Deactivate* = The interface is inactive. +
...
| *Payment*
a| * *Incoming payment pending* = Import orders as unpaid. +
...
a| * *Deactivate* = Order confirmations are not transferred. +
...
The following processes are carried out by the link:https://marketplace.plentymarkets.com/en/plugins/channels/marktplaetze/elasticexportrakutende_4721[ElasticExportRakutenDE plugin^]{nbsp}icon:external-link[].
...
After you have installed the *ElasticExportRakutenDE plugin*, you can use the data format *RakutenDE plugin* for the elastic export. The plugin can be downloaded from the link:https://marketplace.plentymarkets.com/en/plugins/channels/marketplaces[plentyMarketplace^]{nbsp}icon:external-link[]. For further information about setting up the plugin in plentymarkets, refer to the plugin’s user guide.
...
. Activate the referrer *Rakuten.de*.
. *Save* (icon:save[role="green"]) the settings.

[#300]
== Setting the variation availability

You need to activate all variations that you want to sell on Rakuten. This is done in the *Availability* tab of a variation within the *Item » Edit item »* _Open item_ *» Tab: Variation ID* menu.

[TIP]
.Selling all variations of an item on Rakuten
====
If you want to sell all variations of an item on Rakuten, carry out the settings below for the item's main variation. All other variations of the item then inherit the settings of the main variation.
====

[.instruction]
Activating the variation availability for Rakuten:

. Go to *Item » Edit item »* _Open item_ *» Tab: Variation ID » Tab: Settings*.
. Open the variation that you want to sell on Rakuten.
...
. Select the main category for the item from the *Rakuten.de* drop-down list within the *Category link* area. +
...
The values for the delivery periods are also available in Rakuten’s link:https://infos.rakuten.de/academy/das-haendlerkonto-1/produkte/csv-import[interface documentation^]{nbsp}icon:external-link[]. This documentation is currently only available in German.

==== Adding prefixes and suffixes to a parent SKU

If you have installed the link:https://marketplace.plentymarkets.com/plugins/markets/ElasticExportRakutenDE_4721[ElasticExportRakutenDE plugin^]{nbsp}icon:external-link[] in your plentymarkets system, then you can add prefixes and suffixes to parent SKUs. This way you avoid identical parent and child SKUs which will be treated as the same item on Rakuten.
...
====
The prefixes and suffixes are only added to those SKUs which you generate for Rakuten.de after you have saved this setting. Parent SKUs which already exist are not considered for this setting. Add prefixes and suffixes manually to those SKUs if desired.
====
...
. Activate the option *Price update*.
. *Save* (icon:save[role="green"]) the settings.
...
. Click on the *ElasticExportRakutenDE* plugin.
. Go to *Configuration » Update settings*.
. Activate the option *Stock update*.
. *Save* (icon:save[role="green"]) the settings.
...
====
Rakuten sends the invoice directly to the customer when Rakuten Rechnung is selected as the payment method. If you also issue an invoice, enter a *payment instruction* in the *Optional elements in the stock unit table* area. This payment instruction informs the customer that they have to pay Rakuten.
...
Designated use 2: First name & last name
====
...
:table-event-procedure: <<#event-procedure-shipping-preparation-rakuten>>
...
:table-event-procedure: <<#event-procedure-shipping-confirmation-rakuten>>
...
:table-event-procedure: <<#event-procedure-returns-rakuten>>
...
_or:_ *Order generation: New credit note*
|
...
:table-event-procedure: <<#event-procedure-cancellations-rakuten>>
...
a|Select a reference value to search for event procedures of a specific type. +
