...
. Map the marketplace data field *Colour* to a plentymarkets data field: +
*_Alternative 1:_* Field *Value name* of the attribute for *Colour*. +
*_Alternative 2:_* Property for *Colour*.
...
→ No second shipping confirmation is sent to limango.
...
* You check and correct the shipping service provider in the plugin settings and then move the order into the status for *Order status for orders with shipping carrier mapping (> 5 - < 7)*. +
Go to *Plugins » Plugin set overview » [Open plugin set] » Open limango plugin » Shipping service provider* to correct the link.
...
For event procedure 1 for shipping confirmations, set up the <<automation/event-procedures#, event procedure>> described in <<#event-procedure-package-number-limango>>.

:table-event-procedure: <<#event-procedure-package-number-limango>>
...
You set this status in the menu *Plugins » Plugin set overview » [Open plugin set] » Open limango plugin » Order status*.
