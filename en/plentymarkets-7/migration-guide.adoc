...
[IMPORTANT]
.plentymarkets 7 is not compatible with external clients (stores) or the shopware Connector
====
plentymarkets 7 is not compatible with external clients (stores) or the shopware Connector. As such, you cannot upgrade to plentymarkets 7 if you use an external client (store) and/or the shopware Connector.
====
...
image::plentymarkets-7/assets/EN-plentymarkets-7-Migration-Guide-01.png[]
__Image 1: current state of the new item architecture__
...
image::plentymarkets-7/assets/EN-plentymarkets-7-Migration-Guide-02.png[]
__Image 2: Looking ahead; managing data for each variation__
...
* **Hosting**: Your system must be hosted on AWS. plentymarkets 7 cannot be used on own servers or the plentymarkets cloud (cluster). +
→ You can <<start/plentymarkets-account/relocation#, **move**  >> your system before migration if plentymarkets hosts your main domain.
...
* **Shopware Connector**: plentymarkets 7 is not compatible with external clients (stores) or the shopware Connector. As such, you cannot upgrade to plentymarkets 7 if you use the shopware Connector.
* **External clients (stores)**: plentymarkets 7 is not compatible with external clients (stores). As such, you cannot upgrade to plentymarkets 7 if you use an external client (store).
* **plentymarkets Shipping**: plentymarkets Shipping cannot be used with plentymarkets 7 and is no longer supported. Use the current link:https://www.plentymarkets.eu/software/downloads/plentymarkets-client[**plentymarkets Client**] instead.
* **Markets**: At this stage, not all markets are functioning in plentymarkets 7. Before migrating, check the <<plentymarkets-7#, **list**  >> of those markets that have not been migrated yet.
...
This chapter teaches you how to migrate your data to the new item architecture. The description is divided into two main parts, which teach you how to migrate **item data** and **price columns**. Both parts require you to do a little bit of work in advance. The preparatory work is briefly described in this chapter. However, you can find a detailed description in the chapter **<<plentymarkets-7/migration-guide#30., Preparation in detail >>**.
...
__Table 1: migrating the item data__
...
|A new, unique variation number will be generated for each variation. This variation number is composed of a prefix and the next variation number. Before data is migrated, go to **<<item/managing-items#15., Settings » Item » Basic settings >>** and use the settings **Prefix for new variation numbers (plentymarkets 7 and higher)** and **Next variation number (plentymarkets 7 and higher)** to specify this information.

|Exception 2: +
Items/Variations with the same item/variation number
|The variation that is migrated first receives the item/variation number. A new, unique variation number will be generated for all of the other variations. This variation number is determined by the <<plentymarkets-7/migration-guide#35., **migration settings** >> found in the menu **Settings » Basic settings » Migration**. It is controlled with the setting **Duplicate number**.

|Exception 3: +
Items with multiple price sets
|One variation can be turned into several different variations. When the first price set is migrated, the variation receives the item/variation number. A new, unique variation number will be generated for all of the other variations. This variation number is determined by the <<plentymarkets-7/migration-guide#35., **migration settings** >> found in the menu **Settings » Basic settings » Migration**. It is controlled with the setting **Duplicate number**. +
...
|Preparation
|Carry out or check the <<plentymarkets-7/migration-guide#35., **migration settings** >>. +
Go to **<<item/managing-items#15., Settings » Item » Basic settings >>** and configure the settings **Prefix for new variation numbers (plentymarkets 7 and higher)** and **Next variation number (plentymarkets 7 and higher)**. +
<<plentymarkets-7/migration-guide#50., **Check** >> whether you have items with multiple price sets. Then check whether any of these price sets use the same content and unit. Change the content and unit for the additional price sets.
|====

__Table 2: migrating item/variation numbers__
...
[.subhead]
**Unit 1** and **Unit 2**
In the new item architecture, the fields **Unit 1** and **Unit 2** were replaced with the field **Variation name**. Use the <<plentymarkets-7/migration-guide#35., **migration settings** >> to specify how the content of these fields should be migrated. The following pattern is used when migrating data:
...
|Behaviour
|Depending on how you configured the <<plentymarkets-7/migration-guide#35., **migration settings** >>, the system will either discard one of the units or it will merge both of the fields together. +
If you want the fields **Unit 1** and **Unit 2** to be combined together in the field **Variation name**, then open the <<plentymarkets-7/migration-guide#35., **migration settings** >> and specify which separator should be used. The content of both fields will be inserted into the **Variation name** field and will be separated from each other with this character.

|Preparation
|<<plentymarkets-7/migration-guide#50., **Check** >> the units and specify the separator. Change the fields **Unit 1** and/or **Unit 2** if needed.
|====

__Table 3: migrating the fields **Unit 1** and **Unit 2**__
...
__Table 4: migrating the store visibility__

[.subhead]
GTIN1 to GTIN4, ISBN[#25]
...
|Preparation
|<<plentymarkets-7/migration-guide#100., **Check** >> for EAN/GTIN that start with a zero.
|====

__Table 5: Migrating GTIN1 to GTIN4/ISBN__

[#26]
=== Migrating the price columns

The previously existing price sets and price columns were replaced with sales prices in the new item architecture. These sales prices are defined in the **Settings » Item » Sales prices** menu. When you open a variation, you will be able to add these sales prices and then save the actual monetary amounts. The new sales prices are composed of the following settings:
...
|1
|**Settings » Orders » Order referrer**
...
a|
* Check which price columns were set up in the **Order referrer** menu and <<plentymarkets-7/migration-guide#80., **compare** >> them to the price columns saved for individual markets. Make any changes as needed. +
**Fallback:** Some markets allow you to specify an individual price column (e.g. Shopgate). If you selected the standard price for these markets but you also selected a different price column under **Settings » Orders » Order referrer**, then this price column will be used instead of the standard price.

|2
|**Settings » Orders » Payment » Currencies**
...
a|
* Check the **Currency conversion** setting for every client. If your clients use different settings, then consider whether it makes sense to change them so that **<<plentymarkets-7/migration-guide#70., all clients use the same settings >>**, thus reducing the number of newly created sales prices.
* Clients with the setting "Use price column": +
**<<plentymarkets-7/migration-guide#75., Check >>** which price columns were linked to currencies in the menu **Settings » Orders » Payment » Currencies**. Make sure that all of your items have amounts saved in these price columns. +
...
a|
* A sales price is created for every price column and price visibility that is currently used (**Show everyone**, **Only show B2C customer classes**, **Only show B2B customer classes**). Therefore, check the **Price visibility** setting as it pertains to <<plentymarkets-7/migration-guide#90., **customer classes** >>.

|4
|**Settings » Client (store) » Select client » Online store » Basic settings**
|Goes into effect if there are no other settings with a higher priority.
a|
* Check the price columns that were selected in the menus **<<plentymarkets-7/migration-guide#60., Referrer, Currency, Customer class, Client (store) >>**. If the same price column is selected in more than one of these menus, then the settings will be combined. Check these combinations and make sure that the settings do not lead to any unexpected behaviour.

|====

__Table 6: priority when creating sales prices__
...
**Generate new number** = The variation that is migrated first receives the item/variation number. New numbers are generated for the rest of the variations. Go to **<<item/managing-items#15., Settings » Item » Basic settings >>** and enter the prefix and next variation number that should be used for generating these numbers. +
...
__Table 7: migration settings__
...
__Table 8: pattern for the next variation number__

=== Item data

//  h4>Artikel-/Variantennummern eindeutig machen<a id="40." name="40."></a></h4> <p>In der neuen Artikelarchitektur wird pro Variante eine systemweit eindeutige Variantennummer vergeben. Prüfen Sie daher vor der Migration, ob Artikel- und/oder Variantennummern doppelt vergeben wurden. Ersetzen Sie ggf. doppelt vergebene Nummern durch eindeutige Nummern. Verwenden Sie dazu das Datenformat <a href="{% Link(894) %}"><strong>AttributeItemVariation</strong></a> mit den folgenden Datenfeldern.</p> <h5>Artikel-/Variantennummern eindeutig machen:</h5> <ol> <li>Öffnen Sie das Menü <strong>Datenaustausch » Export » Dynamischer Export</strong>.</li> <li>Erstellen Sie ein neues <strong><a href="{% Link(683) %}">Datenformat</a></strong> des Datenformat-Typs <a href="{% Link(894) %}"><strong>AttributeItemVariation</strong></a>.<br /> → Die empfohlenen Einstellungen für Datenfelder und Aktionen entnehmen Sie Tabelle 9.</li> <li>Exportieren Sie das Datenformat.</li> <li>Prüfen Sie die Datensätze der Spalten <strong>ItemNo</strong> und <strong>VariationNo</strong>.<br /> ❏ Gibt es doppelt vergebene Artikel- und/oder Variantennummern?</li> <li>Ersetzen Sie ggf. doppelt vergebene Nummern durch eindeutige Nummern.</li> <li>Speichern Sie die Änderungen in der CSV-Datei.</li> <li><strong><a href="{% Link(3183) %}">Importieren</a></strong> Sie die CSV-Datei zurück in plentymarkets, wenn Sie Änderungen vorgenommen haben.</li> </ol> <table> <tbody> <tr> <th>Feld</th> <th>Importaktion</th> <th>Erläuterung</th> </tr> <tr> <td><b>ItemID</b></td> <td>Abgleich</td> <td>Artikel-ID</td> </tr> <tr> <td><b>ItemNo</b></td> <td>Import</td> <td>Artikelnummer</td> </tr> <tr> <td><b>VariationNo</b></td> <td>Import</td> <td>Variantennummer</td> </tr> </tbody> </table> <p><em>Tab. 9: Datenfelder Datenformat <b>AttributeItemVariation</b></em></p
[#55]
==== Checking EAN/GTIN

Leading zeros in EAN/GTIN codes are truncated during migration. Because this changes the code's character count, the EAN/GTIN is not migrated as a GTIN but instead is saved as a Code128 or UPC. As such, check if GTIN with one or more leading zeros exist before migration. If EAN with leading zeros exist, you will need to correct these GTIN <<plentymarkets-7/migration-guide#100., **after the migration** >>.
...
. Create a new **<<data-exchange/exporting-data#, data format  >>** of the type <<data-exchange/data-formats/item#, **Item**  >>. +
...
. If you find GTIN with leading zeros, save the CSV file for later use. You will need this GTIN data to correct the GTIN <<plentymarkets-7/migration-guide#100., **after the migration** >>.
...
__Table 9: data fields of the data format **Item**__
...
In the new item architecture, the fields **Unit 1** and **Unit 2** were replaced with the field **Variation name**. Use the <<plentymarkets-7/migration-guide#35., **migration settings** >> to specify whether the content of one field or both fields should be migrated. If you want both fields to be migrated, then enter the separator that should be displayed between the two units. Decide on an appropriate separator before you begin migrating your data. This will ensure that the resulting varition name meets your needs. Edit the content of the fields **Unit 1** and/or **Unit 2** if needed.
...
__Table 10: example for migrating the fields **Unit 1** and **Unit 2**__
...
__Table 11: example of identical content saved for the **Content** and **Unit**__
...
__Table 12: example of new content saved for **Content** and **Unit**__

[.subhead]
Instructions
Check whether you have items with multiple price sets. Then check whether any of these price sets use the same content and unit. This can be done with the data format <<data-exchange/data-formats/itemprice#, **ItemPrice**  >> in the dynamic export. To do so, proceed as follows.

[.instruction]
Checking the units:

. Go to **Data exchange » Export » Dynamic export**.
. Create a new **<<data-exchange/exporting-data#, data format  >>** of the type <<data-exchange/data-formats/itemprice#, **ItemPrice**  >>. +
...
. **<<data-exchange/importing-data#, Import  >>** the CSV file back into plentymarkets if you made any changes.
. Go to **Settings » Basic settings » Migration** and configure the settings for the options **Merge Unit 1 + 2** and **Separator for merging Unit 1 + 2**.
...
__Table 13: data fields of the data format **ItemPrice**__
...
__Table 15: menus for price columns__
...
. Go to **Settings » Client (store) » Select client » Online store » Currency**. Access this menu for every client that you work with.
...
. Go to **Settings » Orders » Payment » Currencies**.
...
. **<<data-exchange/importing-data#, Import  >>** the CSV file **ItemPrice** back into plentymarkets if you made any changes.
...
__Table 16a: data fields of the data format **ItemPrice**__
...
__Table 16b: data fields of the data format **Item**__

[#80]
==== Comparing the price columns for referrers

Compare the price columns that were saved in the menu **Settings » Orders » Order referrer** with the settings that were saved for referrers that have their own price columns. If the settings differ, then consider whether it makes more sense to use the same settings. These referrers are:
...
**Fallback:** Some markets allow you to specify an individual price column (e.g. Shopgate). If you selected the standard price for these markets but you also selected a different price column under **Settings » Orders » Order referrer**, then this price column will be migrated instead of the standard price.
...
|Amazon
|**Settings » Markets » Amazon » Basic settings**
...
|eBay
|**Settings » Markets » eBay » Basic settings**

|Hitmeister
|**Settings » Markets » Hitmeister » Basic Settings**

|Allyouneed
|**Settings » Markets » Allyouneed » Basic settings**

|Neckermann
|**Settings » Markets » Neckermann » Neckermann Austria Enterprise +
Settings » Markets » Neckermann » Neckermann Austria Cross-Docking**

|Otto
|**Settings » Markets » Otto » Integration**

|Shopgate
|**Settings » Markets » Shopgate**
|====

__Table 17: markets with their own price columns__
...
__Table 18: data fields of the data format **ItemPrice**__
...
. Go to **Start » plentymarkets account » Support ticket**.
...
. Go to **Data exchange » Export » Dynamic export**.
. Create a new **<<data-exchange/exporting-data#, data format  >>** of the type <<data-exchange/data-formats/variationbarcode#, **VariationBarcode**  >>. +
...
→ **Tip:** To correct a large number of GTIN codes, we recommend using the Excel function **link:https://forum.plentymarkets.com/t/excel-tabelle-vergleichen/19235/2[VLOOKUP, window="_blank"]**.
. Save the changes you made to the CSV file of the **VariationBarcode** export.
. **<<data-exchange/importing-data#, Import  >>** the CSV file of the **VariationBarcode** export back into plentymarkets.
...
__Table 19: Data fields of data format **VariationBarcode**__
...
Your previous prices were migrated to sales prices. Look a bit **<<plentymarkets-7/migration-guide#26., farther up >>** in this migration guide if you want to find out how the data was migrated and the new prices were created. In plentymarkets 7, your sales prices are found in the menu **Settings » Item » Sales prices**.

[.instruction]
Assigning prices:

. Go to **Settings » Basic settings » API » SOAP migration**.
...
Your previous GTIN, ISBN, etc. were migrated to barcodes. Look a bit **<<plentymarkets-7/migration-guide#25., farther up >>** in this migration guide if you want to find out how the data was migrated. In plentymarkets 7, your new barcode definitions are found in the menu **Settings » Item » Barcode**.
...
An overview of the most important changes can be found **<<plentymarkets-7/changes#, here  >>**.
