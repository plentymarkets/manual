...
:keywords: marketplaceStockMovement, report, raw data
:position: 75
:url: business-decisions/business-intelligence/reports/data-formats/marketplacestockmovement
:id: 57VTDG7
:author: team-multi-channel

:dataFormat: pass:quotes[*marketplaceStockMovement*]
:data: stock movements to specific markets
:specifications: <<tabelle-spezifikationen-marketplacestockmovement>>
:dataColumns: <<tabelle-datenspalten-marketplacestockmovement>>

include::./_textblocks/einleitung-en_gb.adoc[]

Stock movements to the following markets currently saved:
...
|plentyBI L
|Minimum fee plan required for making the data available in plentyBI Data Warehouse, i.e. for using, filtering and process key figures.
|====
...
| VARCHAR NOT NULL
| ID
| The unique ID of the stock movement
...
Stock movements can be filtered in the plentyBI tool based on this identifier value.

| referrerId
| NUMERIC(5,2)
| Referrer ID
| The ID of the market’s order referrer +
*_Note:_* At the moment, only stock movements to the market Amazon are saved.
