...
| *Colour*
<<<<<<< HEAD:en/business-decisions/user-accounts-access.adoc
|Define a colour in the editing window by selecting a colour, entering a hexadecimal colour code or the RGB value. The colour makes it easier to identify the user account in other menus, for example the <<welcome/general-functions/tasks-and-calendars#600, Calendar>>.
=======
...
>>>>>>> master:en/basics/admin-tasks/manage-users.adoc
...
| *Roles*
<<<<<<< HEAD:en/business-decisions/user-accounts-access.adoc
|Select roles for the user account. <<business-decisions/user-accounts-access#70, Roles>> must already have been created before they can be selected here.
=======
...
====
The configuration described is only relevant for plentymarkets customers with domains managed by plentymarkets who want to use the external provider link:https://mailbox.org/business/[mailbox.org^]{nbsp}icon:external-link[]. If you manage your email account with a different provider and want the plentymarkets system to send emails from this provider too, you do not need to set up mailbox.org. However, you still need to refer to chapter <<crm/sending-emails#100, Saving the email server's access data>> on the <<crm/sending-emails#, Sending emails>> page of the manual.
====
...
| *Calendar*
|Make changes to the <<welcome/general-functions/tasks-and-calendars#600, Calendar>> settings in this tab.
=======
...
| *Right*
|For further information about this tab, refer to the <<basics/admin-tasks/manage-users#105, Managing rights>> chapter.
...
In the *Setup » Settings » User » Rights » User* menu, you define which areas and settings are visible to the user with this user account when opening a menu. Without visibility rights, a warehouse or orders of a certain status, for example, are invisible to the user.
