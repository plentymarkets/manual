...
Or click on *New configuration* (icon:plus[role="green"]).
...
[IMPORTANT]
.This menu is outdated and will soon be removed
====
We don’t recommend ordering SSL certificates in the setup menu.
The menu is outdated and will soon be removed.
Furthermore, it’s only possible to order DV certificates in the setup menu.
Therefore, we recommend ordering SSL certificates <<business-decisions/system-administration/ssl-certificate#1220, with the assistant>>.
====
