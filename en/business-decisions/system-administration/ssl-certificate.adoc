...
** should be ordered again after 5 months.
...
* Renewal:
** not automatically renewed
** should be ordered again at the end of the validity period.
...
* Renewal:
** Cannot be renewed.
The certificate needs to be ordered again in the assistant at the end of the validity period.
|====
...
[#300]
== Prerequisites for ordering an SSL certificate
...
[IMPORTANT]
.Not all SSL certificates are ordered in the assistant
====
...
Example: If your domain already has an SSL certificate in the Setup menu, and then you order a new certificate in the assistant, then the new certificate will overwrite the existing one.
====
...
[IMPORTANT]
.Not all SSL certificates are ordered in the setup menu
====
...
. Click on the down arrow (icon:chevron-down[role="darkGrey"]) on the far right of the line *Order new SSL for domain: [domain name]*. +
→ You will see the SSL certificates that are available for the domain.
...
SSL certificates are _not automatically renewed_.
Rather, they need to be <<business-decisions/system-administration/ssl-certificate#1200, ordered again>> when they expire.
Even if you already have an SSL certificate, you will still need to order a new certificate when it expires.
...
[.collapseBox]
.*Sectego: OV and EV certificates*
--

. Go to *Setup » Assistants » Basic setup*.
. Click on the *SSL management* assistant. +
→ Your previously configured SSL certificates are listed here.
. Click on an existing SSL certificate to open its settings.
. Navigate to the step *Summary*.
. Expand the field *SSL type* (icon:chevron-down[role="darkGrey"]).
. The line *Active until* shows you when the certificate expires.
. <<business-decisions/system-administration/ssl-certificate#1220, Re-order the certificate if needed>>.

--

[.collapseBox]
.*AlwaysOn or COMODO: DV certificates*
--

. Go to *Setup » Client » [Select client] » SSL*. +
→ You will see a table with the domain’s certificates. +
. The column *Valid until* shows you when the certificate expires.
. <<business-decisions/system-administration/ssl-certificate#1280, Re-order the certificate if needed>>.
...
However, the SSL certificate needs to be saved close to the system.
This means that even if the domain is hosted externally, the actual encoding is done directly in the system after the domain forwarding via IP address is carried out.
As such, it is not possible to use an external SSL certificate.
The SSL certificate needs to be ordered from plentymarkets.
...
If you switch the main domain back to whichever domain already had an SSL certificate, then it can be re-activated, assuming that the SSL certificate hasn’t expired.
...
An SSL certificate that you booked with plentymarkets needs to be saved *_close to the system_*.
Therefore, you cannot take it with you when moving your domain.
...
Once you’ve ordered a new certificate, it will need to be published during a regularly scheduled *system process*.
*This can take up to 120 minutes*.
In rare cases, the certification authority performs a quality control before they activate the SSL certificate.
Therefore, check the confirmation message displayed on the screen.
Normally, quality control takes up to 24 hours.
If the status in the back end is not “Complete” after 24 hours, then contact the support team in the forum and specify which domain is affected.
...
The domain might be classified as "not secure" for a maximum of one hour if the AlwaysOn certificate was just ordered, since it needs to be requested, generated and installed after the order is placed.
Once the order has received the status “Complete” in the back end, the local browser cache must be deleted in order to update the certificate.
