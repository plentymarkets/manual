...
//A number with decimal places can be displayed in the plentyShop.
//Appropriate e.g. for displaying measurements in the plentyShop. For example, a pitcher that holds 1.4 litres.
...
. <<item/settings/properties#710, First>>: Create the various options - like yes and no.
...
. <<item/settings/properties#710, First>>: Create the various options, i.e. all of the Bluetooth profiles.
...
Should the property be sorted into one or more <<item/settings/properties#1200, groups>>?
If so, then select the appropriate groups from the drop-down list.
...
It would take a lot of time to manually select the group for every single property.
