...
*_Tip:_* You can customise the statuses in this drop-down list. This is done in the menu <<orders/managing-orders#1200, *System » System settings » Orders » Status*>>.
...
*_Tip:_* You can customise the warehouses in this drop-down list. This is done in the menu <<stock-management/setting-up-a-warehouse#200, *System » System settings » Stock » Warehouse*>>.
...
|*Template*
|What should the <<Finding the items in the warehouse, list of items>> look like? The plentymarkets app comes with a default template, which is appropriate for most picking processes. However, you can create additional templates in the menu *System » System settings » plenty app » Mobile pick list*. Once you've created a template, you'll be able to select it from the dropdown list here. +
...
* remove the item from the list completely, e.g. if the item is not currently in stock. In this case, all of the orders that contain this item are placed in an <<Configuring one-time settings, error status>>.
...
Once all of the boxes have been filled with items, the warehouse technician walks back to the packing table. He scans the first box and thus loads the corresponding order. The warehouse technician <<Modifying the order process, processes this order as usual>>. Outgoing items are booked from the temporary storage location, i.e. from the box.
