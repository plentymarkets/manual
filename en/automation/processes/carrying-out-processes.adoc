...
Proceed as described below to run a process. Note that you can only see those processes that have been <<automation/processes/setting-up-processes#10, assigned>> to you as a user or that have been assigned to your role. **Variable users** will only be able to see the **System » Processes** menu if they were given the appropriate right. Go to <<basics/working-with-plentymarkets/manage-users#, System » Settings » User » Accounts » Open user » Tab: Right » Tab: Menu>> and activate the right **Processes**.
