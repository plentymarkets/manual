...
:author: team-mobile
:keywords: plentyBase, processes, printer, installing printer, install printer, run processes, execute processes, automation, print history, base, Base, plenty Base user guide, plentyBase user guide, install plentyBase, installing plentyBase, plentyBase installation, HTTPS port, HOSTS file, plentymarkets app, app connection data, save connection data
...
→ The window *Manage access tokens* opens.
. Click on *Add*. +
...
. At the end of the file, enter the IP address of the host computer on which plentyBase is installed followed by the host address `local.plentybase.de`. This results in the following pattern: +
[IP address of the host computer] local.plentybase.de +
*_Example:_* 123.123.1.123 local.plentybase.de +
*_Note:_* Lines beginning with `\#` are comments and will not be interpreted. Thus, your entry cannot start with `#`.
. *Save* the file.
...
image::automation/processes/assets/accesstoken-EN.png[width=640]
...
If you are using a Fritz!Box, connection problems might be directly related to it. Users operating a Fritz!Box can find a suitable instruction to fix connection problems link:https://forum.plentymarkets.com/t/es-kann-keine-verbindung-zu-plentybase-hergestellt-werden-fritzbox-benutzer-loesungsvorschlag/607564[in this forum topic^]{nbsp}icon:external-link[] (German only).
