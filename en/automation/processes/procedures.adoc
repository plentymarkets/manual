...
Use the **Order** procedure along with a sub-procedure to <<orders/managing-orders, process orders>>. For example, you can change the status of selected orders or scan package numbers.
...
The **Warehouse pick list** procedure generates <<orders/order-documents/warehouse-pick-list#, warehouse pick lists>> for selected orders in the process. They can then e.g. be printed or saved with the corresponding sub-procedure.
...
Use the **packing list** procedure to generate a list of picked items prepared for shipping. Use it along with a sub-procedure to print or save the <<orders/order-documents/packing-list#, packing list>>.
...
|**Item status**
|Select the item status that should be used by default when a return is created. Go to <<orders/managing-sales-orders#return, System » Orders » Order types » Return>> to create item statuses for returns.

|**Item value**
|Enter a percentage. This percentage will be applied to returned items. This value will also be displayed once a return has been created. Simply open the <<orders/managing-sales-orders#return, return>> and click on the **Overview** tab. You will find the value next to the stock units.
...
|**Reason for return**
|Select a reason. Returns will receive this reason when they are first created. Go to <<orders/managing-sales-orders#return, Settings » Orders » Order types » Return>> to create reasons for returns.
|====
...
Use the **Return label** procedure along with a sub-procedure to automatically print or save a <<orders/order-documents/generating-a-return-label#, return>> label.
...
Use the **Return slip** procedure along with a sub-procedure to print or save a return slip automatically. This return slip is attached to the item in case customers might need it for a <<orders/order-documents/generating-a-return-label#, return>>.
