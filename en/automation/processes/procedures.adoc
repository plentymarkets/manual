...
|*Mode*
|Specify how the order search should work. +
*Default* = Searches for orders _and_ their corresponding returns, credit notes, delivery orders, etc. For example, if the warehouse technician searches for an order ID, then the search results will not only display the corresponding order but also any returns that are linked to the order. +
*Ignore order relations* = Searches for exact matches _only_. For example, if the warehouse technician searches for an order ID, then the search results will _only_ display the order. The return that's linked to the order will not be included in the search results. +
*Box search* = Is used for <<automation/processes/mobile-box-picking, mobile box picking>>. The warehouse technician scans a storage location's barcode. plentymarkets loads the corresponding order.
...
With the **polling** procedure, you can create a <<fulfilment/preparing-the-shipment#1400, polling file>> in CSV format that you need for particular shipping providers. You can save this file with the Save sub-procedure.
...
Use the **SEPA Pain001** procedure to export transaction files of your <<payment/managing-bank-details#200, SEPA direct debits>> to a ZIP file. Provide your bank with this file after you saved it with a sub-procedure.
...
Use the procedure **SEPA Pain008** to export transaction files of your <<payment/managing-bank-details#200, SEPA direct debits>> into a ZIP file. Provide your bank with this file after you saved it with a sub-procedure.
