...
:author: team-mobile
:keywords: Setting up processes, process setup, process settings, process element, process elements, work step, work steps, import process, import processes, plentyBase
:position: 10
...
*_Note:_* If the menu *Setup » Processes* is not visible, then install the link:https://marketplace.plentymarkets.com/en/plugins/integration/plentybase_5053[plentyBase^]{nbsp}icon:external-link[] plugin.
. Carry out the desired settings. Note <<table-process-settings>>.
. *Save* the settings. +
...
*_Tip:_* The easiest way to find faulty orders is to create a specific status that you’ll use for errors. In the *Orders* menu, you can then filter all the orders that currently have this status.
...
*_Tip:_* A grey beetle identifies the procedures that can be used to deal with errors. +
...
You can download processes, e.g. from the link:https://forum.plentymarkets.com/c/suche-biete/prozess-basar[process bazaar, window="_blank"]{nbsp}icon:external-link[] or from certain pages of the manual. Then you can import these processes into your own system.
