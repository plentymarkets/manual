...
The basic principle of event procedures is simple: Certain *events* that occur in your online shop trigger specific *procedures*.
...
| *Client (store)*
|Orders are filtered according to one or multiple clients and online shops. You can select *ALL* online shops at once.
...
*_Example_*: If a payment provider rejects the payment made by a customer during the order process in the online shop, you can use this event so that it is followed by a procedure such as automatically sending an email notification to the customer.
...
|This procedure notifies the market <<markets/real-de/real-setup#, Kaufland.de>> of the cancellation of an order.
...
You can optionally add a *Comment* that is inserted below the order positions.
...
|This procedure automatically generates a pro forma invoice as soon as the selected event takes place. +
...
|An order is sent to <<payment/payment-plugins/klarna#, Klarna>>.

| *Notify Klarna of changes to the order*
|A notification on changes on an order is sent to <<payment/payment-plugins/klarna#, Klarna>>.
...
|Generates a DHL return label via <<fulfilment/preparing-the-shipment#3200, DHL Retoure Online>> or <<fulfilment/preparing-the-shipment#3100, DHL Retoure with enclosed return labels>>. The generated return label is made available in the *Shipping centre* or in the *My account* area of the online shop.
