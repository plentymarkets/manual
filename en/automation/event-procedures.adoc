...
<<orders/managing-orders#860, Multi-credit note>> +
...
|The procedure generates an <<orders/order-documents/generating-an-entry-certificate-gelangensbestaetigung#, entry certificate>> (Gelangensbestätigung) for an order. +
Optionally, you can enter a *Comment* which will be added in the document underneath the order items.
...
|The procedure creates a <<markets/amazon/amazon-fba#140, redistribution of the Amazon FBA shipment>> from a reorder. +
