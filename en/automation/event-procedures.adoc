...
The basic principle of event procedures is simple: Certain *events* that occur in your online store trigger specific *procedures*.
...
| *Order weight*
|Orders are filtered according to the total weight of the variations included in the order. Use the operators greater than, less than or equal to as well as the measuring units g and kg to filter orders with a certain weight. It is only possible to insert whole numbers.
...
| *Shipped on*
|Orders are filtered according to the shipping date. The options *Not available*, *Available* or *In the future* are available.
...
|Changes the order date (entry date) to the current time at which this action is running. +
The date can only be changed if the order is not locked.
...
_Attention_: The options *Payment due date* and *Early payment discount* refer to the old logic. The old logic will be switched off on the *01.01.2021*. Therefore, we recommend switching to the old logic as soon as possible.
...
|This procedure automatically generates a multipurpose coupon as soon as the selected event takes place.
...
Individual procedures are available for: <<payment/payment-providers/upg#, UPG>>, <<payment/payment-providers/amazon-pay#, Amazon Pay>>, *ClickandBuy*, <<payment/payment-providers/paymorrow#, paymorrow>>, <<markets/plus-gartenxxl#, Netto eStores>>, <<payment/payment-providers/postpay#, POSTPAY>>, <<payment/payment-providers/worldpay#, Worldpay>>, <<payment/payment-providers/sage-pay#, SagePay>>, <<payment/payment-providers/klarna-checkout-uk#, Klarna UK>>.
...
Individual procedures are available for: <<payment/payment-providers/klarna#, Klarna>>, <<markets/amazon/amazon-setup#, Amazon>>, <<markets/plus-gartenxxl#, Netto eStores>>, <<payment/payment-providers/postpay#, POSTPAY>>, <<markets/shopgate#, Shopgate>>, <<markets/flubit#, Flubit>>, <<markets/idealo-checkout/idealo-setup#, idealo>>, <<markets/fruugo#, Fruugo>> and <<markets/cdiscount#, Cdiscount>>. +
...
*_Requirement:_* The shipping service provider allows to register returns.
...
. Select the option *Active* in the *Settings* area. +
