...
|<<payment/managing-payments#160, Partial payment>>
...
|<<payment/managing-payments#170, Overpay>>
...
|The procedure splits the orders according to <<item/settings/availabilities#, item availabilities>> into orders. Create *Availability groups* for the filtered orders. To do so, drag one or more of the availabilities to the right and create the first group. To create a second group, drag further availabilities to icon:plus-square[role="green"]. If needed, create more groups.
...
|The procedure uploads an invoice to Amazon.
...
|This procedure sends a shipping confirmation to Otto Direktversand.
...
|*_Note_*: The procedure is not usable any more because it refers to the old Klarna integration via Callisto and not to the current integration with the Klarna plugin! Due to technical reasons, the procedure is still listed. The procedures in the procedure group *Plugins* have priority. +
This procedure deletes an invoice with the payment method *Klarna* when an order is cancelled.
...
|This procedure informs the market <<markets/idealo-checkout/idealo-setup#, idealo>> about the cancellation of an order. Select a reason for the cancellation from the drop-down list (Customer revoke, Merchant decline).
...
|The procedure informs the market <<markets/real-de/real-setup#, real.de>> about the cancellation of an order. Select the reason for the cancellation from the drop-down list (Customer cancellation, Customer exchange, Incorrect item description, Incorrect shipping address, Customer has not received item, Item sold out, Incorrect price, Shipping address is undeliverable, Other reason).

|*Upload invoice to Kaufland.de*
|The procedure uploads an invoice to real.de.
...
_Note_ that not other warehouses are automatically assigned.
...
| *Send shipping confirmation to [payment provider / market]*
|The procedure sends a shipping confirmation to a market or a payment provider when items bought via this market or payment provider are shipped. The procedures are listed individually. Select the appropriate one. +
*_Note_*: In some cases procedures cannot be used anymore because they exclusively refer to the old integration with Callisto, for example Klarna. Due to technical reasons, the procedures are still listed. The procedures in the procedure group *Plugins* have priority.
...
| *Send credit note to [payment provider]*
|The procedures sends a credit note to a payment provider. The procedures are listed individually. Select the appropriate one. +
*_Note_*: In some cases procedures cannot be used anymore because they exclusively refer to the old integration with Callisto, for example Klarna. Due to technical reasons, the procedures are still listed. The procedures in the procedure group *Plugins* have priority.
...
| *Send reversal / refund to [payment provider]*
|A refund is sent to the payment provider. The procedures are listed individually. Select the appropriate one. +
*_Note_*: In some cases procedures cannot be used anymore because they exclusively refer to the old integration with Callisto, for example Klarna. Due to technical reasons, the procedures are still listed. The procedures in the procedure group *Plugins* have priority.

| *Send cancellation to [payment provider]*
|A cancellation is sent to the payment provider. The procedures are listed individually. Select the appropriate one. +
*_Note_*: In some cases procedures cannot be used anymore because they exclusively refer to the old integration with Callisto, for example Klarna. Due to technical reasons, the procedures are still listed. The procedures in the procedure group *Plugins* have priority.

|*Send invoice to paymorrow*
|*_Note_*: This procedure is no longer usable because it refers exclusively to the old Callisto integration. Due to technical reasons, the procedure is still listed. The procedures in the procedure group *Plugins* have priority. +
The procedure sends an invoice to paymorrow.
...
| *Send return to [payment provider / market]*
|The procedure sends a return to a market or a payment provider. The procedures are listed individually. Select the appropriate one. +
*_Note_*: In some cases procedures cannot be used anymore because they exclusively refer to the old integration with Callisto, for example Klarna. Due to technical reasons, the procedures are still listed. The procedures in the procedure group *Plugins* have priority.

|*Create a return at Amazon*
|The procedure creates a <<maerkte/amazon/amazon-einrichten#_editing_returns_for_amazon_orders, return at Amazon>>.
...
For the procedure *eBay return*, you can additionally choose between the settings *Accept return*, *Accept return without label*, *Reject return* or *Offer partial refund* from a dropdown-list.
...
| *Create card*
|The procedures adds a <<welcome/general-functions/boards#30, new card>> in the *plentymarkets logo (Start) » Boards* menu. +
...
| *Delete cards*
|The procedure <<welcome/general-functions/boards#40, deletes>> selected cards in the *plentymarkets logo (Start) » Boards* menu. +
Select the *Board* and the *Column(s)* whose cards should be deleted within the procedure.

| *Move cards*
|The procedure <<welcome/general-functions/boards#40, moves>> selected cards in the *plentymarkets logo (Start) » Boards* menu. +
...
|The procedure sends a cancellation inquiry to eBay Fullfillment via <<markets/ebay/ebay-fulfillment-by-orange-connex#, Orange Connex>>.
...
_Note_: This procedure can only be used in combination with interfaces or plugins of shipping service providers already set up. Using this procedure with the Amazon service *Amazon Prime* is not possible.
...
*Requirements*: The shipping provider allows registering returns.
