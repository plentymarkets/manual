...
|Enter the variation IDs of the variations according to which orders should be filtered. This way, all orders containing the variation(s) with this particular ID are filtered.

|*Item property available*
|This filter checks the order for items to which you have assigned a particular <<item/managing-items#3000, property>> in the *System » System settings » Settings » Properties » Configuration* menu. Within the filter, select one item property according to which the items are filtered.
...
|*Use order template*
|This procedure automatically uses a certain order template. Order templates are created in the menu *System » System settings » Orders » Order template*.
...
For further information refer to the <<payment/payment-providers#>> page of the manual.
