...
|The procedure informs the market <<markets/real-de/real-setup#, Kaufland.de>> about the cancellation of an order. Select the reason for the cancellation from the drop-down list (Customer cancellation, Customer exchange, Incorrect item description, Incorrect shipping address, Customer has not received item, Item sold out, Incorrect price, Shipping address is undeliverable, Other reason).

|*Upload invoice to Kaufland.de*
|The procedure uploads an invoice to <<markets/real-de/real-setup#, Kaufland.de>>
...
|*_Note_*: The procedure is not usable any more because it refers to the old Klarna integration via Callisto and not to the current integration with the Klarna plugin! Due to technical reasons, the procedure is still listed. The procedures in the procedure group *Plugins* have priority. +
A notification on changes on an order is sent to Klarna.
