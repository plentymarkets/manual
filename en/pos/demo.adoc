...
That’s why you can try the plentymarkets POS demo mode for free and with no strings attached. Check out the point of sale system today and see just how functional, user-friendly and efficient plentymarkets POS really is!
...
====
We’re happy to <<app/installation#400, recommend hardware for plentymarkets POS>>. In general, Android devices with OS version 4.2.2 and higher are supported. iOS devices with OS version 8 and higher are supported.
====
...
* <<pos/demo#1300, Sell and redeem gift cards>>
...
You can use plentymarkets POS on a tablet or a mobile phone. However, since the screens have different formats and sizes, some of the functions are arranged differently depending on the device.
...
Just like any other item, you can add gift cards to the POS shopping cart by <<pos/demo#600, scanning the barcode>>, by <<pos/demo#700, choosing it from the list of favourites>> or by <<pos/demo#800, manually entering its item number>>.

The demo mode includes a fictional gift card that you can try out. link:https://cdn02.plentymarkets.com/pmsbpnokwu6a/frontend/POS_Demo-Mode_data.pdf[Download the PDF file^] and scan the barcode for the example gift card. Alternatively, manually add the “gift card” item (ID 1076) to the shopping cart.
...
--
Customers can pay for their items with gift cards. Once you’ve <<pos/demo#500, entered all of the items>> that the customer wants to buy, your customer can use a gift card to pay for the items in part or in full.
...
[.instruction]
Accepting a gift card as a payment method:

. link:https://cdn02.plentymarkets.com/pmsbpnokwu6a/frontend/POS_Demo-Mode_data.pdf[Download the PDF file containing demo gift cards^].
. Open the PDF file and look for gift cards of the type “multi-purpose”.
. Scan the barcode on the gift card. +
→ The total to be paid is updated dynamically.
. If the gift card doesn’t cover the entire amount, then accept payment for the remaining amount. +
→ The order is created. +
→ A receipt is printed. The demo mode simulates printing.
...
You can enter the gift card code manually instead of scanning the barcode. Tap on the gift symbol (icon:gift[role="blue"]) and manually enter the gift card code into the field.
...
plentymarkets POS allows you to cancel orders that were created _since the last Z report_.
...
Since the order was created <<pos/demo#2230, before the last Z report>>, it can no longer be cancelled. The order needs to be _returned_ instead.
...
.Your customer brought the receipt
[width="100%"]
[cols="2,2"]
|====
...
. Tap on icon:money[role="green"] *Complete and pay out cash* or on icon:gift[role="green"] *Complete and create gift card*.
...
.Your customer did not bring the receipt
[width="100%"]
[cols="2,2"]
|====
...
Just like any other item, you can enter dummy items by <<pos/demo#600, scanning the barcode>>, by <<pos/demo#700, choosing it from the list of favourites>> or by <<pos/demo#800, manually entering its item number>>.
...
. Hand over the items.
. Tap on icon:cube[role="green", stack="arrow-right,green", stackPosition="lr"] *Book outgoing items*. +
...
Do you have further questions about plentymarkets POS or about our rates? We’re happy to assist you! Contact our ink:https://www.plentymarkets.co.uk/company/contact/[sales team^] any time.
