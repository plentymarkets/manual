...
====
Variations for which order properties have been saved cannot be sold via plentyPOS. Starting from version 1.11.5 of the plentymarkets app, an error message is displayed when you try adding such variations to the shopping cart at the POS. 
...
[IMPORTANT]
.Returns are cash or coupon only
====
For technical reasons, the return amount can only be paid out in cash or as a coupon.
====
