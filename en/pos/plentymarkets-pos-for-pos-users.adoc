...
:author: team-mobile
:keywords: plentymarkets POS, plentyPOS, POS, point of sale, integrate POS, setup POS, use POS, POS users, POS staff, POS order, POS order, receipt, x report, z report, cash up
:position: 20
...
<ul>
<li>The ISBN of the item or the variation.</li>
...
→ The menu opens.
. Tap on *Appearance*.
. Carry out the desired settings. Note <<table-POS-view>>.
. *Save* the settings.
...
Processing down payments is similar to <<#430, combining several payment methods>>. +
...
====
The following instruction describes how to handle down payments for orders that have been created directly at the POS. Handling down payments for different order referrers, such as <<pos/plentymarkets-pos-for-pos-users#450, Click & Collect>> is not possible.
====

Proceed as described below to receive a down payment and create an invoice for the remaining amount of the purchase.
...
*_Note:_* You cannot combine discounting individual order items with a discount on the complete POS shopping cart.

|**Add fixed discount**
|Enter the discount in percent and tap **OK** to offer a discount of a fixed amount on the order item. The price is changed only in this order. The item's price in the system remains unchanged. +
*_Note:_* You cannot combine discounting individual order items with a discount on the complete POS shopping cart.
...
//. Tap**Add percentage discount** or **Add fixed discount**, depending on whether you apply a percentage discount or a fixed discount. +
...
Many companies sell items in an online store as well as in a retail outlet. This allows customers to place an order in the online store and pick up the items they ordered in the retail outlet. They can either pay online or pay at the time of pickup. Thus, the following scenarios are possible:
...
*_Note:_* Some online orders can be paid for using gift cards in app version 1.9.2\. and higher. +
...
====
Customers cannot return gift cards. Thus, gift cards are shown in grey in the return view.
====
...
. *_Optional:_* Select a different target storage location for the item: +
  ...
  → The available storage locations are shown in the following order:
    ...
    2. Default repair storage location = The *repair storage location* selected in the *Basic settings* tab of the POS.
...
→ A credit note is created in the plentymarkets back end and linked to the return. +
→ If you opted for a gift card, a new order for the amount of the gift card is created.
. Pay out the refund amount in cash or hand out the gift card.
////
