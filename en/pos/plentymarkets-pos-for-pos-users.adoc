...
:author: team-mobile
:keywords: plentymarkets POS, plentyPOS, plenty POS, POS, point of sale, integrate POS, setup POS, use POS, POS users, POS staff, POS order, POS order, receipt, x report, z report, cash up, loyalty card, loyalty cards, barcode card, barcode cards
:position: 20
...
. Enter the login details. Pay attention to the explanations given in <<table-POS-users-login>>.
. Tap **Login**. +
...
[IMPORTANT]
.Variations with order properties
====
Variations for which order properties have been saved cannot be sold via plentyPOS. Starting from version 1.11.7 of the plentymarkets app, an error message is displayed when you try adding such variations to the shopping cart at the POS. 
Order properties will be available in a future version of the plentymarkets app. Pay attention to our link:https://forum.plentymarkets.com/c/changelog[changelog^] to keep track of the latest updates.
====
...
After you have added all items to the POS shopping cart and applied any discounts or multipurpose coupons, you complete the order by booking the payment. Customers can pay cash. Depending on the setup in your store, they may also be able to pay by card using a POS terminal and/or a coupon.
...
From version 1.8.5 of the plentymarkets app, customers can pay for the same order using more than one payment method. This allows customers who for example do not have enough cash on them to pay part of the order amount in cash and the rest by card.
...
*_Note:_* The coupon code is printed both on the coupon receipt and the POS order receipt. As such, customers should keep both receipts to prevent others from redeeming the coupon.
...
**__Tip:__** If you select a quantity of more than one coupon, the coupon codes are generated automatically. +
→ The **Select gift card** window is displayed.
...
**__Note:__** Because the value of a coupon can only be redeemed once, each coupon code must be sold once only.
...
. Add the coupon to the POS shopping cart.
. Select the quantity 1. +
**__Tip:__** Coupon codes are generated automatically if you select a quantity of more than one coupon. +
→ The **Select gift card** window is displayed.
. Tap on the **Gift card code** field and enter the code.
. Tap **Generate**. +
...
You can select a customer data record for ringing up customers by scanning loyalty cards with barcodes printed on them. For more information on how to order barcode cards, refer to the <<pos/integrating-plentymarkets-pos#990, Integrating plentyPOS>> manual page.

As soon as you have ordered loyalty cards with barcodes printed on them, you do not need to select existing customers using the <<#135, customer search>>. Instead, you scan the barcode on the loyalty card after opening the POS shopping card. The contact saved in the barcode is then selected automatically and assigned tothe order.
Following this step, you can proceed by <<#40, adding items to the POS shopping cart>> and finishing the order as usual.
...
Many companies sell items in an online shop as well as in a retail outlet. This allows customers to place an order in the online shop and pick up the items they ordered in the retail outlet. They can either pay online or pay at the time of pickup. Thus, the following scenarios are possible:
...
* Orders can be payed from version 1.92 and higher using multipurpose coupons. However, coupons can only be used if no payment has been made for the order and no invoice document has been created yet.
...
[IMPORTANT]
.Return refunds via card terminals available in app version 1.11.7 or higher 
====
When working with the following instructions, note that refunds for return via card terminal is only possible in version 1.11.7 (or higher) of the plentymarkets app.
====
...
From version 1.11.7 of the plentymarkets app, you can not only pay out refunds for returns as coupons or cash, but also using a card terminal integrated at your plentyPOS. This way, the refund amount is paid back to your customer's debit card. This refund method is possible for returns <<#370, with>> and <<#380, without>> linking it to the main order.
...
→ The order data is displayed.
. *_Optional:_* Change <<pos/plentymarkets-pos-for-pos-users#140, the item prices>>.
. *_Optional:_* Add a <<pos/plentymarkets-pos-for-pos-users#100, note to the return>>.
. After you have added all returned items, tap the green arrow at the bottom of the POS shopping cart. +
→ The total amount of the return is shown. 
