...
:author: team-mobile
:keywords: plentymarkets POS, point of sale, plentyPOS, POS, plentymarkets point of sale, plentyPOS, POS user, POS users, cashier, till, installing POS, install POS, use POS, using POS, receipt, receipts, till count, till count discrepancy, ring up, ringing up, POS order, POS orders, Z report, Z2 report
:position: 20
...
////
Retourenartikel manuell erfassen. Die Möglichkeit, dass keine Auftragspositionen standardmäßig angezeigt werden, wurde zwar entwickelt, aber nie gemerged. Es weiß scheinbar keiner warum.
Diese Stelle erstmal auskommentiert lassen, bis entschieden wird, diese Option zu mergen. Falls die Option wieder rein kommt, dann auch die Einstellung auf der Seite POS einrichten, wieder einbauen. Da in die Grundeinstellungen der Kasse, Bereich Bedienung.
https://github.com/plentymarkets/ui-ui/pull/2085/files
...
* **link:https://apps.apple.com/gb/app/plentymarkets/id957702618[plentymarkets App in iTunes^]**
...
|Language
|Select the language in which you want to use plentyPOS. Currently, the user interface of plentyPOS is available in English or German.
...
→ The menu opens.
. Tap *View*.
. Carry out the settings. Note <<table-POS-view>>.
. *Save* the settings.
...
After you have added all items to the POS shopping cart and applied any gift cards and/or discounts, you complete the order by booking the payment. Customers can pay cash. Depending on the setup in your store, they may also be able to pay by card using a POS terminal and/or a gift card.
...
Each gift card has a unique gift card code. You enter this gift card code during the checkout process to apply the gift card to the items the customer wants to buy. A gift card code can only be redeemed if the code already exists in the plentymarkets system when the customer tries to redeem the gift card. In app versions 1.8.1 and higher, you can also redeem more than one gift card per order. When the order is created, the gift cards are redeemed in the order in which you enter the gift card codes. If you remove a gift card code before you create the order, the balance the customer needs to pay is recalculated.
...
. *_Optional:_* Redeem additional gift cards by entering their codes the same way. +
*_Note:_* The gift cards are redeemed in the order in which you enter the gift card codes. If you remove a gift card code, the order total is recalculated.
. Tap *Back*.
. Finish processing the POS order as usual. +
...
. Finish processing the POS order as usual. +
→ If you work with gift card receipts, a gift card receipt is created if the value of the gift card is not redeemed in full. Hand out the gift card receipt together with the sales receipt. +
*_Tip:_* Whether or not a gift card receipt is printed depends on the POS settings.

[#150]
==== Selling items on account

If the plugin *plentyPOS Invoice* is provisioned in the plentymarkets back end, you can also sell items on account to existing customers. But you cannot save new customer details using plentyPOS. Only existing customers can pay for items on account.
...
In plentymarkets app version 1.8.5 and higher, customers can pay for the same order using more than one payment method. This allows customers who for example do not have enough cash on them to pay part of the order amount in cash and the rest by card.
...
Sometimes it’s necessary to find previous transactions and continue working with them. For example, It makes sense to find the original transaction when customers <<pos/plentymarkets-pos-for-pos-users#175, return>> items or when they <<pos/plentymarkets-pos-for-pos-users#450, buy items online and pick them up in the store>>.
...
Enter the order ID or the receipt number into the search field. Tap *Search* to access the order data. Now you can <<pos/plentymarkets-pos-for-pos-users#370, finalise the return>> or hand over the <<pos/plentymarkets-pos-for-pos-users#450, items that were purchased online>>.
...
Scan the barcode on the receipt. Now you can <<pos/plentymarkets-pos-for-pos-users#370, finalise the return>> or hand over the <<pos/plentymarkets-pos-for-pos-users#450, items that were purchased online>>.
...
Each gift card has a unique gift card code. This gift card code is shown on the receipt when you sell a gift card. A gift card code can only be redeemed if the code already exists in the plentymarkets system when the customer tries to redeem the gift card. Depending on the workflow in your store, you either generate an automatic code when you sell a gift card or you receive a list of gift card codes from your employer. The codes of this list are either already saved in the system or you need to save them when you sell the gift card. Your employer will advise you on your store's workflow for selling gift cards.
...
. Hand over the receipt.
. If you also print gift card receipts, hand over the gift card receipt as well. +
...
|**Add fixed discount**
|Enter a fixed amount and tap **OK** to offer this much of a discount on the order item. The price is changed only in this order. The item's price in the system remains unchanged. +
*_Note:_* You cannot combine discounting individual order items with a discount on the complete POS shopping card.
...
//. Tap **Percentage** or **Fixed** depending whether the discount should be understood as a percentage or as a fixed amount. +
...
====
If the discount is applied to multiple items, then the total discount will be split up among all of the <<item/managing-items#40, discountable items>>. An item _can not be discounted_ if the option *Promotional coupons/POS discount* was set to *Not permitted* in the item's *Global* tab.
...
====
You cannot apply a discount to the complete POS shopping cart if a discount was applied to one or more items that are already in the POS shopping cart.
====

[#450]
=== Completing online orders at the POS (version 1.9 and higher)
...
* You can only book payments and save order notes. All other options for editing online orders are disabled.
* Orders can be paid for using gift cards in app version 1.9.2 and higher. However, gift cards can only be used if no payment has been made for the order and no invoice document has been created yet.
...
[#470]
==== Booking a payment for an online order (version 1.9 and higher)
...
*_Note:_* Some orders can be paid for using gift cards in app version 1.9.2 and higher. +
...
Go to **POS » Cancel order** to cancel orders created for this POS since the last Z report. Proceed as described below to cancel a POS order.
...
[IMPORTANT]
.Returns are cash or gift card only
====
For technical reasons, the return amount can only be paid out in cash or as a gift card.
====
...
//However, how you do this depends on the POS settings. Depending on the POS settings, when you open the order, either all order items are already selected for return or no items are selected. This means that the return shopping cart is either full or empty when the order opens:
...
*_Tip:_* All order items are greyed out when you open the order. A green plus is shown to the right of the order item. Tap this green plus to add those order items that the customer wants to return.
...
*_Tip:_* If the customer wants to return all order items of the order, just tap *Create return* straight away.
...
*_Tip:_* The invoice amount the customer still needs to pay is shown on the credit note receipt.
|The customer does not receive any money. +
...
* Once the signature unit is functional again, a collective receipt is created and printed in plentyPOS automatically.
