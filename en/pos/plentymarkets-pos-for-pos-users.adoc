...
====
If the discount is applied to multiple items, the total discount will be split up among all <<item/managing-items#40, discountable items>>. An item cannot be discounted if the option *Promotional coupons/POS discount* was set to *Not permitted* in the item’s *Global* tab.
...
[IMPORTANT]
.Return refunds via card terminals available in app version 1.11.5 or higher 
====
When working with the following instructions, note that refunds for return via card terminal is only possible in version 1.11.5 (or higher) of the plentymarkets app.
====
