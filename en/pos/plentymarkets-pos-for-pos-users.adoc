...
//  p>Noch beschreiben:</p> <ul> <li>Artikel oder Variante???</li> <li>Wie setzt sich die Belegnummer zusammen?</li> <li>Preis bearbeiten</li> <li>Dummy-Artikel, wenn Artikel nicht gefunden wird</li> <li>Favoriten in der App anlegen</li> <li>Umbuchen</li> <li>Warenbestand prüfen</li> <li>(Kasse sperren)</li> <li>(Artikel synchronisieren)</li> <li>(Aufträge hochladen)</li> </ul
toc::[]
[#5]
...
//  h2>Barbestand prüfen<a id="20." name="20."></a></h2> <p>Nachdem Sie sich bei plentymarkets POS angemeldet haben, werden Sie aufgefordert, den aktuellen Barbestand zu prüfen und zu bestätigen. </p> <h5>Aktuellen Barbestand prüfen:</h5> <ol> <li>Melden Sie sich bei der plentymarkets App an.</li> <li>Tippen Sie in der Menüleiste auf das <strong>Menüsymbol</strong>.</li> <li>Tippen Sie auf <strong>POS beta</strong>.<br /> → Der Soll-Kassenbestand wird angezeigt.</li> <li>Prüfen Sie, ob der tatsächliche Kassenbestand mit dem angezeigten Soll-Kassenbestand übereinstimmt.</li> <li>Wenn die Kassenbestände übereinstimmen, tippen Sie auf das Häkchen.</li> <li>Wenn die Kassenbestände nicht übereinstimmen, geben Sie den tatsächlichen Kassenbestand ein. Tippen Sie dann auf das Häkchen.<br /> → Der aktuelle Kassenbestand wird gespeichert und der Warenkorb geöffnet.</li> </ol
[#30]
== Selling items
...
Once you have added all items of a customer to the POS shopping cart and applied coupons and/or discounts, you complete the order by accepting payment. You can currently accept payments in cash or via a POS terminal.

[NOTE]
.Cancelling an order
====
You can cancel a POS order directly after you accepted and booked the payment. To do so, tap **Cancel order** in the **Order created** view. Go to <<pos/plentymarkets-pos-for-pos-users#173., **POS beta » Cancel order** >> to cancel orders created for this POS since the last Z report.
====
...
====
You can email receipts to customers automatically when a receipt is generated. To do so, you need to <<pos/integrating-plentymarkets-pos#70., **create an event procedure** >> in the plentymarkets back end. Note that for POS orders created in the offline mode, the event procedure will only take effect once the POS order is uploaded.
====
...
Before completing the order, you can redeem coupons or grant a discount on the complete shopping cart amount.
...
→ The cancellation receipt is imported to plentymarkets and saved in the **Orders » Document archive** menu. +
→ The status of the order changes to **[8] Canceled**.
...
. Open the **POS beta** menu. Tap on **Return**. +
→ The Returns view is displayed.
. Add the items to be returned by scanning them or using the item search. +
→ If the price paid differs from the current price of the item, <<pos/plentymarkets-pos-for-pos-users#140., **adjust the price of the item** >>.
. Optionally, add a note to the return. [.collapseBox]
.Adding a note
--
. Tap the pin. +
...
. Tap **Withdrawal**. +
to book the withdrawal. +
→ The current cash on hand is updated. +
→ A receipt for the withdrawal is printed. → The receipt is imported to plentymarkets and saved in the **Orders » Document archive** menu.
...
Use X reports to get an overview of the revenue generated for this POS since the last Z report during the day. You can create any number of X reports. X reports are not imported into plentymarkets, i.e. their creation is not documented.
