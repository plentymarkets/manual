...
====
You can email receipts to customers automatically when a receipt is generated. To do so, you need to <<pos/integrating-plentymarkets-pos#70, create an event procedure>> in the plentymarkets back end. Note that for POS orders completed in the <<pos/integrating-plentymarkets-pos#1100, offline mode>>, the receipt will only be sent after the POS orders have been uploaded.
====
...
Go to *POS » Cancel order* to cancel orders created for this POS since the last Z report. Since the data is saved in a cache, the cancellation is also possible if POS has been operated in the <<pos/integrating-plentymarkets-pos#100, offline mode>> in the meantime. Proceed as described below to cancel a POS order.
...
*_Note:_* If you carried out the cancellation while plentyPOS was in the <<pos/integrating-plentymarkets-pos#1100, offline mode>>, the cancellation is only visible in the plentymarkets back end after the data has been synchronised. This process is carried out automatically in the background as soon as the internet connection is re-established.
...
During your daily business, certain situations might require that you need to print an additional copy of a receipt. plentyPOS does not offer an option to reprint a receipt via the regular receipt printer. However, you can search for the receipt in the <<orders/document-archive#, Document archive>> in the plentymarkets back end to print it from there.

[IMPORTANT]
.Information on the printing process
====
If you print a receipt from the document archive, the receipt will be downloaded to your computer as a PDF file. After downloading it, you can print the receipt using a regular printer, but *not* on the receipt printer that is connected to your plentyPOS. Thus, make sure that you have <<automation/processes/printer#, set up an appropriate printer that is connected to your computer with plentyBase>>.
====
...
. Select <<orders/document-archive#100, additional filters>> for the search, if needed.
