...
| *Default customer*
|Select the <<pos/integrating-plentymarkets-pos#90, default customer>>, which you created for POS orders and which you want to assign to orders generated with this POS.
...
Activated (default setting) = Items are booked out automatically. The *Book outgoing items* button is not shown to POS users. +
...
| *Outgoing items of external orders: Book out items from POS sales warehouse* +
(app version 1.9.2 and higher)
...
//| *Return with main order: Manually select order items for return*
...
//*_Tip:_*_* Activate this option if your POS does not have a scanner and if your orders typically contain a large number of order items.
...
| *TSE public key*
| After successfully linking a technical security unit with the POS, the public key is added in this field automatically.
...
| *Order item details*
|Select one of the options *Variation ID*, *Variation number*, *Attributes* or *External variation ID* to display additional item information on the receipts generated by POS. 
...
* VAT rate, VAT amount and net revenue
...
**Tip:**__ The external name is printed on the receipt in the language that the POS user selects when logging into the POS.
...
| **Country**
|Select the country in which the POS is to be used from the dropdown list. Default setting is based on the option **Where is the company based (for financial accounting purposes)** in the <<welcome/quick-start/quick-start-initial-settings#100, System » System settings » Settings » Master data>> menu.
...
* <<orders/coupons#15, Decide which items>> customers should be allowed to purchase with gift cards and coupons.
* <<orders/coupons#120, Create gift card items>> with 0% VAT, e.g. add the gift cards to the products that your business sells.
* <<orders/coupons#20, Create a campaign>> for your gift cards. Use the option *Coupon variety* to specifiy whether it is a gift card or a promotional coupon.
* <<orders/coupons#130, link your gift card items with the campaign>>.
...
* Online orders can only be paid for at the POS using coupons from app version 1.9.2 with multi-purpose coupons. However, multi-purpose coupons can only be used if no payment has been made for the order and no invoice document has been created yet.
...
* The option *External order: Book outgoing items automatically* of the POS settings allows you to specify if the order items are booked out of the warehouse automatically after the items have been paid for. For POS that have been created after March 05,2019, this option is activated by default. If you deactivate this option, explain to your POS users in what cases they need to manually book outgoing items.
...
| **Fiscalisation**
| Enter the IP address of the <<pos/pos-legal-compliance#500, technical security unit>> linked to the POS.
...
For further information about creating label templates and printing the labels in the item search of the plentymarkets app, refer to <<item/settings/barcodes#900, Setting up labels>>.
...
| *Posting Account*
|Enter a <<pos/integrating-plentymarkets-pos#950, booking account>> to display the POS transactions for that booking account.
...
→ The POS is deleted. +
*_Note_*: If the POS is located in Austria and the signature unit is activated, an <<pos/integrating-plentymarkets-pos#920, end receipt>> must be created in the app. This deletes the POS automatically.