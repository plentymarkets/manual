...
Check mark placed = Only variations are available in this POS for which the option <<item/managing-items#230, Active>> is activated in the **Settings** tab of the variation. +
...
The settings you need to create a user account for a POS user are described below. For general information about user accounts, refer to the <<business-decisions/user-accounts-access, User accounts and access>> page of the manual.
...
For POS users to edit prices, rename order items and/or give discounts, you need to assign the appropriate rights to POS users. Click <<business-decisions/user-accounts-access#, here>> to learn how to create roles and assign them to users. You also activate additional app functions for users.
...
**__Note:__**For this right to come into effect, open the menu *Setup » Settings » User » Rights » User* and activate the right *CRM » Address » Show*.
...
. Carry out the settings. Note <<tabelle-POS-zahlungsarten>>.
...
. Carry out the settings. Note <<tabelle-POS-versanddienstleister-selbstabholung>>.
. **Save** the settings.
...
. Carry out the settings. Note <<tabelle-POS-versandprofil-selbstabholung>>.
. **Save** the settings. +
...
. Click on **New**. +
→ The **New** tab opens.
. Carry out the settings according to <<tabelle-POS-verkaufspreis>>.
. **Save** the settings.
...
. Carry out the settings according to <<tabelle-POS-standardkunde>>.
. **Save** the settings.
...
* <<orders/coupons#15, Decide which items>> customers should be allowed to purchase with coupons.
* <<orders/coupons#create-coupon-item, Create coupon items>> with 0% VAT, e.g. add the coupons to the products that your business sells.
...
*_Tip_*: The name should contain the name of the POS and the coupon value.
...
**__Note:__** Do not change the discount value once the first coupon has been sold. +
...
Now, create one or more coupon items. Because the campaign ID is linked at the item level, you cannot use variations to create coupons. For further information about creating items, refer to the <<item/managing-items#, Managing items>> page of the manual. Only settings needed for creating coupon items are described here.
...
. Carry out the additional settings. Note <<tabelle-POS-mehrzweckgutscheine-artikeleinstellungen>>.
. **Save** the settings.
...
* Set up a <<pos/integrating-plentymarkets-pos#840, Coupon item with a flexible value>> for this coupon type in plentymarkets.
* Add this coupon item to the <<pos/integrating-plentymarkets-pos#60, favourites list>> of your POS.
* Order link:http://lmgtfy.com/?q=Barcode-Karten+Druck[barcode cards^]{nbsp}icon:external-link[] with pre-printed codes. Most providers of barcode cards expect you to send a CSV file containing the individual codes that should be placed on the cards. Also, you should provide them a layout template in Photoshop or Adobe Illustrator format for the frame design.
...
. **Save** the settings.
. Carry out the settings according to <<tabelle-POS-ereignisaktion-warenausgang-buchen>>.
...
. **Save** the settings.
. Carry out the settings according to <<tabelle-POS-ereignisaktion-beleg-senden>>.
. Activate the option **Active**.
. **Save** the settings.
--
...
. Select filters. Note <<tabelle-POS-kassenjournal-menue>>.
. Click on **Search**. +
