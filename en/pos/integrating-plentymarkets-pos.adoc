...
. Enter the POS user's email address. This allows the user to receive messages from the system.
...
. Go to *System » System settings » Settings » User » Rights » Users*. +
→ The overview opens.
. *_Optional:_* Enter an ID or a name in the filter settings to limit the results.
. Click on *Search*. +
→ The accounts are shown. +
*_Tip:_* Greyed out accounts have admin rights.
. Click on the account. +
→ The *Rights* view is shown.
. In the *Rights* area, activate the rights listed in <<table-POS-REST-rights>>.
. *Save* the settings.

[[table-POS-REST-rights]]
.Rights for variable accounts
[cols="1,3"]
|====
|Section |Right
...
|*Stock*
a|* *Warehouses » Storage location » Show storage location*

|*CRM » Campaign » Code* (optional)
a|* *Campaign » Code » Create*
* *Delete code* +
* *Show code*

*_Note:_* These rights are only required if you want to sell gift cards.
|====

[#160]
=== Activating POS visibility for POS users

Now, you define which functions and key figures are visible to variable users in the plentymarkets App.
...
. Go to **System » Settings » System settings » User » Accounts » Open user**.
...
|**Country**
|Select the country in which the POS is to be used from the dropdown list. Default setting is based on the option **Where is the company based (for financial accounting purposes)** in the <<quick-start/master-data#100, System » System settings » Settings » Master data>> menu.
...
* Assign rights to variable user accounts.
