...
(new POS with location in Austria only)
|Select to <<pos/integrating-plentymarkets-pos#900, create the start receipt>> (Startbeleg). This activates the tamper-proof signature unit for the POS. After the start receipt is created, the location country of the POS cannot be changed.

|**Create end receipt** +
(POS with location in Austria and activated signature unit only)
|Select to <<pos/integrating-plentymarkets-pos#920, create the end receipt>> (Endbeleg). This deactivates the tamper-proof signature unit for the POS. Once an end receipt has been created, the POS can no longer be operated.
|====
...
→ The POS is deleted. +
**__Note:__** If the POS is located in Austria and the signature unit is activated, an <<pos/implementing-plentymarkets-pos#920, end receipt must be created in the app>>. This deletes the POS automatically.
...
|See <<pos/integrating-plentymarkets-POS#50, Basic settings for PO>>

|3.
|Create the start receipt (Startbeleg)
|See <<pos/integrating-plentymarkets-pos#900, Creating the start receipt (Startbeleg)>>
