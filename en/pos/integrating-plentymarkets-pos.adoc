...
The mobile POS solution plentymarkets POS allows you to integrate your brick and mortar store seamlessly both with your online presence and your plentymarkets system. The point of sale system plentymarkets POS is an optional component of the plentymarkets app.
...
The point of sale system plentymarkets POS is an optional component of the plentymarkets app. However, there are additional hardware requirements for using plentymarkets POS. For further information about the system requirements for the plentymarkets app and plentymarkets POS, refer to the <<quick-start/quick-start-system-requirements#50, System requirements>> page of the manual.
...
* link:https://apps.apple.com/app/plentymarkets/id957702618[plentymarkets App in iTunes^]

== Supported currencies

plentymarkets POS can be operated in Euro (EUR), Swiss Franks (CHF) and Pound Sterling (GBP). However, you can only operate the POS in the <<payment/currencies#30, system currency>>. The system currency is set in the *Setup » Orders » Payment » Currencies* menu.
...
Once you have booked a POS, you configure these points of sale in the plentymarkets back end. First, you assign a POS to a client by creating and configuring a POS in the **Setup » Client » Select client » POS** menu. To do so, proceed as described below.
...
|**Header**
|If the component is selected, the field's content is displayed as a header on all receipts issued by the POS. By default, the header contains the information saved in the **Setup » Settings » Master data** menu. Make changes as needed.
...
. **Save** the settings.
. Go to **Setup » Client » Select client » POS » Open POS » Basic settings**.
...
|**Country**
|Select the country in which the POS is to be used from the dropdown list. Default setting is based on the option **Where is the company based (for financial accounting purposes)** in the <<quick-start/quick-start-initial-settings#100, Setup » Settings » Master data>> menu.
...
2+|*Setup » Settings » User » Accounts » Open account » Tab: Mobile » POS functions*
...
//In plentymarkets, gift cards can be understood as a product that customers can buy. If someone has a gift card, then they can redeem it in exchange for products or services.
...
//Even though gift cards and coupons are used for different things, they're both configured the same way in plentymarkets. Therefore, both topics are explained here in the same chapter.

//In order to offer gift cards and promotional coupons with plentymarkets POS, you'll need to complete the following prep work in the plentymarkets back end:
...
//* <<auftraege/gutscheine#20, Create a campaign>> for gift cards and coupons. Use the option *Coupon variety* to specify whether it's a gift card or a promotional coupon.
//* <<auftraege/gutscheine#130, Link the gift card items to the campaign>>.
//* *_Optional:_* <<pos/pos-einrichten#850, Generate codes>> or save your own codes in the back end.
//* *_Optional:_* Create gift card templates and make them available to POS users.
//* *_Optional:_* Make codes available to POS users.

== Preparing gift card sales

You can sell and redeem gift cards in plentymarkets POS gift cards. When you sell a gift card item, you provide customers with a gift card code. A gift card code can only be redeemed if it already exists in the plentymarkets back end when the customer tries to redeem it. You either can generate gift card codes automatically or work with your own gift card codes.
...
* Create a free text field for the campaign and link it in the *Setup » Orders » Coupons* menu.
...
Now, go to **Setup » Orders » Coupons**. Select this free text field and link the field to a coupon campaign. You only need to do this once for all campaigns.

[.instruction]
Selecting a free text field:

. Go to **Setup » Orders » Coupons**.
. Select the campaign field **Kampagne ID** from the drop-down list.
. **Save** the settings. +
...
To use additional printers, you need a printer that can be accessed via the network that is connected to one or more printers on which you want to print the POS receipts. On this computer, you install the plugin link:https://marketplace.plentymarkets.com/en/plugins/integration/plentyBase_5053[plentyBase^]. plentyBase establishes a connection between the plentymarkets app and the local computer. This allows the POS to access any printers that the local computer can also access. Go to the plentymarkets back end menu *Setup » Settings » Printers* to configure these printers.
...
Go to the plentymarkets back end menu *Setup » Settings » Printers* to configure printers.
...
*_Tip:_* The file name contains the export range, for example the file depExport9-1-2017-9-2-2017.json contains data from the 9th of January to the 9th of February 2017.
