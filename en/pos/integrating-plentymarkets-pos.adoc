...
[#10]
== Getting started with the plentymarkets app
...
To use plentyPOS, you need to link:https://www.plentymarkets.co.uk/prices/[book one or more POS in plentymarkets^]. Visit our link:https://www.plentymarkets.co.uk/product/pos-product/[plentyPOS product page^]{nbsp}icon:external-link[] to get further information on prices and terms and conditions.
...
First, create a POS for the desired client (store) in the plentymarkets back end.
...
[#50]
=== Carrying out basic POS settings
...
| *Referrer*
|When you book a POS, a new referrer is generated automatically. This allows you to clearly identify orders generated with this POS. The POS is assigned to an available POS referrer (103.01 to 103.99) automatically.

| *Default customer*
|Select the <<pos/integrating-plentymarkets-pos#90, default customer>>, which you created for POS orders and to which you want to assign orders generated with this POS.
...
*_Note:_* In Germany and other countries, saving the address is a legal requirement. Your tax consultant can advise you on the correct location details. +
...
*_Note:_* In the POS shopping cart, items with no net stock are shown in red but can be sold. Thus, mathematical overselling can occur.
...
Activated (default setting) = Items are booked out automatically. The *Book outgoing items* button is not displayed to POS users. +
...
Not activated = The numpad quick buttons are not displayed.
Activated = The following numpad quick buttons are displayed as an additional row of the POS’s number pad:
...
* *Exact cash amount* = If customers pay the exact amount in cash, POS users can tap this button instead of entering the amount manually. The *Received* field is filled with the amount due.
...
//Not activated (default setting). All order items are selected for return when you open the order. The POS user removes those items from the return that the customer does not want to return. +
//Activated = No order items are selected for return when the order is opened. The POS user adds those items to the return that the customer wants to return. +
...
|Choose whether you’d like to see the expected cash on hand while creating a <<pos/plentymarkets-pos-for-pos-users#230, Z report>> or a <<pos/plentymarkets-pos-for-pos-users#450b, till count report>>. This option is deactivated by default. In other words, POS users _do not_ see how much money is expected to be in the till while they count the cash. It’s a good idea not to change this setting, as it ensures that POS users really count the cash themselves. If POS users see this information, they might be tempted to blindly accept the value rather than count themselves.
...
→ If the option *Automatic printing* is activated in the plentymarkets app under *plentymarkets app » Settings » POS*, the gift card receipt is printed after the POS receipt is printed. +
→ If the option *Automatic printing* is not activated in the plentymarkets app under *plentymarkets app » Settings » POS*, the POS user can print the gift card receipt manually after completing the order. +
...
| *Order item details*
|Activate one of the options *Variation ID*, *Variation number*, *Attributes* or *External variation ID* to display the additional information on the receipts generated by the POS.
 The information is displayed below the item name on the receipt. Activate the option *None* if no additional information should be displayed.

| *Barcode*
|Should barcodes be displayed on your receipts? +
In the event of <<pos/plentymarkets-pos-for-pos-users#175, returns>> or <<pos/plentymarkets-pos-for-pos-users#450, click and collect orders>>, customers typically bring their original receipts with them into your store. If a barcode was printed on the receipt, the POS user only has to <<pos/plentymarkets-pos-for-pos-users#520, scan the barcode>> to access the corresponding order. +
...
The settings you need to create a user account for a POS user are described below. For general information about user accounts, refer to the <<business-decisions/user-accounts-access#, Managing user accounts>> page of the manual.
...
To give new POS users access to plentyPOS, create a <<business-decisions/user-accounts-access#, user account>> of the user class **Backend** in the plentymarkets back end. To do so, proceed as described below.
...
[#150]
=== Activating rights for POS users
...
. In the *Rights* area, activate the rights listed in <<tabelle-POS-REST-rights>>.
. *Save* the settings.

[[table-POS-REST-rights]]
.Rights for Backend accounts
[cols="1,3"]
|====
|Area |Right
...
| *CRM*
a| * *Campaign » Code » Show code*
* *Campaign » Code » Create code*
* *Campaign » Code » Delete code*
...
For POS users to edit prices, rename order items and/or give discounts, you need to assign the appropriate rights to POS users. Find out how to create roles and assign them to users <<business-decisions/user-accounts-access#, here>>. You also activate additional app functions for users.
...
**__Note:__** For this right to come into effect, open the menu *Setup » Settings » User » Rights » User* and activate the right *CRM » Address » Show address*.
...
If you offer <<pos/integrating-plentymarkets-pos#4100, Click & Collect>> in your online store, you also have to activate the order referrers **Manual entry**, **Client (store)**, and, if used, the specific order referrer for the market used for processing Click & Collect orders.
====
...
. Carry out the settings. Note <<table-POS-payment-methods>>.
...
| **Info page**
|Only change the default setting **---** if the payment method is also used in the online store.
...
| **Logo**
|Only change the default setting **Show standard logo** if the payment method is also used in the online store.
...
* link:https://marketplace.plentymarkets.com/en/plugins/payment/sumup_5141[SumUp Air^]{nbsp}icon:external-link[]
...
Check the user guide of the plugin for details on how to use these plugins in plentyPOS.
...
Check the user guide of the plugin for details on how to use this plugin in plentyPOS.
...
. Go to **Setup » Orders » Shipping » Tab: Shipping service provider**.
. Click on **New**. +
→ A new line is added at the bottom of the overview.
. Carry out the settings. Note <<table-POS-shipping-provider-pickup>>.
. **Save** the settings.
...
. Carry out the settings. Note <<table-POS-shipping-profile-pickup>>.
. **Save** the settings. +
...
| **Client**
|Activate the clients (stores) to be linked to this sales price.
...
| **Company**
|Enter a company name, for example plentyPOS.
...
| **Country**
|Select the country in which the POS is to be used from the drop-down list. Default setting is based on the option **Where is the company based (for financial accounting purposes)** in the <<quick-start/quick-start-initial-settings#100, System » System settings » Settings » Master data>> menu.
...
2+| *System » Settings » User » Accounts » Open account » Tab: Mobile » POS functions*
...
In plentymarkets, _gift cards_ are treated as a product that customers can buy. If someone has a gift card, then they can redeem it in exchange for products or services.
...
Even though gift cards and coupons are used for different things, they are both configured the same way in plentymarkets. Therefore, both topics are explained here in the same chapter.

The following preparations in the plentymarkets back end are necessary to sell gift cards via plentyPOS:

* <<auftraege/gutscheine#15, Decide which items>> customers should be allowed to purchase with gift cards and coupons.
* <<auftraege/gutscheine#120#, Create gift card items>> with 0% VAT, e.g. add the gift cards to the products that your business sells.
* <<auftraege/gutscheine#20, Create a campaign>> for gift cards and coupons. Use the option *Coupon variety* to specifiy whether it is a gift card or a promotional coupon.
* <<auftraege/gutscheine#130, Link the gift card to the campaign>>.
* *_Optional:_* <<pos/pos-einrichten#850, Generate codes>> or save your own coupon codes in the back end.
...
Now, create one or several campaigns for selling gift cards in plentyPOS. You have the following options:

* You create one campaign for all gift cards you sell in plentyPOS.
...
*_Tip:_* The name should contain the name of the POS and the gift card value.
...
To redeem gift cards in plentyPOS, POS users enter the gift card code. A gift card code must be present in the plentymarkets back end at the time it is redeemed.
...
. Enter a name. +
*_Tip:_* The name should include the gift card value.
...
Selling gift cards with varying values directly at the POS is a simple and practical method when dealing with coupons. This allows your POS users to sell gift cards with the indivudual values desired by customers directly at the POS.
...
* Set up a <<pos/integrating-plentymarkets-pos#840, Gift card item with a flexible value>> for this gift card type in plentymarkets.
* Add this gift card item to the <<pos/integrating-plentymarkets-pos#60, favourites list>> of your POS.
* Order link:http://lmgtfy.com/?q=Barcode-Karten+Druck[barcode cards^]{nbsp}icon:external-link[] with pre-printed default codes. Most providers of barcode cards expect you to send a CSV file containing the individual codes that should be placed on the cards. Also, you should provide them a layout template in Photoshop or Adobe Illustrator format for the frame design.
...
The following video tutorial provides an overview of the individual steps of selling gift cards.

.Selling gift cards
video::382590843[vimeo]

You want to know how the sold gift cards are redeemed? Take a look at the video tutorial liked below for more detailed information.
...
Users with admin rights are able to create gift cards by default. *Backend* users must be assigned this right.
...
. Click on the desired account. +
→ The view *Rights* opens.
. In the *Rights* area, activate the following rights:
	** *Coupon » Campaign » Code » Create code*
	** *Coupon » Campaign » Code » Delete code*
	** *Coupon » Campaign » Code » Show code*
. *Save* the settings.
...
* By default, POS users are shown the storage location that in the *Basic settings* tab of the POS is set up as the *Default repair storage location*. POS users can select a different storage location for returned items. The possible storage locations are shown in the following order:
  ...
  .. Default repair storage location = The *repair storage location* selected in the *Basic settings* tab of the POS.
...
* Create gift card items with a VAT rate of 0% and link it to the gift card campaign.
...
Create a campaign for gift cards for refunds.
...
. *Save* the settings. +
→ The item is created and opened for editing.
. Carry out the additional settings according to <<table-POS-refunds-gift-cards-item-settings>>.
. *Save* the settings.
...
Users with admin rights are able to create refund gift cards by default. *Backend* users must be assigned this right.
...
[#4100]
== Managing online orders (Click & Collect)
...
* The option *External order: Book outgoing items automatically* of the POS settings allows you to specify if the order items are booked out of the warehouse automatically after the items have been paid for. This option is activated by default for POS created on or after the 05th of March 2019. If you deactivate this option, explain to your POS users in what cases they need to manually book outgoing items.
...
Proceed as described below to create a <<fulfilment/preparing-the-shipment#800, shipping service provider>> for store pickup. In this example, we call this shipping service provider *Store pickup*. Of course, you can also use a different name.
...
In the next step, create a <<fulfilment/preparing-the-shipment#1000, shipping profile>> for store pickup. In this example, we call this shipping profile *Store pickup*. Of course, you can also use a different name.
...
* Provide the plugin link:https://marketplace.plentymarkets.com/en/plugins/payment/payuponpickup_4757[Pay upon pickup]{nbsp}icon:external-link[] for the client. +
...
. If items are to be booked out of the sales warehouse of the POS, activate the option *Outgoing items for external orders: Book items out of POS sales warehouse: Book items out of POS sales warehouse*. +
...
. **Save** the settings.
. Carry out the settings according to <<table-POS-event-procedure-outgoing-items>>.
. Place a check mark next to the option **Active**.
. **Save** the settings.
--
...
. **Save** the settings.
. Carry out the settings according to <<table-POS-event-procedure-send-receipt>>.
. Place a check mark next to the option **Active**.
. **Save** the settings.
--
...
To use additional printers, you need a printer that can be accessed via the network that is connected to one or more printers on which you want to print the POS receipts. On this computer, you install the plugin link:https://marketplace.plentymarkets.com/en/plugins/integration/plentyBase_5053[plentyBase^]{nbsp}icon:external-link[]. plentyBase establishes a connection between the plentymarkets app and the local computer. This allows the POS to access any printers that the local computer can also access. Go to the plentymarkets back end menu *System » System settings » Settings » Printers* to configure these printers.
...
→ The plugin description teaches you how to configure link:https://marketplace.plentymarkets.com/en/plugins/integration/plentyBase_5053[plentyBase^]{nbsp}icon:external-link[]. +
...
→ Find out how to save the plentyBase connection data in the plugin description of the plugin link:https://marketplace.plentymarkets.com/en/plugins/integration/plentyBase_5053[plentyBase^]{nbsp}icon:external-link[].
...
*_Recommendation:_* Configure plentyBase to run automatically when the computer is started up.
* Has the printer been <<automation/processes/printer#, saved and configured>> in the plentymarkets back end?
