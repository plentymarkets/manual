...
<<tabelle-POS-menueleiste>> describes the menu bar of a POS.
...
. Carry out the desired settings. Note <<tabelle-POS-belegvorlage>>.
. **Save** the settings. +
...
| **Country**
|Select the country in which the POS is to be used from the dropdown list. Default setting is based on the option **Where is the company based (for financial accounting purposes)** in the <<quick-start/quick-start-initial-settings#100, System » System settings » Settings » Master data>> menu.
...
. Open the plentymarkets app.
. Go to *plentymarkets App » Settings » POS*.
. Configure the settings in the *Printers* area. Note <<tabelle-POS-drucker>>.
. **Save** the settings.
