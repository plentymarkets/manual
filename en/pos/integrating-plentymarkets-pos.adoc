...
====
.Book a POS
To use plentymarkets POS, you need to link:https://www.plentymarkets.co.uk/prices/[book one or more POS in plentymarkets^]. Visit our link:https://www.plentymarkets.co.uk/product/pos-product/[plentyPOS product page^]{nbsp}icon:external-link[] to get further information on prices and terms and conditions.
====
...
. Click on **Save** (icon:save[role="darkGrey"]). +
...
In case of a <<pos/plentymarkets-pos-for-pos-users#175, return>> or a <<pos/pos-online-orders#, Click & Collect order>>, customers usually bring their receipts to the store. If a barcode was printed on the receipt, the POS user only has to <<pos/plentymarkets-pos-for-pos-users#520, scan the barcode>> to access the corresponding order. +
...
| *Stocks*
a| * *Warehouse » Storage location » Show*
...
. Carry out the settings. Pay attention to the explanations given in <<table-POS-payment-methods>>.
...
| **Name**
|Enter the external name that should be displayed on the POS receipt, in the online shop and on invoices. If no name is saved, the back end name of the payment method is displayed on the POS receipt. +
*_Tip:_*The external name is printed on the receipt in the language that the POS user selects when logging into the POS.

| **Info page**
|Only change the default setting **---** if the payment method is also used in an online shop.

| **Countries of delivery**
|Select one or more countries of delivery. +
*_Note:_* Only select countries of delivery if the payment method is also used in an online shop.

| **Client**
|Select the client of the POS.

| **Logo**
|Only change the default setting **Show standard logo** if the payment method is also used in an online shop.
...
. **Save** (icon:save[role="darkGrey"]) the settings of the **Table of shipping charges** tab without making changes.
...
| **Clients (stores)**
|This option is only displayed if at least one client is set up. Select client(s) or **ALL** if you want to use the shipping profile for all online shops.
...
| **External name EN**
|Enter the external name. The external name is displayed in the online shop. It is visible to customers.
...
**UVP** = Activate if the sales price is a recommended retail price (RRP). If the RRP and another sales price are linked with a variation, the RRP is displayed in the online shop as red strike-through text next to the sales price. +
...
. **Save** (icon:save[role="darkGrey"]) the data. +
...
. Carry out the additional settings. Note <<able-POS-multipurpose-coupon-item-settings>>.
. **Save** (icon:save[role="darkGrey"]) the settings.
...
* You can book returns for all orders saved in the system. As such, customers also can return order items they bought from other referrers, such as the online shop or a market. The POS staff decides if a customer can return an item. As such, you should inform your POS staff about any referrers that are excluded from POS returns.
...
* Assign rights to back end user accounts.
...
| *Search*
|Click on *Search* (icon:search[role="blue"]) without selecting any filter settings. This displays all POS transactions. +
...
→ The POS is deleted. +
*_Note:_* If the POS is located in Austria and the signature unit is activated, an <<pos/integrating-plentymarkets-pos#920, end receipt>> must be created in the app. This deletes the POS automatically.
