...
:author: team-mobile
:keywords: plentymarkets POS, plentyPOS, plenty POS, point of sale, POS system, points of sale, IDEA export, IDEA, GoBD, cash journal, POS journal, RKSV, DEP export, mobile POS, smartphone POS, plentyPOS, POS system, IDEA export, IDEA, GoBD, cash journal, mobile POS, smartphone POS, security device, POS Austria, POS legal, RKSV conformity, QR code, loyalty card, loyalty cards, barcode card, barcode cards, unlink POS, unlink plentyPOS, unlink device, unlink devices
:position: 10
...
| *Default customer*
|Select the <<#95, default customer>> which you created for POS orders and which you want to assign to orders generated with this POS.
...
Do you want to activate the shipping profile for a large number of items at the same time? To do so, use the <<item/import-export-create/create/mass-processing#100, item group function>>.
...
Now, create one or more coupon items. Because the campaign ID is linked at the item level, you cannot use variations to create coupons. For further information about creating items, refer to the <<item/import-export-create/create/new-item#, Creating new items>> page of the manual. Only settings needed for creating coupon items are described here.
...
(new POS with location in Austria only)
|Select to <<pos/pos-legal-compliance#1600, create the start receipt (Startbeleg)>>. This will activate the tamper-proof signature unit for the POS. After the start receipt is created, the location country of the POS cannot be changed.

| **Create end receipt** +
(POS with location in Austria and activated signature unit only)
|Select to <<pos/pos-legal-compliance#2300, create the end receipt (Endbeleg)>>. This will deactivate the tamper-proof signature unit for the POS. Once an end receipt has been created, the POS can no longer be operated.
|====
...
→ Refer to the <<automation/processes/installing-plentybase#, plentyBase>> manual page for instructions on how to set up plentyBase. +
...
→ To learn how to save plentyBase connection data in the app, refer to the <<automation/processes/installing-plentybase#, plentyBase>> manual page. 
...
For further information about creating label templates and printing the labels in the item search of the plentymarkets app, refer to the <<app/functions/item-search#, Searching for items, customers and orders>> manual page.

[IMPORTANT]
.Setting up plentyBase
====
...
→ The POS is deleted. +
*_Note:_* If the POS is located in Austria and the signature unit is activated, an <<pos/pos-legal-compliance#2300, end receipt>> must be created in the app. This deletes the POS automatically.
