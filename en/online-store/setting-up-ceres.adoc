...
:author: team-webshop
:keywords: online store, client, standard, Ceres, plugin, default country of delivery, default shipping method, default shipping profile, default payment method, multilingualism, Trusted Shops, EHI, position number, filter, template, contact page, item lists, item list, ShopBuilder, widget, widgets
:position: 10
:url: online-store/setting-up-ceres
:id: GW5QHSI

link:https://marketplace.plentymarkets.com/plugins/templates/Ceres_4697[Ceres]{nbsp}icon:external-link[] is the official template plugin for the default online store of plentymarkets. In the new plentymarkets online store, design and logic are separated from each other. The online store is integrated into your plentymarkets system via 2 plugins.
The *Ceres* plugin contains the standard design of the online store and can be customised to meet your needs. The link:https://marketplace.plentymarkets.com/plugins/templates/IO_4696[IO^]{nbsp}icon:external-link[] plugin contains the logic of the online store, is a general basis for all design plugins and can also be used by other plugins.
...
Set up the default shipping method for your online store. This shipping method will be used for calculating the shipping costs in the shopping cart before a customer registers in your online store. The default shipping method is preselected in the checkout. For further information about shipping methods and shipping profiles, refer to the <<fulfillment/preparing-the-shipment#1000, Creating a shipping profile>> manual chapter.
...
Set up the default payment method for your online store. Only link:https://marketplace.plentymarkets.com/plugins/payment[Payment method plugins^]{nbsp}icon:external-link[] can be used in *Ceres*. The payment method plugins will be available as default payment methods after the setup. Please refer to the corresponding *User guide* of the payment method plugin in the plentyMarketplace.
...
|Enter the Google reCAPTCHA Website key in order to integrate a Google reCAPTCHA into your contact form. You receive this key when registering your website for link:https://www.google.com/recaptcha/admin#list[Google reCAPTCHA^]{nbsp}icon:external-link[]. Note that you have to select a reCAPTCHA of the type *Hidden reCAPTCHA* during setup. The reCAPTCHA validates the visitor of your online store in the background and remains invisible for your customers. Please note that the Google reCAPTCHA necessitates the inclusion of a reference in your privacy policy.
...
|Activate currencies available to customers in the header of the online store. For further information about supported currencies, refer to the <<payment/currencies#10, Supported currencies>> manual page.
...
You can carry out settings pertaining to trailing slashes of the URLs of your online store in the *Setup » Client » Select client » SEO » URL structure* menu.
...
You can find all necessary information link:https://ehi-siegel.de/shopbetreiber/ehi-siegel/antrag-auf-shopzertifizierung/[here^]{nbsp}icon:external-link[].
...
*Ceres* is a public project and available on link:https://github.com/plentymarkets/plugin-ceres[GitHub]{nbsp}icon:external-link[]. You can use the plugin code to create your own template plugin. Please note the following when creating your own template on the basis of *Ceres*:

* Register with link:https://github.com/join?source=header-home[GitHub^]{nbsp}icon:external-link[]
* Open the link:https://github.com/plentymarkets/plugin-ceres[Ceres project^]{nbsp}icon:external-link[]
...
Theme plugins are an easy way of customising the looks of *Ceres*. Various link:https://marketplace.plentymarkets.com/plugins/themes[themes^]{nbsp}icon:external-link[] for *Ceres* are available on plentyMarketplace. Changes made to the theme plugin have no impact on future updates of the template plugin *Ceres*.

[#1300]
=== Creating an individual theme

Learn how to create your own theme plugin and how to fully customise *Ceres* in our link:https://developers.plentymarkets.com/dev-doc/theme-plugins[theme guide^]{nbsp}icon:external-link[].

[#1400]
=== Integrating additional plugins

In addition to theme plugins, which serve to modify the design of the online store, the link:https://marketplace.plentymarkets.com/[marketplace^] offers a multitude of different plugins which can be used to tailor the functionality of the online store to your personal needs. Apart from the integration of link:https://marketplace.plentymarkets.com/plugins/markets[marketplaces^], you can find link:https://marketplace.plentymarkets.com/plugins/widgets[widgets^], such as the GoogleAnalytics and customer feedback plugins, link:https://marketplace.plentymarkets.com/plugins/payment[payment plugins^] for the implementation of additional payment methods and link:https://marketplace.plentymarkets.com/plugins/integration[integration plugins]{nbsp}icon:external-link[], such as the plugins of shipping providers DHL and DPD. +
