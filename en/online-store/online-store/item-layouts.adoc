...
_*Tip*_: You can modify the error message text *Invalid selection* in the <<omni-channel/online-store/setting-up-clients/extras/language-packages#, System » System settings » Client » Settings » Languages>>. Open the drop-down list *Notes* and edit the note *No. 42*.
...
After making changes to the way that items are sorted on the first and second category level, go to *System » System settings » Item » Categories* and save any category once in order to regenerate the cache.
