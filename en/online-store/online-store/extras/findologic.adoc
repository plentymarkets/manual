...
:position: 140
:url: online-store/online-store/extras/findologic
:id: H57FRZB

*Setup » Client » Select client » Services » FINDOLOGIC*
...
link:http://www.findologic.com/pricing[Order the FINDOLOGIC software^]{nbsp}icon:external-link[] (the homepage is in German only).
...
====
Go to *Setup » Settings » User* and set up a new user account for FINDOLOGIC, which has *API* set as the user class.

For further information, refer to the link:https://www.plentymarkets.eu/handbuch/einstellungen/grundeinstellungen/benutzer/benutzer-anlegen/[Creating a user]{nbsp}icon:external-link[] page of the manual.
====
...
This URL can be found in the *Setup » Settings » API » Data* menu under *SOAP WSDL*. The WSDL is structured as follows: *http://WWW.SHOPURL.COM/plenty/api/soap/version1xx/?xml.*(*xx* = the current SOAP version)
...
. Go to *Setup » Client » Select client » Services » FINDOLOGIC*. +
