...
Pay attention to the tax rates settings in the <<en/order-processing/orders/accounting#, System » Client » Select client » Locations » Select locations » Accounting>> menu.
...
The Allyouneed market has a separate section for link:https://portal.allyouneed.com/verkaufen-mit-allyouneed/highlights-vorteile/b-ware.html[selling factory seconds^] (B-Ware). You have two possibilities to list there:

* Go to *Item » Tab: Other » Tab: Others* and select *Factory seconds* in the *Condition for API* drop-down list.
...
Activate the <<en/order-processing/payment/managing-payment-methods#, payment method>> *Allyouneed Payments* as described below.
...
|*Upload logo*
| If *Show upload logo* has been selected from the *Logo* drop-down list, you can upload an external logo here. +
...
→ Enter a description and save the categories.[.instruction]
...
→ Delete the item from the Allyouneed account. The item should be listed correctly when it is transferred again.[.instruction]
...
→ If this message is not followed by "PRODUCT_NOT_FOUND”, then it is simply a warning and the item is listed anyway. However, the item is not listed with your description. Instead, it is listed with the description in the Allyouneed catalogue.[.instruction]
...
[.indented]
This error message indicates that a different seller has already transferred the item with variations and that your item does not have any variations or has different variations saved for it.[.instruction]
...
→ Delete the item from the account on Allyouneed or contact Allyouneed.[.instruction]
...
→ Carry out the steps listed in the <<en/item/managing-items#440, Linking attributes>> chapter.[#1500]
...
[.indented]
This error message indicates that the attributes were not linked correctly or that a different seller has already transferred the item with variations and that your item does not have any variations saved for it.[.instruction]
...
→ For further information about Allyouneed groups, refer to the <<en/item/managing-items#440, Linking attributes>> chapter.[#1800]
