...
In this chapter, you will find information about the settings and functions for the market Shopgate. To do so, proceed as follows.

[.instruction]
Setting up Shopgate in plentymarkets:

.  Go to **Settings » Markets » Shopgate**.
...
**Client (store) item** = Only exports those items that were made <<omni-channel/multi-channel/shopgate#40, **available**>> for an online store.
...
| Select whether **<<order-processing/orders/coupons#, coupon codes>>** should be checked and accepted for campaigns with the discount type **Item discount**. +
...
**Important**: Item variations will only be transmitted if the **Limitation** option is either set to **No limitation** or **Limited to net stock** in the item's **Stock** tab.

|**Maximum for item upload**
| Enter the maximum amount of items that you want to upload to Shopgate.

|**Item description**
| If you select a free text field, then the text saved for this field will be displayed as the item description.
...
|**Framework URL**
| URL of the interface. The URL is saved automatically.

2+| Mobile forwarding

|**Alias**
| Enter the Alias that you received from Shopgate.
...
*  automatic checking of coupon codes during the order process
*  automatic checking of item stock in real time during the order process (**Stock check**, optional)

[#300]
==  Setting up forwarding for mobile devices

With the help of **<<omni-channel/online-store/setting-up-clients/online-store#forwarding, mobile forwarding>>** you can select whether customers should be forwarded to Shopgate when opening the store in a mobile browser. If you do not activate this setting, then customers will not be forwarded.
...
**Active** = Customers will be forwarded to the specified URLs. +
**Inactive** = Customers will not be forwarded.

|**Target URL (category)**
| Enter the URL for the mobile version of your online store, e.g. **http://mobile.yourplentystore.co.uk**. Customers who access your online store from a mobile device will be forwarded to this URL. +
...
Items have to be available for Shopgate. This is done in the **Availability** tab of an item within the **<<item/managing-items#40, Item » Edit item » Tab: Variation ID>>** menu. In addition, you can select whether all items or only selected items should be transferred. This can be done in the menu **Settings » Markets » Shopgate**.
...
.  Carry out the optional settings as desired. Pay attention to the information given in table 1.
. **Save** the settings.  +
...
* Select **Client (store) item** as the **Export type** in the menu **Settings » Markets » Shopgate**.
*  Activate at least one client under **Markets** in the item's **Available** tab.
...
Proceed as described below to define a sales price for the order referrer Shopgate. This price will be displayed on Shopgate.

[.instruction]
Defining a sales price for Shopgate:

. Open the menu **System » Item » Sales price » Open sales price » Tab: Settings**.
. Place a checkmark next to the referrer **Shopgate**.
. **Save** the settings.

[#500]
==  Linking properties

Link your **<<item/managing-items#480, properties>>**  with the Shopgate properties. To do so, proceed as follows.
...
Activate the **<<order-processing/payment/managing-payment-methods#, payment method>>****Shopgate Payment** as described below.
...
For further information, refer to the **<<order-processing/payment/managing-payment-methods#30, Payment methods and customer classes>>** chapter of the manual.
====
...
[IMPORTANT]
.Amazon Payments payment method
====
If you want to use the payment method **Amazon Payments**, then do not create a separate Shopgate account for Amazon Payments. Use the account created in plentymarkets.
====

Depending on the payment method, Shopgate will generate and send the invoice so that you do not have to send an invoice to the customer. You can check this in your Shopgate orders or create an **<<basics/automation/event-procedures#new-event-procedure, event procedure>>** to automate this.

[#700]
==  Automatically sending cancellations

Set up an **<<basics/automation/event-procedures#, event procedure>>** to automatically send cancellations to Shopgate. Cancellations are handled as returns at Shopgate. Therefore, use the procedure **Send return to Shopgate**.
...
. **Save** the settings.
--
.Event procedure for sending automatic cancellations to Shopgate
[cols="a,a,a"]
|====
...
Set up an **<<basics/automation/event-procedures#, event procedure>>** to automatically send returns to Shopgate.
...
. **Save** the settings.
--
.Event procedure for sending automatic returns to Shopgate
[cols="a,a,a"]
|====
|Setting |Option |Selection
