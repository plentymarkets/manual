...
Create further shipping profiles in the same way. The shipping profiles are then available in the *Base* tab of a <<en/omni-channel/multi-channel/hood#2300, listing>>.
...
Enter your feedback texts first. Then set up an <<en/basics/automation/event-procedures#, event procedure>> to automatically send your feedback texts to your buyers. All buyers who gave you feedback then automatically receive a random feedback text in return.
...
Set up an <<en/basics/automation/event-procedures#, event procedure>> to automatically send a shipping confirmation to Hood after the outgoing items have been booked.
...
. *Save* the settings.
.  Configure the settings as described in <<automatic-shipping-confirmation>> below.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
Set up an <<en/basics/automation/event-procedures#, event procedure>> to automatically send a payment confirmation to Hood.
...
. *Save* the settings.
.  Configure the settings as described in <<automatic-payment-confirmation>> below.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--

[[automatische-payment-confirmation]]
...
Go to *System » Listings* to carry out the settings for all listings regardless of the market. In order to create listings in plentymarkets, you must carry out the following settings and have created your <<en/item/managing-items#20, items>>.
...
plentymarkets provides you with multiple possibilities for creating listings. Make sure that you create your <<en/item/managing-items#20, items>> first. Then create your listings in the item overview using batch processing or via an existing listing. Use the data format <<en/basics/data-exchange/export-import/data-formats/itemlistingmarkethood#, ItemListingMarketHood>> to create new Hood listings or edit existing ones. Also create Hood listings for large quantities of items automatically.
...
|*Stock dependence*
|Select the <<en/stock-management/managing-stocks, stock>> of the listing. Select one of the following options: +
...
image::/assets/toolbar-market-listing.png[]
...
|*Stock dependence*
| Select the <<en/stock-management/managing-stocks#, stock>> of the listing.
