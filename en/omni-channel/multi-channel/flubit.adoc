...
**link:https://www.flubit.com/About[Flubit, window="_blank"]** is an online market that is only available in the United Kingdom and that does not take a commission from sellers.

[#10]
== Registering with Flubit

First, register as a seller on **link:https://www.weflubit.com/start-selling[WeFlubit, window="_blank"]**, the central Flubit interface for sellers.
...
. Go to **Settings » Markets » Flubit » Basic settings**.
...
This item description will be transferred to Flubit. The item description has to be available in **<<omni-channel/online-store/standard/online-store/multilingualism#40, English >>**. Otherwise the item will not be exported.
...
Items have to be available for Flubit. This is done in the **Availability** tab of an item within the link::/[**Item » Edit item » Tab: Variation ID**] menu.
...
* The item description is available in **<<omni-channel/online-store/standard/online-store/multilingualism#40, English >>**.
* The item is visible
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to automatically send shipping confirmations to Flubit when the outgoing items are booked.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to automatically inform Flubit of cancellations. Whenever you are dealing with cancellations, you need to send Flubit a reason for the cancellation. Go to **<<order-processing/orders/settings/order-status#, Settings » Orders » Order status  >>** and create additional order statuses as needed. The following cancellation reasons can be transferred to Flubit:
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to automatically inform Flubit about returns.
