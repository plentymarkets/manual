...
|*Warehouse selection*
|Select the stock quantity for the item export and offer export. Select one of the following options: +
...
*Password*, +
*Path*
|Enter parameter. The access data including the password and user name are provided by bol.com. The standard FTP port is *21*. The file name for BolOfferExport has the structure &lt;SellerID&gt;.csv. The file name for BolItemExport has the structure &lt;SellerID&gt;.txt. The file has to be saved in the folder *inbound*.
...
Activate the *<<en/order-processing/payment/managing-payment-methods#, payment method>> *BOL.com* as described below.
...
Set up an <<en/basics/automation/event-procedures, event procedure>> to automatically send shipping confirmations to bol.com when the outgoing items have been booked.
...
Set up an <<en/basics/automation/event-procedures, event procedure>> to automatically inform bol.com of the status of a return. bol.com requests the status of returns. The following cancellation reasons can be transferred to bol.com:
