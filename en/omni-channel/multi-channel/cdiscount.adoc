...
The options that can be selected here are the settings for Configuration 1 and Configuration 2 that are configured in the **link::/[Settings » Orders » Shipping » Shipping options » Tab: Calculate shipping costs]** menu.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to automatically send returns to Cdiscount.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to automatically update the order status on Cdiscount whenever specific events occur. You can use event procedures to automatically give orders the following statuses on Cdiscount:
