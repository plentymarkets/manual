...
You need to register with Cdiscount before setting up Cdiscount in plentymarkets. Fill in the link:https://seller.cdiscount.com/Account_creation.html[registration form^] and send your data to Cdiscount. One of the market's support employees will contact you to verify the registration.
...
|Enter the value in days. The value entered here is transferred to Cdiscount. Cdiscount increases this value on the basis of the shipping method.
...
The options that can be selected here are the settings for Configuration 1 and Configuration 2 that are configured in the <<en/order-processing/fulfilment/preparing-the-shipment#1700, System » Orders » Shipping » Settings » Tab: Calculate shipping costs>> menu.
...
Items have to be available for Cdiscount. This is done in the *Availability* tab of an item within the <<en/item/managing-items#170, Item » Edit item » Open item » Tab: Variation ID>> menu.
...
Use the link:https://knowledge.plentymarkets.com/en/item/managing-items#330[Variation group function^], the link:https://knowledge.plentymarkets.com/en/item/managing-items#330[Batch processing^] or the Dynamic Export and Import of the format link:https://knowledge.plentymarkets.com/en/basics/data-exchange/data-formats/variationmarketlink[VariationMarketLink^] in order to edit the availability for several variations.
...
Afterwards, link:https://knowledge.plentymarkets.com/en/item/managing-items#240[link^] the sales price with the desired variations.
...
Afterwards, link:https://knowledge.plentymarkets.com/en/item/managing-items#250[link^] the barcode with the desired variations.
...
If you have created <<en/item/managing-items#390, attributes>> in your plentymarkets system, you have to link these attributes with Cdiscount. Proceed as described below to link the attribute Colour with Cdiscount. Link further attributes in the same way.
...
Next create <<en/item/managing-items#480, properties>> and link these to Cdiscount properties. The properties have to be created in French.
...
Activate the <<en/order-processing/payment/managing-payment-methods, payment method>> *Cdiscount* as described below.
...
For further information, refer to the <<en/basics/statistics, Statistics>> page of the manual. For an example of flat rate and percentage costs, refer to the <<en/order-processing/payment/managing-payment-methods, Managing payment methods>> page of the manual.
|====
...
In the *Archive* tab, download the item catalogue or the stock synchronisation as a ZIP file. This makes it possible for you to transfer the files to your Cdiscount contact person in case there are problems during the import. This helps Cdiscount to analyse the situation more quickly in order to eliminate the problem.
...
In rare cases, the automatic item export may not work correctly. In such cases you can export your French item catalogue as a CSV file to Cdiscount. To do so, go to the <<en/basics/data-exchange/exporting-data, Data » Elastic export>> menu and carry out the settings as described below. After you have carried out and saved all necessary settings as described below, copy the complete URL and send it to Cdiscount. The hourly stock synchronisation is still carried out automatically.
...
. Pay attention to the explanations given in the <<en/basics/data-exchange/exporting-data#20, Exporting data>> page of the manual and carry out the settings.
. *Save* the settings. +
...
====
Link the properties *Gender*, *Main colour*, *Note*, *Comment*, *Marketing description*, *Audience* and *Sport* in the <<en/item/managing-items#480, System » Item » Properties>> menu in order to transfer your item catalogue to Cdiscount via a CSV file.
====

[#1200]
== Automatically sending returns

Set up an <<en/basics/automation/event-procedures, event procedure>> to automatically send returns to Cdiscount.
...
Set up an <<en/basics/automation/event-procedures, event procedure>> to automatically update the order status on Cdiscount whenever specific events occur. You can use event procedures to automatically assign the following statuses to orders on Cdiscount:
--
