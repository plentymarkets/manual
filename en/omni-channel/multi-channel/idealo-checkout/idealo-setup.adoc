...
Items that you want to sell on idealo have to be available for idealo Checkout. This is done in the *Availability* tab of an item within the <<item/managing-items#, Item » Edit item » Open item » Tab: Variation ID>> menu. Activate the item availability as described below.
...
.  Go to <<item/managing-items#, System » Item » Properties » Tab: New property>>.
...
. Select the idealo payment method *idealo Checkout*. +
...
Set up an <<en/basics/automation/event-procedures, event procedure>> to automatically send an order confirmation to idealo.
...
Set up an <<en/basics/automation/event-procedures, event procedure>> to automatically send a shipping confirmation to idealo when the outgoing items have been booked.
...
. *Save* the settings.
--
.Event procedure for sending automatic shipping confirmations to idealo
[cols="1,3,3"]
|====
|Setting |Option |Selection
...
Set up an <<en/basics/automation/event-procedures, event procedure>> to automatically inform idealo about returns.
...
Set up an <<en/basics/automation/event-procedures, event procedure>> to automatically inform idealo of cancellations.
