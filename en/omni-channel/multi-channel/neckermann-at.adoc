...
Neckermann Austria provides you with the possibility of presenting your products on the market **neckermann.at**. On this page, you will find detailed information about the settings you need to carry out in order to set up the market Neckermann Austria in your plentymarkets system. Pay attention to this information in order to ensure the smoothest link-up possible with this market.

[#10]
== Registering with Neckermann.at

If you are interested in the partner programme of link:http://www.neckermann.at/[**Neckermann Austria**, window="_blank"], then contact Neckermann.at directly. Please note that the neckermann.at website is currently only available in German.
...
After joining the Neckermann partner programme, use your access data to set up the market in plentymarkets. You can integrate both platforms with plentymarkets: Neckermann.at Enterprise and the Neckermann.at Cross-Docking. To set up Neckermann.at Enterprise in plentymarkets, proceed as follows. Set up Neckermann.at Cross-Docking in the same way.

[.instruction]
Setting up Neckermann.at in plentymarkets:

.  Open the **Settings » Markets » Neckermann » Neckermann Austria Enterprise** menu.
...
|**Partner ID**
|Enter the ID that you received from Neckermann.
...
|**Item name**
|Select a text field that will be transferred to Neckermann as the item name.

|**Item name (maximum length)**
|Select **35 characters** or **65 characters**.

|**Item preview text**
|Select a text field that will be transferred to Neckermann as the item preview text.

|**Item preview text (maximum length)**
|Select **30 characters** or **50 characters**.

|**Short item text**
|Select a text field that will be transferred to Neckermann as the short item text.

|**Item description**
|Select a text field that will be transferred to Neckermann as the item description.

|**Transfer item without assigning it to a brand**
|**No** = Items will be transferred and assigned to a brand by default. +
**Yes** = Items will be transferred without being assigned to a brand.

|**Image position for energy label**
|Enter the image position of the item image if the energy label is saved as an item image instead of as a property. For further information about the energy label, refer to the <<omni-channel/multi-channel/neckermann-at#150, **Providing information on the energy efficiency** >> chapter.

|**Transfer cross-selling**
|**No** = Cross-selling settings will not be transferred by default. +
**Yes** = Cross-selling settings will be transferred.

|**Transfer RRP**
|**No** = RRP will not be transferred by default. +
**Yes** = RRP will be transferred.

|**Item dimensions saved in the item data**
|Select the item dimensions.

|**Transfer client (store) ID?**
|**No** = The ID will not be transferred by default. +
**Yes** = The ID will be transferred.

|**Standard commission model**
|Select the commission model that you received from Neckermann.

|**Stock buffer**
|The transferred stock will be reduced by the entered value. This ensures that an item cannot be completely sold out on this market. +
**Example**: For an item with a stock of 50 and a stock buffer of 10, the value 40 will be transferred as stock. For further information, refer to the **link::/[Stock]** page of the manual.
...
**Item delivery time 1 - 5** = Enter the delivery time in days for each option. Set up the options for item availability in the <<item/managing-items#190, **Settings » Item » Item availability** >> menu.

|**Feature list**
|Select a text field or **Do not transfer**.

|**Transfer item number**
|**No** = The item number will not be transferred. +
**Yes** = The item number will be transferred during item export. A maximum of 20 characters will be transferred.
...
**No** = Orders will be booked as **paid**.

|**Save package number**
|**Yes** = The package number will be transferred by default. +
**No** = The package number will not be transferred.
|====

__Table 1: basic settings for the market Neckermann.at__
...
* Hourly order import
* Hourly forwarding of shipping confirmation
...
In this area you set the order statuses for cancellations and determine when returns are forwarded to Neckermann.
...
__Table 2: setting the order status__

[#40]
== Setting up FTP server access

Set up your FTP server access in this area. Neckermann requires FTP server access for importing orders and invoices.
...
. Open the CSV file.
. **Save** the settings. +
→ The Neckermann components will be available in the drop-down menus of properties.

[#60]
== Linking properties

In the next step, link your <<item/managing-items#480, **properties** >> with the Neckermann components.
...
. Expand a property.
. Select the Neckermann component from the **Neckermann.at EP component** drop-down menu.
. **Save** the settings.

[#70]
== Importing logos

Neckermann provides you with various logos, e.g. for features or characteristics that designate quality. The import can be done by uploading a CSV file which contains the Neckermann data. Link logos in the same way as Neckermann components.

[.instruction]
Importing Neckermann logos:

. Open the file from Neckermann.
. Select and copy the columns **LOGO_ID** and **LOGO_DESCRIPTION**.
. Insert the columns into a new file in a spreadsheet program.
. Save the file in CSV format on your computer.
. Open the **Settings » Markets » Neckermann » Neckermann Austria Enterprise** menu.
...
. Open the CSV file.
. **Save** the settings. +
→ The Neckermann logos will be available in the drop-down menus of properties.

[#80]
== Linking categories

Neckermann provides you with a file which contains the categories that need to be assigned to the categories of your online store. The categories will then be linked and further items from the linked category will be automatically assigned.
...
→ Primary categories will only be available under **Category links** if the market **Neckermann** is activated.
...
If you have created <<item/managing-items#390, **attributes** >> in your plentymarkets system, you will have to link these attributes with Neckermann. To do so, you have to import the <<omni-channel/multi-channel/neckermann-at#50, **Neckermann components** >> first. Proceed as described below to link the attribute Colour with Neckermann. Link further attributes in the same way.
...
. Select the option **Neckermann.at Enterprise** from the **Market** drop-down menu. +
→ An additional drop-down menu **Neckermann.at Enterprise** will be displayed below.
. Select the attribute **Colour** from the **Neckermann.at Enterprise** drop-down menu.
...
. Go to **Data exchange » Dynamic export » Tab: New data format**.
...
. Go to **Data exchange » Data import » Dynamic import**.
. Select the field separator.
. Select your data format under **Format**.
. Click on **Browse...**.
. Open the CSV file.
. Click on **Import**.  +
→ After having imported the brand IDs, you can find the Neckermann ID of a manufacturer in the **Settings » Item » Manufacturers** menu.
...
Search for items already transferred to Neckermann in the **Settings » Markets » Neckermann » Neckermann Austria Enterprise » Tab: Transferred items** menu. Enter the item ID, the name or the Neckermann number into the search field to search for specific items or leave the search field empty and click on **Item search** to display all results.

[#130]
== Displaying error reports

All operations relating to the market Neckermann will be recorded in your plentymarkets system. You can retrieve error reports containing a list of all error messages and notes with the corresponding IDs. In the **Error reports** menu, all errors related to your stock used for Neckermann will be displayed. A distinction will be made between the following types of errors:

* **Note**: A note will be displayed.
* **Warning**: In case of warnings, all items will be imported.
* **Error**: In case of errors, no items with errors will be imported.
* **Critical error**: In case of critical errors, no items will be imported.

[.instruction]
Retrieving error reports:

. Open the **Settings » Markets » Neckermann » Neckermann Austria Enterprise** menu.
. Click on the **Error reports** tab.
. Click on **Retrieve error reports**.  +
→ The error reports will be retrieved and displayed.

In the list **Latest error reports**, you find information about type and number of errors for the following data types:
...
In case of errors due to incorrect item data occurring during the item export from plentymarkets, information about the affected item as well as the error type can be found in the error message. Fix the error and check whether there are additional error messages after you carry out the next automatic item export.

[TIP]
.Description of error messages
====
For further information about the error messages, contact Neckermann directly. Ask your Neckermann contact person for it, if needed.
====

[#140]
== Setting the item availability

Items have to be available for Neckermann.at. This is done in the **Availability** tab of an item within the <<item/managing-items#170, **Item » Edit item » Tab: Variation ID** menu. >>

<<item/managing-items#170, [.instruction]
...
. Activate Neckermann.at Enterprise and/or Neckermann.at Cross-Docking.
. Activate the option **Web API**.
. Click on **Add**. +
→ The Market will be added.
. **Save** the settings.
. Open the **Multi-Channel** tab.
. Select an **Enterprise commission model**.
. Select an **Enterprise item type**.
. **Save** the settings.
...
// h2>Preisspalte festlegen</h2> <p>Gehen Sie wie im Folgenden beschrieben vor, um für die <a href="{% Link(963) %}"><strong>Auftragsherkunft</strong></a> Neckermann eine Preisspalte festzulegen. Dieser Preis wird auf Neckermann angezeigt.</p> <h5>Preisspalte für Neckermann.at festlegen:</h5> <ol> <li>Öffnen Sie das Menü <strong>Einstellungen » Aufträge » Auftragsherkunft</strong>.<br /> → Neckermann.at Enterprise gehört zu den Standardherkünften.</li> <li>Wählen Sie eine <strong>Preisspalte</strong> für Neckermann.at, sofern nicht der Standardpreis verwendet werden soll.</li> <li><strong>Speichern</strong> Sie die Einstellungen.</li> </ol
>>== <<item/managing-items#170, Providing information on the energy efficiency >>

To comply with EU regulations on energy efficiency that came into force on 1 January 2015, you have to provide information on the energy efficiency of certain items to sell them on Neckermann. In addition, you have to make product data sheets available to potential buyers of these items. Create the following <<item/managing-items#480, **properties** >> to comply with these EU regulations:
...
Link the properties in the <<item/managing-items#80, **Properties** >> tab of an item.
...
. **Save** the settings. +
→ The properties will be displayed in the list.
...
Instead of linking the **Energy label** property, you can also upload the energy label file to the <<item/managing-items#90, **Images** >> tab of the item. To do so, proceed as follows.
...
. **Save** the settings.
. Open the **Settings » Markets » Neckermann » Neckermann Austria Enterprise** menu.
. In the **Basic settings** area, enter the image's position number into **Image position for energy label** field.
. **Save** the settings.

[#160]
== Activating the payment method

Activate the **link::/[payment method]****Neckermann Payment** as described below.
...
For further information, refer to the **<<basics/statistics#, Statistics  >>** and **<<basics/statistics/general-statistics/item/tops-flops#, Tops/Flops  >>** pages of the manual. For an example for flat rate and percentage costs, refer to the **<<omni-channel/multi-channel/neckermann-at#20, Basic settings >>** page of the manual.
|====
...
For further information, refer to the **<<omni-channel/multi-channel/neckermann-at#40, Payment methods and customer classes >>** chapter of the manual.
====
...
. Open the order.
. Select the option **create** from the **Return...** drop-down menu in the **Overview** tab. +
→ The **Return for the order** window will open.
. Enter the **package code of the return**.
. Place a check mark in the **Accept** column under **Item**.
. **Save** the settings. +
→ The return will be created and open. The code is saved in the return's **Settings** tab.
...
Since there is no possibility to delete cancellations e.g. of individual items in an order, set the quantity of items affected in such a case to 0. A reason has to be given for each cancellation, either **item not in stock** or **customer cancellation**.

[#185]
== Generate Neckermann invoices
...
You can carry out settings related to the layout of the invoices by going to **Settings » Client (store) » Standard » Locations » Select location » Documents » Invoice**. Go to **Settings » Orders » Event procedures** in order to define which additional procedure should be carried out should be carried out when the invoice is printed. If you would like the procedure to be limited to orders from the Neckermann market only, then make sure that you create a filter of the filter type **Referrer** and select **Neckermann** within the filter.
...
→ A **zip file** will be created, which contains the invoices corresponding to all of the orders that you selected with the filter options.
...
|**Items shipped on**
|Select the date from the calendar. The date when items were shipped can only be used as a filter if you selected status **[7] Outgoing items booked** from the **Order status** drop-down menu.
...
__Table 4: settings for Neckermann invoice__

[#190]
== Viewing the API log

The menu **Data exchange » API log » Tab: API log** contains a history of processes that run via the Neckermann interface. The formats **NeckermannATEP** and **NeckermannATCD** are available for Neckermann.

[.instruction]
Viewing the API log:

.  Go to **Data exchange » API log**.
.  Select the **NeckermannATEP** format from the **Process** drop-down menu.
