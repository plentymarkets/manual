...
In the *Item settings* tab, you carry out settings related to the item export. For example, you can configure automatic processes and export filters. If you select export filters, only items with the selected filters are exported.
...
*Yes* = Amazon credit notes are imported into your plentymarkets system every four hours. +
...
Add additional SKUs manually for Amazon in the menu <<item/managing-items#170, Item » Edit item » Open item » Tab: Variation ID » Tab: Settings » Tab: Availability>>. As an alternative, you can use the data format VariationAdditionalSKU to import additional SKUs.
...
. Open the *Item » Edit item » Open item » Tab: Variation ID » Tab: Settings* menu.
.  Click on the *Availability* tab.
.  Click on *Add* in the *Additional SKU* section. +
→ The editing window *New additional SKU* is displayed.
.  Select the referrer *Amazon* to add the SKU.
.  Select the corresponding market account.
.  Enter the SKU.
.  Click on *Add*.  +
...
Additional SKUs are automatically included in the order import. Activate the stock update and price update for additional SKUs manually in the menu *System » Markets » Amazon » Settings » Tab: Item settings*. The stock update and price update for additional SKUs is carried out along with the normal stock update and price update at the interval that you have selected within *System » Markets » Amazon » Settings » Tab: Item settings* for the options *Price changes* and *Stock synchronisation* in the section *Automatic processes*.

[.instruction]
Activating the stock update and price update:
...
Use the credit note import in order to automatically import FBA credit notes into your plentymarkets system. The credit notes are imported with the status which you have selected for the *Standard status for credit note* setting in the *System » Orders » Settings* menu. FBA credit notes are imported every four hours.

[IMPORTANT]
.Do not import FBA returns as credit notes via an event procedure
====
If you activate the credit note import, FBA credit notes are automatically imported into your plentymarkets system every four hours. Therefore, in this case do not use an event procedure to create credit notes from FBA returns.
====
...
===== Which settings are required for an item to be exported to Amazon?

- *Activate the item* +
In order to export an item to Amazon, the item has to be activated. To do so, go to *Item » Edit item » Open item » Tab: Variation ID » Tab: Settings* and place a check mark next to the setting *Active* in the *Availability* section.

- *Setting the item availability* +
The item has to be available for Amazon. To make an item available, go to *Item » Edit item » Open item »Tab: Variation ID » Tab: Settings* and place a check mark next to the setting *Active* in the *Availability* section. Then click on the *Availability* tab and activate the option *Web API* and the desired *Amazon platform* in the drop-down list in the *Markets* section.

- *Select an Amazon product type* +
Select a *Product type* in the *Amazon* section of the item's *Multi-Channel* tab.

- *Add an SKU* +
In order to export an item to Amazon, add an SKU with the referrer *Amazon* in the *Item » Edit item » Open item » Tab: Variation ID » Tab: Availability* menu.

- *Price settings* +
Make sure that the item's sales price is activated for the Amazon account and the Amazon platform. The settings for the sales price are carried out in the *System » Item » Sales prices* menu.

- Export filter *Store items* +
If you have selected the option *Only items visible in online store* for the export filter *Store items* in the *System » Markets » Amazon » Settings » Tab: Item settings* menu make sure that the item is activated for the web shop. This is done in an item's *Settings* tab.
