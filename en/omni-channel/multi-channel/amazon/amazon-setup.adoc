...
//Learn more about quickly exporting items to Amazon in the following video tutorial:
...
|*Amazon platform*
| A platform has to be selected when the account is created. However, the platform can be changed here. If you have a European Marketplaces Account, then you have the possibility to configure each platform on which you want to offer your products separately. For further information, refer to the section <<Setting up an account, Amazon platform>>.
...
//Learn more about the initial settings in the following video tutorial:
...
|====
 ...
 Learn more about the item settings for Amazon in the following video tutorial:
...
|Select the <<order-processing/orders/managing-orders#1200, order status>> for orders that were successfully transferred to FBA.
...
//Learn more about order settings for Amazon in the following video tutorial:
...
|*Product type*
|Select the <<basics/data-exchange/export-import/data-formats/item#50, Amazon product type>>.
...
Add SKUs manually for specific referrers in the menu <<item/managing-items#170, Item » Edit item » Open item » Tab: Variation ID » Tab: Availability>> if needed. As an alternative, use the data format VariationAdditionalSKU to import additional SKUs.
...
====
The interface does not automatically trigger the refund of the purchase price with Amazon. Rather, this has to be carried out manually or automatically with the help of an event procedure. If you forget to do this, you risk a customer complaint with Amazon and negative feedback.
====
...
First, go to <<order-processing/orders/managing-sales-orders#order-status, System » Orders » Status>>. Create an order status for Amazon returns, e.g. [9.6] Return sent to Amazon. You need this order status for the Amazon settings.
...
. *Save* the settings.
.  Configure the settings as described in <<event-procedure-automatic-return>>.
...
. *Save* the settings.
.  Configure the settings as described in <<event-procedure-update-order-status>>.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
Amazon VCS is the VAT calculation service offered by Amazon. With this service, Amazon automatically generates invoices and calculates the sales tax. plentymarkets enables you to generate Amazon invoices directly in your plentymarkets system. +

Currently, Amazon's VAT calculation service is only available for selected sellers. Those sellers will be contacted by Amazon. After you have been selected for Amazon VCS, plentymarkets must activate a module in your plentymarkets system so that you can use Amazon VCS. To activate the module, contact plentymarkets in the link:https://forum.plentymarkets.com/[forum^]. +

Activate Amazon VCS in plentymarkets to generate invoices directly in your plentymarkets system. Amazon invoices are then automatically transferred to Amazon. Activate Amazon VCS in plentymarkets in the *Initial settings* section of the *System » Markets » Amazon » Select an Amazon account » Tab: Settings* menu.
...
.  Select the account.
.  Click on the tab *Order settings*.
. Place a check mark next to the option *Active* in the section *Shipment by Amazon (FBA)*.
. *Save* the settings.
...
Configure the event procedures that are necessary for the fulfilment process. Such event procedures have invoices generated and sent to your customers with the help of automatic emails, and have the order then switch to another status. +
 For further information, refer to the <<en/basics/automation/event-procedures#, Event procedures>> and <<en/order-processing/fulfilment#, Fulfilment>> pages of the manual.
====
...
In case of a return, the purchaser will send the items back to Amazon. The return is imported into your plentymarkets system and listed in the order menu. It is given the return status that you selected in the <<order-processing/orders/managing-orders#420, System » Orders » Settings>> menu.
...
Set up an <<basics/automation/event-procedures#, event procedure>> to automatically inform Amazon about the order. After Amazon has received this information, the order is processed and the item(s) are made ready for shipping.
...
. *Save* the settings.
.  Configure the settings as described in <<event-procedure-fba-shipping-clearance>>.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
====
Pay attention to the shipping packages saved for the shipping profile to prevent overpriced labels.
====
...
Activate the <<order-processing/payment/managing-payment-methods, payment method>> *Amazon* as described below.
...
For further information, refer to the <<order-processing/payment/managing-payment-methods#30, Permitting the payment method in a customer class>> page of the manual.
====
...
After having activated the Amazon account for Amazon Business, you have to create and link sales prices for B2B items. In addition to the standard price for items with the minimum quantity 1, you can create up to 5 more prices for quantity based discounts. It is possible to create prices for quantity based discounts as fixed prices or percentage.
...
As an alternative, you can also display up to 5 more sales prices in percent for quantity based discounts, using property links. To do so, select the Amazon link *quantity_price_type* when creating the property of the type *Text* or *Selection* and enter the value *percent* when linking the property to the item. Additionally, create properties for *quantity_lower_bound1-5* for the discount quantity and _quantity_price1-5* for the percentage.
