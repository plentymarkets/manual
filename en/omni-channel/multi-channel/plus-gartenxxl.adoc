...
. Go to *System » Markets » Netto eStores*. +
...
The following instruction describes how you set the item availability for Plus.de. Carry out the settings for the other markets in the same way. +
...
Activate the <<order-processing/payment/managing-payment-methods#, payment method>> *Netto* as described below. Afterwards, carry out the settings in the same way for the payment methods *GartenXXL.de*, and *GartenXXL.at*.
...
Set up an <<basics/automation/event-procedures#, event procedure>> to automatically send cancellations to Netto eStores.
...
Set up an <<basics/automation/event-procedures#, event procedure>> to automatically send returns to Netto eStores.
...
.Select a status
====
Select a status for the procedure *Change order status* that you have saved within *System » Orders » Settings* for the option *Status of cancelled orders (No reservation of stocks)* to cancel reservations. Neither select a status that triggers an event action which reports cancellations to Netto eStores.
====
