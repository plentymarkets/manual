...
====
After changing the eBay account data, update your shop categories and return policy. You can reload the **shop categories** directly in this menu here. Go to <<omni-channel/multi-channel/ebay#2100, **Settings » Markets » eBay » Business policies » Tab: eBay return policy** >> to save the **return policy**.
====
...
Bevor linking eBay shipping profiles with the shipping profiles in your online store you have to enter terms of payment and shipping as well as general conditions in the menu <<omni-channel/multi-channel/ebay#220, **Settings » Markets » eBay » Business policies** >>.

[#800]
===  Linking return reasons and statuses

Return reason links and return status links are required, if you want to process <<omni-channel/multi-channel/ebay#740, **eBay returns** >> in plentymarkets and ensure the automatic data exchange with eBay. In the **Settings » Markets » eBay » Basic settings » Tab: Return settings** menu, link eBay return reasons with the reasons set up in the **<<order-processing/orders/settings/sales-order-types/return#, Settings » Orders » Order types » Return  >>** menu in plentymarkets. These will be assigned automatically when importing a return.
...
.  Select a plentymarkets **<<order-processing/orders/settings/sales-order-types/return#, return reason  >>** for each **eBay return reason** from the drop-down menus.
. **Save** the settings.
...
Certain return statuses are used when editing returns in the eBay backend. To import eBay returns into plentymarkets and edit them, link the eBay return statuses to statuses set up in the **<<order-processing/orders/settings/order-status#, Settings » Orders » Order status  >>** menu.
...
.  Select a plentymarkets **<<order-processing/orders/settings/order-status#, return status  >>** for each **eBay return status** from the drop-down menus.
. **Save** the settings.
...
====
Import reasons for return that eBay edited or re-published. To do so, go to **<<omni-channel/multi-channel/ebay#1090, Settings » Markets » eBay » Data exchange » Tab: Other >>** and use the **Synchronise return reasons** function. Imported reasons for return can be linked as described above.
====

[#900]
===  Linking purchase cancellation statuses

Purchase cancellation statuses are used to process purchase cancellation. Create new **<<order-processing/orders/settings/order-status#, order statuses  >>** and link them with the purchase cancellation statuses from eBay to import and edit eBay purchase cancellations in plentymarkets. When importing a purchase cancellation request, the correct status will automatically be set in the order.

[.instruction]
Linking purchase cancellation statuses:

.  Go to **Settings » Markets » eBay » Basic settings » Tab: Purchase cancellation**.
.  Select a plentymarkets **<<order-processing/orders/settings/order-status#, status  >>** for each **eBay status of purchase cancellation** from the drop-down menus.
. **Save** the settings.
...
Alternatively, set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to post positive feedback automatically on eBay.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to automatically send eBay messages.
...
|**PayPal account**
| Select your PayPal account. You will only see those PayPal accounts that were set up in the <<order-processing/payment/paypal#, **Settings » Orders » Payment » Payment methods » PayPal**  >> menu.
...
Use this menu to specify domestic and international shipping conditions. In addition to the region of delivery, select specific <<order-processing/fulfilment/preparing-the-shipment#1000, **shipping profiles** >> or service providers for international shipping. Go to **link::/[Settings » Orders » Shipping » Shipping options » Tab: Countries of delivery]** and check your language settings for the countries that you want to ship your eBay items to. For further information, refer to eBay's **link:http://pages.ebay.co.uk/help/sell/shipping-globally.html[Using the Global Shipping Programme, window="_blank"]** page.
...
====
For further information, refer to the <<omni-channel/multi-channel/ebay#1120, **Selling internationally** >> chapter and eBay's link:http://pages.ebay.co.uk/help/sell/shipping-globally.html[**Using the Global Shipping Programme**, window="_blank"] page.
====
...
For further information, refer to the **<<basics/start/statistics#, Statistics  >>** and **<<basics/start/statistics/general-statistics/item/tops-flops#, Tops/Flops  >>** pages of the manual. For an example for flat rate and percentage costs, refer to the **<<order-processing/payment/managing-payment-methods#, Basic settings  >>** page of the manual.
|====
...
Layout templates for listings use the same structure as the **<<omni-channel/online-store/_cms/web-design#, plentymarkets web design  >>**. The content that you want to display for listings can be saved using template variables and functions in the **Main frame** tab in the **Settings » Listings » Layout templates** menu. The content is structured with HTML and CSS. The different tabs are described below.
...
Make sure that the images used are available via URL. To do so, upload the images on the server e.g. using **<<omni-channel/online-store/global-settings/ftp-access#, FTP  >>** or the <<omni-channel/online-store/_cms/image-gallery#, **image gallery**  >>.
|====
...
**Basic settings** = The option selected for the **<<omni-channel/multi-channel/ebay#65, Automatically activate eBay Plus for new listings >>** setting in the **Settings » Markets » eBay » Basic settings » Tab: Account settings** menu will be used. **Basic settings** is preselected.  +
...
| Add a parts compatibility listing. Enter the name of a list or only a part of the name and select a list. Create parts compatibility listings in the **<<omni-channel/multi-channel/ebay#160, Settings » Markets » eBay » Parts compatibility listing >>** menu.
...
In plentymarkets, there are multiple ways to automatically start listings or restart listings once they were already started. Use the listing scheduler to specify **starting times** for listings. Go to <<omni-channel/multi-channel/ebay#270, **Settings » Listings » Listing scheduler** >> to set up the listing scheduler.
...
Use the **<<omni-channel/multi-channel/ebay#65, Settings » Markets » eBay » Basic settings » Account settings >>** menu to decide whether the loyalty program should be activated for all new listings. Listings can also be activated or deactivated for eBay Plus later on.
...
You can search for eBay Plus orders in the **<<order-processing/orders/editing-sales-orders#, Orders » Edit orders  >>** menu. Set the filter **eBay Plus** in the **Filter** tab to **Yes** to filter eBay Plus orders.
...
Activate Click &amp; Collect by placing a check mark next the option **Buyers can collect the item at Argos with Click &amp; Collect.** in the menu **<<omni-channel/multi-channel/ebay#2000, Settings » Markets » eBay » Business policies » Tab: eBay shipping conditions >>**. Next, you will have to link the shipping conditions with the shipping profile from eBay that is saved for the listing.
...
You can search for Click &amp; Collect orders in the **<<order-processing/orders/editing-sales-orders#, Edit orders  >>** menu. Set the filter **Click &amp; Collect** in the **Filter** tab to **Yes** to filter Click &amp; Collect orders.
...
You need to set up an automatic procedure in the <<order-processing/orders/settings/procedure-manager#, **procedure manager**  >> in order to automatically send dunning information. You can enter the conditions under which the procedures are carried out and define a time for them.
...
For further information about automatic procedures, refer to the **<<order-processing/orders/settings/procedure-manager#, Procedure manager  >>** page of the manual.
...
eBay returns and return statuses of the last 14 days are automatically imported from eBay to plentymarkets and updated every hour. When importing, new returns are created with the **Standard status for return** in the **Orders » Edit orders** menu. The reason for return chosen by the customer on eBay is assigned to a <<order-processing/orders/settings/sales-order-types/return#, **reason for return**  >> in plentymarkets. Imported eBay returns are edited the same way as other **<<order-processing/orders/editing-sales-orders/return#, returns  >>** in plentymarkets.
...
Set up <<order-processing/orders/settings/event-procedures#, **event procedures**  >> to automatically exchange information with eBay. With event procedures, you initiate refunds from PayPal payments, mark returns as received and inform eBay that you have paid the refund. Event procedures will be triggered when you change the status of a return in the menu **<<order-processing/orders/editing-sales-orders/return#, Orders » Edit orders » Open return  >>**.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to automatically inform eBay about accepting a return and processing it with a return label from eBay.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to automatically inform eBay about accepting a return and processing it with your own label, not with a return label from eBay.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to automatically inform eBay about rejecting a return.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to offer your customer partial refund of the purchase price.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to inform eBay about having received the item returned by the customer.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to initiate a refund via PayPal to the customer.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to inform eBay about having initiated a refund for the customer. This event procedure only works for orders that were not paid for with PayPal.
...
Carry out the following settings to import eBay purchase cancellation requests. After the import, the purchase cancellation status will be displayed in the **<<order-processing/orders/editing-sales-orders/changing-a-sales-order#, Orders » Editing orders  >>** menu. The status displayed is the status that you linked to the eBay purchase cancellation status in the **Settings » Markets » eBay » Basic settings » Tab: Purchase cancellation** menu. Therefore, you have to link the statuses first.
...
Use the group function to change several orders at once. If you want to accept the request for unpaid orders, search for all unpaid orders with the status 8.5 and change the status of these orders using the **<<order-processing/orders/editing-sales-orders/changing-a-sales-order#170, group function >>**.

It might be possible to set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** based on your specific workflows that accepts requests automatically using certain filters.

[#9300]
====  Accepting purchase cancellation for outstanding orders

Outstanding orders are those orders for which the buyer has not yet completed the checkout process on eBay. Outstanding orders are only displayed in plentymarkets if the option **Shipping costs for new orders coming from eBay** was set to **Transfer from eBay**. This option is found under <<omni-channel/multi-channel/ebay#260, **Settings » Listings » Checkout** >>. Outstanding orders can be found in the <<omni-channel/multi-channel/ebay#1010, **Settings » Markets » eBay » Data exchange** >> menu.  +
...
Set up two **<<order-processing/orders/settings/event-procedures#, event procedures  >>** to automatically reject purchase cancellation requests for paid orders.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to automatically inform eBay about accepting a purchase cancellation.
...
Set up an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to automatically inform eBay about rejecting a purchase cancellation.
...
In the **Orders** tab, you manually retrieve orders. This might be necessary if you do not use the automatic retrieval under **<<omni-channel/multi-channel/ebay#60, Settings » Markets » eBay » Basic settings >>** or if your token expired and you now want to import the orders from this time period.
...
Outstanding orders are those orders for which the buyer has not paid yet and thus not yet completed the checkout process on eBay. These orders cannot be found in the <<order-processing/orders/editing-sales-orders/tab-search#, **Orders**  >> menu. You can import outstanding orders so that they become available in the **Orders** menu.

[IMPORTANT]
.Outstanding orders
====
Outstanding orders only occur if the option **Shipping costs for new orders coming from eBay** was set to **Transfer from eBay**. This option is found under <<omni-channel/multi-channel/ebay#260, **Settings » Listings » Checkout** >>. It is only possible to import outstanding orders within 30 days.
====
...
→ The orders are now available in the <<order-processing/orders/editing-sales-orders/tab-search#, **Orders**  >> menu.
...
====
Go to <<omni-channel/multi-channel/ebay#200, **Settings » Markets » eBay » Business policies » Tab: eBay shipping conditions** >> to select the **shipping discounts for combined payments** that were imported from eBay.
====
...
In the <<omni-channel/multi-channel/ebay#280, **Settings » Listings » Layout templates** >> menu, enter the description, i.e. text content, and specify the listing's layout. You can **copy the item description** and translate it into another language, for example. It is also possible to use templates.
