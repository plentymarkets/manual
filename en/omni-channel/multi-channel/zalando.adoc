...
If you are interested in the link:https://www.zalando.co.uk/partner-programme/[**partner programme**, window="_blank"], then contact Zalando directly at partner-programme@zalando.co.uk.
...
.  Go to **Settings » Markets » Zalando » Basic settings**. +
...
Items have to be available for Zalando. This is done in the **Availability** tab of an item within the link::/[**Item » Edit item » Tab: Variation ID**] menu.
...
. Go to **Settings » Markets » Zalando » Category link**.
...
In the menu **Settings » Markets » Zalando » Basic settings » Tab: Silhouettes**, you convert Zalando silhouettes to item properties. Silhouettes are transferred during the initial setup of Zalando.
...
|**Property ID**
|Enter the ID of the online store property. You find the property ID in the **Settings » Item » Properties** menu.
...
**selection** = Enter the option's selection ID from the drop-down menu. You find the selection ID for properties of the type **Selection** in the **Settings » Item » Properties** menu under **Selection ID**.
...
You can link the exported Zalando properties with the items in your plentymarkets system. This is done by means of the **<<basics/data-exchange/data-formats/itemproperties#, data format  >>** via the **link::/[dynamic import]**.
...
Activate the **link::/[payment method]****Zalando Payment** as described below.
...
For further information, refer to the **<<basics/start/statistics#, Statistics  >>** and **<<basics/start/statistics/general-statistics/item/tops-flops#, Tops/Flops  >>** pages of the manual. For an example for flat rate and percentage costs, refer to the **<<omni-channel/multi-channel/zalando#20, Basic settings >>** page of the manual.
|====
...
In the **Settings » Markets » Zalando » Basic settings » Tab: Processing reports** menu, you can retrieve processing reports as well as the export log for Zalando.
...
The menu **Data exchange » API log » Tab: API log** contains a history of processes that run via the Zalando interface. The **ZalandoController** format is available for Zalando.
