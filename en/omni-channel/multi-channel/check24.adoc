...
Add SKUs for specific referrers in the *Availability* tab of the menu <<en/item/managing-items#170, Item » Edit item » Open item » Tab: Variation ID>>.
...
. Carry out further settings for item filters and format settings. Pay attention to the information given in table 5 on the <<basics/data-exchange/export-import/exporting-data#60, Exporting data page of the manual>>. +
  _*Tip:*_ In order to only export items to Check24 which are activated for Check 24 and available for the market in the item's *Availability* tab, select the item filter *Active* and the item filter *Markets » Check24*.
. *Save* the settings. +
