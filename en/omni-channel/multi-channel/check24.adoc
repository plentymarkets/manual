...
. Carry out further settings for item filters and format settings. Pay attention to the information given in table 5 on the <<basics/data-exchange/export-import/exporting-data#60, Exporting data page of the manual>>. +
  _*Tipp:*_ Damit Artikel, die für Check24 aktiviert und verfügbar sind, zu Check24 übertragen werden, wählen Sie den Artikelfilter *Aktiv* und den Artikelfilter *Märkte » Check24*.
. *Save* the settings. +
