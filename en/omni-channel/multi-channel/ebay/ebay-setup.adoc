...
*Yes* = The variation number is transferred as the SKU. This setting is not recommended because synchronisation is not possible when the variation number is changed but the listing is not updated. +
...
*_Or:_* Set up an <<basics/automation/event-procedures#, event procedure>> to automatically post positive feedback on eBay.
...
ePIDs are not automatically matched for already existing listings. For existing listings, you have to match ePIDs manually.
====
...
.eBay OAuth2 plugin
====
In order to use ePID matching, the link:https://marketplace.plentymarkets.com/en/plugins/markets/EbayOAuth2_4787[eBay OAuth2 plugin^] is required. The plugin is available in the link:https://marketplace.plentymarkets.com/en/plugins/markets/EbayOAuth2_4787[plentyMarketplace^].
====
