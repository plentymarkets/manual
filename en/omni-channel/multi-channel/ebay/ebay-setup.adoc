...
====
After changing the eBay account data, update your shop categories and return policy. Reload the *shop categories* directly in this menu. Go to <<Determining the return policy, System » Markets » eBay » Business policies » Tab: eBay return policy<< to save the return policy.
====
...
This setting is defined in the *Transfer as item number* field of a new market listing. If the *Transfer UPC* option is activated in the *Item » Edit item » New listing* menu, select one of the *Use EAN 1 - 4* options here.
...
*Yes* = Further item details from the eBay catalogue are automatically added. For further details, refer to the link:https://pages.ebay.co.uk/help/sell/product-details.html[eBay help^]. +
...
*Yes* = Further item images from the eBay catalogue are automatically added. For further details, refer to the link:https://pages.ebay.co.uk/help/sell/product-details.html[eBay help^]. +
...
////
<strong class="subhead">Loading eBay shop categories</strong><a id="500." name="500."></a>
<p>
Import the eBay shop categories to plentymarkets or synchronise them in this area. Categories can be selected in a listing or transferred to listings using listing templates. In order to display categories in a template, the <strong><span>$</span>{% GetListingStoreCategories() %}</strong> function must be used in layout templates. For further information, refer to the <strong><a href="{% Link(3090) %}#2800.">Managing layout templates</a><strong> chapter.</p>
<h5>
Loading eBay shop categories:</h5>
<ol>
<li>
Go to <strong>System » Markets » eBay » Settings » Tab: Account settings</strong>.</li>
<li>
Expand the <strong>eBay shop categories</strong> section to display an overview of your account’s shop categories.<br />
→ The account data is synchronised.</li>
<li>
Select <strong>by position (as saved on eBay)</strong> or <strong>alphabetically by category name</strong> from the <strong>Import shop categories</strong> drop-down list.</li>
</ol>
<div class="plentyInfoBox plentyIsMessage">
<div class="plentyInfoBoxHeader">
Updating shop categories and return policy</div>
<div class="plentyInfoBoxContent">
<p>
After changing the eBay account data, update your shop categories and return policy. Update the <strong>shop categories</strong> directly in this menu. Go to <a href="{% Link(3090) %}#2100."><strong>System » Markets » eBay » Business policies » Tab: eBay return policy</strong> to save the <strong>return policy</strong></a>.</p>
</div>
</div>
////
[#700]
...
Return reasons and return statuses have to be linked if you want to process <<Processing returns, eBay returns>> in plentymarkets and ensure the automatic data exchange with eBay. Go to *System » Markets » eBay » Settings » Tab: Return settings* in order to link eBay return reasons with the reasons set up in the <<order-processing/orders/managing-sales-orders#return, System » Orders » Order types » Return>> menu in plentymarkets. These are assigned automatically when importing a return.
...
.  Select a <<order-processing/orders/managing-sales-orders#return, plentymarkets return reason>> for each *eBay return reason* from the drop-down lists.
. *Save* the settings.
...
The language versions are assigned to the countries of delivery depending on the languages that were set for each country under <<order-processing/fulfilment/preparing-the-shipment, System » Orders » Shipping » Settings » Tab: Countries of delivery>>.
...
With link:https://pages.ebay.co.uk/help/sell/fitment.html[parts compatibility listings^], you link the items that you want to sell to the correct vehicles. That way you increase your sales prospects since items of parts compatibility listings have a higher ranking in the search results. Create, edit and delete parts compatibility listings in the *System » Markets » eBay » Parts compatibility listing* menu.
...
.  Carry out the settings. Pay attention to the explanations in table 6.
. *Save* the settings.
...
Use this menu to define domestic and international shipping conditions. In addition to the region of delivery, select specific <<order-processing/fulfilment/preparing-the-shipment#1000, shipping profiles>> or service providers for international shipping. Go to <<order-processing/fulfilment/preparing-the-shipment#, System » Orders » Shipping » Settings » Tab: Countries of delivery>> and check your language settings for the countries that you want to ship your eBay items to. For further information, refer to eBay’s link:https://sellercentre.ebay.co.uk/private/global-shipping-programme[Using the Global Shipping Programme^] page.

[.instruction]
Setting up domestic shipping:

.  Go to *System » Markets » eBay » Business policies » Tab: eBay shipping conditions » Tab: New *.
.  Carry out the settings. Pay attention to the explanations given in table 7.
. *Save* the settings.
...
*Express shipping* = Items are shipped the same day they were ordered, if the order was placed before a certain time. For further information, refer to eBay’s link:https://pages.ebay.co.uk/help/sell/fast-n-free.html[Selling with eBay FAST & FREE^] page.
...
| Select whether the rate tables should be applied. Rate tables must be set up on eBay. For further information, refer to eBay’s link:https://pages.ebay.co.uk/help/pay/shipping-costs.html#tables[Using shipping rate tables^] page.
...
.  Go to *System » Markets » eBay » Business policies » Tab: eBay shipping conditions » Tab: New*.
.  Pay attention to the explanations given in tables 7 and 8 and carry out the settings.
. *Save* the settings.
...
.  Carry out the settings. Pay attention to the explanations given in table 9.
. *Save* the settings.
...
.  Carry out the settings as desired. Pay attention to the explanations given in table 10.
. *Save* the settings.
...
. Carry out the settings. Pay attention to the explanations given in table 11.
. Place a check mark next to the option *Active*.
. *Save* the settings.
...
.  Carry out the settings as desired. Pay attention to the explanations given in table 12.
. *Save* the settings.
...
.  Carry out the settings and pay attention to the explanations given in table 14.
. *Save* the settings.
...
.  Pay attention to the explanations given in table 15 and carry out the settings in the *Settings* tab as desired.
. *Save* the settings.
...
item ID, title, external listing ID, price, currency, variation ID, variation number, eBay URL, image URL
...
|*Special sale*
| Special sales set up for the listing are displayed here. Create a new link:http://pages.ebay.de/help/sell/items_on_sale.html[special sale^] directly on eBay. Special sales will be available in plentymarkets immediately after they have been created and are automatically synchronised once an hour.
|====
...
.  Carry out the settings as desired. Pay attention to the explanations given in tables 19 to 21.
. *Save* the settings.
...
| Select whether the unit price should be displayed and where it should be displayed. For listings of the *Fixed price* type or *Buy it now listings *with *Weight* or *Volume* options, the link:http://pages.ebay.de/help/sell/unit-price.html[unit price^] must be entered. Values for *units* which are saved for an item are automatically transferred to eBay.
...
Uploading listings with variations is only possible on certain platforms, e.g. Germany, USA, United Kingdom and Italy. Additionally, the function is restricted to certain categories (e.g. clothes). For further information, refer to eBay’s link:https://pages.ebay.co.uk/help/sell/listing-variations.html[Creating a listing with variations^] page.
...
|*Condition*
| Select the item condition. The options become visible once the category has been selected. Item conditions and descriptions are provided by eBay. For further information, refer to eBay’s link:https://pages.ebay.co.uk/help/sell/item-condition.html[Item conditions by category^] page.
...
|*Best offer*
| Activate to allow best offers for listings of the *Fixed price* type. Best offers are not available in all categories. Best offers are unavailable for offers with variations. For further information, refer to eBay’s link:https://pages.ebay.co.uk/help/sell/best-offer.html[Selling with best offer^] page.
...
|*Gallery*
| Selecting the gallery options. For further information, refer to eBay’s link:https://pages.ebay.co.uk/help/sell/gallery-upgrade.html[About gallery upgrades^] page.
...
For further information, refer to eBay’s link:https://pages.ebay.co.uk/help/sell/promoting_ov.html[Promoting your item with listing upgrades^] page.
...
.  Carry out the settings. Pay attention to the information given in table 23.
...
|*Parts compatibility listing*
| Parts compatibility listing Select a parts compatibility listing. Create parts compatibility listings in the <<Setting parts compatibility listings, System » Markets » eBay » Parts compatibility listing>> menu.
...
.  Select the filters. Pay attention to the explanations given in table 24.
.  Carry out the settings. Pay attention to the information given in table 23.
.  Click on *Execute* to apply the settings to the listings.
...
.  Click on *Group function*. +
→ The *Group function* window opens.
.  Carry out the settings as desired. Pay attention to the explanations given in table 23 in the <<Editing listings, Editing listings>> chapter.
...
All active listings are displayed in an overview in the *Listings » Active Listings* menu. Use the filters for *settings*, *categories* or *directories* to narrow down the listings that are displayed. In addition to the filters described in table 22, the following options are available:
...
.  Select the filters. Pay attention to the explanations given in tables 24 and 26.
.  Carry out the settings. Pay attention to the explanations given in table 27.
.  Click on *Execute* to carry out the settings.

[#6000]
==  Uploading offers with variations

You can upload items with multiple variations as a single eBay offer. In order to create variations, you have to create attributes and link them with your items. Variations are combinations of different attributes. Surcharges are applied to every combination, e.g. size, colour and material. Variations linked with a listing are also transferred to eBay when starting, restarting or updating the listing. For further information, refer to eBay’s link:https://pages.ebay.co.uk/help/sell/listing-variations.html[Creating a listing with variations^] page.
...
In August 2018, eBay introduced Product-Based Commerce. For example, you have to match products from specific eBay categories with ePIDs. Furthermore, you have the possibility to create eBay products in your plentymarkets system if those products are not yet available on eBay.
...
.  Carry out further settings as described in table 2 of the <<Carrying out the basic settings, Carrying out the basic settings>> chapter.
. *Save* the settings.
...
.  Carry out the settings. Pay attention to the explanations given in table 30.
. *Save* the settings.
...
. *Save* the settings.
.  Carry out the settings. Pay attention to the explanations given in table 31.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
. *Save* the settings.
.  Carry out the settings. Pay attention to the explanations given in table 32.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
====
Only reject those returns that violate the terms specified in the menu <<Determining the return policy, System » Markets » eBay » Business policies » Tab: eBay return policy>>.
====
...
. *Save* the settings.
.  Carry out the settings. Pay attention to the explanations given in table 33.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
. *Save* the settings.
.  Carry out the settings. Pay attention to the explanations given in table 34.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
. *Save* the settings.
.  Carry out the settings. Pay attention to the explanations given in table 35.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
. *Save* the settings.
.  Carry out the settings. Pay attention to the explanations given in table 36.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
. *Save* the settings.
.  Carry out the settings. Pay attention to the explanations given in table 37.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
The seller sets the return status to *eBay: Offer partial refund*. +
→ The event procedure *Accept return* is triggered and your decision is transferred to eBay.
...
. *Save* the settings.
.  Carry out the settings. Pay attention to the explanations given in table 38.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
. *Save* the settings.
.  Carry out the settings. Pay attention to the explanations given in table 39.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
. *Save* the settings.
.  Carry out the settings. Pay attention to the explanations given in table 40.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
→ The *Create new event procedure* window opens.
.  Enter a name.
.  Select the *event* listed in table 41.
. *Save* the settings.
.  Carry out the settings. Pay attention to the explanations given in table 41.
.  Place a check mark next to the option *Active*.
. *Save* the settings.
--
...
In the *Orders* tab, you manually retrieve orders. This might be necessary if you do not use the automatic retrieval under *System » Markets » eBay » Settings* or if your token expired and you now want to import the orders from this time period.
...
.Outstanding orders
====
Outstanding orders only occur if the option *Shipping costs for new orders coming from eBay* was set to *Transfer from eBay*. This option is found under <<Setting up the checkout process, System » Listings » Checkout>>. It is only possible to import outstanding orders within 30 days.
====
...
Create special sales directly on eBay. Special sales are automatically synchronised with plentymarkets. If the synchronisation is unexpectedly delayed, then you can manually synchronise the special sales here.
...
====
Go to <<Defining shipping conditions, System » Markets » eBay » Business policies » Tab: eBay shipping conditions>> to select the *shipping discounts for combined payments* that were imported from eBay.
====
...
.  Carry out the settings. Pay attention to the information given in table 42.
...
<<Activating the automatic stock update, Activate automatic stock update>> and *End offers automatically*.
...
Go to *Data » Turbo Lister* to import items. Proceed as described below in order to import the CSV file to plentymarkets.
...
Upload your listings to your country’s eBay platform, make them visible on other eBay platforms and set up international shipping. First, check the language settings for the link::/[order-processing/fulfilment/preparing-the-shipment#100, countries of delivery^] to which you want to ship your eBay items. Select both the *global shipping region* and country-specific <<order-processing/fulfilment/preparing-the-shipment#1000, shipping profiles>> or *shipping providers* for international shipping as the <<Defining shipping conditions, shipping conditions>>.
...
If you want to work with the <<Synchronising stock, stock synchronisation>>, then you need to carry out settings to limit the stock and specify how long the listing should be available.
...
Finally, enter the <<Determining the return policy, return policy>> for all relevant eBay platforms.
...
Detail 0: You have reached your sales limit and may currently not list any further items. You can list up to 95 items per month. If one of your offers ends and the item has not been sold, you can list other items. For further information about selling limits and how to raise the limit, refer to the link:https://www.ebay.co.uk/pages/help/sell/sellinglimits.html[eBay help^]. +
