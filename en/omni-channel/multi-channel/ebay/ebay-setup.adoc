...
Use the template function *GetCrossSellingData* to add Cross-Selling items that are linked to an item to a <<Managing layout templates, layout template>> and <<linking a layout template, link>> this layout template to the desired listings. The template function *GetCrossSellingData* displays the following data for a cross-selling item: *item ID*, *title*, *external listing ID*, *price*, *currency*, *variation ID*, *eBay URL*, and *image URL*.
...
A code example for a potential layout template which uses the template function *GetCrossSellingData* is displayed below. Image X shows the preview of a listing that uses this layout template.
