...
You need to register with **link:http://info.fruugo.com/merchants/pages/start_selling.asp[idealo^]** before setting up idealo in plentymarkets.
...
Items have to be available for idealo Checkout. This is done by linking the item to the property **CheckoutApproved** and in the **Available** tab of an item in the **<<item/managing-items#40, Item » Edit item>>** menu. First create the property and link it with the item.

[.instruction]
Creating the property CheckoutApproved:

.  Go to <<omni-channel/multi-channel/idealo-checkout#2, **System » Item » Properties » Tab: New property**>>.
...
Set up an **<<basics/automation/event-procedures#, event procedure>>** to automatically send a shipping confirmation to idealo when the outgoing items are booked.
