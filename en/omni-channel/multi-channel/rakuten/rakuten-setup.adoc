= Setting up Rakuten in plentymarkets
:lang: en
...
Carry out the following settings in plentymarkets in order to set up the interface to Rakuten.de. You can set up multiple accounts for the platform.
...
In order to set up the interface and the item export correctly, you have to carry out the following settings.

[#140]
== Defining a sales price

Proceed as described below to define a sales price for the order referrer Rakuten.de. This price will be displayed on Rakuten.de.

[.instruction]
Defining a sales price for Rakuten festlegen:

. Open the menu *System » Item » Sales prices » Open sales price » Tab: Settings*.
. Place a checkmark next to the referrer *Rakuten.de*.
. *Save* the settings.
...
Activate the payment methods *Rakuten payment*, *Rakuten Rechnung* and *Klarna*. This allows payments to be made for orders on Rakuten and displayed in your plentymarkets system. *Rakuten Rechnung* is only necessary for the platform Rakuten.de.
...
[#200]
== Viewing processes in the log and API log

The menus *Data » Log* and *Data » API log* contain an overview of processes that run via the Rakuten interface.

[.subhead]
Viewing the log

Open the menu *Data » Log* to view an overview of event procedures that run via the Rakuten interface.

[.instruction]
Viewing log entries for Rakuten.de

. Open the menu *Data » Log*.
. Select the identificator *Rakuten.de* from the drop-down list *Identifier*.
. Select a filter from the drop-down list *Reference type* if you only want to see entries of a specific reference type. Pay attention to the information in <<table-reference-types>>.
. Select a filter from the drop-down list *Referece value*, if you only want to see entries of a specific reference value. Pay attention to the information in <<table-reference-types>>.
. Click on *Search*.

Use the following reference types and reference values to filter the search results.

[[table-reference-types]]
.Referece types and reference values for Rakuten.de
[cols="1,3,3"]
|====
|Reference type |Explanation |Referenz values

|*action*
|Filters the search results for event procedures.
|*SendOrderStatusInLogistics* = Event procedure *Send preparation for shipping to Rakuten* +
*SendOrderStatusShipped* = Event procedure *Send shipping confirmation to Rakuten* +
*SendOrderTrackingData* = Event procedure *Send tracking information to Rakuten* +
*SendOrderStatusCanceled* = Event procedure *Send cancellation to Rakuten* +
*SendOrderStatusReturned* = Event procedure *Send return to Rakuten*

|*externalOrderId*
|Filters the search results for entries for a specific external order ID.
|

|*orderId*
|Filters the search results for entries for a specific order ID.
|

[.subhead]
Viewing the API log

The menu *Data » API log » Tab: API log* contains a history of all other processes that run via the Rakuten interface. The *Rakuten* format is available for Rakuten.
...
[.indented]
If the automatic CSV import is set up for Rakuten.de, then the new stock will be synchronised by importing the CSV file again. How often synchronisation takes place, is set up in the Rakuten.de backend.
