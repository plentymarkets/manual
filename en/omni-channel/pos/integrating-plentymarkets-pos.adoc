...
The mobile POS solution plentymarkets POS allows you to integrate your bricks and mortar store seamlessly both with your online presence and your plentymarkets system.
...
|**Operating systems**
a|
* Android devices with OS version 4.2.2 and higher, ARM processor and link:https://cdn.discourse.org/plentymarkets/uploads/default/original/3X/5/b/5b3b12c7adea2b46e1fb4ae4d1b42bc4e8e37fd8.png[device classes^]
Small, Normal, Large and X-Large. You can find out the DPI of your mobile device link:http://dpi.lv/[here^].
...
→ POS with the location country Austria can be operated on Android devices only.
...
// * Miura payment terminal (requires a plentyPOS contract)

|**Signature card reader (AT)**
a|
* Location Austria only
a|
* Feitian smartcard reader bR301
|====

== Supported currencies (version 1.7.3 and higher)

plentymarkets POS can be operated in Euro (EUR), Swiss Franks (CHF) and Pound Sterling (GBP). However, you can only operate the POS in the <<order-processing/payment/currencies#30, system currency>>. The system currency is set in the *System » Orders » Payment » Currencies* menu.
...
Once you have booked a POS, you configure it in the plentymarkets back end. You assign a POS to a client by creating and configuring a POS in the **System » Client » Select client » POS** menu. To do so, proceed as described below.
...
**__Note:__** In Germany and other countries, saving the address is a legal requirement. Contact your tax consultant for legal advice. Your tax consultant can advise you on the correct location details. +
**__Location Austria:__** Once a start receipt has been created for the POS, the location country can no longer be changed.
...
**__Tip:__** Click **Generate key** to generate a random key. +
**__Note:__** Existing POS cannot be made tamper-proof. Create a new POS to activate the signature unit.
...
|**Header**
|If the component is selected, the field's content is displayed as a header on all receipts issued by the POS. By default, the header contains the information saved in the **System » Settings » Master data**. Adjust the data if needed.
...
. In the *Comment* area, select the option *Create comment*.
. In the **Mobile** area, select the option **POS**. +
→ The necessary sub-rights are selected automatically.
. In the **Account** area, activate the following rights: +
**Account » Order summary » Show** +
**Account » Contact » Address » Show** +
**Account » Contact » Address » Type » Show** +
**Account » Contact » Show**
. In the **Order** area, activate the following rights: +
**Order » Create** +
**Order » Show** +
. **Save** the settings.
...
. Go to **System » Settings » User » Accounts » Open user**.
...
Check the user guide of the plugin for details.

[#910]
== Saving booking accounts for plentymarkets POS events (version 1.7.3 and higher)

Orders generated in plentymarkets POS are assigned to the <<order-processing/orders/accounting#800, revenue accounts>> saved in plentymarkets. Because deposits, withdrawals and till count discrepancies are not orders, you can save booking accounts for these POS events. This helps you to manage which booking accounts to associate with different POS deposits and withdrawals. In addition, you can save booking accounts for discrepancies resulting from till counts.
...
. Carry out the settings. Pay attention to the information given in <<table-POS-sales-price>>.
. **Save** the settings.
...
For more detailed information and additional rounding examples, refer to the <<order-processing/orders/accounting#,Accounting>> page of the manual.
...
|**Country**
|Select the country in which the POS is to be used from the dropdown list. Default setting is based on the option **Where is the company based (for financial accounting purposes)** in the **<<basics/first-steps/master-data#, System » Settings » Master data>>** menu.
...
2+|*System » Client » Select client » POS » Open POS*

|**Query price if price is 0**
|Activate option.

2+|*System » Settings » User » Accounts » Open account » Tab: Right » Tab: Mobile » POS functions*
...
Gift cards are sold by selling coupon items in plentymarkets. When you sell a coupon item, you provide customers with a coupon code. Customers only can redeem a coupon code if their coupon code is saved in the plentymarkets back end at the time the customers tries to redeem it. You either can generate coupon codes automatically or work with your own coupon codes.
...
* Create a free text field for the campaign and link it in the *System » Orders » Coupons* menu.
* Create gift card items with a VAT rate of 0%.
* Link the gift card item to the campaign.
* *_Optional:_* Generate coupon codes or save your own coupon codes in the back end.
...
.Activate rights
====
For POS users with a variable user class, you need to activate the rights **Create code** and **Show code**. These rights are set in the *System » Settings » User » Open user » Tab: Right » Tab: REST-API* menu.
====
...
.More than one POS
====
Do you have more than one POS and create your own coupon codes? Divide the printed coupon codes between the different POS or create a separate coupon campaign and gift card items to make sure that each coupon code is sold only once.
====

=== Preparing the free text field

To sell gift cards, you need to link the gift card item to a free text field. You define this free text field and assign it to a coupon campaign. A free text field needs to be defined only once.
...
Now, go to **System » Orders » Coupons**. Select this free text field and link it to a coupon campaign. You only need to do this once for all campaigns.
...
* You create one campaign per gift card value. +
...
If you operate more than one POS and want to use your own coupon codes it also may make sense to create separate campaigns for each POS.

Proceed as described below to create a campaign for gift cards:
...
. Carry out the settings. Note <<table-POS-coupon-campaigns>>.
. **Save** the settings. +
→ The new campaign will be created and will appear in the overview.

[[table-POS-coupon-campaigns]]
...
*_Tip_*: The name should contain the name of the POS and the gift card value if applicable.

|**Description**
|Enter a description (optional).

|**Concept: Code**
|Select **Several codes** so that each coupon code can be redeemed only once and any remaining balance is calculated correctly when the gift card is redeemed.
...
*_Tip:_* This value is assigned to codes automatically when you **Generate coupon codes manually** in the **Codes** tab without entering an amount in the **Value** field. +
...
To redeem gift cards in plentymarkets POS, POS users enter the coupon code. A coupon code must be present in the plentymarkets back end at the time it is redeemed.

From the point of view of POS users, the configuration options in plentymarkets result in three possible scenarios:

* The POS generates coupon codes automatically
* The POS user is issued with a list of coupon codes per gift card value that have been saved in plentymarkets. The POS user enters one of these codes when selling a gift card. The value of the coupon code is retrieved.
...
Administratively, it is easiest if POS users generate a <<omni-channel/pos/plentymarkets-pos-for-pos-users#250, random coupon code>> when they sell a gift card in plentymarkets POS. When the POS order is uploaded, the coupon code and the associated gift card value are then imported to the campaign that is linked to the gift card item. The coupon code is printed on the receipt. In addition, POS users can write the coupon code on a decorative gift card.

==== Working with your own coupon codes

Your administrative work increases if you want to hand out coupon codes to POS users for them to enter when they sell items. However, it also has advantages. For instance, you can create decorative gift cards that already contain the coupon code or the coupon code and the gift card value. POS users can then simply hand out these gift cards.

You can save coupon codes in the system either before or during a sale.

Before a sale in the plentymarkets back end:

* You generate coupon codes with fixed values in a campaign before you start selling gift cards.
* You save coupon codes with fixed values in a campaign before you start selling gift cards.

In both cases, you provide POS users with a list of these coupon codes and the gift card value associated with them.

During a sale in plentymarkets POS:

* POS users enter a coupon code that is not yet saved in plentymarkets and the associated gift card value when selling a gift card.

In such cases, you provide POS users with a list of coupon codes. POS users enter the gift card value during the transaction. This value becomes the price at which the gift card is sold.
...
*_Note:_* Do not circulate duplicate coupon codes. If you operate more than one POS, either split the coupon codes or create separate campaigns per POS.
...
*_Tip:_* Use the data format **CampaignCoupon** to import several coupon codes at once. The **Discount value** is imported as the gift card value. As such, you need to create one campaign per gift card value if you want to use the data format.
. Provide POS users with the coupon codes and their value. +
*_Note:_* Do not circulate duplicate coupon codes. If you operate more than one POS, either split the coupon codes or create separate campaigns per POS.
...
* The value of the existing coupon code must match the price of the item. As such, the gift card item must be linked to the campaign with the coupon codes of the correct values. If you create one campaign per gift card value, each campaign is linked to one item only. If you create a campaign with different gift card values, this campaigns is linked to more than one gift card item.
...
*_Tip_*: The sales price must be linked to the order referrer of the POS.
...
* POS users enter a coupon code saved in plentymarkets. The value of the coupon code becomes the price of the gift card.
* POS users either generate a code or save a coupon code and enters the gift card value.
...
[#600]
== Installing the plentymarkets app on mobile devices
...
. Carry out the settings. Note <<table-POS-link-mobile-device>>.
. **Save** the settings. +
→ Device and POS will be linked.

[[table-POS-link-mobile-device]]
.Linking a POS to a device
[cols="1,2"]
|====
|Setting |Explanation
...
(new POS with location in Austria only)
|Select to <<omni-channel/pos/integrating-plentymarkets-pos#900, create the start receipt>> (Startbeleg). This will activate the tamper-proof signature unit for the POS. After the start receipt is created, the location country of the POS cannot be changed.

|**Create end receipt** +
(POS with location in Austria and activated signature unit only)
|Select to <<omni-channel/pos/integrating-plentymarkets-pos#920, create the end receipt>> (Endbeleg). This will deactivate the tamper-proof signature unit for the POS. Once an end receipt has been created, the POS can no longer be operated.
|====
...
. Select the filters. Pay attention to the information given in <<table-POS-cash-journal-menu>>.
. Click on **Search**. +
...
Delete a POS to permanently take it out of commission. After deletion, the transactions generated with the POS remain saved in the system and can be analysed for tax purposes at any time.
...
→ The POS is deleted. +
**__Note:__** If the POS is located in Austria and the signature unit is activated, an<<omni-channel/pos/implementing-plentymarkets-pos#920, end receipt must be created in the app>>. This deletes the POS automatically.

[#230]
== Operating plentymarkets POS in Austria

According to the Cash Register Directive (Registrierkassensicherheitsverordnung, RKSV) points of sale located in Austria must be equipped with a technical signature unit that ensures that POS transaction data is temper-proof, i.e. undeletable and uneditable. The QR code on the POS receipts provide visual proof that the POS is tamper-proof. Find out how to put a POS in operation in Austria.

[#870]
=== Overview: Implementing the POS

For POS located in Austria, you need to carry out the following steps to activate the signature unit required by the RKSV. Check the website of the link:https://english.bmf.gv.at/taxation/Cash_register_folder.html[Austrian Federal Ministry of Finance^] for detailed information on these legal requirements. This page of the manual describes only those steps necessary to implement a plentymarkets POS located in Austria. This description does not form legal or fiscal advice.
...
|plentymarkets POS is tested for the Feitian smartcard reader bR301. Check the website of the Austrian link:https://english.bmf.gv.at/taxation/Cash_Register_and_Receipt_Issuing_Obligation.html#heading_Where_to_find_further__information_on_the_subject[**Federal Ministry of Finance**, window="_blank"] for further information.

|2.
|Initialise the tamper-proof POS
|See <<omni-channel/pos/integrating-plentymarkets-POS#50, Basic settings for PO>>

|3.
|Create the start receipt (Startbeleg)
|See <<omni-channel/pos/integrating-plentymarkets-POS#900, Creating the start receipt (Startbeleg)>>
...
* The POS identification number of the plentymarkets POS. Go to the **System » Client » Standard » POS » Open POS » Tab: Basic settings** to find the **POS identification number**.
...
plentymarkets provides export formats to allow you to comply with these legal requirements. Talk to your tax consultant to determine what data you need to export and archive to comply with local tax laws.
...
*_Tip:_* The file name contains the export range, e.g. the file depExport_9-1-2017-9-2-2017.json contains data from the 9th of January to the 9th of February 2017.

==== Creating a Z2 receipt (Austria)

A Z2 receipt is a monthly signed control receipt with an amount of zero (0.00). The RKSV requires you to create a Z2 receipt at the end of each month. The Z2 report for the month of December also serves as an end of year report. You need to print and archive this yearly receipt every year and scan it with the BMF receipt validation app.
...
. *Save* the settings. +
→ The POS user <<omni-channel/pos/plentymarkets-pos-fpr-pos-users#270, can create Z2 receipts in plentymarkets POS>>.


=== SOS: The signature unit has failed. What do I need to do?

The RKSV stipulates how you need to proceed if the signature unit of your POS fails:

* If your signature unit fails, you need to move on to a POS with a functioning signature unit.
...
In the case of a POS inspection by Austrian tax authorities, you need to be able to generate a zero amount control receipt (Kontrollbeleg) on the spot. This control receipt allows tax inspectors to validate the signature on the POS receipts on site in your store using an app. Any POS user can create such a <<omni-channel/pos/plentymarkets-pos-fpr-pos-users#270, control receipt on demand>>. No additional user rights are necessary to create the control receipt.
