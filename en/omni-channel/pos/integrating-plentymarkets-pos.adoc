...
* Verifone H5000 with printer (operating mode **ZVT**, connection type **TCP/IP**) - requires the link:https://marketplace.plentymarkets.com/en/plugins/payment/verifonezvt_xxx[Verifone ZVT plugin^]
* Verifone CCV VX 680 with printer (operating mode **ZVT**, connection type **TCP/IP**) - requires the link:https://marketplace.plentymarkets.com/en/plugins/payment/verifonezvt_xxx[Verifone ZVT plugin^]
...
|**Print and save gift card receipt** (version 1.7.6 and higher)
...
*_Note:_* Irrespective of the setting you select here, the gift card code is always printed on the normal POS order receipt when a gift card is sold.
...
* *Campaign » Code » Show* +
*_Note:_* Only required if you want to sell gift cards
|====

[#160]
=== Activating POS visibility for POS users

In the **Mobile** tab, you define which functions and key figures will be visible to variable users in the plentymarkets App.
...
These steps are described below. Find more detailed information on creating campaigns and gift cards on the **<<order-processing/orders/coupons#, Coupons>>** page of the manual.

[NOTE]
.More than one POS
====
Do you have more than one POS and create your own gift card codes? Divide the printed gift card codes between the different POS or create a separate coupon campaign and gift card items to make sure that each gift card code is sold only once.
====
...
If you operate more than one POS and want to use your own gift card codes you may also want to create separate campaigns for each POS. Proceed as described below to create a campaign for gift cards:
...
* The POS generates gift card codes automatically
* The POS user is issued with a list of gift card codes for each gift card value that have been saved in plentymarkets. The POS user enters one of these codes when selling a gift card. The value of the gift card code is retrieved.
...
Administratively, it is easiest if POS users generate a <<omni-channel/pos/plentymarkets-pos-for-pos-users#250, random gift card code>> when they sell a gift card in plentymarkets POS. When the POS order is uploaded, the gift card code and the associated gift card value are then imported to the campaign that is linked to the gift card item. The gift card code is printed on the receipt. In addition, POS users can write the gift card code on a decorative gift card.
...
You can save gift card codes in the system either before or during a sale.
...
. Filter the *GeneratedOn* column of the CSV file by the current date. This will result in a list of all newly created gift card codes. +
...
. **Save** the data. +
→ The gift card code is saved and added to the list of coupon codes. +
*_Tip:_* Use the data format **CampaignCoupon** to import several gift card codes at once. The **Discount value** is imported as the gift card value. As such, you need to create one campaign per gift card value if you want to use the data format.
. Provide POS users with the gift card codes and their value. +
*_Note:_* Do not circulate duplicate gift card codes. If you operate more than one POS, either split the gift card codes or create separate campaigns for each POS.
...
* The value of the existing gift card code must match the price of the item. As such, the gift card item must be linked to the campaign with the gift card codes of the correct values. If you create one campaign per gift card value, each campaign is linked to one item only. If you create a campaign with different gift card values, this campaigns is linked to more than one gift card item.
...
plentymarkets provides export formats to allow you to comply with these legal requirements. These exports are designed to conform to German tax laws. Talk to your tax consultant to determine what data you need to export and archive to comply with local tax laws.
