...
|**Header**
|If the component is selected, the field's content is displayed as a header on all receipts issued by the POS. By default, the header contains the information saved in the **System » Settings » Master data**. Make changes as needed.
...
[#3700]
== Preparing Click & Collect

* Customer orders and pays online and picks up the ordered items in the retail outlet.
* Customer orders online and pays in the retail outlet when they pick up the ordered items.

[#2000]
=== Click & Collect information

Note the following information on Click & Collect orders:

* The order ID is needed to search for orders at the POS. +
→ Make sure that your order confirmations state the order ID clearly and that customers are aware that they need this ID to pick up their order in your retail outlet.
* Train POS users to verify the identity of customers who come to pick up orders that were paid for online.
* If you use processes and/or event procedures to automatically process orders generated in the online store and/or on markets, check these automated sequences before you offer customers to order online and pick up their orders in the retail outlet. +
→ Make sure that orders are not automatically set to the status *Outgoing items booked* by an event procedure or a process before the customer picks up the ordered items in the retail outlet.
* Online store orders cannot be edited at the POS. POS users can only book payments or save order notes. +
* Click & Collect orders cannot be paid for using gift cards.
* Click & Collect orders cannot be cancelled at the POS.
* Payments that are booked at the POS to clear Click & Collect orders do not increase the POS’s revenue. +
→ Payments that are not revenue relevant for the POS are saved as POS events of the type *Payment* in the Cash Journal.
* The POS shows that a customer has received the goods they ordered after the items have been booked out. +
→ If problems occur and a customer insists that they have not yet received the items, the order must be checked in the plentymarkets back end. The items may have been booked out by mistake because they automatic processing of orders might have been configured incorrectly.
