...
====
You remain logged in until you tap *Log out* or *Lock* in the main menu or the list of users. That means that you can open and close the app without having to log in again.
====
...
[#400]
=== Locking and unlocking access (version 1.8.3 and higher)
...
*_Tip:_* You can change users even if access to the app is locked. Tap a different user account in the list to change users.

[#390]
=== Logging out (version 1.8.3 and higher)

Proceed as described below to log out from the plentymarkets app and close the app. The option *Log current user back in automatically* allows you to decide if the last user is logged in automatically when you open the app again.

=== Logging out via the main menu

[.instruction]
Logging out via the main menu and exiting the app:

. Tap the menu icon at the top left corner of the app. +
→ The menu is displayed.
. Tap *Log out*. +
→ The *Log out* window is displayed.
. Select a setting for the option *Log current user back in automatically*. Pay attention to the explanations given in <<table-POS-session>>.
. Tap *Log out*.

=== Logging out via the list of users

[.instruction]
Logging out via the list of users and exiting the app:

. Tap the user icon at the top right of the app. +
→ A list of the real names of the users with an app PIN is shown.
. Tap the *Log out* icon at the bottom right of the window. +
→ The *Log out* window is displayed.
. Select a setting for the option *Log current user back in automatically*. Pay attention to the explanations given in <<table-POS-session>>.
. Tap *Log out*.

[[table-POS-session]]
.Setting *Log current user back in automatically*
[cols="1,3"]
|====
|Setting |Explanation

|*Log current user back in automatically*
|Not active (default) = The user is logged out completely and the login details are discarded. The login view is shown. To log in, language, base URL and user details must be entered. +
Active = The app is closed. The active user is logged out from the server. The same user automatically is logged in when the app is started next. No PIN or password is necessary to log in next time the app is started. +
...
. *_Optional:_* Select a different target storage location (version 1.8.2 and higher): +
  .. Tap the item.
...
. *_Optional:_* Add a <<omni-channel/pos/plentymarkets-pos-for-pos-users#100, note to the return>>.
. On the original receipt, cross out the items the customer is returning.
. Tap *Complete and pay out cash* or (optional, version 1.8.3 and higher) *Complete and create gift card*. +
...
→ By default, the *Default repair storage location* of the POS is selected.
. *_Optional:_* Select a different target storage location for the item (version 1.8.2 and higher): +
  .. Tap the item.
  .. Select the option *Change target storage location* from the drop-down list. +
...
. If necessary, change the quantity and the storage locations of other order items.
. *_Optional:_* Add a <<omni-channel/pos/plentymarkets-pos-for-pos-users#100, note to the return>>.
. On the original receipt, cross out the items the customer is returning.
. Tap *Complete and pay out cash* or (optional, version 1.8.3 and higher) *Complete and create gift card*. +
. If the main order contains shipping costs, select if these shipping costs are to be refunded. +
...
. *_Optional:_* Select a different target storage location for the item (version 1.8.2 and higher): +
 ...
 → The storage location is changed. +
 *_Tip:_* Tap on the menu to the top right and then on the *Change target storage locations* option to change the target storage locations of all returned items at once.
 . *_Optional:_* Change <<omni-channel/pos/plentymarkets-pos-for-pos-users#140, the price of the item>>.
 . *_Optional:_* Add a <<omni-channel/pos/plentymarkets-pos-for-pos-users#100, note to the return>>.
