...
. Enter your login details. Note <<table-POS-users-login>>.
. Tap **Log in**. +
...
====
You will remain logged in to plentymarkets POS until you tap **Log out** in the app menu. That means that you can open and close the app without having to log in again.
====
...
[#310]
=== Switching to a different user account (version 1.8.1 and higher)

Version 1.8.1 and higher of the plentymarkets app allows you to quickly switch user accounts. This means that the logged in user does not need to log out before you can log in. Switching users is only possible if one user is logged into the app. The user symbol at the top right shows the user that is currently logged in. The user symbol shows the initials of the real name of the person that is logged in. The icon’s background colour is the one that was chosen for this user account in the plentymarkets back end. You can switch users in the main menu, the POS menu, when creating POS orders and when booking returns. Once you switch users, the same area of the app opens. If the new user does not have rights for this area of the app, the main menu opens instead. Proceed as described below to switch the user account.

[TIP]
.Order details are saved
====
When you change the user account, the new user can finish the order.
====

[.instruction]
Switching to a different user account (version 1.8.1 and higher):

. Click on the user icon at the top right of the app. +
→ A list of the real names of all users with an app PIN is shown. +
*_Note:_* If your name is not shown, pull to refresh the list. If your name is still not shown, that means that no app PIN is saved for your user account in the plentymarkets back end.
...
If you work with item labels, you can also scan an item’s label to add it to the POS shopping cart. You can scan items using the camera of the mobile device or a connected Bluetooth scanner. To do so, proceed as described below.
...
.Cancelling an order
====
You can cancel a POS order directly after you accepted and booked the payment. To do so, tap *Cancel order* in the *Order created* view. Go to <<omni-channel/pos/plentymarkets-pos-for-pos-users#173, POS » Cancel order>> to cancel any completed orders created for this POS since the last Z report.
====
...
*_Tip:_* If you can see the *Exact cash amount* button in the bottom row of the POS’s number pad, you can tap this button instead of entering the amount if a customer pays the exact amount in cash. The exact amount is then automatically inserted in the *Received* field. An admin user must activate this feature for the POS.
...
Each gift card has a unique gift card code. You enter this gift card code during the checkout process to apply the gift card to the items the customer wants to buy. Customers can redeem a gift card code only if the code exists in the plentymarkets system when a customer tries to redeem the gift card.
...
If the plugin *plentyPOS Invoice* is provisioned in the plentymarkets back end, you can also sell items on account to existing customers. However, because you cannot save new customer data in plentyPOS, only existing customers can pay for items by invoice.
...
→ The *Order created* view opens. +
→ The order is created. +
→ The invoice is printed. Alternatively, tap *Print invoice* to print the invoice.
. Tap *New order* to create a new POS order.
...
====
You can email receipts to customers automatically when a receipt is generated. To do so, you need to <<omni-channel/pos/integrating-plentymarkets-pos#70, create an event procedure>> in the plentymarkets back end. Note that for POS orders completed in the offline mode, the receipt is only be sent once the POS order is uploaded.
====
...
* Open the original order and link the return to this original main order (version 1.8 and higher).
* If the main order is unknown or cannot be opened, create a return without a main order.
...
=== Booking a return by linking it to the main order (version 1.8 and higher)

If you know the order ID or the receipt number (external order number) of the original main order, you can open this main order and book the return directly to this order. Proceed as described below to book a return and link it to the main order.
...
==== Complete return: Returning all items of an order

[.instruction]
Returning all items of an order:
...
. Check that you have actually received all order items from the customer.
. On the original receipt, cross out the items the customer is returning.
. Tap *Create return*. +
→ A return and a credit note are created in the plentymarkets back end and linked to the original main order.
. Pay out the return amount.

==== Partial return: Scanning items for a return

[.instruction]
Scanning items for a partial return:
...
. Tap *Search*. +
→ The order data is displayed. +
→ All order items of the order are flagged for return.
. Scan the barcode of the items that the customer wants to return. +
*_Note:_* When you scan the first item, all other items are removed from the return. The scanned item is added to the return with a quantity of *1*. As such, every item to be returned must be scanned individually.
. On the original receipt, cross out the items the customer is returning.
. Tap *Create return*. +
→ A return and a credit note are created in the plentymarkets back end and linked to the original main order.
. Pay out the return amount.

==== Partial return: Manually selecting items for return

When you open an order in the *Return* menu, all order items of the order are flagged for return. As such, you need to remove those order items from the return that the customer does not want to return.

[.instruction]
Manually selecting items for a partial return:
...
. Tap *Search*. +
→ The order data is displayed. +
→ All order items are flagged for return.
...
. If necessary, change the quantity of other order items.
. On the original receipt, cross out the items the customer is returning.
. Tap *Create return*. +
→ A return and a credit note are created in the plentymarkets back end and linked to the original main order.
. Pay out the return amount.
...
. *_Optional:_* Add a note to the return:
  .. Tap the pin. +
  ...
  → The note is saved and is imported to plentymarkets with the return.
...
. Tap *Complete return*. +
→ A return and a credit note without a main order are created in the plentymarkets back end.
. Pay out the return amount.
