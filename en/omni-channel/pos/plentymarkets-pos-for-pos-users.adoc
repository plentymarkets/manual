...
→ The coupon code is printed on the POS receipt.
. Provide the customer with the receipt.
. Document the coupon code you sold. + **__Note:__** Because the value of a gift card can only be redeemed once, each coupon code must be sold once only.
...
Use X reports to gain insights into the revenue generated for this POS since the last Z report during the day. You can create any number of X reports. X reports are not imported to plentymarkets, i.e. their creation is not documented.
...
=== Selling items if the signature unit is faulty
