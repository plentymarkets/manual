...
* Open the original order and link the return to this original main order (version x.xx and higher).
* If the main order is unknown or cannot be opened, create a return without a main order.

=== Booking a return by linking it to the main order (version x.xx and higher)

If you know the order ID or the receipt number (external order number) of the original main order, you can open this main order and remove the items from the order that the customer wants to return. This creates a return and a credit note.
...
*_Tipp:_* Order items shown in grey have already been returned.
...
. Pay out the return amount to the customer.
...
If you do not know the original main order for which the customer wants to return items or cannot open the order, plentymarkets POS also allows you to create a return without linking it to a main order. Proceed as described below to book a return without linking it to the main order.
