...
*_Note:_* In version 1.8.1 and higher, items can be excluded from discounts. Any discounts that would be applied to specific customer classes are also not applied to the order items.
→ The customer details are applied to the order. +
*_Tipp:_* You can revert to the default customer by clicking on the **Reset** icon.
...
*_Note:_* In version 1.8.1 and higher, items can be excluded from discounts. These items are not discounted.
. Tap **Back**.
. Finish processing the POS order as usual.
...
* <<#310, Open the main order and book the return to this original order>>.
* <<#320, If the main order is unknown or cannot be opened, create a return without a main order>>.
...
When you open a main order to book a return, either all or no order items are selected for return. This depends on the option *Return with main order: Manually select order items for return* in basic settings of the POS in the plentymarkets back end.
