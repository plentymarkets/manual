...
|*Full screen mode* +
(Android devices only, version 1.7.9 and higher)
...
*_Tip (version 1.7.9 and higher):_* If you can see the *X* button in the bottom row of the POS’s number pad, you can tap this button, enter the quantity and then scan the item or select a favourites item. The item is placed in the POS shopping cart with the quantity you entered. An admin user must activate this feature for the POS.
...
====
Orders with the payment method POS invoice can be cancelled until the next z report is generated. However, for technical reasons you cannot return partially or fully unpaid orders with the payment method POS invoice at the moment. Only fully paid orders can be returned. However, the plentymarkets app does not allow you to check if an order has been paid. As such, you need to open the plentymarkets back end to check if an order has been paid in full before you accept a return.
====
...
If you know the order ID or the receipt number (external order number) of the original main order, you can open this main order and book the return directly to this order. This creates a return and a credit note.

Proceed as described below to book a return and link it to the main order.
...
==== Partial return: Returning specific items of an order
