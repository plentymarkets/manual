...
[#310]
=== Switching to a different user account (version 1.xx and higher)

Version 1.xx and higher of the plentymarkets app allows you to quickly switch user accounts. This means that the logged in user does not need to log out before you can log in. Switching users is only possible if one user is logged into the app. The user symbol at the top right shows the user that is currently logged in. The user symbol shows the initials of the real name of the person that is logged in. The icon’s background colour is the one that was chosen for this user account in the plentymarkets back end. You can switch users in the main menu, the POS menu, when creating POS orders and when booking returns. Once you switch users, the same area of the app opens. If the new user does not have rights for this area of the app, the main menu opens instead. Proceed as described below to switch the user account.
