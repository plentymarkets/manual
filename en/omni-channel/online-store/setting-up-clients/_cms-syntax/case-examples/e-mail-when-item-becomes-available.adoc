...
.  Create a new <<omni-channel/online-store/setting-up-clients/cms-syntax#email-email, e-mail template>> in HTML format.
.  Copy the example text (_Message_). Open the tab *E-mail message » Content » Text in HTML format* with the syntax editor and insert the text into the text field.
...
.  Select the e-mail template from the drop-down list *Notification when item is in stock*.
