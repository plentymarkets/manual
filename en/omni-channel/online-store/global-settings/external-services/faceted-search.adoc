...
**plentymarkets » Settings » Client (store) » Global settings » Services » Faceted search**

Facets are the various filter possibilities that are used to narrow down search results in the online store by specific criteria (facets). The faceted search has the advantage that search results are not necessarily limited by the selected criteria. Rather, the criteria are always searched for in the entire database. In other words, users are led through the search process intuitively and search results do not have to be faked if customers select criteria that are not helpful. Offer customers the faceted search to improve usability in your online store. The search only has a one hour delay in data synchronization, so it is always up-to-date.
...
image::omni-channel/online-store/global-settings/external-services/assets/EN-Client-Global-Services-Facets-01.png[]

__Image 1: example of the faceted search in the online store__

[TIP]
.Linking items to facets
====
You do not have to directly link items to the individual facets. Rather, items are automatically assigned to facets. This is done by <<omni-channel/online-store/global-settings/external-services/faceted-search#2-3, **giving facets values** >> that are then saved in the item data record.
====

== Configuration

This function is configured in several different steps. First, facets are created. Then their settings are customized further. Finally, they are linked to the desired values. Furthermore, you need to go to **Settings » Client (store) » Standard » Online store » Item search** and select the option **Faceted search**.
...
. Go to **Settings » Client (store) » Global settings » Services » Faceted search**.
. Click on **New** to add a new facet.
. Enter a **name** for the facet in the system's language.
. Select a **type** from the drop-down menu. Pay attention to the explanations given in table 1.
...
|**Availability**
|Facets of the type **Availability**, e.g. "ships within 24 hours", are linked to the availabilities found under <<omni-channel/online-store/global-settings/external-services/faceted-search#180, **Settings » Item » Item availability** >>.

|**Manufacturer**
|Facets of the type **Manufacturer**, e.g. "Adidas", are linked to the manufacturers found under <<omni-channel/online-store/global-settings/external-services/faceted-search#560, **Settings » Item » Manufacturers** >>.
|====

__Table 1: available facet types__

=== Configuring facets

The facet opens when it is created. There are four tabs for carrying out additional settings and linking values. This chapter explains the settings in the tabs **Settings** and **Name**. Explanations of the tabs **Values** and **Images** are found in <<omni-channel/online-store/global-settings/external-services/faceted-search#2-3, **chapter 2.3** >>.
...
. Go to **Settings » Client (store) » Global settings » Services » Faceted search**.
. Open the facet.
. Pay attention to the explanations given in table 2 and configure the **Settings** tab as desired.
. **Save** the settings.
...
__Table 2: editing facets; tab **Settings**__
...
Use the value's **Settings** tab to save a **Position** and **CSS class** for the value, just as was also done for the facet (table 2). +
...
Enter a number into each field of the **Boost** tab. This specifies how each area should be prioritized when displaying search results. To do so, proceed as follows.

image::omni-channel/online-store/global-settings/external-services/assets/EN-Client-Global-Services-Facets-02.png[]

__Image 2: search index; **Boost** tab__
...
. Enter a number between 1 and 100 into each field. The higher the number, the more this area will be prioritized in the search results. Results will be displayed first for the area with the highest number.
. **Save** the settings.

=== Tab: Keyword

Use the **Keyword** tab to assign the various texts to the areas **Keyword 1** or **Keyword 2**. These texts will be prioritized according to the number that was entered in the Boost tab.

[NOTE]
.Settings in the Keyword tab
====
Select which area (priority) each of the texts listed here should be assigned to. The numbers next to **Category 1 - 6** specify the category level. We recommend prioritizing the higher levels over the lower ones.
====
...
Go to **Settings » Client (store) » Standard / Store » Services » Facet links** to configure the settings. Select which store you want to link to the facets.
...
. Go to **Settings » Client (store) » Standard / Store » Services » Facet links**.
...
The faceted search is defined in the template **NavigationFacetsList**. The template **NavigationFacetsList** is part of our standard design, Callisto Light. This template includes code that you can use to create a faceted search as displayed in image 1. Of course, this code can be modified as needed. +
Use the template function **<<omni-channel/online-store/_cms-syntax/web-design/navigation/container-navigationfacetslist#, Container_NavigationFacetsList  >>** to insert the template **NavigationFacetsList** into the desired area of your online store. To do so, proceed as follows.
...
. Go to <<omni-channel/online-store/_cms/web-design#2-3, **CMS » Web design** >>.
