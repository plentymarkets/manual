...
|*B2B customer class*
|Enter the ID for the default customer class of business customers. For further information, refer to the <<crm/managing-contacs#15, Creating a customer class>> chapter of the manual.
...
Customise the header and the footer of your design. Display your own logo and design the footer content according to your requirements. The store features, for example, are a great way to highlight the features of your online store, such as free shipping. Link content categories in the *Footer* tab to display aditional information in the footer of the online store.
...
Display item lists in a very convenient way in your online store by linking them to *template containers*. Proceed as follows to link the *item list for last seen items* in the single item view of your online store. Link other item lists the same way.
...
*_Manufacturer A-Z_* = Display items in alphabetical order by manufacturer (A-Z). +
//*Manufacturer Z-A* = Display items in alphabetical order by manufacturer (Z-A).  Manufacturer position ⬆ = Display items by manufacturer position in ascending order.  Manufacturer position ⬇ = Display items by manufacturer position in descending order.  Stock ⬆ = Display items by stock in ascending order.  Stock ⬇ = Display items by stock in descending order.
//*_*In order for this function to work, a position number has to be saved for each of the item variations within the variation tab. +
