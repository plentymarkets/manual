...
*Combine variations* = Select to display variations dynamically. Active main variations will be displayed. If the main variation is inactive but has active sub-variations, the sub-variation with the lowest position will be displayed instead. If no position was entered, the variation with the lowest variation ID will be displayed.
...
*Item name A-Z* = Sort items in alphabetical order by item name (A-Z). +
*Item name Z-A* = Sort items in alphabetical order by item name (Z-A). +
*Item position ⬆* = Display items with the highest position number first. +
*Item position ⬇* = Display items with the lowest position number first. +
...
. Select additional languages under *Languages* by holding the Shift key.
. *Save* the settings. +
...
image::omni-channel/online-shop/assets/EN-Setting-up-Ceres-eTrusted-Partner-Certified-Software.png[]
