...
|Enter the ID default customer class for business customers. For further information, refer to the <<crm/managing-customer-data#100, Creating a customer class>> chapter.

|*Enable Callisto route pattern for items*
|Activate to use the Callisto route pattern for items in Ceres. This is required, for example, when using the link:https://forum.plentymarkets.com/t/howto-ceres-checkout-fuer-callisto-einrichten/69276[Ceres checkout with Callisto^].
...
|Select the category types that are displayed in the navigation bar. +
...
|Activate to display variations without stock in the variation drop-down menu in the category view. Variations without stock can be selected and will not be greyed out. When selecting a variation without stock, the customer will be forwarded to the single item view of this variation.
...
* Store specials: Lists with new items, special offers, etc. are displayed in Ceres with the help of tags. For further information, refer to the <<item/managing-items#150, Adding tags to items>> chapter.
* Cross-selling: Cross-selling items are displayed in the online store using *relations*. For further information, refer to the <<item/managing-items#950, Setting up cross-selling>> chapter.
...
|*Columns per page*
|Select the number of columns that is used for calculating the values in the *Items per page* drop-down menu.

|*Rows per page*
|Activate one, multiple or all number values. The activated values are multiplied by the number of columns and displayed in the *Items per page* drop-down menu in the item overview.
...
In the *Currencies* tab, you find settings for the availability and display of currencies in your online store. Here, you specify, e.g. whether your customers are allowed to change the currency in the online store. You also activate the available currencies and how these currencies are displayed.
...
. Activate additional languages under *Languages*.
. *Save* the settings. +
...
Then, create facets for your attributes and properties. For further information about facets, refer to the <<omni-channel/online-store/setting-up-clients/extras/faceted-search#100, Creating facets>> chapter.

Find detailed instructions about setting up filters in our link:https://forum.plentymarkets.com/t/howto-filter-in-ceres-einrichten/46679[forum^] (German link).
...
Find detailed instructions about setting up order properties in our link:https://forum.plentymarkets.com/t/howto-bestellmerkmale-in-ceres-einrichten/63155[forum^].
...
|Enter the Google reCAPTCHA Website key. You receive this key when link:https://www.google.com/recaptcha/admin#list[registring your website for Google reCAPTCHA^].
...
. *Save* the settings.
. Carry out the settings as desired. Pay attention to the information given in <<table-event-procedure-order-confirmation>>.
...
Since Ceres has been pre-certified by EHI, your certification will be handled as fast and easy as possible. You can find all necessary information link:https://ehi-siegel.de/shopbetreiber/ehi-siegel/antrag-auf-shopzertifizierung/[here^].
