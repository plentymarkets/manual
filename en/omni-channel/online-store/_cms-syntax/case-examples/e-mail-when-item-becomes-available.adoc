...
First, create an **<<omni-channel/online-store/_cms-syntax/email/email#, e-mail template  >>**. Once the item is back in stock, this e-mail will be sent to all of the customers who saved their e-mail addresses. Below, you will find a list of template variables and template functions that can be used for this e-mail template. We also provide you with an example text, which you can customize as needed.
...
.  Create a new **<<omni-channel/online-store/_cms-syntax/email/email#, e-mail template  >>** in HTML format.
...
.  Go to **Settings » Client (store) » Standard » E-mail » Automatic e-mail**.
...
Activate the option **Client check** under **Attribute availability** in the menu **Settings » Client (store) » Standard » Item layouts » Basic settings** and add the following code example to dynamically show or hide the previously created form depending on the availability of the variation.
