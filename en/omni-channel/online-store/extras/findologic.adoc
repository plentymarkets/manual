...
**plentymarkets » Settings » Client (store) » Standard » Services » FINDOLOGIC**
...
**link:http://www.findologic.com/pricing[Order the FINDOLOGIC software, window="_blank"]** (the homepage is in German only).
...
====
Go to **Settings » Basic settings » User** and set up a new user account for FINDOLOGIC, which has **API** set as the user class.
...
* The __**URL for the SOAP API WSDL**__ +
This URL can be found in the **Settings » Basic settings » API data** menu under **SOAP WSDL**. The WSDL is structured as follows: **http://WWW.SHOPURL.COM/plenty/api/soap/version1xx/?xml****.**(**xx** = the current SOAP version)
...
* The __**user's login data**__ (see section 2.1) +
...
image::omni-channel/online-store/standard/external-services/assets/EN-Settings-Client-Standard-External-Findologic-01-SI.png[]

__Image 1: FINDOLOGIC **settings**__

[.instruction]
Activating FINDOLOGIC:

. Go to **Settings » Client (store) » Standard » Services » FINDOLOGIC**. +
...
__Table 1: Activating **FINDOLOGIC**__
...
__**Example CSS**__ for FINDOLOGIC:
...
Go to **CMS » Web design » Layout » ItemView » ItemViewSearchResultsList** to carry out the corresponding settings (image 2).

**image::omni-channel/online-store/standard/external-services/assets/EN-Settings-Client-Standard-External-Findologic-02-SI.png[]**

__Image 2: code for search result__
...
__**Example code**__ that you can use for the template:
...
__Table 2: **Template functions** for FINDOLOGIC CODES__

=== Activating the navigation

Go to the **FINDOLOGIC** menu to activate the navigation (see image 1 and table 1).
