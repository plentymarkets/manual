...
**plentymarkets » Settings » Client (store) » Standard » Services » Cliplister**
...
This page contains information about the Cliplister video settings that are available for your individual clients (stores). Before carrying out the configuration, you have to activate Cliplister in the menu **<<omni-channel/online-store/global-settings/external-services/cliplister#, Settings » Client (store) » Global settings » Services » Cliplister  >>**.

== Configuration

The settings available for Cliplister are described below. The configuration has to be carried out __**individually for each client (store)**__. This allows you to save different settings for the **Language**, **Motion picture rating** and **Player in the item layout** for your individual clients (stores).

=== Clip languages

Image 1 shows an overview of all the **languages** that Cliplister currently uses for categorizing its clips.

image::omni-channel/online-store/standard/external-services/assets/EN-Settings-Client-Standard-Services-Cliplister-01-SI.png[]

__Image 1: configuring **languages**__

**Basic rules for configuring the languages:**

* The __**order that the languages are listed in plays a significant role**__, i.e.: when searching for item clips, Cliplister will give priority to a language that is ranked higher up in the list.
* The only clips that will be taken into consideration in the search are the ones that are in the __**marked**__ languages.
* If no value was saved, then the default setting **Layout language / Universally understandable / English** will be used.

In the example in image 1, the __**combination of the order and the marking**__ results in Cliplister primarily searching for clips in the layout language. If Cliplister does not find any clips in the layout language, then universally understandable clips will be used. If no universally understandable clips are available, then English clips will be searched for.
...
Here you enter an age setting that is equivalent to the __**motion picture rating value**__ and thus limits the clips that will be shown in the online store. Enter a whole number or place a check mark next to **The item's motion picture rating** in order to use the motion picture rating saved for the individual item (see image 2).

image::omni-channel/online-store/standard/external-services/assets/EN-Settings-Client-Standard-Services-Cliplister-02-SI.png[]

__Image 2: Cliplister **motion picture rating**__
...
image::omni-channel/online-store/standard/external-services/assets/EN-Settings-Client-Standard-Services-Cliplister-03-SI.png[]

__Image 3: configuring the **player**__

The following table explains the settings in the **Settings » Client (store) » Standard » Services » Cliplister** menu:
...
|**Width / height (pixels)**
|Here, define which __**size**__ you would like the player to be. This entry is based on the size in pixels. Enter a whole number. +
...
|**Design slot number**
|Enter the __**number**__ of a __**design slot**__ that you created in Cliplister. Enter the slot number.

|**Start button**
|This option lets you define whether an available clip should be __**played directly**__ or if the customer should have to __**start the clip by clicking on it**__. Place a check mark in order to have a **start button** be shown. If the following **Pop-up** configuration is activated, then a start button will always be shown.
...
__Table 1: settings for how the **player** should be displayed__
...
. In addition, you can use the template function **CliplisterClipExists()** in order to find out if a video is available ahead of time and to put **<<omni-channel/online-store/_cms-syntax/basics/if#, IF-ELSE logic  >>** into effect.
. Click on the **save icon** to save the settings.
...
Currently, the wildcard character is only available for the **__detailed view of the item__**. Upon demand, Cliplister can also be displayed in other areas, e.g. as a clip in the item list or as a possibility to link with cross-selling items. Ask us about this!
