...
:id: 82PZJ1F
:keywords: pro forma invoice, generate pro forma invoice, order documents, working with documents working with orders, order process, order workflow, document template, template, client, location, order, number, document number, receipt, document type, document, PDF, PDF template, PDF file
...
Before generating pro forma invoices, you have to set up a document template. Go to *Setup » Client » _Select client_ » Locations » _Select location_ » Documents » Pro forma invoice* to set up the template. +
