...
:id: XFYMZBD
:keywords: Warehouse pick list, PDF template, Fulfilment, Order document, Order documents, Order processing, Generating a warehouse pick list, warehouse, fullfilment, fulfillment, storage location, pdf, file, variation, item data, picking process, document, template, quantity
...
Free text fields have to be defined beforehand in the menu *<<items/managing-items#70, Setup » Item » Free text fields>>*. After defining the free text fields, link them to their corresponding items and enter values for the fields. Use the menu <<items/managing-items#170, *Item » Edit item » Default Item » Fields*>>.
