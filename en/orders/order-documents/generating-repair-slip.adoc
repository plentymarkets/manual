...
:id: 7K9NLOI
:keywords: repair slip, generate repair slip, create repair slip, order documents, working with documents, working with orders, order process, order workflow, delivery, document template, template, client, location, order, number, document number, receipt, document type, document, PDF, PDF template, PDF file, repair, return
...
Before generating repair slips, you have to set up a document template. Go to *Setup » Client » _Select client_ » Locations » _Select location_ » Documents » Repair slip* to set up the template. +
