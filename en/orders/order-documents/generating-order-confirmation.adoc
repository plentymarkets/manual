...
:id: SHG0UBG
:keywords: order confirmation, generate order confirmations, order documents, working with documents working with orders, order process, order workflow, delivery, document template, template, client, location, order, number, document number, receipt, document type, document, PDF, PDF template, PDF file
...
Before generating order confirmations, you have to set up a document template. Go to *Setup » Client » _Select client_ » Locations » _Select location_ » Documents » Order confirmation* to set up the template. +
...
| Select by choosing *YES* or *NO* whether value date, payment due or early payment discount are displayed on the order confirmation. +
*Note*: In order for these information to be displayed correctly on the order confirmation, they have to be deposited for the corresponding customer in the *Account* area in the *CRM » Contacts* menu.

| *Payment due*
| If this information is not already deposited in the customer data, enter a number of days in this field to define the payment due date which is displayed on the order confirmation.
...
. Select the *event* listed in <<table-event-procedure-generate-order-confirmation>>.
. *Save* (icon:save[role="green"]) the settings.
. Carry out further settings as described in <<table-event-procedure-generate-order-confirmation>>.
...
→ The *Create new event procedure* window opens.
. Enter a distinct name for the event procedure.
. Select the *event* listed in <<table-event-procedure-send-order-confirmation>>.
. *Save* (icon:save[role="green"]) the settings.
. Carry out further settings as described in <<table-event-procedure-send-order-confirmation>>.
. Place a check mark next to the option *Active*.
. *Save* (icon:save[role="green"]) the settings.
--
