...
| [#intable-default-status-return]*Standard status for return*
...
| [#intable-default-status-credit-note]*Standard status for credit note*
...
| [#intable-default-status-multi-order]*Standard status for multi-invoice*
|Select which status should automatically be set for *<<orders/managing-orders#800, multi-invoices>>*.

| [#intable-default-status-multi-credit-note]*Standard status for multi credit note*
|Select which status should automatically be set for *<<orders/managing-orders#850, multi credit notes>>*.

| [#intable-default-status-reversal]*Standard status for reversal*
...
| [#intable-default-status-order-reservation]*Status for order reservation*
...
| *YES* = Inactive plugin payment methods are displayed in dropdown menus as well as active ones. +
*NO* = Only active plugin payment methods are displayed in dropdown menus.
|====
...
In addition, back end users must be authorised to edit orders in their respective user profile in the <<business-decisions/user-accounts-access#105, Rights » User » Menu visibilities>> menu.
Back end users also need to be authorised to create orders of the type *return*, *warranty* and *repair*. For this, back end users have to be authorised to edit these default status of these order types saved in the *System » System setting » Orders » Settings* menu. The authorisations are assigned in the user profile of the back end user in the <<business-decisions/user-accounts-access#105, Rights » User » Menu visibilities>> menu.
