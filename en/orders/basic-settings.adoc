...
Standard values are selected by default in new systems. For example, these are status [9] for returns and status [11] for credit notes. These settings can be changed by selecting a different status.  Alternatively, go to *<<orders/managing-orders#1200, System » System settings » Orders » Status>>* and create a new status. Statuses that are created there are displayed in the *Basic settings* menu and can be selected as standard statuses.
