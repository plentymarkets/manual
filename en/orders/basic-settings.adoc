...
_Note_: If no valid warehouse can be determined from these settings, the system tries to determine the warehouse with the highest priority that is allowed for the order referrer. If no warehouse is allowed for the order referrer, the warehouse with the highest priority is taken.
