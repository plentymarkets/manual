...
Note that you need at least version 5.0.40 of Ceres/IO to use this setting.
...
You can determine which order information (properties of an order) are transferred into another order. Properties are for example copied when you create delivery orders or returns. Properties are information that are part of an order, e.g. the shipping profile, the payment status or the document language. Properties can belong to an order or to an order item.

Use the menu *Setup » Orders » Property transfer* to determine which properties are to be copied. You determine this individually for different combinations, e.g. for the combination order (source) to delivery order (target). Not all of the properties are available for copying. Use the toggle *Only display transferable properties* to have only those properties displayed you are allowed to activate or deactivate.

[.instruction]
Activating properties for transfer:

. Go to *Setup » Orders » Transfer property*.
. Select for *Order type source* the order type from which the properties are to be transferred.
. Select for *Order type target* the order type to which the properties are to be transferred.
. Activate the properties you want to transfer in the column *Transfer order property*.
. Activate the properties you want to transfer in the column *Transfer order item property*.
