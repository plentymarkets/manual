...
Some orders may not be displayed depending on how the rights were configured for the user. This relates to the **Status**, **Client (store)** and **Warehouse** options that can be restricted in the user configuration. These options are found by clicking on the **Rights** tab and then on the **Visibility** tab. If needed, go to **<</#, Settings » Basic settings » User >>** and check the settings for users who work with the order search.
...
You can see quick information about an order by looking at its title in the overview. You can see the <<orders/settings/order-status#, **status**  >> and logos that show the <</#, **shipping service provider** >> and the <</#, **payment method** >> (image 8, arrow). You may see other icons there as well depending on the configuration and the procedures that were carried out.
...
In our example, the customer is the recipient. You must have created an **<</#, Email template >>** in advance. It is not compulsory to set a **flag**, but this can make sense (image 12, arrow for number 2). Before you carry out the group function by clicking on **Apply** (image 12, number 3), make sure that you have selected all of the desired orders with a check mark (image 12, number 1).
