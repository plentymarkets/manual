...
====
In order for variable users to be able to open orders, go to <<orders/settings/basic-settings#, **Settings » Orders » Basic settings**  >> and save default statuses for **returns** and **credit notes**. No status is saved here as the default setting. Variable users also need to be given the right to edit orders. Go to **Settings » Basic settings » User » Accounts** and open the particular user account. Then click on **<<working-with-plentymarkets/manage-users#110, Tab: Right » Tab: Menu » Area: Orders >>** and allow the user to edit orders.
====

[TIP]
.If an order has POS as referrer, you can edit only very few settings of that order.
====
The editing functions for <<pos/plentymarkets-pos-for-pos-users#7, POS >> orders are limited to comply with legal requirements.
====
...
Go to <<client-store/standard/order-process/basic-settings#, **Settings » Client (store) » Standard » Order process » Basic settings**  >> to activate or deactivate the **Purchaser's IP address** option.
...
You can find an example of how to use dunning levels on the **<<payment/mediafinanz#, mediafinanz  >>** page of the manual.
...
- If the user is not <<working-with-plentymarkets/manage-users#90, **authorised** >> to delete orders +
...
|**New order**
|Creates a <<orders/editing-sales-orders/new-sales-order#, **new order**  >> for the customer. Default settings will automatically be used for the order. You may need to change these settings.
...
|**Bank details**
|Click on the **small plus** to display and edit the customer's <<payment/managing-bank-details#, **bank details**  >>.
...
**__Tip:__** You can also use **<<orders/settings/event-procedures/configuration#, event procedures  >>** to assign storage locations to items.
...
|**Shipped on**
|This is the **release date** that was saved in the item data record. As such, this option is relevant for <<orders/editing-sales-orders/advance-sales-orders#, **advance orders**  >>. If the order includes several items that were all ordered in advance, then the date that is farthest in the future will be displayed here. The date can be changed manually.
...
Check the <<crm/sending-emails#, **access data**  >> if the email was not sent correctly. Go to <<data-exchange/api-log#, **Data exchange » API log**  >> and use the **Email** process to see the error messages.
...
For further information about manually booking payments, refer to the <<payment/managing-payments#, **Managing payments**  >> page of the manual.

[#70]
== Tab: Receipts

You can create receipts in the **Receipts** tab, e.g. an order confirmation, a delivery note or an invoice. Once you have created documents for an order you can no longer edit the input date of the order. The documents are created with the help of document templates that are saved under **<<orders/settings/documents#, Settings » Orders » Documents  >>**. Check the document templates to make sure that the settings are right before you create receipts. Check the templates also if you receive an error message.

[TIP]
.Tip: Copying document templates
====
If you want to create a <<orders/settings/documents#20, **document template** >> quickly, then simply copy a previously configured document template and make changes to the template as needed.
====
...
<<orders/settings/sales-order-types/delivery-orders#, **Delivery orders**  >> are created in the **Delivery** tab. There you can see the order's stock units, which were not yet transferred into delivery orders.
...
The information that can be printed on a reversal document are the same as the ones that you can print on an invoice. Therefore the settings are the same as for an invoice and the settings are explained on the <<client-store/standard/documents/invoice#, **invoice**  >> page of the manual.
...
====
The group function can only be used for the orders that are displayed on one page in the overview. The maximum is 400 orders. Use the dynamic <<data-exchange#, **data exchange**  >> if you want to edit a larger number of orders or if you want to edit all orders at the same time.
====
...
An additional example for using the group function is found on the <<orders/editing-sales-orders/tab-search#, **Order search**  >> page of the manual.
