...
. Carry out the settings. Note the explanations in <<#table-settings-coupon-template>>.
. *Save* (icon:save[role="green"]) the settings.  +
...
This setting has an impact on how the coupon is calculated in the online store. <<orders/coupons#example-calculation-coupons, An example explains the calculation method>>.
...
| *Code length*
|Select the number of characters to generate for gift card codes. Coupon codes can have a length of *6*, *16*, *24* or *32* characters.
...
*_Note:_* Price comparison portals are all referrers > 1. This means that besides price comparison portals, also marketplaces and platforms are included. The referrers *Manual entry* (referrer ID 0) and *Online store* (referrer ID 1) are not affected by these settings.
...
[#example-calculation-coupons]
[discrete]
=== Example for calculating promotional coupons and multipurpose coupons
...
[cols="3,3,3"]
|====
| |Multipurpose coupon |Coupon for special offers
...
This makes it possible to send each customer an automatically generated coupon code. To do so, you have to assign a <<crm/sending-emails#4200, template variable>>, e.g. `$CouponCode1`.
...
[#sell-coupons-in-online-store]
== Selling coupons in your online store

Before you sell coupons in your online store, you first need to create a <<orders/coupons#create-campaign, coupon campaign>> and set the coupon variety to *Multipurpose coupon*. For further information, refer to <<#table-settings-coupon-campaign>>.
