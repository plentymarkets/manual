...
|**Permit** = Coupons are also valid for items coming from <<multi-channel#, **price comparison portals**  >>, i.e. items that are offered simultaneously on other markets.  +
...
|**Clients (stores)**
|Select <<client-store#, **Clients (stores)**  >>. The coupon will be valid for the selected stores. You need to select at least one client.
...
The menu <<orders/editing-sales-orders/tab-search#2-8, **Orders » Edit orders** >> also includes a search filter for coupon codes.
...
Use the data format **<<data-exchange/data-formats/campaigncoupon#, CampaignCoupon  >>** to quickly **<<data-exchange/exporting-data#, export  >>**, edit and **<<data-exchange/importing-data#, import  >>** coupon codes.
...
The particular campaign first has to be assigned a <<cms-syntax/basics/template-variables#, **template variable**  >> for this purpose, e.g. **CouponCode1**.
...
. Find the free text field that was created for coupon campaigns in the menu <<orders/coupons#, **Settings » Orders » Coupons**  >>. In this field, enter the ID of the campaign that you want to link to this item.
. **Save** the settings.
...
. Go to **<<crm/sending-emails#, Settings » Client (store) » Standard » Email » Templates  >>**.
