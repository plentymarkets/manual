...
Before you begin working with coupons, decide which of your items should be purchasable with a coupon or multipurpose coupon.
...
. Click on the <<item/magaging-items#40, Global>> tab.
...
Select *Multipurpose coupon* for campaigns with coupons that are sold via the online store.  +
...
[#25.subhead]
Example for calculating coupons for special offers and multipurpose coupons

In the *Settings* tab of the campaign, you can select the coupon variety *Promotional coupon* or *Multipurpose coupon*. The coupon variety that you select determines how the coupon is calculated in the online store. For example, promotional coupons are calculated similarly to discounts.

[TIP]
.Example for calculating coupons for special offers and multipurpose coupons
====
A customer receives a coupon for EUR 15.00. The customer purchases items in your online store totalling EUR 25.00 excluding VAT. The customer purchases 2 items. One of the items costs EUR 17.95 and is taxed at a rate of 19% VAT. The other item costs EUR 7.05 and is taxed at a rate of 7% VAT. If the customer uses a *multipurpose coupon*, the coupon value will be subtracted from the total amount including taxes. If the customer uses a *promotional coupon*, the coupon value will be subtracted first and the remaining amount will be taxed.
====

[[table-comparison-promotional-coupon-multipurpose-coupon]]
.Comparison of promotional coupons and multipurpose coupons
[cols="3,3,3"]
|====
| |Multipurpose coupon |Promotional coupon
...
| *Coupon variety*
|Refer to the explanations in <<table-settings-coupon-campaign>> and to the <<orders/coupons#25, example>> underneath the table for further information about the coupon varieties *Promotional coupon* and *Multipurpose coupon*.
...
Use ElasticSync in the *Data » Import* menu to import <<data/importing-data/sync-types/elasticSync-coupon-codes#, coupon codes>>. This allows you to import large quantities of coupon codes at once rather than to enter them manually one after the other. To do so, crate a sync of the type *Coupon code*.
...
Before you sell coupons in your online store, you first need to create a <<orders/coupons#20, coupon campaign>> and set the coupon variety to *Multipurpose coupon*. For further information, refer to <<table-settings-coupon-campaign>>.
...
It is useful to send coupons in the form of PDF attachments, e.g. if you offer multipurpose coupons or travel vouchers in your online store.
...
. Carry out the settings as described in <<table-event-procedure-coupon-templates>>.
