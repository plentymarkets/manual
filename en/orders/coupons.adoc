...
|*Variable*
|Select the template variable for <<crm/sending-emails#, email templates>>. The option *No variable* is pre-selected. Once a variable has been selected, it can no longer be used for other campaigns.
|====
...
. Go to *Orders » Coupons*.
. In the *Filter* area, enter the ID or the coupon code or select the filter options in the drop-down lists. Pay attention to the information given in <<table-search-campaign>>.
...
In the *Orders » Coupons* menu in the tab *Campaigns*, the most important information about the created campaigns are displayed in the overview. <<table-campaign-overview>> explains the campaign overview. Use the *Edit view* function and decide which fields are displayed in the overview.

[[table-campaign-overview]]
.Important information in the campaign overview
[cols="1,3"]
|====
|Setting |Explanation
...
|*Export*
|Click on *Export* (icon:download[role="purple"]) at the end of the line in the campaign to export the campaign data to a CSV file.

|*Delete*
|Click on *Delete* (icon:minus-square[role="red"]) to delete the campaign.
...
Use the ElasticSync in the *Data » ElasticSync* menu to import <<data/importing-data/sync-types/elasticSync-coupon-codes#, coupon codes>>. This allows you to import large quantities of coupon codes at once rather than to enter them manually one after the other. To do so, crate a sync of the type *Coupon code*.
...
→ The *Settings* tab opens.
. Select in the *Email template* area, a *variable* from the drop-down list.
. *Save* (icon:save[role="green"]) the settings.
...
The texts are saved in the *System » System settings » Items » Free text fields* menu.
...
. Enter the ID of the campaign that you would like to link to the coupon item in the free text field that you have previously selected for the coupon campaigns in the *System » System settings » Orders » Coupons* menu.
. *Save* (icon:save[role="green"]) the settings.
...
. Mark the document.
. *Save* (icon:save[role="green"]) the settings.  +
*_Tip:_* A success message is displayed confirming that the document was uploaded. If an error message and no success message is displayed after clicking on *Show template* (icon:eye[role="blue"]), contact the support. In some cases, the module for selling coupons may not have been activated in your system.
. Expand the *Coupon template* area (icon:plus-square-o[]).
. Select a character set from the drop-down list.
. Enter values for the X and Y coordinates and for the font size.
. *Save* (icon:save[role="green"]) the settings. +
...
If you sell coupons that you do not want to provide as PDF attachments in the My account area, but would rather send via email, use an event procedure to generate and send the coupons. For the email despatch, you need an <<crm/sending-emails#1200, email template>>. In he email template, select the type *Voucher* for the option *PDF attachment*. The coupon is automatically attached to the email template as a PDF attachment.
