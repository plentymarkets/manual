...
:id: SVWTISV
:keywords: document, template, order document, generating document, generate document, create document, creating document, location-based, fulfillment documents, letterhead, PDF, PDF template, numbering, numbers, tax relevant documents, client, invoice, multi-invoice, delivery note, credit note, adjustment form, dunning letter, return slip, entry certificate, SEPA, SEPA direct debit mandate, reorder, address label, pick list, warehouse pick list, packing list, language, font, character set, page break, currency, margin, positioning, coordinates, document name, order ID, customer ID, referrer, reference, VAT, stock units, POS, data, totals, sum, amount, value, subtotal, net B2B, gross, coupon, barcode, cash, cash on delivery, payment, number range, location, EU, delivery, export, export shipments
...
The following documents are set up and managed in the *Documents* area in the *Setup » Client » _Select client_ » Locations* menu:

* <<orders/order-documents/generating-invoices#, Invoice>>
* <<orders/order-documents/generating-adjustment-forms#, Adjustment form>>
* <<orders/managing-orders#840, Multi-invoice>>
* <<orders/order-documents/generating-delivery-notes#, Delivery note>>
* <<orders/order-documents/generating-credit-notes#, Credit note / Multi credit note>>
...
* <<orders/order-documents/generating-dunning-letters#400, Reversal dunning letter>>
* <<orders/order-documents/generating-repair-slips#, Repair slip>>
* <<orders/order-documents/generating-pick-up-delivery#, Pick-up delivery>>
* <<orders/order-documents/generating-return-slips#, Return slip>>
* <<orders/order-documents/generating-an-entry-certificate-gelangensbestaetigung#, Entry certificate (Gelangensbestätigung)>>
* <<payment/managing-bank-details#200, SEPA direct debit mandate>>
* <<orders/managing-orders#1670, Reversal document invoice>>
* <<orders/managing-orders#540, Reversal document refund>>
* <<orders/order-documents/generating-pro-forma-invoice#, Pro forma invoice>>

[.instruction]
Setting up a location-based document:

. Select a location in the *Setup » Client » _Select client_ » Locations* menu.
. Select a location.
. Click on the menu item *Documents*.
. Select the template you wish to edit, e.g. *Invoice*.
...
Activate the option *Title* if the title saved in the menu point *Adresses* in the <<crm/managing-contacts#100, contact data>> should be positioned in front of the contact name.
...
This area is only displayed if <<crm/managing-contacts#4000, properties>> have been saved for the language of the document template in your system. You can then name the properties using the text fields and position them in the document with the help of the coordinates.
...
In addition, select whether the value of items is displayed summed up for all VAT rates, summed up per VAT rate or per VAT rate if B2B.  +
...
In the *Optional elements below stock unit table* area, you choose which elements appear below the list of order items in the document. <<table-documents-optional-elements-stock-unit-table>> explains these elements, using the *invoice* document as an example.
...
|[#intable-display-payment-terms]*Do you want to use the new payment terms?*
|Select *Yes* if you want to switch to the new logic for the payment terms. Make sure that you transfer your settings before you switch to the new logic. +
Select *No* if you want to use the old logic. _Note_ that the old logic is switched off on _1st Jan. 2021_. Therefore, we recommend switching to the new logic as soon as possible.

|[#intable-payment-terms-documents]*Payment terms* (new logic)
|This field is only displayed if you are using the new logic for payment terms. The old logic will be switched off _1st Jan. 2021_. Therefore, we recommend switching to the new logic as soon as possible. +
...
_Important_: The payment terms will only be displayed on documents if the corresponding order was paid with the _payment method invoice_. If you want to use value date and early payment discount, you must enter a payment due. If there is nothing or the value 0 entered for payment due, the payment terms will not be transferred to documents. +
*Display*: Select if the payment terms are displayed on the document. Enter a *Title* in the free text field for this.

| *Value date/payment due/early payment discount* (old logic)
|_Attention_: This field is only displayed if you are using the old logic for payment terms. The old logic will be switched off on _1st Jan. 2021_. Therefore, we recommend switching to the new logic as soon as possible. +
The template variables are filled with the values saved in the <<crm/managing-contacts#100, customer data>>. Pay attention to sample terms of payment on the <<crm/managing-contacts#100, Managing contacts>> manual page. +
If a value for *value date* is saved for the customer data, this value is used for the document. Otherwise, the value saved for *Payment due* or the value saved in the <<crm/managing-contacts#15, customer class>> is used. The last possibility is to use the number of days entered for the *Payment due* option for the document.

| *Payment due* (old logic)
|_Attention_: This field is only displayed if you are using the old logic for payment terms. The old logic will be switched off on _1st Jan. 2021_. Therefore, we recommend switching to the new logic as soon as possible. +
Select one of the options *YES* or *NO* to determine whether the option is displayed on the document.

| *Early payment discount* (old logic)
|_Attention_: This field is only displayed if you are using the old logic for payment terms. The old logic will be switched off on _1st Jan. 2021_. Therefore, we recommend switching to the new logic as soon as possible. +
...
Use the setting *Value date/payment due/early payment discount* if you wish to display the early payment discount saved in the customer data on the invoice. *_Note:_* The option *Payment due* has to be set to *YES* so that the early payment discount entered in this field is displayed on the invoice.
...
. Go to *Setup » Client » _Select client_ » Locations*.
. Select a location.
. Open the *Documents* menu of the location.
. Select the document whose settings you wish to transfer to another document.
...
. In the *Apply settings from default document template* area, click on *Apply*. +
...
Proceed as described below to upload a PDF file containing the letterhead of your store and link the PDF template to a location-based document template.

[.instruction]
Linking a PDF template to a location-based document template:

. Go to *Setup » Client » _Select client_ » Locations » _Select location_ » Documents*.
. Select the document to which a PDF template should be linked.
. Click on *Add template*. +
→ A separate window containing options for uploading the PDF template opens.
. Select the desired *Payment method* from the drop-down list.
...
Using the example of <<order-documents/generating-invoices#, invoice documents>>, the following instruction describes how to define a number range. Proceed the same way for other documents.

[.instruction]
Defining a number range for invoices:

. Go to *Setup » Client » _Select client_ » Locations » _Select location_ » Documents » Invoice*.
. Select the desired *Language* from the drop-down list.
. Open the tab *Number range*.
. Enter a prefix for the number range consisting of up to 5 numbers and/or letters in the field *Number range*, e.g. *2016*. +
...
For these settings to take effect and be displayed on documents, go to *<<orders/accounting#, Setup » Client » Select client » Locations » Select location » Accounting » Tab: VAT rates>>* and select one of the 4 VAT rates for the option *Margin scheme* for the default country.
The margin scheme setting in plentymarkets is an older customized implementation. The tax rate will be applied to the entire amount of the order items that the margin scheme applies to. In other words, the tax rate will not be applied to the difference between sales price and purchase price.
