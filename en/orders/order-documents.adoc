...
|Select *Yes* if you want to switch to the new logic for the payment terms. Make sure that sou transfer your settings before you switch to the new logic. *LINK LINK LINK LINK* +
Select *No* if you want to use the old logic. _Note_ that the old logic is switched off on 01.01.2020. Therefore, we recommend to switch to the new logic as soon as possible.

|[#intable-payment-terms-documents]*Payment terms*
|The values which are saved in the <<orders/managing-orders#intable-payment-terms-order, payment terms>> of the corresponding order are used. The values can be adapted on the order, if necessary. +
