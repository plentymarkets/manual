...
|You can enter a value as a threshold for deliveries from other countries into the United Kingdom. The threshold indicates up to which amount an order is calculated in gross and with the entered tax rate. The threshold entered here refers to net value of the items of an order, shipping costs are not included. Note, therefore, that the _entered value is net_. Orders with an item net value exceeding the threshold are calculated in net. +
