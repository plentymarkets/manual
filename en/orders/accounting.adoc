...
Either way, it is very important that you make sure to enter the correct data. Otherwise, wrong VAT rates and incorrect configurations could be created which cannot be reversed. Also make sure to enter the highest rate as *Tax rate A* and the others in descending order.
...
*Tax rate E in %* +
*Tax rate F in %*
|Enter the tax rates, starting with the highest rate as *Tax rate A* and continue in descending order. Examples (correctness of tax rates without guarantee): +
For Germany: Tax rate A = 19%, Tax rate B = 7 % +
For France: Tax rate A = 20&, Tax rate B = 10%, Tax rate C = 5,5%, Tax rate D = 2,1% +
*_Important_*: +
...
- Do not enter the same VAT rate twice in one configuration. This leads to errors. +
