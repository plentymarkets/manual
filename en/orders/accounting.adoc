...
|*Threshold in country currency (net)*
|You can enter a value as a threshold for deliveries from other countries into the United Kingdom. The threshold indicates up to which amount an order is calculated in gross and with the entered tax rate. The threshold entered here refers to the net value of the items of an order, shipping costs are not included. Note, therefore, that the _entered value is net_. Orders with an item net value exceeding the threshold are calculated in net. +
The background to this setting is that the UK is a third country from a European perspective and accordingly different import and export regulations apply. Therefore, do not enter a value here for deliveries within the United Kingdom. +
This setting is only displayed when a new United Kingdom configuration is created.
