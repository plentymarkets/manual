...
Learn more about settings in plentymarkets that are relevant for your accounting. These settings include VAT rates, currencies, exchange rates, the calculation of your prices, booking keys, revenue accounts, debtor accounts and just as well your document templates. Most of the settings that are relevant for accounting purposes can be defined per accounting location. Only the setting that defines how your prices are calculated as well as <<orders/accounting#620, the taric numbers>> apply to your entire system. +
...
. Go to *Setup » Client » _Select client_ » POS*.
...
. Select a location under *Setup » Client » _Select Client_ » Locations » _Select location_ » Accounting*. +
→ The *Basic settings* tab will open.
...
====
For Germany, the link:http://www.bzst.de/DE/Steuern_International/Vorsteuerverguetung/02_Unternehmer_Ausland_EU/FAQ/FAQ_node.html[*Federal Central Tax Office*]{nbsp}icon:external-link[] provides tax information about the European countries of delivery as well as the USA and Japan.
====
...
. Select a location under *Setup » _Client_ » _Select client_ » Locations » _Select location_ » Accounting*. +
→ The *Basic settings* tab will open.
...
This can be used, for example, for the temporarily lowered tax rates in Germany. To do this, enter the corresponding revenue accounts in the tax rate configuration which represents the lowered tax rates. By doing this, these have priority. +
_Note therefore_ that the values entered here take precedence over those entered under *Accounts* for the determination of revenue accounts.
...
. Create a <<item/managing-items#500, characteristics>> for digital items and give it a name, e.g. *Digital*. The characteristic may not be an order characteristic.
...
. Open every item that should be sold as digital media and save the characteristic in the <<item/managing-items#500, Characteristics>> tab.
...
_This happens in the background when orders are calculated:_ For example, an order with France as the country of delivery enters the system. VAT rate A is set for the variation of the order item in the order. But in France, this item is taxed with the VAT rate B. That is why there is an entry in the taric code table with the same taric code as the one entered in the variation. The difference is that in the table, this taric code is linked with France and the VAT rate B. The order logic recognises these cases and uses the VAT rate set in the taric code table.
...
In order to import taric codes you can use either ElasticSync or enter them manually. We recommend to use ElasticSync and to enter only single taric codes or carry out small adaptations manually.
_Note_ that only numbers up to 12 characters are allowed. Entering special or blank characters is not allowed.

You can find general information about imports via ElasticSync on the manual page <<data/importing-data/ElasticSync#, Importing data with ElasticSync>>. The <<data/importing-data/sync-types/elasticSync-assignment-taric-code#, sync types>> for importing taric codes can be found under *Assignment taric codes* .
...
With this configuration for revenue accounts for revenues liable to VAT, selected VAT rates are assigned to a specific revenue account. But what do you do if a VAT rate temporarily differs or changes, as is the case for the temporarily lowered VAT rates in Germany, for example? You enter the corresponding revenue account directly in the VAT rate configuration which displays the differing VAT rate. In the system, these then have priority in the determination of revenue accounts.
Also take not of the table entry <<orders/accounting#intable-erlöskonto-optional, Revenue account (optional)>> in the chapter _Setting up VAT rates_.
