...
The values entered on the corresponding order are transferred to the order documents. Note also the <<orders/order-documents#intable-display-payment-terms, document settings>> for this. _Important_: The payment terms will only be displayed on documents if the corresponding order was paid with the _payment method invoice_.
