...
|By activating the toggle you permit the adaptation of VAT rates in the last step of order creation, before completing the order. Changes are then allowed in the <<orders/managing-orders#290, Overview>> step. The VAT rates displayed there are dependent on the delivery address of the order. +
By default, the toggle is deactivated. If you want the VAT rate as read only in the Overview, don't activate the toggle.
...
|*Redirect after completing an order*
|Select where in the system you want to be redirected to after an order is completed. You can choose between the detail view of the just created order, the general order overview or the order overview of the contact. The default setting is to be redirected to the order overview of the contact.
...
|*Cart icon*
|The item is added to the shopping cart by clicking on this cart icon (icon:shopping-cart[role="darkGrey"]). +
This is a default column.

|*Item ID*
|You are redirected to the item data when you click on the item ID. +
This column is not a default column.
...
→ When an item was successfully added, the shopping cart (icon:shopping-cart[role="darkGrey"]) briefly changes size and colour.
...
You can edit the *quantity* as well as the *price*. If a price is entered that is lower than the purchase price, an error message is displayed. You can still set this lower price if you want to.
...
In the order details, the *Shopping method* and the *Shopping costs* can be adapted if required. Apart from the addresses, the currency and the warehouse setting, the order details are editable.

Below the order details, a table with the added order items is displayed. In addition to the information given in the previous adding items view, the prices and the VAT rate are shown. The prices can be edited as long as the order is not completed. The VAT rates are editable if you allowed this in the <<orders/managing-orders#250, default settings>>.

[.collapseBox]
.*Customise table*
--

The table in the last step of the order create is customisable. This means that you can decide which table columns are displayed in which order. The following table columns are selected by default when you go to the purchase order menu:
...
If you change quantities, prices, VAT rates, shipping costs or the shipping method, the order has to be recalculated because the total amounts depend on these factors. Therefore, you have to click on *Recalculate order* before you can complete the order. The total amounts are updated because of the recalculation.
...
Before creating return slips, the document template *Return slip* has to be set up in the menu *Setup » Client » [Select client] » Locations » [Select location] » Documents » Return slip*.
...
====
Before generating credit notes, you have to set up the <<orders/order-documents/generating-credit-notes#, document template for credit notes>>. This is done in the <<orders/order-documents#, Setup » Client » [Select client] » Location » [Select location] » Documents » Credit note>> menu.
====
...
*Owner* +
*Referrer* +
|The order type, the assigned owner as well as the referrer of the order are displayed. All three fields are editable.
...
If a package number was saved, then the *preview* (icon:eye[role="blue"]) can be used for tracking the package. However, this can only be done if a corresponding tracking URL was saved in the <<order-process/fulfillment/preparing-the-shipment#, Setup » Orders » Shipping » Shipping options » Tab: Shipping service provider>> menu. Otherwise, the preview icon will not be visible.
|====
