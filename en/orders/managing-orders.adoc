...
You can combine filters. It is not possible to enter more than one value per filter. Execute the search by clicking on *Search* (icon:search[role="blue"]) or pressing enter. A table with the corresponding items opens.
...
A further editing option of the order items are the properties. To do so, take note of the following chapter <<orders/managing-orders#287, Order item properties>>.
...
* Variation ID
* Barcode
* Properties total amount = The sum of the order item properties surcharges.
...
In order to link a property with an order item, the property has to be marked as *Order property* in the *Options* area of the property settings. Also set possible surcharges and VAT rates for the property in the <<item/settings/properties#property-options>>. The values added here will be included on the order calculations.
...
+
--
The properties are displayed as chips under the order item. Removing a chip deletes the order item property. This means that the property is not included in this order and the order calculations. +
To edit the properties or to add more already existing properties, click on icon:pencil[] underneath *Order item properties*. The same editing window as from the shopping cart as table opens.
...
+
--
The properties are displayed in a second row of the table underneath the corresponding order item. There, you can find information about the *Name*, *Value*, *Surcharge* and *VAT %* of the property. You can edit the *VAT %*, i.e. the saved VAT rate of the property, directly in the table if the property allows for that. Moreover, you can delete (icon:trash-o[]) order item properties in the table. Deleted properties are not included in this order and the order calculations. +
To edit the properties or to add more already existing properties, click on *Edit properties* (icon:pencil[]). The same editing window as from the shopping cart as table opens.
...
Moreover, some further editing options are available to you here. By clicking on *Delete* (icon:trash-o[]), you remove the property from this order item. This property is then not included in the order and the order calculations. +
...
* discount (discount for promotional coupons)
* open amount (amount the customer has to pay)
