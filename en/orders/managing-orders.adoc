...
→ The window *Configure rows* opens.
. Select which rows are to be displayed. Take <<table-overview-item-search>> into account.
...
|*Active / Inactive*
|This column shows whether the variation is set as active or inactive in the webshop. Information about activating or deactivating a variation can be found on the manual page <<item/managing-items#230, Setting a variations availability>>.
...
====
Before generating credit notes, you have to set up the <<orders/order-documents/generating-credit-notes#, document template for credit notes>>. This is done in the <<orders/order-documents#, Setup » Client » _Select client_ » Location » _Select location » Documents » Credit note>> menu.
====
...
_Note_ that created documents cannot be deleted, only external documents. For some receipts, for example invoices or dunning letters, reversal documents can be created. But all documents are stored in the *Receipts* tab once they are created. +
However, it is possible to archive documents. Exempt from this are invoices and documents conditional on invoice. To do so, click on *Archive* (icon:receipt_archive[set=plenty]). The archived documents can be found in the *Archived documents* tab within the *Receipts* tab.
