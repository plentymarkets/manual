...
====
To use delivery orders, you must select the option *b) a warehouse can be assigned for each stock unit* for the *Assignment of warehouses* in the menu *<<orders/basic-settings#, Setup » Orders » Settings>>*.
====
...
====
To open main orders, variable users, variable users must be *<<basics/admin-tasks/manage-users#100, authorised>>* to view all warehouses linked to the delivery orders of the main order. Thus, check the settings in the *Setup » Settings » User » Accounts » Tab: Right » Tab: Visibility* menu.
====
...
====
In plentymarkets, the status *9* is assigned to each return by default. However, you can change the standard status according to your requirements in the *<<orders/basic-settings#, Setup » Orders » Settings>>* menu by selecting another *Standard status for return*.
====
...
|*Status*
|Select a status for the return. The standard status defined in the *<<orders/basic-settings#, Setup » Orders » Settings>>* menu is displayed here by default.
...
Before generating return slips, the document template *<<orders/order-documents/generating-return-slips#, Return slip>>* in the menu *<<orders/order-documents#, Setup » Client » Select client » Locations » Select location » Documents » Return label>>* has to be set up.
...
Before creating credit note orders, go to *<<orders/basic-settings#, Setup » Orders » Settings>>* and check the standard status for credit notes. Change the standard status if necessary and then save your settings.
...
If the conditions for a warranty apply, you as the seller can decide whether to refund the costs to the customer or exchange it for a new item. Proceed with creating a <<orders/managing-orders#500, credit note>> to grant the customer a refund. Create a <<orders/managing-orders#600, warranty>> in your plentymarkets system to replace the damaged item. Go to *<<orders/basic-settings#, Setup » Orders » Settings>>* to determine whether the *shipping costs* should be covered in the warranty.
...
Carry out the basic settings for multi-orders in the *Setup » Orders » Order types » Multi-order*. In this menu, you define details such as the order types and order statuses for which multi-orders can then be created.
...
<<table-basic-settings-scheduler>> explains the settings in the *Setup » Orders » Scheduler* menu:
...
*Important:* Variable users can only select those clients that they have a right to access. Variable users are given rights under *<<basics/admin-tasks/manage-users#, Setup » Settings » User » Accounts » Tab: Right>>*.
|====
...
If orders should only be cleared for shipping when enough stock is available, then go to *<<orders/basic-settings#, Setup » Orders » Settings>>* and set the option *Status changes to 4 instead of 5 if stock is missing* to *YES*. That way, orders remain in status *[4]* until enough stock was booked. The advantage of this is that delivery notes and invoices are only printed for orders that can really be processed.
...
Use the group function in the *Setup » Orders » Status* menu to edit multiple order statuses at a time.
...
====
In order for variable users to be able to open orders, go to *<<orders/basic-settings#, **Setup » Orders » Settings>>* and save default statuses for *returns* and *credit notes*. No status is saved here as the default setting. Variable users also need to be given the right to edit orders. Go to *Setup » Settings » User » Accounts* and open the particular user account. Then click on *<<Tab: Right » Tab: Menu » Area: Orders>>*.
====
