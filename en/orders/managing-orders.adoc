...
| *Split up*
|Splits the order. When splitting an order, 2 new orders are created. The original order is deleted. The shipping costs are recalculated for the new orders. Before splitting the order you are asked how the order items are to be distributed. +
...
By using the group function *Group orders*, you can group certain orders. This means that the selected orders are combined into a new order. The original orders are deleted. Shipping costs are recalculated for the new order. Orders can be grouped when: +
- They are not locked, +
- outgoing items have not been booked yet, +
- no payments have been assigned to the order yet, +
