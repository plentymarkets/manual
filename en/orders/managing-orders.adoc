...
:id: 1HDXOGT
:keywords: order ID, Order processing, Order, Orders, Order status, Order types, Delivery order, Delivery orders, Manually creating orders, Contacts, Customers, Return, Returns, Credit note, Credit notes, Warranty, Warranties, Repair, Repairs, Multi-order, Multi-orders, Advance order, Order Search, Filter options, Order overview, Edit an order, Edit orders, editing orders, generate order, create order, creating order, generating order, order status, advance order, advance orders, searching an order, order overview, referrer, order origin, cash payment, payment method, order document, documents, status, statuses, main status, splitting, split, shipping profile, order item, warehouse, warehouse stock, order item list, event procedure, incoming order, new order, status change, change of status, invoice, creating an invoice, create invoice, generate invoice, invoice generation, invoice creation, generating an invoice, default status, reasons for return, owner, return slip, return slips, create return slip, generate return slip, document template, quantity, quantities, item quantity, reversing stock, incoming items, refund, credit note document, credit note order, repair order, cancel, cancellation, correction, correct, payment, booking payment, incoming payment, amount, currency, shipping costs, replacement, customer class, multi invoice, multi-invoice, multi credit note, purchase order, release date, item availability, maximum order quantity, limitation, payment request, shipping date, default order status, number range, define number range, coupon code, payment status, outgoing items, calculate stock, shipping charges, item list, order overview, dunning letter, dunning level, delivery country, item list, shipping method, shipping service provider, country of delivery, order confirmation, invoice address, delivery address, reorder, order history, status history, client, group function, main order, order details, purchase price, sales price, reversal, reversal document
...
This chapter describes how to create an order or an offer in the contact overview in the *CRM » Contacts* menu. This is a new user interface which is only _beta_ integration at first. All other ways to create orders remain unaffected by this and can still be used.
...
.Default settings for the order/offer creation in the contact overview
[cols="1,3"]
|====
|Setting|Explanation
...
|After order details (e.g. invoice address or payment method) for the new order are entered, items are added in the next step. For this, an item search is used, for which you can select different filters. Set up a filter here which is preselected for this item search. Select from the drop-down list either *No filter preselected*, *Item ID*, *Variation number*, *Variation ID* or *Item name*. Even with a selected default, it is still possible to change the filter in the search if needed.

|*Filter option ‘Only display items in stock’ is preselected in item search*
|Activate this option for it to be preselected in the item search. It means that only items with net stock are displayed in the item search.

|*Filter option ‘Only display sales warehouses’ is preselected in item search*
|Activate this option for it to be preselected in the item search. It means that only items stored in sales warehouses are displayed.
...
Use the drop-down list above the order details to choose between order and offer, even later. The steps described in the following chapters are the same for orders and offers.
...
Clicking on the the preview (icon:eye[role="blue"]) redirects you to the contact data. The already carried out settings in the order details will remain.

|*Delivery address*
|The delivery address is taken from the contact data. If there is more than one address, the one saved as the primary delivery address is taken. If no delivery address is saved as the primary one you can select the right address from the drop-down list. +
Clicking on the the preview (icon:eye[role="blue"]) redirects you to the contact data. The already carried out settings in the order details will remain.
...
|*Payment method*
|If a payment method is defined for the contact, for example via a <<crm/managing-contacts#15, customer class>>, this is preselected. Otherwise select a payment method from the drop-down list. Available are all payment methods integrated in your plentymarkets system.
...
In this step, the order details are not displayed. They are available again in the third step where you finalise the order. Required adaptations can be made there. _But note_ that addresses, currency and the warehouse cannot be edited in the third step.

To complete an order, order items have to be added. Execute an *item search* to add items. You can choose between different filters for the item search. +
...
Additionally, you can activate the options *Only display items in stock* and *Only display sales warehouses*. If you set *a) a warehouse can be assigned for each order* in the basic settings you can only select the option *Only display items in stock* for the search because the warehouse is already determined by the setting. Moreover, the <<orders/managing-orders#250, default settings>> are preselected if you set them.

Click on *Search* (icon:search[role="blue"]) when you have set all the parameters for the search. A table with the corresponding items opens. +
This table is customisable. This means that you can decide which information is displayed in the table rows. To do so, proceed as follows.

[.instruction]
Customise table:

. Click on *Configure rows* (icon:cog[]). +
→ The window *Customise table* opens.
. Select by placing a check mark, which rows are to be displayed. Take <<table-overview-item-search>> into account.
. Move (icon:drag_sort_handle[set=plenty]) the rows, so that they are displayed in the order you need them.
. Click on *Confirm* to save your settings.

When you set the table once, these settings are saved. The table remains like this even when you finalised this order and are creating a new one. Changes to the layout can be made at any time. A list of the available rows can be found in <<table-overview-item-search>>.

Rows of items with net stock are displayed in white. Rows of items not in stock are displayed in red. For example, if you selected the option *Only display items in stock*, none of the rows are displayed in red at first. Only when stock units go below 0 while adding items to the shopping cart, the row is displayed in red. Thus you can see at first glance in which warehouse items are in stock and can finalise the order faster.
...
|*Purchase price*
|The purchase price of the item is displayed.
...
|*Cart icon*
|The item is added to the shopping cart by clicking on the cart icon (icon:shopping-cart[role="darkGrey"]).

|*Item ID / Variation ID*
|You are redirected to the item or variation data when you click on the item ID or the variation ID.
|====

[.instruction]
Adding items:

. Search for the required items by using the filters.
. Enter the quantity of the item you want to add to the order.
. Add the items to the order by clicking on the shopping cart (icon:shopping-cart[role="darkGrey"]) at the end of the row.
...
The number of items added is displayed on the shopping cart icon (icon:shopping-cart[role="darkGrey"]) which is located above the table. If you want to check or delete the added items, open this shopping cart (icon:shopping-cart[role="darkGrey"]).

[.subhead]
Shopping cart

The added items are displayed in the shopping cart. You can edit the *quantity* as well as the *price*. If a price is entered that is lower than the purchase price, an error message is displayed. You can still set this lower price if you want to.

You can also delete items in the shopping cart. To do so, click on *Delete* (icon:minus-circle[role="red"]).

After you added all the items in the required quantity and with the right prices to the shopping cart, click on *Overview*. You could also go back to the order details by clicking on *Order details*. _But note_ that all shopping cart data is lost when you go back.

[#280]
==== Overview: Completing the order

In this last step, the order is completed. The *Total amount gross* and *Total amount net* of the order are displayed above the order details. Also displayed are the number of order items.

In the order details, the *Shopping method* and the *Shopping costs* can be adapted if required. Apart from the addresses, the currency and the warehouse, the order details are editable.

Below the order details, a table with the added order items is displayed. This table is customisable as well. To do so, proceed as described in chapter <<orders/managing-orders#280, Adding items>>. In addition to the information given in the previous adding items view, the *sales price* and the *VAT %* rate are shown. The sales price can be edited as long as the order is not completed.

If you change quantities, prices, shipping costs or the shipping method, the order has to be recalculated because the total amounts depend on these factors. Therefore, you have to click on *Recalculate order* before you can complete the order. The total amounts are updated because of the recalculation.

To complete the order, click on *Complete order*. The order is now created and you are redirected to the order overview of the contact. The order is now also available in the order overview in the *Orders » Edit orders* menu.
...
→ The window *Create new event procedure* opens.
. Assign a distinct name to the event procedure.
. Select an *Event* according to <<table-event-procedure-invoice-generated>>.
. *Save* (icon:save[role="green"]) the settings.
...
To open main orders, variable users have to be <<business-decisions/user-accounts-access#100, authorised>> to view all warehouses linked to the delivery orders of the main order. Check the settings in the menu *Setup» Settings » User »Accounts » Tab: Visibility* for this.
...
Before creating return slips, the document template *Return slip* has to be set up in the menu *Setup » Client » _Select client_ » Locations » _Select location_ » Documents » Return slip*.
...
Because the information in a reversal document is identical with the information included in an invoice, further information on the required settings can be found on the <<orders/order-documents/generating-an-invoice, invoice>> manual page.
...
. Create an <<crm/sending-emails#, email template>>.
...
__**Important**__: When an order has the status >= 7, the status cannot be changed into a status lower than this without reversing the booking of the outgoing items.
...
__**Important**__ The option *Shipped on* in the *Settings* tab has a different function. See table 2.
...
| *Status*
|The order status *[7] Outgoing items booked* cannot be changed to a lower status. This is because the items have already been shipped. However, if you still want to do this, then you first need to reset the outgoing items that were booked. It is possible to switch to a lower order status afterwards.
...
| [#intable-payment-terms-order]*Payment terms*
|Enter values for *Payment due date (days)*, *Value date (days)*, *Early payment discount (days)* and *Early payment discount (%)* either individually or in a combination to create your terms of payment. If the values were already saved in the <<crm/managing-contacts#300, customer data>>, they will be taken from there and automatically be displayed here. The customer data have priority. +
If nothing is saved in the customer data, the customer classes are checked. If values are defined in a <<crm/managing-contacts#15, customer class>>, these are then transferred to the payment terms of the order. If no values are defined in either the customer data or the customer class, the payment terms fields of the order are left empty. +
The payment terms can manually be entered or updated on the order. This can be done regardless of where they are taken from or of whether there are values preselected at all.
...
. Click on *Apply* (icon:cog[]) to carry out the procedure.

[IMPORTANT]
.Default settings for editing orders by variable users
====
In order for variable users to be able to open orders, go to the menu *Setup » Orders » Settings* and save default statuses for *returns* and credit notes. There is no default status saved for *returns* and *credit notes*.
...
In order for variable users to be able to create *Returns*, *Warranties* and *Repairs*, they have to have the right to edit the three default statuses of these order types. The authorisations are assigned in the user profile of the back end user in the <<business-decisions/user-accounts-access#105, Rights » User » Menu visibilities>>.
====
...
If you could place a check mark next to these options, you should be able to edit the order items. In case you are still encountering problems, you can find more detailed information about sales prices in the manual under <<item/settings/prices#, Sales prices and price calculation>> or you can ask our supporters in the link:https://forum.plentymarkets.com/[forum].
