...
Additionally, you can use the filter bookmarks (icon:bookmarks[set=material]) to save selected filters in the UI. These filters are then preselected every time you open the menu. Every user can decide individually which filters to save.
...
* Total amount gross: Shows the calculated total gross amount of the order. The status bar is displayed grey when the amount is zero and green, when the amount is above zero.
* Total amount net: Shows the calculated total net amount of the order. The status bar is displayed grey when the amount is zero and green, when the amount is above zero.
* Total number of items: Shows the total number of order items of this order. The status bar is always displayed in grey.
* Discount: Shows the granted discount (discount for promotional coupons) for this order. The status bar is always displayed in grey.
* Open amount: Shows the open amount the customer still has to pay. The status bar is displayed green when the amount is zero. Red means the amount is above zero and orange means the amount is below zero.
