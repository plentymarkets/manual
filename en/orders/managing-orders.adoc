...
====
Before processing the first orders in your plentymarkets system, adjust the settings in the *<<orders/basic-settings#, Setup » Orders » Settings>>* menu. *Save* (icon:save[role="green"]) those settings once even if you have not changed the default values.
====
...
. Click on *Search* to display a list of variations or use the search fields to search for the icon:search[role="blue"]Item IDicon:search[role="blue"] or other *Item data*.
...
.Same order status
====
The status with the main order is always identical to the status of the delivery order with the lowest status.
For example, if a main order with the status *5.0* has two delivery orders also with the status *5.0*, the status of the main order does not change at first, even if one of the delivery orders changes to status  *6.5*. Only when the status of the second delivery order also changes to  *6.5*, the status of the main order will automatically change to *6.5* as well.
====
...
* *Address label*, *delivery note*, *pro forma invoice* and *entry certificate (Gelangensbestätigung)* can be generated in the *Receipts* tab of the respective order.
* The *invoice* and all other documents are generated centrally in the main order.
...
When working with this event procedure, you must not male any changes to the main order at all. Otherwise, the total item quantities included in the delivery orders does not match the utem quantity included in the main order.
...
[[tableevent-procedure-delivery-order-status-5]]
...
|*Status
|Select a status for the return. The status defined in the menu *Setup » Orders » Settings* is desplayed here by default.
...
|*Reason for return*
|Select an individual reason for return from drop-down menu. +
...
Before generating return slips, the document template *Return slip* has to be set up in the menu  *Setup » Client » Select client » Locations » Select location » Documents » Return label*.
...
Check the list of returned items and change data if necessary, for example the quantities, *before* generating a return slip in the *Receipts* tab of the return order.

[.instruction]
Generating a return slip manually:

. Open the return order and then the tab *Receipts*.
...
====
If a customer has already paid for an order and then decides against the purchase before you have shipped the goods and booked outgoing items, we recommend generating a credit note including all order items in the main order and to cancel the original main order.
...
====
Before generating credit notes, you have to set up the *<<orders/order-documents/generating-credit-notes#, document template for credit notes>>*. This is done in the menu *<<orders/order-documents#, Orders » Fulfilment>>*.
====
...
In order to complete a warranty for which an additional charge is billed for the replacement, an invoice has to be generated within the warranty. In case the customer agrees to a discount on the damaged item instead of proceeding with a return order, you have to create a credit note in the original main order.
...
Proceed as described below to complete a warranty ba creating a credit note order and the corresponding receipt.

[.instruction]
Completing a warranty with a credit note:

. Open the main order or the return order for which you want to generate a credit note.
...
. Click on the icon:gear[role="darkGrey"]gear-wheel iconicon:gear[role="darkGrey"]. +
...
.Checking orders
====
In order to make sure that suitable orders for creating a multi-order are available, use the tab *Orders* in the contact data to look for unpaid orders.
====
...
Advance orders are an important tool in the marketing mix for electronic commerce. Often, items can be ordered at an reduced, introductory price before they actually become available. This represents an unusual case for stock management since items that are not available yet cannot be allocated to a warehouse.
...
. Go to the tab *Settings* of the variation and scroll down to the section *Availability*.
...
====
In order to prevent other online sellers from misusing your advance orders to completely buy out your item a reduced price, enter a value for the option *Maximum order quantity* in the tab *Available*. If you enter the value *1* there, the item can only be purchased once per order. If you set the maximum order quantity to *0*, the item will be sold without limitation.
====
...
. Enter a *name* in the tab *Settings*.
...
The following 28 status entries are available by default in your plentymarkets system and some of them linked to automatic processes in the system. Those statuses linked to an automatic process are not deleteable.
...
|Is automatically assigned when outgoing items of an order were booked. Manually setting this status for an order does not cause the actual booking of outgoing items or the reduction of stock. That is why this status should neither be set manually, via an event procedure, nor via a automated processes.
...
====
If you are searching for numbers or IDs it makes sense not to narrow down the status but set it to *All*. Only those orders are displayed which fulfil all the set criteria. If you cannot find an order despite having entered the exact ID, you might have forgotten to set the filter for *Status* to *ALL*.
====
...
Use the filter *Date* to search for orders that were either in the status *Orders received*, *Incoming payment* or *Outgoing items* during the selected period of time.  After this, choose the start date under *from* and the end date under *to*. All orders which went through the chosen state during the selected period of time are displayed. The filter is set by default to *Orders received* and the time period to the last 2 months.

If you do not want to search for orders within the last 2 months, you can change the time period as described below.

[.instruction]
Searching orders by date:

. Go to *Orders » Edit orders*.
. Select one of the options from the *Date* drop-down list, e.g. *Orders received*.
. Use the *from* and *to* fields to select a start and an end date for the search.
. Select further filter options as required.
. Click on *Search* (icon:search[role="blue"]) to list the orders for the selected time period.

[#1350]
=== Filtering by flags

Irrespective of order statuses, you can also assign flags to orders. You can also search for orders by these flags. Flags can be assigned, for example, to problematic orders so that someone in charge has to clear them before shipping. This person can search for these orders e.g. once a day by using the filter *Flag*, add information and then reset the flag.

[#1360]
=== Filtering by documents

Use the filter *Document* to search for orders for which a specific *<<orders/order-documents#, order document>>* has already been generated. The following two options are available:

To search for all orders with a specific document, proceed as described below:

[.instruction]
Searching for all orders with a specific document:

. Go to *Orders » Edit orders*.
. Select a document from the drop-down list in the tab *Filter*, e.g. *Invoice*.
...
Or you can search for an order containing a document with a specific number. To do so, proceed as described below.

[.instruction]
Searching for a document with a specific number:

. Go to *Orders » Edit orders*.
. Select a document from the drop-down list in the tab *Filter*, e.g. *Delivery note*.
. Enter the number of the delivery note into the field *Number*.
. Take into account the filter for *Date*. By default this is set to the last 2 months. Adjust the filter if required.
. Click on *Search* (icon:search[role="blue"]). +
→ Only the order to which the delivery note belongs is desplayed.
...
. Go to *Orders » Edit orders*.
. In the tab *Filter*, select the setting *Do not show* for the option *Stock unit*.
. Click on *Search* (icon:search[role="blue"]). +
...
|*Email service*
|This icon opens the tab Email templates and thus allows you to directly send emails to customers or to read already sent templates.
...
|*Show reorders*
|Opens the reorders for this order in the menu *Reorder*.
...
All of your orders are displayed in the menu *Orders » Edit orders*. Here you can see the most important information about each order. You are provided with information about the customer, the order status and the payment status. You can also tell which items are included in the order, whether the outgoing items were booked and whether an invoice was generated. In other words, you can easily keep track of the most important processes without having to open the order. Point the cursor over the icons to see tooltips with additional information.
...
**Important:** The option *Shipped on* in the tab *Settings* has a different function. See table 2.
...
The delete icon is not displayed when:+
...
To split orders, go to the menu *Orders » Edit orders*. Open the order you want to split and go to the tab *Overview*.  Orders are only to be split when the following conditions apply:
...
An order’s basic parameters are saved in the tab *Settings*, e.g. the *Status*, *Referrer* or the *Payment method*.
...
. Click on the tab *Email templates* and select the tab that contains the required email template.
...
The tab *Emails sent* provides an overview of all the email templates that were sent. You can see the *date*, *recipient* and name of the *email template* as well as the *order ID*. You can see all of the emails sent to the customer, even those from other orders.
...
Create receipts in the tab *Receipts*, e.g. an order confirmation, a delivery note or an invoice.
...
By clicking on the tab *Customer*, the <<crm/managing-contacts#, contact data record>> opens. These information are editable.

[#1590]
=== Tab: Tickets

The customer's *<<crm/using-the-ticket-system#, tickets>>* are displayed in the tab *Tickets*. You can edit these tickets directly without having to switch to the ticket system.
...
The tab *Stock* displays the *<<stock-management/managing-stocks#, stock data>>* for all of the items that are included in the order.  All of the functions of the *Stock* menu are available in this tab with the exception of the filter functions. You can view stocks and configure them as needed.

[#1620]
=== Tab: Reorder

Select the order items for a reorder in the tab *Reorder* (image 7). Choose whether a reorder is only created for this specific order or whether you want to create a reorder to which items can be added from other orders as well. +
...
. Click on the *shopping cart* (icon:shopping-cart[role="darkGrey"]) in the *reorder* tab of the order. +
...
. Click on the *gear-wheel* ivon (icon:gear[role="darkGrey"]) to create the delivery order.
...
. Go to *Setup » Client » Select client » Locations » Select location » Documents » Reversal document invoiceicon:gear[role="darkGrey", stack="long-arrow-right,darkGrey", stackPosition="lr"].
. Click on the *Transfer* tab.
...
You correct an invoice by reversing it. The reversal of an invoice is used for a transparent correction of relevant invoice data and not for the reversal of the order itself. Once the reversal document was generated, the order settings are unlocked and can be edited and a new invoice can be generated as well.
