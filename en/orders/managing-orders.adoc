...
In plentymarkets, an order is automatically created when customers place orders in your online store. Separate orders with different order types are created for business transactions such as <<orders/managing-orders#300, Delivery order>>, <<orders/managing-orders#400, Return>>, <<+orders/managing-orders#500, Credit note>>, <<orders/managing-orders#600, Warranty>> and <<orders/managing-orders#700, Repair>>. These orders are always linked to the main orders. The <<orders/managing-orders#1400, Order overview>> allows you to open and edit orders regardless of their origin. For many order types, such as <<orders/managing-orders#300, Delivery orders>>, it makes sense to set up <<basics/automating-processes#, Event procedures>> that allow you to automate procedures during order processing. Some examples for using event procedures during order processing are provided in the following chapters. +
Orders that reach you in other ways than via your online store are created <<orders/managing-orders#200, manually>> in your plentymarkets system. For example, this is the case for orders placed via phone, email or in the case of a cash sale.

[NOTE]
.Saving basic settings
====
Before processing the first orders in your plentymarkets system, adjust the settings in the *<<orders/basic-settings#, Basic settings>>* menu. *Save* those settings once even if you have not changed the default values.
====
...
====
To use delivery orders, you must select the option *b) a warehouse can be assigned for each stock unit* for the *Assignment of warehouses* in the menu *<<orders/basic-settings#, System » System settings » Orders » Settings>>*.
====

Before working with delivery orders, go to *System » System settings » Orders » Order types » Delivery order* to select an option for splitting up main orders into delivery orders.

[instruction]
.Selecting an option for splitting up orders into delivery orders

. Go to *System » System settings » Orders » Order types » Delivery order*.
...
====
To open main orders, variable users, variable users must be *<<basics/admin-tasks/manage-users#100, authorised>>* to view all warehouses linked to the delivery orders of the main order. Thus, check the settings in the *System » System settings » Settings » User » Accounts » Tab: Right » Tab: Visibility* menu.
====
...
====
In plentymarkets, the status *9* is assigned to each return by default. However, you can change the standard status according to your requirements in the *<<orders/basic-settings#, System » System settings » Orders » Settings>>* menu by selecting another *Standard status for return*.
====
...
. Go to *System » System settings » Orders » Order types » Return*.
. Enter the desired reasons for return.
. *Save* the settings.

If you allow customers to *<<omni-channel/online-store/setting-up-clients/my-account#_configuration#, register returns themselves>>* in the *My account* area of the Callisto online store, the return reasons will be displayed as a list for the customers to choose from. If the customer does not select any of the return reasons, the first reason for return will be used by default.
...
|*Status*
|Select a status for the return. The standard status defined in the *<<orders/basic-settings#, System » System settings » Orders » Settings>>* menu is displayed here by default.
...
Before generating return slips, the document template *<<orders/order-documents/generating-return-slips#, Return slip>>* in the menu *<<orders/order-documents#, System » System settings » Client » Select client » Locations » Select location » Documents » Return label>>* has to be set up.
...
Credit note orders are used for reimbursing your customers with a certain amount. This can be the case when processing a <<orders/managing-orders#400, return order>>. A credit note in plentymarkets does not represent a reverse invoice but a refund. Credit notes orders are generated from an existing main order in the menu *Orders » Edit orders*. A credit note order is thus always linked to a main order. The credit notes generated in a credit note order are included in your financial accounting such as the *DATEV* export.

[#510]
=== Setting a standard status for credit notes

Before creating credit note orders, go to *<<orders/basic-settings#, System » System settings » Orders » Settings>>* and check the standard status for credit notes. Change the standard status if necessary and then save your settings.
...
→ A window for selecting the order items for the credit note order opens. Use the checkboxes to select particular items or all items. In the *Quantity* field, you can define the item quantity that should be included in the credit note order. 
...
. Go to *System » System settings » Client » Select client » Locations » Select location » Documents » Refund reversal*.
. Click on the *Transfer* tab.
. Click on *Accept* to *Apply the settings from default document template*.

The information that can be printed on a reversal document is identical with the information included in an invoice. Thus, further information on the required settings can be found on the *<<orders/order-documents/generating-an-invoice, invoice>>* manual page.
...
If the conditions for a warranty apply, you as the seller can decide whether to refund the costs to the customer or exchange it for a new item. Proceed with creating a <<orders/managing-orders#500, credit note>> to grant the customer a refund. Create a <<orders/managing-orders#600, warranty>> in your plentymarkets system to replace the damaged item. Go to *<<orders/basic-settings#, System » System settings » Orders » Settings>>* to determine whether the *shipping costs* should be covered in the warranty.
...
All further steps necessary to complete the credit note order are identical to the regular procedure for credit note orders. The further steps to be taken are described in detail in the <<orders/managing-orders#500, credit note>> chapter on this manual page.
...
Carry out the basic settings for multi-orders in the *System » System settings » Orders » Order types » Multi-order*. In this menu, you define details such as the order types and order statuses for which multi-orders can then be created.

[.instruction]
Carrying out basic settings for multi-orders:

. Go to *System » System settings » Orders » Order types » Multi-order*.
...
The first step consists of setting up a document template for multi-invoices in the *System » System settings » Client » Select client » Locations » Select location » Documents » Multi-invoice* menu. In this menu, you can specify detailed settings for the document and you can also save a <<orders/order-documents#, document template>> in the form of a PDF file. Customize the template as desired. Default values are already available in the basic settings of the document template. If needed, you can transfer further settings, e.g. from the document template *Invoice* to complete the values. Further information and instructions for setting up the document template are available on the <<orders/order-documents#, Order documents>> page of the manual. +
...
. Go to *System » System settings » Orders » Scheduler*.
. Pay attention to the explanations given in table 1 and carry out the settings as desired.
. *Save* the settings.

<<table-basic-settings-scheduler>> explains the settings in the *System » System settings » Orders » Scheduler* menu:
...
. Go to **<<omni-channel/online-store/setting-up-clients/my-account#, System » System settings » Client » Select client » My Account » Settings>>**.
...
|*Interval*
|You can choose between the option *All* and the intervals that were selected under *System » System settings » Orders » Scheduler*.
...
*Important:* Variable users can only select those clients that they have a right to access. Variable users are given rights under *<<basics/admin-tasks/manage-users#, System » System settings » Settings » User » Accounts » Tab: Right>>*.
|====
...
The client that is selected determines which <<orders/accounting#, VAT rates>> will be available in the *Item* tab.
...
**System » System settings » Orders » Order status**
...
If orders should only be cleared for shipping when enough stock is available, then go to *<<orders/basic-settings#, System » System settings » Orders » Settings>>* and set the option *Status changes to 4 instead of 5 if stock is missing* to *YES*. That way, orders remain in status *[4]* until enough stock was booked. The advantage of this is that delivery notes and invoices are only printed for orders that can really be processed.

It is especially helpful to work with order statuses in the menu *<<orders/fulfilment#, Orders » Fulfilment>>*. Among other things, invoices are created there. Orders are filtered by their status.
...
. Go to *System » System settings » Orders » Status*.
. Click on *Search*. +
...
Use the group function in the *System » System settings » Orders » Status* menu to edit multiple order statuses at a time.
...
Use the *Document* filter to search for orders for which an *<<orders/order-documents#, order document>>* of a certain type has already been generated.
...
You can see quick information about an order by looking at its title in the overview. You can see the order status and logos that show the *shipping service provider* and the *<<payment#, payment method>>*. You may see other icons there as well depending on the configuration and the procedures that were carried out.
...
====
In order for variable users to be able to open orders, go to *<<orders/basic-settings#, **System » System settings » Orders » Settings>>* and save default statuses for *returns* and *credit notes*. No status is saved here as the default setting. Variable users also need to be given the right to edit orders. Go to *System » System settings » Settings » User » Accounts* and open the particular user account. Then click on *<<Tab: Right » Tab: Menu » Area: Orders>>*.
====
...
Go to *<<omni-channel/online-store/setting-up-clients/order-process#, System » System settings » Client » Select client » Order process » Settings>>* to activate or deactivate the *Purchaser's IP address* option.
...
For an example of using dunning levels, refer to the *<<payment/payment-providers/mediafinanz#, mediafinanz>>* page of the manual.
...
|*Payment history*
|Shows details about *<<payment#, Payments>>* such as *Payment method*, *Currency*, *Payment ID* or *Date*.
...
**__Tip:__**: You can also use *<<automation/event-procedures#configuration, event procedures>>* to assign storage locations to items.
...
If a package number was saved, then the *preview icon* can be used for tracking the package. However, this can only be done if a corresponding tracking code was saved under <<order-process/fulfillment/preparing-the-shipment#, System » System settings » Orders » Shipping » Shipping options » Tab: Shipping service provider>>. Otherwise the preview icon will not be visible.
|====
...
You can create receipts in the *Receipts* tab, e.g. an order confirmation, a delivery note or an invoice. The documents are created with the help of document templates that are saved under *<<orders/order-documents#, Settings » Orders » Documents>>*. Check to make sure that you created the document templates before you create receipts. Check this as well if you receive an error message.

[TIP]
.Tip: Copying document templates
====
If you want to create a <<orders/settings/documents#300, document template>> quickly, then copy a previously configured document template and make changes to the template as needed.
====
