...
→ When an item was successfully added, the shopping cart icon (icon:shopping-cart[role="darkGrey"]) briefly changes size and colour.
...
Depending on the <<orders/managing-orders#intable-shopping-cart-type, shopping cart type>> you selected in the default settings, a sideward overlay opens or the view jumps directly to the shopping cart table. Irrespective of the shopping cart type, you can edit the *quantity* as well as the *price*. If a price is entered that is lower than the purchase price, an error message is displayed. You can still set this lower price if you want to. +
Moreover, you can also delete items in the shopping cart. To do so, click on *Delete* (icon:minus-circle[role="red"]).
