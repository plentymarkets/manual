...
|*Status bet stock*
|In this column, a colour status bar displays whether net stock is available for an item or not. Green indicates that there is net stock for the item in the chosen warehouse. Red indicates that there is no net stock for this item in the chosen warehouse. For example, if you selected the option *Only display items in stock*, none of the status bars are displayed in red at first. The status bar only turns red when stock units go below 0 while adding items to the shopping cart. Thus you can see at once in which warehouse items are in stock.
