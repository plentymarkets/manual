...
Below the order details, a table with the added order items is displayed. This table is customisable as well. To do so, proceed as described in chapter <<orders/managing-orders#280, Adding items>>. In addition to the information given in the previous adding items view, the *sales price* and the *VAT %* rate are shown. The sales price can be edited as long as the order is not completed. The VAT rates are editable if you allowed this in the <<orders/managing-orders#250, default settings>>.
