...
|[#intable-item-search-sales-price]*Sales price*
| All valid sales prices that are determined for this order are available to you in a drop-down list. You can select one of the displayed sales prices, this is then taken as the base of the order calculation. You can manually set a sales price in the <<orders/managing-orders#284, shopping cart>>. +
...
Depending on the <<orders/managing-orders#intable-shopping-cart-type, shopping cart type>> you selected in the default settings, a sideward overlay opens or the view jumps directly to the shopping cart table. Irrespective of the shopping cart type, you can edit the *Quantity* as well as the *Sales price*. As soon as field entries are adapted, recalculations (icon:refresh[set=plenty]) have to be carried out. The following applies to the sales prices:
...
Moreover, you can also delete items in the shopping cart. To do so, click on *Delete* (icon:minus-circle[role="red"]). A further editing option of the order items are the properties. To do so, take note of the following chapter <<orders/managing-orders#287, Order item properties>>.
