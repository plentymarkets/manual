...
When you change the quantity, the sales price changes as well and needs to be updated. To do so, click on *Refresh sales price* (icon:refresh[set=plenty]). After the update the shopping cart button (icon:add_shopping_cart[set=material]) is available again and the items can be added to the shopping cart. +
This is a default column.
