...
If you select a different sales price than the first determined one, the sales price needs to be updated. To do so, click on *Refresh sales price* (icon:refresh[set=plenty]). After the update the shopping cart button (icon:add_shopping_cart[set=material]) is available again and the items can be added to the shopping cart. This also applies to quantity changes. +
...
→ The sales price has to be updated when quantities and prices are changed. Click on *Refersh sales price* to do so.
...
The number of items added is displayed on the shopping cart icon (icon:add_shopping_cart[set=material]) which is located above the table. If you want to check or delete the added items, open this shopping cart (icon:add_shopping_cart[set=material]).
