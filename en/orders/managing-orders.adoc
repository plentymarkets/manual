...
This chapter describes hot to create an order or an offer in the contact overview in the *CRM » Contacts* menu. This is a new user interface which will be a beta integration at first. All other ways to create orders remain unaffected by this and can still be used.
...
.Default settings for the order creation in the contact overview
[cols="1,3"]
|====
|Setting|Explanation
...
The user interface is designed so that orders can be created quickly and easily. This is especially helpful when orders are telephoned in. At first, the order details such as addresses and payment method are entered. After that, order items are added and the last step is the final check and completion of the order.
...
Rows with items with net stock are displayed in white. Rows with items not in stock are displayed in red. For example, if you selected the option *Only display items in stock*, none of the rows are displayed in red at first. Only when stock units go below 0 while adding items to the shopping cart, the row is displayed in red. Thus you can see at first glance in which warehouse items are in stock and can finalise the order faster.

[[table-overview-item-search]]
.Information item search
[cols="1,3"]
|====
|Setting |Explanation

|*Item ID / Variation ID*
|Click on the item ID or the variation ID to be redirected to the item or variation data, if you need to.

|*Variation number*
|Click on the variation number to be redirected to the variation data, if you need to.
...
Below the order details, a table with the added order items is displayed. In addition to the information given in the previous adding items view, the *sales price* and the *VAT %* rate are shown. The sales price can be edited as long as the order is not completed.
