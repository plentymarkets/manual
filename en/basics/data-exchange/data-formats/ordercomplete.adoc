...
The data format **OrderComplete** is used to export orders. This data format can only be used for the export. Create and export the data format in the **Data exchange » Export » Dynamic export** menu. The **<<basics/data-exchange/exporting-data#,Exporting data>>** page of the manual describes how you export the data format.
...
|Debtor account according to your settings in the **<<order-processing/orders/accounting#,Accounting>>** menu
|
...
|**OrderValuta**
|Valuta is the time period that starts when the invoice is created but ends before the payment due time period begins. For further information, refer to **<<crm/managing-customer-data#800, Managing customer data>>**.
|
...
|Activate the filter and select the customer class. Do not activate the filter if you want to export data for all customer classes. Go to **<<crm/managing-customer-data#100,Settings » Customers » Customer classes>>** to set up customer classes.
|====
