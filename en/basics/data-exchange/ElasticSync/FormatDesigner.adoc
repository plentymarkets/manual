...
.Plugin update?
====
ElasticSync is updated and improved constantly. Before creating syncs or export formats, go to *Plugins » Plugin Overview » Open plugin set* to check if a new version of ElasticSync is available.
====
...
|reservedBundles
|Reservations of item bundles

|valueOfGoods
|Value of goods (purchase price *physical stock)
...
|ReorderLevel
|Net stock + amount of reorders + reorder level delta

|deltaReorderLevel
|Necessary amount of reorders to reach the reorder level

|virtualPhysicalStock
|Total physical stock; stock of all warehouses with the warehouse type = 0 (Sales)
|====
...
|Shipping profile
|Shipping profile assigned to the order

|Payment method
|Payment method of the order

|Status of incoming payment
|Status of incoming payment of the order
...
|Date of transfer of the order from marketplace
...
|shippingCostsGross
|Amount of shippping costs (gross)
...
|Field |Explanation
|name
|Name of the preset parcel service

2+|*OrderParcelServicePresetName.name*

|German
|Name of the preset order parcel service (German)

|English
|Name of the preset order parcel service (English)
|====

[#680]
=== OrderParcelService

<<table-fields-order-orderparcelservice>> lists the fields available for exporting order parcel service data.
...
[#800]
== Creating a format of the type order items

=== Order

<<table-fields-orderitem-order>> lists the fields available for exporting order data.
...
<<table-fields-orderitem-order-amount>> lists the fields available for exporting the totality of all calculated amounts belonging to the order.
...
|priceGross
|Actual gross price of the order
...
|referrerId
|Reference of the order item

|referrerName
|ID of the order item reference
...
|shippingProfileId
|ID of the shipping profile of the order item

|countryVatId
|ID of the VAT profile

|vatField
|ID of the field of the VAT profile

|vatRate
|VAT rate that is actually used
...
|External token ID
|ID of the external token of the order item

|External item ID
|External item ID of the order item
...
<<table-fields-order-item-date>> explains the fields available for exporting order item dates.
...
|Created on
|Date at which the order item was created
...
|Completed on
|Date at which the order item was completed

|Returned on
|Date of return of the order item

|Due date for payment
|Date on which payment for the order item is due
...
|Market transfer date
|Date of transfer of the order item from marketplace
...
<<table-felder-orderitem-amount>> lists the fields available for exporting the totality of calculated amounts belonging to the order.

[[table-fields-orderitem-amount]]
...
|isSystemCurrency
|Defines if order item amounts are registered in the system currency.
...
<<table-fields-orderitem-variations>> explains the fields available for exporting variation data.
...
|isMain
|Displays whether the variation is a main variation
...
=======
<<table-fields-orderitem-orderparcelservicepresetname>> lists the fields available for exporting the name of parcel service provider of the order.

[[table-fields-orderitem-orderparcelservicepresetname]]
.Fields for expording the preset parcel service name
[cols="1,3"]
|====
|Field |Explanation
|name
...
|German
|Name of the preset parcel service (German)

|English
|Name of the preset parcel service (English)
>>>>>>> master
|====
...
[#1000]
== Creating a format of the type warehouse


=== Warehouse

<<table-fields-stock-warehouses>> lists the fields available for exporting warehouses.

[[table-fields-stock-warehouses]]
...
|repairWarehouseID
|Assigned repair warehouse

|isInventoryModeActive
|Inventory mode is active

|logisticType
|Assigned repair warehouse (own warehouses, DHL fulfillment, Amazon)
|====

[#1100]
=== WarehouseLocation

<<table-fields-stock-warehouse-locations>> lists the fields available for exporting warehouse locations.

[[table-fields-stock-warehouse-locations]]
.Fields for exporting warehouse locations
[cols="1,3"]
|====
|Field |Explanation
...
|label
|Storage location name (without path)
...
<<table-fields-stock-warehouse-location-levels>> lists the fields available for exporting warehouse location levels.

[[table-fields-stock-warehouse-location-levels]]
.Fields for exporting warehouse location levels
[cols="1,3"]
|====
|Field |Explanation
...
<<table-fields-stock-warehouse-location-dimensions>> lists the fields available for exporting warehouse location dimensions.

[[table-fields-stock-warehouse-location-dimensions]]
.Fields for exporting warehouse location dimensions
[cols="1,3"]
|====
|Field |Explanation
...
|dislplayinname
|Display in storage location name
