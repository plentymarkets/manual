...
* *Import new, update existing data* = If data cannot be found by means of the matching fields, a new data record, e.g. a new item or a new order, is added. At the same time, already existing data is updated.
* *Only update existing data* = Existing data is updated. Data records for which no no data can be found by means of the matching fields is not imported.
* *Only import new data* = Only data for which no matching fields can be identified are imported into the system as new data records.
...
|For owner IDs, see *System » Settings » User*
...
|Flag 1
|Numeric (internal ID)
|ID of flag 1, see *System » Item » Flags » Tab: Flag 1*

|Flag 2
|Numeric (internal ID)
|ID of flag 2, see *System » Item » Flags » Tab: Flag 2*
...
*nofollow_noindex* = Do not follow links and do not include webpage in the search index.

|Translate placehoders
|Alpha-numeric
|*Y* = Yes +
*N* = No +
...
[#2480]
== Setting up a sync of the attribute type
...
[#2510]
== Setting up a sync of the Property type
...
* The Elastic export plugin must be installed, deployed and activated for a client.
...
[#2010]
=== Fields for matching order data
...
[WARNING]
.Mandatory fields for importing order data
====
Pay attention to the fields marked as *Mandatory* in <<table-mapping-fields-order>>. If these fields are not activated, order data are not correctly mapped.
====
