...
|*Message*
|Explanation of the log or, if an error occured, of the error
|====

By clicking on a line, the detail view opens and further infos regarding the log are shown. This window enables you to switch from one entry to the next and you can fold in or fold out the view.

=== Configuring logs

In order to see notifications of the corresponding log areas, you have to activate them first. To do so, go to *Configure logs » Settings*. They are saved on the server for a maximum of 4 weeks. The drop down menu of the log levels lists the different levels with increasing priority. Here you can also set the duration for which you want the log entry to stay active.
...
. Go to *Data » Log*.
. In the toolbar, click on *Configure logs*. +
→ The window *Log configuration* opens. On the left hand side you can see the plugins that are already installed as well as the standard areas. On the right hand side you can carry out the settings.
. Select the area in which you want to configure logs.
. On the right hand side place a check mark in the *Active* box.
. Select the duration for which you want the log level to stay active in this area.
. Select the log level. Pay attention to the information given in the table.
. Save the settings.
...
[cols="1,4,1"]
|====
|Log level |Meaning |Activated by default
...
|*Alert*
|It is necessary to take action immediately (the online store or the data base may not be available).
|Yes
...
On the left hand side you can set different filters that allow you to reduce a large number of log entries to specific matches.
...
. Go to *Data » Log*.
. Click on the field *Integration*. +
...
. Go to *Data » Log*.
. Click on the field *Level*. +
→ A list with different options opens.
. Select a level.
. Click on *Search*. +
→ The corresponding log entries are shown.

*_Note:_* For this, all levels must already be activated.
...
. Go to *Data » Log*.
. Click on the field *Reference type*. +
→ A list with different options opens.
...
. Go to *Data » Log*.
. Click on the field *Reference value*.
