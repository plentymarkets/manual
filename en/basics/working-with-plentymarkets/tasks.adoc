...
|**Description**
| Enter a description to specify what exactly is included in the task. For further information about the editor, refer to the <<omni-channel/online-store/setting-up-clients/cms#web-design-tools-editor, Editor>> page of the manual.
|====
...
.Searching for tasks and overview of tasks
image::basics/working-with-plentymarkets/assets/EN-Start-Tasks-02.png[]

[TIP]
.Default filters display uncompleted tasks
====
By default, the progress is set to **&gt;100%** and the active user is selected from the **Owner** drop-down list. This means that ones own uncompleted tasks are displayed when opening the menu. Click on **Reset** and then on **Search** to remove the filters and display all of the tasks that are saved in the system. Alternatively, select your own search filters (section 3.1).
====
