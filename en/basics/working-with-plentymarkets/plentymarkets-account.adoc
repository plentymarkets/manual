...
The menu **Start » Service center » Tab: Contract** provides you with information about your current plentymarkets contract. Here you can make changes to your contract or you may cancel your contract.
...
. Go to **Start » Service center » Tab: Domain**.
...
Furthermore, go to <<order-processing/payment/managing-bank-details#70, System » Orders » Payment » EBICS>> and save the data there once again, in order to update the basic settings.
====
...
In plentymarkets, the term <<omni-channel/online-store/setting-up-clients#, client (store)>> refers to additional online stores that are all managed with the same plentymarkets system as the default store. Go to **Start » Service Center » Tab: Client (store)** to see all of your clients (stores). If needed, you can also cancel clients (stores) in this menu.
...
Go to **Start » Service center » Tab: Client (store)** and click on **Cancel client (store)** to cancel a client (store). Confirm the cancellation in the window that is displayed. The start and end of the contract will be displayed.
...
Take a look at our link:https://www.plentymarkets.co.uk/blog/Im-Rollout-Mehr-Geschwindigkeit-bei-durchgehender-SSL-Verschluesselung/b-1207/[blog] about this topic (only available in German).
...
.**Deactivate cache temporarily** function
...
.**Permanent SSL redirect** function
...
In the menu **Start » Service center » Tab: Your agency**, you can grant up to two plentymarkets certified service providers access to specific areas of your system. As such, you allow a design agency and/or a consultant to carry out the following tasks in your plentymarkets system:
...
. Go to **Start » Service center » Tab: Your agency**.
...
.Screen seen by authorized agencies
