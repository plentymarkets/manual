...
Users of the type *Back end* can only access limited areas and menus in plentymarkets. Thus, their access to the system is limited. If you want users of this type to open the *Start » Boards* menu, an *Admin* user has to assign them rights as listed in the following.
...
When opening the menu *Start » Boards*, you see the number of cards and columns, the name of the board as well as the time period of the last change on every card that you created. Click on *Rename board* (icon:pencil[role="yellow-background"]) to enter a new name for the board. Click on *Delete board* (icon:minus-square[role="red"]) to delete the board.
