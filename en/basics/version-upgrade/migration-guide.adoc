...
<p>In der neuen Artikelarchitektur wird pro Variante eine systemweit eindeutige Variantennummer vergeben. As such, check if any item or variation numbers were assigned more than once. Replace any duplicate numbers with unique numbers. To do so, use the data format <a href="{% Link(894) %}"><strong>AttributeItemVariation</strong></a> with the following data fields.</p>
...
<p><em>Tab. 9: Data fields data format <b>AttributeItemVariation</b></em></p
////
[#55]
==== Checking EAN/GTIN
...
. Go to **Settings » Client (store)**.
. Click on the plus icon of the first client (store). +
→ The client menu will be displayed.
. Go to **Settings » Client (store) » Open client (store) » Basic settings**.
...
During migration of your system to plentymarkets 7, the data of your external stores will be migrated to fully functional clients (stores). Integration of external Shopware systems with the link:https://github.com/plentymarkets/plentymarkets-shopware-connector/releases[**plentyConnector**, window="_blank"] nehmen Sie nach der Migration vor. Informationen dazu finden Sie in der link:https://docs.google.com/document/d/10mPeV3xqx4We71dYQdPmJK2qvb21Rpym6FG_tKwHKfc/edit#heading=h.h38ew7hip4tl[**plentymarkets Shopware Connector**, window="_blank"].
