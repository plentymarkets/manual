...
→ You can <<basics/working-with-plentymarkets/plentymarkets-account#relocation, **move**  >> your system before migration if plentymarkets hosts your main domain.
...
|The variation that is migrated first receives the item/variation number. A new, unique variation number will be generated for all of the other variations. This variation number is determined by the <<basics/version-upgrade/migration-guide#35, **migration settings** >> found in the menu **Settings » Basic settings » Migration**. It is controlled with the setting **Duplicate number**.

|Exception 3: +
Items with multiple price sets
|One variation can be turned into several different variations. When the first price set is migrated, the variation receives the item/variation number. A new, unique variation number will be generated for all of the other variations. This variation number is determined by the <<basics/version-upgrade/migration-guide#35, **migration settings** >> found in the menu **Settings » Basic settings » Migration**. It is controlled with the setting **Duplicate number**. +
...
**<<basics/version-upgrade/migration-guide#75, Check >>** which price columns were linked to currencies in the menu **Settings » Orders » Payment » Currencies**. Make sure that all of your items have amounts saved in these price columns. +
...
Your previous GTIN, ISBN, etc. were migrated to barcodes. Look a bit **<<basics/version-upgrade/migration-guide#25, farther up >>** in this migration guide if you want to find out how the data was migrated. In plentymarkets 7, your new barcode definitions are found in the menu **Settings » Item » Barcode**.
...
__Table 20: migration status of each market__
