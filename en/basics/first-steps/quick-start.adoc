...
- For example, the class *Admin* contains all of the rights without limitation. At least one user must be an admin. This is because only a user with the user class *Admin* is authorised to manage and edit other user accounts without restriction.
...
Use the <<crm/sending-emails#700,Information service>> to have more control over the emails that are sent by the system. This function can send all outgoing emails as blind copies (BCC) to an email address of your choice. It can also send an email notification every time a new order is placed in your online store. Select the email address the information service should send emails to.
...
|The system generates a new password upon request if the customer loses their login details.
...
|*Subject*
|Enter a sentence to be displayed to the recipient in the subject line of the email. Choose the subject carefully so that the emails will be recognised by the email account and displayed in the customer's inbox instead of their spam folder.
|====
...
|*Flat rate*
|Enter a flat rate charge, e.g. 6.00 EUR. Determine a value above which shipping is free (optional).
...
. Select the clients and/or referrers for which you want to use the image from the drop-down list *Availability*.
. *Save* the settings.
...
You have now created the catalogue of items and set up the categories. Your online store will soon begin to generate revenue. You will now learn how to purchase and install the necessary online store plugins and set up your online store.
...
[TIP]
.Did you book your system after 7 February 2018?
====
...
URL to the image of third slide*
...
Ceres provides you with either a navigation bar with two category levels or a comprehensive mega menu with up to four category levels. Depending on the item catalogue, you may have already created your categories. Now set up a suitable navigation for your online store.
...
Carry out the SEO settings for the *contact page* as well as the pages *Cancellation rights*, *Legal disclosure*, *Privacy policy* and *Terms and conditions* in the same way.
...
Everyone with a little experience in eBusiness likely knows the following problem: Instead of transferring the correct amount of money, the customer has accidentally paid a lower amount. When this happens, you need to notify the customer and ask them to transfer the outstanding amount. In plentymarkets, you can simply define an <<basics/automation/event-procedures#, event procedure>> to automatically process such cases without wasting a lot of time or effort.
...
*Important*: This filter only goes into effect for processes that a status was saved for. It does not go into effect when, for example, manually booking an incoming payment that no status was saved for.
