...
This quick start helps you set up your online store and publish it on the internet. All of the topics are also described in detail in other chapters of the manual. Simply click on the links to access more detailed information.
...
The currently valid tracking URLs of integrated shipping service providers can be found on the <<order-processing/fulfilment/preparing-the-shipment#2500, Preparing the shipment>> page of the manual. You can access the chapters on shipping servie providers via the chapter *Overview of shipping service providers*. Please note that plentymarkets does not assume any responsibility for the accuracy and currency of the tracking URLs. Ask the shipping service provider for the tracking URL.
...
plentymarkets creates all emails based on <<crm/sending-emails#1200, Templates>>. This section teaches you how to create email templates or edit existing templates as needed. The section <<#750, Setting up automatic processes>> teaches you how to have emails sent whenever specific events occur and how to automatically send emails at specific times of day.
...
[[table-recommended-email-templates]]
...
|An important email template to use for legal reasons. You can find further information about sending newsletters on <<crm/sending-newsletters#, Sending newsletters>> chapter of the manual.
|====
...
PDF attachments, such as invoices, are generated in the menu *System » Client » Select client » Locations » Select location » Documents*. Document attachments for other files, such as item brochures or instructions, are saved in the menu *CMS » Documents*.
...
. Click on the *PDF template » Add template* tab.
...
→ Thr countries will be moved to *Selected countries*.
...
|*Clients (stores)*
|Select the online stores or <<omni-channel/online-shop/mandanten-verwalten#, clients>> that the shipping profile should apply to (multiple selection is possible).
...
|Select the link:https://marketplace.plentymarkets.com/plugins/payment[payment methods] and <<crm/managing-customer-data#100,customer class>> that the shipping profile should not apply to (multiple selection is possible).
...
. Click on the small triangle on the right-hand side of *Warehouse*.
...
. In the *Procedures* column, click on *Add*. +
...
You have now created the catalogue of items and set up the categories. There are only a few settings left before your online store can begin generating revenue. Here you will learn how to purchase and install the necessary online store plugins and set up your online store.
...
The <<omni-channel/online-shop/webshop-einrichten/extras#, Services>> chapter lists several partners who can help you create legally sound texts.
...
[TIP]
.Did you book your system after February 07, 2018?
====
...
After you have purchased plugins in the plentyMarketplace, the plugins will be available under *Plugins » Purchases* and can be installed from there.
...
Repeat steps 2 to 4 for the plugins *Ceres* und *Cash in advance*.

[#560]
=== Deploying plugins

The installed plugins are now available in the *Plugins » Plugin overview*. Initially it might be useful to deploy the plugins in the preview mode *Stage*. The preview mode is only accessible for users of your system. The settings that you configure for your online store can then be transferred to the Productive mode. Set the order in which the plugins are loaded before deploying them.

[TIP]
.Plugin position numbers
====
The installeds plugins have to be loaded in a certain order, so that all routes of the online store are working properly. The plugin *IO* contains the logic part of the online store. This plugin receives the highest position number and is thereby loaded last. The online store plugin *Ceres* receives a lower position number than the plugin *IO*. The plugin *Cash in advance* is already assigned the lowest position number *0*.
====
...
Repeat steps 2 to 4 for the *Ceres* plugin. Save a position number for Ceres, e.g. *998*. Select the main client from the dropdown list and activate the plugins for the preview mode *Stage*.

[.instruction]
Deploying plugins in Stage:

. Select the main client from the dropdown list.
. Go to *Plugins » Plugin overview*.
. Click on *Switch to table view* in the toolbar. +
→ The plugins will be displayed in the table view.
. *Save* the settings.
. Activate the *Stage* mode for the *IO* plugin in the *Active* column.
. Repeat step 5 for the plugins *Ceres* and *Cash in advance*.
. Click on *Deploy plugins in Stage* in the toolbar. +
→ The activated plugins are checked and the deployment process starts.

The default template of the online store is available in the preview mode *Stage* after deployment has been completed. Switch to *Stage* in order to access your online store. The *Stage* mode is only available for the main client.

[.instruction]
Accessing the online store in the preview mode:

. Go to *Plugins » Plugin overview*.
. Click on *Switch to Stage* in the toolbar.
. Click on your plenty ID on the right side of the navigation bar of the back end. +
→ The preview mode of the online store opens.

[#570]
=== Setting up plugins

After you have looked at your online store with defaul settings in the preview mode *Stage*, you can now set up plugins according to your desires.
...
[[table-quick-start-number-formats]]
...
. Enter the name of your online store in the *Name of your store* field. +
→ The name of the online store will also be used as the page title.
. Enter the *URL to your company logo*. +
→ The company logo can be uploaded in the *CMS » Webspace* menu in the *layout* folder. Alternatively, you can enter a http-URL that links to the image of your logo.
. *Save* the settings.
...
Ceres provides you with eitehr a navigation bar with two category levels or a comprehensive mega menu with up to four category levels. Depending on the item catalog, you may have already created your categories. Now set up a suitable navigation for your online store.
...
. Go to *Cross-selling: Cross-selling relation* and select the relation for cross-selling items.
. Go to *First item list* and select the option *Cross-Selling* from the dropdown list. +
...
After saving the category settings, the *CategoryTemplates* plugin will be available in the *Plugin » Plugin overview* menu. The plugin has to be deployed in Stage or Productive for the selected client. Then link the category in the configuration of the *Ceres* plugin.
...
Display your contact data and the opening hours of your store on the contact page. Here, you also set up the contact form for customer requests. In order to display the contact form, an email address must be specified in the configuration of Ceres. Customer requests will be sent to this address.
...
Since you have activated the plugin in the preview mode, go to the back end and switch to *Stage*. Then activate the countries of delivery for the *Cash in advance* payment method.
...
→ The cookie for the preview mode is stored and the settings for the *Cash in advance* plugin are loaded.
...
*Important*: This filter only goes into effect for processes that a status was saved for. It does not go into effect when, e.g., manually booking an incoming payment that no status was saved for.
...
* [ ] Did you create all mandatory pages? Cancellation rights and shipping costs, legal disclosure and privacy policy, bank details and terms and conditions? Further information can be found in this link:https://www.plentymarkets.eu/knowledge/video-archiv/arbeiten-mit-plentymarkets/schnelleinstieg/[video tutorial].
...
* [ ] Did you deactivate the <<#1010, test mode>> for email notifications?
...
. Go to *Plugins » Plugin overview*.
. Select the client from the dropdown list in the menu bar.
. Activate the *Productive* mode for the *cash in advance* plugin in the *Active* column.
. Activate the *Productive* mode for the *Ceres* plugin in the *Active* column.
. Activate the *Productive* mode for the *IO* plugin in the *Active* column.
. Click on *Deploy plugins in Productive* in the toolbar. +
→ The activated plugins are checked and the deployment process starts.
