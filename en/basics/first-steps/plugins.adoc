...
. Click on *Add*. +
→ The window *Create a plugin* opens.
...
. Select the online store you would like to preview. +
→ The chosen online store and Plugin Set are displayed in a new tab.
 ...
 *Position*: Define the load order of plugins. If a plugin doesn't work properly, it might help to change the position to a higher number. +
 *Delete*: Delete the plugin form the Plugin Set.
|====

[#100]
== Installing plugins

You can install Marketplace plugins, Git plugins and local plugins in the plugin overview.

[.instruction]
Installing plugins:

. Go to *Plugins » Plugin overview*.
. Click on the Plugin Set for which you want to install a plugin. +
→ The window *Plugin Set details* will open.
. Click on *Install* in the line of the plugin. +
→ The *Install plugin* window will open.
. Select the plugin version (branch for git plugins) in the first drop-down menu. +
→ The text field shows the changelog of the plugin with information of all versions.
. Click on *Install*. +
