...
On Thursday, 24th of May, 2018, plentymarkets is updated in order to manage the conclusion of contracts for processing according to article 28 § 3 of the General Data Protection Regulation (GDPR) in a digital fashion. The contract is concluded digitally in the link:https://marketplace.plentymarkets.com/en[plentyMarketplace^].

[.instruction]
Concluding the contract digitally:

. Go to *Plugins » plentyMarketplace*.
. Click on the user icon in the upper right corner and then on *My Account* (icon:user[role="darkGrey"]).
