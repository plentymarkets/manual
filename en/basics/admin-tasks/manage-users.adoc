...
Create accounts for new users and edit existing user accounts in the user menu. Furthermore, assign roles and classes to users.

* Roles are used for grouping users and assigning the same rights to multiple user accounts in a very convenient way.
* Classes allow you to assign rights to specific users and interfaces. For example, assign API calls to users of the API class individually.

Five free CUL user licences are included in every plentymarkets system. If you need additional user accounts, book them in the link:https://www.plentymarkets.co.uk/prices/[configurator].
...
. Carry out the settings. Pay attention to the information given in <<table-password-security>>.
. *Save* the settings. +
...
. Select a **User class**. Pay attention to the information given in <<table-user-classes>>.
. **Save** the settings.
...
The user classes are preset and the classes' settings cannot be changed. <<table-user-classes>> includes detailed information about preset rights and rights that you can assign individually:
...
|**Variable**
|Class with individual rights for <<#100, menu>> and <<#110, functions>> that can be assigned in the **Rights** tab.
...
|**API**
|Class for <<#120, API calls>> that can be assigned in the user account. API calls are activated individually. REST API calls are not assigned to user accounts by default.
...
|**Roles**
|Select roles for the user account. <<#70, Roles>> must have already been created before they can be selected here.
...
<ol>
<li>
Go to <strong>System » System settings » Settings » User » Accounts</strong>.</li>
<li>
...
<li>
*_Or:_* Enter a hexadecimal or RGB value.</li>
<li>
...
For plentymarkets to send emails such as order confirmations on your behalf, you need to save the access data of the mailbox.org mailbox from which you want to send these emails in the <<crm/sending-emails#100, Settings » System settings » Client (store) » Standard » Email » Access data>> menu.
...
|**Right**
|For further information about this tab, refer to the <<#90, Assigning rights>> chapter.
...
. Pay attention to the explanations given in <<table-group-function>> and carry out the settings as desired.
...
|Give rights for warehouses to the user. Only previously created warehouse types are available.
...
Assign every user account of the **Variable** class only those rights required for their field of work. This way, security is increased and errors can be prevented in your system.
...
Once an incorrect password is entered several times in succession, the login is blocked for the user account for a certain time. The login can be unblocked by an administrator in the menu **System » System settings » Settings » User » Accounts » Open user** by clicking on the button **Unblock login**.
...
. Go to **System » System settings » Settings » User » Roles**.
. Click on **New role**. +
...
After creating a role, it will open in a new tab for editing. The role is also added to the role overview so that you can open and edit it easier. Roles are listed alphabetically in the overview. Every role is divided into 3 tabs. In the **Settings** tab, you can change the role's name. In the **User** tab, you link user accounts to this role. In the **Rights** tab, you assign rights to the role. The same settings used for user accounts of the Variable class are also available for roles. For further information about rights, refer to the <<basics/admin-tasks/manage-users#90, Assigning rights>> chapter.
...
*_Or:_* Drag and drop a user account to the **Assigned users** column.
. **Save** the settings.
...
====
Administrators have all rights. However, it is important that roles are assigned to administrators for some process.  For example, administrators only receive role-specific emails when linked to the corresponding role.
====
...
|Warehouse
|Settings » System settings » Stock » Warehouse +
Stock +
Orders » Fulfilment
...
|
|Start » Vacation +
System » System settings » Client » Select client » Accounting
|====
...
. Go to **System » System settings » Processes » Per role**.
...
[#160]
== Managing the number of logged in users

Depending on your plentymarkets contract, the number of users that can be logged into plentymarkets at the same time may be limited. This limit applies to the plentymarkets back end, the REST API and the plentymarkets app.
...
*_Or:_* Go to *System » System settings » Settings » User*. +
...
Book additional user licenses in the plentymarkets configurator.
...
. Click on one of the buttons for changing your contract. +
→ The plentymarkets configurator opens.
. In the progress bar at the bottom of the configurator, click on *Features*.
. Scroll to the *User licenses (CUL)* area.
. Click on the plus icon underneath the heading *Additional concurrent user license (CUL)* to add additional user licenses.
. In the progress bar at the bottom of the configurator, click on *Your system*. +
→ A summary of the new contract details is shown.
. Check the information carefully.
. Click on *Order system change*. +
