...
:position: 30
:url: basics/admin-tasks/manager-users
...
|**Colour**
|Define a colour in the editing window by selecting a colour, entering a hexadecimal colour code or the RGB value. The colour makes it easier to identify the user account in other menus, for example the <<basics/working-with-plentymarkets/tasks-and-calendars#600, Calendar>>.
...
When switching users, any user account for which a PIN has been saved can be selected. To remove the PIN, for example because the user no longer works with the app, delete the 4 dots and save the user account. +
*_Note: 
...
**__Note:__** You cannot create mailboxes for test domains (for example @plenty-test-drive) or for external domains that are not hosted by plentymarkets. +
...
The maximum number of login attempts is 5 per user account or 25 per IP address. If the maximum number of login attempts is exceeded, the account is blocked for 24 hours. Users with admin rights can unblock a user manually account before this 24 hour period has passed.

[.instruction]
Unblocking a user account:

. Go to *System » System settings » Settings » User » Accounts*. +
→ The user accounts are shown.
