= Best Practices: Event procedures and Procedure manager
:lang: en
...
The email is sent to the email address of the selected employee. For an email address to be available here, it has to be saved with the user in the <<basics/working-with-plentymarkets/manage-users#42, System » Settings » User » Accounts>> menu. +
...
For detailed instructions on setting up the procedure manager and on the available filters and procedures, refer to the <<basics/automation/procedure-manager#, Procedure manager>> page of the manual.
