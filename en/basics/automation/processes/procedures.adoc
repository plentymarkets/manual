...
This page of the manual contains information about the control elements and procedures you can add to work steps of processes in the **Settings » Processes** menu.
...
|**Position**
|Option is only visible after the control element is created. The procedure's current position in the work step. The procedure with the lowest position number is run first. Select a different position from the drop-down list to change the order in which the procedures are run in the work step. +
...
**Yes** (default setting) = Procedure is shown in the **plentymarkets Client » Processes** menu. +
**No** = Procedure is not shown in the **plentymarkets Client » Processes** menu. The procedure runs in the background.
...
The control element **End** is mandatory for some procedures. Find out more in the following video tutorial about **Loops**:
...
Learn more about the control element **Filter** menu in the following video tutorial:
...
Learn more about configuring a filter and the required control elements **Reset filter** and **Jump** in the following video tutorial:
...
Learn more about the control element **Jump** in relation to the control element <<basics/automation/processes/procedures#540, **Loop** >> in the following video tutorial:
...
Learn more about the control element **Loop** in the following video tutorial:
...
//  br /> <b>Teilauftrag</b> = Der Auftrag wird in zwei voneinander unabhängige Aufträge geteilt. Für jeden Auftrag wird eine eigene Rechnung mit eigenem Zahlungseingang generiert.<
...
* <<basics/automation/processes/sub-procedures#250, **Scan package number** >>
...
* <<basics/automation/processes/sub-procedures#180, **Print** >>
* <<basics/automation/processes/sub-procedures#280, **Save** >>
* <<basics/automation/processes/sub-procedures#280, **Save to clipboard** >>
* <<basics/automation/processes/sub-procedures#270, **Sounds** >>
...
Find out more in the following video tutorial:
...
Use the **Finance export** procedure to create a CSV file that can be imported into several <<order-processing/orders/_accounting#8, **financial accounting solutions** >>. You can choose between two formats: **One data record per stock unit (default)** or **One data record per invoice and VAT rate**. The <<basics/data-exchange/exporting-data#10, **Finance export** >> can be saved with a sub-procedure.
...
The **Adjustment form** procedure generates <<omni-channel/online-store/standard/documents/adjustment-form#, **adjustment forms**  >> for selected orders in the process. They can then e.g. be printed or saved with the corresponding sub-procedure.
...
|**Item status**
|Select the item status that should be used by default when a return is created. Go to **<<order-processing/orders/managing-sales-orders#return, Settings » Orders » Order types » Return  >>** to create item statuses for returns.
...
The following wildcard characters are permitted: **$OID** = Order ID **$CID** = Customer ID **$INV** = Invoice number
|====
...
Learn more about the **Serial numbers** procedure in the following video tutorial:
