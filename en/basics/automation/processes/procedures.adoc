...
Use the **Order** procedure along with a sub-procedure to <<order-processing/orders/editing-sales-orders/changing-a-sales-order#, **process orders**  >>. For example, you can change the status of selected orders or scan package numbers.
...
The **Credit note** procedure <<order-processing/orders/fulfillment/generating-credit-notes#, **generates credit notes**  >> for selected orders in the process. They can then e.g. be printed or saved with the corresponding sub-procedure.
...
The **Warehouse pick list** procedure generates <<order-processing/orders/fulfillment/warehouse-pick-list#, **warehouse pick lists**  >> for selected orders in the process. They can then e.g. be printed or saved with the corresponding sub-procedure.
...
Use the **Delivery note** procedure to generate <<order-processing/orders/fulfillment/generating-delivery-notes#, **delivery notes**  >> for the orders. These delivery notes can be printed or saved with a sub-procedure.
...
Use the **Dunning letter** procedure to generate <<order-processing/orders/fulfillment/generating-dunning-letters#, **dunning letters**  >> for the orders. These dunning letters can be printed or saved with a sub-procedure.
...
Use the **packing list** procedure to generate a list of picked items prepared for shipping. Use it along with a sub-procedure to print or save the <<order-processing/orders/fulfillment/packing-list#, **packing list**  >>.
...
Use the **Pick list** procedure along with a sub-procedure to print or save a <<order-processing/orders/fulfillment/pick-list#, **pick list**  >>. The pick list is generated as part of batch picking. Orders in status **5 Cleared for shipping** and higher will be dealt with. An incoming payment has already been booked for these orders (change from status 3 to 4). In addition, the items are available (change from status 4 to 5).
...
Use the **Invoice** procedure to generate <<order-processing/orders/fulfillment/generating-invoices#, **invoices**  >> for the orders. These invoices can be printed or saved with a sub-procedure.
...
|**Item status**
|Select the item status that should be used by default when a return is created. Go to **<<order-processing/orders/settings/sales-order-types/return#, Settings » Orders » Order types » Return  >>** to create item statuses for returns.

|**Item value**
|Enter a percentage. This percentage will be applied to returned items. This value will also be displayed once a return has been created. Simply open the **<<order-processing/orders/editing-sales-orders/return#, return  >>** and click on the **Overview** tab. You will find the value next to the stock units.
...
|**Reason for return**
|Select a reason. Returns will receive this reason when they are first created. Go to **<<order-processing/orders/settings/sales-order-types/return#, Settings » Orders » Order types » Return  >>** to create reasons for returns.
|====
...
Use the **Return label** procedure along with a sub-procedure to automatically print or save a <<order-processing/orders/fulfillment/generating-a-return-label#, **return**  >> label.
...
Use the **Return slip** procedure along with a sub-procedure to print or save a return slip automatically. This return slip is attached to the item in case customers might need it for a <<order-processing/orders/fulfillment/generating-a-return-label#, **return**  >>.
