...
* Save documents
* Print documents
* Opening the print history
* Create processes in plentymarkets back end
* Run processes in plentymarkets back end

If plentyBase is installed on the computer, these tasks can be performed in the plentymarkets back end. Find out how to install and use plentyBase here.
...
. During the installation process, select the port for communication between the computer and plentyBase. +
**__Note:__** Port 7331 is selected by default because this port is free on most computers. Only change this default value if port 7331 is in use on the computer. +
**__Tip:__** The following command line commands allow you to check which ports are in use: +
...
. Select **Open configuration** from the drop-down list. +
...
. Click on **Add**. +
→ The **Add access token** window opens. +
. Paste the access token to the **Access Token** field. +
...
|**Server address**
|Do not modify the saved address \https://local.plentybase.de.

|**Port**
|The port you assigned to plentyBase during installation. Default value is 7331. If a different port was specified during installation, enter the number of that port. +
...
|**Generate token**
|The API access token is used for authentication with the plentyBase API interface. Click on **Generate token** and confirm the generation to generate a new access token. +
*_Note:_* The old token is overwritten when a new access token is generated. Once you have generated a new token, save this new token in the settings of the plentyBase tool.

|**Reset settings**
|Resets server address and port to the default values.
|====
...
__**Tip:**__ From the drop-down list to the bottom right, select **All files**. +
