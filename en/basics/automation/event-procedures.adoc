...
Routine tasks make up a significant portion of order processing. To a large extent, you can let your plentymarkets system take care of these for you. In particular, sending emails to your customers takes up a lot of time, yet is an essential part of a store that is geared towards service and customers.
...
====
For an order confirmation, the event **New order** is always followed by the procedure **Emailing**.
====

The __**creation of an event procedure**__ is broken down into the following steps:

. Creating a link:http://orders/settings/event-procedures/new-event-procedure[new event procedure^]
...
The following table explains the settings in the **System » Orders » Events** menu:
...
|**Invoice generated**, **Delivery note** ..., **List of delivery addresses** ..., **Pick list** ..., **Packing list** ..., **Offer** ..., **Dunning letter** ..., **Reversal document generated**, **Order confirmation generated**, **Returns shipping label saved**
...
Go to <<order-processing/orders/_basic-settings#, System » Orders » Settings>> to save the percent value for __**deposit payments on production items**__.
...
[.linkBox]
link:https://www.plentymarkets.co.uk/a-3426/[Automatically sending an email with a tracking URL^]

[.linkBox]
link:https://www.plentymarkets.co.uk/a-3425/[Adding a free gift to an order^]
...
The following filters can be used to select specific <<crm/managing-contacts#, customer data>> included in the orders:
...
The filter is used to search for orders that are made up of several shipments. You can then inform the recipient that they will receive more than one shipment.
...
You can use this filter to trigger a procedure once a customer has placed a certain number of orders. The procedure could be e.g. sending an email that includes a gift card. You also have to set up the customer filter for this procedure. The customer filter specifies which **customer type** the event procedure should be applied to, e.g. **customer**.
...
|**Use order template**
|Go to <<order-processing/orders/order-template#, System » Orders » Order template>> to create order templates.
...
For further information, refer to the <<crm/managing-contacts#100, Creating a contact>> chapter of the <<crm/managing-contacts#, Managing contacts>> page of the manual.
...
Available for: **Amazon Pay**, **BillSAFE**, **eBay**, **Gimahhot**, **Hood**, **Klarna**, **Masterpayment**, **Payolution**, **PAYONE**, **Plus.de**, **POSTPAY**, **Rakuten**, **Zentralverkauf**, **Worldpay**
|====
...
Available for: **Amazon Pay**, **Payolution**.
...
Available for: **Amazon Pay**, **ClickandBuy**, **Plus.de**, **POSTPAY**, **Sage Pay**, **Worldpay**.
...
|Generates a DHL return label for <<order-processing/fulfilment/preparing-the-shipment#3200, DHL Retoure Online>> or <<order-processing/fulfilment/preparing-the-shipment#3100, DHL Retoure with enclosed return labels>> that is available in the **shipping center** or in the **My account** area of the online store.

|**Send return**
|Sends a return to the payment provider. +
Available for: **Amazon**, **BillSAFE**, **Cdiscount**, **Klarna**, **MeinPaket**, **Plus.de**, **POSTPAY**, **Rakuten**, **Shopgate** +
