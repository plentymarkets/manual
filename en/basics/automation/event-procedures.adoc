...
Event procedures can be used to automate day-to-day processes. For further information, refer to the **<<order-processing/orders/best-practice/automating-processes#, Automating processes  >>** page of the manual.
...
image::order-processing/orders/settings/assets/ENG-Einstell-Auftraege-Ereignisaktionen-01.png[]
...
image::order-processing/orders/settings/assets/ENG-Einstell-Auftraege-Ereignisaktionen-02.png[]
...
image::order-processing/orders/settings/assets/ENG-Einstell-Auftraege-Ereignisaktionen-03.png[]
...
The following **events** can be used to trigger an event procedure. These events all relate to **__generating the following__<<order-processing/orders/editing-sales-orders/changing-a-sales-order#4, types of orders >>**:
...
The following **events** can be used to trigger an event procedure. These events all relate to <<order-processing/orders/editing-sales-orders/changing-a-sales-order#, **order changes**  >>:
...
The following **events** can be used to trigger an event procedure. These events all relate to <<order-processing/orders/fulfillment/generating-invoices#, **generating documents**  >>:
...
The following **events** can be used to trigger an event procedure. These events all relate to <<order-processing/orders/editing-sales-orders/changing-a-sales-order#6, **payment processes** >>:
...
Go to <<order-processing/orders/settings/basic-settings#, **Settings » Orders » Basic settings**  >> to save the percent value for __**deposit payments on production items**__.
...
The following description for creating a **new event procedure** uses the example found in image 1 on the **<<order-processing/orders/settings/event-procedures#, Event procedures  >>** page of the manual. Select a **name** that allows you to recognize the event procedure immediately.

image::order-processing/orders/settings/event-procedures/assets/ENG-Einstell-Auftraege-Ereignisaktionen-Neue-1.png[]
...
Once you have **<<order-processing/orders/settings/event-procedures/new-event-procedure#, created  >>** an event procedure, the name and event will be displayed in the **settings** area and can be changed there.
...
image::order-processing/orders/settings/event-procedures/assets/EN-Settings-Orders-Event-Procedures-Configuration-01.png[]

__Image 1: **filter** section__

===== Filter type: Order

The following filters can be used to select specific **<<order-processing/orders/editing-sales-orders/changing-a-sales-order#, order  >>** characteristics:
...
For further information, refer to the <<order-processing/orders/settings/event-procedures/configuration#1.3.1, **example configuration** >> in section 1.3.1.
...
For further information, refer to the <<order-processing/orders/settings/event-procedures/configuration#1.3.3, **example configuration** >> in section 1.3.3.
...
The following filters can be used to select specific **link::/[items]** included in the orders:
...
The following filters can be used to select specific **link::/[customer data]** included in the orders:
...
For further information, refer to the <<order-processing/orders/settings/event-procedures/configuration#1.3.2, **example configuration** >> in section 1.3.2.
...
image::order-processing/orders/settings/event-procedures/assets/EN-Settings-Orders-Event-Procedures-Configuration-02.png[]
...
image::order-processing/orders/settings/event-procedures/assets/EN-Settings-Orders-Event-Procedures-Configuration-03.png[]
...
image::order-processing/orders/settings/event-procedures/assets/EN-Settings-Orders-Event-Procedures-Configuration-04.png[]
...
image::order-processing/orders/settings/event-procedures/assets/EN-Settings-Orders-Event-Procedures-Configuration-05.png[]
...
image::order-processing/orders/settings/event-procedures/assets/EN-Settings-Orders-Event-Procedures-Configuration-06.png[]
...
|**Use order template**
|Go to <<order-processing/orders/settings/sales-order-template#, **Settings » Orders » Order template**  >> to create order templates.

|**Generate delivery orders**
|Automatic generation of delivery orders when the specified event occurs. +
**Important**: This option will only be displayed if the <<order-processing/orders/editing-sales-orders/delivery-order#, **warehouse configuration**  >> that is necessary for delivery orders has been set up correctly.
...
**Ignore <<Link859#, delivery orders  >>**: If you use the **Change status** procedure for an order that includes delivery orders, then the status of the delivery orders will also be changed by default. Place a check mark next to this option if you only want the status of the main order to be changed.
...
For further information, refer to the **link::/[New customer]** page of the manual.

|**Delete Klarna invoice**
|Deletes an invoice that has the payment method **Klarna** when a cancellation occurs. For further information regarding this and other event procedures, refer to the <<order-processing/orders/settings/event-procedures/configuration#5-3, **Klarna** >> page of the manual.

|**Notify Cdiscount of order changes**
|This procedure is used to notify the market <<order-processing/orders/settings/event-procedures/configuration#7-2, **Cdiscount** >> of any order status changes.

|**Send "Order complete" to DaWanda**
|A procedure to inform the market DaWanda about completed orders. For further information regarding this and other event procedures, refer to the <<order-processing/orders/settings/event-procedures/configuration#EAgreenBox, **DaWanda** >> page of the manual.
|====

__Table 7: overview procedure type: **Orders**__

Note the information regarding **<<order-processing/orders/editing-sales-orders/delivery-order#, delivery orders  >>** if you want to change the status of main orders using event procedures for delivery orders.

===== Procedure type: Documents

In order for a document to be generated, i.e. to receive the desired configuration and the correct layout, it has to be configured in the corresponding menu and a template has to be saved. For further information, refer to the **<<order-processing/orders/settings/documents#, Documents  >>** page of the manual.
...
|Permission will be sent to **link::/[FBA]** so that items are actually shipped by FBA.
...
For further information, refer to the link::/[**payment method**] page of the manual.
|====
...
|**Post feedback**
|Automatically creates link::/[**feedback**] for a listing. +
...
image::order-processing/orders/settings/event-procedures/assets/EN-Settings-Orders-Event-Procedures-Configuration-07.png[]
...
image::order-processing/orders/settings/event-procedures/assets/EN-Settings-Orders-Event-Procedures-Configuration-08.png[]
...
image::order-processing/orders/settings/event-procedures/assets/EN-Settings-Orders-Event-Procedures-Configuration-09.png[]
