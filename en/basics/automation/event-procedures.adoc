...
. <<basics/automation/event-procedures#100, Creating an event procedure>>
. <<basics/automation/event-procedures#110, Selecting an Event>>
. <<basics/automation/event-procedures#120, Add one Filter or more>>
. Determining which <<basics/automation/event-procedures#130, Procedures>> should be carried out by the system once the chosen event takes place
. <<basics/automation/event-procedures#200, Activating>> the event procedure

For practical examples for using event procedures, refer to the <<basics/automation/automating-processes#, Automating processes>> page of the manual.

[#100]
== Creating a new event procedure

Go to *System » Orders » Events* to create a new event procedure. In the same menu, you can access and <<basics/automation/event-procedures#300, edit>> all event procedures that you have previously created.
...
The first step consists of selecting an *Event* that triggers the *new event procedure*. <<table-event-procedures-event-groups>> lists the available events according to their respective event groups.
...
Go to *<<order-processing/orders/basic-settings#, System » Orders » Settings>>* to save the percentage for deposits on production items.
...
*Booked in*, +
*Partly booked in*
|These events only apply to the order type purchase order. Depending on your selection here, the event triggering the event procedure is a complete or partial booking in our booking out of reordered stock.
|====
...
|*Click & Collect*
|Orders are filtered according to the shipping method Click & Collect. Click & Collect can only be used for orders that are referred from eBay UK and have the UK as their delivery country. For further information, refer to the <<omni-channel/multi-channel/ebay/ebay-setup#7010, Click & Collect>> chapter.
...
For more information on Amazon and eBay orders, refer to the respective manual pages on <<omni-channel/multi-channel/amazon/amazon-setup#, Amazon>> and <<omni-channel/multi-channel/ebay/ebay-setup#, eBay>>.
...
*_Example_*: If you enter a minimal length of 4, the system searches for telephone numbers consisting of 4 or more digits.
...
|*Receiving warehouse*
|Orders of the type reorder are searched according to the receiving warehouse to which the reordered goods are sent. In case of the order type stock transfer are searched according to the target warehouse into which stock is rebooked.
|====
...
*Important*: This option is only displayed if the necessary <<order-processing/orders/managing-orders#320, warehouse assignment>> has been carried out correctly.
...
*Ignore <<order-processing/orders/managing-orders#300, Delivery orders>>*: The procedure *Change status* changes the status of both the main order and its delivery orders by default. Activate the option if you only want the status of the main order to be changed.
...
|This procedure deletes an invoice with the payment method *Klarna* when an order is cancelled. Detailed information as well as examples for further event procedures can be found on the <<order-processing/payment/klarna#, Klarna>> page of the manual.

|*Send order confirmation to Fruugo*
|This procedure sends an order confirmation to the market <<omni-channel/multi-channel/fruugo#, Fruugo>>.

|*Send cancellation to Fruugo*
|This procedure notifies the market <<omni-channel/multi-channel/fruugo#, Fruugo>> of the cancellation of an order.

|*Notify Cdiscount of order changes*
|This procedure informs <<omni-channel/multi-channel/cdiscount#, Cdiscount>> of changes to orders received from the market Cdiscount.

|*Send cancellation to Flubit*
|This procedure notifies the market <<omni-channel/multi-channel/flubit#, Flubit>> of the cancellation of an order.

|*Send order confirmation to idealo*
|This procedure sends an order confirmation to the market <<omni-channel/multi-channel/idealo-checkout/idealo-setup#, idealo>>.

|*Send cancellation to idealo*
|This procedure notifies the market <<omni-channel/multi-channel/idealo-checkout/idealo-setup#, idealo>> of the cancellation of an order.

|*Send order cancellation to bol.com*
|This procedure notifies the market <<omni-channel/multi-channel/bol-com#, bol.com>> of the cancellation of an order.

|*Send cancellation to real.de*
|This procedure notifies the market <<omni-channel/multi-channel/real-de/real-setup#, real.de>> of the cancellation of an order.
...
To display the dunning fee, enter the *Variation ID* of the <<order-processing/orders/order-documents/generating-dunning-letters#200, Item representing the dunning charge>> which you have created in your plentymarkets system beforehand. +
An optional *Comment* can be entered and appears below the list of variations.
...
|Sends a permission to <<omni-channel/multi-channel/amazon/amazon-setup#3300, FBA>> so that FBA can start the shipping process.
...
Individual procedures are available for: <<order-processing/payment/amazon-pay#, Amazon Pay>>, <<order-processing/payment/upg#, CrefoPay>>, <<order-processing/payment/klarna#, Klarna>>, <<order-processing/payment/klarna-checkout-uk#, Klarna UK>>, <<order-processing/payment/masterpayment#, Masterpayment>>, <<order-processing/payment/payolution#, Payolution>>, <<order-processing/payment/payone#, PAYONE>>, <<omni-channel/multi-channel/plus-gartenxxl#, Netto eStores>>, <<order-processing/payment/postpay#, POSTPAY>>, *Zentralverkauf*, <<omni-channel/multi-channel/hood#, Hood>>, <<omni-channel/multi-channel/ebay/ebay-setup#, eBay>>, <<omni-channel/multi-channel/rakuten/rakuten-setup#, Rakuten.de>>, <<omni-channel/multi-channel/fruugo#, Fruugo>>, <<omni-channel/multi-channel/flubit#, Flubit>>, <<omni-channel/multi-channel/idealo-checkout/idealo-setup#, idealo>> and <<omni-channel/multi-channel/bol-com#, bol.com>>.

|*Send tracking information to Rakuten*, +
*Send preparation for shipping to Rakuten*
|These procedures send the tracking number or a confirmation that the order is being prepared for shipment to the market <<omni-channel/multi-channel/rakuten/rakuten-setup#, Rakuten.de>>.

2+| Procedure group *Payment methods*

|*Send credit note*
|A credit note is sent to the payment provider. +
Individual procedures are available for: *ClickandBuy*, <<omni-channel/multi-channel/allyouneed/allyouneed-setup#, Allyouneed>>, <<order-processing/payment/payone#, PAYONE>>, <<order-processing/payment/postpay#, POSTPAY>>, <<order-processing/payment/worldpay#, Worldpay>> and <<order-processing/payment/klarna#, Klarna>>.

|*Send order to Klarna*
|An order is sent to <<order-processing/payment/klarna#, Klarna>>.

|*Notify Klarna of changes to the order*
|A notification on changes to an order is sent to <<order-processing/payment/klarna#, Klarna>>.
...
Individual procedures are available for: <<order-processing/payment/amazon-pay#, Amazon Pay>>, <<order-processing/payment/upg#, UPG>>, <<order-processing/payment/payolution#, Payolution>> and <<order-processing/payment/klarna-checkout-uk#, Klarna UK>>.

|*Send refund*
|A refund is sent to the payment provider. +
Individual procedures are available for: <<order-processing/payment/masterpayment#, Masterpayment>>, <<order-processing/payment/paypal-plus#, PayPal PLUS>> and <<order-processing/payment/paypal#, PayPal>>.

|*Send cancellation*
|A cancellation is sent to the payment provider. +
Individual procedures are available for: <<order-processing/payment/upg#, UPG>>, <<order-processing/payment/amazon-pay#, Amazon Pay>>, *ClickandBuy*, <<order-processing/payment/paymorrow#, paymorrow>>, <<omni-channel/multi-channel/plus-gartenxxl#, Netto eStores>>, <<order-processing/payment/postpay#, POSTPAY>>, <<omni-channel/multi-channel/rakuten/rakuten-setup#, Rakuten.de>>, <<order-processing/payment/worldpay#, Worldpay>>, <<order-processing/payment/sage-pay#, SagePay>>, <<order-processing/payment/klarna-checkout-uk#, Klarna UK>>.
...
|Generates a DHL return label via <<order-processing/fulfilment/preparing-the-shipment#3200, DHL Retoure Online>> or <<order-processing/fulfilment/preparing-the-shipment#3100, DHL Retoure with enclosed return labels>>. The generated return label is made available in the *Shipping center* or in the *My account* area of the online store.

|*Send return*
|Sends a return to a market or a payment provider. +
Individual procedures are available for: <<order-processing/payment/klarna#, Klarna>>, <<omni-channel/multi-channel/amazon/amazon-setup#, Amazon>>, <<omni-channel/multi-channel/allyouneed/allyouneed-setup#, Allyouneed>>, <<omni-channel/multi-channel/plus-gartenxxl#, Netto eStores>>, <<order-processing/payment/postpay#, POSTPAY>>, <<omni-channel/multi-channel/rakuten/rakuten-setup#, Rakuten.de>>, <<omni-channel/multi-channel/shopgate#, Shopgate>>, <<omni-channel/multi-channel/flubit#, Flubit>>, <<omni-channel/multi-channel/idealo-checkout/idealo-setup#, idealo>>, <<omni-channel/multi-channel/fruugo#, Fruugo>> and <<omni-channel/multi-channel/cdiscount#, Cdiscount>>. +
More information is available on the <<order-processing/payment#, Payment method>> page of the manual.

|*Send return status to bol.com*
|Sends the return status to the market <<omni-channel/multi-channel/bol-com#, bol.com>>.

2+| Procedure group *Market listings*

|*Post feedback*
|Automatically generates feedback for a listing. +
Individual procedures are available for: <<omni-channel/multi-channel/hood#600, Hood>>, <<omni-channel/multi-channel/ricardo-ch#700, ricardo>> and <<omni-channel/multi-channel/ebay/ebay-setup#1300, eBay>>.

|*Send payment notification*
|Automatically sends a payment confirmation. +
Individual procedures are available for: <<omni-channel/multi-channel/hood#600, Hood>> and <<omni-channel/multi-channel/ebay/ebay-setup#1300, eBay>>.
...
|Once the selected event takes place, the listed procedures are triggered within the management of returns and refunds at <<omni-channel/multi-channel/ebay/ebay-setup#1300, eBay>>.

|*Reject eBay cancellation request*, +
*Accept eBay cancellation request*, +
*Create an order cancellation on eBay*
|Once the selected event takes place, the listed procedures are triggered within the management of order cancellations at <<omni-channel/multi-channel/ebay/ebay-setup#1300, eBay>>.

|*Start dunning/dispute process on eBay*
|Automatically sends a dunning letter or starts a dispute on <<omni-channel/multi-channel/ebay/ebay-setup#1300, eBay>>.
|====
