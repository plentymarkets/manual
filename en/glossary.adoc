...
* Synonyme: Products, goods, commodities
...
* Synonyme: Product bundles, package deals
...
* Synonyme: Variation, Stock unit

[#auction]
Auction:: A <<#listing, listing>> type. Auktionen are possible on the marketplaces eBay, Hood and ricardo. *Auction* type listings start at a predefined price, then customers bid on the item.
...
[#colli]
Colli:: Load unit for the transport of goods. More specifically, it is the packing unit of piece goods, thus, the smallest unit of a goods shipment. The entry Colli: 7 means z.B. means that the package consists of 7 units.
...
[#property]
Property:: Some items stand out from the masses. It could be a waterproof watch or a speaker that is Bluetooth capable. In plentymarkets, such characteristics are called _properties_ and _characteristics_. They highlight the technical features of a product. The difference is that properties characterise specific variations, whereas <<#attribute, attributes>> characterise whole items. Properties and characteristics also differ from <<#attribute, attributes>>, as they characterise products _without_ creating saleable variations. +
...
* Synonyme: Facet navigation, filter search, item search
...
[#fixed-price]
<<#listing, Listing>> offered at a fixed price and not up for auction.
...
[#hauptmandant]
Hauptmandant:: See <<default-client,Default client>>.
...
[#listing]
Listing:: The listing contains basic settings that are independent of the platform, for example settings for stock dependence and listing contents. Here, you can also define the listing type. The listing types are <<#fixed-price, Fixed price>>, <<#auction, Auction>> oder <<#store-item, Store item>>. +
...
[glossary]
[#return-label]
Return label:: Included in a  When returning a package, for example because of warranty or repairs, customers only have to package the item and affix the label. +
* Synonyme: 
...
[glossary]
[#multi-order-picking]
Mulit order picking:: Method of combining items from multiple orders. Multiple orders are processed at the same time. All items are commissioned in a single dispenser and only associated with a specific order at the packing table.
