= Shipping centre 2.0 (closed beta)
:lang: en
...
:position: 0
:url: fulfilment/shipping-centre-2.0-closed-beta-phase
:id: JVNZPRL
:author: team-fulfillment

This page describes the new shipping centre 2.0 which was released as a closed beta phase in September 2021. Closed beta phase means that the shipping centre 2.0 will initially be tested by a small group of participants. +
Once the shipping centre 2.0 is sufficiently developed and possible bugs have been fixed, the open beta phase will begin.

[TIP]
On this page, we will present you the functions of the new shipping centre 2.0. More functions are being continuously developed and added to this page.
...
* Group function to group return labels
...
* *_Option 1_*\: Click on icon:search[set=material] to see a list of all orders.
* *_Option 2_*\: Click on icon:more_vert[set=material] to narrow down the search results with filters.
...
// | *Registered with*
// |Select one or more options (icon:check-square[role="blue"]) to search for orders in this status. +
// *_Note:_* In this column, you see the shipping service provider that you registered the order with. If the field is empty, the order has not yet been registered.

| *Shipping service provider*
|Select a shipping service provider to only search for orders that use this shipping service provider.

| *Shipping profile*
|Select one or more shipping profiles (icon:check-square[role="blue"]) to search for orders with this shipping profile.
...
| *Client (store)*
|Select one or more clients (shops) (icon:check-square[role="blue"]) to search for orders with this client (shop).

| *Warehouse*
|Select one or more warehouses (icon:check-square[role="blue"]) to search for orders with this warehouse.
...
[#adjust-overview]
[discrete]
== Adjusting the overview in the shipping centre 2.0

The overview of the shipping centre 2.0 shows the following columns by default:
...
Click on the button *Configure columns* (icon:settings[set=material]) to deactivate the columns that are already shown. 

[#group-function]
== Carrying out the group function

Once you have selected an order (icon:check-square[]), the different buttons of the group function become visible (see <<#image-group-function-overview>>). Select at least 2 orders from the list to be able to use the group functions. <<#table-group-functions>> lists the elements from the group functions as well as their explanations.
...
*_Note:_* If you installed and deployed the *DHL Retoure Online* plugin, different settings for generating the labels are visible: one label per order (one file), one label per package, one label per package (multiple files), international.
...
|Groups the shipping labels of orders that have already been registered in PDF format. The number of PDF files corresponds to the number of shipping service providers. This means: One PDF file is generated for each shipping service provider.

| *Group return labels*
| icon:order_return_slip[set=plenty]
|Groups the return labels of orders that have already been registered as returns in PDF format. The number of PDF files corresponds to the number of return service providers. This means: One PDF file is generated for each return service provider. +
...
. Select *Register orders* (icon:outgoing_items[set=plenty]) in the toolbar on the top. +
...
Grouping shipping labels::
+
--

Proceed as described below to group shipping labels for multiple orders using the group function.
...
. Search for the order by using the filter settings as described in the <<#search-order, Searching for an order in the shipping centre>> chapter. +
→ The orders that correspond to the search criteria entered are shown in the overview.
. Select (icon:check-square[role="blue"]) the orders that you want to create shipping labels for.
. Click on *Group shipping labels* (icon:order_open[set=plenty]) in the toolbar on the top. +
...
Grouping return labels::
+
--
Proceed as described below to group return labels for multiple orders using the group function.
...
. Search for the order by using the filter settings as described in the <<#search-order, Searching for an order in the shipping centre>> chapter. +
→ The orders that correspond to the search criteria entered are shown in the overview.
. Select (icon:check-square[role="blue"]) the orders that you want to create return labels for.
. Click on *Group return labels* (icon:order_return_slip[set=plenty]) in the toolbar on the top. +
→ The PDF file is generated and you can save it to your computer. +
...
[WARNING]
.Cancelling an order vs. resetting an order
======
...
* <<#show-registered-order, Showing the registered order>>
* <<#retrieve-shipping-label, Retrieving a shipping label>>
* <<#reset-order, Resetting a single order>>
* <<#cancel-order, Cancelling a single order>>
* <<#polling, Creating a polling file for a single order>>
* <<#register-return, Registering a return>>
...
. Select *Register order* (icon:outgoing_items[set=plenty]) at the top of the toolbar. +
...
If you open an order that has already been registered, you will see the following 4 areas in the detail view:
...
If the order has been successfully registered, the shipping label is visible in the *Packages* area. Click on *Shipping label* (icon:order_open[set=plenty]) to save the label to your computer and print it.


[discrete]
==== Automatically sending shipping labels via email

You can automatically generate the shipping labels via an event procedure and send them via email. For this procedure, 2 event procedures are required.
...
→ A window opens and you need to confirm your decision. Note the reference in the box <<#difference-reset-cancel, Cancelling an order vs.  resetting an order>> at the beginning of this chapter. 
...
→ The orders that correspond to the search criteria entered are shown in the overview.
. Open the order.
. In the toolbar on the top, click on *Cancel order* (icon:times[]). +
→ A window opens and you need to confirm your decision. Note the reference in the box <<#difference-cancel-reset, Cancelling an order vs.  resetting an order>> at the beginning of this chapter.
. Confirm your decision by clicking on *Reset order*. +
→ The order is reset.
...
. Go to *Orders » Shipping centre 2.0*.
. Search for the orders that you want to create a polling file for by using the filter settings as described in the <<#search-order, Searching for an order in the shipping centre>> chapter. +
→ The orders that correspond to the search criteria entered are shown in the overview.
. Open the order.
. Click on *Polling* (icon:file_extension_csv[set=plenty]) in the toolbar on the top.
. Select the service provider from the drop-down list. +
...
After you have registered the return, the return label is available for download as a PDF file. You can register a return via the main order or via the order type *Return*. For further information about the order type *Return*, refer to the <<orders/managing-orders#400, Managing orders>> page.
...
Once the return was successfully registered, the return label is available in the *Returns* area. Click on *Return label* (icon:order_open[set=plenty]) to save the label on your computer and print it afterwards.
...
You need a customs declaration for shipments that you send to countries outside the European Union, the so-called _CN23 form_. Once you successfully registered an order where the country of delivery is outside the European Union, the export label is automatically available in the *Packages* area.

[.instruction]
Downloading the export label:

. Go to *Orders » Shipping centre 2.0*.
. Search the order that you want to download the export label from by using the filter settings as described in the <<#search-order, Searching for an order in the shipping centre>> chapter. +
→ The orders that correspond to the search criteria entered are shown in the overview.
. Open the order.
. In the *Packages* area, click on *Download export label* (icon:data_export[set=plenty]). +
→ The PDF file is generated and you can save it to your computer.
