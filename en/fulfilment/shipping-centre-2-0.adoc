...
This page describes the new shipping centre 2.0 which was released as a closed beta phase in September 2021. Closed beta phase means that the shipping centre 2.0 will initially be tested by a small group of participants. Once the shipping centre 2.0 is sufficiently developed and possible bugs have been fixed, the open beta phase will begin.
...
* *Toolbar*: Here you can <<#search-order, search>> for shipping orders, <<#adjust-overview, adjust the columns in the overview>>, set the results to be shown per page and click through the pages with the shipping orders. +
...
* *_Option 2:_* Click on icon:more_vert[set=material] to narrow down the search results with filters.
...
|Groups the shipping labels of orders that have already been registered in PDF format. +
...
*_Tip:_* You can find a detailed <<#step-by-step, step-by-step guide>> on how to group shipping labels below this table.

| *Group return labels*
| icon:order_return_slip[set=plenty]
|Groups the return labels of orders that have already been registered as returns in PDF format. +
...
*_Tip:_* You can find a detailed <<#step-by-step, step-by-step guide>> on how to group return labels below this table.
...
[.instruction]
Grouping shipping labels using the group function:
...
[.instruction]
Grouping return labels using the group function:
...
→ The orders that correspond to the search criteria entered are shown in the overview.
. Open the order.
. Click on *Cancel order* (icon:times[]) in the toolbar on the top. +
→ A window opens and you need to confirm your decision. Note the reference in the box <<#difference-reset-cancel, Cancelling an order vs.  resetting an order>> at the beginning of this chapter.
. Confirm your decision by clicking on *Reset order*. +
→ The order is reset.
