...
* *Toolbar*: Here you can <<#search-order, search>> for shipping orders, <<#configure-columns, adjust the columns in the overview>>, set the results to be shown per page and click through the pages with the shipping orders. +
...
*_Note:_* The colour that you saved in the *Setup » Orders » Status* menu for the corresponding statuses will be shown in the overview in front of the order status. _Example:_ icon:circle[role="red"].
...
*_Note:_* Use drag-and-drop to arrange the filter presets in a specific order by clicking on *Move* (icon:drag_indicator[set=material]). Click on icon:trash[role="darkGrey"] to delete a filter preset.
