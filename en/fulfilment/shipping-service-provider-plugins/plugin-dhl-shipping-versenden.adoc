...
====
The DHL Preferred Delivery services *Preferred day*, *Preferred location* and *Preferred neighbour* are only available in this area once you have downloaded the DHL Preferred Delivery plugin in the link:https://marketplace.plentymarkets.com/en/dhlwunschpaket_5435[plentyMarketplace^]{nbsp}icon:external-link[] and installed it in a plugin set. +
...
If you ship your parcels to non-EU countries, activate (icon:toggle-on[role="green"]) the option *Export delivery*. Two additional fields appear where you can select the franking (see <<#table-franking-incoterms>>) and the type of shipment for export deliveries. These two values are mandatory for export deliveries. +
