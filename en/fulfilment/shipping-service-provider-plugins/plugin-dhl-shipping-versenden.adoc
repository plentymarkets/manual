...
In this step, enter your shipper data. The data is shown as sender address on the shipping label.
...
In this step, you can select the references as they should appear on the shipping label. The setting that you select from the drop-down list *Reference per order* will be shown on the shipping label once. The setting that you select from the drop-down list *Reference per item position* will be shown on the shipping label per item position.
...
In this step, link the settings that you saved in the *Shipping settings* and *DHL Shipping (Versenden): Basic settings* assistants. Link your login details with the DHL billing number, the shipper address, the shipping region and the shipping setting and select the services based on the product and your DHL contract.

If you select a billing number that is used for international shipments, the step *International shipments* automatically appears in the assistant. Further down in this chapter, it is described which settings you can carry out in this step.

Note that you may need to contact DHL to obtain additional activation for certain products and services. Check which options you have selected for *Default shipping method* and *Default shipping profile* in the *Setup » Client » [Select client] » Orders* menu. Adjust the settings, if needed.

[IMPORTANT]
.DHL Wunschpaket services only visible after having downloaded the DHL Wunschpaket plugin
====
...
Furthermore, you can determine in this step whether your customers can use a finder in your online shop to select the address of a packstation or a post office as delivery address and whether you want to transfer a minimum package weight to DHL to register the order. Moreover, you can determine that the phone numbers of your customers are neither transmitted to DHL nor displayed on the shipping label.


[discrete]
==== International shipments

This step is only visible if you selected a billing number for international shipments in the previous step *Settings combination*.
...
If you ship your parcels to non-EU countries, activate (icon:toggle-on[role="green"]) the option *Export delivery*. Two additional fields appear where you have to select the postage and the type of shipment for export deliveries. These two values are mandatory for export deliveries.
...
** This setting is important for orders coming in your plentymarkets system via the online shop.
...
image::fulfilment/assets/DHL-label-ident-check.png[width=640, height=360]
...
[[image-dhl-cn23-form]]
.CN23 form
image::fulfilment/assets/dhl-cn23-form.png[width=640,height=360]
