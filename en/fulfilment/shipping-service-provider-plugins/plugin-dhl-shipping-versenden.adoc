...
→ icon:exclamation-triangle[role="red"] *_Important:_* Not all functions and settings have been transferred from the shipping profile to the *Shipping settings* assistant yet. Note the explanations in the <<fulfilment/shipping-service-provider-plugins/plugin-dhl-shipping-versenden#assistant-shipping-settings, Completing the *Shipping settings* assistant>> chapter.
...
→ The DHL Shipping (Versenden) plugin must have been downloaded in the link:https://marketplace.plentymarkets.com/en/plugins/integration/dhlshipping_4871[plentyMarketplace^]{nbsp}icon:external-link[] in order that you can see it here.
