...
.Show polling data
image::fulfillment/assets/dhl-assistant-show-polling-data.png[width=640, height=360]
...
[TIP]
If you select a billing number that is used for international shipments, the step *International shipments* automatically appears in the assistant. <<#international-shipments, Further down in this chapter>>, it is described which settings you can carry out in this step.
...
If you ship your parcels to non-EU countries, activate (icon:toggle-on[role="green"]) the option *Export delivery*. Two additional fields appear where you can select the postage and the type of shipment for export deliveries. These two values are mandatory for export deliveries.
