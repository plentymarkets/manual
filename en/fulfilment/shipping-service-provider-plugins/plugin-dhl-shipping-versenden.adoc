...
[IMPORTANT]
.Simply setting up DHL Shipping (Versenden) with the help of two assistants
====
You need a plugin to link plentymarkets to DHL Shipping (Versenden). Visit the link:https://marketplace.plentymarkets.com/en/plugins/integration/dhlshipping_4871[plentyMarketplace^]{nbsp}icon:external-link[] to download the DHL Shipping (Versenden) plugin. +
...
Before you complete the two DHL Shipping (Versenden) assistants, you need to complete the *Shipping settings* assistant. This assistant is available by default in your plentymarkets system. With this assistant, you carry out the following settings:
...
Once you have installed the plugin and saved and deployed it in a plugin set, the assistants *DHL Shipping (Versenden): Basic settings* and *DHL Shipping (Versenden): Settings combination* are available in the *Setup » Assistants* menu in the *Integration* area.
...
* You enter a name for the shipping setting. +
-> This name should be unique because you need it later when you link your shipping service provider plugin in the *DHL Shipping (Versenden): Settings combination* assistant.
...
When you have completed the *Shipping settings* assistant and open it again, the following information is shown:
...
In this step, you save your login details for the DHL business customer portal. It is possible to save multiple login details here. In the second assistant *DHL Shipping (Versenden): Settings combination*, you can link this data.
...
When you have completed the *DHL Shipping (Versenden): Settings combination* assistant and open it again, the following information is shown:
...
Make sure that your parcels are only delivered to the recipient in person – after the identity and - where applicable- the age have been checked against the identity card and documented.

If the service is available, customers can state their name and date of birth and/or minimum age to ensure that the parcel is handed over to the stated person only.

In order that your customers can use the Ident check service, the following setting is important:
...
. In the *Authorisations* and *Plugins* areas, activate the rights as listed in <<#table-user-rights-back-end-user>>.
...
. Click on the plugin *DHLShipping*. +
→ The *Configuration > General* area opens.
