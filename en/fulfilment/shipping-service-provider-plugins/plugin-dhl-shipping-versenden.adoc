...
In this assistant, link the settings that you saved in the *Shipping settings* and *DHL Shipping (Versenden): Basic settings* assistants. Link your login details with the DHL billing number, the shipper address, the shipping region and the shipping setting and select the services based on the product and your DHL contract. Note that you may need to contact DHL to obtain additional activation for certain products and services.
...
Furthermore, you can determine in this assistant whether your customers can use a finder in your online shop to select the address of a packstation or a post office as delivery address and whether you want to transfer a minimum package weight to DHL to register the order. In case of international deliveries, select in this assistant also the franking and the shipping type. Moreover, you can determine that the phone numbers of your customers are neither transmitted to DHL nor displayed on the shipping label.

[IMPORTANT]
.Checking the default shipping method and shipping profile
====
Check which options you have selected for *Default shipping method* and *Default shipping profile* in the *Setup » Client » [Select client[] » Orders* menu.
====
...
Therefore, it is important that you enter alternative values for the email address and the telephone number in the shipping profile. Otherwise, it is not possible to successfully register the order with DHL when selecting certain services (e.g. retail outlet routing).
...
** Select in the drop-down list that opens below which email address should be transmitted:
...
[IMPORTANT]
.Settings in the assistant have priority
====
The settings in the assistant regarding the transmission of alternative data have priority over the settings in the shipping profile.
====
