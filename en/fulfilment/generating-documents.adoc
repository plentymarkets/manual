...
Using ElasticSync is the easiest way to import package numbers in your plentymarkets system. To do so, create a sync of the type *Orders*. Note our <<data/importing-data/elasticsync-best-practices/best-practices-elasticsync-package-numbers-fulfilment#, Best Practice: Importing package numbers from shipping service providers>>.
...
|*Customer properties*
|Enter coordinates to position the customer properties. Select the <<crm/managing-contacts#4000, property>> from the drop-down list. The text that was saved in the <<crm/managing-contacts#4000, CRM » Contacts » Open contact » View: Properties>> menu is displayed on the address labels.
