...
. Carry out the settings. Pay attention to the information given in <<table-export-list-of-delivery-addresses>>.
...
Besides the convenient option to import package numbers into your system via the <<fulfilment/generating-documents#package-numbers-elasticsync, ElasticSync>>, you can also import the package numbers <<fulfilment/generating-documents#manually-import-package-numbers, manually>> or with the help of a <<fulfilment/generating-documents#package-numbers-barcode-scanner#, barcode scanner>>.

[#package-numbers-elastic-sync]
=== Importing package numbers with the ElasticSync

Using ElasticSync is the easiest way to import package numbers in your plentymarkets system. To do so, create a sync of the type *Orders*. Note our <<data/importing-data/elasticsync-best-practices-elasticsync-package-numbers-fulfilment#, Best Practice: Importing package numbers from shipping service providers>>.
...
|*Customer properties*
|Enter coordinates to position the customer properties. Select the <<crm/managing-customer-data#4000, property>> from the drop-down list. The text that was saved in the <<crm/managing-contacts#4000, CRM » Contacts » Open contact » View: Properties>> menu is displayed on the address labels.
...
. Carry out the settings for the search filters. Pay attention to the information given in <<table-generate-delivery-list>>.
