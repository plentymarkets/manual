...
. Select the filters. Pay attention to the information given in <<tabelle-versandauftrag-suchen>>.
. Click on the magnifying glass (icon:search[role="blue"]) to start searching. +
...
. In the *Shipping packages* tab, click on the pencil (icon:pencil[role="yellow-background"]) to edit the shipping order.
. Carry out the settings. Pay attention to the information given in <<table-edit-shipping-package>>.
...
*_Important:_* The number of shipping packages entered in the *Orders » Edit orders » Open order » Tab: Settings* menu does not influence the packages saved here, i.e. changes you make in that menu are not updated here. This is why you should change the number of packages in the shipping centre only.
...
. Go to *Orders » Shipping centre » Tab: Search*.
. Select the option *registered DHL Intraship/Versenden* from the drop-down list *Shipping status*.
. Click on the magnifying glass (icon:search[role="blue"]). +
...
. Expand the registered order in the *Orders » Shipping centre » Tab: Search* menu. +
...
. Carry out the settings for the options.
. Click on *Execute*. +
...
In the *Orders » Shipping centre » Tab: Label* menu, you can generate address labels for your shipping orders based on the shipping service provider. These labels are made available as download files in PDF format. A PDF file is generated for each shipping service provider.
...
Some shipping interfaces (APIs) only allow Latin character sets (ISO-8859-X), but no Unicode. Therefore, delivery addresses should not contain Unicode signs. For example, you cannot register orders with DHL Intraship/Versenden if the address was saved in Greek letters.
...
It is not possible to add packages in the <<orders/manage-orders#1540, Settings>> tab. If you want to add further packages to an order or change the quantity, open the order in the shipping centre and make the changes in the <<fulfilment/shipping-centre#add-shipping-package, Add>> tab.
