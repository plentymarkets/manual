...
====
As an alternative to the order summary of one day in the *Orders » Shipping centre » Tab: Summary* menu, you can also carry out a <<basics/data-exchange/export-import/exporting-data#, dynamic data export>>. Use the data format <<basics/data-exchange/export-import/data-formats/order#, Order>> and use filters to search by shipping service provider and date. Use the data field *PackageNo* e.g. to export the package numbers.
====
...
It is not possible to add packages in the order's <<order-processing/orders/managing-orders#changing-order-40, Settings>> tab. If you want to add further packages to an order or change the quantity, open the order in the shipping centre and make the changes in the <<fulfilment/shipping-centre#400, Add>> tab.
