...
:position: 20
:url: fulfillment/shipping-centre

In the *Orders » Shipping centre* menu, you can use the integrated interfaces to control all of the processes that are related to shipping your items. You cannot only search for and edit shipping orders but also <<fulfilment/shipping-centre#700, register>> shipping orders directly with a shipping service provider or alternatively create a <<fulfilment/shipping-centre#1400, polling file>> to hand over to a shipping service provider. The appropriate action depends on the shipping service provider and the software used by these providers.

On this page of the manual, you also learn how to generate an <<fulfilment/shipping-centre#1500, order summary>> of the <<fulfilment/shipping-centre#2500, shipping service providers set up in plentymarkets>>. Furthermore, it is also described how to <<fulfilment/shipping-centre#1600, register returns>> and <<fulfilment/shipping-centre#1900, generate labels>>.
...
. Select filters. Pay attention to the information given in <<table-search-shipping-order>>.
...
*_Note_*: Users with the user class *Variable* who select the filter option *ALL* still only see those orders that are in a particular status. This is done by opening the variable user's account and selecting one or more statuses under *Visible order status*. Afterwards, the variable user only sees orders that have this status, even if they chose the filter setting ALL.
...
Once you have found the shipping orders you are looking for, you can edit these shipping orders or register them with the shipping service provider. Proceed as follows to edit shipping orders. Refer to the chapters <<fulfilment/shipping-centre#700, Registering a shipping order>> and <<fulfilment/shipping-centre#1400, Generating a polling file>> on this page of the manual to learn how to register shipping orders.

[#200]
== Editing a shipping package in a pending shipping order

It is possible to edit shipping packages in a pending shipping order. You can also add shipping packages to or delete them from a pending shipping order. You can change the customer data of the recipient in the <<crm/managing-contacts#300, CRM » Contacts>> menu. Open the customer's data record by clicking on the customer's ID in the shipping order.
...
. Go to *Orders » Shipping centre*.
. Select the search criteria. Pay attention to the information given in the <<fulfilment/shipping-centre#100, Searching for a shipping order>> chapter.
...
. Carry out the settings. Pay attention to the information given in <<table-edit-shipping-order>>.
. *Save* the settings.

[[table-edit-shipping-order]]
.Editing a shipping package
[cols="1,3"]
|====
|Setting |Explanation
...
*_Important_*: The number of shipping packages entered in the *Orders » Edit orders » Open order » Tab: Settings* menu does not influence the packages saved here, i.e. changes you make in that menu are not updated here. This is why you should change the number of packages in the shipping centre only.
...
Also note the chapter <<fulfilment/shipping-centre#2000, Possible errors when registering a shipping order>> on this page of the manual.
...
. Expand the registered order in the *Orders » Shipping centre » Tab: Search*. How to display the registered order is described in the <<fulfilment/shipping-centre#900, Displaying a registered order>> chapter.
. Click on the pencil icon in the *Shipping packages* tab. +
→ The package number opens.
. Enter a different package number.
. *Save* the settings.

For further information about importing package numbers, refer to the <<fulfilment/generating-documents#800, Importing package numbers>> chapter on the <<fulfilment/generating-documents#, Generating documents>> page of the manual.
...
The *Reset* function simply resets the registration. The shipping order itself is not cancelled with the service provider. This is why we recommend cancelling the order as described in the <<fulfilment/shipping-centre#, Cancelling a shipping order>> chapter.
...
. Go to *Orders » Shipping centre*.
. Select the search criteria for the order. Pay attention to the information given in the <<fulfilment/shipping-centre#100, Searching for a shipping order>> chapter.
. Activate the order.
...
[TIP]
.Export the order summary via elastic export
====
As an alternative to the order summary of one day in the *Orders » Shipping centre » Tab: Summary* menu, you can also carry out an <<data/exporting-data/elastic-export#, elastic export>>. To do so, create an export format with the <<data/exporting-data/FormatDesigner#, FormatDesigner>>.
====
...
It is not possible to add packages in the order's <<orders/managing-orders#1540, Settings>> tab. If you want to add further packages to an order or change the quantity, open the order in the shipping centre and make the changes in the <<fulfilment/shipping-centre#400, Add>> tab.
