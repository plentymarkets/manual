...
:keywords: shipping service provider, fulfilment service provider, fulfillment, fulfilment, shipping countries, shipping country, country of delivery, countries of delivery, region, regions, shipping profile, shipping profiles, table of shipping charges, shipping packages, shipping package, shipping costs, DHL packstation, DHL postfinder, tracking URL, c.b.c. logistics, cbc logistics, PARCEL.ONE, easylog, DHL Freight, Versenden, DHL Versenden, DHL Retoure, enclosed return labels, Supply Chain, DHL UK, DPD, DELISprint, EasyPak, GLS Germany, Hermes ProfiPaketService, Hermes 2-Mann-Handling, DPD Business, iloxx, UK Mail, NetDespatch, Swiss Post, DHL Fulfillment, IDS Logistik, iLOPACK, export documents, CN23, CP71
...
. Carry out the settings in the *Overview* section as desired. Pay attention to the information given in <<#table-overview-countries-of-delivery>>.
...
*Backoffice is active* = Countries of delivery with this setting are only visible in the plentymarkets system and not in the online store. +
*Supplier is active* = Countries of delivery with this setting cannot be selected as the *Standard country of delivery* in the <<online-store/online-store/order-process#, Setup » Client » Select client » Order process » Settings>> menu.
...
. Carry out the settings in the *Filter* section as desired. Pay attention to the information given in <<#table-filter-countries-of-delivery>>.
. Click on *Search*. +
...
. Expand the warehouse in the section *Shipper data* (icon:plus-square-o[]).
. Enter the data.
. *Save* (icon:save[role="green"]) the settings.
...
. Enter the data.
. Pay attention to the information about the shipping service providers in the sub-chapters of the chapter <<fulfilment/preparing-the-shipment#2500, Overview of the shipping service providers>>.
. *Save* (icon:save[role="green"]) the settings.
...
. Go to *Setup » Orders » Shipping » Settings*.
. Click on the *Shipping service provider* tab.
. Carry out the settings in the *Filter* section as desired. Pay attention to the information given in <<#table-filter-shipping-service-provider>>.
. Click on *Search*. +
...
| *Search*
|Perform a search. The shipping service providers that correspond to the selected filter criteria are displayed in the *Overview* tab.
...
|Place a check mark to activate the extra shipping charges that are saved in the item data record. Up to 2 extra shipping charges can be saved for the item's main variation in the *Shipping* section. *Extra shipping charge 1* is added to the shipping costs for the first item. *Extra shipping charge 2* is added to the shipping costs for all additional items. Extra shipping charges are useful for large or bulky items that are particularly expensive to ship. +
...
*_Important:_* It is required that the plugins link:https://marketplace.plentymarkets.com/en/plugins/templates/Ceres_4697[Ceres^] and link:https://marketplace.plentymarkets.com/en/plugins/templates/IO_4696[IO^]{nbsp}icon:external-link[] are installed.
...
|Activate the option so that a checkbox is displayed in the Ceres online store. If the customer activates this checkbox, he agrees that his telephone number and email address are transferred to the shipping service provider.
...
|Many shipping service providers charge a higher fee when shipping to island regions. The seller should not be left with these costs. Therefore, open each shipping profile and define a surcharge that is added when shipping to island regions. Postcodes for island regions are saved in the system. The order is automatically synchronised. If you activated the extra charge for islands, this charge is only calculated if the system recognises this postcode as an island region. +
...
*_Example:_* Shipping group A contains the shipping profiles 1 and 2. Shipping group B contains the shipping profiles 3 and 4. If you want to assign an item to shipping profiles 3 and 4, open the item and select shipping group B from the drop-down list in the item's *Global* tab in the *Shipping profiles* section. +
...
*_Important:_* If you select a loyalty program for a shipping profile, you can only use the shipping profile for the market. It is not possible to use the shipping profile for the online store. If, for example, the *eBay Plus* option is activated in a shipping profile, the shipping profile is only used for *eBay Plus* orders. Therefore, create an extra shipping profile that you only use for the loyalty program.
|====
...
* For 1 PC: EUR 3.99 (basic shipping charge) + EUR 5.00 (Extra shipping charge 1) = EUR 8.99
* For 2 PCs: EUR 3.99 (basic shipping charge) + EUR 5.00 (Extra shipping charge 1) + EUR 3.00 (Extra shipping charge 2) = EUR 11.99
* For 3 PCs: EUR 3.99 (basic shipping charge) + EUR 5.00 (Extra shipping charge 1) + EUR 3.00 (Extra shipping charge 2) + EUR 3.00 (Extra shipping charge 2) = EUR 14.99
...
Adapt the error code also in the *Setup » Client » Global » Language packages* menu in the *Error messages* section , if necessary.
====
...
*_Note:_* This function can only be used from Ceres version 4.0.0.

2+|Specific settings of the shipping service provider

|Name of the shipping service provider
| *_Note:_* The settings of the shipping service provider become visible once you save and reload the table of shipping charges.
...
In this case, it would be wise to set up an extra shipping charge for each additional item. You can do so by saving <<item/managing-items#290, extra shipping charges>> within the items' *Shipping* section and by activating the *Item extra shipping charge* option in the *Base* tab of the shipping profile.
...
====
The volume is only calculated correctly if you saved the item's length, width and height within the main variation's *Settings* tab in the <<item/managing-items#270, Dimensions>> section in the _Item » Edit item_ » *Open item* menu. It is not possible to calculate the volume if these entries are missing. Therefore, it is not possible to calculate the shipping costs based on the volume either.
====
...
====
If an order contains items for which no shipping profile was activated, the items are given the default shipping profile of the default online store. The standard shipping profile can be selected in the <<online-store/online-store/order-process#, Setup » Client » Select client » Orders>> menu. If no default shipping profile was specified, the shipping profile with the highest priority is used.
====
...
====
Defined shipping packages: A seller ships their items in 2 different cardboard box sizes. Therefore, they need to set up 2 shipping packages. The volume of these packages results from the dimensions entered and saved. +
...
Proceed as follows to set up the interface to link:https://www.dhl.de/en/geschaeftskunden/paket/versandsoftware.html[DHL Easylog^]{nbsp}icon:external-link[] in plentymarkets.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *DHL Easylog* section. Pay attention to the information given in <<#table-shipping-services-dhl-easylog>>.
. *Save* (icon:save[role="green"]) the settings.
...
Proceed as follows to set up the interface to link:https://www.dhl.de/en/geschaeftskunden/logistik.html[DHL Freight^]{nbsp}icon:external-link[] in plentymarkets.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *DHL Freight* section. Pay attention to the information given in <<#table-shipping-services-dhl-freight>>.
. *Save* (icon:save[role="green"]) the settings.
...
====
Via the interface to the DHL business customer portal, you can use the function *Ship*. The plugin *DHL Shipping (Versenden)* is available in the link:https://marketplace.plentymarkets.com/en/plugins/integration/DHLShipping_4871[plentyMarketplace^]{nbsp}icon:external-link[].
...
Proceed as follows to set up the interface to link:https://www.dhl.de/en/geschaeftskunden/paket/versandsoftware/dhl-geschaeftskundenportal-anmeldung.html[DHL Intraship/Versenden^]{nbsp}icon:external-link[] in plentymarkets.
...
*EKP* = PP = customer number; consists of the first 10 numbers of the billing number. +
...
*_Important:_* Pay attention to the explanations given in the box below the table. +
*Subscription* = The last two characters of the billing number (only numbers or letters are permitted).
...
*_Important:_* Save the logo's URL with the protocol *http://*. +
*Alternatively, use address details from the following accounting number* = Select one of the accounting numbers that you have just saved from the drop-down list.
|====

[IMPORTANT]
.DHL billing number
====
The two middle characters of the billing number *Process* stand for the following DHL products: DHL Paket (01), Regionalpaket AT (02), DHL Europaket (54), DHL Paket International (53), DHL Domestic Express (71) and DHL Officepack (72). +
*_Important:_* DHL Intraship/Versenden uses the number 01 for the product Regionalpaket AT. However, we use the number 02 because there can be conflicts in plentymarkets with DHL Paket. DHL Domestic Express uses the number 71 so that there are no conflicts with DHL Officepack. Of course, the correct number is sent to DHL.
====
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *DHL Intraship/Versenden* section. Pay attention to the information given in <<#table-shipping-services-dhl-intraship-versenden>>.
. *Save* (icon:save[role="green"]) the settings.

[[table-shipping-services-dhl-intraship-versenden]]
.Setting the DHL Intraship/Versenden shipping services in the table of shipping charges
[cols="1,3"]
|====
|Setting |Explanation
...
DHL export documents are only generated for orders that are shipped with the product *DHL Paket International* and that are actually export shipments. Export shipments have to contain a customs declaration, the so-called CN23 document, and a dispatch note, the so-called CP71 document. *DHL Paket International* can also be used for intra-community deliveries. However, no export documents are generated in this case.
...
If you are using the DHL Intraship/Versenden interface, you can manually generate the export documents via the *Orders » Fulfilment* menu. In order for orders to be found for the creation of export documents, they have to be successfully registered with DHL Intraship/Versenden via the *Orders » Shipping centre* menu. Furthermore, an invoice has to exist for these orders.

[TIP]
.Export documents automatically available in the shipping centre via DHL Shipping plugin
====
If you use the DHL Shipping plugin, export documents are automatically available in the shipping centre next to the shipping label when you register the shipping order. This function is available as of version 1.3. The DHL Shipping (Versenden) plugin is available in the link:https://marketplace.plentymarkets.com/en/plugins/integration/DHLShipping_4871[plentyMarketplace^]{nbsp}icon:external-link[].
...
. Expand the *Generate DHL Intraship export documents* sub-menu.
. Carry out the settings. Pay attention to the information given in <<#table-dhl-export-documents>>.
. Click on the gear wheel (icon:plus-square-o[]).  +
→ The export documents are generated as PDF.
...
Further information can be found on the <<item/settings/properties#100, Characteristics and properties>> page of the manual.

[#3030]
==== Carrying out the basic setting

Go to *Setup » Item » Settings* and select the characteristic that you have just created for the option *Item characteristic for visual age check*. *Save* (icon:save[role="green"]) the settings.
...
. Activate the property that you have just created in the *Activate further characteristics* section.
. *Save* (icon:save[role="green"]) the settings. +
→ The property is displayed in the *Characteristics* area.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. In the *DHL plentymarkets Import* section, select the option *DHL Paket* from the drop-down list *Product*.
...
Proceed as follows to set up the interface to link:https://www.dhl.de/en/geschaeftskunden/paket/leistungen-und-services/retoure.html[DHL Retoure with enclosed return label^]{nbsp}icon:external-link[] in plentymarkets.
...
DHL Retoure Online allows you to generate return labels for orders with various countries of delivery. The labels are generated in the *Orders » Shipping centre* menu in the *Return* tab. You can also make the label available to your customer in the *My account* area of your online store or automatically send an email with the label using an event procedure.

[TIP]
.Assistant for easy setup of DHL Retoure Online
======
Purchase the *DHL Retoure Online* plugin free of charge on link:https://marketplace.plentymarkets.com/en/plugins/integration/dhlretoureonline_6714[plentyMarketplace^]{nbsp}icon:external-link[] and install it in a plugin set. The *DHL Retoure Online* assistant is visible in the *Setup » Assistants* menu in the *Integration* section.
======

==== Setting up a warehouse

You need to have a warehouse in order to save the information about returns that you receive from DHL. There are various types of warehouses in plentymarkets. The warehouse type *Repair* is especially well-suited for returns. The setting *Warehouse* assigned for repair in the *Setup » Stock » Warehouse* » _Select warehouse_ » *Settings* menu allows you to assign repair warehouses to main warehouses of the type *Sales warehouse*. When a return label is generated later, the corresponding sales or repair warehouse is used depending on which warehouse matches the order.
...
==== Old online store: Allowing customers to create returns

Now determine whether your customers are allowed to create returns themselves within their My account area in the old online store (Callisto online store).

[.instruction]
Old online store: Allowing customers to create returns:

. Go to *Setup » Client* » _Select client_ » *My account » Settings*.
...
| *Return*
| *activated* = Customer can create returns in the My account area of the old online store (Callisto online store). However, labels are not provided. You need to decide how you want to proceed when customers return items. For example, you can send customers the label via email. +
...
*DHL Retoure Online* = Customers can create returns in the My account area in the old online store (Callisto online store) and download and print the DHL return label afterwards. +
...
Proceed as follows to set up the interface to link:https://www.logistics.dhl/de-en/home/our-divisions/supply-chain.html[DHL Supply Chain^]{nbsp}icon:external-link[] in plentymarkets.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. In the *DHL Supply Chain* section, select the option *One-person handling* or *Two-person handling* from the drop-down list in the line *DHL product*.
. *Save* (icon:save[role="green"]) the settings.
...
Proceed as follows to set up the interface to link:http://www.dhl.co.uk/en.html[DHL UK^]{nbsp}icon:external-link[] in plentymarkets.
...
====
After you entered and saved the data in the sections *Billing information* and *Shipper address* in the *Setup » Orders » Shipping » Shipping service provider » DHL UK* menu, a new tab is added in the *Shipper accounts* section. This tab is named according to the shipper account number that you entered in the *Billing information* section. You may add as many shipper accounts as you like. This is done via the tab *New*.
====
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *DHL UK* section. Pay attention to the information given in <<#table-shipping-services-dhl-uk>>.
. *Save* (icon:save[role="green"]) the settings.
...
[IMPORTANT]
.Using the DPD Shipping Services plugin
====
Use the plugin "DPD Shipping Services". The plugin is available in the link:https://marketplace.plentymarkets.com/en/plugins/integration/dpdshippingservices_6320[plentyMarketplace^]{nbsp}icon:external-link[].
====
...
Proceed as follows to set up the interface to link:https://esolutions.dpd.com/entwickler/dpd-cloud-webservice.aspx[DPD Cloud Webservice^]{nbsp}icon:external-link[] in plentymarkets.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. In the *DPD Cloud Webservice* section, select a service from the drop-down list in the line *Shipping service*.
. *Save* (icon:save[role="green"]) the settings.

Pay attention to the information about the shipments that you later register for DPD Cloud Webservice via the <<fulfilment/shipping-centre#register-shipping-order, shipping centre>>. Go to *Data » API log* and click on the *Shipping centre API log* tab to view this information. For further information about API log entries for DPD Cloud Webservice, refer to the <<data/api-log#, API log>> page of the manual.
...
Before setting up the interface to DPD DELISprint in plentymarkets, register with link:https://www.dpd.com/group/en/[DPD^]{nbsp}icon:external-link[]. DPD customers can use the DPD DELISprint program to easily create DPD parcel labels.

How you set up shipping service providers and shipping profiles and which additional settings you have to carry out and may carry out, is described in the chapters from <<fulfilment/preparing-the-shipment#100, Setting up countries of delivery>> through <<fulfilment/preparing-the-shipment#2400, Retrieving the API log>>. The settings described there are the basic requirements in order to carry out the settings that are specific for each shipping service provider. Therefore, first carry out the settings in the chapters mentioned above. Afterwards, continue with this chapter.

Proceed as follows to set up the interface to link:https://www.dpd.com/nl_en/business_customers/help_en_tools/labelprint_tools/delisprint[DPD DELISprint^]{nbsp}icon:external-link[] in plentymarkets.
...
| *Flex/Predict active*
|Activate to register the notification service link:https://www.dpd.com/de/en/empfangen[Predict^]{nbsp}icon:external-link[] (formerly FlexDelivery) with DPD. If this option is chosen, the CSV file exported from plentymarkets contains the additional information *PRO*.
|====
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *PAT Easypak* section. Pay attention to the information given in <<#table-shipping-services-easypak>>.
. *Save* (icon:save[role="green"]) the settings.
...
*_Important:_* The data entered here is only contained in the CSV file that is generated in the *Orders » Fulfilment * menu in the *List of delivery addresses* section with the data format *GLS preadvise*.
|====
...
During the order process, customers should be able to request that their items are assembled for them. You can offer customers such a service with link:https://www.hermesworld.com/en/about-us/hermes-group/hermes-einrichtungs-service/[Hermes Two-Man Handling^]{nbsp}icon:external-link[]. In order to use this service, create an order property. Create an order property with the options *Yes* and *No*. Depending on which *property type* you select, you are able to configure the options that your customers later selects in your online store.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *Hermes (two-man handling)* section. Pay attention to the information given in <<#table-shipping-services-hermes-2mannhandling>>.
. *Save* (icon:save[role="green"]) the settings.
...
====
Hermes switches off the ProfiPaketService interface by 31 March 2020. Therefore, use *Hermes Shipping Interface* instead. Download the plugin free of charge on link:https://marketplace.plentymarkets.com/en/plugins/integration/hermesshippinginterface_5437[plentyMarketplace^]{nbsp}icon:external-link[].
====
...
Proceed as follows to set up the interface to link:http://www.dpd.iloxx.de/home.aspx[MyDPD Business / iloxx^]{nbsp}icon:external-link[] in plentymarkets.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. In the *MyDPD Business/iloxx* section, select a service from the drop-down list in the line *Shipping service*.
. *Save* (icon:save[role="green"]) the settings.
...
Proceed as follows to set up link:https://www.ukmail.com/[UK Mail^] via the integrated link:http://www.netdespatch.com/[NetDespatch^]{nbsp}icon:external-link[] interface in your plentymarkets system.

[.instruction]
Setting up the interface:

. Go to *Setup » Orders » Shipping » Shipping service provider » NetDespatch*.
. Expand the section *UK Mail settings* (icon:plus-square-o[]).
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *NetDespatch* section. Pay attention to the information given in <<#table-shipping-services-netdespatch-ukmail>>.
. *Save* (icon:save[role="green"]) the settings.
...
Proceed as follows to set up the interface to link:http://www.parcelforce.com/[Parcelforce Worldwide^]{nbsp}icon:external-link[] in plentymarkets.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *Parcelforce Worldwide* section. Pay attention to the information given in <<#table-shipping-services-parcelforce-worldwide>>.
. *Save* (icon:save[role="green"]) the settings.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *PARCEL.ONE* section. Pay attention to the information given in <<#table-shipping-services-parcel-one>>.
. *Save* (icon:save[role="green"]) the settings.
...
|Select *YES*. Our interface has been adapted to reflect modifications made by Swiss Post. All customers receive new access data. Enter the access data here. Contact Swiss Post in case of problems with the new configuration.
|====
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *UPS Service* section. Pay attention to the information given in <<#table-shipping-services-ups>>.
. *Save* (icon:save[role="green"]) the settings.
...
DHL runs a network of Packstations that are accessible any time of the day. Customers can choose to activate or enter a DHL Packstation or DHL post office as the delivery address. Carry out the settings for the DHL Packstation in the *Setup » Client* » _Select client_ » *Order process » DHL Packstation* menu.
...
. Go to *Setup » Client* » _Select client_ » *Order process » DHL Packstation*.
...
*_Important:_* The default entry 0.00 stands for no limitation regarding the order value.

| *Packstation validation*
|Select whether DHL Packstations should be validated (active) or not (inactive). The data synchronisation that is carried out during the validation offers even more security and fraud protection. The address data of the customer (invoice address and email address) are synchronised with the PostNummer. An error message is displayed if the PostNummer does not match the customer or if the customer has been blocked.
...
The texts that are displayed in the order process are saved in the *Customer registration*, *Invoice address* and *Delivery address* sub-menus of the *CMS » Web design » Order process » Checkout* menu.
|====
...
This feature shows your customer a list of post offices and Packstations nearby. The list is displayed when your customer enters his or her postcode in the My account area or during the <<online-store/online-store/cms#web-design-editing-the-web-design-order-process-dhl-postfinder#, checkout process>>. Your customers are able to use the address of a post office or Packstation directly in the delivery address.

In order to use DHL Postfinder, you have to carry out settings in multiple menus. These settings are summarised on the <<fulfilment/preparing-the-shipment#4600, DHL Postfinder>> page of the manual.
...
Table <<#table-overview-fulfilment-service-providers>> lists all of the fulfilment service providers available in plentymarkets. Click on the service provider in the table to jump to the sub-chapter where the specific settings of the fulfilment service provider are described.
...
Registering the shipment with DHL Fulfillment takes place automatically via the interface. The *Data » API log* menu contains information about shipments that were registered with DHL Fulfillment. For further information about API log entries for DHL Fulfillment, refer to the <<data/api-log#, API log>> page of the manual.
...
Go to *Setup » Orders » Shipping » Options* and click on the *Shipping profiles* tab. In the *Table of shipping charges* tab of DHL Fulfillment shipping profile in the *DHL Fulfillment* section, you can select the options Germany, United Kingdom or France from the drop-down list *DHL Fulfillment region*.
...
The *Data » API log* menu contains information about shipments that were registered with DHL Fulfillment. For further information about API log entries for DHL Fulfillment, refer to the <<data/api-log#, API log>> page of the manual.
====
...
| *Interface active*
|Activate the option in order to activate the interface. Do not activate the interface until you <<fulfilment/preparing-the-shipment#4900, created the warehouses>>.
...
|Select the properties that you created for transferring these item properties to DHL Fulfillment. For further information, refer to the <<fulfilment/preparing-the-shipment#5300, Transferring characteristics>> chapter.
...
Create characteristics in the *Setup » Item » Characteristics* menu and transfer item characteristics such as  "Fragile" or "Original packaging" to DHL Fulfillment if you want the items to be handled or shipped in a certain way. How to create a characteristic is described on the <<item/settings/properties#300, Characteristics and properties>> page of the manual. The following characteristics can be transferred to DHL Fulfillment:
...
[IMPORTANT]
.Selecting the correct characteristics type for the properties
====
...
Before setting up the interface to link:http://www.ids-logistik.de[IDS Logistik^]{nbsp}icon:external-link[] in plentymarkets, register with IDS Logistik.
...
The abbreviation FBA stands for Fulfilment by Amazon, a service from Amazon where Amazon takes care of the entire stock management and logistics. Refer to the chapter <<markets/amazon/amazon-setup#3300, Using the FBA service by Amazon>> on the <<markets/amazon/amazon-setup#, Amazon>> page of the manual. This chapter describes which settings you need to carry out.

== Amazon Prime

Use filters to make sure that orders with shipping conditions for Amazon Prime are separately treated in plentymarkets. Furthermore, you can create shipping labels for Amazon Prime orders via the Amazon Merchant Fulfilment API. Refer to the chapter <<markets/amazon/amazon-setup#4200, Editing Amazon Prime orders>> on the <<markets/amazon/amazon-setup#, Amazon page>> of the manual. This chapter describes which settings you need to carry out.
