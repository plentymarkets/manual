...
. Carry out the settings in the *Overview* section as desired. Pay attention to the information given in <<tabelle-uebersicht-lieferlaender>>.
...
. Carry out the settings in the *Filter* section as desired. Pay attention to the information given in <<tabelle-filter-lieferlaender>>.
. Click on *Search*. +
...
. Go to *Setup » Orders » Shipping » Settings*.
. Click on the *Shipping service provider* tab.
. Carry out the settings in the *Filter* section as desired. Pay attention to the information given in <<tabelle-filter-versanddienstleister>>.
. Click on *Search*. +
...
Use the *Table of shipping charges* tab to enter shipping costs for a shipping profile. Save shipping charges for every region that you want to use with the shipping profile. Select the calculation type and the shipping charge. You see additional settings for specific service providers once you have saved and reloaded the table of shipping charges. First carry out the general settings. Pay attention to the information given in <<tabelle-portotabelle-versandprofil>>.
...
<<tabelle-ueberblick-versanddienstleister>> lists all of the shipping service providers available in plentymarkets. Click on the service provider in the table to jump to the sub-chapter where the specific settings of the shipping service provider are described.
...
Proceed as follows to set up the interface to link:https://www.dhl.de/en/geschaeftskunden/paket/versandsoftware.html[DHL Easylog^] in plentymarkets.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *DHL Easylog* section. Pay attention to the information given in <<tabelle-versandservices-dhl-easylog>>.
. *Save* (icon:save[role="green"]) the settings.
...
Proceed as follows to set up the interface to link:https://www.dhl.de/en/geschaeftskunden/logistik.html[DHL Freight^] in plentymarkets.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *DHL Freight* section. Pay attention to the information given in <<tabelle-versandservices-dhl-freight>>.
. *Save* (icon:save[role="green"]) the settings.
...
====
Via the interface to the DHL business customer portal, you can use the function *Ship*. The plugin *DHL Shipping (Versenden)* is available in the link:https://marketplace.plentymarkets.com/en/plugins/integration/DHLShipping_4871[plentyMarketplace^].
...
Proceed as follows to set up the interface to link:https://www.dhl.de/en/geschaeftskunden/paket/versandsoftware/dhl-geschaeftskundenportal-anmeldung.html[DHL Intraship/Versenden^] in plentymarkets.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *DHL Intraship/Versenden* section. Pay attention to the information given in <<tabelle-versandservices-dhl-intraship-versenden>>.
. *Save* (icon:save[role="green"]) the settings.
...
====
If you use the DHL Shipping plugin, export documents are automatically available in the shipping centre next to the shipping label when you register the shipping order. This function is available as of version 1.3. The DHL Shipping (Versenden) plugin is available in the link:https://marketplace.plentymarkets.com/en/plugins/integration/DHLShipping_4871[plentyMarketplace^].
...
. Activate the property that you have just created in the *Activate further properties* section.
. *Save* (icon:save[role="green"]) the settings. +
...
Proceed as follows to set up the interface to link:https://www.dhl.de/en/geschaeftskunden/paket/leistungen-und-services/retoure.html[DHL Retoure with enclosed return label^] in plentymarkets.
...
. Enter a name.
. Select the *event* as listed in <<tabelle-ereignisaktion-dhl-retourenetiketten>>.
. *Save* (icon:save[role="green"]) the settings.
. Carry out the settings according to <<tabelle-ereignisaktion-dhl-retourenetiketten>>.
...
======
We recommend buying the *DHL Retoure Online* plugin free of charge on link:https://marketplace.plentymarkets.com/en/plugins/integration/dhlretoureonline_6714[plentyMarketplace^]. Install the plugin in a plugin set. The *DHL Retoure Online* assistant is visible in the *Setup » Assistants* menu.
======
...
. Select one of the options from the *Return* drop-down list. Pay attention to the information given in <<tabelle-retourenanlage-erlauben>>.
. *Save* (icon:save[role="green"]) the settings.
...
Proceed as follows to set up the interface to link:https://www.logistics.dhl/de-en/home/our-divisions/supply-chain.html[DHL Supply Chain^] in plentymarkets.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *DHL UK* section. Pay attention to the information given in <<tabelle-versandservices-dhl-uk>>.
. *Save* (icon:save[role="green"]) the settings.
...
====
Use the plugin “DPD Shipping Services”. The plugin is available in the link:https://marketplace.plentymarkets.com/en/plugins/integration/dpdshippingservices_6320[plentyMarketplace^].
====
...
Before setting up the interface to DPD DELISprint in plentymarkets, register with link:https://www.dpd.com/group/en/[DPD^]. DPD customers can use the DPD DELISprint program to easily create DPD parcel labels.

How you set up shipping service providers and shipping profiles and which additional settings you have to carry out and may carry out, is described in the chapters from <<fulfilment/preparing-the-shipment#100, Setting up countries of delivery>> through <<fulfilment/preparing-the-shipment#2400, Retrieving the API log>>. The settings described there are the basic requirements in order to carry out the settings that are specific for each shipping service provider. Therefore, first carry out the settings in the chapters mentioned above. Afterwards, continue with this chapter.

Proceed as follows to set up the interface to link:https://www.dpd.com/nl_en/business_customers/help_en_tools/labelprint_tools/delisprint[DPD DELISprint^] in plentymarkets.
...
|*Flex/Predict active*
|Activate to register the notification service link:https://www.dpd.com/de/en/empfangen[Predict^] (formerly FlexDelivery) with DPD. If this option is chosen, the CSV file exported from plentymarkets contains the additional information *PRO*.
|====
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *PAT Easypak* section. Pay attention to the information given in <<tabelle-versandservices-easypak>>.
. *Save* (icon:save[role="green"]) the settings.
...
During the order process, customers should be able to request that their items are assembled for them. You can offer customers such a service with link:https://www.hermesworld.com/en/about-us/hermes-group/hermes-einrichtungs-service/[Hermes Two-Man Handling^]. In order to use this service, create an order property. Create an order property with the options *Yes* and *No*. Depending on which *property type* you select, you are able to configure the options that your customers later selects in your online store.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *Hermes (two-man handling)* section. Pay attention to the information given in <<tabelle-versandservices-hermes-2mannhandling>>.
. *Save* (icon:save[role="green"]) the settings.
...
====
Hermes switches off the ProfiPaketService interface by 31March2020. Therefore, use *Hermes Shipping Interface* instead. Download the plugin free of charge on link:https://marketplace.plentymarkets.com/en/plugins/integration/hermesshippinginterface_5437[plentyMarketplace^].
====
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *NetDespatch* section. Pay attention to the information given in <<tabelle-versandservices-netdespatch-ukmail>>.
. *Save* (icon:save[role="green"]) the settings.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *Parcelforce Worldwide* section. Pay attention to the information given in <<tabelle-versandservices-parcelforce-worldwide>>.
. *Save* (icon:save[role="green"]) the settings.
...
. Click on the *Table of shipping charges* tab.
. Expand the region (icon:angle-down[]).
. Carry out the settings in the *UPS Service* section. Pay attention to the information given in <<tabelle-versandservices-ups>>.
. *Save* (icon:save[role="green"]) the settings.
...
Table <<tabelle-ueberblick-fulfillment-dienstleister>> lists all of the fulfilment service providers available in plentymarkets. Click on the service provider in the table to jump to the sub-chapter where the specific settings of the fulfilment service provider are described.
...
|Select the properties that you created for transferring these item properties to DHL Fulfillment. For further information, refer to the <<fulfilment/preparing-the-shipment#5300, Transferring properties>> chapter.
...
Create properties in the *Setup » Item » Properties* menu and transfer item properties such as  “Fragile” or “”Original packaging” to DHL Fulfillment if you want the items to be handled or shipped in a certain way. How to create a property is described on the <<item/managing-items#, Managing items>> page of the manual in the <<item/managing-items#500, Managing properties>> chapter. The following properties can be transferred to DHL Fulfillment:
...
====
Go to *Setup » Item » Properties* and create the following 3 properties of the type *None*:

* *Expiration date*
* *Original packaging*
* *Fragile*

Create the property *Serial number* with the property type *Selection* and save the following 3 values:
...
Before transmitting the order to DHL Fulfillment, the recipient's address is checked with respect to a valid form and structure; i.e. plentymarkets checks forexample if all mandatory fields such as the name are filled out. +
...
Before setting up the interface to link:http://www.ids-logistik.de[IDS Logistik^] in plentymarkets, register with IDS Logistik.
...
Proceed as follows to set up the interface to iLOPACK. Below <<tabelle-schnittstelle-ilopack>>, it is described how to set up the data transfer to iLOPACK.
