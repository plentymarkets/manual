...
plentymarkets allows sellers to manage their fully customised shipping processes and trigger nearly all processes automatically. Your plentymarkets system efficiently controls the entire process with the **<<tools/plentymarkets-client#, plentymarkets Client  >>**. It carries out everything from linking items with shipping profiles to registering shipments and printing shipping labels. The initial time that it takes to configure the settings will pay off when you have an efficient shipping process that automatically carries out routine tasks.
...
. Go to **Settings » Orders » Shipping » Shipping options » Tab: Countries of delivery » Tab: Overview**.
. Carry out the settings as desired. Pay attention to the information given in table 1.
. **Save** the settings.

.Overview of countries of delivery in the **Settings » Orders » Shipping » Shipping options » Tab: Countries of delivery** menu
[cols="a,a"]
|====
|Setting |Explanation
...
**Supplier is active** = Countries of delivery with this setting cannot be selected as the **Standard country of delivery** in the **<<client-store/standard/order-process/basic-settings#, Settings » Client (store) » Standard » Order process » Basic settings  >>** menu.

|**Warehouse**
|Select which warehouse the country of delivery should be assigned to. This warehouse will automatically be selected for orders from this country. Select **ALL** if the country of delivery should be assigned to all of the warehouses. In this case, the warehouse with the lowest ID will automatically be selected for the orders. Pay attention to the <<orders/settings/basic-settings#, **warehouse settings**  >> in the **Settings » Orders » Basic settings** menu as well.
...
. Go to **Settings » Orders » Shipping » Shipping options » Tab: Countries of delivery » Tab: Filter**.
...
Use the menu **Settings » Orders » Shipping » Shipping service provider » Basic settings** to specify which data should be transferred to shipping service providers when <<item/managing-items#920, **item bundles** >> are used. You can either transmit shipping data for the basic item or for the item bundle. The item bundle setting is valid for all shipping providers that are set up in your plentymarkets system. Furthermore, use this menu to save return data for all of the <<stock-management/setting-up-a-warehouse#, **warehouses**  >> in your plentymarkets system. Return data is saved for each warehouse. This menu lists the warehouses for all of your active countries of delivery.

[.instruction]
Entering item bundles and return data:

. Go to **Settings » Orders » Shipping » Shipping service provider » Basic settings**.
...
. Go to **Settings » Orders » Shipping » Shipping service provider**.
...
.Overview in the **Settings » Orders » Shipping » Shipping options » Tab: Shipping service provider** menu
[cols="a,a"]
|====
|Setting |Explanation
...
A functional tracking URL should include the package number and the recipient's postcode. Therefore, make sure that your tracking URLs include the template variables **$PaketNr** and, if necessary, **$PLZ**. Contact your shipping service provider to obtain the tracking URL. For further information about shipping service providers, refer to the sub-chapters within the chapter **<<fulfilment/preparing-the-shipment#2500, Overview of the shipping service providers >>**.
====
...
The individual shipping profiles are different shipping methods, which your customers can choose later on during checkout or which are assigned automatically to certain types of orders by the online store. For example, you can offer your customers a cheap standard delivery and a more expensive express delivery. You can also set up another shipping profile for eBay listings which the system will then exclusively use for orders from the market eBay. Find tips and tricks in the sub-chapters which you can make use of when creating your shipping profiles. Afterwards, it is describes how you create <<fulfilment/preparing-the-shipment#1500, **tables of shipping charges** >> for every shipping service provider and region.
...
__**Important**__: You need to have set up at least two accounts, otherwise this option is not visible. Further information can be found in the chapter **<<fulfilment/preparing-the-shipment#1200, Tips and tricks: Shipping costs for orders placed on eBay >>**.
...
__**Important**__: Make sure that the shipping service provider offers express delivery. This is required to use the option.

|**Loyalty program**
|Select one or more loyalty programs. You can use the loyalty programs with <<multi-channel/ebay#6600, **eBay Plus** >> or <<multi-channel/amazon#4200, **Amazon** >>. +
__**Important**__: If you select a loyalty program for a shipping profile, then you will only be able to use the shipping profile for the market. It will not be possible to use the shipping profile for the online store. If, for example, the **eBay Plus** option is activated in a shipping profile, then the shipping profile will only be used for **eBay Plus** orders. Therefore, create an extra shipping profile that you only use for the loyalty program.
|====
...
We recommend that you do not apply the shipping costs that are calculated by eBay, but rather that you let these shipping costs be calculated by plentymarkets. This has the advantage that the shipping costs can also be correctly calculated with the purchase of additional items. In some cases, it can also happen that no shipping costs are transmitted by eBay. This problem would also be solved by having plentymarkets calculate the correct shipping costs for you.

[NOTE]
.Setting up the calculation of eBay shipping costs
====
Go to **Settings » Listings » Checkout** and under the option **Shipping costs for new orders coming from eBay**, select **Have it calculated by plentymarkets**.
====
...
====
If you have activated several eBay accounts in your plentymarkets system, you can define an individual shipping profile for each eBay account. Use the option **eBay account** in the **Settings » Orders » Shipping » Shipping options » Tab: Shipping profiles » Tab: Base** menu to select one of the different accounts.
====
...
====
Not every shipping service provider includes additional shipping services. If available and once you have saved and reloaded the table of shipping charges, the shipping services will be displayed in the table of shipping charges. They are displayed underneath the name of the service provider. Go to **Settings » Orders » Shipping » Shipping options » Tab: Shipping profiles** and open the shipping profile. Click on the **Table of shipping charges** tab and expand the region. The shipping services will be displayed in the lower portion of the table of shipping charges.
====
...
.Template variables
====
You will find <<cms-syntax/basics/template-variables#, **template variables**  >> for the various calculation types in the menu <<cms-syntax/web-design/itemview/container-itemviewsingleitem#, **CMS » Web design » Layout » ItemView » ItemViewSingleItem**  >>. The template variable **$Volumen** doesn't display dimensions, but rather the volume in cubic metres.
====

[.subhead]
Calculation type: Flat rate[#1610]
...
[.subhead]
Calculation type: Weight-dependent[#1620]
...
[.subhead]
Calculation type: Volume-dependent[#1630]
...
[.subhead]
Calculation type: Quantity-dependent[#1640]
...
[.subhead]
Calculation type: Price-dependent[#1650]
...
[.subhead]
Calculation type: Item shipping charge[#1660]
...
For further information, refer to the chapter <<data-exchange/exporting-data#30, **Exporting data formats for price search engines** >> on the <<data-exchange/exporting-data#, **Exporting data**  >> page of the manual.
...
. Go to **Settings » Orders » Shipping » Shipping options » Tab: Calculate shipping costs**.
...
. Go to **Settings » Orders » Shipping » Shipping packages**.
...
The <<tools/plentymarkets-client#, **plentymarkets Client**  >> is a tool that you can use to manage all of your shipping processes. It is possible to set up and customise very specific processes and then carry out these processes automatically. For example, you can use the procedures <<processes/procedures#340, **Polling** >>, <<processes/procedures#380, **Return label** >>, <<processes/procedures#390, **Return slip** >> and <<processes/procedures#430, **Shipping centre** >> to process your shipments.
...
For further information about API log entries for DHL Fulfillment and DPD Cloud Webservice, refer to the <<data-exchange/api-log#, **API log**  >> page of the manual.
...
|**<<fulfilment/preparing-the-shipment#2600, c.b.c. logistics >>**
...
|<<fulfilment/preparing-the-shipment#3900, **Hermes (two-person handling)** >>
|For sellers within Germany. Shipping: Germany and Europe.

|<<fulfilment/preparing-the-shipment#4000, **Hermes ProfiPaketService** >> (professional parcel service)
|For sellers within Germany. Shipping: Germany and Europe.
...
Proceed as follows to set up the interface to link:http://en.cbc-logistics.com/[**c.b.c. logistics**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » CBC Logistics**.
...
Proceed as follows to set up the interface to link:https://www.dhl.de/en/paket/geschaeftskunden/versandsoftware.html[**DHL Easylog**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » DHL Easylog**.
...
Proceed as follows to set up the interface to link:http://www.dhl.de/de/logistik/frachttransport/land-schienentransport.html[**DHL Freight**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » DHL Freight**.
...
This section explains how to generate DHL export documents.

Proceed as follows to set up the interface to link:http://www.dhl.de/de/paket/geschaeftskunden/versandsoftware.html[**DHL Intraship/Versenden**, window="_blank"] in plentymarkets.

[IMPORTANT]
.Renaming of the interface DHL Versenden
====
Several months ago, DHL began offering a new service called "Versenden" in its business portal (VLS API). This service is not currently offered in plentymarkets since the VLS interface has not yet been adequately documented. This step is, however, in progress. During this transitional period, the shipping service provider is listed in plentymarkets as **DHL Intraship/Versenden**. Both this interface and the settings that relate to DHL Intraship/Versenden in other menus are valid for existing DHL Intraship customers as well as for new DHL Versenden customers.
====

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » DHL Intraship/Versenden**.
...
**__Important__**: Pay attention to the explanation given in the box below the table. +
...
**__Important__**: Save the logo's URL with the protocol **http://**. +
...
**__Important__**: DHL Intraship/Versenden uses the number 01 for the product Regionalpaket AT. However, we use the number 02 since there can be conflicts in plentymarkets with DHL Paket. DHL Domestic Express uses the number 71 so that it does not conflict with DHL Officepack. Of course, the correct number is sent to DHL.
====
...
DHL export documents are only generated for orders that are shipped with the product **DHL Paket International** and that are actually export shipments. DHL Paket International can also be used for shipments within the EU. However, no export documents are generated in this case.

Shipments to countries outside of the EU may contain a maximum of 5 stock units and the individual data for each stock unit will be reported (weight, customs tariff number, manufacturing country). In order for orders to be found for the creation of export documents, they have to be successfully registered with DHL Intraship/Versenden via the **Orders » Shipping centre** menu.
...
[.subhead]
Carrying out settings in the DHL business portal
Carry out the settings in the link:https://www.dhl-geschaeftskundenportal.de/webcenter/portal/gkpExternal?_afrLoop=2465780077903302#!%40%40%3F_afrLoop%3D2465780077903302%26_adf.ctrl-state%3D11moofew5r_4[**DHL business portal**, window="_blank"].

[.subhead]
Creating a property
Go to **Settings » Item » Properties** and create a property of the type **Selection**. Save values for the different age restrictions. For example, create the values **A16** and **A18**. Enter a name for the property, e.g. visual age check.

Further information can be found in the chapter **<<item/managing-items#500, Creating properties >>** within the **<<item#, Items  >>** topic.

[.subhead]
Carrying out the basic setting
Go to **Settings » Item » Basic settings** and select the property that you just created for the option **Item property for visual age check**. **Save** the settings.
...
Proceed as follows to set up the interface to link:https://www.dhl.de/en/paket/geschaeftskunden/ab-200-pakete/nationaler-versand/retoure.html[**DHL RETOURE with enclosed return labels**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » DHL Retoure enclosed return labels**.
...
Set up an **<<orders/settings/event-procedures#, event procedure  >>** to automatically generate return labels.
...
Proceed as follows to set up the interface to link:http://www.dhl.de/en/paket/geschaeftskunden/ab-200-pakete/nationaler-versand/retoure.html[**DHL Retoure Online**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » DHL Retoure Online**.
...
If you allow customers to create returns themselves, then they will be able to choose the <<orders/settings/sales-order-types/return#, **reason for return**  >> in their **My account** area. +
...
Proceed as follows to set up the interface to link:http://www.dhl.de/en/logistik/supply-chain-loesungen.html[**DHL Supply Chain**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » DHL Supply Chain**.
...
Proceed as follows to set up the interface to link:http://www.dhl.co.uk/en.html[**DHL UK**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » DHL UK**.
...
Proceed as follows to set up the interface to link:https://esolutions.dpd.com/entwickler/dpd-cloud-webservice.aspx[**DPD Cloud Webservice**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » DPD Cloud Webservice**.
...
Pay attention to the information about the shipments that you will later register for DPD Cloud Webservice via the <<fulfilment/shipping-centre#700, **shipping centre** >>. Go to **Data exchange » API log » Tab: Shipping centre API log** to view this information. For further information about API log entries for DPD Cloud Webservice, refer to the <<data-exchange/api-log#, **API log**  >> page of the manual.

[#3600]
=== DPD DELISprint

Before setting up the interface to DPD DELISprint in plentymarkets, register with link:https://www.dpd.com/portal_de[**DPD**, window="_blank"]. DPD customers can use the DPD DELISprint program to easily create DPD parcel labels.

How you set up shipping service providers and shipping profiles and which additional settings you have to carry out and may carry out, is described in the chapters from **<<fulfilment/preparing-the-shipment#100, Setting up countries of delivery >>** through **<<fulfilment/preparing-the-shipment#2400, Retrieving the API log >>**. The settings described there are the basic requirements in order to carry out the settings that are specific for each shipping service provider. Therefore, first carry out the settings in the chapters mentioned above. Afterwards, continue with this chapter.

Proceed as follows to set up the interface to link:https://www.dpd.com/ch/home/esolutions/clientloesungen/delisprint_6[**DPD DELISprint**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » DPD DELISprint**.
...
|**Flex/Predict active**
|Activate to register the notification service link:https://www.dpd.com/de/home/produkte_services/zusatzleistungen/national/predict[**Predict**, window="_blank"] (formerly FlexDelivery) with DPD. This service is subject to additional charges. If this option is chosen, the CSV file exported from plentymarkets will contain the additional information **PRO**.
|====
...
Proceed as follows to set up the interface to link:https://www.post.at/en/business_parcels_easypak_shipping_software.php[**EasyPAK**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » EasyPAK**.
...
Proceed as follows to set up the interface to link:https://gls-group.eu/DE/de/home[**GLS Germany**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » GLS Germany**.
...
// div class="plentyInfoBox plentyIsMessage"> <div class="plentyInfoBoxHeader">Semikolon als Trennzeichen für CSV-Export</div> <div class="plentyInfoBoxContent"> <p>Für einen korrekten Datenexport zu GLS Germany im Menü <strong>Aufträge » Fulfillment » Lieferanschriftenliste</strong> stellen Sie als Trennzeichen beim CSV-Export das Semikolon ein.</p> </div> </div
[#3900]
=== Hermes (two-person handling)

How you set up shipping service providers and shipping profiles and which additional settings you have to carry out and may carry out, is described in the chapters from **<<fulfilment/preparing-the-shipment#100, Setting up countries of delivery >>** through **<<fulfilment/preparing-the-shipment#2400, Retrieving the API log >>**. The settings described there are the basic requirements in order to carry out the settings that are specific for each shipping service provider. Therefore, first carry out the settings in the chapters mentioned above. Afterwards, continue with this chapter.

While placing their orders, customers should be able to request that their items are assembled for them. You can offer customers such a service with link:https://www.hermesworld.com/de/unsere-dienstleistungen/distribution/2-mann-handling/2-mann-handling/[**Hermes (two-person handling)**, window="_blank"]. In order to use this service, create an order property. Create an order property with the options **Yes** and **No**. Depending on which **property type** you select, you will be able to configure the options that your customers will later select in your online store.

[TIP]
.Interface for existing customers only
====
The interface to Hermes 2-Mann-Handling (two-man handling) can only be used by existing customers. New customers are asked to contact link:https://www.hermesworld.com/en/about-us/hermes-group/hermes-einrichtungs-service/[**HERMES Einrichtungs Service GmbH &amp; Co.KG**, window="_blank"].
====

[.instruction]
Creating the assembly service as an order property:

. Go to **Settings » Item » Properties » Tab: New property**.
...
. Go to **Settings » Orders » Shipping » Shipping service provider » Hermes (two-person handling)**.
...
Proceed as follows to set up the interface to link:https://profipaketservice.hlg.de/wps/portal/PROPS_DEU/Home/Start/[**Hermes (ProfiPaketService)**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » Hermes (ProfiPaketService (professional parcel service))**.
...
Proceed as follows to set up the interface to link:http://www.dpd.iloxx.de/home.aspx[**MyDPD Business / iloxx**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » MyDPD Business / iloxx**.
...
Proceed as follows to set up link:http://www.royalmail.com/[**Royal Mail**, window="_blank"] and link:https://www.ukmail.com/[**UK Mail**, window="_blank"] via the integrated link:http://www.netdespatch.com/[**NetDespatch**, window="_blank"] interface in your plentymarkets system.

[#4210]
==== Royal Mail

Proceed as follows to set up the interface to set up link:http://www.royalmail.com/[**Royal Mail**, window="_blank"] via the integrated link:http://www.netdespatch.com/[**NetDespatch**, window="_blank"] interface in your plentymarkets system.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » NetDespatch**.
...
Proceed as follows to set up the interface to set up link:https://www.ukmail.com/[**UK Mail**, window="_blank"] via the integrated link:http://www.netdespatch.com/[**NetDespatch**, window="_blank"] interface in your plentymarkets system.
...
Before setting up the interface to link:https://www.post.ch/de/geschaeftlich/themen-a-z/swiss-post-gls[**Swiss Post**, window="_blank"] in plentymarkets, register with Swiss Post.
...
. Go to **Settings » Orders » Shipping » Shipping service provider » Swiss Post**.
...
**__Important__**: You will not be able to select a service group until you have set up the technical user.
...
Proceed as follows to set up the interface to link:https://www.ups.com/one-to-one/register?sysid=myups&loc=de_DE&WT.svl=Footer[**UPS**, window="_blank"] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to **Settings » Orders » Shipping » Shipping service provider » UPS**.
...
**__Important__**: The setting **Register all together** is mandatory for cash on delivery orders with multiple packages. +
...
. Click on the **Table of shipping charges** tab.
. Expand the region.
. Carry out the settings in the **UPS Service** area. Pay attention to the information given in table 39.
. **Save** the settings.
...
For further information about configuring Postfinder, refer to the <<fulfilment/preparing-the-shipment#4600, **Setting up the DHL Postfinder** >> chapter.
...
**__Important__**: The default entry 0.00 stands for no limitation.
...
This feature shows your customer a list of post offices and Packstations nearby. The list is displayed when your customer enters his or her postcode in the My account area or during the <<cms/web-design/editing-the-web-design/order-process/checkout#, **checkout**  >> process. You will then be able to use the address of a post office or Packstation directly in the delivery address.

In order to use DHL Postfinder, you must carry out settings in multiple menus. These settings are summarised on the <<cms/web-design/editing-the-web-design/order-process/dhl-postfinder#, **DHL Postfinder**  >> page of the manual.
...
Table 41 lists all of the fulfilment service providers available in plentymarkets. Click on the service provider in the table to jump to the sub-chapter where the specific settings of the fulfilment service provider are described.
...
|**<<fulfilment/preparing-the-shipment#5600, Fulfilment by Amazon >>**
|For sellers within Germany. Shipping: international.
|====
...
This chapter describes how to set up link:http://www.dhl.de/fulfillment[**DHL Fulfillment**, window="_blank"] in plentymarkets. Set up special warehouses, order statuses and reasons for return before you set up the interface. To do so, proceed as follows.

[IMPORTANT]
.Klarna orders will not be transferred to DHL Fulfillment
====
Currently, it is not possible to transfer orders that are paid with <<payment/klarna#, **Klarna**  >> to DHL Fulfillment. Only orders with a valid invoice can be transferred to DHL Fulfillment. However, Klarna only transmits the invoice to plentymarkets after outgoing items have been booked.
====
...
[.subhead]
Creating warehouses for DHL Fulfillment[#4900]
...
[.subhead]
Setting up order statuses[#5000]

Go to **Settings » Orders » Order status** and set up two <<orders/settings/order-status#, **order statuses**  >> for the orders transferred to DHL Fulfillment. Below are two examples:
...
The **Data exchange » API log** menu contains information about shipments that were registered with DHL Fulfillment. For further information about API log entries for DHL Fulfillment, refer to the <<data-exchange/api-log#, **API log**  >> page of the manual.
====
...
[.subhead]
Creating reasons for returns[#5100]

If you want returns from DHL Fulfillment to be imported into your plentymarkets system, then you need to create reasons for the returns. Go to **<<orders/settings/sales-order-types/return#, Settings » Orders » Order types » Return  >>** to create reasons for returns. Then link the reasons that you created with the reasons given by DHL Fulfillment in the **Settings » Orders » Shipping » Fulfilment service provider » DHL Fulfillment** menu.

[.subhead]
Setting up the interface[#5200]
...
. Go to **Settings » Orders » Shipping » Fulfilment » DHL Fulfillment**.
. Carry out the settings as desired. Pay attention to the information given in table 42.
. **Save** the settings.
...
__**Important**__: The order status that has been selected here has an impact on the registration of new orders to DHL Fulfillment. This means that new orders with this status will not be taken into consideration upon registration.
...
|Select the properties that you created for transferring these item characteristics to DHL Fulfillment. For further information, refer to the **<<fulfilment/preparing-the-shipment#5300, Transferring properties to DHL Fulfillment >>** chapter.

|**Reasons for return**
|Link the reasons for return that you created under **<<orders/settings/sales-order-types/return#, Settings » Orders » Order types » Return  >>** with the reasons given by DHL Fulfillment.
|====

[.subhead]
Transferring properties to DHL Fulfillment[#5300]
...
Go to **Settings » Item » Properties**. Create the properties **Expiration date**, **Original packaging** and **Fragile** in your system. Assign them to the property type **No**. Create the property **Serial number** and assign it to the property type **Selection**. Give it the following three values: **SN** (Used for scanning an item's serial numbers), **SNK** (Used for scanning ranges of serial numbers per item) and **SNK2** (Used for scanning ranges of serial numbers per order item).

After you have created the properties, link these properties to items within the item's **Properties** tab. Finally, go to **Settings » Orders » Shipping » Fulfilment service provider » DHL Fulfillment** and use the drop-down menus to select the properties that you created. Further information can be found in the **<<item/managing-items#500, Creating properties >>** chapter within the **<<item#, Items  >>** topic.
...
Before setting up the interface to IDS Logistik in plentymarkets, register with link:http://www.ids-logistik.de[**IDS Logistik**, window="_blank"].

Then create the fulfilment service provider in the **Settings » Orders » Shipping » Shipping options » Tab: Shipping service provider** menu. Select **Not defined** from the **Shipping service provider** drop-down menu. Further information can be found in the chapter **<<fulfilment/preparing-the-shipment#800, Creating a shipping service provider >>**.

Afterwards, you create the shipping profile in the **Settings » Orders » Shipping » Shipping options** menu. Carry out the settings as described in the chapter **<<fulfilment/preparing-the-shipment#1000, Creating a shipping profile >>**.
...
. Go to **Settings » Orders » Shipping » Fulfilment service provider» IDS Logistik**.
...
- Delivery note number (6 digits) +
- Number of colli (2 digits) +
- Check digit (1 digit) +
**Example**: 800 6913 000001 00 0 +
16 is entered by default. This number of digits is adequate for a 6-digit delivery note number. +
Enter a higher value if the number of digits is not sufficient. Enter e.g. 17 if you need to use a 7-digit delivery note number.
|====

[#5500]
=== iLOPACK

Before setting up the interface to iLOPACK in plentymarkets, register with link:http://www.ilopack.de/[**iLOPACK**, window="_blank"].
...
. Go to **Settings » Orders » Shipping » Fulfilment service provider» iLopack**.
...
The abbreviation FBA stands for Fulfilment by Amazon, a service from Amazon where Amazon takes care of the entire stock management and logistics. Refer to the chapter <<multi-channel/amazon#3300, **Using the FBA service by Amazon** >> on the <<multi-channel/amazon#, **Amazon**  >> page of the manual. This chapter describes which settings you need to carry out.

== Amazon Prime

Use filters to make sure that orders with shipping conditions for Amazon Prime will be separately treated in plentymarkets. Furthermore, you can create shipping labels for Amazon Prime orders via the Amazon Merchant Fulfilment API. Refer to the chapter <<multi-channel/amazon#4200, **Editing Amazon Prime orders** >> on the <<multi-channel/amazon#, **Amazon**  >> page of the manual. This chapter describes which settings you need to carry out.
