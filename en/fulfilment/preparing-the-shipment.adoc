...
:keywords: shipping service provider, fulfilment service provider, fulfillment, fulfilment, shipping countries, shipping country, country of delivery, countries of delivery, region, regions, shipping profile, shipping profiles, table of shipping charges, shipping packages, shipping package, shipping costs, DHL packstation, DHL postfinder, tracking URL, c.b.c. logistics, cbc logistics, PARCEL.ONE, parcel one, DHL Easylog, DHL Freight, DHL Intraship, DHL Versenden, DHL Intraship, Intraship/Versenden, DHL plentymarkets Import, DHL enclosed return label, DHL Retoure Online, DHL Supply Chain, DHL UK, DPD Cloud Webservice (My DPDBusiness), DPD Print, EasyPak, GLS Germany, Hermes 2-Mann-Handling, Hermes ProfiPaketService, Iloxx, UK Mail, NetDespatch, Swiss Post, UPS, DHL Fulfillment, IDS Logistik, iLOPACK, DHL export document, export documents, DHL customs declaration, CN23 document, CN23, CN 23, CP71 document, CP71, CP 71
...
|Old integration <<fulfilment/preparing-the-shipment#2900, DHL Intraship/Versenden>>
|For sellers within Germany. Shipping: international.
...
|Plugin <<fulfilment/shipping-service-provider-plugins/plugin-dhl-wunschpaket#, DHL Wunschpaket>>
...
We recommend setting up the *DHL Shipping (Versenden)* plugin. You can easily set up the interface with the help of assistants.

In order to get further information about how to set up the DHL Shipping (Versenden) plugin, click on *Plugin DHL Shipping (Versenden)* (default selection). To see further details about the old interface DHL Intraship/Versenden, click on *Old integration DHL Intraship/Versenden*.
...
+
--
Via the interface to the DHL business customer portal, you can use the function *Ship*. For further information, refer to the <<fulfilment/shipping-service-provider-plugins/plugin-dhl-shipping-versenden#, DHL Shipping (Versenden)>> page. The plugin *DHL Shipping (Versenden)* is available in the link:https://marketplace.plentymarkets.com/en/plugins/integration/DHLShipping_4871[plentyMarketplace^]{nbsp}icon:external-link[].
...
Old integration DHL Intraship/Versenden::
+
--
...
Further down in this chapter, you find information about the DHL export documents.
...
| *Billing numbers*
|Enter the billing numbers in accordance with the DHL contract. It is possible to maintain individual shipper data for each billing number. The 14-digit DHL billing number is composed of the following: +
...
[discrete]
=== Setting up the DHL Packstation
...
The DHL Postfinder shows your customer a list of post offices and Packstations nearby. The list is displayed when your customers enter their postcode in the My account area or during the <<online-store/online-store/cms#web-design-editing-the-web-design-order-process-dhl-postfinder#, order process>>. Your customers are able to use the address of a post office or Packstation directly in the delivery address.

In order to use DHL Postfinder, you have to carry out settings in multiple menus. These settings are summarised on the following <<omni-channel/online-store/cms#web-design-editing-the-web-design-order-process-checkout#, page of the manual>> in the “DHL Postfinder” section.
...
+
--
The plugin *DHL Retoure Online* is available free of charge in the link:https://marketplace.plentymarkets.com/en/plugins/integration/dhlretoureonline_6714[plentyMarketplace^]{nbsp}icon:external-link[]. Install the plugin in a plugin set. The *DHL Retoure Online* assistant is visible in the *Setup » Assistants* menu in the *Integrations* area. For further information, refer to the <<fulfilment/shipping-service-provider-plugins/plugin-dhl-retoure-online#, DHL Retoure Online>> page.
