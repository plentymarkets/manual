...
. On the link:https://marketplace.plentymarkets.com/en/plugins/channels/marktplaetze/ebayoauth2_4787[eBay OAuth2 plugin^] page, click on *Go to checkout*.
. Buy the plugin. +
...
After you have been activated by eBay, go to the plentyMarketplace and purchase the link:https://marketplace.plentymarkets.com/plugins/en/integration/ebayfiege_6018[eBay Fulfillment plugin^] free of charge and install it. Proceed in the same way as described in the instructions in chapter <<fulfilment/ebay-fulfillment#20, Downloading and installing the eBay OAuth2 plugin>>.
...
If the inbound or outbound order could not be sent, go to *System » System settings » Orders » Shipping » Fulfillment » eBay Fulfillment » Settings*. For *Inbound order not sent* and *Outbound order not sent* set the statuses so that the orders can be found and checked accordingly.
...
[#130]
==== Order status for inbound (used for event procedures)

[.instruction]
Creating the order status for inbound:
...
. In the field *Name for new status*, enter the name, e.g. *eBay cancellation inbound sent*.
. *Save* (icon:save[role="green"]) the settings. +
→ The new order status *[8.2] eBay cancellation inbound sent* is created.
...
. From the *Status* drop-down list, select the newly created status for Inbound.
. *Save* (icon:save[role="green"]) the settings.
...
. From the Event drop-down list, select the option *Order change > Status change*.
. From the *Status* drop-down list, select the newly created status for outbound.
. *Save* (icon:save[role="green"]) the settings.
...
With your plentymarkets system, it is possible to send a shipping confirmation via email to your customers. It is required that you have set up the corresponding email template in your system.
...
How to proceed is described in the following chapter <<fulfilment/ebay-fulfillment#280, Saving the email login details>>.
====
...
.Carrying out settings in the eBay Fulfillment plugin (example)
...
If you have already set up item packages in your plentymarkets system, there are still two setting options for the items that you must make so that they can be processed by eBay Fulfillment.
...
Add the items to the order that you want to send to your fulfilment service provider and set the <<fulfilment/ebay-fulfillment#560, item price to 0 EUR>>. *Save* the order.
...
====
Do not create the delivery note at the order, but as described in chapter <<fulfilment/ebay-fulfillment#400, Inbound packages>>. Otherwise, the fulfilment service provider cannot stock the items.
====
...
*_Note:_* If the order could not be sent, check the settings or have a look in the chapter <<fulfilment/ebay-fulfillment#450, FAQ/Troubleshooting>>.
...
Go to *Items » Edit item* and search (icon:search[]) for the item. Open the item and check if the options *eBay Fulfillment* and *WEB API* are activated under *Availability*. The EBFSKUs will be automatically saved in the item after approx. 1 hour.
...
A correct selection of the stock dependency in the eBay listing is also required for an ideal warehouse selection. Further information can be found in the <<quick-start/quick-start-ebay#2800, plentymarkets manual>>.
...
If it is not possible to change the status, the outgoing items might have been already booked (status ≥ 7). Or the required user rights are not activated (see chapter <<fulfilment/ebay-fulfillment#545, Order is stuck in status "eBay Fulfillment Inbound">>).
...
You have sent an <<fulfilment/ebay-fulfillment#365, advice note (goods delivery announcement)>> with your plentymarkets system to eBay Fulfillment. In this notification, you transfer a delivery of 15 pieces of a certain item. Actually, you only send only 13 pieces, however.
...
See chapter <<fulfilment/ebay-fulfillment#480, Check the token>>.

[#650]
==== User rights not activated

See chapter <<fulfllment/ebay-fulfillment#545, Order is stuck in status "eBay Fulfillment Inbound">>.
