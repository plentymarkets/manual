...
:keywords: eBay Fulfillment, Inbound, Outbound, Fiege, Ebay Fulfillment integration guide, ebay fulfilment, EBFSKU, ebay fulfillment sku, ebay sku, automatically assign ebay fulfillment warehouse, eBay Fulfillment warehouse, error inbound,error Outbound, ebay customer support, Avise, Avis, Inbound not sent, eBay Fulfillment shipping release, eBay Fulfillment warehouse
...
. Select the option (see <<image-activate-order-referrern>>).
. *Save* (icon:save[role="green"]) the settings.
...
→ *_Tip:_* If you have already set up a shipping profile with this shipping service provider, use an other name, e.g._ eBay Fulfillment_ plus *name of the shipping service provider* (see <<image-set-up-shipping-service-provider>>.
...
[[image-set-up-shipping-service-provider]]
.Setting up the shipping service provider
...
. Carry out the settings. Note <<table-ebay-fulfillment-email-server-login-detailn>>.
. *Save* (icon:save[role="green"]) the settings.
