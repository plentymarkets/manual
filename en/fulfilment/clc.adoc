...
====
You need a plugin to link plentymarkets to CLC. Visit the link:https://marketplace.plentymarkets.com/en/clc_7015[plentyMarketplace^]{nbsp}icon:external-link[] to download the CLC plugin.
====
...
If you do not carry out any settings in this menu, your plentymarkets system does *not* transfer any additional documents to CLC.
...
. Enter the IDs of the order referrers for the invoice export, delivery note export and shipping label export.
. *Save* (icon:save[role="green"]) the settings.
...
For further information, refer to the <<welcome/general-functions/properties#, Properties>> page of the manual.
