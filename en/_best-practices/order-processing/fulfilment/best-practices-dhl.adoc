...
This error message would be returned trying to register an cash on delivery shipment if the bank details set in the DHL Shipping Plugin is not the default bank account in the DHL Business Customer Portal.
The registration might be successful after changing the default bank account in the DHL Business Customer Portal to the bank account also used for the DHL Shipping Plugin.
...
If you use the DHL plugin, the error message could also be displayed because of the deployed plugin version: It is not possible to register export deliveries with the DHL plugin version older than 1.3.0. Therefore, you must use plugin version 1.3.0 at least to register export deliveries. In order to check which version you are using check the version number displayed in the *Plugins » Plugin overview* in the *version productive* row.

.plugin overview plugin version
...
After you have downloaded an update of the plugin, you have to deploy the plugin in Productive once again by clicking on *Save & publish plugins*. Downloading the update alone does not automatically activate new features and changes in the new version.

.Plugin overview save and publish Plugins
image::_best-practices/order-processing/fulfilment/assets/DHL_plugin_overview_version.png[]
