...
You need to register with PAYONE before setting it up in plentymarkets. Register with PAYONE and set up an account there. Fill out the link:https://www.payone.de/en/contact/request-a-quote/[**form**, window="_blank"] on the PAYONE website. You will receive the access data that you need for the configuration in plentymarkets. Then, save the transaction status URL according to the following pattern in the PAYONE back end:
...
|Activate if you want to use PAYONE for link:https://docs.google.com/document/d/10mPeV3xqx4We71dYQdPmJK2qvb21Rpym6FG_tKwHKfc/edit#heading=h.h38ew7hip4tl[**Shopware (German link)**, window="_blank"]. For further information, refer to **<<payment/payone#140, Using PAYONE for Shopware >>**.
|====
...
For PAYONE, a large variety of payment methods is available. Go to **<<payment/managing-payment-methods#20, Settings » Orders » Payment » Payment methods >>** to activate the payment methods.

[#115]
== Displaying PAYONE in the online store

In the **CMS** menu, you can customise the text that is displayed during the order process when customers select the payment method PAYONE. In addition, you will find out where to customise the design by modifying the CSS. You can customise the forms for credit card data and online bank transfers. For further information, refer to the <<cms/web-design/editing-the-web-design/order-process/checkout#, **Checkout**  >> page.
...
You can also customise the order confirmation form. For further information, refer to the **<<cms/web-design/editing-the-web-design/order-process/order-confirmation#, Order confirmation  >>** page.
...
Set up an <<orders/settings/event-procedures#, **event procedure**  >> to automatically send shipping confirmations to PAYONE when the outgoing items are booked.
...
To send refunds automatically, an **<<orders/settings/order-status#, order status  >>** for credit notes is required. You can use one of the standard statuses or set up your own order status. This status is then only assigned to completed credit notes. These credit notes must not be changed after creation.
...
Set up an <<orders/settings/event-procedures#, **event procedure**  >> to automatically send refunds to PAYONE.
