...
**link:http://www.upgplc.com/[UPG, window="_blank"]** is a fully integrated payment solution for e-commerce offered by Creditreform.
...
Register with UPG before you carry out the relevant settings in plentymarkets. To do so, link:http://www.upgplc.com/services/online-payments/[**contact**, window="_blank"] UPG.

[#30]
== Setting up UPG in plentymarkets

Proceed as described below to set  up the interface to UPG.
...
Set up and activate the UPG **<<payment/managing-payment-methods#20., payment method >>** in the **Settings** tab of the payment method. In addition, check the settings for **Permitted payment methods** in the **<<payment/managing-payment-methods#30., customer classes >>** and for **Blocked payment methods** in the **<<fulfilment/preparing-the-shipment#1000., shipping profiles >>**. You need to select the payment methods that you want to offer in your UPG account.
...
[#60]
== Displaying UPG in the online store
...
. Change the values of both variables from **false** to **true**. Pay attention to the information given in table 2.
. **Save** the settings.
...
You need to confirm UPG that you have sent the items to your customer before you will receive the customers payment from UPG. Set up an <<orders/settings/event-procedures#, **event procedure**  >> to automatically send shipping confirmations when the outgoing items are booked.
...
. Select the option **create** from the **Return...** drop-down menu.
...
Set up an <<orders/settings/event-procedures#, **event procedure**  >> to automatically inform UPG of cancellations.
