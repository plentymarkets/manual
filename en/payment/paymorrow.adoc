...
Set up event procedures to inform Paymorrow of outgoing items, credit notes and cancellations. For this to work correctly, the documents need to be created in the order's **Receipts** tab. Set up an <<orders/settings/event-procedures#, **event procedure**  >> to automatically send invoices to Paymorrow and to report outgoing items to Paymorrow.
...
If a customer sends back some or all of the items, then you must create a credit note and report this to Paymorrow in order to keep the customer from receiving a dunning letter from Paymorrow. Set up an <<orders/settings/event-procedures#, **event procedure**  >> to automatically send credit notes to Paymorrow.
...
If an order was cancelled, then the status has to match the settings under **Settings » Orders » Basic settings** so that the cancelled operation will also be communicated to Paymorrow. Set up an <<orders/settings/event-procedures#, **event procedure**  >> to automatically send cancellations to Paymorrow.
