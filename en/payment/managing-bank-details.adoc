...
:id: 69AQ724
:keywords: Bank, Bank details, EBICS, SEPA, SEPA direct debit mandate, account, account number, account no., sort code, account holder, SWIFT, cash in advance, invoice, payment method, payment, country-specific, country of delivery, ID, bank code, debit, bank data, IBAN, BIC, mandate, payment frequency, total, bank statement, transaction, bookkeeping, participants, participant ID, order type, STA, VMK, EU Type, EU, EU type T, transaction report, payment retrieval, INI, initilisation letter, HIA, CAMT, commercial transactions, bank transfer, direct debit, activation, interim transaction, document, document template, pdf
...
You can use an IF query to only show the bank details for the country of delivery. In other words, a buyer in Germany would only see the German account details and a customer in Switzerland would only see the Swiss account details. You need the template variable *$CustomerShippingCountry* for this. It only displays the corresponding content if the particular country of delivery was selected. You can find the country IDs under *Setup » Orders » Shipping » Settings* in the *Countries of delivery* tab. Insert the following code into the <<payment/managing-bank-details#30, Bank details>> category. Add further countries of delivery with the respective ID, if needed.
...
. Go to *Setup » Settings » Data » Migration*.
...
====
In plentymarkets, EBICS is integrated via the link:https://marketplace.plentymarkets.com/plugins/payment/Ebics_5098[plugin in the plentyMarketplace^]{nbsp}icon:external-link[].
====
