...
Once the PayPal plugin is installed, payments are automatically imported every hour from the linked PayPal account into the plentymarkets system. After the import, PayPal payments are automatically <<payment/beta-managing-payments#10, assigned>> to the corresponding orders if the criteria for assignment are met. Due to technical restrictions of PayPal, there may be a delay of up to three hours during the import.
