...
====
The containers needed by default are automatically linked during the set-up with the PayPal assistant. From version 5.4.0 on, required as well as optional containers are linked in the same plugin set in which the assistant is completed.
====
