...
With the plugin Online bank transfer, you integrate the Klarna payment method Online bank transfer (Klarna Sofort) in your online shop and make them available to your customers. To do so, download the Online bank transfer plugin from the link:https://marketplace.plentymarkets.com/en/sofort_5050[plentyMarketplace^]{nbsp}icon:external-link[]. You also have to carry out a few basic settings in your plentymarkets system so that the payment methods are activated in your system. +
...
|*No* is selected as default. To activate the payment method, select *Yes*.
