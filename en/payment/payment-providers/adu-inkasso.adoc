...
:position: 30
:url: payment/payment-providers/adu-inkasso
:id: OVNEYEC
...
Set up link:http://www.adu-inkasso.de/en/home/[ADU-Inkasso^]{nbsp}icon:external-link[] (Allgemeiner Debitoren- und Inkassodienst GmbH) in plentymarkets to manage outstanding debts by transferring the data automatically to ADU-Inkasso with the help of a specific sales order status. The current processing status of outstanding debts is visible in the back end. Learn how to set up the interface to ADU-Inkasso in your plentymarkets system.

[#10]
== Registering with ADU-Inkasso

Register with link:https://www.auskunft.inkassounternehmen.de/?action=register[ADU-Inkasso^]{nbsp}icon:external-link[] (German only) in order to use its services. You will receive your access data to the ADU-Inkasso online portal once you have registered and signed a contract.

[#20]
== Carrying out settings in the ADU-Inkasso account

Log into the link:https://www.auskunft.inkassounternehmen.de[ADU-Inkasso online portal^]{nbsp}icon:external-link[] with your access data and open the menu *Aktenimport* (file import) in order to activate the interface. Carry out the following settings in this menu.

* Enter access data
* Enter PlentyID and SOAP-WSDL. This information can be found in the *Setup » Settings » API » Data* menu.
* Save login data

Once you have activated the interface, you can select an <<orders/managing-orders#1200, order status>> to display the corresponding debts in the online portal. Furthermore, you can specify whether debts should be transferred to ADU-Inkasso automatically or manually. For further information, refer to the link:http://www.adu-inkasso.de/fileadmin/user_upload/Beschreibung_plentymarkets.pdf[Interface description^]{nbsp}icon:external-link[] page (German only) of ADU-Inkasso.
