...
:position: 240
:url: payment/payment-providers/sofort-ueberweisung
:id: MNRND21
...
====
This page only contains instructions for integrating *SOFORT Überweisung* in Callisto online stores. For further information on using *Sofort Überweisung* in Ceres online stores, refer to the detailed information in the plugin description available in the link:https://marketplace.plentymarkets.com/en/plugins/payment/SOFORT_5050[plentyMarketplace^]{nbsp}icon:external-link[].
====

link:https://www.sofort.com/eng-DE/kaeufer/su/online-zahlen-mit-sofort-ueberweisung/[*SOFORT Überweisung^]{nbsp}icon:external-link[] is the direct payment method of SOFORT GmbH. When using this payment method, the customer initiates the transfer directly from their online bank account when placing their order. A successful transfer is instantly confirmed to you as the seller. This allows you to instantly deliver stocked items and digital services - and allows the customer to receive their order even faster. Directly after the payment has been received, the order will be marked as paid in plentymarkets.
...
In the status history of orders that have been paid with the payment method SOFORT Überweisung, a payment confirmation is saved and the orders are cleared for shipping.  The bank transfer has been executed by the customer, but the payment has not been transferred to your bank account yet. The payment confirmation entry is also saved in the payment history. If you already set up <<payment/managing-bank-details#70, EBICS>> in your system, the order will be marked as paid automatically after payment import. A second entry will be created in the payment history section of the order. If you do not use EBICS, you have to <<payment/managing-payments#40, book the payment manually>>.

[#20]
== Registering with SOFORT Überweisung

You need to register with link:https://www.sofortueberweisung.de/payment/createNew/go[SOFORT Überweisung^]{nbsp}icon:external-link[] before you can set up the payment method in plentymarkets. Once you have registered, you will receive the access data that you need to save in plentymarkets.
...
Set up and activate the <<payment/managing-payment-methods#20, payment method>> Sofortüberweisung.de in your plentymarkets system. As soon as these settings have been made, the customer will have the option to select SOFORT Überweisung when choosing *bank transfer/cash in advance* as their payment method. In this way, your customer will have the choice of either paying with a normal bank transfer or using SOFORT Überweisung. In addition, check the settings for *Permitted payment methods* in the <<payment/managing-payment-methods#30, customer classes>> and for *Blocked payment methods* in the <<fulfilment/preparing-the-shipment#1000, shipping profiles>>.
