= Old online store: Klarna Checkout UK
:lang: en
...
:position: 130
:url: payment/payment-providers/klarna-checkout-uk
:id: QLOVOII
...
An iframe is used to integrate Klarna Checkout UK directly in plentymarkets. Thus, an <<basics/admin-tasks/plentymarkets-account#ssl-certificate, SSL certificate>> is required for using this payment method.
...
* MasterCard
* Pay after delivery
* Further <<payment/payment-providers/klarna-checkout-uk#80, external payment methods>>

== Registering with Klarna

[#50]
Before you set up Klarna Checkout UK in your plentymarkets system, you have to register with link:https://www.klarna.com/uk/business/sell-online-with-klarna[Klarna^]{nbsp}icon:external-link[].

== Setting up Klarna Checkout UK for Callisto

[#60]
Proceed as described below to set up the interface to Klarna Checkout UK for your Callisto online store in plentymarkets.
...
[#90]
== Displaying Klarna Checkout UK in the Callisto online store
...
Set up the following event procedures in order to inform Klarna UK when items are shipped, orders are cancelled and credit notes or refunds are paid. Set up an <<automation/event-procedures#, event procedure>> to automatically send shipping confirmations to Klarna. Payment providers usually wait for the shipping confirmation from the merchant before they disburse and transfer the amount paid by the customer to the merchant’s account.
...
[#110]
To refund customers the total purchase amount, create a *Return order*. Set up an <<automation/event-procedures#, event procedure>> to automatically send refunds to Klarna UK. For further information about creating returns, refer to the <<orders/managing-orders#400, Return>> page of the manual.
...
Set up an <<automation/event-procedures#, event procedure>> to automatically send cancellations to Klarna UK.
