...
link:http://www.sagepay.co.uk/[Sage Pay^]{nbsp}icon:external-link[] is a payment provider that was founded in Great Britain and is available in the United Kingdom and in Ireland. Sage Pay offers various payment options, an integrated fraud prevention tool and e.g. transaction reports.
...
If you use the first payment method, then the amount will be transferred to you directly. If you use the payment methods *deferred* and *authenticate*, then the amount will be transferred to you after you have requested payment. For further information about these payment methods, refer to link:http://www.sagepay.co.uk/support/15/36/transaction-types[Sage Pay^]{nbsp}icon:external-link[].

[#40]
== Registering with Sage Pay

You need to register with Sage Pay before setting it up in plentymarkets. link:http://www.sagepay.co.uk/contact[Contact^]{nbsp}icon:external-link[] Sage Pay in order to register. Once you have registered you will receive access data from Sage Pay. You need this access data for the setup in plentymarkets. After registering, access your Sage Pay account and enter the IP address of the server that your store runs on. Save this entry in the Sage Pay back end under *Settings » Valid IPs*.
