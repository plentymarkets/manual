...
:position: 180
:url: payment/payment-providers/payone
:id: JZAWRPZ
...
You need to register with PAYONE before setting it up in plentymarkets. Register with PAYONE and set up an account there. Fill out the link:https://www.payone.de/en/contact/request-a-quote/[form^]{nbsp}icon:external-link[] on the PAYONE website. You will receive the access data that you need for the configuration in plentymarkets. Then, save the transaction status URL according to the following pattern in the PAYONE back end:
...
|Activate if you want to use PAYONE for link:https://docs.google.com/document/d/10mPeV3xqx4We71dYQdPmJK2qvb21Rpym6FG_tKwHKfc/edit#heading=h.h38ew7hip4tl[Shopware^]{nbsp}icon:external-link[] (German link). For further information, refer to <<payment/payment-providers/payone#140, Using PAYONE for Shopware>>.
|====
...
For PAYONE, a large variety of payment methods is available. Go to <<payment/managing-payment-methods#20, Setup » Orders » Payment » Payment methods>> to activate the payment methods.
...
Set up an <<automation/event-procedures#, event procedure>> to automatically send shipping confirmations to PAYONE when the outgoing items are booked.
...
Set up an <<automation/event-procedures#, event procedure>> to automatically send refunds to PAYONE.
...
The interaction with link:http://github.com/plentymarkets/plentymarkets-shopware-connector[Shopware]{nbsp}icon:external-link[] works as follows:
