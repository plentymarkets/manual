...
====
In order to display the bank details of Paymorrow correctly on your invoice, save a width of 120 millimetres for the payment instruction in the *System » System settings » Client » Select client » Locations » Select location » Documents » Invoice* menu.
====
...
If an order was cancelled, then the status has to match the settings under *System » System settings » Orders » Settings* so that the cancelled operation will also be communicated to Paymorrow. Set up an <<automation/event-procedures#, event procedure>> to automatically send cancellations to Paymorrow.
