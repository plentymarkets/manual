...
If an order was cancelled, then the status has to match the settings under *System » Orders » Settings* so that the cancelled operation will also be communicated to Paymorrow. Set up an <<automation/event-procedures#, event procedure>> to automatically send cancellations to Paymorrow.
