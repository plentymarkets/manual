...
:position: 120
:url: payment/payment-providers/klarna-checkout
:id: XAAEUNW
...
link:https://klarna.com/de/mit-klarna-verkaufen/klarna-checkout[Klarna Checkout^]{nbsp}icon:external-link[] is an integrated checkout solution that allows you to offer your customers several payment methods. Klarna Checkout is responsive. Klarna is a European provider of payment solutions for online trading. Klarna Checkout is available for several countries of delivery in continental Europe. Only customers based in these countries can pay for orders using Klarna Checkout. Please refer to Klarna Checkout UK if you are interested in a checkout solution for the United Kingdom.
...
If you would like to register with Klarna, then get in contact with them at the following email address: link:mailto:salj@klarna.com[salj@klarna.com]{nbsp}icon:external-link[]. You will receive the access data that you need for setting up Klarna in plentymarkets.
...
Set up and activate the desired <<payment/managing-payment-methods#20, payment methods>> in the *Setup » Orders » Payment » Methods* menu in your plentymarkets system.
...
→ The following link leads to the terms of use for Germany: link:https://cdn.klarna.com/1.0/shared/content/legal/terms/EID/de_de/checkout[https://cdn.klarna.com/1.0/shared/content/legal/terms/EID/de_de/checkout^]{nbsp}icon:external-link[] (only available in German).
...
Set up event procedures to inform Klarna when items are shipped, orders and returns are cancelled and when invoices are created or deleted. Ask your contact person in the Klarna integration team to test the functionality before you start using the interface live. Set up an <<automation/event-procedures#, event procedure>> to automatically send shipping confirmations to Klarna.
...
Set up an <<automation/event-procedures#, event procedure>> to automatically send cancellations to Klarna.
