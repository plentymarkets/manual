...
This page of the manual describes and explains how to manage payments working with the new user interface. The functions explained here are currently in a test phase, but the old user interface is also still available at the same time. Note that the information in chapter <<payment/managing-payments#10, Carrying out basic settings for incoming payments>> as well as those in chapter <<payment/managing-payments#20, Booking payments>> also apply when you work with the new user interface.
