...
* Payment is assigned to an order: <<payment/beta-managing-payments#40, Detach>> the payment from the order. But before doing so, check whether detaching the payment can safely be performed. After detaching it, it can be deleted.
* Payment was split and partial payments exist: You first have to delete the partial payments. You can only do this if they are not assigned to an order. If they are assigned, you first have to <<payment/beta-managing-payments#40, detach>> the payment from the order. But before doing so, check whether detaching the payment can safely be performed. After that, delete the partial payments. The parent payment can now be deleted as well.
--
