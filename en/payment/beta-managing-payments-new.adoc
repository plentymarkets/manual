...
You can search for payments in the *Orders » Payments* menu. This table is customisable. This means that you can decide which information is displayed in the table rows. To do so, proceed as follows:

[.instruction]
Customise table:

. Click on *Configure now* (icon:cog[]). +
→ The window *Customisable table* opens.
. Select by placing a check mark, which rows are to be displayed.
