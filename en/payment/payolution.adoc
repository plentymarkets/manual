...
[#30]
=== SSL certificateYou need to have an <<start/plentymarkets-account/service-center/ssl-certificate#, **SSL certificate**  >> in order to correctly use the payment method.
...
Set up an <<orders/settings/event-procedures#, **event procedure**  >> to automatically send shipping confirmations to Payolution when the outgoing items are booked. You do not need a shipping confirmation for instalment purchases. As such, this event procedure should only be set up for payment on invoice.
...
====
Once you have set up this event procedure, the shipping confirmation will automatically be sent to Payolution when the outgoing items are booked. Go to **<<orders/editing-sales-orders/changing-a-sales-order#, Orders » Edit orders » Open order » Tab: Overview » Tab: Status history  >>** to check this process.
====
...
To send refunds automatically, an **<<orders/settings/order-status#, order status  >>** for credit notes is required. You can use one of the standard statuses or set up your own order status. This status is then only assigned to completed credit notes. These credit notes must not be changed after creation. Set up an <<orders/settings/event-procedures#, **event procedure**  >> to automatically send refunds to Payolution.
