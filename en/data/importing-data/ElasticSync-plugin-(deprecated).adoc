= Importing data with ElasticSync (Plugin) (deprecated)
:lang: en
...
:position: 30
:url: data/importing-data/ElasticSync-plugin-(deprecated)
...
[IMPORTANT]
.Termination of software maintenance and deactivation of ElasticSync plugin
====
In order to focus more on the new ElasticSync version that is now in the core of plentymarkets, the *software maintenance for the plugin will end on 28th February 2019*. This concerns only the ElasticSync part since the FormatDesigner will be further maintained and updated. *The final deactivation of the ElasticSync plugin will take place on 30th April 2019*. With the deactivation, all existing Syncs are migrated automatically into the ElasticSync version and on the marketplace the plugin will be renamed to FormatDesigner. For those who have already installed the ElasticSync plugin, the FormatDesigner will still be available in the system without any additional effort. All others have to install the FormatDesigner plugin in order to be able to access the functionalities. +

We recommend switching to the new ElasticSync version as soon as possible. To find out how to already transfer Syncs from the plugin into the new version see this link:https://forum.plentymarkets.com/t/how-to-wechsel-von-elasticsync-plugin-auf-elasticsync-new-change-from-elasticsync-plugin-to-elasticsync-new/527224[How to]. You will find the new version in the menu *Data » ElasticSync* which is accessible right away. +

For further information on the new ElasticSync version and to find out about its advantages see link:https://www.plentymarkets.co.uk/blog/ElasticSync-ndash-New-beta-version-new-functions/b-2171/[here].
====
...
|Amazon product type
|Numeric
|For IDs see <<data/export-import/data-formats/item#50, amazon_product_type value table>>
...
|ID
|Numeric (internal ID)
|See *System » System settings » Item » Properties*
...
|ID of the status
|Numeric
|ID of the status. You determine the IDs in the range between 1.0000 and 18.9999. The statuses can be found in the *System » System settings » Orders » Status* menu.

|ID of the owner
|Numeric
|ID of the owner. An owner is a user who is assigned as owner. The ID of the owner thus corresponds to the ID of the user. The user ID can be found in the *System » System settings » Settings » User » Accounts* menu. Orders can be assigned to an owner. This is optional.

|PlentyID +
**Mandatory field**
|Numeric
|ID of the client. The plentyID can be found in the *System » System settings » Client » Select client » Settings* menu.

|ID of the location
|Numeric
|ID of the location. The locations can be found in the *System » System settings » Client » Select client » Locations* menu.
...
|Referrer
|Numeric
|The ID of the referrer is expected by default. The IDs can be found in the *System » System settings » Orders » Order referrer* menu. If your column does not contain the IDs, carry out the mapping yourself.
...
|Exchange rate
|Numeric
|Select the column that contains the exchange rate. The exchange rate must contain the currency translation rate from the order currency to the system currency. The system currency is set in the *System » System settings » Orders » Payment » Currencies* menu. Whether the number is expected to be separated by a comma or a dot depends on the selected import options. Set the import options in the *Matching* area.
...
|Country ID
|Numeric
|Select the invoice address or delivery address and select the column that contains the country ID. The IDs of the countries of delivery are expected by default. The IDs can be found in the *System » System settings » Orders » Shipping » Settings* menu.
...
2+|*Shipping package*
|Regardless of which package information is selected, you have to select a shipping package and use this to compare this information. Shipping packages are set up in the *System » System settings » Orders » Shipping » Shipping packages* menu.
