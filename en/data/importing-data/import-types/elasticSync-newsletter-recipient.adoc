...
[IMPORTANT]
.Important note when importing new email addresses
====
Currently, the import of new newsletter email addresses triggers the automatic dispatch of confirmation mails. This means that an email will be sent to those contacts who confirmed the newsletter subscription as well as to those contacts who _did not_ confirm the newsletter subscription.

This error is known and we will fix it soon. If you want to import newsletter data, we recommend deactivating the two options *Newsletter opt-in* and *Confirmation newsletter opt-in* in the *Setup » Client » [Select client] » Email » Automatic* menu for that time until we fixed this error.
====
