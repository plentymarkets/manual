...
|Package number
|
a|Select the invoice address or delivery address and select the column that contains the package number. Separate multiple package numbers by commas in one line or in multiple lines with one package number in each line.
