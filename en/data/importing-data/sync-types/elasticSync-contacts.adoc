...
|Type*
|Numeric (internal ID)
a|This is a mandatory field. It is possible to create further types. +
...
male +
diverse
a|Gender of the contact. Is saved in the contact data record in the *Form of address* field. +
...
|0 +
1
a|Contacts can be blocked for a certain client. +
...
-4 +
-5
a|The feedback of the contact is used as internal feedback only and cannot be seen by third parties. +
...
The IDs for payment methods, payment method interfaces and marketplace payment methods are listed on the <<data/internal-IDs# , Internal IDs in plentymarkets>> page of the manual in the <<data/internal-IDs#65# Payment methods>> chapter.
...
0 +
1
a|Account type in the old online store (Callisto online store). +
...
<<#table-mapping-fields-contact-properties>> lists the fields available for importing contact property data. During the import, plentymarkets checks if the imported data record already exists based on the matching field.

[[table-mapping-fields-contact-properties]]
.Fields for importing contact property data
[cols="1,1,2"]
|====
|Mapping field |Permitted import values |Explanation
...
|Property type
|Text
|Type of the property, e.g. *Short text*, *Date*, *Multi-selection*
...
|Contact ID
|Numeric
|ID of the contact that is linked to the property

|Language
|Text
|Can be set as own value in the mapping if the properties should be imported in another language than the system language, e.g. *de* or *fr*.

|Delete (Yes/No)
|0 +
1
a|Can be set as own value in the mapping. +
...
|0 +
1
a|Allow the payment method "Debit" for the contact. Go to *Setup » Orders » Payment » Payment methods* to activate the payment method. +
...
|0 +
1
a|Allow the payment method "Invoice" for the contact. Go to *Setup » Orders » Payment » Payment methods* to activate the payment method. +
