...
. Click on the plus sign *Add sync* (icon:plus-square[role="green"]).
...
Last, but not least: click on the play button *Execute sync* (icon:play-circle-o[]). Depending on the data volume to be imported, this may take a while. Once the sync is carried out, the message *Sync finished* is displayed.
