...
In order to create listings in plentymarkets with ElasticSync, a CSV file is required. To create a CSV file, use the dynamic export in the *Data » Dynamic export* menu to create a data format of the types *ItemListingMarketEbay*, *ItemListingMarketHood*, or *ItemListingMarketRicardo*, depending on which market the listings should be created for.

In order to create a market listing and a listing, the CSV file must contain at least the following fields:
