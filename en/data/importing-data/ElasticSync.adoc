...
After running a sync, the file is automatically deleted from the webspace. This is preset by default. In order to save the file, remove the check mark. *_Note_*: storing the file on the webspace is subject to a fee.
...
====
In the mapping overview, select the option *Mathematical calculations* in order to adjust values in the CSV file during the import. The options addition (+), subtraction (-), multiplication (*), division (/) and the adaption of percentages (+)/(-) are available. The value that is being adapted in the CSV column must be a number.
====
