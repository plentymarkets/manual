...
=== Video tutorial on how to import data with ElasticSync

Get an overview of the tool ElasticSync and learn how to use it for importing data into plentymarkets, to upload a CSV file, to match and map your data and to start the import.
...
=== Video tutorial on how to manually import a file one time

You are new to plentymarkets and want to manually import data one time? Watch this video and learn how to initially transfer product or customer data into your system.

video::321227231[vimeo]

=== Video tutorial on how to automatically import files on a regular basis
...
=== Video tutorial on how to add additional information via the Own value.
...
=== Video tutorial on how to modify information with RegEx

You want to import a file into plentymarkets, but first you need to modify some of its information? No need to modify the file by hand! You can use ElasticSync's "Regular Expression" function to modify the data.
...
In order to consider a CSV or TXT file during the import as desired, the so-called RegEx function can be applied. Thanks to this function, data is manipulated during the import by using commands, so that the CSV or TXT file does not need to be changed anymore.
...
=== Video tutorial on how trade one piece of information for another with the Own Assignment
...
. Click on a mapping. +
→ The mapping opens.
. Click on *Filter* (icon:filter[role="yellow-background"]) in the toolbar.
...
=== Video tutorial on how to import and export syncs
