...
. Alternatively, use the toggle button *Activate row* (icon:toggle-off[role=”red”]) on the right to activate lines for the import individually.
. *Save* (icon:save[role="green"]) the settings.
...
|*Start date from*
|Select the date from which you want to filter.
...
|*Start date to*
|Select the date to which you want to filter.
