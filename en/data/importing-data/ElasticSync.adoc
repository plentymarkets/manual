...
|*Automatic mapping*
|Select whether the mapping is to be done automatically. The columns in the CSV file can be automatically mapped with the plentymarkets data fields. To ensure that the mapping works properly, the names of the fields that you selected in the FormatDesigner must not be changed.

2+^|*Separator*

|*Separator*
|The separator of the CSV file is automatically detected. This setting is activated by default. +
...
====
In the mapping overview, select the option *Mathematical calculations* in order to adjust values in a CSV file during the import. The options addition (+++), subtraction (-), multiplication (*), division (/) and the adaptation of percentages (+)/(-) are available. The value that is being adapted in the CSV column must be a number.
====
...
.View of the mapping
====
On the left side of the mapping you can see the import data of the CSV file that have to be mapped on the plentymarkets page. The plentymarkets data fields are shown on the right. The preview (icon:eye[role="blue"]) displays the first 6 values of the CSV file.
====
...
→ The window *My files* opens.
. Select a file and click on *Select*. +
