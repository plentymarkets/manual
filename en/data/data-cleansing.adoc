...
* In addition, you can use the data cleansing function to reduce memory usage. For example, this can help if the free memory capacity included in your plentymarkets tariff is exceeded.
...
. Activate in the *Authorisations* section, the options *Data > Cleansing > Show data cleansing* and *Data > Cleansing > Edit data cleansing*.
...
. Enter a value in the input field. Pay attention to the explanations given in <<table-settings-data-cleansing>>.
. Activate the toggle button (icon:toggle-off[role="red"]).
. *Save* (icon:save[role="green"]) the settings. +
→ The data cleansing is activated (icon:toggle-off[role="green"]).

[[table-settings-data-cleansing]]
.Settings in the *Setup » Settings » Data » Cleansing* menu
[cols="1,3"]
|====
|Setting| Explanation
...
|Enter the number in days. After this time period has expired, expired newsletter subscriptions that are not confirmed are automatically deleted from your plentymarkets system. +
*_Note:_* The check for unused data records takes place every day. In this context, the system checks whether there are expired and unconfirmed newsletter subscriptions that have been unused for a time longer than the duration you set. If expired and unconfirmed newsletter subscriptions exist, these are deleted the next day. 
...
Delete the tickets automatically from your ticket system. This setting is not carried out in the *Setup » Settings » Data » Cleansing* menu, but via the procedure manager in the ticket system in the *Setup » CRM » Ticket system » Procedures* menu.
...
All changes made in the *Setup » Settings » Data » Cleansing* menu are saved in the *Data » Data log* menu for a period of 30 days. The date and time as well as the user who made changes to the values are logged in this menu.
