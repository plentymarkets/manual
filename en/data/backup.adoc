...
Use the backup function in the **Setup » Settings » Data » Backup** menu to reset data in the database to a previous state or restore deleted data.
...
The **Setup » Settings » Data » Backup** menu offers an overview of the existing backups with the corresponding dates. They are automatically created on a daily basis.
...
|**ItemVariation**
|Restores variation data records including all links such as barcodes, categories, standard categories, images, ASIN, prices, suppliers, warehouses etc.
Item bundles are restored, too. +
Assumes that the barcodes, categories, items etc. to be linked already exist or have been restored with the corresponding backup type.
