...
| *Test connection*
| Test the export settings that you entered:

* Can plentymarkets connect to the server?
* Can an export file be saved at the storage location?

*_Note:_* The option checks if a file export is possible with the current export settings. The option does _not_ test if a connection to a market can be established.
