...
| Exports the payment due date in days of an order. The automatically generated export key for this data field is *order.paymentTerms.payment.AllowedDays*.Days*.

| *Discount (days)*
| Exports the discount in days of an order. The automatically generated export key for this data field is *order.paymentTerms.valutaDays*.Days*.
...
Select the language of the shipping profile. If you offer your online shop in several languages and set a name for different shipping profiles, you can enter the data field several times and select a different language. Adapt the export key if you add the data field more than once.
