...
* In addition to the data fields listed, you can also add your <<data/exporting-data/kataloge-verwalten#164, own values>> to the export. The same value is then exported for each record.
* Use the option *Formula* to update numerical data during export. For example, you could update all exported variation prices during the export.
...
* You can edit the Export key. The “Export Keys” are the names of the column headers, as they will appear in your export file. Every “Export Key” needs to be unique.
