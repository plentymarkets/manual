= Practical examples for order and order item catalogues
:lang: en
...
This page of the manual lists practical examples on how to create a catalogue for the export of orders and order items. If you are not yet familiar with the catalogues, refer to the manual chapter <<data/exporting-data/managing-catalogues#, Managing catalogues>> first. There, you get all the general information you need for creating and managing catalogues.

The catalogue format Order comprises order data as well as order item data. Thus, depending on your workflow, you can either create separate catalogues for the export of each data or you can combine them in one catalogue. +
...
- Delivery country: To evaluate how many deliveries are shipped to which country.
...
.Data fields:
* *Order > Order ID*
* *Order > Amounts > Invoice amount*
* *Order > Delivery country* +
After you added the data field, select how the delivery country is to be exported, e.g. as ISO-Code or as ID.
...
- Item ID = To break down which items were sold.
- Quantity = To break down how many of the items (defined by the ID) were sold.

Add the following data fields for this:

* *Order > Positions > Item ID*
* *Order > Positions > Quantity*
...
You can link data fields (icon:link[rotate=90]) so that they are exported together, for example in one column of a CSV file. This is useful for some of the data, e.g. for the first and last name of a contact. You decide which links are useful for you. +
...
* *Order > Warehouse > Warehouse ID*
* *Order > Payment method*
* **Order > Positions > Item ID*
* **Order > Positions > Variation ID*
* **Order > Positions > Quantity*
* *Order > Positions > Tax rate (A, B, C etc.)* link with *Order > Positions > Tax rate in %* +
...
* *Documents > User ID* +
Here, the ID of the user who created the document is exported.
