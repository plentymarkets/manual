...
|priceCalculationId
|<<item/settings/prices#2300, Old price calculation>>

|priceCalculationUUID
|<<item/settings/prices#1000, New price calculation>>
...
|valueAddedTax
|<<orders/accounting#550, Value Added Tax in percent>>

|vatId
a|<<orders/accounting#550, ID of the tax rate>>
...
Also remember to tell plentymarkets which property you want information about. In other words, _assign a property_. Click on *assignment* (icon:sign-in[role="darkGrey"]) and choose the desired property from the drop-down list.

The field *all* is an exception. Here you do not need to assign a specific property since you are receiving information about _all of the properties_.
====
...
|all
|Returns all of the properties. The information is formatted as follows: +
...
|id
|ID for the ASIN. You need this ID if you want to <<data/importing-data/sync-types/elasticSync-item#2430, delete ASINs with the help of the import tool>>. The ID is not visible in your plentymarkets system.
...
|deliveryTimeInDays
|<<item/managing-items#310, Delivery time in days>>

|discount
|<<item/managing-items#310, Discount>>

|isDiscountable
|<<item/managing-items#310, Discountable>>

|itemDescription
|<<item/managing-items#310, Supplier item name>>

|itemNumber
|<<item/managing-items#310, Supplier item No.>>

|lastPriceQuery
|<<item/managing-items#310, Last price query>>

|lastUpdateTimestamp
|Time stamp of the last update

|minimumPurchase
|<<item/managing-items#310, Minimum purchase>>

|PackagingUnit
|<<item/managing-items#310, Packaging unit>>
