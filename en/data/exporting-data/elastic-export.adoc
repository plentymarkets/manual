...
:keywords: Export, elastic, Elastic Export, FormatDesigner, export format, export formats, Elastic Export plugin, price comparison, price comparison portal
:position: 10
:url: data/exporting-data/elastic-export
:id: COEO41G
:author: team-multi-channel, team-elasticsync

The elastic export is used to export data from your plentymarkets system. There are two possible ways to use the elastic export:

* Export data from your plentymarkets system. To do so, first use the link:https://marketplace.plentymarkets.com/en/plugins/integration/formatdesigner_6483[FormatDesigner^] to create so-called data formats. In other words, you'll decide which data should be exported. Second, use the link:https://marketplace.plentymarkets.com/en/plugins/channels/marktplaetze/elasticexport_4763[Elastic Export^]{nbsp}icon:external-link[] to export the data formats that you have just created.

* Create export formats for price search engines, price comparison portals, markets, and affiliate service providers in order to export data to the interfaces. Further information can be found in the <<Creating export formats for interfaces, Creating export formats for interfaces>> chapter.

[#10]
== Installing the Elastic Export plugin

[IMPORTANT]
.Download the Elastic Export plugin
====
The Elastic Export is a plugin. In order to use the Elastic Export plugin, download it in the link:https://marketplace.plentymarkets.com/en[plentyMarketplace^]{nbsp}icon:external-link[].
====

The Elastic Export plugin allows you to create exports for the areas mentioned above. It is possible to add more data formats to the Elastic Export through Marketplace plugins. In order to use further plugin formats, the Elastic Export plugin has to be deployed first.

[#20]
=== Licence
This project is licensed under the GNU AFFERO GENERAL PUBLIC LICENSE - find further information in the link:https://github.com/plentymarkets/plenty-plugin-elastic-export/blob/master/LICENSE.md[LICENSE.md^]{nbsp}icon:external-link[].

[#50]
== Creating export formats for interfaces

Use the *Data » Elastic export* menu to create export formats for price search engines, price comparison portals, markets and affiliate service providers in order to export data to the interface partners. In order to use the elastic export, install the plugin in your plentymarkets system. +
...
====
Install the link:https://marketplace.plentymarkets.com/en/plugins/channels/marktplaetze/elasticexport_4763[Elastic Export^]{nbsp}icon:external-link[] plugin and the plugins for the export formats in a plugin set that is linked to the default client.
====

[#100]
=== Registering with an interface partner

Before you can transfer your export format to an interface partner, you have to register on the partner's website and follow the instructions given there. Refer to <<#export-formats>> for links to the price search engines, price portals, markets and affiliate service providers that have a plugin interface in plentymarkets. All data formats, except *BolOfferExport* and *BolItemExport* (these two are available without additional plugins from the Marketplace), have to be installed from the plentyMarketplace first.
...
|*BolItemExport*
|Items are sold on the market *bol.com*. This export format is required for your item catalogue to be exported to bol.com. For further information about setting up bol.com in plentymarkets, refer to the <<markets/bol-com#, Setting up bol.com>> manual page. +
The export format *BolItemExport* is included in the *ElasticExport* plugin.

|*BolOfferExport*
|Items are sold on the market *bol.com*. This export format is required for your offers to be exported to bol.com. For further information about setting up bol.com in plentymarkets, refer to the <<markets/bol-com#, Setting up bol.com>> manual page. +
The export format *BolOfferExport* is included in the *ElasticExport* plugin.

|*CdiscountCOM*
|Items are sold on the market link:https://marketplace.plentymarkets.com/en/plugins/channels/marketplaces/elasticexportcdiscountcom_4738[Cdiscount^]{nbsp}icon:external-link[]. For further information about this market, refer to the <<markets/cdiscount#, Setting up Cdiscount>> manual page.
...
|*FashionDE*
|link:https://marketplace.plentymarkets.com/en/plugins/channels/price-comparisons/elasticexportfashionde_4742[Fashion.de^]{nbsp}icon:external-link[] is a price comparison portal for fashion and lifestyle. Please note that this website is currently only available in German.
...
|*KaufluxDE*
|Items are sold on the market link:https://marketplace.plentymarkets.com/en/plugins/markets/ElasticExportKaufluxDE_4737[Kauflux^]{nbsp}icon:external-link[]. For further information about setting up Kauflux, refer to the <<markets/kauflux#, Setting up Kauflux>> manual page.
...
|*RakutenDE*
|Items are sold on the market link:https://marketplace.plentymarkets.com/en/plugins/channels/marktplaetze/elasticexportrakutende_4721[Rakuten^]{nbsp}icon:external-link[]. For further information about setting up Rakuten, refer to the <<markets/rakuten/rakuten-setup#, Setting up Rakuten>> manual page.
...
This chapter teaches you how to create new export formats and which settings you can carry out. All data formats except *BolOfferExport* and *BolItemExport* have to be installed from the link:https://marketplace.plentymarkets.com/en[plentyMarketplace^]{nbsp}icon:external-link[] first.
...
====
Please note that no item filters for the elastic export are set by default. If you do not add any item filters, all items are exported.
====
...
|*Limit*
|Enter a number. If you want to transfer more than 9,999 data records to the interface, the output file will not be generated again for another 24 hours. This is to save resources. If more than 9999 data records are necessary, the option *Generate cache file* has to be active.

|*Generate cache file*
|Place a check mark if you want to transfer more than 9,999 data records to the interface. We recommend that you do not activate this option for more than 20 export formats. This is to ensure a high performance of the elastic export.

|*Provisioning*
|Select *URL*, *FTP* or *Admin area*. +
Different settings will appear depending on which option is selected here. +
...
*Path*, +
*Transfer*
|If you selected the option *FTP* under *Provisioning*, enter the parameters. The standard FTP port is *21*. +
*Transfer:* If you selected the option *FTP* under *Provisioning*, select an interval for the export. If you select the setting *Schedule*, determine a time frame for the data transfer under *Schedule*.

|*File name*
|If you selected the option *Admin area* under *Provisioning*, enter the file name. Each provisioning type requires that the file name has the ending of the file format, e.g. use the suffix .csv for a CSV file.

2+| Item filters

|*Add item filters*
a|Select the items you wish to export. To do so, select an item filter from the drop-down list and click on *Add* (icon:plus-square[role="green"]). There are no filters set by default. It is possible to add multiple item filters from the drop-down list one after the other.

* *Active* = Only active variations are exported.
* *Item type* = Only items of the selected item type are transferred.
...
* *Prices* = Only items with the selected price are exported. +
  ** *Without price* = Items without price are transferred. +
  ** *With price* = Items with price are transferred. +
  ** *With price lower* = Enter a price. Items with a lower price than the price that was entered are exported. +
  ** *With price higher* = Enter a price. Items with a higher price than the price that was entered are exported. +
  ** *With price between* = Enter a lower limit and upper limit. Items with a price between the two values are exported.
* *Tag (AND mode)* = Enter one or more tag names. Items that are linked to _all_ of the selected <<basics/working-with-plentymarkets/tags#, tags>> are exported.
* *Tag (OR mode)* = Enter one or more tag names. Items that are linked to _at least one_ of the selected <<basics/working-with-plentymarkets/tags#, tags>> are exported.
...
|*Product URL*
|Choose the URL that you wish to transfer to the interface. You can choose between the item’s URL and the variation’s URL. URLs of variations can only be transferred in combination with the Ceres online store.
...
If you only want to allow specific HTML tags to be exported, enter these tags into the *Permitted HTML tags, separated by comma (def. text)* field.  Use commas to separate multiple tags.
...
. Go to *Data » Elastic export*.
. Carry out the settings in the *Filter* section. Pay attention to the explanations given in <<#filter-settings-elastic-export>>.
. Click on *Search* (icon:search[role="blue"]).

[[filter-settings-elastic-export]]
...
|*Format*
|Select a format from the drop-down list. A list of all the export formats that match the selected format is displayed.
|====

[NOTE]
.Deleting search criteria
====
Click on *Reset* (icon:undo[role="orange"]) to delete the search criteria. Then click on *Search* (icon:search[role="blue"]) again. All of your export formats are displayed in the overview and you can select new filters.
====
