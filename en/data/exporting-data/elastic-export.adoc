...
====
Install the *Elastic Export* plugin and the plugins for the export formats in a plugin set that is linked to the default client.
====
...
|*Add item filters*
a|Select the items you wish to export. To do so, select an item filter from the drop-down list and click on *Add*. There are no filters set by default. It is possible to add multiple item filters from the drop-down list one after the other.
...
image::daten/assets/elastischer-export-symbolleiste.png[Symbolleiste eines Exportformats]
...
. Go to *Data » Elastic export*.
. Carry out the settings in the *Filter* section. Pay attention to the information given in <<filters-elastic-export>>.
. Click on *Search*.
...
====
Click on *Reset* to delete the search criteria. Then click on *Search* again. All of your export formats are displayed in the overview and you can select new filters.
====
