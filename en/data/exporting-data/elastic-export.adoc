...
|*Limit*
| Enter a number. If you want to transfer more than 9,999 data records to the price search engine, then the output file will not be generated again for another 24 hours. This is to save resources. If more than 9999 data records are necessary, the setting *Generate cache file* has to be active.
...
|*Item availability*
| Activate the *overwrite* option and enter item availabilities into the fields *1* to *10*. The fields represent the IDs of the availabilities. This overwrites the item availabilities that are saved in the *System » System settings » Item » Item availability* menu.
|====
...
. Carry out the desired changes. Pay attention to the information given in <<settings-elastic-export>> and <<toolbar-export-format>>.
. *Save* the settings.
