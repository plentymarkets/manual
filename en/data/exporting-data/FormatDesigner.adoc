...
The FormatDesigner is part of the plugin ElasticSync and allows you to create formats for exporting using the ElasticExport. All formats created with FormatDesigner are available in the *Data » Elastic export* menu. Using the FormatDesigner, you can not only set CSV options and select data fields, but also manipulate numeric data values during export.

[TIP]
.Plugin updated?
====
The FormatDesigner is part of the plugin ElasticSync. ElasticSync is constantly updated and improved. As such, go to *Plugins » Plugin overview » Open plugin set* and check that you use the latest version of ElasticSync before creating syncs or export formats.
====
...
|rating
|Rating of the contact. This is an internal rating only. 5 red stars are the worst and 5 yellow stars are the best rating. Possible values: +
