...
The second step, i.e. the actual import, is done in the menu *Data » Elastic export*. You can find further information about the second step on the <<data/exporting-data/elastic-export#, elastic export>> manual page.
...
The plugin has to be installed in the linked plugin set to make it visible in the menu. In the plugin overview, make sure to place it above the plugins *Ceres* and *IO* (in case they are active).
Refer to the <<plugins#, plugin chapter>> for basic information about adding plugins to your plentymarkets system.

[TIP]
.Is the plugin up to date?
====
Plugins are constantly being updated and improved. Before you start creating data formats, open the menu *Plugins » Plugin overview » _Open plugin set_* and make sure you're using the most current version of the plugin.
====
