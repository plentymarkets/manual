...
You want to export specific data such as order or item data from your plentymarkets system? Use <<data/exporting-data/managing-catalogues#110, catalogues with standard formats>> to export such data. +
...
A sales channel can offer more than one marketplace format, for example for different product categories (pet food, food, non-food, etc.) or for different languages. Check the documentation to find out which marketplace formats are available for the different sales channels.

[TIP]
.Could not find a marketplace format?
====
The marketplace formats of some sales channel are loaded into the *Data » Catalogues* menu when you deploy the sales channel’s plugin.
====
...
→ A new view opens.
. Carry out the settings. Pay attention to the explanations given in <<#table-basic-settings>>.
. *Save* (icon:save[role="green"]) the settings. +
...
|*Catalogue name* (mandatory field)
...
. Go to *Data » Catalogues*. +
→ The catalogue overview opens.
. Select the catalogue that you want to carry out the export settings for. +
→ The catalogue opens.
...
. Carry out the settings. Pay attention to the explanations given in <<#table-export-settings>>.
. *Save* (icon:save[role="green"]) the settings.
...
|*Provisioning*
|Select the form from the drop-down list in which you want the export to be provided. +
If you select FTP, FTPS (coming soon) or SFTP (coming soon), further fields become visible.

|*File name*
|Enter the file name. There is no need to add the ending .csv or .txt as this is done automatically by the system.

|*Placeholder number range*
|Enter a number range. This field is only available if you are using the Wildcard with the placeholder _numberRange_. _The maximum of the number range is limited by the amount of characters._

|*Server*
|Enter the server. This field is only visible if you have selected FTP, FTPS (coming soon) or SFTP (coming soon).

|*Port*
|Enter the port. This field is only visible if you have selected FTP, FTPS (coming soon) or SFTP (coming soon).

|*User name*
|Enter the user name. This field is only visible if you have selected FTP, FTPS (coming soon) or SFTP (coming soon).

|*Password*
|Enter a password. This field is only visible if you have selected FTP, FTPS (coming soon) or SFTP (coming soon).

|*Path*
|Enter the path for the storage location of you file. This field is only visible if you have selected FTP, FTPS (coming soon) or SFTP (coming soon).
...
|*Schedule*
|Select the time from the drop-down list at which you want the export to happen. This field is only visible if you have selected *Schedule* for the transfer.
|====

[#80]
=== Mapping and linking data fields

In the *Mapping* tag of a catalogue you can map the sales channel’s data sources to your plentymarkets data fields. Depending on the product data you want to transfer, you either map a data field or a value. For example, you select which data field in plentymarkets contains the weight of variations. In the lower section you can map values. For example, you map the sales channel’s category “Pet food” to your plentymarkets category “Animal food”.
...
. Click on a data field or on *Own value*. +
...
In the *Mapping* tab of a catalogue with marketplace format you can delete individual or several data fields at once. For marketplace formats, you can also remove the selected fallback data fields.
...
→ All data fields that have been added before are deleted. *_Note_*: This only works for data fields that have just been added, but have not been saved yet.
...
With standard formats you can create catalogues for specific data. This includes formats such as items, orders etc. which you can use to export item or order data from you plentymarkets system.

[#120]
=== Creating catalogues with standard formats
Use catalogues with standard format to export specific data such as item or order data from your plentymarkets system. Proceed as described below to create a catalogue with a standard format.
 ...
 *_Note_*: At the moment, only a demo format is available for this.
...
. Go to *Data » Catalogues*. +
→ The catalogue overview opens.
. Select the catalogue that you want to carry out the format settings for. +
→ The catalogue opens.
. Click on the *Settings* tab on the left. +
→ The different settings options are shown.
. Click on *Format settings*. +
→ The format settings open.
. Carry out the settings. Pay attention to the explanations given in <<#table-format-settings>>.
. *Save* (icon:save[role="green"]) the settings.
...
. Go to *Data » Catalogues*. +
→ The catalogue overview opens.
. Click on the catalogue you want to carry out the export settings for. +
→ The catalogue opens.
. Click on the *Settings* tab on the left. +
→ The different settings options are shown.
. Click on *Export settings*. +
...
. On the left, click on a data field or on *Own value*. +
→ The data field or the field for the own value is added to the view.
. If you selected *Own value*, enter a value into the input field below.
...
→ The selected data fields are added.
. On the right in the data row, click on *Settings* (icon:cog[]) to select a separator. +
→ The window *Data column settings* opens.
. Select a separator from the drop-down list or add an own separator. +
. *Save* the settings. +
→ The data fields are linked and will be exported with the respectively selected separator.
. Link all desired data fields.
. *Save* (icon:save[role="green"]) the settings.
...
In the *Mapping* tab of a catalogue with standard format you can delete individual or several data fields at once.
...
icon:map-signs[] *_Or_*: If you want to delete a complete row, click on *Delete data row* (icon:trash-o[]) on the right in the respective row. +
→ The complete row is deleted. +
icon:map-signs[] *_Or_*: Tick the box for the data fields that you want to delete. Afterwards, click on *Delete selected data fields* (icon:trash-o[]) at the top right. +
→ All selected data fields are deleted.

[TIP]
====
.(De)activating catalogues and changing the name
The toggle button on the left side in the catalogue overview allows you to activate or deactivate a catalogue. To edit the name of a catalogue, go to the settings (icon:cog[role="black"]) at the top.
====
...
The following sections will provide information on how to copy, delete and restore catalogues. These information apply to both marketplace and standard formats.
...
. Optional: If you want to edit the automatically created names of the copied catalogues, open the catalogues individually and adjust the name in the settings (icon:cog[]).
...
There may be situations when you delete things, but would like to have access to it again afterwards. When working with the catalogues, this is no problem. In the following you will learn how to restore a deleted catalogue or a specific version of a catalogue.
...
. Go to *Data » Catalogues*. +
→ The catalogue overview opens.
. Click on *Restore deleted catalogue* (icon:undo[role="purple"]). +
→ A new window opens.
...
icon:map-signs[] *_Or_*: Open a catalogue and at the top, click on *Restore catalogue version* (icon:undo[role="orange"]). +
→ A new window opens.
...
Once you have carried out all settings and added at least one data field, you can export your catalogue and download the export file in the desired format.
...
icon:map-signs[] *_Or_*: In the row of the catalogue that you want to export, click on the further actions (icon:ellipsis-v[]).
