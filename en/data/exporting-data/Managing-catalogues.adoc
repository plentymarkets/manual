...
*_Note_*: The standard formats will be transferred to the catalogue step by step. At the moment, the formats *Manufacturers*, *Newsletter recipients*, <<data/exporting-data/exporting-stocks#, Stocks>>, <<data/exporting-data/exporting-stock-movements#, Stock movements>>, <<data/exporting-data/catalogues-orders#, Order>> and <<data/exporting-data/catalogues-purchase-orders#, Purchase order>> are available.
...
The table in the catalogue overview is customisable. This means that you can decide which information is displayed in the table columns. To do so, proceed as follows:
...
image::data/exporting-data/assets/menu-structure-marketplace-format.png[]
...
[TIP]
====
.Additional assignment for specific data fields
Data fields such as VariationSalesPrice.price require an additional assignment, because more than one price may exist for each variation. The assignment allows you to export a specific sales price. To export more than one sales price, you can add the data field several times. Data fields that require an additional assignment have a drop-down list. The default value is always preselected.
====
...
icon:map-signs[] *_Or:_* Enter the name of the data field into the search field and navigate to the desired entry. Afterwards, press Enter to add it to the view. +
...
icon:map-signs[] *_Or_*: On the right side in the data row, click on *Reset* (icon:undo[set=material]). +
...
Check the information in the user guide of the sales channel’s plugin for details on the target data fields. <<#table-catalogue-fields>> contains information regarding the plentymarkets data fields and how to map them.

[[table-catalogue-fields]]
.Available plentymarkets data fields
[cols="1,1,2"]
|====
|Type |Available data fields |Explanation
...
|The internal attribute name is used in the data field picker.
...
|Once you selected a data field, a drop-down list containing the available languages is shown.
...
|<<item/managing-items#260, ASIN>> (Amazon Standard Identification Number)
a| * *All* +
* *Amazon Germany* +
* *Amazon Espania* +
* *Amazon France* +
...
|<<markets/otto/otto-market#905, MOIN>> (“My Otto” Identification Number)
a| * *MOIN*
| The MOIN that was assigned to the variation by OTTO Market is transferred to the sales channel.

|Manufacturer
a| * The manufacturers saved in the system
|Via the data field picker in the mapping, the manufacturers of the sales channel are mapped to the manufacturers saved in plentymarkets one by one. +
*_Note:_* The manufacturer name of the sales channel is published on the sales channel.

|Category
a| * The categories saved in the system
|Via the data field picker in the mapping, the categories of the sales channel are mapped to the categories saved in plentymarkets one by one. +
...
====
The catalogues with marketplace format created in the *Data » Catalogues* menu allow you to map the data fields of a sales channel to the correct plentymarkets data fields. Transferring the variation data to the sales channel is activated in the settings of the sales channel.
====
 ...
 *_Note_*: At the moment, the formats *Manufacturers*, *Newsletter recipients*, *Stocks* and *Stock movements* are available.
...
. Click on *Delete data field* (icon:delete[set=material]). +
→ The data field is deleted. +
icon:map-signs[] *_Or_*: If you want to delete a complete row, click on *Delete data row* (icon:delete[set=material]) on the right in the respective row. +
→ The complete row is deleted. +
icon:map-signs[] *_Or_*: Tick the box on the left for all rows containing data fields that you want to delete. Afterwards, click on *Delete selected data fields* (icon:delete[set=material]) at the top right. +
...
In catalogues with marketplace formats, both user-defined filters and template filters are visible. The latter apply automatically and cannot be deselected.
...
. Go to *Data » Catalogues*. +
→ The catalogue overview opens.
. In the row of the catalogue that you want to restore, click on the further actions (icon:more_vert[set=material]).
...
Catalogues can be exported as well as imported. While it is possible to export several catalogues at once, only one catalogue can be imported at a time. Use the import function to import catalogues from other plentymarkets systems or catalogues that have been saved as backup beforehand.
...
→ The catalogue is imported and displayed in the overview. The name of the imported catalogue contains the addition (1). If the catalogue originates from another system, the name from this system is maintained.
