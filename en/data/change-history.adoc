...
[#100]
== Change history (available 15 Feb 2021)
...
////
*_Example of an "or" filter_*:
You enter the sales price ID 123 and the variation ID 4567 into this field.
The results include data records that _either_ have the sales price ID 123 _or_ the variation ID 4567.

*_Example of an "and" filter_*:
You enter the sales price ID 123 into this field.
You enter the variation ID 4567 into the field <<Correlation>>.
The results include data records that have _both_ the sales price ID 123 _and_ the variation ID 4567.
////
...
////
*_Example_*:
You enter the variation ID 9876 into this field.
When searching, you’ll see all of the changes that were made to this variation.


*_Example of an "and" filter_*:
You enter the variation ID 9876 into this field.
You enter the sales price ID 543 into the field <<Unique IDs>>.
The results include data records that have _both_ the variation ID 9876 _and_ the sales price ID 543.
////
...
//kann gelöscht werden sobald Feedback vom Team da ist
//*_Note_*: The tags are predefined by plentymarkets.
//You cannot configure the tags yourself.
!===
|====
...
You can see your usage, i.e. your actual costs, in the <<business-decisions/plenty-bi/myview-dashboard#, plentyBI dashboard>>.
...
You can try out the change history free of charge until March 15th, 2021.
Afterwards, the following costs are incurred based on usage:
...
Yes.
You can see your usage, i.e. your actual costs, in the <<business-decisions/plenty-bi/myview-dashboard#, plentyBI dashboard>>.
In the future, it will be possible to see the number of logged changes on a daily basis via a free plentyBI key figure.
...
People with back end access rights can only see this menu if an admin gave them the right A > B.


=== Requirement: Necessary user rights

Users of the type *Back end* can only access limited areas and menus in the plentymarkets back end.
Thus, their access to the system is limited.
If you want users with *Back end* access to work with the change history, then someone with *Admin* access will need to give them the necessary rights.
...
. In the *Authorisations* section, activate the rights as listed in <<table-change-history-user-rights>>.
...
| *ABC > DEF > GHI*
|The right to search for changes.
|====
