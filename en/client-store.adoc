...
One central system is used for processing orders. This includes the printing of invoices as well as an e-mail service. It is possible to use different <<orders/settings/documents#5, **numbering** >> systems for each client (store) and each country of delivery.
...
After the client (store) has been activated, go to **Settings » Client (store)** and carry out the settings within the menu of the client in question. The setting options here are mainly the same as those for standard online stores. Therefore, refer to the **<<client-store/standard#, Settings » Client (store) » Standard  >>** pages of the manual.
...
2+|**<<client-store/standard/basic-settings#, Basic settings  >> menu**
...
|**Domain**
|The client's (store) **<<start/plentymarkets-account/service-center/domain#, domain  >>**. A domain is an Internet address in the form of a name and an extension, i.e. the Top Level Domain. The name and extension are separated from each other with a period. +
If a **<<start/plentymarkets-account/service-center/ssl-certificate#, SSL certificate  >>** was set up, the client (store)'s URL will be displayed with **https** instead of **http**. +
...
|**eBay account**
|Displays the eBay accounts that were saved in the **<<multi-channel/ebay#65, Settings » Markets » eBay » Accounts >>** menu. Select which eBay account should be used for the client (multiple selection using **Ctrl + mouse click**).

2+|**<<client-store/standard/online-store/content-pages#, Online store » Pages  >> menu**
...
**__Important__**: If you use this setting you also need to go to **<<payment/managing-payment-methods#, Settings » Orders » Payment » Payment methods  >>** and select the option **Show bank details** (place check mark) in the payment methods **Invoice** and **Cash in advance**.

2+|**<<client-store/standard/order-process/basic-settings#, Order process » Basic settings  >> menu**
...
__**Important:**__VAT rates have to be saved for the standard country of delivery. This is done in the **<<client-store/standard/accounting#, Settings » Client (store) » Standard » Accounting  >>** menu. +
