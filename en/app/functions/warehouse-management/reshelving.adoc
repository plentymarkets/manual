...
*Activated* (icon:toggle-on[role="green"]) = Your mouse cursor will automatically start out in the barcode field when you reshelve items. You can scan the source storage location right away. +
*Deactivated* (icon:toggle-off[role="darkGrey"]) = Your mouse cursor will not start out in the barcode field. Of course, it will still be possible to manually place the cursor in the field.
