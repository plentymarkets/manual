...
:author: team-mobile
:keywords: Box picking, mobile box picking, picking, packing, mobile picking, box, boxes, picking, packing
:position: 20
...
The plentymarkets app includes a function for box picking, which is described on this page. First, you'll read a short description of box picking to help you understand the concept. Then you'll learn about the <<Configuring one-time settings, settings>>, which need to be configured one time only. These settings go into effect when you begin <<Picking items with the plentymarkets app, using the function>>.
...
Before you <<Picking items with the plentymarkets app, use mobile box picking>>, you'll need to configure a few settings one time only in your plentymarkets system. These settings specify, e.g. which boxes are available, which warehouse technicians are in charge of box picking and when the order and box should be unlinked.
...
Under *Authorisations*, open the *Items » Item » Item image » Item image name* area. Place a check mark next to *Show*.

| *Update orders*
|Allow your warehouse technician to change the order's status. +
Under *Authorisations*, open the *Orders* area. Place a check mark next to *Update*.

| *Show orders*
|Allow your warehouse technician to see the order. +
Under *Authorisations*, open the *Orders* area. Place a check mark next to *Show*.

| *Create comment*
|Allow your warehouse technician to write notes and attach them to the order. +
Under *Authorisations*, open the *Comments* area. Place a check mark next to *Create*.

| *Show warehouse location*
|Allow your warehouse technician to see where the item is stored. +
Under *Authorisations*, open the *Stock » Warehouses » Warehouse location* area. Place a check mark next to *Show*.
|====

[TIP]
.Edit several user accounts at once
====
Use <<business-decisions/user-accounts-access#70, roles>> to grant the same rights to multiple users. Roles save you from having to modify each user account individually.
====

[TIP]
.Do admin users also need these rights?
====
*Admin* <<business-decisions/user-accounts-access#10, users>> are allowed to work with any function. You won't need to make any changes to their account settings. On the other hand, *Backend* and *REST-API* accounts don't automatically have rights.
====
...
* Open the <<automation/processes/procedures#510, filter control element>> and choose *Progress: Completely registered*.
...
Before you <<Picking items with the plentymarkets app, use mobile box picking>>, you'll need to configure a few settings one time only in the plentymarkets app. These settings specify, e.g. how many orders should be picked at once and how the app should react if something goes wrong.
...
. Carry out the settings. Note <<table-settings-box-picking>>.
...
| *Initial status*
|Which orders do you want to pick? When you use mobile box picking later, you'll see a list of items to pick. This list only includes items from orders that are currently in this status. +
*_Tip:_* You can customise the <<orders/managing-orders#1200, statuses>> in this drop-down list. This is done in the menu *Setup » Orders » Status*.
...
| *Warehouse*
|In which warehouse do you want to pick? When you use mobile box picking later, you'll see a list of items to pick. This list only includes items that are stored in this warehouse. +
*_Tip:_* You can customise the <<stock-management/setting-up-a-warehouse#200, warehouses>> in this drop-down list. This is done in the menu _Setup » Stock » Warehouse_.

| *Default number of boxes*
|How many orders do you typically pick at once? When you use mobile box picking later, this number will be suggested by default. However, you will still be able to manually increase or decrease the number.

| *Template*
|What should the <<Finding the items in the warehouse, list of items>> look like? The plentymarkets app comes with a default template, which is appropriate for most picking processes. However, you can create additional templates in the menu *Setup » plenty app » Mobile pick list*. Once you've created a template, you'll be able to select it from the drop-down list here. +
...
. Scan the box that you've just picked or tap the green check mark to finish the picking process. +
