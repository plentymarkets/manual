...
Currencies have 2 decimal places and that is why at some point in the calculation of sums the price will be rounded to two decimal places. Further calculation will then be carried out with these rounded values. You can define at what point prices will be rounded. This means that you define how long the systems carries out the calculation with 4 decimal places and when it will start to round to two decimal places. The settings in plentymarkets can lead to the following results:
...
. Go to **Settings » Client (store) » Select store » Locations » Select location » Accounting**.  +
...
In our example fabrics were sold. Several panels of the same fabric were bought, but with different widths. The widths are 50 cm and 30 cm. The 50 cm panel was boungt 8 times and the 30 cm panel 7 times. 30 centimetres cost 2.187 € and 50 centimetres cost 3.645 €. The prices are calculated based on the price for one metre of 7.29 €. The settings that must be carried out to get one of the results are also listed in the table.

.Comparison of calculation methods
[cols="7,16,7,7,7,7,7,7,7,7,7,7,7"]
|====
|Calculation method |Settings            |Number of panels |Variation |Single unit price gross |Single unit price net |VAT amount single unit |Total gross |Total net |VAT for order item |Net value of items |VAT total |Invoice amount

.2+|Rounded totals
.2+|You need to select 4 decimal places and activate the setting to round totals to achieve that only totals are rounded. The settings are selected by default.
|7
|Summer meadow, cotton, gras green
|2.6025
|2.187
|0.4155
|18.2175
...
|8
|Summer meadow, cotton, sunset red
|4.3376
|3.645
|0.6926
|34.700
...
.2+|Rounded order items
.2+|You need to select 4 decimal places and nothing else for rounding on order items level. The prices will be saved with 4 decimal places, but will be displayed with two decimal places. Further calculation will also be carried out with the rounded amounts.
|7
|Summer meadow, cotton, gras green
|2.6025
|2.187
...
.2+|Rounded single unit prices
.2+|You need to select 2 decimal places to round the unit price. The prices will also be saved with 2 decimal places only.
|7
|Summer meadow, cotton, gras green
|rounded to 2.60
|rounded to 2.18
...
The VAT settings are specifically for one location. You can have different settings for each location. The basic VAT relevant settings include settings that define how the system reacts if the buyer has a VAT ID or if the delivery leaves the European Union and how the VAT amount for shipping costs will be displayed. The descriptions in this section and the way the system reacts are based on the assumption that your business is located in the European Union.
...
====
The small business owner setting is only available if your system was set up with Germany as system country. The system country is a setting that you have no access to and that is selected based on the information that you provided when you ordered your plentymarkets system.
====

[.instruction]
Using the German small business owner regulation:

. Select a location under *Settings » Client (store) » Select store » Locations » Select location » Accounting*. +
→ The *Basic settings* tab will open.
...
. Select a location under *Settings » Client (store) » Select store » Locations » Select location » Accounting*. +
→ The *Basic settings* tab will open.
. Select one option from the *VAT number* drop-down menu. Pay attention to the information given in table 2.
. *Save* the settings.
...
.  +
→ The *Basic settings* tab will open.
. Select one option from the *export shipping* drop-down menu. Pay attention to the information given in table 3.
. **Save** the settings.
...
If a VAT rate was saved for a country of delivery that is outside the EU, then gross invoices will always be issued.
|====
...
image::order-processing/orders/assets/DefaultLocationsAndVATRates.png[]
__Image 2: Default VAT rates and locations__

[TIP]
.Replacing the default VAT rates and locations
====
The VAT configurations shown in image 2 is a default configuration and contains a fantasy VAT number. This VAT number is not yours and is not correct and that is why you need to set up your own configuration with a correct VAT number. You can delete the default configuration.
====
...
. Go to **Settings » Client (store) » Select store » Locations » Select location » Accounting » Tab: VAT rates**.
...
. Go to **Settings » Item » Basic settings** and select the property for the option **Item property for identifying digital items**.
...
|Select a criteria from the drop-down menu that will be used to assign the debtor accounts. +
The actual account information must be filled according to the selected criteria. +
*Initial letter* = Debtor accounts will be assigned based on the initial letters of customer information. From the second drop-down menu you can choose the customer information and the order of the information that will be used. +
If you choose the order **Company, surname, first name**, the name of the company will be prioritized. If no company name was entered, then the last name will be used and so on. +
...
|Save debtor accounts for the countries of delivery saved in your system. Do not enter debtor accounts here if you have selected anhother criteria.

|**Payment methods**
|Save debtor accounts for the payment methods saved in your system. Do not enter debtor accounts here if you have selected anhother criteria.
...
Use the **Copy settings** function to copy the settings of the current location and apply these settings to one or more other locations.
You can copy all of the settings or only specific settings.
