...
====
This page of the manual only relates to the **Settings » Orders » Coupons** menu. For comprehensive instructions on creating a coupon, refer to the **<<order-processing/orders/coupons#, Orders » Coupons  >>** page of the manual.
====

== Backend settings

Go to **Settings » Orders » Coupons** and select the **free text field** that you want to use for **coupon campaigns** (image 1, red arrow). After you have saved the setting, the field **Kampagne ID** will be displayed in the item's **link::/[Fields]** tab (image 2, red arrow). Please note that the name of the field is automatically assigned and can not be changed.

image::order-processing/orders/settings/assets/EN-Settings-Orders-Coupons-01.png[]
...
→ The **Kampagne ID** field will be displayed in the item's **link::/[Fields]** tab.

image::order-processing/orders/settings/assets/EN-Settings-Orders-Coupons-02.png[]
...
|**Clients (stores)**
|Select <<omni-channel/client-store#, **Clients (stores)**  >>. The coupon will be valid for the selected stores. You need to select at least one client.
...
The menu <<order-processing/orders/editing-sales-orders/tab-search#2-8, **Orders » Edit orders** >> also includes a search filter for coupon codes.
...
The particular campaign first has to be assigned a <<omni-channel/online-store/_cms-syntax/basics/template-variables#, **template variable**  >> for this purpose, e.g. **CouponCode1**.
...
. Find the free text field that was created for coupon campaigns in the menu <<order-processing/orders/coupons#, **Settings » Orders » Coupons**  >>. In this field, enter the ID of the campaign that you want to link to this item.
. **Save** the settings.
