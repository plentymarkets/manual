...
[#100]
== General information on the creation of orders in plentymarkets
...
For individual solutions, this list can be extended with __**further sub-entries**__ (1.3, 1.4, 2.1, 2.2, ...). See chapter
...
<<default order statuses, Default order statuses>> that are linked to automatic processes cannot be deleted. All other statuses can be edited or deleted.
...
. Use the **from** and **to** fields to select a **starting** and **ending date** for the search.
...
You can see quick information about an order by looking at its title in the overview. You can see the <<order-processing/orders/managing-sales-orders#order-status, **status**>> and logos that show the **shipping service provider** and the **<<order-processing/payment#, payment method>>**. You may see other icons there as well depending on the configuration and the procedures that were carried out.
...
*Important*: If the order status is *7*, you cannot lower this status un you have reset the outgoing items that were booked.
...
. Go to *Orders » Edit orders*.
. Open the desired order and click on the *Calculate stock* icon in the order overview.
. Click on the *shopping cart* icon in the *stock* area.+
→ A separate window will open for *creating a reorder*.  +
→ If you receive an error message telling you that no *supplier* is linked to this item, link a supplier to the item first.
. Enter your desired *quantity* for the reorder into the text field on the left.
