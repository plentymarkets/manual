...
Before you create your first credit note, go to *<<order-processing/orders/basic-settings#, System » Orders » Settings>>* and check the standard status for credit notes. Change this setting if necessary and then save your settings.

[IMPORTANT]
.Save the basic settings
====
The basic settings in the *<<order-processing/orders/basic-settings#, Settings » Orders » Basic settings>>* menu have to be saved once when the system is first set up. The settings will not go into effect until they have been saved. As such, you should click on the save icon even if you have not made any changes.
====
...
→ The order will open in a separate tab.

By selecting the option *Credit note: create* in the *Overview* tab, a new credit note order will be created in plentymarkets. You have now created a special order, which is linked to the original order and includes all items and prices.
...
If a credit note has been created for an order, there will be a note on the right-hand side of the *Overview* tab, telling you that there is a *Credit note available*.

If a credit note was created for an order, it is linked in the *Overview* tab and can be found by using the *Credit note: search* drop-down list.
...
You have to create the document template *Credit note* before generating receipts for credit notes. Go to *<<order-processing/orders/order-documents#, System » Orders » Documents>>* to set up this document template.
...
. Carry out the settings according to <<table-settings-reverse-stock>>.
...
==== Refunds in the *Settings* Tab

The *Refund to customer* option is only displayed for the payment methods *PayPal*, *ClickandBuy* and *heidelpay*.

[.instruction]
Granting a refund:

. Open the credit note's *Settings* tab and click on the *payment icon* next to the option *Refund*.
. Check the refund *amount* and adjust if necessary.
. Click on the *gear-wheel icon* to grant the refund.
...
Warranties are quite similar to returns. In both cases, the items of an order in *status 7* will be sent back to you. In contrast to a return, customers expect either a new delivery or a refund in the form of a credit note. In case of a warranty, please note that you will usually not book items back in from the ones returned but rather that you will have a second delivery with a separate order ID and overall with its own outgoing items.

The terms *warranty* and *guarantee* are often used as synonyms. In plentymarkets, they are used as follows:

* *Warranty* +
In plentymarkets, a warranty is defined as the *seller's* legal obligation to correct deficiencies, which were already present at the time the item was delivered to the customer at no charge for the customer. Warranty regulations are subject to national law. Please read up on regulations and time limits in your country or contact a legal advisor for such information.
* *Guarantee* +
In plentymarkets, a guarantee is defined as a voluntary affirmation of the *manufacturer and/or seller* how to proceed if damages or problems should occur. A guarantee is not based on legal specifications but on a contractual agreement between manufacturer/seller and customer. Therefore, regulations and time limits are not subject to legal regulations but are within the discretion of the guarantor.
...
If you want to reimburse the item's costs, create a *credit note* in the system.

In case of exchange or repair, create a *warranty*. Go to *<<en/order-processing/orders/basic-settings#, System » Orders » Settings>>* to select whether the *shipping costs* should be covered in the warranty.
...
In general, you create a warranty out of a return. However, it is also possible to create it out of the original main order.

[.instruction]
Creating a warranty:

. Open the *return* or the original main *order*.
. Click on the *Warranty**drop-down list and select *create*. +
→ The window *Warranty for the order* with the ID opens.
...
→ The outgoing items are booked and the *status* changes automatically to *status 7*
...
If you have carried out changes, entered data or changed settings, *save* your changes.
...
[IMPORTANT]
.Visibility of conditions for repairs
====
...
The *scheduler order* is a subscription module. It allows your customers to subscribe to certain items in your online store. The scheduler menu is only visible in your plentymarkets system if you booked this module.
