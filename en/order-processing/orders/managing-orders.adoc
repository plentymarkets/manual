...
In plentymarkets, an order is automatically created when customers place orders in your online store. Separate orders with different order types are created for business transactions such as <<en/order-processing/orders/managing-orders#300, Delivery order>>, <<en/order-processing/orders/managing-orders#400, Return>>, <<en/order-processing/orders/managing-orders#500, Credit note>>, <<en/order-processing/orders/managing-orders#600, Warranty>> and <<en/order-processing/orders/managing-orders#700, Repai>>. These orders are always linked to the main orders. The <<en/order-processing/orders/managing-orders#1400, Order overview>> allows you to open and edit orders regardless of their origin. For many order types, such as <<en/order-processing/orders/managing-orders#300, Delivery orders>>, it makes sense to set up <<en/basics/automating-processes#, Event procedures>> that allow you to automate procedures during order processing. Some examples for using event procedures during order processing are provided in the following chapters. +
Orders that reach you in other ways than via your online store are created <<en/order-processing/orders/managing-orders#200, manually>> in your plentymarkets system. For example, this is the case for orders placed via phone, email or in the case of a cash sale.

[NOTE]
.Saving basic settings
====
Before processing the first orders in your plentymarkets system, adjust the settings in the *<<en/order-processing/orders/basic-settings#, Basic settings>>* menu. *Save* those settings once even if you have not changed the default values.
====
...
. Click on the contact for which a new order should be created. +
→ The data set of the contact opens
. Select the option *New order* from the menu bar on the left to create a new order for the contact. +
...
. Select the variations and corresponding quantitites that should be added to the order.
. *Save* the settings. +
→ The order is created and opens in a new tab.

[#220]
=== Creating an order in the customer overview

The menu *CRM » Edit customer* enables you to create a new order via the customer overview. To create a new order for a customer, proceed as explained below.
...
. Click on the customer for which a new order should be created. +
→ The data set of the customer opens.
. Click on *New order* in the tool bar on top of the page to create a new order for the contact. +
→ The tab *New order* opens.
. Click on *Search* to display a list of variations or use the search fields to search for the *Item ID* or further *Item data*.
...
In the menu *CRM » Edit customer*, it is possible to process cash sales without assigning them to a specific customers. This is done by creating an order via the customer profile *Barverkauf* (cash sale). This profile is available by default in your plentymarkets system. To create an order for a cash sale, proceed as described below.

[WARNING]
.Payment method and order documents for cash sales
====
When creating an order for a cash sale, make sure to select the correct *Payment method* in the *Order data*. Also, sales orders are not assigned to a specific customer saved in your system. Thus, the necessary *<<en/order processing/orders/order-documents#, order documents>>*, such as the invoice, have to be manually generated in the *Receipts* tab of the order.
====
...
====
To use delivery orders, you must select the option *b) a warehouse can be assigned for each stock unit* for the *Assignment of warehouses* in the menu *<<en/order-processing/orders/basic-settings#, System » Orders » Settings>>*.
====
...
====
We recommend to create delivery orders for *all* items included in the main order. One or several items remaining in the main order without being assigned to a delivery order may cause problems in your plentymarkets system.
...
====
To open main orders, variable users, variable users must be *<<en/basics/working-with-plentymarkets/manage-users#100, authorised>>* to view all warehouses linked to the delivery orders of the main order. Thus, check the settings in the *System » Settings » User » Accounts » Tab: Right » Tab: Visibility* menu.
====
...
====
In plentymarkets, the status *9* is assigned to each return by default. However, you can change the standard status according to your requirements in the *<<en/order-processing/orders/basic-settings#, System » Orders » Settings* menu by selecting another *Standard status for return*.
====
...
|*Status*
|Select a status for the return. The standard status defined in the *<<en/order-processing/orders/basic-settings#, System » Orders » Settings* menu is displayed here by default.
...
Before generating return slips, the document template *<<en/order-processing/orders/order-documents/generating-return-slips#, Return slip* in the menu *<<en/order-processing/orders/order-documents#, System » Client » Select client » Locations » Select location » Documents » Return label>>* has to be set up.
...
====
An overview of all incoming items for which returns have already been announced is available in the menu *<<en/stock-management/managing-reorders/managing-backlog-lists#400, Stock » Returns » Supply>>*.
====

[#500]
== Working with credit note orders

Credit note orders are used for reimbursing your customers with a certain amount. This can be the case when processing a <<en/order-processing/orders/managing-orders#400, return order>>. A credit note in plentymarkets does not represent a reverse invoice but a refund. Credit notes orders are generated from an existing main order in the menu *Orders » Edit orders*. A credit note order is thus always linked to a main order. The credit notes generated in a credit note order are included in your financial accounting such as the *DATEV* export.
...
====
If a customer has already paid for an order and decides against the purchase before you have shipped the goods and booked outgoing items, we recommend to generate a credit note including all order items in the main order and to cancel the original main order.

In case payment, outgoing items and delivery have already taken place and a <<en/order-processing/orders/managing-orders#400, return order>> or a <<en/order-processing/orders/managing-orders#700, repair>> is created, the credit note order is generated directly in the corresponding return order or repair.
====
...
The information that can be printed on a reversal document is identical with the information included in an invoice. Thus, further information on the required settings can be found on the *<<en/order-processing/orders/order-documents/generating-an-invoice, invoice>>* manual page.
...
Warranties are quite similar to <<en/order-processing/orders/managing-orders#400, returns>>. In both cases, the items of an order in *status 7* will be sent back to you. In contrast to a return, customers expect a new delivery or a refund in the form of a <<en/order-processing/orders/managing-orders#500, credit note>>.
...
If the conditions for a warranty apply, you as the seller can decide whether to refund the costs to the customer or exchange it for a new item. Proceed with creating a <<en/order-processing/orders/managing-orders#500, credit note>> to grant the customer a refund. Create a <<en/order-processing/orders/managing-orders#600, warranty>> in your plentymarkets system to replace the damaged item. Go to *<<en/order-processing/orders/basic-settings#, System » Orders » Settings>>* to determine whether the *shipping costs* should be covered in the warranty.
...
All further steps necessary to complete the credit note order are identical to the regular procedure for credit note orders. The further steps to be taken are described in detail in the <<en/order-processing/orders/managing-orders#500, credit note>> chapter on this manual page.

[#700]
== Working with repairs

In plentymarkets, repairs are represented by a separate order type. Repairs consist of returned items that need to be repaired. Thus, repairs can only be created from <<en/order-processing/orders/managing-orders#400, returns>>.
...
.Options for setting up multi-orders
[cols="1,3"]
|====
|Setting
|Explanation
...
You can configure multi-invoices in the *<<en/order-processing/orders/basic-settings#, System » Client » Select client » Locations » Select location » Documents » Multi-invoice>>* menu. In this menu, you can specify detailed settings for the document and you can also save a *<<en/order-processing/orders/order-documents#, document template>>* in the form of a PDF file. Customize the template as desired. For example, you may want to copy the values that are saved in the *Invoice* template and then make adjustments to these values as needed here.
...
*Important:* Variable users can only select those clients that they have a right to access. Variable users are given rights under *<<basics/working-with-plentymarkets/manage-users#, System » Settings » User » Accounts » Tab: Right>>*.
|====
...
This tab includes the same menus that are available when you access data for a particular customer within the *<<crm/managing-customer-data#, Customers>>* menu. Changes that you make here will also go into effect for the particular customer within the *Customers* menu.
...
. Click on *Search*. +
→ A list of all existing order statuses opens.
. Click on a status to open.
→ The details of the status are opened.
...
.Store visibility
====
The option *Store visibility* cannot be used for order statuses yet. This function is currently being developed.
====
...
Some orders may not be displayed depending on how the rights were configured for the user. This applies to the *Status*, *Client (store)* and *Warehouse* options that can be restricted in the user configuration. These options are found by clicking on the *Rights* tab and then on the *Visibility* tab. If needed, go to *<<en/basics/working-with-plentymarkets/manage-users#90, System » Settings » User>>* and check the settings for users who work with the order search.
...
====
If you are searching for one or several orders with the help of one of the search fields, set the search option *Status* correctly. If you are searching for a specific invoice number, we recommend that you select *ALL* as the status option, since the order could have already been delivered or could have a status beyond *7*. Hence, it is important not to forget that only orders which match all selected criteria are displayed. If you can not find an order despite having entered the exact ID, you might have forgotten to select *ALL* for the search option *Status*.
====

[#1320]
=== Filtering by customer data, item data and invoice numbers

If you are searching for specific customer or item data, then it is recommended that you use the search fields *Customer data* and *Item data*. You can enter search terms as well as numbers into these fields. You can use the *Customer data* field to search for customer names and zip codes. However, you can not search for telephone numbers. Search terms entered into the *Item data* field will be searched for in the fields item ID, item number and name. However, the fields containing description texts (short_description, description) will not be browsed. When searching for *Invoice numbers*, you can also search for external invoice numbers.
...
|*Shipped on*
|This is the *shipping date* that was saved in the item data record. As such, this option is relevant for *<<en/order-processing/orders/managing-orders#advance-order, advance orders>>*. If the order includes several items that were all ordered in advance, then the date that is farthest in the future will be displayed here. The date can be changed manually.
...
Check the *access data* if the email was not sent correctly. Go to *<<en/basics/data-exchange/api-log#, Data » API log>>* and use the *Email* process to see the corresponding error messages.
...
You can create receipts in the *Receipts* tab, e.g. an order confirmation, a delivery note or an invoice. The documents are created with the help of document templates that are saved under *<<en/order-processing/orders/order-documents#, Settings » Orders » Documents>>*. Check to make sure that you created the document templates before you create receipts. Check this as well if you receive an error message.

[TIP]
.Tip: Copying document templates
====
If you want to create a *<<en/order-processing/orders/settings/documents#20, document template>>* quickly, then copy a previously configured document template and make changes to the template as needed.
====
...
The *Customer* tab displays information about the *<<en/crm/managing-customer-data#, customer>>*. You can edit this information directly without having to switch to the customer's data record.

[#1590]
=== Tab: Tickets

The customer's *<<en/crm/using-the-ticket-system#, tickets>>* are displayed in the *Tickets* tab. You can edit these tickets directly without having to switch to the ticket system.
