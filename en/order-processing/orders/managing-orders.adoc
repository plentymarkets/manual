...
====
To open main orders, variable users, variable users must be *<<basics/working-with-plentymarkets/manage-users#100, authorised>>* to view all warehouses linked to the delivery orders of the main order. Thus, check the settings in the *System » Settings » User » Accounts » Tab: Right » Tab: Visibility* menu.
====
...
Some orders may not be displayed depending on how the rights were configured for the user. This applies to the *Status*, *Client (store)* and *Warehouse* options that can be restricted in the user configuration. These options are found by clicking on the *Rights* tab and then on the *Visibility* tab. If needed, go to *<<basics/working-with-plentymarkets/manage-users#90, System » Settings » User>>* and check the settings for users who work with the order search.
