...
In plentymarkets, you can manually generate a single return slip in the order overview.
...
|**Change order status to status**¤|Select a status that should be applied to the order after generating the return slip. Choose the option **No change** if a status change is not desired.
...
If you wish to generate several return slips at a time, use the search filter in the **Orders » Fulfilment** menu to determine the orders for which return slips should be generated. In addition, a status change can take place after the return slips have been generated.
