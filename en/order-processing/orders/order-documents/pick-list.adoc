...
Use the menu **System » Orders » Documents » Pick list** to determine which item data should be displayed on your pick lists, such as **Item ID**, **Manufacturers** or **Item price**. +

You can carry out individual settings for most of the fields described in <<table-orders-options-pick-list>> concerning the parameters **width**, font size and font weight that allow you to individualise the layout of the warehouse pick list. Use the **Preview** button to check the results of the settings while preparing the PDF template.
To display the fields on the pick list in the order of your choice, assign the position numbers 1 to 14 from the respective drop-down menu to them. Select the option *Pos* for fields that should not be displayed on the pick list.
