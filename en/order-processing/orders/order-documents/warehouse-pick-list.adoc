...
In plentymarkets, you can generate warehouse pick lists as PDF files. In contrast to the regular pick list, the warehouse pick list enables you to see which orders or invoices belong to particular items. This way, items can be matched to specific orders during the picking process and can be packaged directly afterwards.
...
image::en/order-processing/orders/order-documents/assets/EN-Settings-Orders-Documents-Warehouse-Pick-List-01.png[]
...
|**Top margin**
|Enter a value (in mm) for the top margin on the page. If you use a PDF template containing the company information and the logo at the top of the page and want to position this "header" above the warehouse pick list, adjust this value (see chapter 3).
...
.Mandatory setting
====
At least one of the options **Order ID** or **Invoice number** have to be activated. Select a position to activate these options. Make sure to assign a position number that is not already being used.
====
...
In order to generate a warehouse pick list, proceed as explained below. +
