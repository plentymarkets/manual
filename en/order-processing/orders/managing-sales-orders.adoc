...
Some orders may not be displayed depending on how the rights were configured for the user. This relates to the **Status**, **Client (store)** and **Warehouse** options that can be restricted in the user configuration. These options are found by clicking on the **Rights** tab and then on the **Visibility** tab. If needed, go to **<<basics/working-with-plentymarkets/manage-users#, Settings » Basic settings » User>>** and check the settings for users who work with the order search.
...
If a package number was saved, then the **preview icon** can be used for tracking the package. However, this can only be done if a corresponding tracking code was saved under **<<order-process/fulfillment/preparing-the-shipment#, Settings » Orders » Shipping » Shipping options » Tab: Shipping service provider>>**. Otherwise the preview icon will not be visible.
|====
...
Check the **access data** if the email was not sent correctly. Go to <<basics/data-exchange/api-log#, **Data exchange » API log**>> and use the **E-Mail** process to see the corresponding error messages.
...
. Go to **Settings » Client (store) » Open store » Locations » Open location » Documents » Refund reversal**.
...
.  Select the option **Refund reversal** from the **Create receipt** drop-down menu.  +
...
Variable users also need to be given the right to create and edit orders. Go to **Settings » Basic settings » User » Accounts** and open the particular user profile. Then click on **Tab: Right » Tab: Menu » Area: Orders**.
