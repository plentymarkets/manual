...
Fulfillment comprises all the individual steps necessary for processing orders, e.g. generating invoices and printing labels. It is also possible to have the order status change automatically with each step. The following pages describe all the individual steps of the fulfillment process. The goal is to show you how to make your link::/[**fulfillment process as efficient as possible**]. This helps you provide quick and efficient service, which is extremely important for online businesses and can turn first-time buyers into regular customers.

[NOTE]
.Requirement: Create document templates first
====
Create **<<order-processing/orders/settings/documents#, document templates  >>** before you generate PDF documents such as invoices or delivery notes.
====
...
To set up templates for the following documents, go to **Settings » Client (store) » Select store » Location » Select location**.
...
* link::/[**SEPA direct debit mandate**]

[.instruction]
Setting up a localised document:

. Select a location under **Settings » Client (store) » Select store » Locations**.
...
To set up templates for fulfilment documents, go to **Settings » Orders » Documents**. You can set up templates for the following fulfilment documents:

* <<order-processing/orders/settings/documents/reorder#, **Reorder**  >>
* link::/[**Address label**]
* <<order-processing/orders/settings/documents/pick-list#, **Pick list**  >>
* <<order-processing/orders/settings/documents/warehouse-pick-list#, **Warehouse pick list**  >>
* <<order-processing/orders/settings/documents/packing-list#, **Packing list**  >>
...
image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-01.png[]
...
For further information about generating the actual documents, e.g. an invoice, refer to the **<<order-processing/orders/fulfillment#, Fulfilment  >>** page of the manual.
...
The margins are important for the automatic page break and the positioning of body text. They are also important for positioning the <<order-processing/orders/settings/documents#170, **PDF templates** >>.
...
The document format is defined by the PDF template. The standard format for e.g. invoices is DIN A4 if no PDF template with another format is linked to the document template. For further information, refer to the <<order-processing/orders/settings/documents#170, **PDF templates** >> chapter.
...
image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-02.png[]
...
This area is only displayed if link::/[**characteristics**] were saved in the system for the language of the document template. You can use text fields to name these characteristics and coordinates to position them in the document.
...
image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-03.png[]
...
image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-04.png[]

__Image 4: barcode settings; optional elements__

image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-05.png[]
...
image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-06.png[]
...
|The wildcard characters will be replaced by the actual values that are saved in the link::/[**customer data**]. Pay attention to the example for terms of payment on the <<order-processing/orders/settings/documents#30, **New customer** >> page of the manual. +
...
image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-07.png[]
...
image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-08.png[]
...
image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-09.png[]
...
image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-10.png[]
...
If you do not already have a business letter template, then create one including a logo and all of your company information. You can use a word processing program for this. Create a PDF file from this document. This can be done e.g. with the free office package link:http://www.openoffice.org/[**OpenOffice**, window="_blank"]. OpenOffice allows you to directly **export your documents as PDFs** (image 11).

image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-11.png[ ]
...
image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-12.png[ ]
...
image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-13.png[ ]
...
. Go to **Settings » Client (store) » Open store » Locations » Open location » Documents » Invoice**.
...
. Go to **Settings » Client (store) » Open store » Location » Open location » Documents » Invoice**.
...
Once you have generated document templates, you can go to **<<order-processing/orders/settings/event-procedures#, Settings » Orders » Event procedures  >>** and set automatic procedures. One option that you may find useful is to have variations booked out when invoices are printed. You can also define additional procedures here, e.g. automatically sending emails.
...
In order for these settings to take effect and to be displayed on the documents, you need to go to **<<omni-channel/online-store/standard/accounting#, Settings » Client (store) » Standard » Accounting » Tab: VAT rates  >>** and set the **margin scheme** to one of the four tax rates.
include::{includedir}/presets/draft-sina.adoc[]

image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-14.png[ ]

__Image 14: carrying out the settings for margin scheme__

image::order-processing/orders/settings/assets/EN-Settings-Orders-Documents-15.png[ ]
