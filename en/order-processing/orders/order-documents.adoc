...
Once you have generated document templates, you can go to **<<basics/automation/event-procedures#, Settings » Orders » Event procedures  >>** and set automatic procedures. One option that you may find useful is to have variations booked out when invoices are printed. You can also define additional procedures here, e.g. automatically sending emails.
...
In order for these settings to take effect and to be displayed on the documents, you need to go to **<<order-processing/orders/_accounting#, Settings » Client (store) » Standard » Accounting » Tab: VAT rates  >>** and set the **margin scheme** to one of the four tax rates.
include::{includedir}/presets/draft-sina.adoc[]
