...
* <<order-processing/orders/order-documents/generating-invoices#, Invoice>>
* <<order-processing/orders/multi-orders#_generating_a_multi_invoice#, Multi-invoice>>
* <<order-processing/orders/order-documents/generating-delivery-notes#, Delivery note>>
* <<order-processing/orders/order-documents/generating-credit-notes#, Credit note / Adjustment form>>
* <<order-processing/orders/order-documents/generating-dunning-letters#, Dunning letter>>
* <<order-processing/orders/order-documents/generating-return-slips#, Return slip>>
* <<order-processing/orders/order-documents/generating-an-entry-certificate-gelangensbestaetigung#, Entry certificate (Gelangensbestätigung)>>
* <<order-processing/payment/managing-bank-details#200, SEPA direct debit mandate>>

[.instruction]
Setting up a localised document:

. Select a location in the *System » Client (store) » Select store » Locations* menu.
. Select a location.
...
* Reorder
* <<order-processing/fulfilment/generating-documents#300, Address label>>
...
Some settings are already available by default once you open one of the menus described above. You can edit those settings and carry out additional settings. If no settings are available by default, you can apply settings from a <<order-processing/orders/order-documents#1600, default document template>>. Furthermore, you can add notes that are automatically included in documents based on certain options and settings made. Separate templates have to be set up for each language. Select the language at the top of the document menu.
...
[[tale-documents-mandatory-fields]]
...
. Go to *System » Client » Select client » Locations*.
. Select a location.
...
Using the example of <<order-processing/orders/order-documents/generating-invoices#, invoice documents>>, the following instruction describes how to define a number range. Proceed the same way for other documents.
...
For these settings to take effect and be displayed on documents, go to *<<order-processing/orders/accounting#, System » Client » Select client » Locations » Select location » Accounting » Tab: VAT rates>>* and select one of the 4 VAT rates for the option *Margin scheme* for the default country.
The margin scheme setting in plentymarkets is an older customized implementation. The tax rate will be applied to the entire amount of the order items that the margin scheme applies to. In other words, the tax rate will not be applied to the difference between sales price and purchase price.
