...
*_Note_*: Users with the user class *Variable* who select the filter option *ALL* still only see those orders that are in a particular status. This is done by opening the variable user's account and selecting one or more statuses under *Visible order status*. Afterwards, the variable user only sees orders that have this status, even if he or she chose the filter setting ALL.
