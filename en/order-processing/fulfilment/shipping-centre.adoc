...
*_Note_*: Users with the user class *Variable* who select the filter option *ALL* still only see those orders that are in a particular status. This is done by opening the variable user's account and selecting one or more statuses under *Visible order status*. Afterwards, the variable user will only see orders that have this status, even if he or she chose the filter setting ALL.
...
If you have not yet configured shipping packages in the *Orders » Shipping » Shipping packages* menu, then standard packages will be assigned to the shipping order. +
...
For further information, refer to the <<order-processing/orders/managing-orders#, Orders>> area.
...
If you reset an order, it is available as a pending order in plentymarkets. However, it is still registered with the shipping service provider and you will be billed for it. The *Reset* function simply resets the registration. The shipping order itself is not cancelled with the service provider. Therefore, we recommend that you always cancel the order.
====
...
→ The polling file will be generated. Save it to your computer.
...
You can provide the labels to end customers for download in the My account area of the online store. For further information, refer to the <<auftragsabwicklung/fulfillment/versand-vorbereiten#, Preparing the shipment>> page of the manual. Specifically, refer to the <<auftragsabwicklung/fulfillment/versand-vorbereiten#3200, DHL Retoure Online>> chapter.
...
It is not possible to add packages in the order's <<order-processing/orders/managing-orders#changing-order, Settings>> tab. If you want to add further packages to an order or change the quantity, then open the order in the shipping centre and make the changes in the <<order-processing/fulfilment/shipping-centre#400, Add>> tab.
