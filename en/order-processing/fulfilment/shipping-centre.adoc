...
→ You can combine this function using an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** which, for example, automatically books outgoing items.
...
It is not possible to add packages in the order's <<order-processing/orders/editing-sales-orders/changing-a-sales-order#40, **Settings** >> tab. If you want to add further packages to an order or change the quantity, then open the order in the shipping centre and make the changes in the **<<order-processing/fulfilment/shipping-centre#400, Add >>** tab.
