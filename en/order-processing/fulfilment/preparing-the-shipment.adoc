...
Proceed as follows to set up link:https://gls-group.eu/DE/en/home[GLS Germany^] in plentymarkets. Note that the data you enter in the *System » Orders » Shipping » Shipping service provider » GLS Germany* menu is available only in the CSV file generated in the *Orders » Fulfilment » Area: List of delivery addresses* menu with the data format *GLS preadvise*.
...
*_Important:_* The data that you entered here is exported only in the CSV file generated in the *Orders » Fulfilment » List of delivery addresses* menu with the data format *GLS preadvise*.
...
While placing their orders, customers should be able to request that their items are assembled for them. You can offer customers such a service with link:https://www.hermesworld.com/de/unsere-dienstleistungen/distribution/2-mann-handling/2-mann-handling/[Hermes 2-Mann-Handling^] (two-man handling). In order to use this service, create an order property. Create an order property with the options *Yes* and *No*. Depending on which *property type* you select, you are able to configure the options that your customers later selects in your online store.
...
Proceed as follows to set up the interface to Hermes 2-Mann-Handling (two-man handling) in plentymarkets.
...
.Settings for the Hermes 2-Mann-Handling (two-man handling) interface
[cols="a,a"]
|====
|Setting |Explanation
...
.Hermes 2-Mann-Handling (two-man handling) shipping services in the *Table of shipping charges* tab
[cols="1,3"]
|====
|Setting |Explanation
...
Proceed as follows to set up the interface to link:https://profipaketservice.hlg.de/wps/portal/PROPS_DEU/Home/Start/[Hermes ProfiPaketService^] (professional parcel service) in plentymarkets.
