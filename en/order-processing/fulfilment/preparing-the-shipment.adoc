...
|**Warehouse**
|Select which warehouse the country of delivery should be assigned to. This warehouse will automatically be selected for orders from this country. Select **ALL** if the country of delivery should be assigned to all of the warehouses. In this case, the warehouse with the lowest ID will automatically be selected for the orders. Pay attention to the <<order-processing/orders/_basic-settings#, **warehouse settings**  >> in the **Settings » Orders » Basic settings** menu as well.
...
====
In order to combine the generation of export documents with the booking of outgoing items set up an event procedure. To do so, go to **Settings » Orders » Event procedures**. For further information, refer to the **<<basics/automation/event-procedures#, Event procedures  >>** page of the manual.
====
...
Go to **Settings » Orders » Order status** and set up two <<order-processing/orders/managing-sales-orders#order-status, **order statuses**  >> for the orders transferred to DHL Fulfillment. Below are two examples:
...
|**Reasons for return**
|Link the reasons for return that you created under **<<order-processing/orders/managing-sales-orders#return, Settings » Orders » Order types » Return  >>** with the reasons given by DHL Fulfillment.
|====
