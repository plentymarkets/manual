...
Proceed as follows to set up the interface to link:https://gls-group.eu/DE/en/home[GLS Germany^] in plentymarkets.

[.instruction]
Setting up the interface:

. Go to *System » Orders » Shipping » Shipping service provider » GLS Germany*.
. Carry out the settings. Pay attention to the information given in <<table-interface-gls-germany>>.
. *Save* the settings.

[[table-interface-gls-germany]]
