...
You need to register with **link:https://www.mediafinanz.de/en/collection-services[mediafinanz, window="_blank"]** before setting up the interface in plentymarkets.

[#20]
== Setting up mediafinanz in plentymarkets
...
. Go to **Settings » Basic settings » Master data**.
...
. Go to **Settings » Orders » Debt collection service provider » mediafinanz**. +
...
In order to assign a dunning level to unpaid orders, an **<<order-processing/orders/settings/order-status#, order status  >>** is required. You can use one of the standard statuses or set up your own order status. The following procedure will assign a dunning level to unpaid orders.
...
Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically assign the dunning charge item to unpaid orders.
...
. Click on the **Procedure** tab.
. Select **ACTIVE** from the **mediafinanz** drop-down menu.
. **Save** the settings.
