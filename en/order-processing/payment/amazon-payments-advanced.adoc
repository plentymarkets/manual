...
[IMPORTANT]
.Amazon Payments Advanced and store design
====
Note that the following conditions must be met before integrating Amazon Payments Advanced successfully in plentymarkets:
...
[#20]
===  Payment process with Amazon Payments Advanced

After the customer has added an item to the shopping cart (template: **ItemViewItemToBasketConfirmationOverlay**) a button is displayed (template variable: **$AmazonPaymentsAdvancedButton**). By clicking this button, the customer will be forwarded directly to Amazon Payments Advanced. The customer then has to log into Amazon Payments Advanced. The customer will then go through the widgets confirming the delivery address and the payment method in your online store. The customer will then go through the rest of the order process as usual. The payment by Amazon Payments Advanced will not be triggered before the order has been completed. +
Amazon Payments Advanced payments can also be started directly from the shopping cart, if the customer clicks on the corresponding button there. Furthermore, the customer can select Amazon Payments Advanced later when selecting the payment method during the checkout. In this case, the customer will be forwarded to the Amazon Payments Advanced page  and continues with the same process as described above.

[#30]
=== SSL certificate

Your online store needs to have an **<<basics/start/plentymarkets-account/service-center/ssl-certificate#, SSL certificate  >>** in order for you to be able to use Amazon Payments Advanced. Several elements of this payment method are also included in the **CMS**. This makes it necessary to <<omni-channel/online-store/_cms/web-design/editing-the-web-design#, **publish the design**  >> after configurations or changes have been carried out.

[#40]
==  Registering with Amazon Payments Advanced

You need to set up a separate account for Amazon for the **Amazon Payments Advanced** payment method. You cannot use the Amazon Seller Central account that you created in the **Settings » Markets » Amazon » Basic settings** menu for this payment method. For further information about how to register with **Pay with Amazon**, refer to the **link:https://payments.amazon.co.uk/help/201212200[Amazon Payments Integration Guide, window="_blank"]**.

Creating a seller account usually takes about 1 to 2 weeks. This estimate includes the time needed for information to be verified. During this time, you will receive emails from Amazon Payments Advanced containing further instructions. Make sure that these emails are forwarded to the correct employee and that he or she follows the instructions in the emails to avoid any unnecessary delays.

[IMPORTANT]
.Entering the merchant URL and the integrator URL
====
You have to save the merchant URL and the integrator URL **your-plentystore.co.uk/plenty/api/payment_notification.php?payment=apa** in Amazon Payments Advanced. Replace **your-plentystore.co.uk** with the address of your online store. The address of your online store can be found under **Host** in the **Settings » Basic settings » API » Data** menu. The **https://** protocol must be entered. Refer to the **link:https://payments.amazon.co.uk/help/201212200[Amazon Payments registration manual, window="_blank"]** for further information.
====

[#50]
==  Setting up Amazon Payments Advanced in plentymarkets

Proceed as follows to set up the interface to Amazon Payments Advanced in plentymarkets.

[.instruction]
Setting up the interface to Amazon Payments Advanced:
...
.  Click on **Amazon Payments Advanced**. +
...
|**WalletWidget Width**
| Enter the amount of pixels in order to determine the width of the Amazon payment method widget. If Amazon Payments Advanced was selected, then this widget will be displayed instead of the payment methods during the checkout. The selected payment method will be displayed to the customer.
...
**1** - **10** = Only items available up to the selected availability can be paid with Amazon Payments Advanced. +
...
**Important:** Amazon Payments Advanced generally requires you to ship an item within a maximum of 30 days. Otherwise the payment will be cancelled.
|====

[IMPORTANT]
.One account per client (store)
====
You must have a separate Amazon Payments account for every client. Therefore, you will find a submenu for each client in the **Interface** tab. Enter the individual access data for each store.
====

[#70]
== Activating the payment method

Set up and activate the Amazon Payments Advanced **<<order-processing/payment/managing-payment-methods#20, payment method >>** in the **Settings** tab of the payment method. In addition, check the settings for **Permitted payment methods** in the **<<order-processing/payment/managing-payment-methods#30, customer classes >>** and for **Blocked payment methods** in the **<<order-processing/fulfilment/preparing-the-shipment#1000, shipping profiles >>**.

[#75"]
== Displaying Amazon Payments Advanced in the online store

An option to the default display of Amazon Payments Advanced in the online store, the template functions **Container_CheckoutAmazonPaymentsAdvancedAddressWidget()** and **Container_CheckoutAmazonPaymentsAdvancedPaymentWidget()** can be used to display the address and payment widgets for Amazon Payments Advanced individually in the online store. In addition, you will find multiple examples for integrating the **Pay with Amazon** button at the desired position in the design.
...
[#110"]
=== Displaying the Amazon Payments Advanced widgets

Proceed as described below to display the address and payment widgets for Amazon Payments Advanced individually in the online store.

[.instruction]
Displaying the Amazon Payments Advanced widgets in the order process:
...
[.subhead]
Code example for the Amazon Payments Advanced widgets:
...
. **Save** the settings.
--
.event procedure for automatically sending shipping confirmations to Amazon Payments Advanced
[cols="a,a,a"]
|====
| Setting | Option | Selection
...
|**Procedure**
|**Shipping &gt; Send shipping confirmation to Amazon Payments Advanced**
|
|====
...
Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically inform Amazon of cancellations.
...
. **Save** the settings.
--
.event procedure for automatically sending cancellations to Amazon Payments Advanced
[cols="a,a,a"]
|====
| Setting | Option | Selection
...
|**Procedure**
|**Payment methods &gt; Send cancellations to Amazon Payments Advanced**
|
|====
...
Create an **order status** for **returns** from Amazon Payments Advanced. This status will indicate that the return was checked and the correct amount was refunded. Corresponding returns then have to be given this order status. Give this order status an appropriate name, e.g. **Return checked**. Save this order status for the event procedures described below.
...
. **Save** the settings.
--
.event procedure for automatically sending refunds to Amazon Payments Advanced
[cols="a,a,a"]
|====
| Setting | Option | Selection
...
|**Event**
|**Order change: Status change**
| Select the status for the **Credit note to Amazon Payments Advanced**.
...
. **Save** the settings.
--
.event procedure for automatically reporting rejected payments by Amazon Payments Advanced
[cols="a,a,a"]
|====
| Setting | Option | Selection
