...
If you already have an **link:https://de.allyouneed.com/de/customerlogin.html[Allyouneed.com, window="_blank"]** seller contract, you do not need to register again. You can directly begin to **<<order-processing/payment/postpay#30, set up POSTPAY >>**.
...
Set up and activate the POSTPAY **<<order-processing/payment/managing-payment-methods#20, payment method >>** in the **Settings** tab of the payment method. In addition, check the settings for **Permitted payment methods** in the **<<order-processing/payment/managing-payment-methods#30, customer classes >>** and for **Blocked payment methods** in the **<<order-processing/fulfilment/preparing-the-shipment#1000, shipping profiles >>**.
...
Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send shipping confirmations when the outgoing items are booked.
...
Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send returns to POSTPAY. For further information about creating returns, refer to the **<<order-processing/orders/editing-sales-orders/return#, Return  >>** page of the manual.
...
To send credit notes automatically, an **<<order-processing/orders/settings/order-status#, order status  >>** for credit notes is required. You can use one of the standard statuses or set up your own order status. This status is then only assigned to completed credit notes. These credit notes must not be changed after creation. Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send credit notes to POSTPAY.
...
Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically inform POSTPAY of cancellations.
