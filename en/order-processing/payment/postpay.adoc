...
Set up an <<basics/automation/event-procedures#, **event procedure**  >> to automatically send shipping confirmations when the outgoing items are booked.
...
Set up an <<basics/automation/event-procedures#, **event procedure**  >> to automatically send returns to POSTPAY. For further information about creating returns, refer to the **<<order-processing/orders/managing-sales-orders#return, Return  >>** page of the manual.
...
To send credit notes automatically, an **<<order-processing/orders/managing-sales-orders#order-status, order status  >>** for credit notes is required. You can use one of the standard statuses or set up your own order status. This status is then only assigned to completed credit notes. These credit notes must not be changed after creation. Set up an <<basics/automation/event-procedures#, **event procedure**  >> to automatically send credit notes to POSTPAY.
...
Set up an <<basics/automation/event-procedures#, **event procedure**  >> to automatically inform POSTPAY of cancellations.
