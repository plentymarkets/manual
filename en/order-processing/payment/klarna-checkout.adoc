...
====
If you activated Klarna Checkout, then do not insert the container for Klarna Checkout **{% Container_CheckoutKlarnaCheckout() %}** into one of the checkout templates. Doing so may cause Klarna Checkout to interfere with the individual order process in plentymarkets. If you use Klarna Checkout, then make sure that end customers are able to access Klarna Checkout starting from the shopping cart preview. The shopping cart preview is in the template <<omni-channel/online-store/cms-syntax/web-design/itemview/container-itemviewbasketpreviewlist#, **ItemViewBasketPreviewList**  >> in the plentymarkets default designs.
====
