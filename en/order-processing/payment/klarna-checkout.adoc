...
An iframe is used to integrate Klarna Checkout directly in plentymarkets. Thus, an <<basics/start/plentymarkets-account/service-center/ssl-certificate#, **SSL certificate**  >> is required for using this payment method.
...
**__Important__**: In plentymarkets, Klarna Checkout is available for the <<order-processing/fulfilment/preparing-the-shipment#100, **countries of delivery** >> Germany, Austria, Finland, Norway and Sweden. This means that only customers based in these countries can pay orders using Klarna Checkout.
...
Set up and activate the desired **<<order-processing/payment/managing-payment-methods#20, payment methods >>** in the **Settings » Orders » Payment » Payment methods** menu in your plentymarkets system.
...
====
If you activated Klarna Checkout, then do not insert the container for Klarna Checkout **{% Container_CheckoutKlarnaCheckout() %}** into one of the checkout templates. Doing so may cause Klarna Checkout to interfere with the individual order process in plentymarkets. If you use Klarna Checkout, then make sure that end customers are able to access Klarna Checkout starting from the shopping cart preview. The shopping cart preview is in the template <<omni-channel/online-store/_cms-syntax/web-design/itemview/container-itemviewbasketpreviewlist#, **ItemViewBasketPreviewList**  >> in the plentymarkets default designs.
====
...
Set up event procedures to inform Klarna when items are shipped, orders and returns are cancelled and when invoices are created or deleted. Ask your contact person in the Klarna integration team to test the functionality before you start using the interface live. Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send shipping confirmations to Klarna.
...
Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send cancellations to Klarna.
