...
. Go to **Settings » Orders » Payment » Incoming payments**.
...
**__Important__**: In some cases, you may want to book a payment in a different currency than is currently selected for the payment (default currency), e.g. in US dollars. In this case, select the option **USD** under **Currency** and enter the amount received under **Amount**. +
If you calculated the exchange rate in the menu **<<order-processing/payment/currencies#, Settings » Orders » Payment » Currencies  >>** and you want to use this exchange rate, then you do not have to enter anything under **Exchange rate**, because the exchange rate is automatically used. You will need to make an entry under **Exchange rate** if you want to use a different exchange rate for this payment, e.g. the exchange rate at the exact moment when the payment is received. In this case, enter the exchange rate that is applicable at this moment.
...
The value date is displayed under **Imported**. The account is charged on this date. The interim transaction report is confirmed (image 2, green arrow) when payment is received (image 2, blue arrow).
...
. Place a check mark next to incoming payments that you want to assign.
...
.  Go to **Orders » Payments**.
. Select the **filters**. Pay attention to the information given in table 4.
...
**__Example__**: Select **&gt;=** and enter 300 as the **value** in order to display all payments that have a payment amount of more than 300 EUR.
...
Manually booked incoming payments, imported bank postings as well as EBICS payments can be split in the **Orders » Payments** menu. In this way, one payment can be assigned to multiple orders. Not that interim transaction reports cannot be split. To split payments, proceed as follows.
...
Before setting up an event procedure, adjust the **<<crm/sending-emails#1200, email template >>****Partial payment received** with your company data. The following table explains relevant template variables for the **Partial payment received** email template.
...
At least once a week, you should edit and sort out the unassigned payments so that you don't lose track of them in your day-to-day business. For further information about assigning unassigned payments and bookings to orders, refer to **<<order-processing/payment/managing-payments#70, Assigning payments >>**.
