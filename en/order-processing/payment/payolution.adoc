...
[#30]
=== SSL certificateYou need to have an <<basics/working-with-plentymarkets/plentymarkets-account#ssl-certificate, **SSL certificate**>> in order to correctly use the payment method.
...
Set up an <<basics/automation/event-procedures#, **event procedure**>> to automatically send shipping confirmations to Payolution when the outgoing items are booked. You do not need a shipping confirmation for instalment purchases. As such, this event procedure should only be set up for payment on invoice.
...
====
Once you have set up this event procedure, the shipping confirmation will automatically be sent to Payolution when the outgoing items are booked. Go to **<<order-processing/orders/managing-sales-orders#changing-order, Orders » Edit orders » Open order » Tab: Overview » Tab: Status history>>** to check this process.
====
