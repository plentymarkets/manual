...
*  If the customer **<<order-processing/payment/klarna#100, cancels his or her order >>**, then give it the status **cancelled** as soon as possible so that the shopping cart value will no longer be reserved for the customer.
...
Before you save the Klarna access data for shipping to different countries, make sure that you set up the **<<order-processing/payment/currencies#, currencies  >>** and **<<omni-channel/online-store/best-practice/multilingual-online-store#, languages  >>** in the online store.
...
Go to **<<omni-channel/online-store/standard/online-store/basic-settings#, Settings » Client (store) » Standard » Online store » Basic settings  >>** and make sure that you have selected the option **Enable login for every store** in the line **Login**. This makes it possible to support several clients.
...
**__Important:__** In plentymarkets, Klarna is available for the <<order-processing/fulfilment/preparing-the-shipment#100, **countries of delivery** >> Germany, Austria, the Netherlands, Denmark, Finland, Norway and Sweden. This means that only customers based in these countries can pay orders using Klarna.
...
.updating campaigns
image::order-processing/payment/assets/EN-Payment-Methods-Klarna-03.png[]
[#70]
== Activating payment methods

For Klarna, the payment methods Klarna invoice and Klarna account are available. Go to **<<order-processing/payment/managing-payment-methods#20, Settings » Orders » Payment » Payment methods >>** to activate the payment methods. In addition, check the settings for **Permitted payment methods** in the **<<order-processing/payment/managing-payment-methods#30, customer classes >>** and for **Blocked payment methods** in the **<<order-processing/fulfilment/preparing-the-shipment#1000, shipping profiles >>**.
...
Set up the following event procedures in order to inform Klarna when items are shipped, orders and returns are cancelled and when invoices are created or deleted. Note that the event procedures also have to be created for Klarna account. Ask your contact person in the Klarna integration team to test the functionality before you start using the interface live. Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send shipping confirmations when the outgoing items are booked.
...
Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send returns to Klarna. For further information about creating returns, refer to the **<<order-processing/orders/editing-sales-orders/return#, Return  >>** page of the manual.
...
Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically inform Klarna of cancellations.
...
====
Currently, it is not possible to transfer orders that are paid with Klarna to <<order-processing/fulfilment/preparing-the-shipment#4800, **DHL Fulfillment** >>. Only orders with a valid invoice can be transferred to DHL Fulfillment. However, Klarna only transmits the invoice to plentymarkets after outgoing items have been booked.
====
...
image::order-processing/payment/assets/EN-Payment-Methods-Klarna-04.png[]
.the Klarna invoice is available in the **receipts** tab once the shipping confirmation was sent to Klarna
image::order-processing/payment/assets/EN-Payment-Methods-Klarna-05.png[]
[IMPORTANT]
...
.pending order
image::order-processing/payment/assets/EN-Payment-Methods-Klarna-06.png[]
