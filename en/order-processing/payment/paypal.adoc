...
.SSL certificate
====
Use the URL with http instead of https if you do not use SSL encryption in your online store. We recommend that you use an <<basics/start/plentymarkets-account/service-center/ssl-certificate#, **SSL certificate**  >>.
====
...
. Select the **UTF-8** option from the **Encoding** drop-down menu (image 2).
...
. Select the option **Settlement report** from the drop-down menu **Document**. +
...
Then create an **<<order-processing/orders/settings/event-procedures#, event procedure  >>** to automatically send the refunds to PayPal.
