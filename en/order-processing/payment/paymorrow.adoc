...
If an order was cancelled, then the status has to match the settings under **Settings » Orders » Basic settings** so that the cancelled operation will also be communicated to Paymorrow. Set up an <<basics/automation/event-procedures#, **event procedure**>> to automatically send cancellations to Paymorrow.
