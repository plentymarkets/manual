...
You need to have an <<basics/start/plentymarkets-account/service-center/ssl-certificate#, **SSL certificate**  >> in order to correctly use the payment method.
...
In order to set up Paymorrow in plentymarkets, you will need an acceptance contract with Paymorrow and the necessary access data. In the Paymorrow **link:http://www.paymorrow.de/onlinehaendler/integration/index.php[seller area, window="_blank"]**, you will find the necessary instructions related to the contract.
...
If you use Paymorrow and you generate invoices for the orders that are paid for with Paymorrow invoices, then make sure to display Paymorrow's bank details on the invoice rather than your own bank details. This is done automatically with a payment instruction. Proceed as described below to use your own **<<order-processing/orders/settings/documents#170, PDF template >>**.

[IMPORTANT]
.Respect the payment instruction width
====
In order to display the bank details of Paymorrow correctly on your invoice, save a width of 120 millimetres for the payment instruction in the **Settings » Client (store) » Open store » Locations » Open location » Documents » Invoice** menu.
====
...
Set up event procedures to inform Paymorrow of outgoing items, credit notes and cancellations. For this to work correctly, the documents need to be created in the order's **Receipts** tab. Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send invoices to Paymorrow and to report outgoing items to Paymorrow.
...
If a customer sends back some or all of the items, then you must create a credit note and report this to Paymorrow in order to keep the customer from receiving a dunning letter from Paymorrow. Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send credit notes to Paymorrow.
...
If an order was cancelled, then the status has to match the settings under **Settings » Orders » Basic settings** so that the cancelled operation will also be communicated to Paymorrow. Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send cancellations to Paymorrow.
