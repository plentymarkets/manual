...
An iframe is used to integrate Klarna Checkout UK directly in plentymarkets. Thus, an <<basics/start/plentymarkets-account/service-center/ssl-certificate#, **SSL certificate**  >> is required for using this payment method.
...
Set up the following event procedures in order to inform Klarna UK when items are shipped, orders are cancelled and credit notes or refunds are paid. Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send shipping confirmations to Klarna. Payment providers usually wait for the shipping confirmation from the merchant before they disburse and transfer the amount paid by the customer to the merchant’s account.
...
[#110]
It is possible to refund customers the total purchase amount. Simply create an order of the type **Return**. Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send refunds to Klarna UK. For further information about creating returns, refer to the **<<order-processing/orders/editing-sales-orders/return#, Return  >>** page of the manual.
...
Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send cancellations to Klarna UK.
