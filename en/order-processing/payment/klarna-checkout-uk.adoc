...
[#110]
It is possible to refund customers the total purchase amount. Simply create an order of the type **Return**. Set up an <<basics/automation/event-procedures#, **event procedure**  >> to automatically send refunds to Klarna UK. For further information about creating returns, refer to the **<<order-processing/orders/managing-sales-orders#return, Return  >>** page of the manual.
