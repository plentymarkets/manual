...
. Go to **System » Client » Select client » Services » atriga**.
...
If atrigapaymax is activated, then registered customers will not be able to change an order's payment method in their account. The payment method cannot even be changed if the option **Permit changes to orders retroactively** is set to **Yes** in the **System » Client » Select client » Order process » Settings** menu.
