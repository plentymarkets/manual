...
**link:http://www.sagepay.co.uk/[Sage Pay, window="_blank"]** is a payment provider that was founded in Great Britain and is available in the United Kingdom and in Ireland. Sage Pay offers various payment options, an integrated fraud prevention tool and e.g. transaction reports.
...
After confirming the order, the customer will be redirected to the Sage Pay website. Here, he or she needs to enter the data of the selected credit card. Once the customer has entered the data and confirmed the payment, he or she is directed back to your online store.
...
If you use the first payment method, then the amount will be transferred to you directly. If you use the payment methods **deferred** and **authenticate**, then the amount will be transferred to you after you have requested payment. For further information about these payment methods, refer to **link:http://www.sagepay.co.uk/support/15/36/transaction-types[Sage Pay, window="_blank"]**.

[#40]
== Registering with Sage Pay

You need to register with Sage Pay before setting it up in plentymarkets. **link:http://www.sagepay.co.uk/contact[Contact, window="_blank"]** Sage Pay in order to register. Once you have registered you will receive access data from Sage Pay. You need this access data for the setup in plentymarkets. After registering, access your Sage Pay account and enter the IP address of the server that your store runs on. Save this entry in the Sage Pay back end under **Settings » Valid IPs**.

[#50]
== Setting up Sage Pay in plentymarkets

Proceed as described below to set up the interface to Sage Pay.
...
**__Important__**: Please note that the activated iframe option is not compatible with local European payment methods.
...
Set up and activate the **<<order-processing/payment/managing-payment-methods#20, payment methods >>** Sage Pay, Sage Pay deferred and Sage Pay authenticate in the **Settings** tab of the payment methods. In addition, check the settings for **Permitted payment methods** in the **<<order-processing/payment/managing-payment-methods#30, customer classes >>** and for **Blocked payment methods** in the **<<order-processing/fulfilment/preparing-the-shipment#1000, shipping profiles >>**.
...
With the help of an event procedure you send the **<<order-processing/payment/sage-pay#110, cancellation >>** to Sage Pay.
...
Set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically inform Sage Pay of cancellations.
...
If you already received a payment and you want to refund a customer, set up an <<order-processing/orders/settings/event-procedures#, **event procedure**  >> to automatically send credit notes to Sage Pay.
